...
- If `research/README.md` is absent, or it contains no such section → forward no
  lessons and note _"No prior lessons forwarded"_ in the brief. Do not invent any.
...
> to scope which features they bind. If adopted, it MUST ship with a _deterministic_
...
> _"Prior lessons that apply (from `research/README.md`) — the plan MUST account for
...
> plan's design or its risks section."_
...
introduce _new_ endpoints or events (e.g., an internal sync call, a derived event) not
...
- **Prior lessons applied (v1.6, W5-D1)** — list each forwarded lesson (title) and the
  one-line note on how the plan honors it, or _"none"_ if Step 2 forwarded no lessons.
