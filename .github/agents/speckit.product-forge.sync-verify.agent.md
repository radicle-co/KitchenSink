...
Evidence so the finding reads, e.g., _"orphan `src/lib/rate-limit.ts` — no
matrix row, no task — introduced by `a1b2c3d` (J. Doe)"_. The suggested
...
This makes the constitution a _continuous_ contract (the
