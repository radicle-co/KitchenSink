...
#
# Usage:  cfnExport.sh [--optional] <export-name> [region]
#
...
# Allow both `source cfnExport.sh` (for the test harness and multi-lookup callers) and direct
# execution. `${BASH_SOURCE[0]}` differs from `$0` only when the file was sourced.
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    if [ "$#" -lt 1 ]; then
        echo "usage: cfnExport.sh [--optional] <export-name> [region]" >&2
