...
#
# Usage — as a CLI:
#     deployGate.sh decide     <intent> <changed> <forced> <healthCode> <name=STATUS> [<name=STATUS> …]
#     deployGate.sh evaluate   <intent> <service> <changed> <forced> <healthUrl> <region> <stack> [<stack> …]
#     deployGate.sh stacks-for <entrypoint> <stage> [<manifestFile>]
#
...
#
# So absence alone no longer deploys. It deploys when the sandbox is SUPPOSED to be up — `intent` — and then
# only under the original ensure-exists rules. ⚠️ `intent` was once carried by a `sandbox-up` PR label; that
# label is GONE (a fact kept in two places is a fact that can disagree, and this one was maintained by hand).
# The callers pass the term directly today. A manual dispatch still deploys unconditionally, because pressing the
...
deploy_gate_decide() {
    ...
    shift 4 2>/dev/null || {
        echo "usage: deployGate.sh decide <intent> <changed> <forced> <healthCode> <name=STATUS>…" >&2
        return 2
    }
...
deploy_gate_evaluate() {
    ...
    if [ "$#" -lt 7 ] || [ -z "$service" ] || [ -z "$health_url" ] || [ -z "$region" ]; then
        echo "usage: deployGate.sh evaluate <intent> <service> <changed> <forced> <healthUrl> <region> <stack>…" >&2
        return 2
    fi
    shift 6
...
# `docs/generated/infrastructure/crossAppImports.tsv`, under the same regenerate-and-diff staleness gate the
...
DEPLOY_GATE_EDGE_FILE='docs/generated/infrastructure/crossAppImports.tsv'
...
deploy_gate_close() {
    local unmet="${1-}"
    shift 1 2>/dev/null || {
        echo "usage: deployGate.sh close <unmetEdges> <flag=value@entrypoint>…" >&2
        return 2
    }
...
#
# ⛔ The lookup goes through `.github/scripts/cfnExport.sh`, never an open-coded
...
deploy_gate_unmet_imports() {
    ...
    if [ -z "$region" ] || [ -z "$stage" ] || [ -z "$base" ]; then
        echo 'usage: deployGate.sh unmet-imports <region> <stage> <baseStage> [edgeFile]' >&2
        return 2
    fi
    if [ ! -f "$edges" ]; then
    ...
    # ⛔ PRECONDITION, not a courtesy. `cfnExport.sh --optional` answers EMPTY for both "the export is
    ...
    while IFS=$'\t' read -r consumer producer template; do
        ...
        if [ -z "$found" ]; then
            value=$(bash "${script_dir}/cfnExport.sh" --optional "$resolved" "$region") || return 2
            if [ -z "$value" ]; then
                found='absent'
...
# under the same regenerate-and-diff staleness gate `crossAppImports.tsv` carries — so the file is current
# with the commit that reads it, and a stack added tomorrow is probed the day it is written.
#
# ⚠️ WHY THE MANIFEST AND NOT `cdk ls`. `verifyDeployment.sh` derives ITS stack list by synthesising the very
...
deploy_gate_stacks_for() {
    ...
    if [ -z "$entrypoint" ] || [ -z "$stage" ]; then
        echo 'usage: deployGate.sh stacks-for <entrypoint> <stage> [manifestFile]' >&2
        return 2
    fi
    if [ ! -f "$manifest" ]; then
