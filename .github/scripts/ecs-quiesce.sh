...
# ## What this does
#
# For every ECS cluster tagged as this PR's: delete each service with `--force` (which scales it to
...
# ## Scope
#
# Clusters are discovered ONLY by an `Environment` tag whose value is one this PR can own — the same
# authority that already licenses `teardown-sandbox-pr.sh` to delete whole stacks, so this widens nothing.
# The `pr-{N}` token is validated through the shared `pr-scope.sh` guard first, and the exact values are
# PRINTED by that same file (`pr_scope_environment_tag_values`) rather than spelled here.
#
# ⛔ THAT INDIRECTION IS THE POINT, not ceremony. This filter used to be the literal `Values=$pr`, which the
# tagging API matches EXACTLY — so the day the CDK apps started emitting `pr-{N}-sandbox` it would have found
# no clusters at all, drained nothing, and exited 0. Every per-PR stack would then have gone back to dying on
# `ClusterCapacityProviderAssociations DELETE_FAILED`, which is the defect this script exists to remove, and
# the only symptom would have been stacks failing to delete for a reason no one had changed.
#
# The shared persistent tier is tagged with its TIER (`sandbox` / `production`), which no token can claim —
# a token always carries digits and neither tier word does — so it can never be selected. Note that the
# per-PR cluster NAME (`kitchensink-{service}-pr-{N}-…Cluster…`) is deliberately NOT used for matching:
# `pr_scope_belongs` is a prefix rule and would not match it, and loosening that rule is exactly what
# ADR-0005 forbids.
#
# Usage:
...
#
# Print the ARN of every ECS cluster whose `Environment` tag belongs to <pr>, one per line. Empty output is
# a legitimate and common answer (a PR that deployed no service at all) — which is exactly why the filter is
# not allowed to be wrong: "no clusters" and "the filter matches nothing" look identical from here.
#
# @sideEffect calls the AWS resource-groups tagging API
ecs_quiesce_clusters() {
    local pr="$1" region="$2" values

    # ⛔ FAIL CLOSED on a token the scope rule refuses. Inlining the substitution would leave a bare
    # `Values=` on a malformed token — a filter the API reads as "no constraint on the value", i.e. every
    # tagged cluster in the account, prod's included.
    #
    # The status DOES reach the caller: `ecs_quiesce_pr` captures this function's output before iterating,
    # precisely so a refusal cannot arrive looking like "nothing to drain".
    values=$(pr_scope_environment_tag_values "$pr") || return 2

    # ⛔ A FAILED call is not an empty answer. The comment above says "no clusters" and "the filter matches
    # nothing" look identical from here — and a third case looked identical too: the call not completing.
    # stderr went to /dev/null and the pipeline ended in `|| true`, so a denial, a throttle or a bad region
    # printed "nothing to drain" and exited 0. That is silent in the direction that LEAKS COST, because the
    # PR's tasks keep running and the teardown reports success.
    local arns
    arns=$(aws resourcegroupstaggingapi get-resources --region "$region" \
        --tag-filters "Key=Environment,Values=$values" \
        --resource-type-filters ecs:cluster \
        --query 'ResourceTagMappingList[].ResourceARN' --output text) || return 1

    # `grep` exits 1 on no match, which here IS the legitimate "this PR deployed no service" answer.
    printf '%s' "$arns" | tr '\t' '\n' | grep ':cluster/' || true
}
...
ecs_quiesce_pr() {
    local pr="$1" region="$2"
    local failed=0 found=0 cluster clusters

    # ⛔ CAPTURE before iterating. `for cluster in $(f)` discards f's exit status, which made "we could not
    # ask the tagging API" indistinguishable from "this PR owns no cluster" — and the second is a common,
    # legitimate answer, so the first was invisible.
    clusters=$(ecs_quiesce_clusters "$pr" "$region") || {
        echo "[ecs-quiesce] could NOT determine which clusters $pr owns, so nothing was drained; its tasks may still be running." >&2
    ...
    if [ "$found" -eq 0 ]; then
        echo "[ecs-quiesce] no ECS clusters tagged for $pr — nothing to drain."
    fi
