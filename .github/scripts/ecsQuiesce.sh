...
# authority that already licenses `teardownSandboxPr.sh` to delete whole stacks, so this widens nothing.
# The `pr-{N}` token is validated through the shared `prScope.sh` guard first, and the exact values are
# PRINTED by that same file (`pr_scope_environment_tag_values`) rather than spelled here.
#
# ⛔ THAT INDIRECTION IS THE POINT, not ceremony. The tagging API matches `Values=` EXACTLY, so a filter spelled
# here would silently miss the moment the apps' tag values change: it would find no clusters, drain nothing, and
# exit 0 — and every per-PR stack would die on `ClusterCapacityProviderAssociations DELETE_FAILED`, the defect
# this script exists to remove, with stacks failing to delete as the only symptom.
#
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    PR="${1:?usage: ecsQuiesce.sh <pr-N> [region]}"
