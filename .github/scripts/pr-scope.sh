...
# `ecs-quiesce.sh`, everything a stack does not own) cannot come to disagree about which values are this
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1-}" in
        ...
        *)
            echo "usage: pr-scope.sh is-token|belongs|path-belongs|env-belongs|env-tag-belongs|env-tag-values|protected-env <pr-N> [name]" >&2
            exit 2
            ;;
    esac
fi
