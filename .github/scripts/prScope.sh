...
# RDS cluster behind them — is named `kitchensink-*` and tagged with its persistent TIER. Nothing but the
...
#
# Usage — source it:
#     . "$(dirname "${BASH_SOURCE[0]}")/prScope.sh"
...
#     prScope.sh is-token       <token>
#     prScope.sh belongs        <token> <name>
#     prScope.sh path-belongs   <token> <path>
#     prScope.sh env-belongs    <token> <environment>
#     prScope.sh env-tag-belongs <token> <tag-value>
#     prScope.sh protected-env  <environment>
#
# …with ONE exception, which prints instead of answering (exit 0 = printed, 2 = misuse):
#     prScope.sh env-tag-values <token>    →  `pr-{N},pr-{N}-sandbox`
...
pr_scope_path_belongs() {
    ...
    # where `pr-57` is preceded by `-`, not `/`. The sweep in `teardownSandboxPr.sh` §4 therefore ran on
...
# The suffix a per-PR resource's `Environment` TAG carries, after the stage it belongs to. The CDK apps
# emit `pr-{N}-sandbox` for an ephemeral preview and the bare stage word — `sandbox` or `production` — for
# the persistent tier, so the tag now says WHICH tier a resource is in as well as which PR owns it.
PR_SCOPE_ENVIRONMENT_TAG_SUFFIX='-sandbox'

# pr_scope_environment_tag_belongs <token> <tag-value>
#
# True iff <tag-value> is an `Environment` tag value that belongs to PR <token>.
#
# ⛔ A TAG IS NOT A NAME. `pr_scope_belongs` is a delimiter-aware PREFIX rule, correct for a resource NAME we
# choose; a tag value is drawn from a closed set the CDK apps emit, so the right rule is EQUALITY against
# that set. Reusing `belongs` here would newly admit `pr-91-anything`, widening the one match ADR-0005 says
# must never widen — and it would still be wrong, because the tagging API's `Values=` filter (the other half
# of the same question, in `pr_scope_environment_tag_values`) can only ever express exact values.
#
# TWO values, and the legacy one is deliberate rather than leftover: resources tagged before the scheme
# changed carry the bare token, and a predicate that accepted only the new spelling would make every live
# preview unreapable the moment it shipped. Both are unambiguous — no persistent value can be produced by
# any token, because a token always contains digits and `sandbox`/`production` contain none.
pr_scope_environment_tag_belongs() {
    pr_scope_is_token "${1-}" || return 2
    case "${2-}" in "$1" | "$1$PR_SCOPE_ENVIRONMENT_TAG_SUFFIX") return 0 ;; *) return 1 ;; esac
}

# pr_scope_environment_tag_values <token>
#
# Print the `Values=` list for `aws resourcegroupstaggingapi get-resources --tag-filters` — the same set
# `pr_scope_environment_tag_belongs` accepts, enumerated, because that API matches exact values only.
#
# It exists so the equality read (teardown §2, one stack at a time) and the tag sweeps (teardown §3 and
# `ecsQuiesce.sh`, everything a stack does not own) cannot come to disagree about which values are this
# PR's. A sweep that finds nothing reports success, so a divergence here is silent in the direction that
# costs money.
pr_scope_environment_tag_values() {
    pr_scope_is_token "${1-}" || return 2
    printf '%s,%s%s\n' "$1" "$1" "$PR_SCOPE_ENVIRONMENT_TAG_SUFFIX"
}
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1-}" in
        ...
        env-tag-belongs) pr_scope_environment_tag_belongs "${2-}" "${3-}" ;;
        env-tag-values) pr_scope_environment_tag_values "${2-}" ;;
        protected-env) pr_scope_is_protected_environment "${2-}" ;;
        *)
            echo "usage: prScope.sh is-token|belongs|path-belongs|env-belongs|env-tag-belongs|env-tag-values|protected-env <pr-N> [name]" >&2
            exit 2
            ;;
    esac
fi
