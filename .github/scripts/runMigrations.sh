...
# ## Usage
#
#     runMigrations.sh run      <region> <stackName> <outputKey> <label> <migrationsDir>
#     runMigrations.sh classify <functionError> <payload>
#     runMigrations.sh manifest <migrationsDir>
#
...
run_migrations_classify() {
    ...
    if [ "$#" -lt 2 ]; then
        echo "usage: runMigrations.sh classify <functionError> <payload>" >&2
...
run_migrations_manifest() {
    ...
    if [ "$#" -lt 1 ] || [ -z "$dir" ]; then
        echo "usage: runMigrations.sh manifest <migrationsDir>" >&2
...
run_migrations_run() {
    ...
    if [ "$#" -lt 5 ] || [ -z "$region" ] || [ -z "$stack" ] || [ -z "$output_key" ] || [ -z "$label" ] ||
        ...
        echo "usage: runMigrations.sh run <region> <stackName> <outputKey> <label> <migrationsDir>" >&2
    ...
    # ⛔ "The stack does not exist" and "we could not find out whether it exists" are DIFFERENT answers, and
    # only the first licenses a skip. This discarded stderr and read EVERY non-zero status as absence, so a
    # denied, throttled, or wrong-region call announced "there is no schema of ours to migrate" and returned
    # 0 — and because ADR-0035 runs this AHEAD of the deploy, the service then shipped onto an unmigrated
    # schema. That is the same `applied: []`-shaped silence ADR-0035 says hoisting would cause, arriving
    # through the lookup rather than through the runner. The invoke below already got this right.
    # Anything we cannot positively recognise as absence fails CLOSED: a red deploy is recoverable, a silent
    # skip is what ships the outage.
    local outputs describe_stderr describe_message
    describe_stderr=$(mktemp)

    if ! outputs=$(aws cloudformation describe-stacks --region "$region" --stack-name "$stack" \
        --query "Stacks[0].Outputs" --output json 2>"$describe_stderr"); then
        describe_message=$(cat "$describe_stderr" 2>/dev/null)
        rm -f "$describe_stderr"

        if ! printf '%s' "$describe_message" | grep -q 'does not exist'; then
            echo "::error::[${label}] could NOT determine whether ${stack} exists, so it is unknown whether this schema is migrated: ${describe_message}"
    ...
    # ⛔ `expectManifestSha` and NOTHING else: every runner parses its event `.strict()` (ADR-0039 deleted the
    # `drop` doors, so there is no action to select), and an extra key is refused — the whole deploy with it.
    local payload_file function_error payload verdict reason
    payload_file=$(mktemp)
    function_error=$(aws lambda invoke --region "$region" --function-name "$fn" \
        --payload "{\"expectManifestSha\":\"${expect_manifest}\"}" \
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1-}" in
        ...
        manifest)
            ...
            ;;
        *)
            echo "usage: runMigrations.sh run|classify|manifest …" >&2
            exit 2
            ;;
    esac
fi
