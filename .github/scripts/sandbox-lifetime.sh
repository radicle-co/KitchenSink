...
# This file owns that arithmetic and nothing else repeats it, which is the same reason `pr-scope.sh` owns
...
# deploy passing no value ERASED it. `sandbox-reap.yml` now feeds `expires-at` a stack's own
