    ...
    while true; do
        ...
        case "$class" in
            ready)
                ...
                ;;
            fatal)
                echo "::error::sandbox database ${identifier} is '${status}' — terminal or unrecognised, so a deploy would run its schema migration against an unusable instance. Fix the instance (or teach sandbox-wake.sh this status) before redeploying."
