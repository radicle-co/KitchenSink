...
#
# ⚠️  DELIBERATE — a preview is torn down at midnight America/New_York of the day it was LAST DEPLOYED.
# This file owns that arithmetic and nothing else repeats it, which is the same reason `prScope.sh` owns
# exactly one copy of the pr-{N} matcher.
#
# ⛔ THE INSTANT IT IS ASKED ABOUT COMES FROM CLOUDFORMATION, NOT FROM A STAMP. An earlier design computed
# the expiry once at the press of a button and wrote it onto every `pr-{N}` stack as a `SandboxExpiresAt`
# tag. That tag is gone: `UpdateStack` REPLACES a stack's tag set, so every deploy was a writer of it and a
# deploy passing no value ERASED it. `sandboxReap.yml` now feeds `expires-at` a stack's own
# `LastUpdatedTime` — a fact CloudFormation maintains and no deploy can lose. See ADR-0028 §2 and
# `sandboxLifecycleSingleSource.test.ts`, which refuses the tag's return.
#
...
#
#   sandboxLifetime.sh expires-at <startedEpoch>
#   sandboxLifetime.sh is-expired <expiresAtEpoch> <nowEpoch>
#
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    case "${1-}" in
        ...
        is-expired)
            ...
            ;;
        *)
            echo "usage: sandboxLifetime.sh {expires-at <startedEpoch>|is-expired <expiresAt> <now>}" >&2
            exit 2
            ;;
    esac
fi
