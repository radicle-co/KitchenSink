...
# instance:
#
# ⚠️ ADR-0035 has since SUPERSEDED that mechanism — the Trigger is gone and each database's schema stack
# migrates in its own pipeline step. The account below is kept as the incident record, and this script is
# unaffected: what it waits for is the instance, and a migrate step against a stopped one fails just as a
# Trigger did. References to the Trigger below are history, not current machinery.
#
...
# actually authorises the call — the same belt-and-braces shape as `prScope.sh`.
#
# ## What it does NOT do
#
...
#
# Usage — as a CLI:
#     sandboxWake.sh classify            <status>      # pure: ready | wake | wait | fatal   (RDS)
#     sandboxWake.sh classify-ec2        <state>       # pure: ready | wake | wait | fatal   (EC2)
#     sandboxWake.sh is-sandbox-instance <identifier>  # pure: exit 0 = yes, 1 = no, 2 = misuse
#     sandboxWake.sh is-sandbox-nat      <nameTag>     # pure: exit 0 = yes, 1 = no, 2 = misuse
#     sandboxWake.sh next-stop           <epoch>       # pure: the next 00:00 ET stop, as an epoch second
#     sandboxWake.sh headroom            <epoch> <n>   # pure: clear | crossing
#     sandboxWake.sh ensure              <region>      # impure: discover, wake, wait — bounded, BOTH
#
    ...
    while true; do
        ...
        case "$class" in
            ready)
                ...
                ;;
            fatal)
                echo "::error::sandbox database ${identifier} is '${status}' — terminal or unrecognised, so a deploy would run its schema migration against an unusable instance. Fix the instance (or teach sandboxWake.sh this status) before redeploying."
        ...
        if [ "$now" -ge "$deadline" ]; then
            echo "::error::sandbox database ${identifier} did not reach 'available' within ${timeout}s (last status: ${status}). Refusing to deploy: the schema stack's migrate step (ADR-0035) would time out against it, and the service deploy that follows would serve an unmigrated schema.${last_error:+ Last StartDBInstance error: ${last_error}}"
...
nat_wake_instance() {
    ...
    while true; do
        ...
        now=$(date +%s)
        if [ "$now" -ge "$deadline" ]; then
            echo "::error::sandbox NAT ${identifier} did not reach 'running' within ${timeout}s (last state: ${state}). Refusing to deploy: its VPC Lambdas would fail with ETIMEDOUT on their first AWS or Clerk call.${last_error:+ Last StartInstances error: ${last_error}}"
...
sandbox_wake_ensure() {
    ...
    if [ "$#" -ne 1 ] || [ -z "$region" ]; then
        echo 'usage: sandboxWake.sh ensure <region>   (there is deliberately NO instance argument — see the header)' >&2
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1-}" in
        ...
        is-sandbox-nat)
            ...
            ;;
        *)
            echo 'usage: sandboxWake.sh classify <status> | classify-ec2 <state> | is-sandbox-instance <identifier> | is-sandbox-nat <nameTag> | next-stop <epoch> | headroom <epoch> <requiredSeconds> | ensure <region>' >&2
            exit 2
            ;;
    esac
fi
