...
# shellcheck source=./cfn-resting-states.sh
. "$SCRIPT_DIR/cfn-resting-states.sh"
...
# against that set, never by the name prefix rule. Both sites below read it from `pr-scope.sh` so the
...
##    `pr-scope.sh` rather than spelled here: a literal `Values=$PR` is what silently matched nothing the
