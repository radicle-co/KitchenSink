...
# ## Why this exists
#
# `cdkAppDeployCoverage.test.ts` asks whether every CDK app has a deployer. `deployGate.sh` asks whether a
...
# ## Usage
#
#     verifyDeployment.sh verify         <region> <cdkAppCommand>
#     verifyDeployment.sh verify-stacks  <region> <stackName> [<stackName> …]
#     verifyDeployment.sh stacks         <cdkAppCommand>
#     verifyDeployment.sh drift          <region> <stage> <cdkAppCommand> [--warn-only]
#     verifyDeployment.sh drains         <region> <stackName> [--warn-only]
#     verifyDeployment.sh drains-app     <region> <cdkAppCommand> [--warn-only]
#     verifyDeployment.sh classify-resource  <ResourceStatus>
#     verifyDeployment.sh classify-reference <envKey> <envValue>
#
...
# `docs/architecture/2026-08-28/ingredientPipelineState.md` §1 fell through: it claimed `verifyLine` and
...
#
#     $ bash -e .github/scripts/verifyDeployment.sh stacks "packages/infra/global/node_modules/.bin/tsx packages/infra/global/bin/app.ts"
...
#
# ⚠️ `UPDATE_ROLLBACK_COMPLETE` is `stale`, not `ok`, and the asymmetry with `deployGate.sh` is deliberate:
...
verify_deployment_classify_resource() {
    ...
    if [ "$#" -lt 1 ] || [ -z "$status" ]; then
        echo "usage: verifyDeployment.sh classify-resource <ResourceStatus>" >&2
...
#   3a. a host whose SUFFIX names an AWS service (an RDS endpoint) is classified by that suffix — the same
#      shape argument as (2), one identifier form over.
...
verify_deployment_classify_reference() {
    ...
    if [ "$#" -lt 2 ]; then
        echo "usage: verifyDeployment.sh classify-reference <envKey> <envValue>" >&2
    ...
    # An RDS endpoint names its own service in its SUFFIX, which is why it is classified here by shape
    # rather than by adding `_DB_HOST`/`_DB_ENDPOINT`/`DB_ENDPOINT` to the key table below. Those three
    # spellings are what the recipe, food and bootstrap Lambdas happen to use today; the fourth consumer
    # would arrive with a fifth spelling and go unchecked again, which is the defect this file exists for.
    # Covers instance, `cluster-` and `proxy-` endpoints — all three end the same way, and the resolver
    # below probes both APIs because `describe-db-instances` matches only the first.
    case "$value" in
        *.rds.amazonaws.com)
            verify_deployment_reference rds-endpoint "$value"
...
verify_deployment_stacks() {
    ...
    if [ -z "$app" ]; then
        echo "usage: verifyDeployment.sh stacks <cdkAppCommand>" >&2
    ...
    # ⛔ The CLI comes from the `infra/` package that owns this app, never from the root. Both halves of CDK
    # live there — the root carries neither, because a root `dependencies` entry ships into every image built
    # from this tree. Derived from `$app` rather than hardcoded, so a new CDK app needs no edit here.
    cdk_prefix=${app#*node }
    cdk_prefix=${cdk_prefix%%/dist/*}
    case "$cdk_prefix" in
        packages/infra/global*) cdk_prefix=packages/infra/global ;;
        */infra) : ;;
        *) cdk_prefix=packages/infra/global ;;
    esac
    # ⛔ STILL `npx`, with the CLI APPENDED to PATH rather than invoked through `npm exec --prefix`.
    # `deploymentVerification.integration.test.ts` stubs `npx` by putting an executable FIRST on PATH, so the
    # whole suite can drive this function without a real CDK synth. `npm exec --prefix` resolves the package's
    # own binary directly and walked straight past that stub — every scenario then reported "yielded no stack
    # names" and the suite failed on a script that was fine. Appending (not prepending) keeps the stub winning
    # where one exists, while a real run finds the CLI in the app's own package.
    listing=$(PATH="${PATH}:${cdk_prefix}/node_modules/.bin" npx cdk ls --long --json --app "$app" 2>"$diagnostics") || listing=''
...
# verify_deployment_resolve_rds <region> <endpointAddress>
#
# Resolve an RDS endpoint address to a live instance or cluster. Returns 0 when it exists — AND 0 when the
# question could not be asked.
#
# ⛔ AN UNASKABLE QUESTION IS A FAILURE, NOT A PASS. (Owner ruling — see below for what it overrides.)
#
# There is no `describe-db-*-by-endpoint` API, so this filters client-side over the account's instances and
# clusters, which needs `rds:DescribeDBInstances` / `rds:DescribeDBClusters`. Every other resolver here
# returns its AWS command's exit status, so a call that fails for want of a PERMISSION is indistinguishable
# from a resource that is absent — and this reference appears on ten Lambdas across six stacks, so a
# credential without those grants would turn one missing IAM statement into a hard red on every deploy in
# the repository, for infrastructure that is demonstrably fine.
#
# This function first returned 0 — a PASS — when the call errored, so that one missing IAM statement could
# not red every deploy in the repository. ⛔ That reasoning was rejected, and the rejection is right: 0 means
# VERIFIED, so "I could not check this" was being reported identically to "I checked this and it is fine",
# in the one script whose entire job is telling those apart. The permission-gap cost is real and accepted —
# a missing grant now fails loudly at the first deploy, which is a one-line IAM fix made once, against an
# unverified deploy going green indefinitely.
#
# So there are still three outcomes, but the third is now a FINDING with its own message rather than a
# silent success: found (0), absent (1, "no such resource"), could-not-ask (1, "could not ask").
#
# @sideEffect Calls the RDS API.
verify_deployment_resolve_rds() {
    local region="$1" target="$2" found

    if ! found=$(aws rds describe-db-instances --region "$region" \
        --query "DBInstances[?Endpoint.Address=='${target}'].DBInstanceIdentifier" --output text 2>/dev/null); then
        echo "::error::could not ask RDS about ${target} in ${region} (describe-db-instances failed). A verifier that cannot ask has not verified anything, and reporting that as a pass is how an unverified deploy goes green." >&2

        return 1
    fi
    [ -n "$found" ] && [ "$found" != 'None' ] && return 0

    # A `cluster-` or `proxy-` endpoint is never an instance address, so the second call is not a retry.
    if ! found=$(aws rds describe-db-clusters --region "$region" \
        --query "DBClusters[?Endpoint=='${target}' || ReaderEndpoint=='${target}'].DBClusterIdentifier" \
        --output text 2>/dev/null); then
        echo "::error::could not ask RDS about ${target} in ${region} (describe-db-clusters failed)." >&2
...
# The answers `verify_deployment_resolve` has already paid for, keyed `<probe>|<target>`.
#
# ⛔ MEMOISED because the references are heavily SHARED, not because the API is slow: `RecipeWorkersStack`
# spreads one `commonDbEnv` across ten Lambdas, so the same secret ARN, database endpoint and queue URL are
# resolved once per function. Each miss is an `aws` process (~0.2s before the network), and this runs on every
# deploy — three times in the identity pipeline alone.
declare -A VERIFY_DEPLOYMENT_RESOLVED=()

# verify_deployment_resolve <region> <probe> <target>
#
# Resolve one cross-stack reference against the live account. Returns 0 when the resource exists. Repeated
# lookups of the same reference are served from {@link VERIFY_DEPLOYMENT_RESOLVED}.
#
# @sideEffect Calls the AWS API on a cache miss.
verify_deployment_resolve() {
    local region="$1" probe="$2" target="$3"
    local memo_key="${probe}|${target}"

    if [ -n "${VERIFY_DEPLOYMENT_RESOLVED[$memo_key]+set}" ]; then
        return "${VERIFY_DEPLOYMENT_RESOLVED[$memo_key]}"
    fi

    verify_deployment_resolve_uncached "$region" "$probe" "$target"
    VERIFY_DEPLOYMENT_RESOLVED[$memo_key]=$?
...
# verify_deployment_resolve_uncached <region> <probe> <target>
#
# The lookup itself. Call {@link verify_deployment_resolve} instead.
#
# @sideEffect Calls the AWS API.
verify_deployment_resolve_uncached() {
    local region="$1" probe="$2" target="$3"
    ...
    case "$probe" in
        ...
        rds-endpoint) verify_deployment_resolve_rds "$region" "$target" ;;
        *) return 0 ;;
    esac
}
...
verify_deployment_check_lambda() {
    ...
    while IFS=$'\t' read -r key value; do
        ...
        # Parameter expansion rather than two `sed` processes per environment variable: `classified` is
        # `probe=<p>\ntarget=<t>`, and this loop runs once per variable on every Lambda in every stack.
        probe="${classified%%$'\n'*}"
        probe="${probe#probe=}"
        target="${classified#*$'\n'}"
        ...
        case "$probe" in
            none) ;;
            unchecked)
                echo "::warning::[${stack}/${logical}] ${key}=${target} looks like one of our physical resource names, but verifyDeployment.sh has no resolver for it. Extend classify_reference rather than leaving the reference unverified — this is the shape the CRF outage took."
                ;;
            *)
...
verify_deployment_verify_stacks() {
    ...
    if [ "$#" -lt 2 ] || [ -z "$region" ]; then
        echo "usage: verifyDeployment.sh verify-stacks <region> <stackName> [<stackName> …]" >&2
...
verify_deployment_verify() {
    ...
    if [ "$#" -lt 2 ] || [ -z "$region" ] || [ -z "$app" ]; then
        echo "usage: verifyDeployment.sh verify <region> <cdkAppCommand>" >&2
...
verify_deployment_preflight() {
    ...
    if [ "$#" -lt 2 ] || [ -z "$region" ] || [ -z "$stack" ]; then
        echo "usage: verifyDeployment.sh preflight <region> <stackName>" >&2
...
# verify_deployment_drains <region> <stackName> [--warn-only]
#
# Do this stack's log drains actually EXIST in the account, and do they point at something real?
#
# ⛔ THIS IS THE CHECK ADR-0042 RECORDS AS OWED, and it is owed because every other signal for a broken
# drain is an ABSENCE OF LOGS — which is exactly what a healthy quiet system looks like. `logDrainRegister.
# test.ts` proves the CODE cannot attach a filter without a forwarder ARN or drop one with it; no test can
# prove a DEPLOYED stage has them. That gap widened when every filter became conditional on a CI-resolved
# `LOG_FORWARDER_ARN`: a deploy that resolved an empty value is green, correct, and silently undrained.
#
# ## The two states this separates, because conflating them is the whole difficulty
#
#   * a filter whose DESTINATION CANNOT BE CONFIRMED is an unambiguous defect and FAILS. CloudWatch keeps
#     delivering to a deleted Lambda and the lines are dropped — no error surfaces anywhere, on either side.
#   * a group with NO filter is REPORTED, never failed by default. Absence is a supported state: a fresh
#     account has no forwarder to point at, and `--warn-only` is how a pipeline says "converge later".
#     Failing it outright would make a first deploy impossible, which is the deadlock this all came from.
#   * a call that FAILED is a finding, never either of the above. Folding a failed lookup into "no filter"
#     would make a principal without `logs:DescribeSubscriptionFilters` pass this check green having asked
#     nothing — the inert-verifier shape, in the direction that hides.
#   * a destination this check cannot resolve (Kinesis, Firehose, a cross-account `logs:destination`) is
#     reported UNCHECKED. It is a legal subscription target, so failing on it would be a false claim.
#
# ## ⚠️ IAM, stated because it is NOT proven here
#
# `logs:DescribeSubscriptionFilters` is a NEW action for this pipeline — `lambda:GetFunctionConfiguration`
# and `logs:DescribeLogGroups` are already exercised by `verify` and `preflight`, this one is not, and the
# deploy principal's policy is console-managed rather than declared in this repository. If the grant is
# missing, the failed-call branch above says so on the first run instead of passing silently.
#
# ## Why it enumerates nothing
#
# The groups come from `cloudformation list-stack-resources` — the stack's own model — so a group added
# tomorrow is covered the day it deploys, and no copy of ADR-0042's table lives here to rot. The ADR names
# ONE group that must stay unfiltered (the forwarder's own, because a filter on it feeds the Lambda its own
# output), and this does not try to identify it: linking a CDK-generated group back to its function is not
# derivable from a resource listing, so that group is simply one of the "no filter" rows this REPORTS. A row
# that reports the truth is not a false positive; pretending to know which one it is would be the error.
#
# @sideEffect Calls CloudFormation, CloudWatch Logs and Lambda (read-only).
verify_deployment_drains() {
    local region="${1-}" stack="${2-}" mode="${3-}"

    if [ "$#" -lt 2 ] || [ -z "$region" ] || [ -z "$stack" ]; then
        echo "usage: verifyDeployment.sh drains <region> <stackName> [--warn-only]" >&2

        return 2
    fi

    local rows logical type status physical
    local checked=0 filtered=0 undrained=0 broken=0 unchecked=0 destinations destination fn probe errors

    rows=$(aws cloudformation list-stack-resources --region "$region" --stack-name "$stack" \
        --query 'StackResourceSummaries[].[LogicalResourceId,ResourceType,ResourceStatus,PhysicalResourceId]' \
        --output text 2>/dev/null) || rows=''

    # Same reason as `preflight`: an empty listing is "unreadable", never "nothing to check". A sweep that
    # matches nothing reports SUCCESS, which is the silent direction.
    if [ -z "$rows" ]; then
        echo "::error::[${stack}] cloudformation:ListStackResources returned nothing. The stack is absent or unreadable, so this check verified NOTHING."
    ...
    while IFS=$'\t' read -r logical type status physical; do
        ...
        # ⛔ THE CALL'S STATUS IS OBSERVED, NOT FOLDED INTO ITS VALUE. `|| destinations=''` made an API
        # FAILURE indistinguishable from "this group has no filter" — and since absence is only a warning,
        # a principal missing `logs:DescribeSubscriptionFilters` would have every group reported undrained,
        # the step exit 0, and the whole check pass GREEN having verified nothing. That is the inert-verifier
        # shape this file's header documents for `set +e`, one branch over and in the direction that hides.
        # An unanswerable question is a FINDING.
        # ⛔ stderr GOES TO A FILE, NOT INTO THE VALUE. Merging it with `2>&1` reads the error text on the
        # failing path and CONTAMINATES the value on the succeeding one: one warning line from the CLI (a
        # urllib3 notice, a credential-source message) makes an undrained group report as drained, and the
        # `has NO subscription filter` warning — the whole signal for a deploy that resolved an empty ARN —
        # is never emitted. Measured: a successful call with one stderr line and NO filters reported
        # "1 drained, 0 undrained". The four states this function separates collapse on a stray byte.
        errors=$(mktemp)

        if ! destinations=$(aws logs describe-subscription-filters --region "$region" \
            --log-group-name "$physical" --query 'subscriptionFilters[].destinationArn' \
            --output text 2>"$errors"); then
            broken=$((broken + 1))
            echo "::error::[${stack}] logs:DescribeSubscriptionFilters failed for ${physical}: $(tr '\n' ' ' <"$errors")"
            rm -f "$errors"
            continue
        fi
        ...
        if [ -z "$destinations" ] || [ "$destinations" = 'None' ]; then
            undrained=$((undrained + 1))
            echo "::warning::[${stack}] ${physical} has NO subscription filter, so its lines reach CloudWatch and stop there. Expected for the forwarder's own group (ADR-0042); for any other, the app deployed with LOG_FORWARDER_ARN empty and attaches on its next deploy."
            continue
        fi

        filtered=$((filtered + 1))

        # ⛔ EVERY filter, not `subscriptionFilters[0]`. CloudWatch permits two per group, so reading only the
        # first lets a healthy one mask a broken one — and makes the counts wrong whenever both exist.
        for destination in $destinations; do
            # ⛔ DISPATCH ON THE ARN'S OWN SERVICE FIELD, which is the shape `verify_deployment_resolve_uncached`
            # already uses 380 lines up, for the reason the header states: an ARN names its own service, and a
            # target this check cannot resolve is reported UNCHECKED rather than passed in silence. The first
            # version stripped `*:function:` unconditionally — which returns the string UNCHANGED on a
            # non-match, so a Kinesis, Firehose or cross-account `logs:destination` target (all valid here)
            # produced the literal `arn` and then FAILED THE DEPLOY claiming a function of that name was
            # missing. A verifier that fails on a false claim is worse than one that misses.
            case "$(printf '%s' "$destination" | cut -d: -f3)" in
                lambda)
                    fn="${destination##*:function:}"
                    fn="${fn%%:*}"

                    # ⚠️ stderr is KEPT: `>/dev/null 2>&1` reads an AccessDenied as "the function does not
                    # exist", which is a second false claim in the failing direction.
                    if ! probe=$(aws lambda get-function-configuration --region "$region" \
                        --function-name "$fn" 2>&1 >/dev/null); then
                        broken=$((broken + 1))
                        echo "::error::[${stack}] ${physical} is subscribed to ${destination}, and that function could not be confirmed. Delivery fails silently, so the only symptom is an absence of logs. AWS said: ${probe}"
                    fi
                    ;;
                *)
                    unchecked=$((unchecked + 1))
                    echo "::warning::[${stack}] ${physical} is subscribed to ${destination}, whose destination type this check does not resolve. Reported UNCHECKED rather than passed."
                    ;;
            esac
        done
    done <<<"$rows"

    echo "drains: ${stack} — ${checked} log group(s): ${filtered} with a filter, ${undrained} with none, ${broken} unconfirmed destination(s), ${unchecked} of a type this check does not resolve."

    if [ "$broken" -ne 0 ]; then
        if [ "$mode" = '--warn-only' ]; then
            echo "::warning::drains: ${broken} destination(s) could not be confirmed (reported, not enforced)."
...
# verify_deployment_drains_app <region> <cdkAppCommand> [--warn-only]
#
# Run {@link verify_deployment_drains} over every stack an app synthesises.
#
# ⛔ THE LOOP LIVES HERE, ONCE. It was written in YAML inside the composite action, which meant the two
# workflows that deploy WITHOUT that action — `prod-deploy.yml` and `sandbox-identity-deploy.yml`, i.e. the
# only paths that reach `prod` and `sandbox` — could only get the check by copying it. So the stage where an
# undrained log group costs most was the one stage the check could not reach, including by hand.
#
# ⛔ THE STACK LIST'S STATUS IS OBSERVED. `verify_deployment_stacks` calls its own empty-output guard "the
# load-bearing half of this function", and the first version of this loop threw that verdict away by reading
# the list through process substitution — which `pipefail` does not reach. A failed `cdk ls` then produced
# zero iterations and a GREEN result that had checked nothing.
#
# ⚠️ A here-string rather than a pipe, so one stack with a finding does not abort the sweep: each round-trip
# here costs a full deploy, and an operator should see every finding at once.
#
# @sideEffect Synthesises the app, then calls CloudFormation, CloudWatch Logs and Lambda (read-only).
verify_deployment_drains_app() {
    local region="${1-}" app="${2-}" mode="${3-}" stacks stack failed=0

    if [ "$#" -lt 2 ] || [ -z "$region" ] || [ -z "$app" ]; then
        echo "usage: verifyDeployment.sh drains-app <region> <cdkAppCommand> [--warn-only]" >&2

        return 2
    fi

    if ! stacks=$(verify_deployment_stacks "$app"); then
        echo "::error::could not list this app's stacks, so the log-drain sweep checked NOTHING."
    ...
    while read -r stack; do
        ...
        verify_deployment_drains "$region" "$stack" "$mode" || failed=1
    done <<<"$stacks"
...
verify_deployment_drift() {
    ...
    if [ "$#" -lt 3 ] || [ -z "$region" ] || [ -z "$stage" ] || [ -z "$app" ]; then
        echo "usage: verifyDeployment.sh drift <region> <stage> <cdkAppCommand> [--warn-only]" >&2
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    case "${1-}" in
        ...
        drains-app)
            ...
            ;;
        *)
            echo "usage: verifyDeployment.sh verify|verify-stacks|stacks|drift|drains|drains-app|preflight|classify-resource|classify-reference …" >&2
            exit 2
            ;;
    esac
fi
