...
- **ADR-0005 — the `Environment` tag governs teardown: `global` persists, `pr-{N}` is deleted on PR close.**
  ...
  The delimiter-aware match lives once, in `.github/scripts/prScope.sh` (so pr-1 ≠ pr-15) — do not add a
...
- **A database's SCHEMA is a stack of its own, deployed and migrated by its own pipeline step AHEAD of everything that reads it — and the invoke STATES which migration set it expects.** `kitchensink-{identity,food,recipe}-schema-{stage}` holds the migration runner and **nothing that reads the schema**; every pipeline runs deploy-schema → migrate → deploy-everything-else. ⛔ Do NOT re-add an `aws-cdk-lib/triggers` Trigger to a service stack. That was ADR-0022's answer to a real defect — `cdk deploy` returns only once ECS has stabilised, so "deploy, then migrate" served the new image against the OLD schema for the whole window, and prod is behind CloudFront which CACHES the 500s — but `DependsOn` cannot leave a stack, so a barrier ordered only its own stack. That cost recipe **TWO runners for ONE database** (`RecipeWorkersStack` shipped a copy of recipe-service's bundle, reaching into another package's `dist-lambda/`, purely so its eight DB-touching Lambdas had something to be ordered behind), and it made the schema a hostage of the application's release: a stage behind for a reason no code change explained had no way to catch up. ⛔ Hoisting the migrate step was **silently WORSE** and that reasoning still binds: `esbuild.mjs` copies `migrations/*.sql` into the bundle at BUILD time and the bundle ships WITH the deploy, so invoking first runs the PREVIOUS release's migration set — exit 0, `applied: []`, nothing applied, and byte-identical to "nothing was pending". What makes hoisting safe NOW is the **required `expectManifestSha`**: `runMigrations.sh run` takes the migrations directory as a fifth argument, digests it, and sends it with the invoke; a runner holding a different set THROWS. The digest is computed TWICE by two independent implementations (`@kitchensink/db-schema-guard` in the bundle, `sha256sum` in the shell) — a single shared helper can be wrong identically on both sides and still agree, whereas two cannot. ⚠️ Migration DISCOVERY is unchanged and must stay automatic: ordered `.sql`, `readdir`+`sort`, tracked in `schema_migrations` by FILENAME. Nothing is registered by hand. ⚠️ The apply loop lives ONCE, in `@kitchensink/db-schema-guard`'s `applyMigrations` (advisory lock, ledger, rollback, post-run validation) — it used to be three copies and they had already drifted. ⚠️ The standing precondition is **EXPAND-FIRST** migrations, now the whole of the contracting rule: a contracting migration ships a release LATER than the code that stopped reading the column. `WebhooksStack` is still ordered only by deploying later, which is now ASSERTED over the workflow text rather than stated; `DataStack`'s bootstrap Lambdas CREATE the databases and are correctly exempt. Read **`docs/architecture/decisions/0035-schema-stacks-decoupled-from-service-deploys.md`** (ADR-0022 is SUPERSEDED by it and kept as the record of the in-stack posture), including its residual risk: the manifest proves the runner's SQL matches the working TREE, not that the tree matches what was reviewed, and a stage deployed by hand outside a pipeline gets no barrier at all.
- **Each database has an OWNER, a MIGRATOR and a SERVICE role, and nothing a service runs connects as the RDS master (ADR-0039).** `<svc>_owner` (NOLOGIN, never `rds_iam`) owns the database and every object in it; `<svc>_migrator` (IAM) migrates as it via `SET ROLE`; `<svc>_app` / `identity_service` (IAM) holds DML only. Role names live ONLY in `DATABASE_ROLES` (`@kitchensink/db-schema-guard`) and every `rds-db:connect` grant goes through `grantConnect` with one of them — both GUARDED (`databaseRoleLiterals`, `dbUserGrantRegister`). ⛔ The master (`identity_app`, password) must NEVER be on any membership chain to `rds_iam` — AWS then forces IAM auth on it, locking out every in-VPC tool that could undo it; the bootstrap re-reads the catalog before every `rds_iam` grant for exactly that reason, so do not "simplify" the re-reads away. The master password is read by exactly two functions, `DataStack`'s role-model bootstrap and the non-prod reaper (`masterSecretConsumers`); only the reaper and the one-shot legacy recreate issue `DROP DATABASE` (`dropDatabaseAuthority`). ⛔ Every lock-out question counts EVERY `pg_auth_members` row, ADMIN-only included (`lockOutReading` guards the relaxed reading to the test harness). ⚠️ The legacy recreate is ARMED by `LEGACY_RECREATE_ARMED_STAGES` in `DataStack.ts` (event token AND function env flag) until the mandatory disarm commit DELETES the recreate path — see `roleSplitLegacyRecreateArmed.test.ts`. Read **`docs/architecture/decisions/0039-database-role-split.md`**.
...
large pans…`) and would come back if position REPLACED head-finality. ⛔ The governed test is word-anywhere
  ...
  unit, so it is a genuine `quantityDiffers` and still resolves to half a cup.
                    ...
                    engines that each STATE a unit and differ still go to the CRF),
                    ...
                    ⛔ **U36a (owner ruling 2026-08-26, ADR-0026 §8a) — AND THE RESCUE CARRIES THE AMOUNT.** U36 left the
                    number on the CRF, so `one and a half quarts of boiling water` merged as **`1 quart`** — which fails U36's
                    OWN acceptance bar, "blatantly incorrectly parsing measurement values". The licence is the rescue's own,
                    one field over: a CRF that named NO unit mis-segmented the measure phrase, so the number it read out of
                    that same phrase is RESIDUE, not independent evidence. `rescuedWinners` now takes `quantity` with
                    `statedMeasure` and `unit`.

                    ⚠️ **Re-derived through the REAL adapters, the population is 115 lines — NOT U36's 53.** U36's table
                    counts bare-number CRF measures; this counts every line the rescue FIRES on, which also includes lines
                    where the CRF produced no measure text at all. Of the 115 (86 `ingredient`, 29 `dropped`): 42 amounts
                    already agree, **57 the CRF read no amount at all** (the LARGEST class — the merge published a unit with
                    `ABSENT_QUANTITY`, e.g. `a tablespoon of flour`; invisible to a leading-digit comparison and unknown when
                    this change was scoped), **4 dropped fractions**, **8 ranges collapsed to their low end**, 4 neither.
                    **71 lines change what is stored.**

                    ⛔ **EXCEPT where the LLM's phrase states a unit but NO amount — absence is silence, one field over
                    AGAIN.** On `a large mixing bowl whip to a cream two eggs` the LLM reads the whole measure as `large`,
                    which `readStatedMeasure` resolves to `{ ABSENT_QUANTITY, 'large' }`; taking it DELETES the CRF's `2`,
                    the very failure the ruling exists to stop. So the amount moves only when the LLM's phrase states one —
                    ADR-0026 §3 applied to the amount exactly as U36 applied it to the unit. Holds on 29 of 115 (27
                    both-absent, 2 that save a stated number).

                    ⚠️ **ONLY WHAT IS STORED MOVED.** A number the CRF DID state and read differently is DISSENT, not the
                    silence `statedMeasure`/`unit` are silenced for, so `differ: ['quantity']` is still reported on every
                    changed line — do NOT "tidy" that by adding `quantity` to the silenced set. It is what keeps
                    `a cup the whites of three eggs` (garbled prose, neither reading clearly right) visible to a human.

                    ⛔ **A NUMBER IS NEVER A UNIT — §14.6's census/merge divergence is REPAIRED** (U37, ADR-0026 §8b).
                    `normalizeMeasure` read the unit POSITIONALLY, so the JOINED CRF measure `2 3 tablespoons` — which is
                    `crfProcess.ts` joining `[('2',''),('3','tablespoons')]`, i.e. "the CRF read MORE THAN ONE amount" —
                    answered `unit: '3'`, the empty-unit branch never fired, and 8 lines the merge RESCUED disposed `crfWins`.
                    The unit position now rejects a token the quantity reader produced: the first amount stated no unit, so
                    `unit` is `''` and the number falls to `residue`, whose contract already names that case. ⛔ Do NOT
                    "improve" it into "skip to the next non-numeric word" — that manufactures a unit for an amount that stated
                    none AND leaves `crf.unit !== ''`, so the branch STILL never fires: symptom fixed, bug preserved. ⚠️ Narrow
                    to a NUMBER deliberately — dropping the connective (`two or three tablespoons` folds to `unit: 'or'`, the
                    same weakness one word over) would fold BOTH sides to the same empty-unit reading, the census would answer
                    `agree` while the merge rescues, and the divergence would RE-OPEN one verdict over. ⛔ The shipped leg is
                    untouched (`readStatedMeasure`, `promoteCrfReading`, `parseComparator.ts`; the rescues stay at 115). ⚠️ The
                    deferral's stated reason was measured FALSE: over the same 1,975 replayed answers NO agreement figure moves
                    (all-three-agree 739, `measure` 1,108, `names` 975, `prep` 1,163, `differ` 356 — identical); 26 lines change
                    verdict, ALL of them leaving `unitDiffers`, and 15 change disposition, all `crfWins` → `llmWins`. ⚠️ **7 of
                    8, not 8** — L00777 (`quart 15`) is a REAL unit joined to a stray amount, so its units MATCH and it is
                    `amountCountDiffers`; closing it means ruling on whether a unit with no adjacent number is STATED, which
                    lives in `readStatedMeasure` and is still open. Report §16 carries the full before/after.

                    ⚠️ **One limit stays open.** Alternation (`one large onion or two small ones`, 4 lines) is two measures
                    against ONE measure field — a modelling gap, and a sensible single reading is taken. ⚠️ A vessel HEADING the measure
...
- **ADR-0032 — an END-TO-END TEST TARGETS A DEPLOYED ENVIRONMENT, and SKIPS when the PR's sandbox is not
  ...
  ONE manual job — `.github/workflows/deployedE2e.yml` — runs the whole suite on demand. ⛔ **There is NO
...
- **ADR-0030 — first-party analytics events: one store, two doors, lifetime counts (2026-09-01).**
  ...
  CONTROLLER detail handler (the service's `getById` is also reached by non-view routes); saves fire only on a
...
- **ADR-0029 — authored foods: the single-writer rule is about SUBSTANCES, not writers (amends T150,
  ...
  row — structurally never-synced), the dedup index is SPLIT per-author vs catalog, the food is PRIVATE
  until a human-moderated promotion (corroboration TRIGGERS a `food_promotions` queue row; only a
  `food:admin` operator PUBLISHES; publication is TWO-PHASE by database boundary and the recipe-side
  rewrite is an idempotent curated correction), erasure is delete-if-unreferenced / orphan-if-referenced
  (pseudonymous `user_id` kept on referenced rows — the `recipes.owner_id` posture), and R20 scopes every
  shared tier (search both sides, memo/mapping globals, band statistics, the viewer's detail overlay,
  clone-unbind). ⛔ T150's core is UNCHANGED: a recipe is still never a food — do not read this door as
  the write-back T150 refuses, and do not "restore" the full-table unique or a single CREATE path. See
  `docs/architecture/decisions/0029-authored-foods-substances-only.md`.
