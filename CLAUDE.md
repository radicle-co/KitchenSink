...
- **`Environment` tag governs teardown — `global` persists, `pr-{N}` is deleted on PR close.** Every CDK app tags at the `App` level: persistent global infra (network/data/domain/global/alb, identity-service, identity-webhooks) is `Environment=global` and named `kitchensink-*`; an ephemeral per-PR feature deploy (food etc., `stage=pr-{N}`) is `Environment=pr-{N}` and named with a `pr-{N}` prefix. `.github/workflows/pr-cleanup.yml` (on PR close) deletes everything matching `pr-{N}` **by tag OR name** — there is **no denylist**, so the safety depends entirely on **never naming/tagging a global resource `pr-{N}`** and on the delimiter-aware match (`pr-{N}` exactly or `pr-{N}-…`, so pr-1 ≠ pr-15). Any new **feature** service must tag `Environment=pr-{N}` + prefix its names. Read **`docs/architecture/decisions/0005-environment-tagging-and-pr-cleanup.md`**.
