...
No component, container or hook calls a service client directly. Every read is a `useQuery` and every write
a `useMutation`, so status, errors, retries and invalidation live in the query cache instead of in
hand-rolled `useState`. `ENGINEERING_EXCELLENCE.md` states the principle — _"Treat server data as a cached
