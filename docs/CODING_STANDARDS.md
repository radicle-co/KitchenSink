...
**Hyphens and underscores are PROHIBITED in file names**, with only the three exception classes listed
...
### The only three exception classes

1. **Names a library or framework resolves BY FILENAME** (allowed, not renameable) — and only where the tool
   genuinely offers no hyphen-free spelling: Next.js special files (`not-found.tsx`, `global-error.tsx`,
   `instrumentation-client.ts`, `next-env.d.ts`); Expo Router route files (`_layout.tsx`, `+*.tsx`,
   `[param].tsx` — none is tracked today, so the guard carries no mandate for them until one lands, because a
   mandate that excuses nothing fails); `package-lock.json`; husky hook scripts, which must carry the git hook name
   (`.husky/pre-commit`); `nest-cli.json`, the only spellings the Nest CLI discovers being `nest-cli.json` and
   `.nest-cli.json`; and the Spec Kit tree (`.specify/`, `v-model-config.yml`, and product-forge's
   `.forge-status.yml` / `sync-report.json` inside a feature directory). "The tool's default is hyphenated" is
   NOT this class when the tool also accepts another name — `docker-compose.yml` is `compose.yml`,
   `lint-staged.config.js` is `.lintstagedrc.js`, and Playwright's hyphenated baseline names are overridden
   by `expect.toHaveScreenshot.pathTemplate`.
2. **Persisted identifiers**: SQL migrations under `**/database/migrations/**` and `**/db/migrations/**`
   (e.g. `0005_identity_reset.sql`). `@kitchensink/db-schema-guard`'s `applyMigrations` records each one in
   every deployed database's `schema_migrations` ledger **by filename**, with no checksum — renaming one
   re-applies it against every live database.
3. **Exempt by owner ruling (2026-09-13): GitHub workflow files and markdown files.** Every `*.md` file,
   anywhere, and every workflow directly under `.github/workflows/`. This is a POLICY exemption, not a tool
   requirement — a workflow's name is not resolved by GitHub, and a markdown file's is not resolved by
   anything — so it is recorded as the owner's ruling rather than dressed up as a mandate. A markdown file
   _named_ freely still has to keep every path it cites _correct_.

`packages/infra/global/__tests__/fileNameConvention.test.ts` carries classes 1 and 2 as `MANDATED` (each
entry naming the tool and its evidence) and class 3 as `EXEMPT_BY_OWNER_RULING`. Nothing else is exempt, and
there is no legacy allow-list.
...
### ⛔ Barrel Files — only where a PACKAGE EXPORT needs one (owner directive)

A **barrel** is a file whose entire body is `export … from` / `export *` re-exports and which declares
nothing of its own. A barrel is permitted **only when it is the target of a `package.json` `exports`
entry** — that is, when it IS the public surface of a workspace package (`@kitchensink/*`, `@commise/*`),
including the `src` entry of a package whose `exports` name its built `dist` counterpart. Everywhere else
a barrel MUST NOT be created, and an existing one is to be removed rather than extended.

**Import the module that declares the thing, not a file that forwards it.**

```typescript
// Good — the declaring module, named directly
import { applyDraftAction } from '../form/props.js';
import { recipeVersionMessages } from './messages.js';

// Bad — an intra-package barrel that only forwards
import { applyDraftAction, recipeVersionMessages } from '../index.js';
```

**Why this is a rule and not a preference.** A package export is a **contract**: consumers write
`@kitchensink/recipe-service-client/hooks`, so `src/hooks.ts` must keep naming all 55 hooks and is
correctly a barrel. An intra-package barrel is a contract with nobody — it buys one shorter specifier and
charges for it four ways:

- **It hides the real dependency graph.** Every consumer appears to depend on the whole folder, so
  "who uses this?" and "what does this pull in?" stop having honest answers.
- **It manufactures import cycles.** Two modules in one folder that both import the folder's barrel are
  cyclic through it even though neither references the other, and the cycle is invisible at both call
  sites. `versions/model.ts` hit exactly this shape when split by concern.
- **It is a second place the surface is stated**, so it drifts from the modules it forwards — the same
  DRY failure as any other copied list.
- **It defeats the file-level rules above.** A barrel re-exporting 44 symbols reads as one small file
  while naming the surface of a 1,281-line module.

### The four barrels that stay, and the measured reason for each

The rule was applied across the repo in one pass. **Four** files survive it, and each was kept on evidence
rather than preference — do not "finish the job" by deleting them:

| File                                                                                                  | Why removing it breaks something                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `services/recipe-service/src/database/schema/index.ts`                                                | `database/client.ts` does `import * as schema` from it and hands that namespace to `drizzle(pool, { schema })`. It is the ORM's schema object, not an indirection. It is also the DISCOVERY surface for `schemaModelConformance.integration.test.ts` — replacing it with an explicit table list would mean a new table is no longer automatically conformance-checked, which trades a barrel for a weaker guard. |
| `schemas/food/src/schemas.ts`<br>`schemas/identity/src/schemas.ts`<br>`schemas/recipe/src/schemas.ts` | **GENERATED** — each carries "⚠️ GENERATED FILE — DO NOT EDIT", is produced by `npm run contract:generate --workspace=@kitchensink/<svc>`, and CI fails on any difference from a fresh regeneration. The aggregator is the generator's output format under ADR-0014, so the change would belong in `packages/tools/contract-gen` and in that ADR, not here.                                                      |

⚠️ A barrel with **zero** importers is not a barrel question at all — it is dead code. Delete it outright
(eleven were, in that pass).
...
### ⚠️ `exactOptionalPropertyTypes` is a CONVENTION here, not a compiler setting

An optional property is **omitted**, never set to an explicit `undefined` — spread-when-present
(`...(x === undefined ? {} : { x })`) rather than `{ x: undefined }`. Write every optional field that way.

⛔ **But the compiler does NOT enforce it.** `exactOptionalPropertyTypes` is absent from
`packages/tools/typescript/base.json`, so nothing fails when someone writes the explicit `undefined`. Several
docstrings used to say the flag "forbids" it, which told readers `tsc` had their back when it did not — the
same false-safety shape as a docstring claiming an alarm exists. They now name this convention instead.

⚠️ **Measured 2026-09-12, so the next person has a number instead of a guess:** turning it on produces **417
errors** repo-wide — `@commise/features-recipes` 150, `@commise/mobile` 68, `@kitchensink/recipe-service` 43,
`@kitchensink/food-service` 27, then a long tail. Mostly `TS2375`/`TS2379`. Note the two packages whose
docstrings cited it hardest are the two worst offenders, so enabling it is its own migration, not a
tidy-up — and per-package adoption is not a cheap first step either, for the same reason.
...
5. **A single manual job runs the whole e2e suite on demand** — `.github/workflows/deployedE2e.yml`. That is
...
| E2E (Maestro, mobile) | `.maestro/`                          | `*.yaml`                   | `runMaestroFlows.sh`           |
...
### ⛔ 11.0 react-query fronts every call to one of our services (owner directive)

No component, container or hook calls a service client directly. Every read is a `useQuery` or a
`useSuspense*` read (or an `ensureQueryData` promise handed to `use`), and every write a `useMutation`, so
status, errors, retries and invalidation live in the query cache instead of in hand-rolled `useState`. `ENGINEERING_EXCELLENCE.md` states the principle — _"Treat server data as a cached
snapshot you don't own"_ — and this is the enforceable form of it.

⚠️ **What counts, and what does not.** Three shapes are NOT violations, and naming them here is cheaper than
re-auditing:

- **A direct-to-S3 PUT with a presigned URL.** It reaches AWS, not us; the URL is itself the credential; and
  there is no cache key to own. `useAvatarUpload` wraps its presign **and** PUT in one `mutationFn` because
  the pair is atomic to the caller, not because the PUT needed react-query.
- **`fetch(asset.uri)` to read a locally picked file into a `Blob`.** No network.
- **A fire-and-forget startup probe on an injected `fetch`** (`contractSkew.ts`) — it is the thing that
  REPORTS contract skew, so it sits outside the client funnel by design and renders nothing.

⛔ **Do NOT wrap a SEQUENCE of mutations in an outer `useMutation` where the steps are already mutations.**
Intent is already satisfied (CLAUDE.md §1 — "TanStack mutations ARE Command"), so the wrapper adds machinery
and nothing else — and it can subtract. `useRecipePhotoUpload` was converted this way on 2026-09-12 and
**reverted**: react-query propagates status through its notify manager (asynchronously, correctly), while
`useRecipePhotoUploadQueue` used `uploading` as a **scheduling** primitive that must flip synchronously. The
"settle" effect then fired before the upload began and marked a photo `ok` that never uploaded; eight queue
tests caught it. The cache is a snapshot, not a scheduler. Where a sequence needs a per-step verdict, have it
RESOLVE an outcome value and await that — never infer one from a status flag's edge.

⛔ **A read's first load suspends, under ONE boundary (owner directive).** A read a surface cannot render
without is a `useSuspense*` read under `QueryBoundary` (`@commise/query/boundary`), which composes
TanStack's `QueryErrorResetBoundary` into `react-error-boundary`'s reset — without that reset, Try again
re-renders the cached failure instead of refetching. Suspense owns pending, the boundary owns failed, and the
leaf receives settled data and takes no `status` or `onRetry`.

On web every suspense read sits under `ClientQueryBoundary`. It renders `loading` on the server and through
hydration **unless every key in `prefetchedKeys` already holds data in the cache**. On a prefetched route that data
IS the dehydrated prefetch, so a successful prefetch ships its HTML and a failed one ships the skeleton, with no
server fetch. Build the query options once and pass the same object's key to the boundary and to the read. ⛔ Never
let React's server-error recovery carry a designed degradation: Next reports every client-rendered Suspense boundary
through `onRecoverableError`, so it would file one phantom error per degraded request. The browser query client
retries nothing during the server pass (`createAppQueryClient`), because the Clerk session its retry waits for exists
only in the browser.

⛔ **Chrome above a read's first data-dependent element renders OUTSIDE the boundary** (a frame + results split: the
list, collections and discovery surfaces each ship `*Frame`, `*Results`, `*Loading` and `*LoadError` leaves), so a
pending or failed read never unmounts the search field or the tabs. What the frame needs from inside the boundary
crosses it as a value, never a callback into render: a `useRecoverySignal` counter for heading focus after a
recovered refresh, and a PASSIVE observer (`enabled: false` on the read's own options) for anything the chrome shows
from the settled data, such as discovery's facets.

⛔ **A surface's primary results read whose key changes under a focused input suspends on a `useDeferredValue`
key**, which keeps the previous results on screen, marked `aria-busy` with the design-system `PendingBar`, while
the next key resolves (discovery's `useDeferredDiscoveryCriteria`). The deferred value must be memoized on
primitives, because `useDeferredValue` compares with `Object.is`; and the boundary's `resetKeys` take that settled
value, so a failed read clears once the viewer changes what they asked for. Debounce and deferral are not
alternatives: the debounce limits requests, the deferral keeps results visible.

These stay OFF suspense, each for a stated reason:

| Shape                                                                                  | Why not                                                                                                    |
| -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| A mutation                                                                             | A Command, not a render read; its pending state is `busyControlProps` and its error an inline banner.      |
| An `enabled`-gated read                                                                | Suspense forces `enabled`; compose a child that renders only when the condition holds, or keep `useQuery`. |
| Debounced typeahead or disambiguation list, polling, a background refetch or load-more | The query already holds data, or its next keystroke is the retry; the state is inline.                     |
| A lazy chunk                                                                           | `React.lazy` caches a rejection, so a retry cannot recover; its boundary offers none.                      |
| Enrichment the surface renders without (a heading's name, a profile tier)              | Not a read the surface cannot render without; on a suspense read's cache key it costs no second request.   |

⛔ **A failed REFRESH never replaces what is on screen.** TanStack sets `isError` when a refetch fails too, while the
data it already loaded stays in the cache; only `isLoadingError` means there is nothing to show. A suspense read
gets this structurally, and a remaining `useQuery` surface's page error comes from `toDetailQueryView`
(`@commise/features-core`), which takes the query result itself and reads `isLoadingError`, never a hand-written
`isError` branch. A failed refresh of data on screen (`isRefetchError`) is reported by `useRefreshNotice`
(`@commise/query/refresh-notice`) through the `RefreshNotice` primitive (`@commise/ui/refresh-notice`): a polite announcement, a Try again, and focus moved to
the surface's heading only when a retry started from the notice succeeds. A suspense read gets this for free —
`isRefetchError` does not throw into the boundary — and takes the same notice. A group of reads that fail together
(the discover rails share one endpoint) reports ONE notice for the group. Stay silent only where a retry would
change nothing on screen: an editor or form seeded once from the read (a stale base is caught at save by the
409), not a surface showing what the read returned. Two shapes are exempt from the notice, each for its reason:
a typeahead or disambiguation list, whose next keystroke IS the retry (the key changes); and the erasure donation
list, which blocks on ANY `isError` on purpose, because erasure destroys every recipe the viewer is not shown.

⛔ **`useErrorBoundary().showBoundary` escalates ONE kind of failure**: an async failure that starts in an event handler or effect, is not returned into a query or mutation promise (those already reach the boundary, or their designed inline state), and has no designed inline state of its own. Reaching for it anywhere else throws away context — a form, a dialog, loaded rows — that the failure did not destroy.
