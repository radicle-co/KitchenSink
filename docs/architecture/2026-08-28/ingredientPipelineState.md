...
- The deployment-drift check (`.github/scripts/verifyDeployment.sh drift …`), which reads the git-sha tag
...
4. **ADR-0024 model drift.** The verifier still pins `NOVA_MICRO_MODEL_ID` while the importer moved to
   ...
   ⚠️ STALE (2026-09-12): the parse leg is **`amazon.nova-2-lite-v1:0` (Nova 2 Lite)** —
   `PARSE_LEG_MODEL_ID = NOVA_2_LITE_MODEL_ID` in
   `packages/services/recipe-workers/src/handlers/parseLine.ts`. It briefly ran Nova Lite v1 while residency
   was unresolved; an owner ruling of 2026-09-12 (ADR-0024 §4b) recorded a `residencyApproval` on the Nova 2
   Lite entry, accepting that user recipe text rests in us-east-2/us-west-2 for AWS abuse detection, so the
   runtime and the IAM policy now both admit it. The drift this item names is real and has GROWN: the pool now carries **four**
