...
**Still hand-made, deliberately not automated here** (recorded so nobody assumes previews are self-serving): creating the Route 53 record, adding the Vercel project domain, issuing the cert, and above all **the per-deployment alias (`POST /v2/deployments/{id}/aliases`), which is still set by hand on every build.** Until that is wired into the preview workflow, a preview keeps serving whatever deployment was last aliased — which is why PR #73's alias is pinned to a specific `dpl_…` above. Teardown is therefore currently ahead of setup: it will correctly reclaim an address that a human created.
