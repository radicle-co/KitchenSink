...
- **Listener-rule priorities must be globally unique on the shared listener.** Allocation: identity = **100**, food = **200**; future services pick **300, 400, …**. Two rules with the same priority on one listener is a synth/deploy error. Keep the allocation comment in each service stack in sync.
...
3. **Per-service priority allocation:** identity = 100, food = 200, next = 300, … — unique across the shared listener, documented at each rule.
