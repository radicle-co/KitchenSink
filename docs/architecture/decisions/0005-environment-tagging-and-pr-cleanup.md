...
4. **Teardown also reclaims the preview's PUBLIC ADDRESS, which CloudFormation does not own** (added 2026-07-28; ADR-0001's _Update (2026-07-28)_ item 3). A sandbox web preview is reachable through a Route 53 `CNAME pr-{N}.sandbox.{domain} → cname.vercel-dns.com` plus a Vercel project-domain binding, neither of which is a stack resource — so before this, a closed PR left the CNAME pointing at a provider where the hostname was no longer claimed, i.e. a **subdomain-takeover vector**. `packages/apps/commise/web/scripts/teardownPreviewDomain.ts` removes both, and the teardown script runs it **first**, before any stack delete (which can hang for many minutes). Two properties are load-bearing:
    - **DNS is deleted BEFORE the Vercel claim is released.** The takeover window is exactly "record still points at Vercel, nobody claims the name", so an interrupted run may only ever leave the safe half-state (still claimed, no longer resolving).
...
- The DNS scope (`previewHostForPrToken` / `prTokenForPreviewRecordName`, `packages/apps/commise/web/scripts/teardownPreviewDomain.ts`) is exact **label equality**, and both the Route 53 and Vercel adapters re-assert it. `packages/apps/commise/web/scripts/__tests__/teardownPreviewDomain.test.ts` asserts the apex, the `*`/`\052` wildcard, the ACM validation records and `identity.sandbox.…` are never claimed — including when they arrive on the same `ListResourceRecordSets` page as the preview record.
