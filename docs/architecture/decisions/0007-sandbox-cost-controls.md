...
- **Container Insights is a CONSTANT, not stage-derived, and it is `DISABLED` everywhere.**
  ...
  below — prod `ENHANCED → STANDARD` with `pr-{N}` disabled, then disabled everywhere once the residual was
  measured to be all prod and read by nothing.
...
listener imports and is not worth ~7h/night).

⚠️ It is no longer the residual cost either, and the reasoning above was overturned rather than kept:
[ADR-0028](0028-on-demand-sandbox.md) put `kitchensink-alb-sandbox` and the shared sandbox identity service
on an ON-DEMAND lifecycle, deleted and rebuilt rather than scheduled, so the "delete/recreate churns DNS"
objection was re-argued there and lost. An ALB still has no stop primitive; what changed is that it is no
longer kept running.
...
- **What this schedule governs, and all it governs, is the two resources that can only be STOPPED**: the
  sandbox RDS instance and the `t4g.nano` NAT instance. Sandbox compute is no longer merely off ~9h/day —
  under [ADR-0028](0028-on-demand-sandbox.md) it is deleted when idle and rebuilt on demand. Of this ADR's
  three original savings premises only the RDS `small → micro` downsize survives unchanged; Container
  Insights is disabled everywhere rather than STANDARD in non-prod, so no cluster inherits anything.
...
## Amendment — the deploy-time wake gate wakes the NAT too
...
inside the window runs against stopped infrastructure. `.github/scripts/sandboxWake.sh` closes that, and
...
script is renamed `sandboxWake.sh` because "db-wake" is no longer what it does. ECS stays out of scope on
...
## Amendment — the prod exemption became the cost, and is withdrawn
...
2. **The tier decision lives in ONE place** — `CONTAINER_INSIGHTS_TIER`
   (`packages/infra/security/src/containerInsights.ts`) — not a ternary repeated in three stacks. The
   "one place" property is what survived every later change to the VALUE.
...
## Amendment — the residual was ALL prod, nothing reads it, and the tier becomes a constant
