...
The classifier now reads freshness from the recipe's own contributing lines, so a stale food behind a line that
added no catalog figure does not caveat the reading.
...
> The claim is stated in the **web** entry (`packages/apps/commise/features/recipes/src/widget/RecipeHomeWidget.tsx:14`, and echoed in `docs/generated/components/groups/featuresRecipes.json`); `RecipeHomeWidget.native.tsx:33-34` now carries this correction itself, in the same words. The correction still stands — only "which file says it" changed.
