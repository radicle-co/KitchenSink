...
- **Relates to**:
  ...
  `kitchensink-cost-guardrails` budget carries the persistent tier tag (ADR-0005) and audits the whole account; the budget in
...
| 0a  | Per-line CLAIM before any reservation                           | Paying twice for one unit of work          | **Real-time**, one indexed write              | Per-line. Cheaper than layer 0 — it never reserves      |
| 1   | Explicit `maxTokens` + input-token cap                          | Worst-case cost of one call                | **Real-time**, in-process                     | Per-call. Precondition for layer 3                      |
| 2   | `reservedConcurrency` (1 in non-prod, scaled in prod)           | Burst _rate_                               | **Real-time**                                 | Per-function. **Blast radius, not the ceiling**         |
...
⛔ **Layer 0a — a unit of work is CLAIMED before anything is reserved.** The cheapest call is the one never
made, and both consumers used to make it: the parse leg ran its whole pipeline and decided what to land
afterwards, and the verification gate reserved, called, and only then computed the key its verdict lands
under. Every input to that key comes from the message, so a duplicate delivery could always have been
answered from storage for free. Each consumer now asserts, in one statement ahead of any spend, that the
work is still wanted (the job unexpired, the line unparsed, no verdict under this model) and that no other
delivery holds it — **zero rows returned IS the refusal.** This sits below layer 0 because it stops the
spend rather than the loop: layer 0 bounds how many times a doomed message may be retried, layer 0a means
the retry costs an indexed read instead of an inference call. Its counter is deliberately NOT
`maxReceiveCount`: a receive is spent whenever SQS hands the message over, including when the consumer
cannot reach its database at all, while an attempt is recorded only by a successful claim.
...
⛔ **Layer 2 is a per-stage POLICY, not a constant — and non-prod is what this ruling depends on.** Prod
scales both consumers to raise throughput; sandbox and every `pr-{N}` keep the reservation of 1. The two
numbers that must agree — the function's `reservedConcurrentExecutions` and its SQS event source mapping's
`maximumConcurrency` — come from one module, `recipe-workers/infra/lib/consumerConcurrency.ts`, because they
sit on different resources and neither validates the other: raise the reservation alone and nothing speeds
up, raise the mapping alone and the surplus is throttled, which spends a receive and dead-letters healthy
messages. The guard asserts the reservation is SOURCED from that module rather than reading its value, since
a gate that only reads numbers cannot tell "raised deliberately in prod" from "deleted", and those are
opposite changes. Scaling prod does not weaken the ceiling — the reservation is charged BEFORE the call and
the row lock serialises callers, so higher concurrency reaches the $100 sooner and never past it. What it
does change is that the monthly pool is consumed earlier in the month, and denials then arrive as DLQ depth.

⛔ **Layer 0's `maxReceiveCount` is DERIVED from the nightly stop, not a flat number.** A consumer whose
database is stopped fails every delivery and each failure spends a receive, so a queue whose redrive count
is shorter than ADR-0007's nine-hour non-prod shutdown dead-letters perfectly healthy messages because the
stage was asleep — filling the DLQ depth layer 4 alarms on with work nothing was wrong with. Each consumer
queue's count is therefore `window / its own visibility timeout`, with one delivery to spare. ⚠️ This does
NOT raise what layer 0a bounds: a receive is spent whenever SQS hands a message over, an ATTEMPT is recorded
only by a claim, and a claim needs the database — so an outage spends receives and never a line's attempt
allowance.
...
⛔ **The series carries the PERIOD'S RUNNING TOTAL, and every emitter into it must.** The threshold is a
fraction of the monthly ceiling, so the number compared against it has to be the same quantity the ceiling
bounds — `reserve`'s return, which the type names "the period's total AFTER this charge". This is stated
because it is invisible at the call site and a second emitter got it wrong: the parse leg published ONE
CALL'S cost into the same metric, which does not read as corrupt (a per-call figure is simply small under
`Maximum`) but silently UNDER-REPORTS — that leg's calls raised the shared counter without ever publishing
the raised total, so a month whose spend was dominated by parsing showed the alarm only whatever the
verification leg last sent, and nothing at all when that leg was idle, which `treatMissingData:
NOT_BREACHING` reads as healthy. An emitter with no reservation to report (an ungated stage, which tracks no
ceiling) may publish its own cost; an emitter that reserved must publish the total.
...
`docs/architecture/2026-08-28/ingredientPipelineState.md` §3.
...
⛔ **A `residencyApproval` IS recorded for Nova 2 Lite — owner ruling, 2026-09-12 — and what it admits is
stated here rather than left in a marker.** Whether user recipe text may come to REST in us-east-2/us-west-2
— where AWS documents that prompts and outputs "may be stored … for abuse detection purposes" — is the
question the refusal existed to hold open. The owner answered it: **yes, for the parse leg.** Every inference
profile that exists for Nova 2 Lite leaves us-east-1, so running the leg on that model means recipe text
rests in those regions; the ruling accepts that consequence rather than reinterpreting it.

⚠️ **This does NOT transfer the determination away from 016.** The ruling was made ahead of the
legal-compliance framework, not instead of it: when 016 is built it owns this question and may narrow or
revoke the warrant, and the registry entry says so where the marker lives. What the enforcement guarantees is
that a revocation is ONE edit — `planReservation` and `bedrockInvokeStatements` both read that single field,
so the runtime and the IAM policy cannot disagree about it.

⚠️ **Claude Haiku 4.5 is deliberately NOT warranted by the same ruling**, though it reaches the same three
regions and the argument would carry. It is not a model we ship, the ruling was about the parse leg, and an
approval nobody needs is an approval nobody reviewed. It also keeps the refusal branch exercised:
`spendArithmetic.test.ts` asserts that an unwarranted cross-region entry still exists, because approving the
last one would delete that coverage silently rather than loudly.

**The parse leg therefore runs Nova 2 Lite**, the model ADR-0026 §9 selected on gold-set accuracy (84%/53%
against Nova Lite v1's 73/41). The cost is real and is not a rounding error: Nova 2 Lite is dearer in every
token class — 5.5× input, 11.5× output, 5.5× cache reads — so the $100 pool stretches less far than it did
under the fallback.

⚠️ ADR-0026's amendment and `docs/architecture/2026-08-28/ingredientPipelineState.md` both describe the
parse model, and both cite `PARSE_LEG_MODEL_ID` rather than restating the id — which is what makes the pin
the single place it is recorded. ADR-0026 also carries the accuracy evidence the choice rests on (84%/53%
against Nova Lite v1's 73/41 on the same external gold set).
...
**The failure mode this creates, and why it is observable rather than fixed.** The byte bound is a claim
about how known tokenizers behave, not a theorem — a tokenizer that NORMALISES before encoding would beat
it, since NFKC can expand a single compatibility character considerably. When the bound is beaten, nothing
refuses the call: the reservation is simply too small, and settlement's delta is **deliberately unclamped**,
so the overshoot is charged in full and disappears into the month's total. That is the accepted direction
(the ceiling still binds; it is the per-call reservation that was a lie), but it is silent, and a silent
under-reservation is indistinguishable from ordinary volume in the ledger.

⛔ So the detector IS the control, and it is not optional: `VerificationInputBoundExceeded`
(`spendMetrics.ts`) is emitted with the overshoot whenever a billed response reports more input tokens than
the bound reserved for, and `VerificationInputBoundAlarm` (`RecipeWorkersStack.ts`) fires on it. Removing
either returns this to the state where the only symptom is a bill. The metric's name is asserted equal to
the constant on both sides, because an alarm watching a metric nobody emits is the quietest failure here.

⚠️ **How to measure the two constants, so the next person does not re-guess them.** They are solvable from
the same responses the gate already receives: issue Converse calls whose prompts differ only in TURN COUNT
at a fixed byte length, and again only in BYTE LENGTH at a fixed turn count, then read `usage.inputTokens`
off each billed response. The per-turn slope and the intercept fall out of two points each; the residual
against `inputTokenBound` is the allowance actually needed. Until someone does that, the detector's
observed overshoot distribution is the better evidence — it measures the real prompts rather than a probe.
