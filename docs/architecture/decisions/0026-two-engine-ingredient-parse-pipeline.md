...
Every figure in `docs/reports/2026-08-23/ingredientParseModelComparison.md` is denominated in the
...
this change, recorded in `docs/reports/2026-08-23/ingredientParseModelComparison.md` §13
...
> ⛔ **THE MODEL IS §9'S CHOICE, BUT NOT FOR §9'S REASON ALONE — AND THAT IS THE MAIN THING READERS GET
> WRONG ABOUT ADR-0026.** The shipped parse model is **`amazon.nova-2-lite-v1:0` (Nova 2 Lite)**:
> `PARSE_LEG_MODEL_ID = NOVA_2_LITE_MODEL_ID` at
> `packages/services/recipe-workers/src/handlers/parseLine.ts`, value in
> `packages/shared/recipe-core/src/spend/spendArithmetic.ts`. It is callable because of **residency, on an
> owner ruling of 2026-09-12** (ADR-0024 §4b) — not because §9 picked it. Every inference profile that exists
> for Nova 2 Lite leaves us-east-1 (`us.` reaches three regions, `global.` reaches wider, and there is no
> single-region or application profile) while AWS stores prompts and outputs in destination Regions for abuse
> detection, so running this leg means **user recipe text rests in us-east-2 and us-west-2**. The owner
> accepted that; `residencyClearance` answers `approved` only because a `residencyApproval` is recorded on
> the registry entry, and BOTH the runtime and the IAM policy read that same field. ⚠️ THE HISTORY, so nobody
> reads §9 as sufficient on its own: this leg ran **Nova Lite v1** (73/41 static, ~11 points worse on the
> ingredient half and ~12 on the instruction half) for as long as the question was open, because
> **selecting a model on accuracy never made it residency-clear**. What changed is the warrant, not the
> measurement. ⚠️ 016 still owns the determination and may narrow or revoke it, which is one edit. §9's
...
prompt against **Nova 2 Lite**, fell back to **Nova Lite v1** while residency was unresolved, and returns to
**Nova 2 Lite** under the 2026-09-12 warrant (banner above). ⛔ NOT on the `flex` service tier — no caller
sets `serviceTier` on the parse call, and `gatedLlm.test.ts` asserts it undefined on all four gated legs.
⚠️ That is now a CHOICE rather than a constraint: Nova Lite v1 publishes no flex usage type at all, while
Nova 2 Lite publishes `USE1-Nova2.0Lite-*-flex` at half price — so asking for the tier is newly possible and
still refused, because the registry holds standard on-demand rates and a flex call priced off them would
reserve 2× the real cost. Tier-aware rates are an ADR-0024 decision, not a flag. The RULINGS below stand; the RATES do not describe the
...
### 10. THE RETRY BUDGET AND THE MERGE, under five owner rulings (2026-09-19)

Measured first: **28 of 1,265 ingredient lines** landed `status='parsed'` with `foods: []` and
`needsReview: false` — a measurement stored against no ingredient, with nothing flagged and nobody sent to
look. Production telemetry corroborated it independently. Five rulings followed.

**R1 — an EMPTY LLM food list does not beat a non-empty CRF one.** §3's "absence is silence" was applied to
units and measures and never to `foods`, so `DEFAULT_WINNERS.foods = 'llm'` discarded a food the CRF had
read whenever the LLM answered nothing. `crfRescuedTheFoods` scopes the override to the EMPTY case only —
it does **not** change which engine wins when both answered, and §9's warning that this entry is not
evidence the LLM files modifiers better still stands.

**R2 — a foodless answer is retried ONCE, not three times.** Consecutive, not cumulative: empty → disputed
name → empty keeps the full budget, because that middle attempt is the class R3 protects.

**R3 — a DISPUTED NAME still gets the full `MAX_PARSE_ATTEMPTS`.** Unchanged.

**R4 — a retry that repeats the previous answer stops the loop.** "Same" is the ordered food names plus
`statedMeasure`, case- and whitespace-folded — exactly the facts a judge is asked about. ⚠️ R4 NARROWS R3
where they meet: a model repeating a rejected name stops at attempt 2. A parser repeating itself has
stopped producing readings, and each further attempt is a billed call that cannot change the answer.

**R5 — where nobody read a food, the SOURCE LINE becomes the food name**, flagged `name_is_source_line` and
landed `unparseable`. The text is never lost and is never presented as a reading.

⛔ **TWO DEFECTS THE FIRST IMPLEMENTATION INTRODUCED, both the same shape: a MEANING changed and only some
of its readers moved.** `foods: []` stopped meaning "no reader named a food" and `not_a_food` stopped
meaning "nothing is bound" — three consumers encoded rules on the old meaning and one was updated.

1. **`not_a_food` was one token for two facts.** A validator disputing EVERY name is EVIDENCE; a model
   returning nothing is SILENCE. R1 is right to drop silence the CRF has just answered, and it was dropping
   the verdict with it — a line whose every name a judge rejected landed `parsed`, unflagged. Split:
   `no_food_returned` carries silence, `not_a_food` keeps the verdict, and only the former is dropped.
   ⚠️ A rescued line carrying a surviving verdict still lands `parsed` deliberately — it HAS a food, read by
   a real engine, and `unparseable` is terminal with no retry. Downgrading a read line is worse for the cook.
2. **R5's placeholder overwrote a real ingredient name in published recipes.** `cookbook-import`'s
   `applyPipelineReading` substitutes a reading only when it names exactly ONE food and declines on zero —
   and R5 made zero unrepresentable, so the decline branch died and the whole clause replaced the library
   parser's name, quantity and unit. ⚠️ The repair needed a TYPE change, not a condition: the consumer
   received `ParsedFacts`, which carries no `reviewReasons`, so the safety check was UNREPRESENTABLE rather
   than merely unwritten. Inferring the case from the parse's SHAPE was rejected for the reason
   `mergedReasons` already records.

⚠️ **Residual, escalated not fixed: a validator's verdict does not survive the parse cache.** `rememberFresh`
stores `ParsedFacts`, which has no `reviewReasons`, and rehydration re-derives them from the measure alone.
So the same line parsed a second time comes back without the verdict. Pre-existing — `not_a_food` and
`measurement_unverified` always evaporated — but R1 makes the consequence worse, because the warm line now
lands `parsed` where it previously landed with no food at all. Either the cache stores validator-raised
reasons (costing the "nothing derivable is stored" invariant) or a validator-terminal record is not cached
(costing a re-paid billed loop per affected line). **Open.**

⚠️ **The corpus-wide diff this class owes was NOT run.** R1 changes WHICH engine's name is stored, which is
precisely the question that instrument answers — and precisely how defect 2 above would have surfaced as a
count rather than needing a code read.
