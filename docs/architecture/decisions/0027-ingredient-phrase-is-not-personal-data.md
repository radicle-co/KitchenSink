...
- **Relates to**:
  ...
  [ADR-0040](0040-test-principals-containment-and-purge.md) — amends this ADR for TEST principals only: the test
  purge (not erasure) may hard-delete a signed, registered test principal's correction rows by `user_id`. No erasure
  sweep targets these tables.
...
The three tables lose their sweep steps in `eraseRecipeRows`. Every other step is untouched: they cover
the tables holding genuinely personal content — the recipe text a cook wrote, their collections, their
ratings, their cleartext handle. This ADR removes sweeps; it never constrains what a LATER sweep may cover,
so a sweep landing after it is not an exception to it. The roster is deliberately not enumerated here —
`erasureSweepCoverage.test.ts` derives it from the migrations and asserts coverage, which is a check rather
than a copy.

Sweeps that landed later illustrate the boundary rather than contradicting it: `recipe_parse_jobs` (the
cook's pasted text), the unreferenced private authored foods in `ingredients` (ADR-0029 §6), and
`analytics_events` (ADR-0030 §2, deliberately STRICTER than this ruling, because a typed search query is
...
The recipe database's user-bearing tables are counted by `erasureSweepCoverage.test.ts`, not
listed here: the count has moved three times since this decision and the decision did not move with it.
...
The non-vacuity floor is pinned to the schema's CURRENT user-bearing count rather than a
historical one, and is asserted by set equality rather than a `>=` threshold — a floor with slack lets a fold
drop tables without going red, which is the failure this bound exists to prevent.
...
- The erasure transaction loses three statements. Its absolute size has grown since, as later features
  added their own user-bearing tables; none of those additions touches the tables this ADR retained.
  The current step list lives in `accountErasureWorker.ts` and its coverage is asserted by
  `erasureSweepCoverage.test.ts`.
