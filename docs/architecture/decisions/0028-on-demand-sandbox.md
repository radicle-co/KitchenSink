...
last deployed". `gh pr view` says whether the pull request is still open. `sandbox-reap.yml` reads those
three and presses `Sandbox Down` on whatever has outlived its day; the deadline itself is still
`.github/scripts/sandbox-lifetime.sh expires-at`, unchanged, fed a deploy time instead of a press time.
...
implementation — `teardown-sandbox-pr.sh` — because two implementations of "what belongs to `pr-{N}`" is the
...
| `sandbox-down.yml`                      | someone asked for this one                        | dispatch               | directly                       |
| `sandbox-reap.yml`                      | is this preview past midnight of its last deploy? | hourly cron            | no — it presses `Sandbox Down` |
| `sandbox-reconcile.yml`                 | is any preview live at all?                       | hourly cron            | no — shared TIER only          |

⚠️ An earlier draft of this clause said reaping had "exactly one owner". It was written when
`sandbox-reap.yml` took the deadline question off the reconciler, and it overstated in two directions:
