...
**1. A preview is started by a button, not by a push.** `sandboxUp.yml` is a `workflow_dispatch`
workflow — GitHub renders it as _Run workflow_ in the Actions tab. It dispatches the deploys, and that is
all it does: it applies no label and stamps no deadline, because neither is a fact worth keeping a second
copy of (see 2).

**2. A preview dies at midnight America/New_York of the day it was LAST DEPLOYED,** and both facts the
lifecycle turns on are read from the systems that already record them rather than kept as copies.

A `kitchensink-*-pr-{N}` stack EXISTING is "this PR has a sandbox". Its `LastUpdatedTime` is "when was it
last deployed". `gh pr view` says whether the pull request is still open. `sandboxReap.yml` reads those
three and presses `Sandbox Down` on whatever has outlived its day; the deadline itself is still
`.github/scripts/sandboxLifetime.sh expires-at`, unchanged, fed a deploy time instead of a press time.

⛔ The two copies this replaces were each a single point of failure, and neither may come back
(`sandboxLifecycleSingleSource.test.ts` refuses them):

- A **`sandbox-up` label** carried intent by hand, and a label a human must remember fails SILENTLY —
  forget it and everything skips, green.
- A **`SandboxExpiresAt` stack tag** carried the deadline, but `UpdateStack` REPLACES a stack's tag set
  rather than merging into it. Every deploy was therefore a writer of that tag, a deploy passing no value
  ERASED it, and one untagged stack condemned the whole token. Keeping it meant threading the number
  through four workflows, teaching five CDK apps to write it, having ordinary pushes read it back so they
  would not wipe it, and refusing hand-dispatched deploys that could not — machinery whose entire purpose
  was protecting a value from the pipeline meant to be carrying it.

The consequence is deliberate: a preview being actively pushed to stays alive, and an abandoned one dies
that night. That is a change from "the deadline is fixed at the press", which killed a sandbox out from
under someone still working in it at 23:00.

**3. Every reclamation path runs the same teardown, and each owns a different QUESTION.** One
implementation — `teardownSandboxPr.sh` — because two implementations of "what belongs to `pr-{N}`" is the
drift ADR-0005 exists to prevent. `sandboxReclamationReachability.test.ts` pins the set of jobs that invoke
it, so a fourth cannot appear unnoticed:

| owner                                   | question it answers                               | trigger                | runs the teardown              |
| --------------------------------------- | ------------------------------------------------- | ---------------------- | ------------------------------ |
| `sandbox-deploy.yml` › `cleanup`        | this PR just closed                               | `pull_request: closed` | directly                       |
| `sandbox-deploy.yml` › `reap-abandoned` | is this PR closed, or long abandoned?             | daily cron             | directly                       |
| `sandboxDown.yml`                       | someone asked for this one                        | dispatch               | directly                       |
| `sandboxReap.yml`                       | is this preview past midnight of its last deploy? | hourly cron            | no — it presses `Sandbox Down` |
| `sandboxReconcile.yml`                  | is any preview live at all?                       | hourly cron            | no — shared TIER only          |

⚠️ An earlier draft of this clause said reaping had "exactly one owner". It was written when
`sandboxReap.yml` took the deadline question off the reconciler, and it overstated in two directions:
`reap-abandoned` has swept closed and abandoned PRs on a daily cron throughout, from the default branch, and
the on-close `cleanup` job predates both. They do not conflict — each asks a different question, and a
preview reclaimed by any of them is simply gone when the next one looks — but a reader told there is one
owner goes looking for one and mis-models the system.

⛔ Deletes, never stops. The last step invokes `.github/scripts/sandboxSharedTier.sh down`, which removes
...
**4. ADR-0010's ensure-exists gate gains a precondition.** `deployGate.sh decide` takes `<intent>` as a
...
The reconciler shells out to `teardownSandboxPr.sh`, the same script the on-close cleanup and the
...
- **The reconciler's discovery reads CloudFormation stack names**, so a per-PR resource that is not part of
  a `kitchensink-*-pr-{N}` stack is invisible to it and relies on `teardownSandboxPr.sh`'s tag matching.
...
> `sandboxSharedTier.sh` refused to continue to the ALB — so the ALB was never left with its importer gone.
...
|       | Create                                                                   | Destroy                                                              |
| ----- | ------------------------------------------------------------------------ | -------------------------------------------------------------------- |
| Owner | `sandboxUp.yml` → dispatches `sandbox-identity-deploy.yml` and **waits** | `sandboxReconcile.yml` → `.github/scripts/sandboxSharedTier.sh down` |
...
`sandboxWake.sh` **excludes ECS by construction** ("ECS is excluded for a reason rather than by omission"),
...
service, and `sandboxUp.yml` (which greps for exactly that string) exits 1. **The button would fail on
...
⛔ `prScope.sh` and `prScope.test.ts` are **unchanged and still correct**: those two stack names remain
...
`sandboxSharedTier.sh`'s own allowlist so the two cannot drift.
...
The offsetting benefit is not only convenience. `sandboxWake.sh` exists because ADR-0007's stop window and
...
- ⚠️ **The reclaim can now run against a stopped database, and that was already true.** The ALB/identity
  ...
  `sandboxWake.sh ensure` ahead of the reclaim, exactly as the teardown script now does.
...
- The per-PR logical databases are reclaimed by `teardownSandboxPr.sh` (see its §1), which is the "just
