...
capture lives at the **controller detail handler**, deliberately not `RecipesService.getById`, which
other routes also reach without a view taking place (a rating write returns the rated recipe's detail) —
capturing in the service would inflate every lifetime count; the IT's mutation test drives those routes
and asserts zero views.
...
view capture, for the same reason: the service's `getById` is also reached by non-view routes that must
not pay — or serve — analytics). Three rules hold it up:
