...
- **Relates to**: [ADR-0001](0001-sandbox-front-end-addressing.md) (preview reachability — the browser half is
  ...
  [ADR-0040](0040-test-principals-containment-and-purge.md) (the "separate decision" §5 names, for the service half:
  a signed test-principal pool, containment on production, and a repeatable purge — it does not by itself run the
  full web suite on production)
...
3. **ONE manual job runs the whole suite on demand** — `.github/workflows/deployedE2e.yml`, §4. That is the
...
_"playwright, e2e, maestro, etc"_. The entrypoint is `.github/workflows/deployedE2e.yml`.
...
'deliberately reaped at midnight'"_ — and made `intent` a required first parameter of `deployGate.sh
...
hourly reclaim step in `.github/workflows/sandboxReconcile.yml:243-247` would fail on it **every hour** —
and it carries **no `continue-on-error` by explicit decision**, recorded in its own comment at
`sandboxReconcile.yml:233-241`: _"`continue-on-error: true` reports the JOB green, which means an ALB that
