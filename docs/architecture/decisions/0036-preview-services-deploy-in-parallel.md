...
- **Amends**: [ADR-0010](0010-ensure-exists-per-pr-deploy-gate.md) §5 — its ordering clause only. The
  ensure-exists gate itself (§§1–3), its four conditions, `deployGate.sh`, and the 401-is-the-PASS smoke
...
`service: recipe` — where `deploy-food` genuinely skips — stayed runnable. `sandboxPreview.yml` has no
