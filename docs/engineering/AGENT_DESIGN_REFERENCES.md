...
> Sibling: [`UX_AGENT_DESIGN_REFERENCES.md`](UX_AGENT_DESIGN_REFERENCES.md) does the same for
> `staff-ux-engineer`, and inherits the agent-construction rules established here.
...
→ `~/.claude/agents/` (user) → plugin. A project copy therefore overrides a generic user-level one and
gets reviewed in PRs like any other file.

⚠️ **Correction (2026-09-07): our agents do NOT currently take that path.** `staff-architect` and
`staff-ux-engineer` both live at `~/.claude/agents/` (user scope); this repo's `.claude/agents/` is
empty. So neither is reviewed in a PR here, neither has version history, and both are available in
every project rather than overriding anything. An earlier version of this paragraph asserted the
opposite. The consequences are recorded rather than hidden: the portable half of each agent's
rationale now travels beside the agent (`~/.claude/agents/<name>.rationale.md`), and only
project-specific decisions live here. Making `~/.claude` a repository would restore history and
review; that decision is open.
