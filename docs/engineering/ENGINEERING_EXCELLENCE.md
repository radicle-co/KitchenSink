...
- **YAGNI — "You Aren't Gonna Need It," read precisely (Fowler).** It applies **only** to _presumptive features_ — capability built now for a future need you're guessing at (speculative abstractions, config knobs, single-caller "frameworks"). You avoid them because of their real cost: **build + delay + carry + repair**. It **does NOT apply** to effort that makes code easier to change: _"Yagni is not a justification for neglecting the health of your code base. Yagni requires (and enables) malleable code."_ **YAGNI/KISS is never an excuse to skip** correctness/robustness of the _current_ requirement (error paths, edge cases, validation, authz, concurrency), tests (self-testing code _enables_ YAGNI), good structure, a _known_ requirement, or a cheap seam where reversing later is expensive (public APIs, schemas/migrations, security boundaries, wire contracts — the cost asymmetry weakens YAGNI). The check: "speculative future I'm guessing at?" (decline) vs "skipping today's correctness/tests/structure?" (that's corner-cutting, not YAGNI).
- **KISS.** Build the simplest thing that _fully and correctly_ solves the problem in front of you — simple ≠ incomplete; cleverness is a loan against future comprehension.
