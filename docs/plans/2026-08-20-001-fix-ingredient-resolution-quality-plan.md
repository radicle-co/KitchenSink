...
> ⚠️ **Superseded as a description of current state** by [`docs/architecture/2026-08-28/ingredientPipelineState.md`](../architecture/2026-08-28/ingredientPipelineState.md) (2026-08-28, PR 91).
>
...
`deployGate.sh`'s pure `deploy_gate_decide` against its impure `deploy_gate_evaluate` — and it is what makes
...
`packages/infra/global/__tests__/engineVersionDiff.test.ts` (new), `docs/runbooks/pg18Upgrade.md` (new),
`compose.yml`, `compose.test.yml`, `.github/workflows/_ci.yml`,
...
`infra/localstack/compose.yml` and `packages/services/identity/infra/docker/compose.yml`.
...
`docs/runbooks/pg18Upgrade.md` MUST carry a rehearsed restore leg: restore to a new instance identifier,
...
measurements in `docs/reports/2026-08-23/ingredientParseModelComparison.md`. Units cite them.
...
`docs/mockups/briefs/recipeIngredientEntryFigmaMakePrompt.md`.
...
7. **Retrieval, not parsing, was the dominant failure** in `docs/reports/2026-08-22/ingredientResolutionMeasurement.md`,
...
`sandboxWake.sh ensure` already exists and is called from both deploy workflows — never from `_ci.yml`.
...
**Files.** Modify `.github/workflows/_ci.yml`. ⚠️ Reuses `.github/scripts/sandboxWake.sh ensure` — do NOT
