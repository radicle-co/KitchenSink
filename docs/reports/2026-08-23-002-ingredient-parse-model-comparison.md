...
## 16. The FOURTH arm — the drain WITHOUT the deletion (2026-08-26)
...
`butter`, `brandy` and `flour`. **It does not ship.** §16.9 grades it criterion by criterion; §16.6 is the
mechanism.
...
§16.8 has the separation.

### 16.1 ⚠️ Why all four arms were re-measured rather than three cited
...
2. ⛔ **A second sentence was added that §15.11 does not contain**: _"Every entry in foods must name a food"_.
   §15.11's draft carried only the clause. **This addition is the arm's undoing** — see §16.6.
...
### 16.5 ⛔ Nameless entries — v4 fixes the defect it was written to fix, completely
...
### 16.6 ⛔⛔ THE DEFECT v4 INTRODUCED — it withholds real ingredients
...
⚠️ **v4 is worse than v2 on `names`, and v4 is v2 plus a sentence v2 deleted.** That is the result §16.8 exists
to unpick.

### 16.8 ⛔ §15.7 IS FALSIFIED — the deleted sentence was not the cost
...
### 16.9 ⛔ Does v4 clear the bar? — criterion by criterion
...
### 16.11 ⛔ THE RECOMMENDATION — do NOT ship v4; run a fifth arm, and it is a ONE-SENTENCE deletion
...
- **It is cheap and it is decisive.** ~$0.03, and it separates v4's two additions by attribution rather than
  by population — which §16.8 could only bound.
- ⚠️ **It should be run beside a re-measured v1 and v4**, for §16.1's reason. Budget ~$0.10.

⛔ **What a fifth arm CANNOT fix, and what the owner should decide before spending anything.** §16.8 shows the
...
### 16.12 ⛔ What is NOT measured here

- **One book, one model, one temperature.** Nova Micro only, `temperature: 0`, `maxTokens: 200`. Nothing here
  says how Haiku 4.5 or Nova Pro respond to any of these wordings.
- **WHY a drain slot costs `names` agreement.** §16.8 locates the cost; it does not explain it, and the CRF is
  not ground truth.
...
- **v4's two additions are bounded, not attributed.** §16.8 separates them by population. Only a fifth arm
  attributes them.
