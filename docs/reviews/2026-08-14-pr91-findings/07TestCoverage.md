...
- **Gap:** `KNOWN_UNRUN_FLOWS = ['homeRecentRecipeTap', 'recipes/sourceTabs']` — two story flows that exist
  under `packages/apps/commise/mobile/.maestro/` and are in no vertical of `FLOW_PLAN`, so the emulator never
  runs them. `recipes/sourceTabs` is the mobile half of a round trip whose web pair
...
- **Verified (how):** Read `maestro-flow-selection.test.ts:227-245,390-425`; listed the 33 flow YAMLs; read
  the `FLOW_PLAN` construction in `packages/apps/commise/mobile/tests/e2e/runMaestroFlows.sh`.
...
- **Smallest fix:** Add `--coverage` + a per-package `thresholds` floor to the `Test` CI job for the four
  ...
  blocker (the `boundariesBaseline.json` pattern at `_ci.yml:169-184` is the in-repo precedent).
