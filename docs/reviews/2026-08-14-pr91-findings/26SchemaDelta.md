...
**Builds on** (not repeated here): `09DataModel.md` F-DB1–F-DB16 and D-1–D-10; `12AdversarialStatusShells.md`
A-3/A-4/A-8; `15AdversarialFoodRecipeModel.md` A-1/A-4/A-5; and `16AdversarialLiveReference.md` A-1–A-6,
...
`nutrition_compliance` and every other 009 table. Verified absent by the search recorded in `09DataModel.md`
...
`15AdversarialFoodRecipeModel.md` A-1 recommended _"Match nutrients by `external_code` first"_. **That is not
...
`meal_plan_id` is an opaque un-FK'd identifier. `09DataModel.md` F-DB14 owns that correction; the carve-out above
...
**Recipe side.** `ingredients_backing_coherent` (`09DataModel.md` F-DB3) — unchanged, adopted verbatim, and it is
...
`12AdversarialStatusShells.md` A-8 recommended a reaper that _"deletes"_ a stale shell, and
`09DataModel.md` D-7 recommended a sweep. **I am reversing the delete half**, and the reason is D-LIVE itself.
...
1. **Each migration file is applied inside an explicit transaction** — `client.query('BEGIN')`, the whole file as
   ...
   `food-service/src/lambdas/migrate/handler.ts:121-126`). Therefore:
    - **`CREATE INDEX CONCURRENTLY` cannot be used.** Every `CONCURRENTLY` recommendation in `09DataModel.md`
...
`09DataModel.md`'s "Not examined" flagged that adding these constraints **may break seeds**, and that is still
unchecked.
...
`09DataModel.md` F-DB12 and F-DB15 recommended two `DROP INDEX`es on reasoning alone and said so; this section is
...
- **`database/seed.ts` and the `__fixtures__` factories** were not audited against the new constraints. `09DataModel.md`
