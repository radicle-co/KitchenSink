...
3. **Lambda, which is a separate bill.** 100 M records at `batchSize: 100` ≈ 1 M invocations ≈ $0.20 in
   ...
   [`28ResearchMessagingAws.md`](28ResearchMessagingAws.md).
...
```ts
fn.addEventSource(new DynamoEventSource(table, {/* … */})); // grants stream read automatically
...
`.github/scripts/teardownSandboxPr.sh` reclaims it; a persistent table must be `RETAIN` + `Environment=global`
...
- [17MessageSubstrate.md](17MessageSubstrate.md) — the earlier, superseded requirement framing
- [28ResearchMessagingAws.md](28ResearchMessagingAws.md) — the cross-technology comparison that selected DynamoDB
