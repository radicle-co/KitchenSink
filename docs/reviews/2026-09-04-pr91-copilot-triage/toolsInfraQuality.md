...
`auth/provisionUsers.mjs:50`, `auth/grantAdmin.mjs:166`, `observe/collectMetrics.mjs:60` and `:155`,
...
or delete test users, and write the resulting metrics series back to disk (`collectMetrics.mjs` uses a
...
- `packages/services/recipe-service` → **13 errors** (`esbuild.mjs`, `tests/load/foodNutritionStub.mjs`,
  `tests/load/prepareDb.mjs`: `console`, `process`, `Buffer`, `URL`, `setTimeout` `no-undef`, plus **one
...
2. A `run:` step invoking a script whose filename contains the word, e.g. `scripts/fetchSecrets.mjs`
   (`packages/apps/commise/{web,mobile}/scripts/fetchSecrets.mjs` both exist). The `-` is a word boundary,
   so `\b` does not help.
