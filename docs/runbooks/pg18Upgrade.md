...
(`compose.yml`, `compose.test.yml`, `packages/services/identity/infra/docker/compose.yml`);
...
`teardownSandboxPr.sh` §1 hardcoded food's migration-runner output, so `kitchensink_food_pr_{N}` was
