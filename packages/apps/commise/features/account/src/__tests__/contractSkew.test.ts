/**
 * Unit tests for @commise/features-account's DRIFT LAYER 3 (Skew) comparison — `docs/CODING_STANDARDS.md` §15.2.5.
 *
...
const BASE = 'http://identity.example.test';
const OTHER_BASE = 'http://identity-two.example.test';
...
describe('checkContractSkew (the probe)', () => {
    it('warns once when the service serves a different fingerprint', async () => {
        const warn = vi.fn();
        const fetchImpl = stubFetch(200, { status: 'ok', service: 'identity', contractHash: OTHER_HASH });
    ...
    it('names both fingerprints, which side is which, and the remedy', async () => {
        ...
        expect(message).toContain('@kitchensink/schema-identity');
    ...
    it('stays silent when the fingerprints agree', async () => {
        const warn = vi.fn();
        const fetchImpl = stubFetch(200, { status: 'ok', service: 'identity', contractHash: CONTRACT_HASH });
    ...
    it('stays silent when the service publishes no fingerprint (an older deployed service)', async () => {
        const warn = vi.fn();
        const fetchImpl = stubFetch(200, { status: 'ok', service: 'identity' });
    ...
    it('stays silent, and never rejects, when the transport fails outright', async () => {
        const warn = vi.fn();
        const fetchImpl = vi.fn().mockRejectedValue(new Error('ENOTFOUND identity.test')) as FetchDouble;
