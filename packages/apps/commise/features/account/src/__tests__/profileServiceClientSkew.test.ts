...
describe('ProfileServiceClient — contract-skew reporting', () => {
    ...
    it('sends no Authorization header on the probe, even when the client holds a token', async () => {
        ...
        await vi.waitFor(() => {
            ...
            expect(calls.filter(([url]) => new Request(url).url.endsWith('/health'))).toHaveLength(1);
        });

        const calls = (fetchImpl as unknown as ReturnType<typeof vi.fn>).mock.calls;
        const [, probeInit] = calls.find(([url]) => new Request(url).url.endsWith('/health')) as [unknown, RequestInit];
        const [, apiInit] = calls.find(([url]) => !new Request(url).url.endsWith('/health')) as [unknown, RequestInit];
