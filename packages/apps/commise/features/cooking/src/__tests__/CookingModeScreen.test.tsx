/**
 * Component tests for the WEB {@link CookingModeScreen} — the single orchestrational component of
 * Cooking Mode (FR-032, FR-032a, FR-033, FR-034, FR-034a, FR-035).
 *
 * The leaves are proven in their own suites; what is proven HERE is the wiring between them and the
 * session: that the screen SELECTS the right step surface, that an intent raised by a leaf reaches the
 * right pure reducer, that the injected wake-lock port is acquired and released with the session, that a
 * completion alert reaches the banner exactly once, and that nothing on this screen can write a recipe.
 *
 * Every query is by role or accessible name, and every assertion names a specific string or a specific
 * call count — an assertion that would still pass with the wiring crossed proves nothing.
 */
...
/** The instruction currently on screen, read through the step region's accessible name. */
const shownInstruction = (): string =>
    screen.getByRole('region', { name: 'Current step instruction' }).textContent ?? '';

describe('CookingModeScreen (web) — surface selection', () => {
    it('mounts at the FIRST step, inside a named Cooking Mode region', async () => {
        renderScreen();
        await settle();

        expect(screen.getByRole('region', { name: 'Cooking mode' })).toBeTruthy();
    ...
    it('selects the ERROR surface, and its retry reports upward without fetching anything itself', async () => {
        const onRetry = vi.fn();
...
describe('CookingModeScreen (web) — navigation wiring (FR-033)', () => {
    ...
    it('does not move on the first step, where the previous zone is marked unavailable', async () => {
        renderScreen();
        await settle();
...
describe('CookingModeScreen (web) — screen wake lock (FR-035)', () => {
...
describe('CookingModeScreen (web) — session resume (FR-033 / REQ-013)', () => {
...
describe('CookingModeScreen (web) — timers (FR-034)', () => {
    ...
    it('does NOT start a second countdown for the same step, and does not surface an error', async () => {
        ...
        // The original countdown survived: a silent replace would read 1:00 again.
...
describe('CookingModeScreen (web) — the completion alert fires exactly ONCE (REQ-006)', () => {
    it('announces the finished timer, then never announces it again however long the cook waits', async () => {
        ...
        // Acknowledged, not cancelled — the timer is still listed at 0:00.
...
describe('CookingModeScreen (web) — ingredient checkoff and yield scaling (FR-032a, FR-034a)', () => {
    it('opens the checklist, round-trips a checkoff, and closes without leaving the step', async () => {
        ...
        expect(screen.getByRole('status', { name: '0 of 2 checked' })).toBeTruthy();
        ...
        expect(screen.getByRole('status', { name: '1 of 2 checked' })).toBeTruthy();
        ...
        expect(screen.queryByRole('region', { name: 'Ingredient checklist' })).toBeNull();
        // The step behind the panel was never disturbed.
        expect(screen.getByRole('heading', { name: 'Step 1 of 3' })).toBeTruthy();
    });

    it('scales the DISPLAYED quantities and raises the advisory, leaving cook times alone (D-002)', async () => {
        vi.useFakeTimers();
        vi.setSystemTime(new Date('2026-08-09T12:00:00.000Z'));
        ...
        // The countdown is untouched by the yield change.
        expect(screen.getByRole('timer', { name: 'Sweat the onion' }).textContent).toBe('1:00');
    });
});

describe('CookingModeScreen (web) — voice control (US-006 / D-004)', () => {
    ...
    it('re-announces the current step ASSERTIVELY on "repeat", and announces nothing before it is asked', async () => {
        ...
        // The step itself did not move, and the announcement is offscreen rather than duplicated on it.
        expect(screen.getByRole('heading', { name: 'Step 1 of 3' })).toBeTruthy();
    });
...
describe('CookingModeScreen (web) — Cooking Mode is READ-ONLY on the recipe (REQ-CN-001)', () => {
