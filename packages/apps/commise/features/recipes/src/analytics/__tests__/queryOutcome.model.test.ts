 ...
 * served list); a WHOLESALE RETYPE is an ABANDONMENT of the first phrasing (owner ruling 2026-09-01,
 * REVIEW F2) — the old session settles as a no-pick and a NEW session (new id) begins; an outcome
 * otherwise emits exactly once, at pick, clear-to-empty, non-suggestion resolve, or unmount. This
 * suite drives those transitions as a table over the pure model — the hook wires them, the model
 * decides them.
 */
...
describe('observeServedList (KTD6 — begin/continue/replace)', () => {
    it('begins a session when a non-empty list settles, and an empty list begins NOTHING', () => {
        expect(observeServedList(null, 'salt', [], mintId)).toEqual({ session: null, abandoned: null });

        const { session, abandoned } = observeServedList(null, 'salt', TWO_SECTION_LIST, mintId);
    ...
    it('CONTINUES the session on EXTENSION — same event id, updated query and list, nothing abandoned', () => {
        const first = observeServedList(null, 'sal', TWO_SECTION_LIST, mintId);
        const second = observeServedList(first.session, 'salt', TWO_SECTION_LIST.slice(0, 2), mintId);

        expect(second.abandoned).toBeNull();
        expect(second.session?.eventId).toBe(first.session?.eventId);
        expect(second.session?.query).toBe('salt');
        expect(second.session?.served).toHaveLength(2);
    });

    it('CONTINUES the session on backspace-refinement — the old text starts with the new', () => {
        const first = observeServedList(null, 'butter', TWO_SECTION_LIST, mintId);
        const second = observeServedList(first.session, 'butt', TWO_SECTION_LIST, mintId);

        expect(second.abandoned).toBeNull();
        expect(second.session?.eventId).toBe(first.session?.eventId);
    });

    it('⛔ a WHOLESALE RETYPE ABANDONS the session (owner ruling, REVIEW F2): old no-pick, NEW id', () => {
        const first = observeServedList(null, 'butter', TWO_SECTION_LIST, mintId);
        const second = observeServedList(first.session, 'sugar', TWO_SECTION_LIST.slice(0, 2), mintId);

        // The abandoned event carries the FIRST phrasing's query + served list, as a no-pick.
        expect(second.abandoned).not.toBeNull();
        expect(second.abandoned?.query).toBe('butter');
        expect(second.abandoned?.served).toHaveLength(4);
        expect(second.abandoned?.outcome).toEqual({ kind: 'no_pick' });
        expect(second.abandoned?.eventId).toBe(first.session?.eventId);

        // And the replacement is a NEW logical event — new id, new query, new list.
        expect(second.session?.eventId).not.toBe(first.session?.eventId);
        expect(second.session?.query).toBe('sugar');
        expect(second.session?.served).toHaveLength(2);
    });

    it('a retype that settles an EMPTY list still abandons the old session, and begins nothing', () => {
        const first = observeServedList(null, 'butter', TWO_SECTION_LIST, mintId);
        const second = observeServedList(first.session, 'sugar', [], mintId);
...
describe('abandonOutcome (AE2)', () => {
    it('emits ONE no-pick carrying the FINAL settled query and served list', () => {
        ...
        const final = observeServedList(first.session, 'buckwheat honey', TWO_SECTION_LIST.slice(0, 1), mintId);
        const event = abandonOutcome(final.session);
...
describe('⛔ the keystroke table (KTD6): "b → bu (pause) → buck (pause) → pick" is ONE event, zero no-picks', () => {
    it('holds', () => {
        ...
        const begun = observeServedList(session, 'bu', TWO_SECTION_LIST, mintId);
        expect(begun.abandoned).toBeNull();
        session = begun.session;
        // "buck" settles a refined list → session CONTINUES (no abandonment of the "bu" prefix).
        const continued = observeServedList(session, 'buck', TWO_SECTION_LIST.slice(0, 3), mintId);
        expect(continued.abandoned).toBeNull();
...
describe('id minting (KTD5)', () => {
    it('the event id is STABLE across the session — a retry of the same logical event reuses it', () => {
        ...
        const event = pickOutcome(continued.session, TWO_SECTION_LIST[0] as IngredientSuggestion);
    ...
    it('two DIFFERENT logical events mint DIFFERENT ids', () => {
        ...
        expect(first.session?.eventId).not.toBe(second.session?.eventId);
    });
});
