...
/**
 * ⛔ THE TITLE BELONGS IN THE REMOVE CONTROL'S NAME, NEVER IN ITS VISIBLE TEXT. Remove is `shrink-0` beside a
 * `flex-1` title, so a visible "Remove <long title>" takes its full width and squeezes the title to its narrowest
 * possible width. The native leaf did exactly that on device (Maestro run 34863424203, `collectionsPull`). The
 * visible text is the short verb; the accessible name keeps the title, which also satisfies WCAG 2.5.3 because the
 * visible label starts the name.
 */
describe('CollectionMemberRow (web) — Remove never repeats the title on screen', () => {
