    ...
    return (
        <View accessibilityLabel={recipe.title} style={styles.container}>
                ...
                {scaled.ingredients.map((ingredient) => {
                    ...
                    return (
                        <View key={ingredient.ingredientId} style={styles.ingredientRow}>
                            ...
                            {/* The food's author withdrew it. WARNING tint like needs-review — the cook can
                                act (re-match the line in the editor), unlike the neutral badge above, which
                                is somebody else's privacy and not actionable. Suppressed when every line is
                                removed: a badge on every row is wallpaper, and the tile already says it. */}
                            {isLineFoodRemoved(ingredient) && !allLinesFoodRemoved(recipe.ingredients) && (
                                <Text style={styles.needsReviewBadge}>{detail.removedFoodBadge}</Text>
                            )}
                        </View>
                    );
                })}
            ...
            {/* U14 — see the web leaf: a THIRD admission, and the only one that is our own doubt rather than a
                ...
                The half of this note that USED to say RN "maps it onto the platform role on device exactly
                as `accessibilityRole` would" was FALSE, and reading the RN sources settled it: Android's
                `fromRole` has no `Role.NOTE` case and iOS has no `note` trait, so the prop is inert on both
                platforms. Keep it — it states intent and costs nothing — but do not build on the claim.
                Still do not "fix" it into `accessibilityRole`, which does not type-check. */}
...
const styles = StyleSheet.create({
    ...
    // tone change to one silently change the other — the reason `needsReviewBadge` is not `userBadge`.
    removedFoodNotice: {
        backgroundColor: tint(palette.warning, 0.25),
        borderRadius: nativeTokens.radius.lg,
        paddingHorizontal: nativeTokens.spacing[3],
