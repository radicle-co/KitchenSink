            ...
            <header>
                <GradientSurface gradient="hero" className="flex flex-col gap-4 rounded-2xl p-6">
                    <h1
                        ref={titleRef}
                        tabIndex={-1}
                        className="font-display text-2xl font-bold leading-tight text-charcoal sm:text-4xl"
                    >
                        {recipe.title}
                    </h1>
                    ...
                    {scaled.ingredients.map((ingredient) => {
                        ...
                        return (
                            <li key={ingredient.ingredientId} className="flex items-center gap-3 px-3 py-3">
                                ...
                                {/* The removed-food badge, WARNING tint like needs-review — a cook can act
                                    on it (re-match the line in the editor), unlike the neutral
                                    "details unavailable" above, which is somebody else's privacy and not
                                    actionable at all.

                                    ⛔ SUPPRESSED when every line is removed: a badge on every row is
                                    wallpaper, not signal, and the tile above already says so in words. */}
                                {isLineFoodRemoved(ingredient) && !allLinesFoodRemoved(recipe.ingredients) && (
                                    <span className="ml-auto shrink-0 rounded-full bg-warning/25 px-2 py-0.5 text-caption font-medium text-charcoal">
                                        {detail.removedFoodBadge}
                                    </span>
                                )}
                            </li>
                        );
