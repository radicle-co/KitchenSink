...
/**
 * U13 — the WEB half of the batched AMBIGUITY REVIEW surface: entry, disclosure, per-row fresh shortlist,
 * pick → one correction, per-row failure isolation, the sibling-binding caption, the unavailable badge,
 * and the one-time clone banner. Every state, absence cases included.
 *
 * The native mirror is `ambiguityReviewSurface.native.test.tsx`; the two assert the same set (§14).
 * Client hooks are mocked at the module seam (the resolver-test harness style) — the pure models and the
 * correction controller run for real.
 */
...
describe('AmbiguityReview (web) — every state', () => {
     ...
     * refuses the second press — which is why the proof is `mutate` staying at zero, not the attribute alone.
     */
    it('⛔ a pick in flight busies the candidates without natively disabling them, and refuses a second write', () => {
        const mutate = vi.fn();

        useRecordIngredientCorrectionMock.mockReturnValue(correctionMutation({ mutate, isPending: true }));
        render(<AmbiguityReview ingredients={[ambiguousLine('apple sauce')]} />);

        fireEvent.click(screen.getByRole('button', { name: en.ambiguousReviewToggle }));
        const candidate = screen.getByRole('button', { name: 'Apple sauce, canned' });
