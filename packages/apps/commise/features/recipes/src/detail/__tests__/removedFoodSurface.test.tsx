/**
 * The WITHDRAWN-FOOD surface on the web recipe detail (owner rulings 3 + 4, 2026-09-07).
 *
...
describe('RecipeDetailView (web) — one ingredient whose food was withdrawn', () => {
    const oneRemoved = () =>
        makeRecipeDetail({
    ...
    it('⛔ sits INSIDE the Ingredients section, not above the recipe', () => {
        render(<RecipeDetailView recipe={oneRemoved()} />);

        const section = screen.getByLabelText(en.ingredientsHeading);

        expect(section.contains(screen.getByRole('note'))).toBe(true);
    });
});

describe('RecipeDetailView (web) — several removed, but not all', () => {
    const someRemoved = () =>
        makeRecipeDetail({
            ingredients: [
...
describe('RecipeDetailView (web) — every ingredient removed', () => {
...
describe('RecipeDetailView (web) — the states that must stay silent', () => {
