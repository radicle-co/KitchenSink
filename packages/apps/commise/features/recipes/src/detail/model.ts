 ...
 * says we had it and its author took it away: permanent, true for everyone, and the line still knows its
 * own name because that name belongs to the RECIPE.
 *
 * @param line - One recipe ingredient line as the detail read returns it.
 ...
 * together. Confirmed at the data layer rather than assumed: `is_user_entered` marks a FREEFORM
 * `ingredients` row (migration 0006's unique index is scoped to it "so food-backed rows may share a
 * name"), and a freeform row carries no `food_id` — so the live-status lookup has no entry for it and
 * `foodPresenceStatus` can never reach `FOOD_REMOVED`. Recorded here because the layout consequence is
 * real: that pair would be the widest possible ingredient row, and it is unreachable rather than untested.
 */
 ...
 *
 * ⛔ It names `line.name` — the RECIPE'S line — and never the food record. A public recipe can be read by
 * someone with no entitlement to the catalog row, which is precisely what `RESOLVED_UNAVAILABLE` exists to
 * protect; reaching for the food's name here would leak across that boundary on the one surface built to
 * respect it.
 *
 * @param ingredients - The recipe's ingredient lines.
 * @param notices - The localized copy for the active locale.
...
export const removedFoodNotice = (
    ...
    if (removed.length === 1) {
        return fillTemplate(notices.removedFoodNoticeOne, { name: removed[0]?.name ?? '' });
    }
