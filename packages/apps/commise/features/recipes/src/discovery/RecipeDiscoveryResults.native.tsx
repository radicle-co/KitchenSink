...
export const RecipeDiscoveryResults: FC<RecipeDiscoveryResultsProps> = ({
    ...
    const columns = recipeGridColumns(width, { phone: 2, tablet: 3 });
    ...
    if (browsing) {
        ...
        // FlashList (U4): the wrapping grid (U7), and v2 auto-measures — no `estimatedItemSize`.
        // ⛔ WIDTH-DERIVED, not the pinned `2` this shipped with. `app.json` declares `supportsTablet: true`,
        // so at 820–1024dp two columns meant ~500dp cards — a phone layout stretched, not a tablet one.
        body = (
            <FlashList
                role="list"
                data={results}
