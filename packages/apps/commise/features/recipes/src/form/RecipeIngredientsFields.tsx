...
import { lineCalories, recipeNutritionTotal } from './nutrition.js';
            ...
            <li key={index} className="flex flex-wrap items-center gap-2">
                ...
                {line.resolutionStatus !== undefined && (
                    <span
                        ...
                        // The two statuses a cook can ACT ON, in the one place they can act: the gate's
                        // contradiction (U14 — re-pick the food) and a food its author withdrew. Wearing
                        // the same neutral pearl as "Resolved" would put those affordances in front of
                        // them in the colour of "nothing to do here".
                        //
                        // ⛔ `FOOD_REMOVED` belongs here, and its absence was an incoherence a UX audit
                        // caught: the recipe DETAIL leaf justifies its warning tint on the grounds that
                        // "a cook can act on it — re-match the line in the editor", and the editor then
                        // painted the same status neutral. The alarm fired where nothing could be done and
                        // went quiet where the picker actually lives.
                        //
                        // ⛔ CHARCOAL on a `warning` TINT, never `warning` as the text colour:
                        // `@commise/ui`'s palette JSDoc is explicit that #F5B041 is a light FILL that
