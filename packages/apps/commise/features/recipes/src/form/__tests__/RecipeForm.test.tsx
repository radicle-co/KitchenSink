...
describe('RecipeForm (web) — submit + cancel', () => {
    ...
    it('busies the submit button and does not submit while submitting', async () => {
        const user = userEvent.setup();
        const onSubmit = vi.fn();
    ...
    it('⛔ does not submit a second time when Enter is pressed in a field while submitting', async () => {
        // Implicit submission: Enter in a field clicks the form's default button. The busy submit button is not
        // natively `disabled`, so it cancels the click instead, and this is the case that proves it.
        const user = userEvent.setup();
        const onSubmit = vi.fn();
        renderForm({ submitting: true, onSubmit });
