...
describe('RecipeForm (web) — B8 error accessibility wiring (aria-invalid + aria-describedby)', () => {
    ...
    // ⚠️ The offending input CHANGED with the 2026-09-12 ruling, not the wiring this test covers. A typed
    // `0` used to be the simplest invalid quantity; it is now normalised to "no amount" and is submittable,
    // so the line is driven by the one cause that survives — a bound outside the wire's storage range.
    it('wires only the offending QUANTITY lines to an ingredientsQuantityInvalid alert (WCAG 3.3.1)', () => {
        renderForm({
            values: filledValues({
                ingredients: [
                    { ingredientId: 'ing_1', name: 'Salt', quantity: 2_000_000 },
...
describe('RecipeForm (web) — ranged quantity (U9/R42)', () => {
    ...
    // ⚠️ REVERSED INPUT, same guarantee. An inverted range used to be the canonical incoherent pair; the
    // 2026-09-12 ruling SWAPS it instead, so the case that still marks both bounds is a range whose upper
    ...
    it('marks BOTH bounds invalid and wires them to the alert when a bound is out of range (WCAG 3.3.1)', () => {
        renderForm({
            values: filledValues({
                ingredients: [{ ingredientId: 'ing_1', name: 'Flour', quantity: 2, quantityHigh: 2_000_000 }],
            }),
            errors: { ingredients: 'ingredientsQuantityInvalid' },
        });
        ...
        renderForm({
            values: filledValues({
                ingredients: [
                    ...
                    { ingredientId: 'ing_2', name: 'Flour', quantity: 2, quantityHigh: 2_000_000 },
                ],
            }),
            errors: { ingredients: 'ingredientsQuantityInvalid' },
        });
