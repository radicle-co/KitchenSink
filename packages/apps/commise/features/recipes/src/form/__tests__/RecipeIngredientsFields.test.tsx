...
describe('RecipeIngredientsFields (web) — the states', () => {
    ...
    it.each([
        ...
        renderLeaf({ values: valuesWith([{ ...RESOLVED, resolutionStatus: status }]) });

        const badge = screen.getByLabelText('Ingredient 1 status');
    ...
    it('STATUS: leaves a NON-actionable status neutral, so the warning tint keeps meaning something', () => {
        renderLeaf({
            values: valuesWith([{ ...RESOLVED, resolutionStatus: FoodResolutionStatus.RESOLVED_UNAVAILABLE }]),
        });
