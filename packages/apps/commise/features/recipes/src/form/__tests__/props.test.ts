...
import type * as PropsModule from '../props.js';
import type * as PackageSurface from '../../index.js';
import {
    applyDraftAction,
    addChip,
    difficultyOptions,
...
describe('U28 — appendResolvedIngredient inherits the section being built', () => {
     ...
     * ship it.
     *
     * ⚠️ There is no `form/index.js` barrel (`CODING_STANDARDS §4`: a barrel is permitted only as a package
     * export), so the package barrel below is the only aggregate surface these symbols could reach, and
     * `props.js` is the only module that could declare them.
     */
    it('⛔ neither addIngredient nor blankIngredient is exported any more', () => {
        // ⛔ REWRITTEN as a COMPILE-TIME assertion over type-only namespace imports. A runtime `await import()`
        // of the package barrel pulls the whole package graph (`@commise/ui` included) into the worker (~2.8s
        // under a full `turbo run test`, flaking without a 30s budget) for a question the type system answers
        // exactly: a value re-exported from either surface is a member of that module's namespace type. `tsc`
        // (this package's `typecheck`, which includes `src/**`) fails the build if either symbol reappears on
        // either surface.
        expectTypeOf<typeof PropsModule>().not.toHaveProperty('addIngredient');
        expectTypeOf<typeof PropsModule>().not.toHaveProperty('blankIngredient');
        expectTypeOf<typeof PackageSurface>().not.toHaveProperty('addIngredient');
        expectTypeOf<typeof PackageSurface>().not.toHaveProperty('blankIngredient');
    });
});
...
describe('applyDraftAction — one entry point, every draft transition', () => {
    ...
    it('writes the LOWER bound through the ingredient patch — where NaN is the absent sentinel', () => {
        // ⚠️ The LOWER bound only, and named so. A "low and high alike" title over a low-only assertion makes
        // a suite look complete while covering half of it — and the skipped half is the one that could lose a
        // cook's amount. The upper bound is asserted separately below, because it is a
