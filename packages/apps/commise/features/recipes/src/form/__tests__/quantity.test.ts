 ...
 *
 * An inverted pair and a high-without-low are repaired rather than refused, so the cook's numbers survive.
 *
 * ⚠️ Why the rule exists at all: two functions reading the raw pair can DISAGREE on a non-finite upper
 * bound. A gate that filters the bad high away sees a lone lower bound and answers `stated`; a reading that
 * hands the raw pair on answers `absent`. So `{quantity: 2, quantityHigh: NaN}` would pass validation and
 * reach the wire as "no amount" — the cook's 2 gone, with no error anywhere. Both functions read through
 * ONE normaliser, so that class of divergence is unrepresentable.
 *
 ...
 *
 * ⚠️ CONSEQUENCE WORTH KNOWING: after normalisation the pair cannot contradict itself, so the `invalid`
 * verdict — and the `quantityInvalid` error the cook sees — means ONLY "outside the wire's allowed range"
 * (below `MIN_RECIPE_INGREDIENT_QUANTITY`, or above the max). It never means "these two numbers disagree",
 * because that state is repaired instead of reported.
 */
...
describe('draftQuantityVerdict (U9)', () => {
    ...
    it('reads a stated zero or negative as ABSENT — REVERSED by the 2026-09-12 owner ruling', () => {
        // ⚠️ "Not a number and not a valid amount should be treated the same" (owner ruling): a typed `0` is
        // normalised away like `NaN`, so the line states no amount and is submittable. The INPUT class needs
        // an owner: without this, nothing pins what a typed zero does. The `invalid` verdict belongs to the
        // wire-bounds case alone, asserted below.
    ...
    it('rejects a bound the storage column cannot hold, on EITHER side of the range', () => {
        ...
        // written against a scalar quantity would miss entirely.
