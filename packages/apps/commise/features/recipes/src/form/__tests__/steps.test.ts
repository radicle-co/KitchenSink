...
describe('stepErrorsFor / canAdvanceFromStep (W3 — the wizard field->step map)', () => {
    ...
    // REWRITTEN for the U33 step model: step 4 is REVIEW, and it carries no errors not because photos are
    // decoupled from validation but because Review owns no field at all. The gate a cook meets on Review is
    // `Publish`'s WHOLE-form `validateRecipeForm`, which is a different question from "may I leave this step".
