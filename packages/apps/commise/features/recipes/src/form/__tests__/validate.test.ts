...
describe('validateRecipeForm', () => {
     ...
     * REWRITTEN for U9.
     *
     * Resolution and quantity are two different failures with separate codes and separate copy: under U9 an
     * absent quantity is VALID, so "every ingredient needs a resolved item and a quantity greater than zero"
     * is not a true sentence. The assertion below proves the quantity failure gets the quantity code.
     */
    // ⚠️ Per the 2026-09-12 owner ruling the draft's bounds are NORMALISED before they are read ("not a
    // number and not a valid amount should be treated the same"), so a typed zero states no amount rather
    // than an invalid one — and a line that states no amount is submittable (R40). The
    // `ingredientsQuantityInvalid` code has exactly one cause, the wire-BOUNDS case asserted below, and
    // `form/quantity.ts` carries the argument.
    ...
    // ⚠️ Per the same ruling an inverted pair is SWAPPED rather than refused, so the cook's
    ...
    // ⚠️ Per the same ruling a lone upper bound is PROMOTED to the amount ("if low is not a
