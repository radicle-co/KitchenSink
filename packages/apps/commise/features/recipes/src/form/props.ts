 ...
 *
 * DESIGN PATTERN: the variant half of a Visitor — {@link applyDraftAction}'s exhaustive switch is the other
 * half, so a new member is a COMPILE error until it is handled rather than a silently ignored edit.
 */
export type DraftAction =
    | { readonly kind: 'appendResolvedIngredient'; readonly line: ResolvedRecipeFormIngredient }
    | { readonly kind: 'removeAt'; readonly field: DraftListField; readonly index: number }
    | { readonly kind: 'updateIngredientAt'; readonly index: number; readonly patch: Partial<RecipeFormIngredient> }
...
export const applyDraftAction = (values: RecipeFormValues, action: DraftAction): RecipeFormValues => {
    switch (action.kind) {
        case 'appendResolvedIngredient':
            return appendResolvedIngredient(values, action.line);
        case 'removeAt':
        ...
        case 'setIngredientQuantityHigh':
            ...
            // amount at all. Measured end-to-end through `toCreateRecipeInput`: a line reading `2 tbsp` whose
            // range had been typed and then cleared went to the wire as `{ kind: 'absent' }`, with
            // `draftQuantityVerdict` still reporting `stated`, so nothing flagged it. That is the "looks
 ...
 */
const appendResolvedIngredient = (values: RecipeFormValues, line: ResolvedRecipeFormIngredient): RecipeFormValues => {
...
export const resolutionStatusLabel = (messages: RecipeFormMessages, status: FoodResolutionStatus): string => {
    switch (status) {
        ...
        case 'FOOD_REMOVED':
            ...
            // food is gone for everyone. The line keeps its own name, amount and unit — those are the
            // recipe's, never the food's — so there is nothing broken to fix, only nutrition that no longer
            // counts.
            return messages.statusFoodRemoved;
    }
};
