 ...
 * to `{low: 3}`.
 *
 * ⛔ THIS EXISTS TO MAKE A DIVERGENCE UNREPRESENTABLE. If `draftQuantityVerdict` filtered non-finite bounds
 * itself while {@link draftQuantity} passed the raw pair to `statedQuantity`, the two would answer different
 * questions about the same line: for `{quantity: 2, quantityHigh: NaN}` the gate would say `stated` and the
 * reading `absent`, so validation would let the save through and the wire mapper send "no amount" — the
 * cook's 2 gone, silently, with `draftToSnapshot` writing the same absence into the conflict snapshot. Both
 * functions read the pair ONLY through here.
 *
 * ⚠️ Repairing rather than refusing is the ruling's substance: an inverted pair and a high-with-no-low are
 * corrected, not reported to the cook as errors. What survives as `invalid` is exactly the wire
 ...
 * which is correct as a reading, and useless as a gate. Without this function the validator cannot tell
 * "the source stated no amount" from "the user is half-way through typing", so it refuses BOTH, and an
 * absent-quantity recipe can be opened in the editor and never saved.
 *
 ...
 * here instead of round-tripping to a 400. A validator written against a scalar quantity would check only
 * the lower one.
 *
 * @param line - The form's ingredient line.
