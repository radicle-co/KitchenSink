 ...
 *
 * ⚠️ THE MAPPERS TARGET `@kitchensink/schema-recipe`, NOT A LOCAL TWIN (§15 rule 4 / ADR-0014). A
 * hand-written interface mirroring `createRecipeRequestSchema` / `updateRecipeRequestSchema` field for field,
 * with nothing comparing them, is the silent-drift shape §15.1 measures. The editor's output type IS the
 * request type the service authors, so a backend field change fails this package's `typecheck` instead of an
 * emulator run.
 *
 * ⚠️ `toUpdateRecipeInput` spreads the create projection, which carries `visibility`. `UpdateRecipeRequest`
 * has no such key — visibility moves through `PATCH /api/v1/recipes/{id}/visibility`, where the C-004 policy
 * evaluator gates it — and the service silently STRIPS it. A spread is exempt from excess-property checking,
 * so the published type alone would compile while still sending the field; it is dropped explicitly below.
 *
 ...
 * B2 — the ONE seed adapter both platforms and the `useRecipeEditor` headless hook use; no web-local or
 * mobile-local copy of this mapping exists). Persisted ingredient lines already reference a catalog id, so each carries
...
export const toRecipeFormValues = (detail: RecipeDetail): RecipeFormValues => ({
    ...
    ingredients: detail.ingredients.map((line) => ({
        ...
        // ⛔ THE LINE'S OWN STATUS, falling back to `RESOLVED` (U14) — never hard-coded. "A saved recipe's
        // lines are, by definition, resolved" is FALSE once the detail read publishes the verification gate's
        // verdict: a contradicted line would open in the editor badged "Resolved", the opposite of what the
        // recipe screen just told the cook, on the one surface where they can re-pick the food. The fallback
        // covers a line the server said nothing about: it still shows a badge rather than none.
