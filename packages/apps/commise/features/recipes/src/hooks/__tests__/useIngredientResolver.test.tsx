...
describe('useIngredientResolver — the U16 create-your-own-food sub-machine', () => {
    ...
    /**
     * ⛔ open() on an ALREADY-open form must not re-derive the draft from the query: the leaves keep the trigger
     * mounted beside the form, so a stray second open would silently discard the macros the cook typed.
     */
    it('⛔ open() while the form is open keeps the draft the cook has typed', () => {
        const { result } = renderHook(() => useIngredientResolver(vi.fn()));
        ...
        const state = result.current.createFood.state;
        expect(state.kind).toBe('open');
        expect(state.kind === 'open' ? state.draft.calories : null).toBe('100');
    });
    ...
    // ⛔ The form's controls stay FOCUSABLE while a request is in flight (native `disabled` on a focused control
    // drops focus to <body> in a real browser — WCAG 2.2 SC 2.4.3), so "not now" cannot be left to the
    // DOM. The guard lives here, once, for both platforms: cancelling mid-submit would clear the form and let the
    // pending response resurrect `duplicate` or `submitFailed` onto a form the cook had closed.
    it('cancel() is IGNORED while the create is in flight', () => {
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(mutation(null, { mutate: vi.fn(), isPending: true }));
        ...
        expect(result.current.createFood.state.kind).toBe('submitting');
    ...
    it('submit() is IGNORED while the create is in flight — one press, one create', () => {
        const mutate = vi.fn();
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(mutation(null, { mutate, isPending: true }));
        ...
        // The raw phase is still `open` while the request runs (`submitting` is DERIVED from `isPending`), so a
        // phase check alone let a second press create twice once the leaf stopped using native `disabled`.
        expect(mutate).not.toHaveBeenCalled();
    });

    it('reuseExisting() is IGNORED while a reuse is already in flight — one press, one admission', () => {
        const byFoodMutate = vi.fn();

        useCreateAuthoredFoodViaPickerMock.mockReturnValue(
            mutation({ created: false, reason: 'duplicate', existingFoodId: 'F_prior' }),
        );
        useAddIngredientByFoodMock.mockReturnValue(mutation(null, { mutate: byFoodMutate, isPending: true }));
        ...
        expect(byFoodMutate).not.toHaveBeenCalled();
    });
...
/**
 * ⛔ WHAT WAS ADDED IS STATE, SO BOTH PLATFORMS SAY IT THE SAME WAY (staff-ux-engineer).
 *
 * A resolve resets the picker to a blank search and unmounts the control that was pressed; the new line lands in a
 * sibling list the picker does not own, often off-screen. Nothing said what happened. The hook now keeps the name
 * of the last line it added — each leaf renders it as a visible, announced status — and clears it on the next edit,
 * so the same food added twice (with typing in between) is announced twice.
 */
describe('useIngredientResolver — lastAdded (what the picker just put on the recipe)', () => {
    it('names the line after a resolve, and clears on the next keystroke', () => {
        const added = makeIngredient({
            id: 'ing_q',
            name: 'Quinoa',
            foodResolutionStatus: FoodResolutionStatus.PENDING,
        ...
        expect(result.current.lastAdded).toBe('Quinoa');
...
/**
 * ⛔ ONE PRESS, ONE REQUEST — enforced HERE, because the web leaf does not use native `disabled` on a pending
 * control (a real browser drops focus from it, WCAG 2.2 SC 2.4.3). Each add action refuses while its own mutation
 * is in flight.
 */
describe('useIngredientResolver — add actions refuse a second press while pending', () => {
    it('findNutrition()', () => {
        const mutate = vi.fn();
        useAddIngredientByNameMock.mockReturnValue(mutation(null, { mutate, isPending: true }));
        const { result } = renderHook(() => useIngredientResolver(vi.fn()));
    ...
    it('addFreeform()', () => {
        const mutate = vi.fn();
        useCreateIngredientMock.mockReturnValue(mutation(null, { mutate, isPending: true }));
        const { result } = renderHook(() => useIngredientResolver(vi.fn()));

        act(() => result.current.setQuery('Grandma spice'));
        act(() => result.current.addFreeform());

        expect(mutate).not.toHaveBeenCalled();
    });

    it('selectSuggestion() on a catalog suggestion', () => {
        const mutate = vi.fn();
        useAddIngredientByFoodMock.mockReturnValue(mutation(null, { mutate, isPending: true }));
        const { result } = renderHook(() => useIngredientResolver(vi.fn()));

        act(() => result.current.selectSuggestion(catalogSuggestion('F_1', 'Oats')));

        expect(mutate).not.toHaveBeenCalled();
    });

    it('selectLiveHit() — by food, and by name', () => {
        const byFood = vi.fn();
        const byName = vi.fn();
        useAddIngredientByFoodMock.mockReturnValue(mutation(null, { mutate: byFood, isPending: true }));
        useAddIngredientByNameMock.mockReturnValue(mutation(null, { mutate: byName, isPending: true }));
        const { result } = renderHook(() => useIngredientResolver(vi.fn()));

        act(() => result.current.selectLiveHit({ name: 'Broccoli, raw', foodId: 'food_1' }));
        act(() => result.current.selectLiveHit({ name: 'Broccoli rabe' }));
    ...
    it('pickCandidate()', () => {
        const mutate = vi.fn();
        useResolveIngredientMock.mockReturnValue(mutation(null, { mutate, isPending: true }));
        ...
        act(() => result.current.setQuery('Pepper'));
        act(() => result.current.findNutrition());
        act(() => result.current.pickCandidate('cand_1'));
