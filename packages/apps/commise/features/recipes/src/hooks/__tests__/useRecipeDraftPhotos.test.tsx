...
describe('useRecipeDraftPhotos — the flush, once the recipe has an id', () => {
    ...
    it('carries the file’s bytes and metadata to the queue, not just the descriptor', () => {
        ...
        // ⚠️ ANTI-VACUITY: assert the call and the payload EXIST before reading them. Written as
        // `file?.fileName` over an optional-chained lookup, all three assertions below pass when nothing
        // was enqueued at all — the test would go green on a hook that dropped every pick.
        expect(enqueue).toHaveBeenCalledTimes(1);
        const picks = enqueue.mock.calls[0]![0] as readonly DraftPhotoPick[];
