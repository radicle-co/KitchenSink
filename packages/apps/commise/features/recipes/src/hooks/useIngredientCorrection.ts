...
export interface IngredientCorrectionController {
    ...
    /**
     * Whether a write is in flight — exposed separately ONLY so a leaf can mark its control unavailable.
     *
     * ⚠️ Derivable from `viewState.kind === 'saving'`, and deliberately not re-derived at two call sites per
     * leaf: `aria-disabled` and `aria-busy` must never disagree about whether the control is working. The flag
     * is presentation only — {@link IngredientCorrectionController.correct} already refuses while it is set.
     */
...
export function useIngredientCorrection(surfacing: CorrectionSurfacing): IngredientCorrectionController {
    ...
    const correct = useCallback(
        (phrase: string, foodId: string): void => {
            // ⛔ The controller, not the control, refuses a second write while one is in flight: a web leaf keeps
            // its control focusable (`aria-disabled`, never native `disabled`, which drops focus — WCAG 2.2 SC
            // 2.4.3), so a press still reaches here and must be a no-op.
            if (isPending) {
                return;
            }
