...
export interface UseIngredientResolverResult {
    ...
    /**
     * The name of the line the picker last put on the recipe, or `null` — cleared by the next edit of the query.
     *
     * A resolve resets the picker to a blank search and unmounts the pressed control, and the new line lands in a
     * list the picker does not own (often off-screen), so nothing said what happened. Each leaf renders this as a
     * visible, announced status (staff-ux-engineer). Cleared on the next edit so adding the same food twice, with
     * typing in between, is announced twice.
     *
     * ⚠️ A plain name, not a counter, and the leaves key their after-add focus move on it — safe because no path adds
     * the same name twice WITHOUT an edit between: every query-based add needs a query and the resolve clears it,
     * live hits fall back to idle once the query moves off the searched one, and a concurrent add's callbacks are
     * dropped by the resolve's `reset()`. The one leftover — an authored-food create settling after a concurrent add
     * of the same name — is not worth a counter, which would move focus but could not make identical text be
     * announced again anyway.
     */
...
export function useIngredientResolver(
    ...
    // The two effects below EMIT; they are not re-run BY the emitter. An Effect Event reads the latest emitter
    // without making it a dependency, which is the only honest spelling of the unmount flush: listing the emitter
    // would flush open sessions whenever its identity changed (a re-created client), and omitting it was a
    // dependency list the lint could not trust.
    const emitFromEffect = useEffectEvent((event: Parameters<typeof emitAnalytics>[0]): void => {
        emitAnalytics(event);
    });
    ...
    useEffect(() => {
        ...
        // Mount-only on purpose: re-running this effect would flush open sessions spuriously mid-search. The
        // emitter is reached through `emitFromEffect`, an Effect Event, so it is not a dependency.
    }, []);
    ...
    const selectSuggestion = (suggestion: IngredientSuggestion): void => {
        // One press, one admission: the leaf keeps a pending row focusable (aria-disabled), so the hook refuses.
        if (suggestion.provenance === 'catalog' && addIngredientByFood.isPending) {
            return;
        }
    ...
    const findNutrition = (): void => {
        if (addIngredientByName.isPending) {
            return;
        }
    ...
    const pickCandidate = (candidateId: string): void => {
        if (disambiguating === null || resolveIngredient.isPending) {
            return;
        }
    ...
    const selectLiveHit = (hit: LiveIngredientHit): void => {
        // One press, one admission — whichever path this hit takes, refuse while that path is already in flight.
        if (hit.foodId === undefined ? addIngredientByName.isPending : addIngredientByFood.isPending) {
            return;
        }
    ...
    const createFood: UseIngredientResolverResult['createFood'] = {
        ...
        open: (): void => {
            // Never over an open form: the leaves keep the trigger mounted beside it, and re-deriving the draft
            // from the query would silently discard what the cook typed. The trigger collapses via `cancel`.
            if (createFoodPhase !== null) {
                return;
            }
        ...
        cancel: (): void => {
            // Not while the create is in flight: the leaves keep this control focusable (aria-disabled, not
            // `disabled`), and closing now would let the pending response resurrect a state onto a closed form.
            if (createAuthoredFood.isPending) {
                return;
            }
        ...
        submit: (): void => {
            // `isPending` too, not the phase alone: the raw phase stays `open` while the request runs (`submitting`
            // is DERIVED from `isPending`), and the leaves keep the submit control focusable rather than disabled.
            if (createFoodPhase === null || createFoodPhase.kind !== 'open' || createAuthoredFood.isPending) {
                return;
            }
        ...
        reuseExisting: (): void => {
            // One press, one admission — the guard native `disabled` would provide, which the leaves do not use
            // because it drops focus from the control being pressed.
            if (createFoodPhase === null || createFoodPhase.kind !== 'duplicate' || addIngredientByFood.isPending) {
                return;
            }
