 ...
 *
 * ⚠️ THIS IS NOT WHERE B24 IS FIXED. B24 — "a second pick mid-upload must not start a second concurrent
 * sequence" — is satisfied by `useRecipePhotoUploadQueue`, by ENQUEUEING: a tap during an upload is
 * welcomed, which is the queue's whole reason to exist. This
 ...
 *
 * ⛔ THE POINT: a consumer that needs a per-attempt verdict reads it HERE, never from the falling edge of
 * `uploading` plus `errorMessage`. Inferring it from a flag couples the consumer's scheduling to the exact moment
 * this hook flips a boolean, and that moment moves with the implementation (a react-query mutation propagates
 * status asynchronously through its notify manager) — a queue reading the edge settles an attempt that has not
 * started and marks a photo `ok` that never uploaded. A value has no edge to mis-time.
 *
