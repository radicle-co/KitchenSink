...
export interface RecipeDetailMessages {
    ...
    /**
     * Badge on a line whose food its author WITHDREW (owner rulings 3 + 4).
     *
     * ⛔ Distinct from {@link unavailableBadge}, and the difference is not cosmetic. That one says the food
     ...
     * 2. **"marked above", never "marked in the list".** The plural string used "list" twice, one sentence
     ...
     * this from `unavailableBadge` ("exists, not served to you"), and "is no longer in" collapses REMOVAL
     ...
     *
     * ⛔ Names the RECIPE'S LINE (`RecipeIngredientView.name`), never the food record. A public recipe can
     * be read by someone with no entitlement to the catalog row — which is exactly what
     * `RESOLVED_UNAVAILABLE` exists to protect.
     */
    ...
    en: {
        ...
        detail: {
            ...
            removedFoodNoticeMany:
                'Nutrition only: {count} ingredients were removed from our food database, so they aren’t counted below. They’re marked above, and their names and amounts are unchanged.',
            removedFoodNoticeAll:
                'Nutrition only: every ingredient here was removed from our food database, so there’s no nutrition to show. Their names and amounts in this recipe are unchanged.',
