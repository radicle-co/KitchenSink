...
describe('RecipeCalorieChip (web) — known', () => {
    ...
    it('⛔ keeps "may be out of date" in the name of a RANGE-DERIVED stale figure (the caveat the range used to drop)', () => {
        renderChip(reading({ rangeDerivedBound: 'low', freshness: 'stale' }));

        expect(
            screen.getByRole('img', {
                name: 'About 420 cal, counted from the lower amount of each stated range; the figure may be out of date',
            }),
        ).toBeTruthy();
    });
    ...
    it('renders a reading that is BOTH stale and incomplete with the combined caveat', () => {
        ...
        expect(
            screen.getByRole('img', {
                name: 'About 420 cal, some items aren’t counted yet, and the figure may be out of date',
            }),
        ).toBeTruthy();
    });
