...
export interface RecipeNutritionMessages {
     ...
     *
     * ⚠️ "Incomplete", not "Approximate": approximate is the VISIBLE-text word and covers a range-derived figure
     * too, so naming a label key after it is what let the range and incompleteness caveats be confused.
     */
    readonly caloriesIncompleteLabel: string;
    /** Accessible name for a reading that is both incomplete AND stale (contains `{calories}`). */
    readonly caloriesIncompleteStaleLabel: string;
    /**
     ...
     *
     * ⛔ EVERY RANGE CELL HAS ITS OWN STALE AND INCOMPLETE VARIANTS. The range caveat is independent of the other
     * two — one line can collapse a range while another goes uncounted, and freshness is a fact about the read —
     * so a range label that stood alone silently dropped whichever of them also applied.
     */
    readonly caloriesRangeLowLabel: string;
    /** The LOW-bound name for a figure that is also STALE. */
    readonly caloriesRangeLowStaleLabel: string;
    /** The LOW-bound name for a figure that is also INCOMPLETE. */
    readonly caloriesRangeLowIncompleteLabel: string;
    /** The LOW-bound name for a figure that is also INCOMPLETE and STALE. */
    readonly caloriesRangeLowIncompleteStaleLabel: string;
    ...
    /** The HIGH-bound name for a figure that is also STALE. */
    readonly caloriesRangeHighStaleLabel: string;
    /** The HIGH-bound name for a figure that is also INCOMPLETE. */
    readonly caloriesRangeHighIncompleteLabel: string;
    /** The HIGH-bound name for a figure that is also INCOMPLETE and STALE. */
    readonly caloriesRangeHighIncompleteStaleLabel: string;
...
export const recipeNutritionMessages: LocalizedMessages<RecipeNutritionMessages> = {
    en: {
        ...
        caloriesIncompleteLabel: 'About {calories} cal, some items aren’t counted yet',
        caloriesIncompleteStaleLabel:
            'About {calories} cal, some items aren’t counted yet, and the figure may be out of date',
        caloriesRangeLowLabel: 'About {calories} cal, counted from the lower amount of each stated range',
        caloriesRangeLowStaleLabel:
            'About {calories} cal, counted from the lower amount of each stated range; the figure may be out of date',
        caloriesRangeLowIncompleteLabel:
            'About {calories} cal, counted from the lower amount of each stated range; some items aren’t counted yet',
        caloriesRangeLowIncompleteStaleLabel:
            'About {calories} cal, counted from the lower amount of each stated range; some items aren’t counted yet, and the figure may be out of date',
        caloriesRangeHighLabel: 'About {calories} cal, counted from the upper amount of each stated range',
        caloriesRangeHighStaleLabel:
            'About {calories} cal, counted from the upper amount of each stated range; the figure may be out of date',
        caloriesRangeHighIncompleteLabel:
            'About {calories} cal, counted from the upper amount of each stated range; some items aren’t counted yet',
        caloriesRangeHighIncompleteStaleLabel:
            'About {calories} cal, counted from the upper amount of each stated range; some items aren’t counted yet, and the figure may be out of date',
