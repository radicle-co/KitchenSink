...
export interface RecipeCalorieChipModel {
    ...
    /** The figure was served from the recipe service's saved copy of food data — "serve stale, MARKED" (KTD-3b). */
    readonly isStale: boolean;
    /** Some ingredient line could not be accounted for, so the figure is a floor, not a total. */
    readonly isApproximate: boolean;
}

/** The message keys that may name a chip: the plain figure, or a `calories…Label` template — never other copy. */
type ChipLabelKey = Extract<keyof RecipeNutritionMessages, 'calories' | `calories${string}Label`>;

/**
 * Which accessible-name template names each caveat combination — range bound × completeness × freshness.
 *
 * ⛔ A TOTAL LOOKUP, never a chain of early returns: a chain that asks about one caveat first drops the others
 * from a figure carrying several. Every cell is typed here, so a missing combination is a compile error rather
 * than a caveat that silently disappears. A COMPLETE, FRESH figure from no range is named by its visible text
 * (`calories`) — there is nothing to qualify.
 *
 * ⚠️ A FOURTH independent caveat would make this 24 cells, past worth maintaining; the answer then is an ICU
 * `select` message, never concatenated sentences.
 */
const CHIP_LABEL_TEMPLATE: Readonly<
    Record<
        'none' | 'low' | 'high',
        Readonly<Record<'complete' | 'incomplete', Readonly<Record<'fresh' | 'stale', ChipLabelKey>>>>
    >
> = {
    none: {
        complete: { fresh: 'calories', stale: 'caloriesStaleLabel' },
        incomplete: { fresh: 'caloriesIncompleteLabel', stale: 'caloriesIncompleteStaleLabel' },
    },
    low: {
        complete: { fresh: 'caloriesRangeLowLabel', stale: 'caloriesRangeLowStaleLabel' },
        incomplete: { fresh: 'caloriesRangeLowIncompleteLabel', stale: 'caloriesRangeLowIncompleteStaleLabel' },
    },
    high: {
        complete: { fresh: 'caloriesRangeHighLabel', stale: 'caloriesRangeHighStaleLabel' },
        incomplete: { fresh: 'caloriesRangeHighIncompleteLabel', stale: 'caloriesRangeHighIncompleteStaleLabel' },
    },
};
...
export const toCalorieChipModel = (
    ...
    const labelTemplate =
        messages[
            CHIP_LABEL_TEMPLATE[reading.rangeDerivedBound ?? 'none'][reading.isComplete ? 'complete' : 'incomplete'][
                reading.freshness
            ]
        ];
