                ...
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            // A blank draft is a rule the press did not cause (native `disabled`); a save in flight
                            // follows the press, so Save keeps focus (`busyControlProps`).
                            {...busyControlProps({
                                busy,
                                blocked: draftAdmission?.canSubmit !== true,
                                onClick: () => {
                                    const next = draft.trim();

                                    // An edit is not a delete — a blank replacement is refused, exactly as the
                                    // service's own schema refuses it.
                                    if (draftAdmission?.canSubmit !== true) {
                                        return;
                                    }
                            ...
                            className={`rounded-full bg-seafoam px-3 py-1 text-caption font-semibold text-ocean-dark ${BUSY_CONTROL_CLASS}`}
                        >
...
export const ParseJobReview: FC<ParseJobReviewProps> = ({
    ...
    const retryControl = (
        <button
            type="button"
            {...busyControlProps({ busy: retry.busy, onClick: retry.run })}
            className={`self-start rounded-full bg-seafoam px-4 py-2 text-body-sm font-semibold text-ocean-dark ${BUSY_CONTROL_CLASS}`}
        >
