    ...
    return (
        <section aria-label={messages.pasteHeading} className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
                ...
                <button
                    type="button"
                    // Two different reasons to be unavailable, handled differently: an inadmissible paste is a
                    // rule the press did not cause (native `disabled`); a create in flight follows the press, so the
                    // control keeps focus (`busyControlProps`).
                    {...busyControlProps({ busy: submitting, blocked, onClick: onSubmit })}
                    className={`self-start rounded-full bg-seafoam px-5 py-2 text-body-sm font-semibold text-ocean-dark disabled:opacity-60 ${BUSY_CONTROL_CLASS}`}
                >
