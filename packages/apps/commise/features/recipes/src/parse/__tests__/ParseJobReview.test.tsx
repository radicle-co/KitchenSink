...
describe('ParseJobReview (web) — a job in progress', () => {
    ...
    it('marks the retry control busy and refuses a second press while one is in flight', async () => {
        ...
        // REWRITTEN from `disabled`: the pressed control keeps focus while busy (`busyControlProps`, SC 2.4.3).
...
describe('ParseJobReview (web) — editing one line', () => {
     ...
     * contrast at all.
     *
     * ⚠️ IT STAYS HAND-ROLLED ON PURPOSE, so this guard is load-bearing rather than redundant: it sits in a
     * dense per-line edit row where the primitive's 44px floor and padding would reflow every line. The
     * standalone actions in this file DID move to `Button`; this one keeps the fill and takes the corrected
     * label colour, which means only a test can hold the pairing.
     *
     * ⚠️ `utilityContrast` reads the class list OFF THE DOM. Re-spelling it here would measure the test.
     */
    it('⛔ the inline edit submit clears WCAG 1.4.3 on its seafoam fill', async () => {
        renderReview(withJob('ready', editable));

        await userEvent.click(screen.getByRole('button', { name: 'Edit line 4' }));

        const submit = screen.getByRole('button', { name: messages.lineEditSubmit });

        expect(utilityContrast(submit.className), 'the inline edit submit label on its fill').toBeGreaterThanOrEqual(
            4.5,
        );
    });
    ...
    it('⛔ marks only the BUSY line busy — two rows must not both look like they are saving', async () => {
        ...
        // REWRITTEN from `disabled`: busy is `aria-disabled`, so the idle row carries no such attribute at all.
        expect(screen.getByRole('button', { name: 'Edit line 1' }).hasAttribute('aria-disabled')).toBe(false);
        const busy = screen.getByRole('button', { name: 'Edit line 2' });
