 ...
 *
 * ⚠️ This is NOT a second occurrence of the `Intl.PluralRules`-select-then-`fillTemplate` shape, so the
 * wait-for-the-third rule is no reason to defer: `list/model.ts`, `rating/model.ts`, `card/model.ts` and
 * `filters/model.ts` already carry it.
 *
 * ⛔ Reusing `formatRecipeCount` is refused all the same: its name would lie at this call site. The helper
 * lives once in `parse/model.ts` rather than as a private copy inside each platform leaf, which is the drift
 * the one-contract-two-renderers shape exists to make impossible.
 */
...
export const recipeParseMessages: LocalizedMessages<RecipeParseMessages> = {
    en: {
        ...
        reasons: {
            ...
            // ⛔ SAYS SOMETHING DIFFERENT FROM `not_a_food`, because the two reasons say different things
            // and can sit side by side with `name_is_source_line`. That one is a verdict — what we read was
            // not an ingredient — and this one is silence: nothing came back at all. Two chips repeating
            // "we couldn't find an ingredient" would read as one bug rendered twice.
            //
            // ⚠️ Phrased about the LINE, like every other string here, and never about our machinery: a
            // cook has no reader, no engine and no retry loop to reason about, and telling them one came
            // back empty explains nothing they can act on.
            no_food_returned: 'We couldn’t read anything on this line',
            measurement_unverified: 'Double-check the amount on this line',
            // R5: the ingredient name a cook is looking at IS the line they pasted, so the copy says that
            // rather than describing a failure they cannot see.
            name_is_source_line: 'We kept this line as you wrote it — nothing on it could be read as an ingredient',
        },
