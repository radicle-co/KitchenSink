                            ...
                            return (
                                ...
                                >
                                    ...
                                    <input
                                        type="radio"
                                        name={groupName}
                                        className="absolute inset-0 cursor-pointer opacity-0 aria-disabled:cursor-not-allowed"
                                        aria-label={optionLabel}
                                        checked={selectedStars === value}
                                        // The star just chosen is the focused control when the write starts, so
                                        // pending is `aria-disabled` + a refused change, never native `disabled`
                                        // (which drops focus to <body>, WCAG 2.2 SC 2.4.3). `checked` stays
                                        // controlled, so a refused change leaves the saved star selected.
