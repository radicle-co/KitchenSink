...
describe('RecipeRatingInput (web) — interactive variant', () => {
     ...
     * `aria-disabled` (`busyControlProps`), and the press is refused — for the star radios as well as the remove button, since the star the cook just chose is the
     * focused control when the write starts.
     */
    it('marks the control busy and keeps the radios focusable but refused while a write is in flight', async () => {
        const user = userEvent.setup();
        const onRate = vi.fn();
        const onRemove = vi.fn();
        renderInput({ selectedStars: 4, pending: true, onRate, onRemove });

        expect(screen.getByRole('status', { name: 'Saving your rating…' })).toBeTruthy();
        const star = screen.getByRole<HTMLInputElement>('radio', { name: 'Rate 2 stars' });
        const remove = screen.getByRole<HTMLButtonElement>('button', { name: 'Remove my rating' });
        ...
        expect(screen.getByRole<HTMLInputElement>('radio', { name: 'Rate 4 stars' }).checked).toBe(true);
    });
