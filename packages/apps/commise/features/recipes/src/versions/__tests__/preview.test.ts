...
/**
 * B21 — the version preview's state is derived ONCE, purely, from the container's own inputs, for both call
 * sites (the web `RecipeVersionsContainer` and the native `RecipeVersionsScreen`). A per-site copy that
 * hard-codes `isLoading={false}` and never passes `error` renders an unrecoverable spinner for a preview
 * target the loaded history does not contain. Settled-but-absent is a FAILURE, not a pending fetch.
 */
describe('resolveVersionPreview', () => {
    ...
    it('reports ERROR — not loading — when the previewed version is absent from the settled list', () => {
        ...
        // Not the spinner, which would strand the viewer with no way to learn the lookup had failed.
        expect(state.isLoading).toBe(false);
