...
export const composeMergedRecipe = (
    ...
    // ⚠️ `Object.fromEntries`, not a literal written through `merged[key] = …`, and the honest reason is that
    // the guard is deliberately over-broad rather than that this code was exploitable. ⛔ It was NOT: the key
    // set comes from `Object.keys(mineRecord)`, so every key written was ALREADY an own data property, and an
    // assignment to an own data property never consults the prototype — measured, a `__proto__` key survives
    // the old loop with its value intact and `Object.prototype` untouched. Do not "restore" the loop on that
    // reasoning, though: `prototypePollutionSinks.test.ts` flags the shape, its docstring records that this
    // class was in five places at once, and the remedy is cheap enough to take rather than argue.
    //
    // ⚠️ The other remedy — `Object.create(null)` — is NOT used here on purpose: this value is cast to
    // `RecipeFormValues` and handed to the editor's form state, and a null-prototype object is a behaviour
    // change for any consumer that does a plain-object check. `Object.fromEntries` returns an ordinary one.
    //
    ...
    const merged = Object.fromEntries(
        Object.keys(mineRecord).map((key) => [key, selections[key] === 'theirs' ? theirsRecord[key] : mineRecord[key]]),
    );
 ...
 * ever sets top-level keys sees IDENTICAL behavior to `composeMergedRecipe`. Pure.
 *
 * @param mine - The user's in-progress draft.
 * @param theirs - The latest saved recipe projected to the editable form shape.
