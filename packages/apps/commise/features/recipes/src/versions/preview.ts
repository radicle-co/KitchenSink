...
export interface VersionPreviewModalProps {
     ...
     *  `version` does not imply "still loading", or a settled-with-nothing caller strands the viewer. */
...
// preview modal from the SAME four inputs. A per-site copy of the derivation — with a hard-coded
// `isLoading={false}` and no `error` — makes ONE state unreachable and another a dead end: when the preview
// target is not in the (already loaded, already settled) history, `version` is `undefined`, the modal's own
// precedence reads that as "still loading", and it spins forever with no way to say the lookup failed. The
// derivation lives HERE, once, so a lookup miss is reported as the FAILURE it is on both platforms at once.
