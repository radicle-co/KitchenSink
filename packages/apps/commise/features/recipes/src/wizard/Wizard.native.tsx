...
const WizardRoot: FC<WizardProps> = (props) => {
    ...
    // ⛔ THE SYSTEM BACK BUTTON IS THE SECOND ENTRY POINT TO THE DISCARD GUARD (data-loss fix).
    //
    // `Wizard.Header`'s arrow has always routed through `requestCancel`; Android's hardware back button did
    // not, and `RecipesScreen` popped this surface for it — so a cook who pressed the phone's own back button
    // lost every unsaved edit with no warning, on CREATE as well as EDIT. Both exits are now this one guard
    // and this one dialog.
    //
    // ⛔ `true` IN BOTH BRANCHES, and that is not tidiness. When the draft is clean `requestCancel` calls
    // `onCancel` SYNCHRONOUSLY, and `onCancel` is the host's own `nav.back()` — answering `false` here would
    // let the host's default run as well and pop TWO surfaces for one press. A mounted wizard always consumes
    // the press; only its ANSWER varies.
    //
    // No "is the dialog already open" check is needed in either direction: `requestCancel` is idempotent
    // (`setPendingAction` re-stores the same action), and on a device the press never arrives here at all once
    // the dialog is up — React Native's `Modal` catches it on the dialog window and fires `onRequestClose`,
    // which `ConfirmDialog` maps to Keep-editing. The visible consequence is that leaving a dirty editor takes
    // two presses, which is the more protective direction.
    useBackIntercept(() => {
        requestCancel();
