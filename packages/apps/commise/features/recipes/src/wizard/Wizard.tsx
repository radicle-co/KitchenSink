...
const WizardControls: FC = () => {
    ...
    return (
        // ⛔ `z-60`, ABOVE `HomeTabBar`'s `z-50`. Both are `fixed bottom-0` and there is no stacking context
        // between them and the root, so at `z-30` the tab bar painted OVER all three controls at every width
        // below `lg` — Prev, Save Draft and Next were each unreachable, and recipe create and edit could not
        // be completed on any phone or tablet web viewport. The bar is 80–90% opaque with a 24px blur, so the
        // controls stayed faintly visible underneath and it read as a rendering oddity rather than a dead end.
        //
        // ⚠️ THIS IS THE BELT. The braces are `AppShell`'s `focusedTask`, which suppresses the tab bar on
        // wizard routes entirely — a destination change mid-creation discards work, so the foot belongs to
        // the task. This `z-60` is what keeps a FUTURE pinned surface from reintroducing the same collision.
        //
        // ⚠️ The old spec could not see it: it asserted the bar's Y POSITION, and a covered bar sits exactly
        // where it should. `recipeWizardActionBar.spec.ts` hit-tests with `elementFromPoint` now.
        <div className="fixed inset-x-0 bottom-0 z-60 flex flex-col gap-2 border-t border-border bg-card px-4 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] lg:static lg:z-auto lg:border-0 lg:bg-transparent lg:p-0">
            {blocking.length > 0 && (
