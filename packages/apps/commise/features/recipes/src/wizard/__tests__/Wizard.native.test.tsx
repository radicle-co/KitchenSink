...
import { type RecipeWizardStep, canAdvanceFromStep, stepErrorsFor } from '../../form/steps.js';
import { type RecipeFormValues, defaultRecipeFormValues } from '../../form/values.js';
import { Wizard } from '../Wizard.native.js';

/**
 * `WizardRoot` installs a hardware-back interceptor, and `useBackIntercept` THROWS without a provider above it
 * — deliberately, so a back guard can never be silently absent. Every render here therefore goes through a
 * real `BackInterceptProvider`; `onUnhandled` answers `false`, standing in for a host with nothing to pop, so
 * a press the wizard declines is visibly unhandled rather than quietly swallowed.
 */
...
describe('Wizard (native) — the SYSTEM back button routes through the same discard guard (data-loss fix)', () => {
    // ⛔ THE DEFECT. `RecipesScreen`'s hardware-back handler popped the surface unconditionally, so the
    // editor's header arrow asked before discarding and the phone's own back button did not — the same
    // screen, left two ways, one of them destroying an unsaved draft with no warning. The wizard now
    // installs an interceptor, so BOTH entry points reach this one guard and this one dialog.

    it('⛔ raises the discard dialog on a system back press with unsaved edits, and navigates nowhere', () => {
        const onCancel = vi.fn();
        render(<Harness initialValues={validValues()} isDirty onCancel={onCancel} />);
    ...
    it('discards only once the cook confirms, and leaves exactly once', () => {
        const onCancel = vi.fn();
        render(<Harness initialValues={validValues()} isDirty onCancel={onCancel} />);
        ...
        // EXACTLY once: `onCancel` is the host's `nav.back()`, so a second call would pop a second surface.
        expect(onCancel).toHaveBeenCalledTimes(1);
    });

    it('"Keep editing" after a system back press discards nothing', () => {
        const onCancel = vi.fn();
        render(<Harness initialValues={validValues()} isDirty onCancel={onCancel} />);
    ...
    it('leaves straight away on a system back press when there is nothing unsaved to lose', () => {
        const onCancel = vi.fn();
        render(<Harness initialValues={validValues()} isDirty={false} onCancel={onCancel} />);
    ...
    it('⛔ CONSUMES the press in BOTH branches — a clean wizard must not also let the host pop', () => {
        // The clean branch calls `onCancel` SYNCHRONOUSLY, and `onCancel` is the host's own back navigation.
        // Returning `false` here would run the host default too and take TWO surfaces for one press.
        render(<Harness initialValues={validValues()} isDirty={false} />);

        expect(back.press()).toBe(true);
    });

    it('guards the CREATE wizard exactly as it guards edit — a typed-from-scratch recipe is the same loss', () => {
        const onCancel = vi.fn();
        render(<Harness mode="create" initialValues={validValues()} isDirty onCancel={onCancel} />);
    ...
    it('guards the EDIT wizard on the same press', () => {
        const onCancel = vi.fn();
        render(<Harness mode="edit" initialValues={validValues()} isDirty onCancel={onCancel} />);
    ...
    it('repeated presses raise ONE dialog, never a stack of them', () => {
        render(<Harness initialValues={validValues()} isDirty />);
    ...
    // ⚠️ "the interceptor is REMOVED when the wizard unmounts" is deliberately NOT asserted here. This
    // harness supplies the provider itself, so unmounting the wizard unmounts the provider with it and the
    // assertion would be trivially true whatever the hook did. The real guarantee — a consumer's handler
    // going away while the provider's subscription survives — is proved where the two can be unmounted
    // independently, in `@commise/ui`'s `backIntercept.native.test.tsx` ("unregisters on ITS unmount while
    // the provider subscription survives"), and end-to-end through the real navigator in
    // `@commise/mobile`'s `RecipesScreen.backGuard.native.test.tsx`.
});
