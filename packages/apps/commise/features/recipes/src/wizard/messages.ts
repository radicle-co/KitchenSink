...
export interface WizardMessages {
    ...
    /**
     * The bare Previous label, shown INSTEAD of {@link prevLabel} below `md`.
     *
     * ⛔ THE STEP NAME DOES NOT FIT ON A PHONE. Measured on the shipped geometry, the three-control row needs
     * 388px against 288 available at 320 — so "Next: Instructions" was clipped 84px off the right edge, and
     * because a `fixed` element is excluded from scrollable overflow the page gained no scrollbar and no
     * tooling flagged it. The short form is the VISIBLE text only: the button keeps the full templated label
     * as its accessible name, so a screen reader still hears which step it is going to.
     */
    readonly prevLabelShort: string;
    /** The bare Next label, shown INSTEAD of {@link nextLabel} below `md`. See {@link prevLabelShort}. */
    readonly nextLabelShort: string;
    /**
     * The bare Save label, shown INSTEAD of {@link saveDraft} below `md`.
     *
     * ⛔ MEASURED, not trimmed on taste. With "Save Draft" in the middle slot the three-action row still
     * wrapped to two lines at 320-375 even after the step names were dropped — it is the longest label in
     * the row, so shortening the other two moved the overflow onto it rather than removing it.
     */
    readonly saveDraftShort: string;
