/**
 * @module test-utils/flashListStub — a jsdom stand-in for `@shopify/flash-list`'s `FlashList`, aliased in by
 * `vitest.native.config.ts`. FlashList is a native (Fabric) recycler with no jsdom runtime — importing it
 * under the native component tests would abort the same way `expo-image` does. This stub renders the SAME
 * observable output the tests query against: the `ListHeaderComponent`, every `data` row through `renderItem`
 * (no virtualization — every row is in the DOM, so `getByRole`/`getByText` find them all), the
 * `ListFooterComponent`, and the `ListEmptyComponent` when there is no data. `keyExtractor` supplies stable
 * React keys; `refreshControl`, `accessibilityLabel`, `style`, `contentContainerStyle`, and
...
interface FlashListStubProps<TItem> extends Pick<ScrollViewProps, 'style' | 'contentContainerStyle' | 'keyboardShouldPersistTaps' | 'accessibilityLabel'> {
