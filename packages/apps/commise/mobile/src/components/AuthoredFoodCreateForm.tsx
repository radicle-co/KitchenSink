 ...
 *
 * ⛔ SCREEN-READER FOCUS, NOT KEYBOARD FOCUS (web's WCAG 2.2 SC 2.4.3 handling, ADAPTED per staff-ux-engineer).
 * On native the question is where the reading cursor lands; `autoFocus` would raise a keyboard over the macro
 * fields the cook needs next. So each branch moves the SCREEN-READER cursor as it mounts — the form to its
 * heading, the duplicate branch to its notice — and every failure or progress line is a {@link LiveRegion}, so
 * VoiceOver users hear it too (iOS has no live region of its own). The duplicate notice is deliberately NOT a
 * live region: the focus move already reads it.
 *
...
export function AuthoredFoodCreateForm({ state, copy, actions }: AuthoredFoodCreateFormProps): JSX.Element {
    if (state.kind === 'duplicate') {
        return <DuplicateBranch state={state} copy={copy} actions={actions} />;
    }

    return <FormBranch state={state} copy={copy} actions={actions} />;
}

/** The per-author collision: its own sentence, which takes screen-reader focus as it replaces the form. */
function DuplicateBranch({
    ...
    const noticeRef = useScreenReaderFocusOnMount<Text>();

    return (
        <View style={styles.card}>
            <Text ref={noticeRef} style={styles.muted}>
                {fillTemplate(copy.duplicateNotice, { name: state.draft.name })}
            </Text>
            {/* Mounted empty, so Android's live region exists before the failure is written into it. */}
...
/** The open/submitting form, whose heading takes screen-reader focus as it opens. */
function FormBranch({
    state,
    copy,
    actions,
}: {
    readonly state: Exclude<AuthoredFoodCreateFormProps['state'], { kind: 'duplicate' }>;
    readonly copy: IngredientCreateFoodMessages;
    readonly actions: AuthoredFoodCreateFormActions;
}): JSX.Element {
    const headingRef = useScreenReaderFocusOnMount<Text>();
    ...
    return (
        <View style={styles.card} accessibilityLabel={fillTemplate(copy.formTitle, { query: state.draft.name })}>
            ...
            <LiveRegion politeness="assertive" style={styles.error}>
                {state.kind === 'open' && state.submitFailed ? copy.submitFailed : ''}
            </LiveRegion>
