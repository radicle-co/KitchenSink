...
export function IngredientPicker({ onResolve, ref }: IngredientPickerProps): JSX.Element {
    // The search input's handle, for `focusSearch` and the screen-reader focus moves below — the one thing a ref is
    // allowed for (see the web leaf and `useScrollResetOnChange`). No node escapes this leaf.
    const searchRef = useRef<TextInput>(null);
    // Where screen-reader focus returns when the create-food form is cancelled (the form's own control is removed).
    const createFoodTriggerRef = useRef<View>(null);
    // Where screen-reader focus goes when the live-search Try again control — which the retry removes — is pressed.
    const liveSearchTriggerRef = useRef<View>(null);
    ...
    // AFTER AN ADD the picker resets and the pressed row is gone, so the SCREEN-READER cursor returns to the search
    // box, ready for the next ingredient — never `focus()`, which would raise the keyboard over the recipe. An
    // EFFECT, not the resolve callback: on the disambiguation path the search box only returns on this render.
    // UNCONDITIONAL, unlike web's "only if focus was lost": React Native cannot read where the screen-reader cursor
    // is, so there is no way to tell whether the cook moved it elsewhere during a slow add.
    useEffect(() => {
        if (lastAdded !== null) {
            moveScreenReaderFocus(searchRef.current);
        }
    }, [lastAdded]);
     ...
     *
     * ONE panel for every non-idle kind, so its two live regions are mounted from the moment the search starts:
     * Android speaks a CHANGE to a live region it already holds, and a failure region that mounted with its text
     * would not reliably be spoken.
     */
    const liveSearchPanel = ((): JSX.Element | null => {
        ...
        const failure = state.kind === 'busy' ? liveCopy.busy : state.kind === 'failed' ? liveCopy.failed : '';

        return (
            <View
                accessibilityLabel={state.kind === 'results' ? liveCopy.regionLabel : undefined}
                style={styles.livePanel}
            >
                <LiveRegion politeness="polite" style={styles.muted}>
                    {state.kind === 'searching' ? liveCopy.searching : ''}
                </LiveRegion>
                <LiveRegion politeness="assertive" style={styles.error}>
                    {failure}
                </LiveRegion>
                {state.kind === 'searching' && <Text style={styles.liveDetail}>{liveCopy.searchingDetail}</Text>}
                {state.kind === 'results' && (
                    <>
                ...
                {failure !== '' && (
                    <View style={styles.liveHeader}>
                        <Pressable
                            ...
                            onPress={() => {
                                // This control is REMOVED as the retry starts: move the reading cursor first, to the
                                // Search USDA control, which runs the same search and stays mounted reporting busy.
                                moveScreenReaderFocus(liveSearchTriggerRef.current ?? searchRef.current);
                                liveSearch.search();
                            }}
                        >
    ...
    if (viewState.kind === 'disambiguating' || viewState.kind === 'resolving') {
        ...
        return (
            <View accessibilityLabel={title} style={styles.card}>
                ...
                {/* Progress and failure regions stay mounted for the life of the card, empty until they have
                    something to say (Android speaks a change to a region it holds, not one that mounts). */}
                <LiveRegion politeness="polite" style={styles.muted}>
                    {viewState.kind === 'disambiguating' && viewState.isLoading ? t.disambiguateLoading : ''}
                </LiveRegion>
                <LiveRegion politeness="assertive" style={styles.error}>
                    {viewState.kind === 'disambiguating' && viewState.isError ? t.disambiguateError : ''}
                </LiveRegion>
                ...
                <LiveRegion politeness="assertive" style={styles.error}>
                    {viewState.kind === 'disambiguating' && resolveError ? t.resolveError : ''}
                </LiveRegion>
    ...
    return (
        <View accessibilityLabel={t.heading} style={styles.card}>
            ...
            {/* What was just added — ALWAYS MOUNTED, empty until then, because Android speaks a CHANGE to a live
                region it already holds. Visible, not screen-reader-only: the new line lands in the recipe below,
                often off-screen. Cleared by the next keystroke (the hook), so a repeat add is announced again. */}
            <LiveRegion politeness="polite" style={styles.added}>
                ...
                <>
                    <View style={styles.actions}>
                        ...
                        <PressScale
                            ...
                            expanded={createFood.state.kind !== 'closed'}
                            onPress={() => {
                                // A disclosure: the second press collapses what the first expanded (the reading
                                // cursor is already on this control, so nothing needs to move it).
                                if (createFood.state.kind !== 'closed') {
                                    createFood.cancel();

                                    return;
                                }

                                // The search keyboard would otherwise stay up over the form it just opened.
                                Keyboard.dismiss();
                                createFood.open();
                            }}
                        >
            ...
            {createFood.state.kind !== 'closed' && (
                <AuthoredFoodCreateForm
                    ...
                    actions={{
                        cancel: () => {
                            // Before the form unmounts: the control the cook pressed is inside it.
                            moveScreenReaderFocus(createFoodTriggerRef.current ?? searchRef.current);
                            createFood.cancel();
                        },
            ...
            {/* Mounted empty, so Android's live regions exist before progress or a failure is written into them. */}
            <LiveRegion politeness="polite" style={styles.muted}>
                {addByNameStatus.isPending ? t.addingByName : ''}
            </LiveRegion>
            <LiveRegion politeness="assertive" style={styles.error}>
                {addByNameStatus.isError ? t.addByNameError : ''}
            </LiveRegion>
            {/* ⛔ THE FREEFORM CREATE'S OWN FAILURE, which this picker used to swallow. `createStatus` was
                read for its BUSY state only, so a failed create stopped the spinner and said nothing — while
                the web twin has always rendered this. A §14 parity defect, and the one most likely to bite:
                creating a freeform ingredient is the fallback this picker pushes a cook toward when the
                catalog has no match. Assertive, matching its siblings, because it reports an action the cook
                just took failing. */}
            <LiveRegion politeness="assertive" style={styles.error}>
                {createStatus.isError ? t.createError : ''}
            </LiveRegion>
            <LiveRegion politeness="polite" style={styles.muted}>
                {addByFoodStatus.isPending ? t.addingFromCatalog : ''}
            </LiveRegion>
            <LiveRegion politeness="assertive" style={styles.error}>
                {addByFoodStatus.isError ? t.catalogAddError : ''}
            </LiveRegion>
            <LiveRegion politeness="polite" style={styles.muted}>
                {createStatus.isPending ? t.creating : ''}
            </LiveRegion>
        </View>
    );
}
...
const styles = StyleSheet.create({
    ...
    added: { fontSize: 13, color: palette.slate },
    badge: {
        borderRadius: 999,
