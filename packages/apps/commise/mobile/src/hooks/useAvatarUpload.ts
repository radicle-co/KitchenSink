 ...
 *
 * ⛔ REACT-QUERY FRONTS THE CALL (owner directive: every network call goes through it), so an upload in flight is
 * part of the query client's mutation cache like every other write. The seam returns only `upload`: its one
 * caller, `AvatarField`, owns a busy state spanning the picker and the file read as well as the upload, and
 * localized copy for every failure — so a mutation status here would have no reader.
 *
...
export function useAvatarUpload(): UseAvatarUpload {
    ...
    return { upload: mutation.mutateAsync };
}
