 ...
 * DRAFT (`useRecipeDraftPhotos`) and handed to the upload queue by the create mutation's `onSuccess`, which calls
 * the hook's `flush` in the same batch that records the new id. The edit screen never needs this: its recipe
 * already exists, so its uploader enqueues directly. The uploader is shared: only the DESTINATION of a pick
 * differs (`onPick` vs its own queue).
 *
...
export function RecipeCreateScreen({ onCreated, onCancel }: RecipeCreateScreenProps): JSX.Element {
    ...
    const isDirty = useDiscardGuard(values, { justSaved: create.isSuccess });

    /** The created recipe's id, once the POST has returned. `null` until then. */
    const [createdId, setCreatedId] = useState<string | null>(null);

    // Created unconditionally (Rules of Hooks) but fed only by `flush`, which runs in the same batch as `setCreatedId`
    // — the queue's drive effect reads the uploader after that commit, so `''` is never used as a recipe id.
    ...
    const draftPhotos = useRecipeDraftPhotos({
        ...
        // container's twin. The queue's `remaining` already subtracts what is uploading; the draft's own picks
        // come off here.
        capacity: {
            remaining: Math.max(0, queue.remaining - values.photos.length),
            overCapMessage: photoMessages.overCapError,
        },
    });
    ...
    // holds no bytes for — which would otherwise be dropped by handing the id upward with nothing said.
    ...
    const persist = (nextErrors: RecipeFormErrors, status: RecipeStatus): void => {
        ...
        create.mutate(toCreateRecipeInput(values, status), {
            // Records the id and hands the held photos to the queue, in one batch. Handing the id upward is the
            // effect's job, once the photo queue drains. `flush` reads the draft as of this press, which is why the
            // photo slot withholds pick and remove while the create is pending.
            onSuccess: (recipe) => {
    ...
    // Withheld while the create is in flight — see `persist`. The uploader offers no add control without a pick
    // destination, and no draft Remove without a remove handler.
    const photosLocked = create.isPending;
            ...
            photosSlot={
                <RecipePhotoUploader
                    ...
                    {...(photosLocked
                        ? {}
                        : {
                              onPick: (pick: DraftPhotoPick) => draftPhotos.addPhotos([pick]),
                              onRemoveDraft: (index: number) => {
