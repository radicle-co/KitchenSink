            ...
            filterSlot={
                <RecipeFilterBar
                    ...
                    onToggleFacet={(dimension: FacetDimension, value: string) =>
                        setFilters((current) => applyFilterAction(current, { kind: 'toggleFacet', dimension, value }))
                    }
                    onSetCuisine={(cuisine) =>
                        setFilters((current) => applyFilterAction(current, { kind: 'setCuisine', cuisine }))
                    }
                    onSetMaxPrepTime={(minutes) =>
                        setFilters((current) =>
                            applyFilterAction(current, { kind: 'setTimeBound', field: 'maxPrepTime', minutes }),
                        )
                    }
                    onSetMaxCookTime={(minutes) =>
                        setFilters((current) =>
                            applyFilterAction(current, { kind: 'setTimeBound', field: 'maxCookTime', minutes }),
                        )
                    }
                    onSetMaxTotalTime={(minutes) =>
                        setFilters((current) =>
                            applyFilterAction(current, { kind: 'setTimeBound', field: 'maxTotalTime', minutes }),
                        )
                    }
                    ingredientSearch={{
                    ...
                    onAddIngredientFilter={(ingredient) =>
                        setFilters((current) => applyFilterAction(current, { kind: 'addIngredient', ingredient }))
                    }
                    onRemoveIngredientFilter={(id) =>
                        setFilters((current) => applyFilterAction(current, { kind: 'removeIngredient', id }))
                    }
                    onClearAll={() => setFilters(applyFilterAction(EMPTY_RECIPE_FILTERS, { kind: 'clearAll' }))}
                />
            }
        />
    );
