...
export function RecipeEditor({
    ...
    const appendResolved = (line: ResolvedRecipeFormIngredient): void => {
        onChange(applyDraftAction(values, { kind: 'appendResolvedIngredient', line }));
    };
    ...
    return (
        // ⛔ `KeyboardAvoidingView`, which `login.tsx`, `signup.tsx` and `profile.tsx` all have and this screen
        // did not. On iOS the keyboard OVERLAYS the window rather than resizing it, so without this the
        // fields a cook is typing into can sit under it. The compounding factor is specific to this screen:
        // `Wizard.Controls` is deliberately OUTSIDE the ScrollView (U32), pinned to the physical screen
        // bottom — so with the keyboard up it is the FIRST thing the keyboard covers, and step 2's
        // ingredient typeahead has nowhere to render.
        //
        // ⚠️ iOS only, by the same `Platform.OS` test the three sibling screens use: Android's Expo default
        // (`softwareKeyboardLayoutMode: 'resize'`) already shrinks the window, and applying padding on top of
        // that double-counts the inset.
        //
        // ⚠️ REASONED FROM THE PLATFORM AND FROM THIS APP'S OWN PRECEDENT, not observed — there is no
        // simulator here. The three screens that already do this are the evidence that it is the house
        // answer; a device check is still owed before treating it as verified.
