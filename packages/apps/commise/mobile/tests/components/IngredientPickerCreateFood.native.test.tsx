...
/**
 * The screen-reader and keyboard APIs are MOCKED: react-native-web implements neither `sendAccessibilityEvent`
 * nor a keyboard. What these cases prove is the contract with React Native — which node, which event, in which
 * order. Whether VoiceOver/TalkBack honour it on a device is a device check this tier cannot make.
 */
vi.mock('react-native', async (importOriginal) => {
    ...
    return {
        ...
        Keyboard: { ...actual.Keyboard, dismiss: vi.fn() },
    };
});
...
/** The node the most recent screen-reader focus move targeted. */
const lastScreenReaderFocus = (): unknown => vi.mocked(AccessibilityInfo.sendAccessibilityEvent).mock.calls.at(-1)?.[0];
...
describe('IngredientPicker (mobile) — the U16 create-your-own-food vertical', () => {
    ...
    /**
     * ⛔ WEB'S FOCUS MANAGEMENT, ADAPTED — NOT COPIED (staff-ux-engineer). On native the question is where the
     * SCREEN-READER cursor goes, and `autoFocus` would raise a keyboard over the macro fields. So the reading
     * cursor moves and the keyboard stays down.
     */
    it('⛔ opening the form dismisses the search keyboard and moves screen-reader focus to its heading', () => {
        openForm();

        expect(Keyboard.dismiss).toHaveBeenCalled();
        const heading = screen.getAllByRole('heading').find((node) => node.textContent?.includes('grandma blend'));
        expect(heading).toBeDefined();
        expect(lastScreenReaderFocus()).toBe(heading);
    });

    it('the create-food trigger reports its disclosure state', () => {
        render(<IngredientPicker onResolve={vi.fn()} />);
        fireEvent.change(screen.getByLabelText('Search ingredients'), { target: { value: 'grandma blend' } });
        act(() => {
            vi.advanceTimersByTime(INGREDIENT_SEARCH_DEBOUNCE_MS);
        });
        const trigger = screen.getByLabelText('Create your own food');
        expect(trigger.getAttribute('aria-expanded')).toBe('false');

        fireEvent.click(trigger);

        expect(screen.getByLabelText('Create your own food').getAttribute('aria-expanded')).toBe('true');
    });

    /** ⛔ The trigger announces `expanded`, so its second press collapses the form rather than resetting it. */
    it('⛔ pressing the trigger again collapses the form, as its expanded state promises', () => {
        openForm();
        fireEvent.click(screen.getByLabelText('Create your own food'));

        expect(screen.queryByLabelText('Create and add')).toBeNull();
        expect(screen.getByLabelText('Create your own food').getAttribute('aria-expanded')).toBe('false');
    });

    it('⛔ cancel moves screen-reader focus back to the trigger that opened the form', () => {
        openForm();
        fireEvent.click(screen.getByLabelText('Cancel'));

        expect(lastScreenReaderFocus()).toBe(screen.getByLabelText('Create your own food'));
    });

    it('⛔ the duplicate branch moves screen-reader focus onto its notice — which is NOT also a live region', () => {
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(
            settlingMutation({ created: false, reason: 'duplicate', existingFoodId: 'F_prior' }),
        );
        ...
        const notice = screen.getByText('You already have a food named “grandma blend”.');
        expect(lastScreenReaderFocus()).toBe(notice);
        // Focus reads it; a live region too would read it twice.
        expect(notice.hasAttribute('aria-live')).toBe(false);
    });

    it('⛔ failures and progress are SPOKEN on both platforms — live regions, not silent text', () => {
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(settlingMutation(new Error('down')));
        openForm();
        fillMacros();
        fireEvent.click(screen.getByLabelText('Create and add'));
        ...
        expect(
            screen
                .getByText('Could not create the food. Check your connection and try again.')
                .getAttribute('aria-live'),
        ).toBe('assertive');

        cleanup();
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(idleMutation({ mutate: vi.fn(), isPending: true }));
        openForm();

        expect(screen.getByText('Creating your food…').getAttribute('aria-live')).toBe('polite');
    });

    it('a failed REUSE is spoken too', () => {
        useCreateAuthoredFoodViaPickerMock.mockReturnValue(
            settlingMutation({ created: false, reason: 'duplicate', existingFoodId: 'F_prior' }),
        );
        useAddIngredientByFoodMock.mockReturnValue(settlingMutation(new Error('down')));
