...
describe('IngredientPicker (mobile) — the on-demand USDA search affordance (U29)', () => {
    ...
    // ⛔ Web's twin of this case caught a real defect: its action row lived in a different container per local
    // view state, so the local search settling under the USDA hits remounted them and a click already aimed at
    // one was lost. This leaf renders the panel in ONE slot; the case keeps it that way.
    it('⛔ keeps the SAME hit element mounted when the local suggestion settles underneath it', () => {
        const byFood = vi.fn();
        useAddIngredientByFoodMock.mockReturnValue(idleMutation({ mutate: byFood }));
        useSearchIngredientsLiveMock.mockReturnValue(
            liveMutation({ data: { hits: [{ name: 'Broccoli, raw', foodId: 'food_1' }] } }),
        ...
        fireEvent.click(screen.getByLabelText('Search USDA for “broccoli”'));
        const hit = screen.getByText('Broccoli, raw');
    ...
    /**
     * ⛔ The web panel's lines are `role="status"` / `role="alert"`, which a browser speaks. A React Native `Text`
     * with `accessibilityRole="alert"` is not announced by VoiceOver, so the searching line and both failures are
     * `LiveRegion`s — and the failure region is already mounted while the search runs, because Android speaks a
     * CHANGE to a live region it holds, not one that mounts with its text.
     */
    it('⛔ the searching line is a polite live region, and a failure an assertive one mounted before it arrives', () => {
        const { rerender } = render(<IngredientPicker onResolve={vi.fn()} />);
        fireEvent.change(screen.getByLabelText('Search ingredients'), { target: { value: 'broccoli' } });
        act(() => {
            vi.advanceTimersByTime(INGREDIENT_SEARCH_DEBOUNCE_MS);
        ...
        useSearchIngredientsLiveMock.mockReturnValue(liveMutation({ isPending: true }));
        rerender(<IngredientPicker onResolve={vi.fn()} />);
        expect(screen.getByText('Searching the USDA database…').getAttribute('aria-live')).toBe('polite');
        const assertiveWhileSearching = [...document.querySelectorAll('[aria-live="assertive"]')];

        useSearchIngredientsLiveMock.mockReturnValue(liveMutation({ error: { name: 'SourceUnavailableError' } }));
        rerender(<IngredientPicker onResolve={vi.fn()} />);
        expect(assertiveWhileSearching).toContain(screen.getByText(/didn’t answer/u));

        useSearchIngredientsLiveMock.mockReturnValue(liveMutation({ error: { name: 'SourceBusyError' } }));
        rerender(<IngredientPicker onResolve={vi.fn()} />);
        expect(screen.getByText(/rate-limited/u).getAttribute('aria-live')).toBe('assertive');
    });
    ...
    /**
     * ⛔ The Try again control is REMOVED as the retry starts, so the screen-reader cursor would be left on nothing.
     * It moves first — to the Search USDA control, which runs the same search, stays mounted and reports busy.
     * Screen-reader focus only: the keyboard is not raised.
     */
    it('⛔ pressing Try again moves screen-reader focus to the Search USDA control BEFORE the retry starts', () => {
        const mutate = vi.fn();
        useSearchIngredientsLiveMock.mockReturnValue(
            liveMutation({ mutate, error: { name: 'SourceUnavailableError' } }),
        );
        ...
        expect(focus).toHaveBeenLastCalledWith(screen.getByLabelText('Search USDA for “broccoli”'), 'focus');
        expect(focus.mock.invocationCallOrder.at(-1)).toBeLessThan(mutate.mock.invocationCallOrder.at(-1) ?? 0);
    });
