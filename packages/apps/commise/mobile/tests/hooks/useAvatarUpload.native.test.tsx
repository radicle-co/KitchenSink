 ...
 * under `.maestro/` is navigational (`accountDangerZone.yaml` reaching ProfileScreen via the Profile tab). So
...
/**
 * Render the hook under a QueryClientProvider.
 *
 * ⚠️ REQUIRED since the upload became a react-query mutation (owner directive: react-query fronts every
 * network call). Retries are off so a rejected upload rejects once, which is what the `rejects` cases below
 * assert — the default mutation retry is already 0, pinned here so a config change cannot make them hang.
 */
...
describe('useAvatarUpload', () => {
    ...
    /**
     * ⛔ THE POINT OF FRONTING THIS WITH REACT-QUERY: the upload is a mutation, so whether one is in flight is part
     * of the query client's cache — observable to any surface (`useIsMutating`) — not a component's private state.
     *
     * ⚠️ The in-flight window is held open by a DEFERRED presign rather than by polling for it. Asserting
     * `isPending` against a promise that settles immediately is a race: `waitFor` can easily first observe the
     * settled state, which is how a test like this becomes flaky under parallel load.
     */
    it('exposes the upload as an observable mutation — pending while in flight, settled after', async () => {
        ...
        fetchMock.mockImplementation(async (url: string | URL) => {
            ...
            if (target.endsWith('/health')) {
                return new Response(
                    JSON.stringify({ status: 'ok', service: 'identity', contractHash: CONTRACT_HASH }),
                    {
                        status: 200,
                    },
                );
            ...
            if (target.includes('/avatar/presign')) {
                ...
                return new Response(JSON.stringify(PRESIGNED), { status: 200 });
            }

            return new Response(null, { status: 200 });
        });

        const { client, result } = renderUploadHook();

        expect(client.isMutating()).toBe(0);

        const blob = new Blob(['x'], { type: 'image/png' });
        const inFlight = result.current.upload({ blob, contentType: 'image/png' });

        await waitFor(() => expect(client.isMutating()).toBe(1));

        releasePresign();

        await expect(inFlight).resolves.toBe(PRESIGNED.publicUrl);
        await waitFor(() => expect(client.isMutating()).toBe(0));
    });

    /** A failure is RECORDED on the mutation, not only thrown — the other half of what the cache buys. */
    it('records a failed upload as the mutation error, not only as a rejection', async () => {
        routeFetch({ contractHash: CONTRACT_HASH, put: { status: 500 } });

        const { client, result } = renderUploadHook();
        const blob = new Blob(['x'], { type: 'image/png' });

        await expect(result.current.upload({ blob, contentType: 'image/png' })).rejects.toThrow(/500/u);
        await waitFor(() => expect(client.getMutationCache().getAll()[0]?.state.error).toBeInstanceOf(Error));
    });
});
