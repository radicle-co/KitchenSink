 ...
 *     changes on every "load more", which re-batches page one for nothing — gives each browse rail its own
 *     batch, and asks nothing about the results hidden behind the rails.
...
const { getCollectionByIdMock, listRecipesMock, searchRecipesMock } = vi.hoisted(() => ({
 ...
 * `useBrowseRailsRefresh`, `useDebouncedValue`, `useDeferredDiscoveryCriteria`, `useIngredientFilterSearch`,
 * `useRecentSearches`) stays REAL, so a screen cannot pass this file by losing one of them.
 */
...
beforeEach(() => {
    ...
    // The recipe list and the Home slot both read through the client's `listRecipes`, via the query cache.
    listRecipesMock.mockResolvedValue(makeRecipePage([makeRecipe({ id: RECIPE_ID, title: 'Weeknight Pasta' })]));

    // Discovery searches through the client's `searchRecipes`, via the query cache.
...
/** A query cache around `element` — fresh unless the test seeded one; every wired screen reads through it. */
...
const searchKeys = recipeQueries({} as RecipeServiceClient);

/** A settled infinite search whose pages each carry the given recipes. */
function settledSearch(...pages: readonly (readonly string[])[]) {
    return {
        pages: pages.map((ids) =>
            makeSearchResponse(ids.map((id) => makeRecipeSearchResult({ id, title: `Dish ${id.slice(-1)}` }))),
        ),
...
const SCREENS: readonly (readonly [string, () => ReactElement])[] = [
    ['RecipeListScreen', () => withQueryClient(<RecipeListScreen onSelectRecipe={noop} />)],
    ...
    [
        'RecipeWidgetSlot (Home)',
        () => withQueryClient(<RecipeWidgetSlot onSeeAllRecipes={noop} onSelectRecipe={noop} />),
    ],
];
...
describe('which ids each screen batches', () => {
    ...
    it('⛔ RecipeListScreen batches the LOADED PAGE, not the rows left after client-side filtering', async () => {
        ...
        await act(async () => {
            render(withQueryClient(<RecipeListScreen onSelectRecipe={noop} />));
        });
    ...
    it('⛔ RecipeDiscoveryScreen batches ONE PAGE PER FETCHED PAGE, never a flattened list', async () => {
        ...
        seeded.setQueryData(
            ...
            settledSearch([RECIPE_ID], [OTHER_ID]),
        );
    ...
    it('⛔ CollectionDetailScreen batches EVERY member, not just the revealed window', async () => {
        ...
        getCollectionByIdMock.mockResolvedValue(makeCollectionWithRecipes(members));
    ...
    it('⛔ RecipeDiscoveryScreen wires its BROWSE RAILS too — ONE batch per rail, none for the hidden results', async () => {
        // No query and no filters ⇒ the screen is BROWSING, and the rails (not the result list) are what a viewer sees
        // first. Each rail reads behind its own boundary and runs its own batch; forgetting its renderer would leave
        // Discover's opening view the one card grid with no figure. The main search still runs behind the rails (it
        // feeds the cuisine shortcuts), but its cards are hidden, so it must ask about none of them. The searches are
        // SEEDED as settled: settling three sibling suspense reads inside `act` is a harness loop, not the behaviour here.
        const seeded = new QueryClient({ defaultOptions: { queries: { retry: false, staleTime: Infinity } } });
        seeded.setQueryData(
            searchKeys.searchInfinite(
                discoverySearchParams({ filters: {}, query: '', sortBy: RecipeSearchSortBy.RELEVANCE }),
            ).queryKey,
            settledSearch([OTHER_ID]),
        );

        for (const rail of RECIPE_BROWSE_RAILS) {
            seeded.setQueryData(
                searchKeys.searchInfinite(browseRailSearchParams(rail)).queryKey,
                settledSearch([RECIPE_ID]),
            );
        }
        ...
        expect(
            useNutritionMock.mock.calls.filter(([pages]) => pages.some((page) => page.includes(OTHER_ID))),
            'the results hidden behind the rails are not batched',
        ).toHaveLength(0);
    });

    it('RecipeWidgetSlot batches the recent recipes it renders', async () => {
        await act(async () => {
            render(withQueryClient(<RecipeWidgetSlot onSeeAllRecipes={noop} onSelectRecipe={noop} />));
        });
