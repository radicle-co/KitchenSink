...
/**
 * Publishes from a COMMITTED render only. Assigning the test's binding during render would be the very impurity
 * `react-hooks/globals` forbids, and it would also publish the handler from a render React later discards.
 */
function publish(handler: (event: Event) => void): void {
    restoreFocus = handler;
}

function Probe({ open }: { readonly open: boolean }): null {
    const handler = useReturnFocusOnClose(open);

    useEffect(() => publish(handler), [handler]);
