...
vi.mock('@radicle-co/infra-shared/security', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@radicle-co/infra-shared/security')>();
 ...
 *
 * `teardownSandboxPr.sh` deletes a closed PR's resources with NO denylist. It decides ownership two ways —
 * a `pr-{N}` NAME (a delimiter-aware PREFIX rule, `prScope.sh`) or an `Environment` TAG whose value the PR
 * owns — and the tag is the only one of those that generalises: §2 reads it back per stack from
 * `describe-stacks`, and §3 sweeps `resourcegroupstaggingapi get-resources` for every value this PR can own
 * that no stack owned.
 ...
 *
 * ## Why the persistent TIER word, and why it is DERIVED rather than written down
 *
 * `sandbox-router-deploy.yml` — the ONLY deployer of this app — pins `STAGE: sandbox`, so the tag's value at
 * every real deploy is `sandbox`: the tier this singleton lives in, and a value no `pr-{N}` token can claim
 * (every matcher is keyed on a token, a token always carries digits, and `sandbox` carries none). That is
 * correct, because this is a persistent SINGLETON. Its out-of-band deletion once left `sandbox.commise.app`
 * NXDOMAIN for ~3 weeks with every web preview broken, which is exactly the outcome a PR-close sweep must
 ...
 * wrong are not symmetric. A hardcoded tier word on a per-PR deploy would make that stack IMMORTAL — and the
...
describe('the sandbox router CDK app (infra/bin/app.ts) — ADR-0005 Environment tag', () => {
    ...
    async function synthesizeEntrypoint(stage: string): Promise<{
        ...
        // The entrypoint builds its App the way a real deploy does — with no `outdir` — so CDK mkdtemps one
        // under the OS temp directory and nothing would ever remove it. That is correct for the entrypoint
        // and this suite's business to clean up: it is the caller that made the App exist.
        synthOutputs.push(app.outdir);

        // By DEPLOYED stack name, not by construct id: that is the identity `describe-stacks` reports and
        // `teardownSandboxPr.sh` matches on, so looking it up this way pins the name convention too.
    ...
    it('tags the deployed router Environment=sandbox — the value every real deploy produces', async () => {
        // `sandbox-router-deploy.yml` sets STAGE: sandbox, so this is the tag that actually reaches the
        // account. It names the persistent TIER, which is what keeps the singleton out of every per-PR
        // teardown match (ADR-0005) — not because the word means "protected", but because no `pr-{N}`
        // token can produce it.
        const { tags } = await synthesizeEntrypoint('sandbox');

        expect(tags['Environment']).toBe('sandbox');
    });
    ...
    it('DERIVES the tag from the stage, so a pr-{N} deploy is reclaimable rather than immortal', async () => {
        ...
        expect(tags['Environment']).toBe('pr-9-sandbox');
    });
    ...
    it('tags the RESOURCES too, not just the stack — what the §3 resourcegroupstaggingapi sweep reads', async () => {
        ...
        template.hasResourceProperties('AWS::CloudFront::Distribution', {
            Tags: Match.arrayWith([{ Key: 'Environment', Value: 'sandbox' }]),
        });
    });
});
