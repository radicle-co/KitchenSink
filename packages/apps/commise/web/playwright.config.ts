...
export default defineConfig({
    ...
    // A screenshot baseline's FILE NAME obeys CODING_STANDARDS §1 (no hyphens). Playwright's default template is
    // `{arg}{-projectName}{-snapshotSuffix}{ext}`, which hyphenates every baseline it writes; this is that same
    // template with `.` as the separator, so the arg, project and platform still all appear in the name and a
    // baseline is still per-project and per-platform (`recipeDetailDesktop.chromium.linux.png`).
    expect: {
        toHaveScreenshot: {
            pathTemplate:
    ...
    // baselines (`recipeDetailDesktop.chromium.linux.png`), so renaming it — or moving a spec that owns a
    ...
    // signs in as its OWN fixed test-pool slot (`COMMISE_E2E_SHARD` → `webPoolSlot`) and derives its own run key,
    // so a shard must authenticate as ITS user and write ITS own state file (`authStatePath` is run-key scoped).
    // One shared file would hand shard 2 a session belonging to shard 1's slot.
    projects: [
        {
            name: 'setup',
