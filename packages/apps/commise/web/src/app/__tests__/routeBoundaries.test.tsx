...
describe.each(ERROR_BOUNDARIES)('%s/error.tsx', (_segment, ErrorBoundary, expectedRoute) => {
    it('renders the shared error state, retries via retry() — which re-fetches — and reports via DA9', async () => {
        const user = userEvent.setup();
        const retry = vi.fn();
        const reset = vi.fn();
        const error = Object.assign(new Error('boom'), { digest: 'd1' });

        // Next hands `error.tsx` both; only `retry` re-fetches, so `reset` must stay unused.
        renderWithProviders(<ErrorBoundary error={error} retry={retry} {...{ reset }} />);
 ...
 * (`CollectionListResults.tsx`), so the recipe skeleton's four-column rhythm would reflow rather than reserve — the
