...
export function RecipeWidgetSlot(): JSX.Element {
    ...
    // One stable promise per client and cache, so neither the nutrition container's `<Suspense>` nor the widget's
    // re-suspends on every host re-render (a fresh promise each render would reset both boundaries and flash the
    // skeleton back over a settled card). The read goes through the shared query cache (§11.0): a page the cache
    // already holds costs no request, and the read is visible to the invalidations every recipe write issues.
    //
    // The viewer's Clerk token is client-only, so the fetch must not run during SSR: the request would be wasted,
    // and a server-side rejection — the token being unavailable — is not something the browser tree ever gets a
    // chance to retry. Resolve to empty on the server; the real fetch runs once, in the browser.
    const recipesPromise = useMemo<Promise<readonly Recipe[]>>(
        () =>
            typeof window === 'undefined'
                ? Promise.resolve<readonly Recipe[]>([])
                : queryClient
                      .ensureQueryData(recipeQueries(client).list({ pageSize: RECENT_RECIPE_LIMIT }))
                      .then((page) => page.data),
        [client, queryClient],
    );
