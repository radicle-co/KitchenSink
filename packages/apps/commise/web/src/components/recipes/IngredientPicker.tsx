 ...
 *     nothing. Its imperative seams exist only to MOVE FOCUS: a Facade publishing `focusSearch` (never the node
 *     itself), returning focus to the create-food trigger when that form is cancelled, to the Search USDA control
 *     when Try again is pressed, and to the search box after an add.
 */
import {
    fillTemplate,
...
import { useEffect, useId, useImperativeHandle, useRef, type FC, type JSX, type Ref } from 'react';
...
export const IngredientPicker: FC<IngredientPickerProps> = ({ onSelect, ref }) => {
    // ⛔ The refs in this leaf exist for the one thing a ref is allowed for: there is no declarative way to
    // move focus. Neither leaves this component — `useImperativeHandle` publishes a method, not the node.
    // React 19's plain `ref` prop, so no `forwardRef` wrapper is needed.
    const searchRef = useRef<HTMLInputElement>(null);
    // WCAG 2.2 SC 2.4.3: cancelling the create-food form removes the element holding focus, so focus goes back
    // to the disclosure that opened it (or to the search box, when a query edit has since unmounted that).
    // Opening needs no ref — the form's first field takes focus as it mounts.
    const createFoodTriggerRef = useRef<HTMLButtonElement>(null);
    const createFoodFormId = useId();
    // The picker's own region, which owns the after-add focus move (see `focusIfLost`).
    const regionId = useId();
    // Where focus goes when the live-search Try again button — which the retry removes — is pressed.
    const liveSearchTriggerRef = useRef<HTMLButtonElement>(null);
    ...
    const {
        ...
        ingredientPickerStatus: addedCopy,
    ...
    // AFTER AN ADD the picker resets to a blank search and the pressed control is gone, so focus returns to the
    // search box — ready for the next ingredient (WCAG 2.2 SC 2.4.3). An EFFECT, not the resolve callback: on the
    // disambiguation path the search box is unmounted when the line resolves and only returns on this render.
    // Only when focus was LOST (on <body>) or is still inside the picker — a cook who moved elsewhere during a slow
    // add keeps their place, and a parent whose `onSelect` moved focus has already won by the time this runs.
    useEffect(() => {
        if (lastAdded === null) {
            return;
        }

        focusIfLost(searchRef.current, document.getElementById(regionId));
    }, [lastAdded, regionId]);
    ...
    const correctionControl = (foodId: string | undefined): JSX.Element | null => {
        ...
        return (
            <button
                type="button"
                {...busyControlProps({ busy: correction.isSaving, onClick: () => correction.correct(trimmed, foodId) })}
                className={`shrink-0 rounded-full border border-border px-2 py-0.5 text-caption text-slate transition hover:bg-pearl ${BUSY_CONTROL_CLASS}`}
            >
    ...
    const catalogRow = (suggestion: Extract<IngredientSuggestion, { provenance: 'catalog' }>): JSX.Element => (
        <li key={suggestionKey(suggestion)} className="flex items-center gap-2">
            <button
                type="button"
                {...busyControlProps({ busy: addByFoodStatus.isPending, onClick: () => selectSuggestion(suggestion) })}
                className={`flex-1 rounded-lg px-3 py-2 text-left text-body-md text-charcoal transition hover:bg-pearl ${BUSY_CONTROL_CLASS}`}
            >
                {suggestion.name}
            </button>
                        ...
                        {state.hits.map((hit) => (
                            <li key={`${hit.name}:${hit.foodId ?? ''}`} className="flex items-center gap-2">
                                <button
                                    type="button"
                                    {...busyControlProps({
                                        busy: addByFoodStatus.isPending || addByNameStatus.isPending,
            ...
            <div className="flex flex-col gap-1 rounded-xl border border-border bg-card p-2">
                ...
                <div className="flex items-center gap-2 px-2">
                    <button
                        type="button"
                        onClick={() => {
                            // This button is REMOVED as the retry starts, so focus moves first — to the Search USDA
                            // control, which runs the same search, stays mounted and announces busy (WCAG 2.2 SC
                            // 2.4.3). Falls back to the search box when that control is not offered.
                            (liveSearchTriggerRef.current ?? searchRef.current)?.focus();
                            liveSearch.search();
                        }}
                        // Native `disabled` is right HERE: this control never goes unavailable because it was
                        // pressed — the press removes it — so there is no focused control for `disabled` to strand.
     ...
     * the control the cook pressed stays the control they are waiting on (unavailable, but still focusable).
    ...
    const actionRow = (
        <>
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                    type="button"
                    {...busyControlProps({ busy: addByNameStatus.isPending, onClick: findNutrition })}
                ...
                <button
                    type="button"
                    {...busyControlProps({ busy: createStatus.isPending, onClick: addFreeform })}
                ...
                <button
                    ref={createFoodTriggerRef}
                    type="button"
                    // A disclosure: the second press collapses what the first expanded, and focus stays here. It
                    // cannot collapse a form whose create is in flight, and says so while that lasts.
                    {...refusedPressProps({
                        unavailable: createFood.state.kind === 'submitting',
                        onClick: createFood.state.kind === 'closed' ? createFood.open : createFood.cancel,
                    })}
                    aria-expanded={createFood.state.kind !== 'closed'}
                    aria-controls={createFoodFormId}
                    className={`rounded-full bg-seafoam/10 px-4 py-1.5 text-body-sm font-medium text-ocean-dark transition hover:bg-seafoam/20 ${BUSY_CONTROL_CLASS}`}
                >
            ...
            {/* U29: the ON-DEMAND source search. ⛔ A button, never a keystroke trigger — see the module doc
                ...
                ⚠️ ONE control that goes UNAVAILABLE while its search runs, never a second element swapped in: a
                swap detaches the node a screen reader (and a test) is holding, and re-announces a control
                the cook never left. */}
            {isLiveSearchOffered && (
                <button
                    ref={liveSearchTriggerRef}
                    type="button"
                    // The control the cook just pressed stays focusable while its search runs, and refuses a second
                    // press in the DOM as well as in `search` itself (`canSearch`).
                    {...refusedPressProps({ unavailable: !liveSearch.canSearch, onClick: liveSearch.search })}
                    aria-busy={liveSearch.state.kind === 'searching'}
                    className={`flex w-full items-center gap-2 rounded-lg border border-border px-3 py-2 text-left text-body-sm text-slate transition hover:bg-pearl ${BUSY_CONTROL_CLASS}`}
                >
            ...
            {/* The results come AFTER the control that asked for them, as on mobile: above it, their arrival
                pushed the button the cook had just pressed down the page. */}
            {liveSearchPanel}
    ...
    return (
        <section id={regionId} aria-label={picker.regionLabel} className="flex flex-col gap-2">
            ...
            {/* What was just added — ALWAYS MOUNTED, because a live region has to exist before its text changes to be
                announced. Visible, not screen-reader-only: the new line lands in a list below, often off-screen.
                `aria-live` rather than `role="status"`, so the picker's other status regions stay distinguishable.
                Cleared by the next keystroke (the hook), so the same food added again is announced again. */}
            <p aria-live="polite" className="px-2 text-body-sm text-slate empty:sr-only">
                {lastAdded === null ? '' : fillTemplate(addedCopy.added, { name: lastAdded })}
            </p>
                            ...
                            {viewState.candidates.map((candidate) => (
                                <li key={candidate.candidateId}>
                                    <button
                                        type="button"
                                        {...busyControlProps({
                                            busy: viewState.kind === 'resolving',
                                            onClick: () => pickCandidate(candidate.candidateId),
                                        })}
                                        className={`w-full rounded-lg px-3 py-2 text-left text-body-md text-charcoal transition hover:bg-pearl ${BUSY_CONTROL_CLASS}`}
                                    >
                                        {candidate.name}
            ...
            {/* ⛔ ONE container for every view that carries the action row, never one per view kind. The local
                suggestion is debounced behind typing and routinely settles AFTER the cook has pressed "Search
                USDA" — and when `searching`, `terminal` and `results` each rendered their own container, that
                settle unmounted the whole action row and mounted a copy: a click already aimed at a USDA hit
                hit a detached node and was lost, and keyboard focus on it fell to <body>. Only the view-specific
                content switches below; the container and `actionRow` keep their place in the tree. */}
            {(viewState.kind === 'searching' || viewState.kind === 'terminal' || viewState.kind === 'results') && (
                <div className="flex flex-col gap-2 rounded-xl border border-border bg-card p-2 shadow-sm">
                    ...
                    {viewState.kind === 'searching' && <IngredientRowsSkeleton label={picker.searching} />}
                        ...
                        <>
                            ...
                            {correctionNotice !== undefined && (
                                <p
                                    ...
                                    aria-label={
                                        correctionNotice.tone === 'error' ? undefined : correctionMessages.regionLabel
                                    }
            ...
            {createFood.state.kind !== 'closed' && (
                <AuthoredFoodCreateForm
                    ...
                    actions={{
                        cancel: () => {
                            (createFoodTriggerRef.current ?? searchRef.current)?.focus();
                            createFood.cancel();
                        },
