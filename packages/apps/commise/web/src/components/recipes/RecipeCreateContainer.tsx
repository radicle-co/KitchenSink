...
/**
 * Container for the recipe-create route (w3/e1,e2 on the 4-step `Wizard` shell; U28/U32/U33/U34). Owns the
 ...
 *
 * ⛔ **INGREDIENTS ARE PICKER-FIRST (U28).** The only way a line enters `values.ingredients` here is
 * `appendResolvedIngredient`, whose parameter is a `ResolvedRecipeFormIngredient` — an unresolved row is not
 * a value this container can construct. "+ Add ingredient" inside the fields leaf raises a REQUEST which this
 * container answers by focusing the picker it owns, because a controlled presentational leaf cannot.
 *
 ...
 * the DRAFT (`useRecipeDraftPhotos`) and handed to the upload queue by the create mutation's `onSuccess`,
 * which calls the hook's `flush` in the same batch that records the new id. The edit route never needs this:
 * its recipe already exists, so its uploader enqueues directly.
 *
...
export const RecipeCreateContainer: FC<RecipeCreateContainerProps> = ({ locale }) => {
    ...
    /** The created recipe's id, once the POST has returned. `null` until then. */
    const [createdId, setCreatedId] = useState<string | null>(null);
    /** A post-save panel pick the queue refused for want of slots — nothing was queued, so no item carries it. */
    const [panelCapError, setPanelCapError] = useState<string | undefined>(undefined);

    const listRoute = `/${locale}/recipes` as Route;
    const detailRoute = createdId === null ? listRoute : (`/${locale}/recipes/${createdId}` as Route);

    // The upload queue is created UNCONDITIONALLY (Rules of Hooks) but is fed only once `createdId` is set: by
    // `flush`, in the same batch as `setCreatedId`, or by a pick on the post-save panel. The queue's drive effect
    // reads the uploader after that commit, so `''` is never used as a recipe id on any request.
    ...
    const draftPhotos = useRecipeDraftPhotos({
        ...
        // all. The queue's `remaining` already subtracts what is uploading; the draft's own picks come off here.
        capacity: {
            remaining: Math.max(0, queue.remaining - values.photos.length),
            overCapMessage: recipes.photos.overCapError,
        },
    });
    ...
    const addIngredient = (line: ResolvedRecipeFormIngredient): void => {
        setValues((current) => applyDraftAction(current, { kind: 'appendResolvedIngredient', line }));
    };
    ...
    const isDirty = useDiscardGuard(values, { justSaved: createRecipe.isSuccess });

    const pendingUploads = visibleQueueItems(queue.items);
    // ⛔ BOTH halves. The queue half holds the photos `flush` handed over, which land in the same commit as the id.
    // The DRAFT half holds what `flush` could NOT hand over — a refusal from the queue, or a descriptor the hook
    // holds no bytes for — which would otherwise be dropped by navigating away with nothing said.
    ...
    const handleFileChange = (event: ChangeEvent<HTMLInputElement>): void => {
        const picks = Array.from(event.target.files ?? []).map((file) => {
        ...
        // ⛔ Two destinations, decided by whether the recipe exists yet. Before the create a pick is held in the draft
        // and handed over by the create's `flush`. On the post-save panel that flush has already run and will not run
        // again, so a pick held in the draft would sit "Queued" forever and pin the panel open — it goes straight to
        // the queue instead, the way the edit route's uploader enqueues.
        if (createdId === null) {
            draftPhotos.addPhotos(picks);
        } else if (picks.length > 0) {
            const admission = queue.enqueue(picks);

            setPanelCapError(
                admission.status === 'overCap'
                    ? fillTemplate(recipes.photos.overCapError, { count: admission.remaining })
                    : undefined,
            );
        }
    ...
    const persist = (nextErrors: RecipeFormErrors, status: RecipeStatus): void => {
        ...
        createRecipe.mutate(toCreateRecipeInput(values, status), {
            // Records the id and hands the held photos to the queue, in one batch. Navigation is the effect's job,
            // once the photo queue has drained. `flush` reads the draft as of this press, which is why photo add
            // and remove are withheld while the create is pending (see `photosLocked`).
            onSuccess: (created) => {
    ...
    // Withheld while the create is in flight: `flush` hands over the draft as it stood when Publish was pressed, so a
    // pick added now would never be handed over and a pick removed now would upload anyway.
    const photosLocked = createRecipe.isPending;
    ...
    const photoError = draftPhotos.capError ?? panelCapError ?? uploader.errorMessage;
    ...
    const removeQueueItem = (fileId: number): void => {
        if (fileId >= 0) {
            queue.remove(fileId);
            // A slot just came free, so a panel refusal's count is no longer true.
            setPanelCapError(undefined);
    ...
    const canAddPhoto = !photosLocked && !isAtPhotoCap(values.photos.length + visibleQueueItems(queue.items).length);
    ...
    const photoManager = (
        <RecipePhotoManager
            ...
            {...(photosLocked ? {} : { onRemoveQueueItem: removeQueueItem })}
