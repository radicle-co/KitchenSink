...
describe('the correction control — every state it can be in', () => {
    ...
    async function correctWith(answer?: CorrectionAnswer): Promise<void> {
        await correctWithSpy(answer);
    }

    /** {@link correctWith}, handing back the correction spy so a test can count the writes. */
    async function correctWithSpy(answer?: CorrectionAnswer): Promise<MockInstance> {
        const client = await typeQuery([catalogRow]);
    ...
    /**
     * REWRITTEN from `toBeDisabled()`: native `disabled` on the control the cook just pressed drops focus to <body>
     * in a real browser (WCAG 2.2 SC 2.4.3). The control stays focusable and `aria-disabled`, and the correction
     * controller refuses the second press — so the double-submit guarantee is asserted on the write count.
     */
    it('announces the in-flight write and BUSIES the control, so it cannot be double-submitted', async () => {
        const spy = await correctWithSpy();

        expect(await screen.findByText(m.saving)).toBeInTheDocument();
        const control = await correctionControlFor(catalogRow.name);
