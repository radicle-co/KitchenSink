...
describe('IngredientPicker — the on-demand USDA search affordance (U29)', () => {
    ...
    it('disables the affordance while a search is running, so one press cannot spend the lane twice', async () => {
        ...
        // REWRITTEN from `toBeDisabled()`: native `disabled` on the control the cook just pressed drops focus to
        // <body> in a real browser (WCAG 2.2 SC 2.4.3). It is `aria-disabled` instead — still announced as
        // unavailable — and the hook refuses the second press, which the call count below still proves.
        await waitFor(() => {
    ...
    it('picks a hit we already hold, resolving the line without a second source call', async () => {
        ...
        const byFood = vi.spyOn(client, 'addIngredientByFood').mockResolvedValue(
            makeIngredient({
                id: 'ing_1',
                name: 'Broccoli, raw',
    ...
    /**
     * ⛔ THE HIT A COOK IS HOLDING SURVIVES THE LOCAL SEARCH SETTLING UNDERNEATH IT.
     *
     * The local suggestion is debounced behind typing, so it routinely lands AFTER the USDA hits are on screen.
     * The action row — hits included — used to be rendered inside a DIFFERENT container per local view state
     * (`searching`, `terminal`, `results`), so that settle unmounted the whole row and mounted a fresh copy: a
     * click already aimed at a hit went to a detached node and was lost, and keyboard focus on it fell to
     * `<body>`. The previous case only flaked on it under load, when its steps took long enough for the
     * 300 ms debounce to land mid-test. This one holds the local search open on purpose, so the ordering is
     * pinned rather than left to the clock.
     */
    it('⛔ keeps the SAME hit button, and focus on it, when the local suggestion settles underneath it', async () => {
        const onSelect = vi.fn();
        const client = createFakeRecipeServiceClient();
        let settleLocal: (value: { suggestions: never[]; catalogAvailability: 'ok' }) => void = () => undefined;
        vi.spyOn(client, 'suggestIngredients').mockReturnValue(
            new Promise((resolve) => {
                settleLocal = resolve;
            }),
        );
        ...
        vi.spyOn(client, 'addIngredientByFood').mockResolvedValue(
            makeIngredient({ id: 'ing_1', name: 'Broccoli, raw', foodResolutionStatus: FoodResolutionStatus.RESOLVED }),
        );
        const user = userEvent.setup();

        renderWithRecipeClient(<IngredientPicker onSelect={onSelect} />, client);
        await typeQuery(user, 'broccoli');
        // The local search is IN FLIGHT: the debounce has fired and its answer is being held back.
        await waitFor(() => {
            expect(client.suggestIngredients).toHaveBeenCalled();
        });
        await user.click(await screen.findByRole('button', { name: actionName('broccoli') }));
        const hit = await screen.findByRole('button', { name: 'Broccoli, raw' });
        hit.focus();

        settleLocal({ suggestions: [], catalogAvailability: 'ok' });
        await screen.findByText('No matching ingredients found.');
    ...
    it('renders the USDA results AFTER the control that asked for them, so pressing it never pushes it down', async () => {
        // Web↔mobile parity (staff-ux-engineer): mobile places the live panel after its control. Above it, the
        // results landing shoved the button the cook just pressed further down the page.
        const { client, user } = mountWithEmptyLocal();
        vi.spyOn(client, 'searchIngredientsLive').mockResolvedValue({
            hits: [{ name: 'Broccoli, raw', foodId: 'food_1' }],
        });
        ...
        const action = await screen.findByRole('button', { name: actionName('broccoli') });
        await user.click(action);
        const results = await screen.findByRole('region', { name: 'USDA search results' });

        expect(action.compareDocumentPosition(results) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    });
    ...
    it('⛔ pressing Try again hands focus to the Search USDA control, which stays mounted through the retry', async () => {
        // The Try again button is REMOVED as the retry starts, so focus would fall to <body> (WCAG 2.2 SC 2.4.3).
        // The Search USDA control runs the same search, stays mounted and announces busy — and it is the one
        // control still on screen whatever the retry's outcome (staff-ux-engineer).
        const { client, user } = mountWithEmptyLocal();
        vi.spyOn(client, 'searchIngredientsLive')
            .mockRejectedValueOnce(new SourceUnavailableError())
            .mockReturnValueOnce(new Promise(() => undefined));

        renderWithRecipeClient(<IngredientPicker onSelect={vi.fn()} />, client);
        await typeQuery(user, 'broccoli');
        const trigger = await screen.findByRole('button', { name: actionName('broccoli') });
