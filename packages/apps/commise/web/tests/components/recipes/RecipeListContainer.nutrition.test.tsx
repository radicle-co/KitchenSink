...
describe('RecipeListContainer — the deferred calorie lookup', () => {
    ...
    it('renders every card WITHOUT a figure or a spinner once a REJECTED batch has exhausted its retry', async () => {
        ...
        try {
            ...
            renderWithRecipeClient(<RecipeListContainer locale="en" />, client);
            // TWO turns, in order. The list is a SUSPENSE read, so its rows — and with them the batch — commit only when
            // `act` flushes; a single advance would spend its whole window before the first attempt existed, and the
            // retry scheduled behind it would never run.
            await act(async () => {
...
/**
 * The SERVER half. `/recipes` prefetches the list, so its rows render during the Next server pass — and a nutrition
 * read started there runs with a token source that has no session, and suspends a card boundary the server cannot
 * finish. React then client-renders every such boundary on hydration and reports each one as a recoverable error:
 * one phantom error per card, per page view, drowning the signal a real SSR crash would send.
 */
describe('RecipeListContainer — the deferred calorie lookup across the server render', () => {
    /** The request's cache after a SUCCESSFUL `/recipes` prefetch. */
    ...
    function page(client: ReturnType<typeof clientWithNutrition>['client'], queryClient: QueryClient): ReactNode {
        return (
    ...
    it('⛔ a server render of the prefetched rows starts NO nutrition read', () => {
        const { client, batch } = clientWithNutrition(async () => NUTRITION);

        const html = renderToString(page(client, prefetchedQueryClient(client)));

        expect(html, 'the prefetched rows ship in the HTML').toContain('Weeknight Pasta');
    ...
    it('hydrates with no recoverable error, and starts the ONE batch only once hydrated', async () => {
        const { client, batch } = clientWithNutrition(async () => NUTRITION);
        const container = document.createElement('div');
        container.innerHTML = renderToString(page(client, prefetchedQueryClient(client)));
        document.body.append(container);
        const onRecoverableError = vi.fn();

        await act(async () => {
            hydrateRoot(container, page(client, prefetchedQueryClient(client)), { onRecoverableError });
        });
        await settle();
