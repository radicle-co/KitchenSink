...
test.describe('recipes — paste an ingredient list and review the parse (U9)', () => {
    test('pastes a list, watches it settle, and reviews the parsed lines', async ({ page }) => {
        ...
        //
        // ⚠️ That transition is FIXED — the loading branch (`RecipeListLoading`) renders no dial at all, so a
        // first-run cook is never handed a control that is about to unmount. The seed here is
        ...
        // ⚠️ The COUNT line, not the page heading: `RecipeListFrame` renders its `<h1>` in every branch, so a
