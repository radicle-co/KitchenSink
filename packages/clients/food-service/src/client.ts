...
import type { AuthoredFoodTestPurgeResponse, FoodError, LiveSearchResponse } from '@kitchensink/schema-food';
...
export class FoodServiceClient {
    ...
    public async addByName(name: string): Promise<AddResponse> {
        ...
        return this.expect<AddResponse>(res, 202, addResponseSchema);
    }
    ...
    public async batch(names: readonly string[]): Promise<BatchResponse> {
        ...
        return this.expect<BatchResponse>(res, 201, batchResponseSchema);
    }
    ...
    public async purgeOwnAuthoredFoods(): Promise<AuthoredFoodTestPurgeResponse> {
        ...
        return this.expect<AuthoredFoodTestPurgeResponse>(res, 200, authoredFoodTestPurgeResponseSchema);
    }
    ...
    public async getStatus(id: string): Promise<StatusResponse> {
        ...
        return this.expect<StatusResponse>(res, 200, statusResponseSchema, id);
    }
    ...
    public async getCandidates(id: string): Promise<CandidatesResponse> {
        ...
        return this.expect<CandidatesResponse>(res, 200, candidatesResponseSchema, id);
    }
    ...
    public async search(query: string, withNutrition = false): Promise<SearchResponse> {
        ...
        return this.expect<SearchResponse>(res, 200, searchResponseSchema);
    }
    ...
    public async resolve(id: string, candidateIds: readonly string[]): Promise<ResolveResponse> {
        ...
        return this.expect<ResolveResponse>(res, 200, resolveResponseSchema, id);
    }
    ...
    private async send(method: string, path: string, body?: unknown, deadline?: AbortSignal): Promise<RawResponse> {
        ...
        try {
            response = await this.fetchImpl(`${this.baseUrl}${path}`, {
                ...
                headers,
                body: payload,
                // `AbortSignal.any` (platform) rather than a hand-rolled listener pair: whichever of this request's
                // timeout and the caller's deadline fires first aborts the socket, and the body read below with it.
                signal: deadline === undefined ? controller.signal : AbortSignal.any([controller.signal, deadline]),
            });
