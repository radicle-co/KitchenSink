/**
 * Unit tests for the recipe client's DRIFT LAYER 3 (Skew) comparison — `docs/CODING_STANDARDS.md` §15.2.5.
 *
 ...
 *
 * This is the copy that ships in the released MOBILE binary, so the 'never refuses' cases below are the ones
...
const BASE = 'http://recipe.example.test';
const OTHER_BASE = 'http://recipe-two.example.test';
...
describe('checkContractSkew (the probe)', () => {
    ...
    it('stays silent when the fingerprints agree', async () => {
        const warn = vi.fn();
        const fetchImpl = stubFetch(200, { status: 'ok', service: 'recipe', contractHash: CONTRACT_HASH });
    ...
    it('stays silent when the service publishes no fingerprint (an older deployed service)', async () => {
        const warn = vi.fn();
        const fetchImpl = stubFetch(200, { status: 'ok', service: 'recipe' });
    ...
    it('stays silent, and never rejects, when the transport fails outright', async () => {
        const warn = vi.fn();
        const fetchImpl = vi.fn().mockRejectedValue(new Error('ENOTFOUND recipe.test')) as FetchDouble;
...
describe('reportContractSkewOnce (the latch)', () => {
    ...
    // exactly what a server-rendered burst produces — all pass the check and each fire a probe.
