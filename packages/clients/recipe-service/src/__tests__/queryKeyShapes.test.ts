 ...
 *
 * ⛔ TanStack does NOT distinguish a flat and an infinite page shape by which hook subscribes. The cache holds
 * ONE `data` per key: an infinite observer stores `{ pages, pageParams }` there, and a flat observer stores the
 * bare page body, so whoever populates a shared key first decides what the second reader is handed. With one
 * key per pair, `discover/page.tsx` and `collections/page.tsx` would have to dodge that by discipline at every
 * prefetch; separate keys make it unrepresentable.
 *
