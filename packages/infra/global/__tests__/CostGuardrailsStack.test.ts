...
/**
 * The "created ONCE" invariant lives in the APP, not in the stack.
 *
 * Behind `if (stage === 'prod')` in the per-stage `bin/app.ts`, an account-scoped stack is reachable only by
 * deploying a STAGE — the right number of copies (one) by the wrong means. The account's cost monitoring then
 * cannot be touched without a production deploy, and the stack inherits the per-stage app's `Environment`
 * tag, labelling an account-wide resource `production`.
 *
 * So it lives in `bin/account.ts`, which HAS NO STAGE AT ALL. That makes "one per account, never per
