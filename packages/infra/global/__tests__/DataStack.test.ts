...
describe('Food DB IAM auth + role/database bootstrap (feature 003, ADR-0006)', () => {
    ...
    // The bootstrap FUNCTION itself — one for all three databases since the role split — is asserted in
    // `dbBootstrapStack.test.ts`.
...
describe('IAM database authentication on prod (safety of the merge-time change)', () => {
    it('is enabled on prod too — every migrator and service role logs in by IAM token', () => {
        const instance = Object.values(dataTemplate('prod').findResources('AWS::RDS::DBInstance'))[0] as any;
...
describe('RDS deletion protection (owner ruling 2026-08-08)', () => {
     ...
     * Per-PR cleanup works on LOGICAL databases inside this instance and on `pr-{N}`-tagged resources, so
...
/**
 * ⛔ THE TWO PLATFORM FUNCTIONS CAN REPORT (plan U18).
 *
 * Both have a documented path where they run to completion and do nothing they were there to do: the
 * bootstrap's master-login rollback releases the master from the owner roles a per-PR `DROP DATABASE` depends
 * on, and the reaper then runs daily, drops nothing and returns a clean census while every abandoned preview
 * keeps billing. Their groups are on the drain, so both already write a LINE — and a forwarded log line does
 * not group, carries no fingerprint, and nothing alerts on it. The reporter is what turns it into an issue,
 * and a reporter with no DSN is inert by construction, silently, which is the exact failure shape again.
 *
 * ⚠️ Asserted over the SYNTHESIZED environments rather than the source, because the defect this class of
 * guard exists for is a function whose environment is assembled somewhere else and never gets the key.
 */
describe('DataStack platform observability (plan U18)', () => {
    /**
     * ⚠️ TWO on a non-prod stage and ONE on prod, asserted rather than ranged over. The per-PR reaper exists
     * only where per-PR databases do — `DataStack` does not instantiate it on prod, and the handler refuses
     * at runtime as well — so a count that accepted either would also accept the reaper vanishing from
     * sandbox, which is the change that silently ends reclamation.
     */
    it.each([
        ['prod', 1],
        ['sandbox', 2],
    ])('⛔ hands every platform function on %s a Sentry DSN', (stage, expected) => {
        const functions = Object.values(dataTemplate(stage as string).findResources('AWS::Lambda::Function'));
        const platform = functions.filter((fn) => {
            const handler: unknown = fn.Properties?.Handler;

            return typeof handler === 'string' && /^db-(bootstrap|reaper)\/handler\./u.test(handler);
        });

        expect(platform).toHaveLength(expected as number);

        for (const fn of platform) {
            const variables: unknown = fn.Properties?.Environment?.Variables;

            expect(JSON.stringify(variables)).toContain('SENTRY_DSN');
        }
    });

    /**
     * ⛔ FROM THE BASE STAGE'S PARAMETER, resolved at deploy. A hard-coded DSN in a public repository is a
     * leaked ingest key, and a per-PR parameter would be one more thing to create before a preview can report.
     */
    it('⛔ resolves the DSN from the base stage’s SSM parameter, never a literal', () => {
        const body = JSON.stringify(dataTemplate('sandbox').toJSON());

        expect(body).toContain('/kitchensink/sandbox/sentry/platform-dsn');
        // ⚠️ `toContain`, not a regex. This is a SUBSTRING search over a synthesized template, and an
        // unanchored URL-shaped pattern is what CodeQL's "missing regular expression anchor" rule flags —
        // correctly in general, since such a pattern matches `evil-ingest.sentry.io.attacker.test` too.
        // Saying "does this text contain this literal" directly is both what the test means and unflaggable.
