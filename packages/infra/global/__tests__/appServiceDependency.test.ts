 ...
 *
 * WHY A TEST AND NOT THE RATCHET. The code-quality toolchain's boundaries ratchet reports UNDECLARED dependencies.
 * These were declared, so it was structurally blind to them — and so was every other gate. The edges were
 ...
 * length in the code-quality toolchain's boundaries ratchet
 * (`~/.claude/skills/code-quality-toolchain/scripts/boundariesRatchet.mjs`).
 *
 ...
 * `cdk.out/` — can never contribute a finding: that trap is documented at length in the code-quality toolchain's
 * boundaries ratchet, where a stray `cdk synth` invented three phantom violations.
 *
