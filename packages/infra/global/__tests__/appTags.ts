 ...
 * A test file that imports another test file silently re-collects the imported file's whole suite inside
 * the importer: its tests run twice, and a failure in them is attributed to whichever file happens to be
 * running. The convention here is a helper module (`cdkApps.ts`, `serviceSources.ts`); this follows it.
 *
