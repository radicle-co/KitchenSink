 ...
 *
 * `.github/scripts/verifyDeployment.sh` derives the stacks it verifies by running
...
describe('every CDK app entrypoint keeps stdout machine-readable', () => {
    ...
    it.each(cdkApps())('%s loads its .env QUIETLY', (app) => {
        ...
        for (const [, options] of calls) {
            expect(
                options,
                `${app} calls dotenv without \`quiet: true\`. dotenv@17 prints a banner to STDOUT, and this file's stdout is parsed as JSON by \`.github/scripts/verifyDeployment.sh\` — one advertising line there makes the post-deploy verifier report nothing.`,
