...
/** A repo-relative `.github/scripts/*.sh` path, as a workflow names the script a step runs. */
const INVOKED_SCRIPT = /\.github\/scripts\/[A-Za-z0-9._-]+\.sh/gu;

/**
 * A workflow's text with the body of every `.github/scripts/*.sh` it names appended, comments stripped from both.
 *
 * ⛔ A script is a workflow body by another name, exactly as a composite action is (see {@link ACTIONS_DIR}). A
 * guard asking "what does this workflow DO" that reads only the YAML does not fail when the logic moves into a
 * script — it finds FEWER subjects and passes over them. `sandboxReap.yml`'s whole sweep, including the
 * `gh workflow run sandboxDown.yml` dispatch, runs from `sandboxReap.sh`; without this the dispatch-edge
 * assertions lose that edge silently.
 *
 * Only scripts named in CODE are inlined, never in prose, and a named script that does not exist THROWS rather
 * than being skipped: skipping is the vacuity this exists to prevent.
 *
 * @param workflowText - One workflow's text, comments already stripped by {@link withoutComments}.
 * @returns That text followed by each invoked script's comment-stripped body. Impure.
 * @throws {Error} when the workflow names a script that is not in the working tree.
 * @sideEffect Reads the named scripts.
 */
export function withInvokedScripts(workflowText: string): string {
    const scripts = [...new Set([...withoutComments(workflowText).matchAll(INVOKED_SCRIPT)].map((match) => match[0]))];
    ...
    return [
        ...
        ...scripts.map((script) => {
            ...
            if (!existsSync(file)) {
                throw new Error(`a workflow runs ${script}, which does not exist — the guard cannot read what it does`);
            }
...
export interface Deployment {
     ...
     *
     * The verification step has to be given the SAME string the deploy used — `verifyDeployment.sh` derives
...
export function deployedApps(): readonly Deployment[] {
    ...
    // are at the call site. Reading the files directly attributed ZERO apps to `deployInfra.yml`, which
