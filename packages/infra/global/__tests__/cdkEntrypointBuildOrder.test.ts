 ...
 *
 * `@radicle-co/infra-shared/security` and `@radicle-co/infra-shared/alb` export `./dist`, not `./src`, because the
 ...
 *     url: .../node_modules/@radicle-co/infra-shared/alb/dist/index.js
 *
...
// ⚠️ TWO FILES, because a `tsx` CDK entrypoint now runs from either.
//
// `_sandbox-preview.yml` keeps the one in its `gate` job — resolving the food origin is how the gate knows
// which host to probe. Every other entrypoint moved into the composite action when each `infra/` folder got
// its own job, so scanning the workflow alone found ONE where it used to find two, and the
// closure-build rule silently stopped applying to the one that deploys.
const SOURCES = ['.github/workflows/_sandbox-preview.yml', '.github/actions/infra-package/action.yml'].map((file) =>
...
function tsxEntrypoints(): readonly Entrypoint[] {
    // A composite action has no `jobs:` — its steps hang off `runs.steps`, which is one implicit job. Both
    // shapes reduce to "an ordered list of steps", which is all the closure rule needs.
    const jobs = SOURCES.flatMap((file) => {
        const doc = parse(readFileSync(file, 'utf8')) as { jobs?: Record<string, Job>; runs?: { steps?: Step[] } };
        const label = file.split('/').slice(-1)[0] ?? file;

        return doc.jobs
            ? Object.entries(doc.jobs).map(([jobId, job]) => [`${label}::${jobId}`, job] as const)
            : [[`${label}::runs`, { steps: doc.runs?.steps ?? [] }] as const];
    });
    ...
    for (const [jobId, job] of jobs) {
        ...
        steps.forEach((step, index) => {
            ...
            found.push({
                ...
                owningPackage,
                // ⛔ INCLUDING THE PART OF THIS STEP'S OWN SCRIPT THAT RUNS FIRST. The build and the
                // entrypoint sit in ONE `run:` block in the composite action, which satisfies the rule
                // strictly harder than two steps do — they cannot be reordered or separated by an edit that
                // only looks at one of them. Truncating at the match keeps the ORDER real: a build written
                // after the entrypoint would still not count.
                precedingRuns: [
                    ...steps.slice(0, index).map((earlier) => earlier.run ?? ''),
                    run.slice(0, script.index),
                ],
            });
        });
    }
    ...
    it('never names an infra package individually in a build filter', () => {
        const named = SOURCES.flatMap((file) =>
            readFileSync(file, 'utf8')
                ...
                .map(({ line, number }) => `${file.split('/').slice(-1)[0]}:${number}: ${line.trim()}`),
        );
