 ...
 */
import { Aspects, CfnResource, type IAspect } from 'aws-cdk-lib';
import { ArtifactMetadataEntryType } from 'aws-cdk-lib/cloud-assembly-schema';
import { attachSecurityChecks } from '@radicle-co/infra-shared/security';
...
function synthesizePlatform(stage: string, options: { readonly aspect?: IAspect | 'security' } = {}): Templates {
    ...
    new GlobalStack(app, `Global-${stage}`, {
        ...
        stage,
        domainName,
        // ⛔ Both halves of the parity proof synthesize from this one helper, so the flag is the same on
        // both sides by construction. It is ON so the alarm resources are INSIDE the bytes being compared —
        // a parity proof run with alarms off would compare a template the flag had emptied.
        alarmsEnabled: true,
    });
...
describe('exactly the reviewed cdk-nag suppressions are in force', () => {
    ...
    it('states a readable justification for every one of them', () => {
        ...
        // instead of the argument. See @radicle-co/infra-shared/security's own suite for the mechanism.
