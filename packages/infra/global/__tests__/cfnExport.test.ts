/**
 * Guards `.github/scripts/cfnExport.sh` — the ONE place a CloudFormation export is resolved by name.
 *
...
const script = path.join(repoRoot, '.github/scripts/cfnExport.sh');
...
function runWithPages(pages: readonly string[], exportName = 'kitchensink-domain-sandbox:HostedZoneId'): Result {
    const dir = scratchDirectory('cfn-export-');
...
describe('cfnExport.sh — paginated ListExports', () => {
...
function runWithStub(stubBody: string, args: readonly string[]): Result {
    const dir = scratchDirectory('cfn-export-stub-');
    const stub = path.join(dir, 'aws');
 ...
 */
describe('cfnExport.sh — a CLI failure is neither "found" nor "absent"', () => {
