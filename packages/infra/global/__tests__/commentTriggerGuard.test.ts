 ...
 * shared one in `./workflowExpression.ts` (see its header for why the atom resolver is a parameter);
...
import { type Truth, closingParen, evaluateCondition, isSkipTolerant } from './workflowExpression.js';
...
function fixture(files: Readonly<Record<string, string>>): readonly Workflow[] {
    const directory = scratchDirectory('comment-trigger-guard-');
