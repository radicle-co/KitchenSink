 ...
 *
 * `docs/architecture/2026-08-28/ingredientPipelineState.md` §1 claimed `verifyLine` and thirteen other
...
// ⛔ ONE SPELLING. The migration out of the npm workspace is complete — `packages/infra/security` is deleted
// — so the transitional "either specifier" allowance is gone with it. A stamp from anywhere else is a
// finding again, which is the state this guard is strongest in.
const STAMP_MODULES = ['@radicle-co/infra-shared/security'] as const;
...
export function stampsCommitProvenance(source: string, file: string): boolean {
    ...
    visit(parsed, (node) => {
        if (
            ...
            (STAMP_MODULES as readonly string[]).includes(node.moduleSpecifier.text)
        ) {
...
describe('the reader can tell a call from a mention', () => {
    const header = `import { attachSecurityChecks, ${STAMP} } from '${STAMP_MODULES[0]}';\nconst app = new App();\n`;
