...
function makeContractRepo(): string {
    ...
    write(root, 'packages/schemas/recipe/src/contractHash.ts', "export const CONTRACT_HASH = 'aaaa';\n");
    write(root, 'packages/schemas/recipe/src/schemas.ts', "export * from './schemas/x.schema.js';\n");
    write(
        root,
        'packages/services/recipe-service/src/contract/contractHash.ts',
...
describe('the gate against a real repository', () => {
    ...
    it('fails on a MODIFIED artifact and names it as stale', () => {
        withContractRepo((root) => {
            write(root, 'packages/schemas/recipe/src/contractHash.ts', "export const CONTRACT_HASH = 'bbbb';\n");

            const result = runGate(root);

            expect(result.status).toBe(1);
            expect(result.output).toContain('Stale');
            expect(result.output).toContain('packages/schemas/recipe/src/contractHash.ts');
