 ...
 *
 * ⚠️ The RUNNER is ignored, whatever it is. Three spellings have now been in use — `node …/dist/bin/app.js`,
 * `npx tsx …/bin/app.ts`, and `<pkg>/infra/node_modules/.bin/tsx …/bin/app.ts`, the last because each CDK
 * package supplies its own runner rather than relying on one at the repo root (which `npm prune --omit=dev`
 * removes, and which `npx` would otherwise silently INSTALL from the registry). Matching the runner by name
 * meant a fourth spelling read as a path with spaces in it, and the closure was handed keys naming no file.
 * The app path is the LAST token either way, so take that and stop caring how it is launched.
 *
...
function entrypointOf(appArgument: string): string {
    return (appArgument.trim().split(/\s+/u).at(-1) ?? '').replace(/\/dist\/bin\/app\.js$/u, '/bin/app.ts');
}
...
/** Set B — every app handed to `deployGate.sh close`, mapped to the flag it is given under. */
...
describe('prod-deploy.yml — every reachable cross-app import has a producer this workflow can force', () => {
     ...
     * written tomorrow is covered tomorrow. `deployGate.sh close` refuses at run time when a deploying
    ...
    it('reports no import whose producer the manifest could not name at all', () => {
        // An unresolved import is an edge the closure cannot see. `deployGate.sh unmet-imports` refuses to
...
describe('prod-deploy.yml — the probe and the closure are wired to each other, in that order', () => {
    ...
    it('resolves the exports with the shared helper, never an open-coded `list-exports`', () => {
        ...
        // CloudFormation only through `deployGate.sh`, which reaches it only through `cfnExport.sh`.
        const probe = steps().find((step) => step.id === 'imports');

        expect(probe?.run).toContain('deployGate.sh unmet-imports');
    ...
    it('the flags step closes the graph before publishing, and reads the probe as DATA', () => {
        ...
        expect(step.run).toContain('deployGate.sh close');
