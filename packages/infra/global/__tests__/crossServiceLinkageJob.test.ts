 ...
 *
 * `deployedE2eTiers.yml::e2e-cross-service-linkage` (called by `_ci.yml` and by `deployedE2e.yml`) closes that. This suite is what stops it from being deleted,
 ...
 * `Integration (…)` tier — the same open item `deployedE2eTiers.yml::e2e-backend`'s header records for identity-webhooks.
 */
...
/**
 * Where the linkage tier is DEFINED. It moved out of `_ci.yml` into the reusable tiers workflow so the manual
 * door runs it too; its origins arrive as that workflow's inputs, and which resolver each caller feeds them from
 * is asserted per caller in `deployedE2eTiers.test.ts` §2.
 */
const TIERS_FILE = 'deployedE2eTiers.yml';

/** The expression a linkage origin must be — the caller-resolved input, never a literal. */
const originInput = (service: 'recipe' | 'food'): string => `\${{ inputs.${service}_origin }}`;
...
/** The real tiers workflow, where the linkage job lives. */
const tiersDoc = (): Doc => docOf(readFileSync(join(WORKFLOW_DIR, TIERS_FILE), 'utf-8'));

/** The real `_ci.yml`, whose call of the tiers workflow carries the deploy gate. */
...
/** The one linkage job in the tiers workflow, or a failure that says so rather than dereferencing undefined. */
function linkageJob(): Job {
    const [, found] = linkageJobs(tiersDoc())[0] ?? [];

    if (found === undefined) {
        throw new Error(`${TIERS_FILE} has no job running the linkage spec — this guard has no subject.`);
    }
...
describe('the live linkage proof is wired into CI', () => {
    it('the tiers workflow has exactly one job that runs the cross-service linkage spec', () => {
        const jobs = linkageJobs(tiersDoc());
    ...
    it('targets BOTH deployed services, not just one, and resolves neither by hand', () => {
        ...
        expect(recipe, 'the job names no recipe origin').toBe(originInput('recipe'));
        expect(food, 'the job names no food origin').toBe(originInput('food'));
    ...
    it('skips, rather than failing, when nothing is deployed at that stage', () => {
        // The gate sits on `_ci.yml`'s CALL of the tiers workflow: a skipped `uses:` job runs none of the called
        // jobs, so it gates this one.
        const calls = Object.values(ciDoc().jobs ?? {}).filter(
            (job) => (job as { uses?: string }).uses === `./.github/workflows/${TIERS_FILE}`,
        );
...
describe('the guard itself can fail', () => {
    ...
    it('detects both linkage origins pointed at the same resolved service', () => {
        const doc = docOf(
            `jobs:\n  linkage:\n    env:\n` +
                `      LINKAGE_RECIPE_URL: ${originInput('recipe')}\n` +
                `      LINKAGE_FOOD_URL: ${originInput('recipe')}\n` +
                `    steps:\n      - run: ${SPEC_INVOCATION}\n`,
        );
        const [, job] = linkageJobs(doc)[0] ?? [];
