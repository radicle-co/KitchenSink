 ...
 *
 * A handle declared as `PostgresJsDatabase` — a driver this repo does not install, while production passes a
 * `NodePgDatabase` — forces every call site to bridge the difference with `as unknown as`, and one docstring
 * saying "per the repo cast convention" turns that into policy.
 *
 * A suite casting to build the thing it means to exercise is a suite reaching past an interface, so the
 * interface is wrong rather than the callers. `eraseIdentityRow.ts` states it: *"a cast that appears at every
 * call site is not a call-site problem: the signature was simply too narrow."*
 *
 * The cure is a structural write surface per package (`IdentityWriter`, `FoodWriter`, recipe-service's
