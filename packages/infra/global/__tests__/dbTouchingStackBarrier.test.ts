 ...
 */
// ⚠️ THE THIRD SPELLING IS A LOOKUP, NOT A DERIVATION, and it is the strongest of the three. A stack can
// name its database by calling the shared `*DatabaseNameForStage(...)` function, by importing the
// platform export, or — best — by READING the parameter the stack that CREATED the database published
// (`*DatabaseNameParameter(...)`). The first two still let two stacks derive different names from
// different arguments, which is exactly how #119 put six destructive workers on the shared sandbox
// database; the third cannot, because there is one value with one producer.
//
// ⛔ Omitting it reported `RecipeWorkersStack` as naming its database by hand at the very moment it
// stopped doing so — the guard failing on the fix.
const NAMES_ITS_DATABASE = /DatabaseNameForStage\s*\(|DatabaseNameParameter\s*\(|kitchensink-data-\$\{/u;
        ...
        if (addressesADatabase(stack) && !exempt.has(stack.name) && !NAMES_ITS_DATABASE.test(stack.contents)) {
            ...
            violations.push(
                `${stack.file}: deploys compute that addresses a service database without naming that ` +
                    'database through a shared authority (`*DatabaseNameParameter(...)`, `*DatabaseNameForStage(...)` or a ' +
