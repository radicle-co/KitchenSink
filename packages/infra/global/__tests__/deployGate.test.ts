/**
 * Repo-wide guard: the per-PR **ensure-exists** deploy gate (`.github/scripts/deployGate.sh`).
 *
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/deployGate.sh', import.meta.url));
...
describe('deployGate.sh — the file exists where the workflows invoke it from', () => {
    it('is present at .github/scripts/deployGate.sh', () => {
...
describe('deploy_gate_decide — misuse fails loudly instead of guessing', () => {
    ...
    describe('intent — a reaped sandbox must not resurrect itself', () => {
         ...
         *
         * `intent` is the precondition that separates the two readings: "a sandbox is SUPPOSED to exist
         * here". It is a REQUIRED first parameter rather than a defaulted one, because a default is a
         * position — silently asserted on behalf of every caller that never considered it.
         *
         * ⚠️ No PR label carries it. Every caller passes `true` — `sandboxPreview`,
         * `deployed-e2e` and `_ci-heavy` alike — because the decision is made UPSTREAM of the gate: `_ci.yml`
         * calls the preview only when the shared tier is up, and `Sandbox Up` dispatches it forced. So the
         * `intent = false` branch asserted here is reached by THIS test and by no production caller. It is
         * kept as the seam, not as live behaviour, and the absent-stack reading it guards is the one that
         * matters if a caller ever passes `false`.
         */
 ...
 * CDK source by AST and projects them to `docs/generated/infrastructure/crossAppImports.tsv` under the same
...
describe('deploy_gate_stacks_for — a probe never has to write down what an app declares', () => {
    ...
    it('leaves a prod-only stack out of the sandbox answer, and a sandbox-only stack out of prod', () => {
        ...
        // ⚠️ `kitchensink-cost-guardrails` is absent from BOTH answers, a stronger claim than "present at
        // prod, absent at sandbox". It is ACCOUNT-scoped — one budget and one anomaly monitor for the whole
        // AWS account — and lives in `bin/account.ts`, which has no stage at all. A per-stage probe should
        // never see it, whichever stage it is asked about.
        expect(sandbox).not.toContain('kitchensink-cost-guardrails');
        expect(prod).not.toContain('kitchensink-cost-guardrails');
    });
