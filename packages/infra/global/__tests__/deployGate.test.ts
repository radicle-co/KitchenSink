...
describe('deploy_gate_decide — misuse fails loudly instead of guessing', () => {
    ...
    describe('intent — a reaped sandbox must not resurrect itself', () => {
         ...
         *
         * ⚠️ Where it comes from has moved, and this used to say otherwise. It was once carried by a
         * `sandbox-up` PR label; that label is deleted. Today every caller passes `true` — `_sandbox-preview`,
         ...
         * kept as the seam, not as live behaviour, and the absent-stack reading it guards is still the one
         * that matters if a caller ever passes `false` again.
         */
...
describe('deploy_gate_stacks_for — a probe never has to write down what an app declares', () => {
    ...
    it('leaves a prod-only stack out of the sandbox answer, and a sandbox-only stack out of prod', () => {
        ...
        // ⚠️ `kitchensink-cost-guardrails` is absent from BOTH answers now, and that is a stronger claim
        // than the one this used to make ("present at prod, absent at sandbox"). It is ACCOUNT-scoped —
        // one budget and one anomaly monitor for the whole AWS account — and it moved out of this per-stage
        // app into `bin/account.ts`, which has no stage at all. A per-stage probe should never see it,
        // whichever stage it is asked about; when it was reachable through `if (stage === 'prod')` here,
        // "present at prod" was the closest a stage-shaped question could get to an account-shaped fact.
