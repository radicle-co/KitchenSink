 ...
 * how `ingredientCatalogBlend.yaml` sat unexecuted for months behind a `FLOWS` array nobody updated.
 *
...
// ⚠️ `sandboxPreview.yml`, not `sandbox-deploy.yml`: the deploy jobs moved to `sandboxPreview.yml`, a REUSABLE workflow, because GitHub Actions has no cross-workflow `needs` — `_ci.yml` has to be able to run them as one branch of its own graph.
const WORKFLOW = join(repoRoot, '.github/workflows/sandboxPreview.yml');
...
describe('every post-gate deploy step is guarded by the gate', () => {
    it('is not vacuous: the workflow really does have a gated job with steps after the gate', () => {
        ...
        // of them moved to `deployInfra.yml` so that one workflow owns each stack. What follows the gate
