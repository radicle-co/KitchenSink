 ...
 *
 * `verifyDeployment.sh` discovers the stacks to verify by synthesising the app it is handed, because a
...
const VERIFIER = 'verifyDeployment.sh';

/** A `verifyDeployment.sh verify <region> "<app>"` invocation, in either quote style. */
const VERIFY_INVOCATION = /verifyDeployment\.sh\s+verify\s+\S+\s+(?:"([^"]+)"|'([^']+)')/gu;

/** The log-drain sweep's invocation, same shape — see the drain-coverage assertion below. */
const DRAINS_INVOCATION = /verifyDeployment\.sh\s+drains-app\s+\S+\s+(?:"([^"]+)"|'([^']+)')/gu;
...
interface Coverage {
    ...
    /** Every CDK entrypoint this job sweeps for log drains, repo-relative. */
    readonly drainedEntrypoints: readonly string[];
...
function coverage(job: WorkflowJob): Coverage {
    ...
    // A `cdk deploy --app "X"` and a `verifyDeployment.sh verify <region> "X"` both carry an `--app`-shaped
    ...
    return {
        ...
        drainedEntrypoints: [
            ...new Set(
                [...folded.matchAll(DRAINS_INVOCATION)].map((match) =>
                    entrypointFor((match[1] ?? match[2] ?? '').trim()),
                ),
            ),
        ],
...
describe('deploy verification coverage', () => {
    it('is not vacuous: real deploying jobs are discovered across the real workflows', () => {
        ...
        // ⚠️ `deployInfra.yml`, not `sandboxPreview.yml`: every `cdk deploy` moved there, one job per
        // `infra/` folder, so a single workflow owns each stack. The preview now only DECIDES and delegates.
        expect(workflows).toContain('deployInfra.yml');
    ...
    it('⛔ a DIRECT deploy and its verification pass the byte-identical `--app` string', () => {
        // Stricter than the entrypoint rule above, and it is the one with teeth: `verifyDeployment.sh`
    ...
    it('⛔ every job that PUSHES an image also proves, in that job, what is RUNNING', () => {
        ...
        // a live check after deploy". `verifyDeployment.sh` asks whether the resources arrived;
    ...
    it('⛔ every `aws lambda invoke` in a workflow READS the answer, not the exit status', () => {
        ...
        // `sandboxUp.yml` and `sandboxReconcile.yml` grepped for one specific phrase each, and the FOOD
        ...
        // the check the day it lands. A step that delegates to `runMigrations.sh` satisfies it by
        // construction — that script IS the one definition of "did the runner succeed".
        const invokes = /aws lambda invoke/u;
        const inspects = /FunctionError|errorType|runMigrations\.sh/u;
    ...
    it('⛔ a preflight runs BEFORE THE DEPLOY OF THE STACK IT GUARDS, never after', () => {
        ...
        const preflights = jobs().flatMap((job) => {
            ...
            const match = /verifyDeployment\.sh\s+preflight\s+\S+\s+"([^"]+)"/u.exec(body);
    ...
    it('⛔ the migration safety net is never gated on a PATH DIFF', () => {
        ...
        const gated = jobs()
            ...
            .filter((entry) => /runMigrations\.sh\s+run/u.test(entry.step))
            .filter((entry) => pathDiffGate.test(entry.step))
            .map(
                (entry) =>
                    `${entry.job.workflow}:${entry.job.name} gates a runMigrations.sh step on a path-diff ` +
    ...
    it('is not vacuous: the workflows really do invoke migration runners', () => {
        ...
        const invoking = jobs().filter((job) => /runMigrations\.sh\s+run/u.test(foldContinuations(job.body)));
    ...
    it('every verified app string names a CDK entrypoint this repository defines', () => {
        ...
        // ⚠️ `cdkApps()`, not a third copy of "what is a CDK app". This was
        // `trackedFiles(...).filter(file.endsWith('/bin/app.ts'))` — the same predicate spelled out again,
        // and it went stale the moment a package held an app not named `app.ts`: the account-scoped
        // `bin/account.ts` was reported as an UNKNOWN app being verified, by a guard whose whole purpose is
        // to notice a verifier pointed at something that is not an app. One authority, so it cannot drift.
        const apps = new Set(cdkApps().map(toSourceEntrypoint));
...
/**
 * ⛔ AN APP THAT IS VERIFIED IS ALSO SWEPT FOR LOG DRAINS — the other half of ADR-0042's deployed-state
 * check, and the half that was missing for two rounds without anything noticing.
 *
 * `drains` began life inside `.github/actions/infra-package`, which only `deployInfra.yml` uses, and that
 * workflow serves per-PR previews. `prod-deploy.yml` and `sandbox-identity-deploy.yml` deploy directly — so
 * the two stages where an undrained log group costs most were the ones nothing asked about, while the ADR
 * read as though they were covered. Ten sweeps were added by hand to match ten `verify` sites; a hand-made
 * pairing is exactly what this file exists to stop being necessary.
 *
 * ⚠️ TWO `verify` SITES HAVE NO SWEEP AND THAT IS CORRECT, so the rule is stated as an ALLOWLIST with its
 * reason rather than as a blanket equality: `accountDeploy.yml`'s account-scoped app and the sandbox
 * router's app declare no `logs.SubscriptionFilter` at all, so there is nothing for a sweep to read. The
 * exemption is DERIVED — an app is exempt when no stack under its package declares a filter — so an app
 * that grows one loses its exemption by growing it, rather than by anyone remembering this list.
 */
describe('every verified app is also swept for log drains (ADR-0042)', () => {
    /**
     * The stack CLASSES whose source declares a subscription filter, named by their file.
     *
     * ⛔ BY ENTRY, NOT BY PACKAGE, and the coarser reading was wrong on a real case rather than in theory:
     * `packages/infra/global` owns three filter-declaring stacks, but its SECOND entry — `bin/account.ts`,
     * the account-scoped cost guardrails — constructs `CostGuardrailsStack` and no drain at all. Grading by
     * package demanded that `accountDeploy.yml` sweep an app with nothing to sweep. `logDrainRegister.test.ts`
     * made the same mistake and states the same limit: this matches a class NAME in a `new` expression, so a
     * construction through an alias, a loop or a factory is not detected (ADR-0042's residual list).
     */
    const filterOwningClasses = (): readonly string[] =>
        productionSources()
            .filter((path) => /infra\/lib\/.*\.ts$/u.test(path) || path.includes('/lib/platform/'))
            .filter((path) => {
                const text = withoutTsComments(readSource(path));

                // ⚠️ EITHER declares a filter OR carries the ARN prop, because a drain can live one level
                // below the entry: `bin/app.ts` builds `GlobalStack`, which declares no filter itself and
                // hands `props.logForwarderArn` down to `DataStack` and `SandboxSchedulerStack`, which do.
                // Matching only the declaration exempted the global app — the very app whose deadlock
                // started all of this.
                return text.includes('new logs.SubscriptionFilter(') || text.includes('props.logForwarderArn');
            })
            .map((path) => path.replace(/^.*\/|\.ts$/gu, ''));

    /** Whether an app entry constructs any stack that declares a filter. */
    const ownsADrain = (entrypoint: string): boolean => {
        const path = join(repoRoot, entrypoint);

        // ⚠️ AN ENTRY THAT DOES NOT EXIST OWNS NO DRAIN, and one really does not: `shared/infra` is a
        // library package with no `bin/` at all, and its job opts out of synth for exactly that reason.
        // The verify site resolves there anyway, so reading it unguarded threw ENOENT — and a guard that
        // dies on a caller it was never about is a guard nobody can keep.
        if (!existsSync(path)) {
            return false;
        }

        const text = withoutTsComments(readFileSync(path, 'utf8'));

        return filterOwningClasses().some((className) => text.includes(`new ${className}(`));
    };

    it('⛔ holds for every job that verifies an app owning a drained log group', () => {
        const gaps: string[] = [];

        for (const job of jobs()) {
            const { verifiedEntrypoints, drainedEntrypoints } = coverage(job);

            for (const entrypoint of verifiedEntrypoints) {
                if (!ownsADrain(entrypoint) || drainedEntrypoints.includes(entrypoint)) {
                    continue;
                }

                gaps.push(`${job.workflow}: ${job.name} verifies ${entrypoint} but never sweeps its log drains`);
            }
        }
    ...
    /**
     * ⚠️ The exemption is DERIVED, so it is asserted rather than trusted: these two entries really do build
     * no drained stack, and an app that grows one loses its exemption by growing it.
     */
    it('⛔ exempts exactly the entries that construct no drained stack', () => {
        expect(ownsADrain('packages/infra/global/bin/account.ts')).toBe(false);
        expect(ownsADrain('packages/apps/commise/web/infra/bin/app.ts')).toBe(false);
        expect(ownsADrain('packages/infra/global/bin/app.ts')).toBe(true);
    });

    it('is not vacuous — the tree really does pair the two', () => {
        const paired = jobs().flatMap((job) => coverage(job).drainedEntrypoints);

        // ⚠️ 22 today. Anchored near the real figure rather than at the number of WORKFLOW SITES, so
        // losing half the pairings is a failure rather than a pass with room to spare.
