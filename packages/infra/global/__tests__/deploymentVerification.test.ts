/**
 * Repo-wide guard: the post-deploy RESOURCE verifier (`.github/scripts/verifyDeployment.sh`).
 *
...
describe('verifyDeployment.sh exists and is executable as bash', () => {
    ...
    it('refuses an unknown subcommand with the misuse status', () => {
        // Exit 2 rather than 1, exactly as deployGate.sh does: a verifier that answers "nothing wrong"
...
describe('classify-reference — an ARN names its own service, so nothing is enumerated', () => {
    ...
    it('resolves an RDS ENDPOINT by shape, not by the key', () => {
        // `RECIPE_DB_HOST`, `FOOD_DB_ENDPOINT` and `DB_ENDPOINT` all carry the same thing — an RDS endpoint
        // address — and no key suffix matches them, so without a rule describing the SHAPE all three fall
        // through to `unchecked`. The host names its own service in its suffix, so it is classified there
        // rather than by adding three key spellings that the fourth consumer would not carry.
        const endpoint =
            'kitchensink-data-sandbox-databaseb269d8bb-p76w6xmz1xlk.cmyrq3goxloz.us-east-1.rds.amazonaws.com';

        for (const key of ['RECIPE_DB_HOST', 'FOOD_DB_ENDPOINT', 'DB_ENDPOINT']) {
            expect(classifyReference(key, endpoint)).toMatchObject({ probe: 'rds-endpoint', target: endpoint });
        }
    });

    it('classifies a CLUSTER and a PROXY endpoint as the same shape', () => {
        // `describe-db-instances` cannot match either, which is why the resolver probes clusters too —
        // but the CLASSIFIER must still recognise them, or they return to being silently unchecked.
        expect(classifyReference('DB_ENDPOINT', 'kitchensink-x.cluster-abc.us-east-1.rds.amazonaws.com').probe).toBe(
            'rds-endpoint',
        );
        expect(classifyReference('DB_ENDPOINT', 'kitchensink-x.proxy-abc.us-east-1.rds.amazonaws.com').probe).toBe(
            'rds-endpoint',
        );
    });

    it('does not classify a NON-RDS host as an endpoint', () => {
        // Discriminating power: a rule that answered `rds-endpoint` for everything host-shaped would pass
        // the test above while destroying every other verdict. An origin is still the smoke's job, and a
        // `kitchensink-` name this classifier does not know is still `unchecked`.
        expect(classifyReference('FOOD_SERVICE_URL', 'https://food-pr-91.commise.app').probe).toBe('none');
        expect(classifyReference('SOME_HOST', 'kitchensink-thing.execute-api.us-east-1.amazonaws.com').probe).toBe(
            'unchecked',
        );
    });
