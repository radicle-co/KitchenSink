...
describe('classify-reference — an ARN names its own service, so nothing is enumerated', () => {
    ...
    it('resolves an RDS ENDPOINT by shape, not by the key', () => {
        // The ten warnings the 2026-09-09 `Sandbox Up` press produced. `RECIPE_DB_HOST`,
        // `FOOD_DB_ENDPOINT` and `DB_ENDPOINT` all carry the same thing — an RDS endpoint address — and
        // all three fell through to `unchecked` because no rule described the SHAPE and no key suffix
        // matched. The host names its own service in its suffix, so it is classified there rather than by
        // adding three key spellings that the fourth consumer would not carry.
        const endpoint =
