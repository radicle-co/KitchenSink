...
const PREVIEW_WORKFLOW = '.github/workflows/sandboxPreview.yml';
...
function touchesDepsCache(steps: readonly Step[], action: 'cache/save' | 'cache/restore'): boolean {
    return steps.some((step) => {
        ...
        return matches && /(^|\n|\s)node_modules/.test(scalarText((step.with ?? {})['path']));
    });
}
...
function pipelineJobs(): readonly CacheJob[] {
    ...
    return Object.entries(ci).map(([name, job]) => {
        const own = job.steps ?? [];
        const steps = job.uses?.includes('sandboxPreview.yml') ? [...own, ...calleeSteps] : own;
