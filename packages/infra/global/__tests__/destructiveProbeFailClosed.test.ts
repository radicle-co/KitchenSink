 ...
 *
 * 1. `sandboxReconcile.yml`'s tier probe ended `|| echo 0` — a rate-limited `gh` meant "no runs in
 ...
 * 3. `sandboxLifetime.sh live-expiry` and `verify_deployment_resolve_rds` were both written the correct
...
const DESTRUCTIVE = [/teardownSandboxPr\.sh/u, /sandboxSharedTier\.sh\s+down/u, /cdk\s+destroy/u];
