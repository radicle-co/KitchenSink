...
// Registered at import time, so it attaches to the suite of whichever test file pulled this fixture in —
// each of which gets its own module instance under vitest's per-file isolation. A fixture that leaves its
// directory behind is a leak nobody owns, because no test created it directly.
afterAll(() => {
    for (const directory of cache.values()) {
