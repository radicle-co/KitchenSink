 ...
 * Sentry log facade — and therefore bypasses `sentryScrubbers.ts`, the one place that pseudonymises
 ...
 *    `recipe-service/src/account/erasureMetrics.ts` keeping `ownerId` as an EMF *property* (never a
 ...
 * multiplies the billed series count AND changes what `serviceInfraWiringInvariants.test.ts`'s W4 requires
