 ...
 * `docs/runbooks/pg18Upgrade.md` carries that restore leg. This gate is what keeps anyone from taking that
 ...
 * `docs/runbooks/pg18Upgrade.md` executed — snapshot verified, window scheduled, restore leg rehearsed.
 */
    ...
    it.each(STAGES)('synthesizes the reviewed engine and upgrade flags for %s', (stage) => {
        ...
        expect(
            ...
            'The shared instance carries kitchensink_identity — live production user data — single-AZ, no ' +
                ...
                'docs/runbooks/pg18Upgrade.md executed.',
        ).toEqual([]);
    });
