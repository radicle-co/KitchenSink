 ...
 *
 * ADR-0005 makes `Environment` the primary teardown signal — a persistent tier word (`sandbox`/`production`)
 * survives, `pr-{N}-sandbox` is deleted when the PR closes — and `.github/scripts/teardownSandboxPr.sh`
 * reclaims by **tag OR name**. An app that tags nothing sits unnoticed beside seven siblings that do,
 * because nothing looks.
 *
 * ⛔ The name half of the teardown rule does NOT cover every app. `pr_scope_belongs` is delimiter-anchored
 * on the LEADING token, and the web app's stack is `kitchensink-sandbox-router-{stage}` — so a per-PR router
 ...
 * for this PR's `Environment` values, which reads RESOURCE-level tags, and a stack-only tag is invisible to it.
 *
...
import { ENVIRONMENT_TAG, tagsEnvironmentOnApp } from './appTags.js';
