 ...
 *    matches nothing, and reports success. That is the expensive direction and the silent one — log groups
 *    accumulate behind a routine that looks implemented.
 * 2. **Something shared, deleted.** The per-PR value `pr-{N}-sandbox` CONTAINS the persistent value
 *    `sandbox`. So "can a per-PR matcher reach a persistent value" is a question that must
...
const SCOPE_SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/prScope.sh', import.meta.url));
...
function tagFilterValues(token: string): readonly string[] {
    ...
    expect(result.status, `prScope.sh refused to print a filter for ${token}`).toBe(0);
...
describe('every value a preview can carry IS reclaimed (nothing becomes unreapable)', () => {
    ...
    // ⛔ The equality read (teardown §2, per stack) and the API filter (teardown §3 and `ecsQuiesce.sh`,
...
describe('both sweeps read the shared predicate rather than spelling the filter themselves', () => {
    ...
    const readers = ['.github/scripts/teardownSandboxPr.sh', '.github/scripts/ecsQuiesce.sh'] as const;

    it.each(readers)('%s derives its Environment filter from prScope.sh', (script) => {
