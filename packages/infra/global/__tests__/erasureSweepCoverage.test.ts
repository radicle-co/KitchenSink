 ...
 *
 * DESIGN PATTERN: Specification module over eight pure verdicts — `columnEffectsIn` (folded by
 ...
 * as well as at the working tree. The first three and {@link sweptTablesIn} live in `sweepCoverageReaders.ts`
 * since 2026-09-13, shared with `testResetSweepCoverage.test.ts` (ADR-0040's second recipe sweep); the fakes
 * below still drive those exact functions, and the decisions — every map and pinned count — stay in this file.
 */
...
import { repoRoot, type SourceFile } from './serviceSources.js';
import {
    OWNER_COLUMNS,
    alterClausesIn,
    balancedBody,
 ...
 * 0040 gave `ingredients` its `food_owner_id`, 9 → 11 on 2026-09-04 for `recipe_parse_jobs.owner_id`
 * (0039) and `analytics_events.user_id` (0043), and 11 → 13 on 2026-09-13 for ADR-0040's
 * `test_principals.user_id` (0044) and `test_reset_jobs.user_id` (0045) — measured at 13 by this assertion going
 * red on those two migrations before the number moved, not transcribed from the design.
 *
 ...
 */
const EXPECTED_OWNER_BEARING_TABLES = 13;
...
describe('account-erasure sweep coverage', () => {
    for (const database of SWEPT_DATABASES) {
        describe(database.sweepFunction, () => {
            it('reaches every user-bearing table, or exempts / retains it for a written reason', () => {
                const owned = userBearingTablesAfter(migrationsOf(database.migrations));
            ...
            it('⛔ WRITES every handle-bearing column, or deletes the row that carries it, per COLUMN', () => {
                const handles = handleBearingColumnsAfter(migrationsOf(database.migrations));
            ...
            it('carries no handle-column claim for a column that is gone, or one the sweep already writes', () => {
                const handles = new Set(handleBearingColumnsAfter(migrationsOf(database.migrations)));
                const written = new Set(writtenColumnsIn(readSource(database.sweepFile), database.sweepFunction));
            ...
            it('⛔ never REMOVES a table the union found — a fold may only ever subtract', () => {
                const migrations = migrationsOf(database.migrations);
                ...
                // the current count by however many user-bearing tables have since been dropped (14 vs 13
            ...
            it('⛔ pins every RETAINED table’s whole column set, so a new column is a decision', () => {
                const migrations = migrationsOf(database.migrations);
                const owned = new Set(userBearingTablesAfter(migrationsOf(database.migrations)));
    ...
    it('⛔ reads every clause of the REAL 0016 ALTER, not just its first', () => {
        ...
        const columns = new Set(currentColumnsOf(migrationsOf(SWEPT_DATABASES[0]!.migrations), 'collections'));
