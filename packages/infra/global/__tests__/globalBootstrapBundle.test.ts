...
/**
 * Whether a step exports the Clerk edge key — which is what makes a workflow an EDGE bundler.
 *
 * ⛔ THE EDGE VERIFIER IS PROD-ONLY (`bin/app.ts` gates `EdgeStack` on `stage === 'prod'`, and the stack
 * refuses any stage without an internal origin), and `esbuild.mjs` SKIPS the edge bundle entirely when
 * `CLERK_JWT_KEY` is unset. So a workflow that bundles the global handlers is not necessarily one that
 * builds the verifier: `sandbox-identity-deploy.yml` bundles the DataStack's handlers and never exports the
 * key, so no edge bundle is produced there at all.
 *
 * ⚠️ That distinction is why the DSN assertion is not applied to every global deployer. In the first version
 * of this fix it was — which added an SSM read to a sandbox step that builds no edge bundle, and a guard
 * that certified it. An inert step is worse than a missing one, because it reads as coverage.
 *
 * @param step - A workflow step.
 * @returns Whether it exports the key the edge bundle requires.
 */
function exportsEdgeKey(step: WorkflowStep): boolean {
    return /CLERK_JWT_KEY/.test(withoutComments(step.run ?? ''));
}
...
describe('every workflow that deploys the global infra app bundles its handlers first', () => {
    ...
    /**
     * ⛔ THE EDGE VERIFIER'S DSN IS A BUILD INPUT, and nothing supplied it. Lambda@Edge accepts no
     * environment variables, so `esbuild.mjs` INLINES the DSN at bundle time from
     * `process.env['SENTRY_EDGE_DSN']` — a variable set in no workflow, no script and no action in this
     * repository. Every edge bundle ever shipped therefore carried an empty DSN and reported nothing, while
     * `/kitchensink/{stage}/sentry/edge-dsn` sat in SSM holding a perfectly good one. The symptom is an
     * absence of events, which is what a healthy quiet verifier looks like — the same shape as the ECS
     * command that discarded `--import ./dist/src/instrument.js`, and as ADR-0042's dead drain.
     *
     * ⚠️ It asserts the step TRIES, not that it succeeds. `esbuild.mjs` deliberately treats an absent DSN as
     * non-fatal (a verifier with no DSN reports nothing, which is how it behaved before, and making it fatal
     * would break every local bundle), so the parameter read tolerates absence. What must never happen again
     * is a bundle step that does not look.
     */
    it.each(workflows)('%s supplies the edge verifier its Sentry DSN when it builds one', (file) => {
        const steps = runSteps(file);

        // A workflow that never exports the Clerk key builds no edge bundle, so it owes no DSN.
        if (!steps.some(exportsEdgeKey)) {
            expect(steps.some(bundlesGlobalHandlers)).toBe(true);

            return;
        }

        const bundle = steps.find(bundlesGlobalHandlers);
        const run = withoutComments(bundle?.run ?? '');

        expect(bundle, `${file} has no global bundle step`).toBeDefined();
        expect(run, `${file} bundles the edge verifier without reading its DSN`).toMatch(/sentry\/edge-dsn/u);
        expect(run, `${file} reads the DSN but does not export it to the bundler`).toMatch(/SENTRY_EDGE_DSN/u);
    });

    it('⚠️ finds exactly one workflow that builds the edge verifier — it is prod-only', () => {
        const builders = workflows.filter((file) => runSteps(file).some(exportsEdgeKey));

        // Anchor: if this ever empties, the assertion above passes vacuously for every workflow.
        expect(builders).toEqual(['prod-deploy.yml']);
    });
