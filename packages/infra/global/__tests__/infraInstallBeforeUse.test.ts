 ...
 *
 * Two such jobs, each a job away from the action that would install the folder:
 *
 * - `sandboxPreview.yml` › `gate` runs `infra/bin/printFoodHost.ts`. Without the install every PR's
 *   "Decide whether this preview deploys" fails at `Cannot find package 'aws-cdk-lib'`.
 * - `deployInfra.yml` › `ecosystem-smoke` runs the same helper. It runs only on a real deploy, so the first
 *   sign of a missing install would be a red post-deploy step on production.
 *
    ...
    return stepLists().flatMap(({ label, steps }) => {
        return steps.flatMap((step, index) =>
            [...withoutComments(step.run ?? '').matchAll(INFRA_PATH)].flatMap((match) => {
                ...
                // `sandbox-identity-deploy.yml` and `sandboxUp.yml`'s verify targets all NAME an
