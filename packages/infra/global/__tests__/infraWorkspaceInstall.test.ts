 ...
 *
 * For `infra — global` that failure is a run of `TS2307`s, starting with
 * `File '@kitchensink/typescript/base.json' not found`. The Typecheck failure skips the package's unit Test step,
 * so none of its guards execute in CI, and the package jobs that `needs:` it are skipped with it — invisibly,
 * because it looks like a type error, not a missing install.
 *
...
const INFRA_WORKFLOW = path.join(repoRoot, WORKFLOWS_DIR, 'deployInfra.yml');
...
function infraJobs(): readonly InfraJob[] {
    ...
    return Object.entries(doc.jobs).flatMap(([job, definition]) =>
        (definition.steps ?? [])
            ...
            .map((step) => ({
                job,
                directory: scalarText(step.with?.['directory']),
                workspaceInstall: scalarText(step.with?.['workspace-install'], 'false') === 'true',
            })),
    );
}
