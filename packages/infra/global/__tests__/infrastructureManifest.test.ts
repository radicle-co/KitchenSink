 ...
 * dropped, so the hole announces itself. Same principle as `verifyDeployment.sh`'s `classify_reference`.
 *
 * ## Mutation evidence
 *
...
function scope(source: string, name: string) {
    ...
    if (found === undefined) {
        throw new Error(
            `fixture has no scope '${name}'; scopes: ${read.scopes.map((s: { name: string }) => s.name).join(', ')}`,
        );
    }
...
describe('readInfrastructureSource — resources', () => {
    ...
    it('reports a construct it does not classify instead of dropping it', () => {
        // The honest limit, made loud — the same contract `verifyDeployment.sh`'s `classify_reference`
    ...
    it('reports a construct from ANOTHER WORKSPACE as an unfollowed hole', () => {
        ...
        const shared = scope(
            `import { SharedThing } from '@radicle-co/infra-shared/alb';\nclass S extends Stack { constructor() { new SharedThing(this, 'T', {}); } }`,
            'S',
        );

        expect(shared.unfollowedConstructs).toEqual(['@radicle-co/infra-shared/alb.SharedThing']);
    });
 ...
 * PRECISELY how `docs/architecture/2026-08-28/ingredientPipelineState.md` §1 came to claim two handlers
...
describe('the manifest covers every CDK app', () => {
    ...
    it('reads a Lambda handler out of the stack this whole change exists for', () => {
        ...
        // ⚠️ Found by ENTRYPOINT, not by package name. A CDK app is attributed to its nearest manifest, and
        // `packages/services/recipe-workers/infra/` now carries its own so that `aws-cdk-lib` installs there
        // instead of hoisting into the service's image — which renamed the owner to
        // `@kitchensink/recipe-workers-infra`. The app's LOCATION is what this assertion is actually about,
        // and it does not move when the packaging does.
        const workers = buildManifest().apps.find((app) =>
            app.entrypoint.startsWith('packages/services/recipe-workers/'),
        );
