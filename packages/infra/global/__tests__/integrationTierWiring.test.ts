...
describe('every integration tier is called by CI', () => {
    ...
    it.each(workspacesWithIntegrationScript(PACKAGES_DIR).map((workspace) => [workspace.name, workspace]))(
        ...
        (_name, workspace) => {
            ...
            // ⚠️ A THIRD SHAPE, for packages outside the npm workspace. Every CDK directory installs on its
            // own (so `aws-cdk-lib` never reaches the root tree), and `--workspace=` cannot address a package
            // turbo and npm do not know — CI runs those with `working-directory:` plus a bare script. The
            // assertion is unchanged: the tier must be RUN by some step. Only the spelling widened, and it is
            // still anchored to this package's own directory rather than accepting any bare invocation.
            const dir = (workspace as Workspace).directory;
            const byWorkingDirectory =
                new RegExp(`working-directory:\\s*${dir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(ci) &&
                ci.includes('npm run test:integration');

            expect(
                byName || byPath || byWorkingDirectory,
                `${(workspace as Workspace).name} declares a \`test:integration\` script that no _ci.yml step ` +
                    `runs. The suite never executes — which looks exactly like a passing suite. Add a step ` +
                    `running \`npm run test:integration --workspace=${(workspace as Workspace).name}\`, or, ` +
                    `for a package outside the npm workspace, a step with \`working-directory: ${dir}\`.`,
            ).toBe(true);
        },
    );
});
