 ...
 * `heavyE2eLoadTierGate.test.ts` proves the tier is REACHED on the right triggers. Neither says anything
 ...
 * | 3 | a prepare step writing credentials OUTSIDE the ignore rule | `.gitignore`'s own note: the food tier grew its own `prepareClerkTokens.ts` "within hours and put 50 live JWTs … outside the rule" |
 *
 ...
 * recipe both ship a `serviceErasure.load.js`, so a step's argument is resolved against its
 ...
 *   3b. Continuation folding removed → food's `localStoreThroughput.load.js` reports unwired. Recorded
 ...
 *   7. `outDir` in `prepareClerkTokens.ts` repointed one level up → "mints them into the script's own
...
function artifactGlobs(step: WorkflowStep): readonly string[] {
    return scalarText(step.with?.['path'])
        .split('\n')
        .map((line) => line.trim())
...
function tokenPrepareScripts(): readonly {
    ...
    for (const service of serviceNames()) {
        ...
        for (const entry of readdirSync(loadDir).sort()) {
            if (/^prepare[A-Za-z0-9]*[Tt]oken[A-Za-z0-9]*\.ts$/.test(entry)) {
...
describe('every committed k6 load script is invoked by a workflow step', () => {
    it('is not vacuous: the tree ships load scripts for every service that has the tier', () => {
        ...
        expect(scripts).toContain('packages/services/identity/tests/load/sessionHotPath.load.js');
        expect(scripts).toContain('packages/services/food-service/tests/load/search.load.js');
        expect(scripts).toContain('packages/services/recipe-service/tests/load/sc009ReadWrite.load.js');
    });
    ...
    it("does NOT credit one service for another service's identically-named script", () => {
        // food and recipe both ship `serviceErasure.load.js`. Basename matching would let either job's step
                            ...
                            steps: [
                                {
                                    ...
                                    run: 'k6 run tests/load/serviceErasure.load.js',
                                },
                            ],
                        },
                    },
        ...
        expect(
            findUnwiredLoadScripts(['packages/services/recipe-service/tests/load/serviceErasure.load.js'], workflows),
        ).toEqual([]);
        expect(
        ...
        ).toMatch(/food-service.*serviceErasure\.load\.js → committed but invoked by no workflow step/);
    });
...
describe('every derived-tier invocation states its target', () => {
    // ⛔ `printLoadTier.ts` refuses to run without `--target` (a scenario may declare `@loadExcludeTarget`, and an
    // invocation that omitted the target would run it where it cannot mean anything). That refusal happens at RUN
    // time, inside a deployed k6 job; this makes a step that drops the flag fail here instead.
    it('passes --target on every printLoadTier.ts call in the real tree', () => {
        const calls = locatedSteps(realWorkflows()).flatMap(({ file, job, step }) =>
            (joinContinuations(step.run ?? '').match(/printLoadTier\.ts[^\n)]*/gu) ?? []).map((call) => ({
                site: `${file}::${job}::${stepLabel(step)}`,
                call,
            })),
        );

        expect(calls.length, 'is not vacuous: the deployed k6 job runs the derived tier').toBeGreaterThan(0);
        expect(
            calls.filter(({ call }) => !/--target\s+"?\$\{?LOAD_TEST_TARGET\}?"?/u.test(call)).map(({ site }) => site),
        ).toEqual([]);
    });
});
                            ...
                            steps: [
                                {
                                    ...
                                    run: 'k6 run tests/load/provisioningAndRename.load.js',
                                },
                                {
                                    name: 'Then reads',
                                    'working-directory': 'packages/services/identity',
                                    run: 'k6 run tests/load/sessionHotPath.load.js',
                                },
                            ],
                        },
                    },
                ...
                doc: {
                    jobs: {
                        load: {
                            ...
                            steps: [
                                { name: 'Erasure', run: 'k6 run tests/load/serviceErasure.load.js' },
                                { name: 'Read', run: 'k6 run tests/load/localStoreRead.load.js' },
                                { name: 'Throughput', run: 'k6 run tests/load/localStoreThroughput.load.js' },
