...
import { consumerConcurrencyFor } from '../../../services/recipe-workers/infra/lib/consumerConcurrency.js';
...
/**
 * The ONE expression the verifier's reservation may be written as.
 *
 * ⛔ A spelling, deliberately. The gate cannot evaluate the stack's stage, so it asserts PROVENANCE instead
 * of value: the reservation must come from the single policy module whose own suite holds non-prod at one.
 * Anything else — a literal, a ternary, an env read, a second module — is a different string and fails.
 */
const REQUIRED_RESERVED_CONCURRENCY_EXPRESSION = 'consumerConcurrency.reserved';
...
/**
 * The EXPRESSION each `*Function` construct sets `reservedConcurrentExecutions` to, as written.
 *
 * ⛔ Added because U8 made the verifier's reservation a per-stage POLICY rather than a literal, and the
 * numeric reader above is deliberately blind to that — it reports "undeclared", which is the same answer it
 * gives for the setting being deleted. Two very different changes must not look alike to a gate whose whole
 * job is to notice the second one.
 *
 * So the gate now asks a sharper question than "is it 1": it asks whether the reservation is the ONE
 * authority that is allowed to decide it. A literal, a ternary, an env read or a second policy module all
 * read differently here and all fail — while the policy module's own suite holds non-prod at one.
 *
 * @param source - One infra source file.
 * @returns Construct id to the expression text. Pure.
 */
export function reservedConcurrencyExpressionIn(source: SourceFile): ReadonlyMap<string, string> {
    ...
    visit(parse(source), (node) => {
        ...
        // ⛔ `referenceText`, not `getText()`. These nodes are parsed standalone, so `getText()` has no
        // source text to slice and throws — and more usefully, `referenceText` answers `undefined` for
        // ANYTHING that is not a plain dotted reference. A ternary, a literal, a call, an env read: all
        // unreadable here, which is the failing answer this gate wants for every one of them.
        const reference = declared === undefined ? undefined : referenceText(declared);

        if (reference !== undefined) {
            found.set(id.text, reference);
        }
    });
...
export function reservedConcurrencyIn(source: SourceFile): ReadonlyMap<string, number> {
    ...
    visit(parse(source), (node) => {
        ...
        // real function as undeclared. Numeric literals are read directly, and any OTHER expression is left
        // absent on purpose: "I could not read it" must fail rather than be interpreted.
...
describe('ADR-0024 layer 2 — the burn-rate bound', () => {
    /**
     * ⛔ REWRITTEN for U8, and the claim is UNCHANGED: **no stage is uncapped.** What changed is how it is
     * true. The verifier used to set one literal `1`, so stage-independence was a property of the syntax and
     * this gate read the number. U8 raises PROD to scale the queue (R1) while non-prod must keep the
     * reservation ADR-0024 layer 2 depends on — so the value is now a per-stage policy, and a gate that only
     * reads numeric literals reports it as UNDECLARED, which is the same answer it gives for the setting
     * being deleted. Two opposite changes must not look alike to the gate that exists to catch one of them.
     *
     * So the assertion moves up one level: the reservation must come from THE policy module, and that
     * module's own suite (`infra/__tests__/consumerConcurrency.test.ts`) holds non-prod at exactly one while
     * `RecipeWorkersStack.test.ts` asserts the synthesized pair per stage. A literal, a ternary, an env read
     * or a second policy module all read differently here and all fail.
     */
    it("⛔ sources the verifier's reservation from THE concurrency policy — never a literal or a branch", () => {
        const declared = infraSources()
            .flatMap((source) => [...reservedConcurrencyExpressionIn(source)])
            .filter(([id]) => id === VERIFIER_FUNCTION);

        expect(declared, `${VERIFIER_FUNCTION} must declare reservedConcurrentExecutions`).toHaveLength(1);
        expect(declared[0]?.[1]).toBe(REQUIRED_RESERVED_CONCURRENCY_EXPRESSION);
    });

    it('⛔ the policy it sources from still caps EVERY non-prod stage at one', () => {
        // The other half of the claim, asserted against the policy itself rather than against its spelling.
        // This is what makes the indirection safe: the gate no longer reads the number, so something must.
        for (const stage of ['sandbox', 'pr-1', 'pr-91', 'dev']) {
            expect(consumerConcurrencyFor(stage).reserved, stage).toBe(REQUIRED_RESERVED_CONCURRENCY);
        }
    });

    it('detects the reservation being sourced from anything ELSE', () => {
        const found = reservedConcurrencyExpressionIn({
            file: 'fake/OwnPolicy.ts',
            contents: `new lambda.Function(this, '${VERIFIER_FUNCTION}', { reservedConcurrentExecutions: stage === 'prod' ? 10 : 1 });`,
        });

        expect(found.get(VERIFIER_FUNCTION)).not.toBe(REQUIRED_RESERVED_CONCURRENCY_EXPRESSION);
    });
