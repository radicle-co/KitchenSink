...
function selectorExpression(): string {
    const value = caller.jobs?.['e2e-mobile-maestro']?.with?.[SELECTOR_INPUT];

    if (value === undefined) {
        throw new Error(
            `${CALLER_FILE}::e2e-mobile-maestro passes no \`${SELECTOR_INPUT}\` input, so the emulator job can never be ` +
...
describe('the workflow thread — caller → reusable workflow → job env → script', () => {
    it('composes one selector pair per vertical the script accepts, and no others', () => {
        ...
        // literal `false` asserted one case down, and the full-run door moved to `deployed-e2e.yml`.
    ...
    it('⛔ asks for a NARROWED set, and leaves the full-run door to the workflow that owns it', () => {
        ...
        //
        // ⛔ The full run did not disappear with it. ADR-0032 §3 makes `deployed-e2e.yml` the ONE manual
        ...
        // matches no vertical attributes nothing, and `run-maestro-flows.sh` treats that as attribution
