 ...
 * it); choosing FEWER FLOWS is the big one. `recipes/collectionsPagination` alone is 7.75m — 22% of all
 ...
 * 2. **A flow that exists and never runs.** `recipes/ingredientCatalogBlend.yaml` was committed, reviewed,
 *    and then never executed by CI, because `runMaestroFlows.sh` iterates an explicit list and nobody added
 ...
 * with a usage error, and the inventory assertion failed on `recipes/ingredientCatalogBlend` — the real
 ...
 *     assertions fail (loginFlow first, `accountDangerZone` before `recipes/delete`, delete last).
 */
 ...
 * (same posture as `KNOWN_BUILDS_AFTER_PRUNE` in `workflowInvariants.test.ts`): the equality assertion
 ...
 *   - `recipes/sourceTabs` — the My recipes ⇄ Discover round trip (L5). Its web pair caught a one-way trip
 *     that no per-surface test could see, so the mobile half is worth having. Belongs to the `discovery`
 *     vertical when promoted.
 *   - `recipes/ingredientCorrection` — U14's teach-the-resolver control. ⛔ UNSATISFIABLE in the Maestro
...
const KNOWN_UNRUN_FLOWS: readonly string[] = [
    // The offline write path: the sync layer beneath these is complete and unit-tested
    // (`@kitchensink/sync`, `@commise/query/sync`), and both apps mount the queue — but NO recipe write is
    // registered with it yet, so `submit` has zero call sites and nothing enqueues. These flows assert the
    // cook-visible half (a save that succeeds offline, a queued write surviving an app restart), which is
    // exactly the behaviour that does not exist until registration lands.
    //
    // ⛔ RECORDED HERE RATHER THAN DELETED, because the restart flow is the ONLY test that can prove the
    // outbox is genuinely durable — everything else about offline is reproducible with an in-memory queue.
    // Losing it would lose the one check that justifies persisting on device at all. Promote both to
    // FLOW_PLAN in the same change that registers the first recipe write.
    'offline/offlineSave',
    'offline/offlineSurvivesRestart',
    'homeRecentRecipeTap',
    'recipes/sourceTabs',
    'recipes/ingredientCorrection',
    ...
    'recipes/ingredientCatalogBlend',
];
...
/**
 * The workflow that RELAYS the selector: every deployed tier moved into it, so `_ci.yml` computes the selector
 * and hands it to that workflow, whose Maestro job hands it on to `_ci-heavy.yml` unchanged.
 */
const RELAY_FILE = 'deployedE2eTiers.yml';
const relay = loadWorkflow(RELAY_FILE);

/**
 * The selector expression the caller passes to the reusable workflow.
 *
 * REWRITTEN when the Maestro call moved into the relay: the expression is read where it is COMPUTED (the
 * caller's call of the relay), and the relay hop is asserted here, in the one helper every case goes through,
 * so a relay that drops the input fails every case rather than none.
 */
function selectorExpression(): string {
    const relayCall = Object.entries(caller.jobs ?? {}).find(
        ([, job]) => job.uses === `./.github/workflows/${RELAY_FILE}`,
    );
    const value = relayCall?.[1].with?.[SELECTOR_INPUT];

    if (value === undefined) {
        throw new Error(
            `${CALLER_FILE} passes no \`${SELECTOR_INPUT}\` input to ${RELAY_FILE}, so the emulator job can never be ` +
                'told which vertical was implicated and every run pays the full 35.5m of flows.',
        );
    }

    const relayed = scalarText(relay.jobs?.[MAESTRO_JOB]?.with?.[SELECTOR_INPUT]);
    const declared = (relay.on?.['workflow_call'] as { inputs?: Record<string, { default?: unknown }> } | undefined)
        ?.inputs?.[SELECTOR_INPUT];

    if (relayed !== `\${{ inputs.${SELECTOR_INPUT} }}` || declared === undefined || (declared.default ?? '') !== '') {
        throw new Error(
            `${RELAY_FILE}::${MAESTRO_JOB} does not relay \`${SELECTOR_INPUT}\` verbatim from a fail-safe (empty) input ` +
                `— it passes '${relayed}'.`,
        );
    }
...
function detectFilters(): Readonly<Record<string, readonly string[]>> {
    const step = (caller.jobs?.['detect-mobile-flows']?.steps ?? []).find((candidate) =>
        /dorny\/paths-filter@/.test(candidate.uses ?? ''),
    );

    if (step === undefined) {
        throw new Error(`${CALLER_FILE}::detect-mobile-flows has no pinned \`dorny/paths-filter\` step`);
    }

    return parse(scalarText(step.with?.['filters'])) as Readonly<Record<string, readonly string[]>>;
}
...
const VERTICAL_PROBES: Readonly<Record<string, readonly string[]>> = {
    auth: [
        ...
        'packages/apps/commise/mobile/.maestro/accountDangerZone.yaml',
    ],
    home: [
 ...
 *
 * `appShell.ts` is the finding that forced `features/core` to be attributed per MODULE instead of as
 * `src/**`: it is imported by `AppRoot`, so it can break every screen, while its neighbours cannot. A
 * package-level mapping would have run every flow for a change reaching any module in the package — an
 * over-selection nothing would have reported.
 *
 * ⚠️ `queryStatus.ts` used to sit here too, for the same reason via `RecipeVersionsScreen`. It was DELETED
 * when the last non-Suspense read (the mobile profile screen) was converted, which left the repo with one
 * read pattern and this list with a probe pointing at nothing.
 */
const CROSS_CUTTING_PROBES: readonly string[] = [
    ...
    'packages/apps/commise/mobile/tests/e2e/runMaestroFlows.sh',
...
describe('the flow inventory — every committed flow is accounted for', () => {
    it('runs every story flow, or classifies it as a sub-flow / inert / a recorded gap', () => {
        ...
        expect(
            ...
            'a flow that is committed but executed by nothing is a test that does not exist. Add it to ' +
                'FLOW_PLAN in runMaestroFlows.sh (with its vertical), or record it in KNOWN_UNRUN_FLOWS ' +
                'with the reason it cannot run yet.',
        ).toEqual([]);
    });

    it('keeps ingredientCatalogBlend ACCOUNTED FOR, now that it cannot run here', () => {
        ...
        expect(ALL_FLOWS).not.toContain('recipes/ingredientCatalogBlend');
        expect(KNOWN_UNRUN_FLOWS).toContain('recipes/ingredientCatalogBlend');
    });

    /**
     * ⛔ EXISTENCE IS NOT RUNNABILITY, and this test used to assert only the former.
     *
     * `recipes/createAuthoredFood.yaml` was committed as a ZERO-BYTE file and sat in `FLOW_PLAN` for its
    ...
    it('never plans a sub-flow as a story (a `runFlow` target is not a test)', () => {
        ...
        // `auth/loginFlow` deliberately `runFlow`s `signin.yaml`, so the sub-flows ARE covered — as part of
...
describe('the plan — ordering invariants the flows depend on', () => {
    it('starts with the only signed-out flow, so a cleared session is re-established first', () => {
        expect(ALL_FLOWS[0]).toBe('auth/loginFlow');
        expect(SPINE).toEqual(['auth/loginFlow']);
    });

    it('runs accountErasure last, after recipes/delete, with the cancel-only flow before both', () => {
        // ⛔ `accountErasure` is last because it REALLY ERASES now. Against the runner-local stack its
        ...
        expect(ALL_FLOWS.at(-1)).toBe('accountErasure');
        expect(ALL_FLOWS.indexOf('recipes/delete')).toBeLessThan(ALL_FLOWS.indexOf('accountErasure'));
        expect(ALL_FLOWS.indexOf('accountDangerZone')).toBeLessThan(ALL_FLOWS.indexOf('recipes/delete'));
    });
...
/**
 * The Maestro tier's share of Clerk's EMAIL-CODE verification budget: two per full run, and only on purpose.
 *
 * The deployed k6 pool failed on Clerk refusing its email-code steps, and that counter's window and scope are
 * unmeasured (possibly instance-wide, shared with every tier signing in against the sandbox dev instance). On
 * the device a sign-in goes through `LoginScreen`: email, password, then a new-device email code — one
 * verification. The credential block in `auth/signinHome.yaml` runs only when the login form is showing, and
 * the app's session persists in `expo-secure-store` between flows (the runner reinstalls the DRIVER, never the
 * app, and `e2e-seed reset` resets the database), so the form shows only after `clearState`. Two planned flows
 * clear it, and each is a UI sign-in on purpose:
 *
 *   - `auth/loginFlow` — the spine, and the mobile sign-in UI test itself;
 *   - `accountErasure` — must sign in as a DIFFERENT identity (the run's erasure subject).
 *
 * ⚠️ Replacing `accountErasure`'s UI sign-in with `e2e-seed`'s stored session is NOT available: that handle
 * belongs to the runner's Clerk client (a dev-browser JWT on the host), and nothing lets `@clerk/expo` on the
 * device adopt it without a new, test-only sign-in entry point shipped in the release APK. It would save one
 * verification per run; it is recorded as the option, not built.
 *
 * ⚠️ Scope is the PLANNED flows. Three `visual/*` flows also `clearState` (and so would each spend one), but they are
 * excluded from the plan while `assertScreenshot` has no baselines — activating them grows the budget, and the
 * first assertion below will say so the moment one is planned.
 *
 * ⚠️ A flow that LOSES its session mid-run (an app crash that also loses secure storage, a revoked session) would
 * silently re-sign-in through the same guarded block and spend another verification. That cannot be counted
 * statically; it shows up as an extra `Verification code` step in the flow's log.
 */
describe('the Clerk email-code budget — only the flows that test sign-in clear the session', () => {
    const planned = (): readonly { readonly flow: string; readonly body: string }[] =>
        ALL_FLOWS.map((flow) => ({ flow, body: readFileSync(join(MAESTRO_DIR, `${flow}.yaml`), 'utf8') }));

    it('clears app state only in the sign-in UI test and the erasure flow', () => {
        const clearing = planned()
            .filter(({ body }) => /^\s*clearState:\s*true\b/mu.test(body))
            .map(({ flow }) => flow);

        expect(
            clearing.sort(),
            'a flow that clears state forces a UI sign-in, which spends a shared Clerk email-code verification',
        ).toEqual(['accountErasure', 'auth/loginFlow']);
    });

    it('types the fixed email code in exactly one flow — the guarded credential sub-flow', () => {
        const typing = discoverFlowFiles().filter((flow) =>
            /^\s*-\s*inputText:\s*['"]?424242/mu.test(readFileSync(join(MAESTRO_DIR, `${flow}.yaml`), 'utf8')),
        );

        expect(typing).toEqual(['auth/signinHome']);
    });

    it('keeps that credential block guarded on the login form being visible', () => {
        const body = readFileSync(join(MAESTRO_DIR, 'auth/signinHome.yaml'), 'utf8');
        const guard = body.indexOf("visible: 'you@example.com'");

        expect(guard, 'without the guard every flow would sign in again').toBeGreaterThan(0);
        expect(body.indexOf("inputText: '424242'")).toBeGreaterThan(guard);
    });

    it('never wipes the app between flows — only the Maestro driver is reset', () => {
...
describe('selection — a narrowed run is a SUBSEQUENCE of the plan, never a re-ordering', () => {
    it('selects the spine plus the auth flows for an auth-only change', () => {
        ...
        // `accountErasure` joined the auth vertical with plan U2's mobile erasure story — the flow that
        ...
        expect(selection.flows).toEqual(['auth/loginFlow', 'accountDangerZone', 'accountErasure']);
        ...
        expect(selection.skipped).toContain('recipes/collectionsPagination');
    });
    ...
    it('keeps EVERY selection a subsequence of the plan (order is load-bearing, not cosmetic)', () => {
        // `recipes/delete` must stay last and `accountDangerZone` ahead of it; a selector that concatenated
        ...
        for (const vertical of VERTICALS) {
            ...
            expect(flows[0]).toBe('auth/loginFlow');
        }
    });
...
describe('selection — vertical membership is matched on WHOLE tokens', () => {
    ...
    const TRAP_PLAN =
        'spine:auth/loginFlow home:home home:homeRecentRecipeTap recipes:recipes/create recipesx:recipes-archive';
    ...
    it('does not let a vertical name match a longer vertical name', () => {
        ...
        expect(flows).toEqual(['auth/loginFlow', 'recipes/create']);
    ...
    it('selects both flows of one vertical without matching the other vertical`s prefix', () => {
        ...
        expect(flows).toEqual(['auth/loginFlow', 'home', 'homeRecentRecipeTap']);
    });
...
describe('the workflow thread — caller → reusable workflow → job env → script', () => {
    it('composes one selector pair per vertical the script accepts, and no others', () => {
        // ⚠️ `full` is no longer among the FORMATTED pairs, and that is the migration rather than a gap.
        // In `heavy-e2e.yml` it was a computed argument, because that workflow served both the narrowing PR
        // path and the full nightly/dispatch path. This caller is only ever a pull request, so `full` is the
        // literal `false` asserted one case down, and the full-run door moved to `deployedE2e.yml`.
        const expression = selectorExpression();
        const named = [...expression.matchAll(/(\w+)=\{\d+\}/g)].map((match) => match[1] ?? '');

        expect([...named].sort()).toEqual([...VERTICALS].sort());
        expect(expression, 'every pair the script accepts must still be present').toContain('full=');
    });
    ...
    it('feeds every pair from a paths-filter key that exists and has positive patterns', () => {
        ...
        for (const vertical of VERTICALS) {
            ...
            expect(selectorExpression()).toContain(`needs.detect-mobile-flows.outputs.${key}`);
    ...
    it('⛔ asks for a NARROWED set, and leaves the full-run door to the workflow that owns it', () => {
        // ⚠️ REWRITTEN, not relaxed. This used to assert a full-set clause covering the `heavy-e2e` label,
        // the non-PR events and a dispatch — all three of which belonged to `heavy-e2e.yml`, which is
        // deleted. This caller runs on a pull request and nothing else, so there is no event here that
        // lacks a diff to narrow against, and a computed `full` would only ever render `false`.
        //
        // ⛔ The full run did not disappear with it. ADR-0032 §3 makes `deployedE2e.yml` the ONE manual
        // door, and it reaches the same reusable WITHOUT this input — taking the fail-safe default asserted
        // in the next case, which means "run every flow". Coverage is not at risk either way: a change that
        // matches no vertical attributes nothing, and `runMaestroFlows.sh` treats that as attribution
        // FAILURE and runs the whole suite.
        const expression = selectorExpression();

        expect(expression).toContain('full=false');
        expect(expression, 'this caller has no non-PR event to widen for').not.toContain(
...
describe('the path mapping — narrow where it is provable, widen everywhere else', () => {
    ...
    it('does NOT give the food service a trigger — the catalog flow asserts the DEGRADED path', () => {
        // `recipes/ingredientCatalogBlend` now RUNS, and it still buys the food service no trigger: the
        ...
        const foodPaths = [
            ...
            'packages/services/food-service/src/foods/dao/foodSearch.dao.ts',
        ];
