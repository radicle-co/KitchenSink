 ...
 *
 * `recipes/parseIngredients` therefore failed on `Assert that "2 of 2 lines read" is visible`, twice, after
...
export function auditParsePath(job: WorkflowJob): readonly ParsePathFinding[] {
    ...
    if (consumerIndex !== -1 && scalarText(steps[consumerIndex]?.env?.['RECIPE_PARSE_QUEUE_URL']) !== producedUrl) {
