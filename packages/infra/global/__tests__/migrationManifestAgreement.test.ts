 ...
 * `runMigrations.sh` reading the working tree, and `@kitchensink/db-schema-guard` inside the shipped
...
const SCRIPT = join(REPO_ROOT, '.github', 'scripts', 'runMigrations.sh');
...
/**
 * The digest `runMigrations.sh manifest` computes for a directory.
 *
...
function makeMigrationsDir(files: Readonly<Record<string, string>>): string {
    const dir = scratchDirectory('manifest-agreement-');
...
describe('migration manifest — shell and TypeScript agree', () => {
    ...
    it('agrees on every REAL service migrations directory, not only on fixtures', () => {
        ...
        for (const dir of dirs) {
            ...
            expect(shell.status, `runMigrations.sh manifest failed for ${dir}`).toBe(0);
