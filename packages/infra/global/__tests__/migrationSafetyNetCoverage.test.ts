 ...
 * `CfnOutput`. That is the same shape `.github/scripts/teardownSandboxPr.sh` §1 uses to find per-PR
...
import { workflowJobs } from './cdkApps.js';
import { publishedRunnerOutputs, readRepoFile, stackSources } from './migrationRunners.js';

/**
 * A workflow's text with the composite actions it uses resolved into it.
 *
 * ⛔ The migration invoke and the runner's NAME no longer live in the same file. `runMigrations.sh run` is
 * written once, in `.github/actions/infra-package/action.yml`, against `"${EXPORT_NAME}"`; the literal
 * `FoodMigrationFunctionName` is supplied at the call site in `deployInfra.yml`. Reading either file alone
 * matches neither — and a coverage guard that matches nothing reports full coverage.
 */
function resolvedWorkflow(file: string): string {
    const basename = file.split('/').slice(-1)[0] ?? file;
    const bodies = workflowJobs()
        .filter((job) => job.workflow === basename)
        .map((job) => job.body);

    return bodies.length === 0 ? readRepoFile(file) : bodies.join('\n');
}
...
const SANDBOX_DEPLOYS = [
    // ⚠️ `deployInfra.yml`: every `cdk deploy` and every migration invoke moved there, one job per
    // `infra/` folder, so a single workflow owns each stack. `sandboxPreview.yml` only decides and
    // delegates now, and holds no runner invoke of its own.
    '.github/workflows/deployInfra.yml',
    '.github/workflows/sandbox-identity-deploy.yml',
];

/**
 * An output no `runMigrations.sh` invoke names, in the workflows it must appear in.
 *
...
/**
 * The output ids passed to `runMigrations.sh run` in one workflow's text.
 *
...
export function runnerOutputsInvokedBy(workflow: string): readonly string[] {
    // ⚠️ The delimiter is `[\s"]`, not `\s`. The invoke is written once in a composite action and passes its
    // arguments QUOTED (`"${EXPORT_NAME}"`), which is correct shell and which a whitespace-only delimiter
    // does not match — so the coverage check found nothing and reported everything covered.
    return [
        ...workflow.matchAll(/runMigrations\.sh\s+run\s+[\s\S]{0,200}?["\s]([A-Za-z]+MigrationFunctionName)["\s]/g),
    ].map((match) => match[1] ?? '');
}
...
describe('every migration runner a pipeline can reach has an ADR-0022 §4 safety net', () => {
    ...
    it('agrees with the teardown script, which finds the same doors by the same shape', () => {
        ...
        expect(readRepoFile('.github/scripts/teardownSandboxPr.sh')).toContain('MigrationFunctionName');
    });

    it('invokes each of them from prod and from a sandbox deploy', () => {
        const invokedIn = Object.fromEntries(
            [PROD_DEPLOY, ...SANDBOX_DEPLOYS].map((file) => [file, runnerOutputsInvokedBy(resolvedWorkflow(file))]),
        );
    ...
    it('reads the output id out of a real invocation, and never out of prose', () => {
        expect(
            runnerOutputsInvokedBy(
                'bash .github/scripts/runMigrations.sh run "${REGION}" \\\n  "stack-${STAGE}" FoodMigrationFunctionName food\n',
            ),
