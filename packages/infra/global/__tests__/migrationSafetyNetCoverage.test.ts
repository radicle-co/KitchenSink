 ...
 *
 * ⛔ The migration invoke and the runner's NAME no longer live in the same file. `run-migrations.sh run` is
 * written once, in `.github/actions/infra-package/action.yml`, against `"${EXPORT_NAME}"`; the literal
 * `FoodMigrationFunctionName` is supplied at the call site in `deploy-infra.yml`. Reading either file alone
...
const SANDBOX_DEPLOYS = [
    // ⚠️ `deploy-infra.yml`: every `cdk deploy` and every migration invoke moved there, one job per
    // `infra/` folder, so a single workflow owns each stack. `_sandbox-preview.yml` only decides and
...
export function runnerOutputsInvokedBy(workflow: string): readonly string[] {
    ...
    return [
        ...workflow.matchAll(/run-migrations\.sh\s+run\s+[\s\S]{0,200}?["\s]([A-Za-z]+MigrationFunctionName)["\s]/g),
    ].map((match) => match[1] ?? '');
}
