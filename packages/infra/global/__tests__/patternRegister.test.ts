...
const REF_SITES: Readonly<Record<string, RefSite>> = {
    ...
    'web/components/recipes/AuthoredFoodCreateForm': {
        verdict: 'sanctioned',
        why: '`reuseRef` and nothing else: when the duplicate branch replaces the form, the control the cook last used is gone and focus is on <body>, so a mount-once effect calls `.focus()` on the reuse control — and ONLY when `document.activeElement` is <body>/null, because the branch mounts on a network response and must not pull focus from a cook already typing elsewhere. `autoFocus` cannot express that condition; `.focus()` has no declarative form.',
    },
    'web/components/recipes/IngredientPicker': {
        verdict: 'sanctioned',
        why: 'Two focus handles, both for `.focus()`, which has no declarative form. (1) `useImperativeHandle` publishes ONE method, `focusSearch`, over the search input; the node itself never escapes. The declarative alternative was considered and rejected in writing at `ingredientResolver.model.ts` — a `focusSignal` epoch prop still needs a ref and an effect inside the picker and makes correctness depend on observing a render exactly once. (2) `createFoodTriggerRef`: cancelling the create-your-own-food form removes the element holding focus, so focus returns to the disclosure that opened it (WCAG 2.2 SC 2.4.3). `useReturnFocusOnClose` was considered and rejected: it snapshots `document.activeElement` from Radix `onCloseAutoFocus`, and this form is not a Radix dialog — the return target is known in advance, so a snapshot would be a second source of the same fact. (3) `liveSearchTriggerRef`: the live-search Try again button is removed as the retry starts, so focus moves first to the Search USDA control, which runs the same search and stays mounted (SC 2.4.3). The search-input handle is also read by one effect after a line is added — the pressed control is gone, so focus returns to the search box, only when focus was lost or is still inside the picker; an effect because on the disambiguation path the input remounts on that render.',
    },
    ...
    'mobile/components/IngredientPicker': {
        verdict: 'sanctioned',
        why: 'The native mirror of the web picker — `useImperativeHandle` publishing `focusSearch` over a `TextInput`, node never escaping, same rejected alternative recorded. Two more handles and one effect, all for SCREEN-READER focus (`AccessibilityInfo.sendAccessibilityEvent`, which takes a node and has no declarative form), never keyboard focus: `createFoodTriggerRef` (cancel removes the control the reading cursor is on, so it returns to the disclosure) and `liveSearchTriggerRef` (Try again is removed as the retry starts, so the cursor moves to the Search USDA control first); and after a line is added, an effect moves the cursor back to the search input, since the pressed row is gone. Same WCAG 2.2 SC 2.4.3 reasoning as the web entry, adapted rather than copied.',
    },
 ...
 *    the components. The DOM handle that stayed inside the hook became state on 2026-09-13, when
 *    `react-hooks/refs` flagged its render-time write, so the hook holds no ref.
...
const REF_MODULES: Readonly<Record<string, RefSite>> = {
    'packages/apps/commise/ui/src/screenReaderFocus/useScreenReaderFocusOnMount.native.ts': {
        verdict: 'sanctioned',
        why: "ONE `useRef` over a mounted native host node whose sole use is `AccessibilityInfo.sendAccessibilityEvent(node, 'focus')` — the imperative screen-reader API React Native requires, with no declarative form. It moves the READING cursor only (no keyboard), runs once per mount, and the node never leaves the module except through the ref the caller attaches.",
    },
    'packages/apps/commise/ui/src/backIntercept/useBackIntercept.native.ts': {
        verdict: 'sanctioned',
        why: "ONE `useRef`, `latest`, a latest-callback latch over the interceptor a component hands to the ancestor back-press registry. It holds nothing render-affecting and nothing derived: the registration identity must be STABLE (a link's position in the chain is its MOUNT order, and re-registering per render would put a host ahead of the very surfaces it hosts in React Native's LIFO list — the defect this seam fixes) while the function it calls must be the CURRENT render's. A handler captured at mount closes over mount-time state, which for the recipe wizard is `isDirty === false` FOREVER — hardware back would discard the cook's unsaved draft behind a suite that only ever pressed back on a clean form. It is assigned in an EFFECT, never the render body (the hazard `useRecipeEditor`'s `submitDraftRef` was rewritten for), and read only from the registry's dispatch. ⚠️ `useEffectEvent` is deliberately NOT the answer here and the refusal is narrow: React documents an Effect Event as non-transferable out of the component that declares it, and this wrapper is handed to a registry owned by an ANCESTOR — it happens to work, but a data-loss guard must not rest on unspecified behaviour. The provider one file over IS the legal `useEffectEvent` case (declared and consumed in one component) and therefore carries no ref at all. The latch is pinned by a mutation-proven test: `backIntercept.native.test.tsx`'s latest-closure case fails with `[0]` against a mount-captured registration.",
    },
    'packages/apps/commise/ui/src/dialogFocus/useFocusOnSignal.ts': {
        verdict: 'sanctioned',
        why: "`node` holds a DOM element whose sole use is the imperative `.focus()` (through `focusIfLost`), which has no declarative form, and it never leaves the module except through the ref the caller attaches. `seen` is a previous-value latch compared and advanced INSIDE the effect, the `useScrollResetOnChange` shape: a discarded render can neither consume the signal's change nor advance the latch, and a remount with a non-zero signal moves nothing. The IngredientPicker entry's rejection of a `focusSignal` epoch prop was about correctness depending on observing one render exactly once; comparing the value inside a committed effect is what removes that dependency.",
    },
    'packages/apps/commise/ui/src/screenReaderFocus/useScreenReaderFocusOnSignal.native.ts': {
        verdict: 'sanctioned',
        why: "The native twin: `node` is a host node whose sole use is `AccessibilityInfo.sendAccessibilityEvent(node, 'focus')` (through `moveScreenReaderFocus`), the imperative screen-reader API with no declarative form, moving the reading cursor only. `seen` is the same in-effect previous-value latch as the web hook.",
    },
 ...
 *
 * ⚠️ It went 129 → 127 when the Home nudge CONTEXT moved into a module of its own: `HomeNudgeContext` had
 * been counted as a component on both platforms, stating no layer on either, and a context provider is not
 * a component to classify. So this is TWO components leaving the census, not two gaining a layer word —
 * which is why the number went down without anybody documenting anything. It was masked for a while: the
 * committed `docs/generated/components` census was stale by exactly those two entries, and turbo caches
 * this package's `test` task against its OWN inputs, so a change anywhere else in the repo does not
 * invalidate it. Run this suite directly after a structural change; a green `npm run test` does not prove
 * a repo-wide guard ran.
 *
 * ⚠️ It went 124 → 125 when `RefreshIcon` joined `wizard/icons.tsx` for the parse flow's retry action. An
 * icon glyph is one of the two shapes §11.2 puts OUT of scope for a layer statement — it is a `<path>` with
 * no state, no branching and no data — so the increment is the census recording a new glyph, not a
 * component that forgot to say what it is. Its seven siblings state no layer either. ⛔ Do not "fix" this
 * by writing a layer word into an icon: that is the census-as-backlog reading CLAUDE.md rules against.
 */
...
const DECLARED_ORCHESTRATION: readonly string[] = [
    ...
    'featuresRecipes/detail/RecipeDetailView',
    'featuresRecipes/detail/RecipeHero',
