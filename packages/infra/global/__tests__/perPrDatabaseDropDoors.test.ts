 ...
 * longer invokes those doors at all: `teardownSandboxPr.sh` §1 now invokes `PerPrDatabaseReaperFunction`,
 ...
 *
 * `.github/scripts/teardownSandboxPr.sh` §1 dropped exactly ONE of them. It hardcoded the stack name
...
const TEARDOWN_SCRIPT = '.github/scripts/teardownSandboxPr.sh';
...
describe('per-PR logical databases (ADR-0006) all have a drop door', () => {
    ...
    it('⛔ is covered by the REAPER — its base register equals the families the tree derives', () => {
        // ⛔ THE CLAIM ADR-0031 RESTS ON. `teardownSandboxPr.sh` §1 invokes one reaper for the whole PR,
