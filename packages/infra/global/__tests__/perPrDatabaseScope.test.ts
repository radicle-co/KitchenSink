 ...
 * regression suite that executes it, exactly like `.github/scripts/prScope.sh` and
 ...
 * `teardownSandboxPr.sh` applies to GitHub Environments ("the scope predicate let it through, which is a
 * bug in prScope.sh").
 *
...
describe('isReapablePerPrDatabase — the verdict that authorises a DROP', () => {
    ...
    it('⛔ refuses pr-1 the database of pr-15, pr-100 and pr-01', () => {
        // The delimiter trap `prScope.sh` was written around, arriving here as a SUFFIX rather than a
