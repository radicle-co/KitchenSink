...
function fixture(files: Readonly<Record<string, string>>): readonly Workflow[] {
    const directory = scratchDirectory('post-prune-toolchain-');
...
describe('after `npm prune --omit=dev`, a job needs nothing the prune removed', () => {
    ...
    it('is NOT vacuous: the real tree prunes somewhere, and the catalog classifies the known tools', () => {
        ...
        //
        // ⛔ `cdk` is DELIBERATELY absent. This catalog is built from the ROOT lockfile, and the CDK CLI is no
        // longer a root dependency of any kind — it belongs to each `infra/` package, which installs outside
        // the npm workspace and is therefore untouched by `npm prune --omit=dev`. That is what lets the
        // post-prune deploys run `npm exec --prefix <app> -- cdk` without the root carrying a production
        // dependency that would ship into every image built from this tree. Asserted as absent rather than
        // deleted, so re-adding it to the root fails HERE with the reason.
        expect(catalog.packageForBin('cdk')).toBeUndefined();
        expect(catalog.packageForBin('tsc')).toBe('typescript');
        expect(catalog.classify('typescript')).toBe('declared-runtime');
        // ⚠️ `declared-runtime`, not `workspace`, and the flip is deliberate. The shared CDK constructs used
        // to be a workspace devDependency (`@kitchensink/infra-security`); they are now
        // `@radicle-co/infra-shared`, declared as a RUNTIME dependency because `infra-global`'s stacks import
        // them at SYNTH time and `prod-deploy.yml` runs `cdk deploy` on the far side of its prune. Both
        // classifications survive the prune — which is the property this anchor is really about — but the
        // label changed with the declaration, so the premise is restated rather than the expectation quietly
        // adjusted.
        expect(catalog.classify('@radicle-co/infra-shared')).toBe('declared-runtime');
...
function sourceTree(file: string, contents: string): string {
    const root = scratchDirectory('inert-entrypoint-');
...
describe('every compiled entrypoint a workflow runs under `node` can actually execute', () => {
    ...
    /**
     * ⛔ THE INVARIANT INVERTED, and the inversion is the safety property.
     *
     * This anchored non-vacuity on two compiled entrypoints the workflows ran under plain `node`. There are
     * none left, and there must not be: a CDK app or smoke script launched by bare `node` cannot load a
     * workspace package, because every one of them exports raw `.ts` whose internal specifiers end in `.js`,
     * and Node resolves those LITERALLY — `extensionAlias` is a TypeScript idea Node has no equivalent for.
     * That is not theoretical; it took down a production-shaped deploy with
     * `Cannot find module .../packages/shared/messaging/src/OutboundMessage.js`, and it reproduces in one
     * line: `node -e "import('@kitchensink/messaging')"`.
     *
     * Every CDK entrypoint now runs under its own package's `tsx`, which applies that mapping. So the useful
     * assertion is the opposite of the old one: nothing may go back to compiled-under-`node`.
     */
    it('runs NO infra entrypoint under bare node — that cannot load a raw-TS workspace package', () => {
        const invocations = realWorkflows().flatMap(({ file, jobs }) =>
            jobs.flatMap(({ steps }) =>
                steps.flatMap((step) =>
                    [...shellCode(step.run ?? '').matchAll(NODE_INVOCATION)]
                        .map((match) => match[1] ?? '')
                        .filter((target) => /\/infra\/|\/infra\b|packages\/infra\/global/u.test(target))
                        .map((target) => `${file}: node ${target}`),
                ),
            ),
        );
        ...
        expect(
            invocations,
            "Launch it with the package's own `tsx` instead: `<pkg>/infra/node_modules/.bin/tsx " +
                '<pkg>/infra/bin/app.ts`. Bare `node` resolves a `.js` specifier inside a `.ts` file ' +
                'literally, so the first workspace import fails with ERR_MODULE_NOT_FOUND.',
        ).toEqual([]);
    });
});
...
describe('a deploy-only tool declared at the root stays out of the service images', () => {
    /**
     * ⛔ THE INVARIANT INVERTED, and this records why rather than quietly tracking it.
     *
     * This used to assert a PAIR: the CDK CLI is a root `dependencies` entry (so it survives
     * `npm prune --omit=dev` and the post-prune `cdk deploy` steps still work) AND `.dockerignore` re-excludes
     * it (so that declaration does not grow every image). Both halves were needed because the root tree is
     * COPIED into the images — `identity/Dockerfile` does `COPY node_modules ./node_modules`.
     *
     * That pair was two compensating mechanisms for one wrong declaration. A build tool is not a production
     * dependency of anything, and declaring it as one cost 24 MB in every image built from this tree plus a
     * `rm -rf node_modules/aws-cdk-lib …` line in `food-service/Dockerfile` to claw some of it back.
     *
     * Now each CDK package owns its whole toolchain — `aws-cdk-lib`, `constructs` and the CLI — and installs
     * outside the npm workspace, so the root prune cannot reach it and the deploys resolve `cdk` from there
     * (`npm exec --prefix …`). The root needs to declare nothing, and does not.
     */
    it('declares NO production dependency at the root, so none can reach an image', () => {
        const root = JSON.parse(readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8')) as Manifest;

        expect(
            Object.keys(root.dependencies ?? {}),
            'A root `dependencies` entry ships into every image built from this tree, because the service ' +
                'Dockerfiles COPY the root `node_modules`. A build- or deploy-only tool belongs in the ' +
                'package that runs it — for CDK, that is each `infra/` package, which installs on its own.',
        ).toEqual([]);
    });

    it('gives every CDK package its own CLI, since no root entry survives the prune for it', () => {
        const packages = execFileSync(
            'git',
            ['ls-files', '*/infra/package.json', 'packages/infra/global/package.json'],
            {
                cwd: REPO_ROOT,
                encoding: 'utf8',
            },
            ...
            .split('\n')
            .filter(Boolean)
            // ⚠️ `shared/infra` is the published CONSTRUCTS library. It defines stacks; it never synthesizes
            // or deploys one, so it needs no CLI — and shipping a build tool to consumers would be wrong.
            .filter((file) => !file.startsWith('shared/'));

        // Non-vacuity: the discovery must actually find the CDK packages.
        expect(packages.length).toBeGreaterThan(5);

        const missing = packages.filter((file) => {
            const manifest = JSON.parse(readFileSync(path.join(REPO_ROOT, file), 'utf8')) as Manifest;

            return (manifest.devDependencies ?? {})['aws-cdk'] === undefined;
        });

        expect(
            missing,
            'This CDK package cannot run `cdk` after the root prune — the CLI is no longer a root ' +
                'dependency, deliberately, so each package that synthesizes or deploys must declare it.',
        ).toEqual([]);
    });
