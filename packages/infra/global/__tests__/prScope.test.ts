/**
 * Repo-wide guard: the per-PR teardown scope predicates (`.github/scripts/prScope.sh`).
 *
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/prScope.sh', import.meta.url));
...
describe('prScope.sh — the file exists where teardown sources it from', () => {
    it('is present at .github/scripts/prScope.sh', () => {
...
describe('pr_scope_environment_tag_belongs — ownership of an AWS `Environment` TAG value', () => {
    ...
    // ⚠️ THE LEGACY SPELLING IS PART OF THE RULE, not an accident. Resources deployed under the bare
    // `Environment=pr-{N}` value — including log groups and ECR repos the name sweep does not reach — stay
    // live until reaped. A predicate that accepted only `pr-{N}-sandbox` would make every one of them
    // unreapable: failure mode (a) of ADR-0005.
