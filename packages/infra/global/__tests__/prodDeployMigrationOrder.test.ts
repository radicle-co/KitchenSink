...
import { EPHEMERAL_SLOT_ORDER, cutOverServicesFromEnv } from '@radicle-co/infra-shared/alb';
 ...
 * exits 0 when the FUNCTION threw — and now goes through `.github/scripts/runMigrations.sh`, which is the
 ...
 */
const MIGRATION_INVOCATION = /aws lambda invoke|runMigrations\.sh/;
...
const DEPLOY_WORKFLOWS = [
    ...
    // ⚠️ `deployInfra.yml`: every deploy moved there, one job per `infra/` folder.
...
function databaseAddressingSources(serviceDir: string): readonly DatabaseAddressingSource[] {
    return infraSources(serviceDir).flatMap((file) => {
        ...
        const authorities = [
            ...
            // ⚠️ THE STRONGEST AUTHORITY, AND THE ONE THAT ALMOST WENT UNSEEN. Two stacks that read the SAME
            // published SSM parameter address the same database by construction — better evidence than two
            // stacks importing the same naming function, which still lets each pass its own arguments (#119).
            // But this matcher knew only the older spellings, so when `RecipeWorkersStack` switched from
            // deriving the name to reading the parameter, it stopped looking like a peer at all and the
            // recipe pair silently vanished from the ordering guard.
            ...[...source.matchAll(/(\w*DatabaseNameParameter)\s*\(/g)].map((match) => `ssm:${match[1] as string}`),
        ];
...
describe('every stack that reads a service database deploys AFTER that schema is applied', () => {
    ...
    it('discovers a shared-database pair at all, in every workflow that deploys one', () => {
        ...
        expect(
            ...
            // ⚠️ `sandboxPreview.yml` sorts first: the per-PR deploy jobs moved to a REUSABLE workflow so
            // `_ci.yml` can run them as one branch — GitHub Actions has no cross-workflow `needs`.
            // ⚠️ NO `deployInfra.yml` entry. This finder pairs an owner and a peer INSIDE ONE JOB, and
            // there each `infra/` folder has a job of its own — the ordering it looks for is carried by
            // `needs:` between jobs instead, and by the schema jobs that now precede every reader
            // (ADR-0035). `deployedE2eEntrypoint.test.ts` asserts that graph directly.
