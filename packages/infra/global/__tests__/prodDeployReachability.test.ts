...
function computeFlags(expressions: Readonly<Record<string, string>>): Record<string, string> {
    ...
    const directory = scratchDirectory('prod-deploy-flags-');
    ...
    // `.github/scripts/deployGate.sh` for the deploy-graph closure, so running from vitest's own cwd would
 ...
 *
 * These scenarios drive the step's REAL bash against the REAL `deployGate.sh`, so deleting the closure call
