...
describe('every env-gated suite in this package is switched on by a workflow', () => {
    it('finds the suite-level gate variable in the test sources', () => {
        ...
        expect(findEnvGates(TEST_DIRS)).toEqual([
            ...
            // The role split's privilege, applier and engine suites: they create and drop roles and databases
            // on the server they are pointed at — the same throwaway container, the same step.
            // The role-split bootstrap pass, including the armed legacy recreate — the same container, the same step.
            { file: 'tests/dbBootstrap.integration.test.ts', variable: 'DATABASE_URL' },
            { file: 'tests/dbRoleModel.integration.test.ts', variable: 'DATABASE_URL' },
