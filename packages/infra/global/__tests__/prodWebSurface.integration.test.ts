 ...
 * `prodWebSurface.test.ts` for the incident narrative and the hermetic proofs of every classifier used
 ...
 *   run: npx vitest run __tests__/prodWebSurface.integration.test.ts
 * ```
 *
 * That pairing is itself asserted, by `prodDeploySmokeDepth.test.ts`: it discovers this suite's gate
 ...
 * `workflowInvariants.test.ts` inventories, which is why the pairing is guarded rather than trusted. It runs
