 ...
 * tokens by enumerating live resources; stage two hands each token to `teardownSandboxPr.sh`, which
...
/**
 * A resource class `teardownSandboxPr.sh` deletes, paired with the discovery signal the reaper needs to
...
const RECLAIMED_CLASSES: readonly ReclaimedClass[] = [
    {
        ...
        deletedBy: '§2 (name match or a per-PR Environment tag)',
...
describe('GH_ENVIRONMENT_ADMIN_TOKEN reaches both reclamation jobs', () => {
    ...
    it('spells the variable exactly as the teardown script reads it', () => {
        ...
        expect(readRepoFile('.github', 'scripts', 'teardownSandboxPr.sh')).toContain('${GH_ENVIRONMENT_ADMIN_TOKEN:-}');
    });
