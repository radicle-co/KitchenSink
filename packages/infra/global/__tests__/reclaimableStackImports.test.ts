 ...
 * ## What is asserted
 *
 * For every stack in {@link RECLAIMABLE_EXPORT_PREFIXES} — the stacks `sandboxSharedTier.sh` deletes — no
 ...
 *
 * Kept in step with `.github/scripts/sandboxSharedTier.sh`'s allowlist by the last assertion below, so
...
describe('no persistent stack imports from a reclaimable one (ADR-0028)', () => {
    ...
    it('covers exactly the stacks the teardown script deletes, so the two cannot drift', () => {
        const allowlist = readFileSync(path.join(repoRoot, '.github/scripts/sandboxSharedTier.sh'), 'utf8');
