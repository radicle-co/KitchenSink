...
describe('reclamation jobs are never gated behind a GitHub Environment', () => {
    ...
    it('the deploying jobs ARE bound, so this guard is not just asserting the absence of all bindings', () => {
        // ⚠️ `deploy-infra.yml`: every `cdk deploy` moved there, one job per `infra/` folder, when
        // `_sandbox-preview.yml`'s two deploy jobs were retired to leave a single writer per stack.
        const jobs = readWorkflow('deploy-infra.yml')['jobs'] as Record<string, Record<string, unknown>>;
        ...
        //
        // ⚠️ An EXPRESSION, not the constant `Sandbox`, because one workflow now serves every stage AND both
        // modes: a DEPLOY binds `Production` (required reviewers and a branch policy — the correct answer for a
        // prod deploy) or `Sandbox` (no protection rules); a `deploy: false` CHECKS run binds nothing at all.
        // That last clause is what lets `main`'s checks synthesize at `prod` — the stage `main` actually ships —
        // without waiting on Production's reviewers (owner ruling 2026-09-11, "option A"). Every branch of the
        // expression is a non-empty literal, so it is not an unsound pseudo-ternary.
        const bound = Object.entries(jobs).filter(([, job]) => job['environment'] !== undefined);

        expect(bound.length, 'every deploying job must be bound to a GitHub Environment').toBeGreaterThanOrEqual(9);

        for (const [name, job] of bound) {
            expect(job['environment'], `${name} is bound to an unexpected environment`).toBe(
                "${{ inputs.deploy && inputs.stage == 'prod' && 'Production' || inputs.deploy && 'Sandbox' || '' }}",
            );
        }
    });
});
