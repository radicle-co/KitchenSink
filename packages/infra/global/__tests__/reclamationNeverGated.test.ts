...
describe('reclamation jobs are never gated behind a GitHub Environment', () => {
    ...
    it('the deploying jobs ARE bound, so this guard is not just asserting the absence of all bindings', () => {
        // ⚠️ `deployInfra.yml`: every `cdk deploy` moved there, one job per `infra/` folder, when
        // `sandboxPreview.yml`'s two deploy jobs were retired to leave a single writer per stack.
        const jobs = readWorkflow('deployInfra.yml')['jobs'] as Record<string, Record<string, unknown>>;
