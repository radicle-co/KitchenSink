...
describe(`the guard is IN FORCE inside ${PROBE_PACKAGE}/src`, () => {
    ...
    beforeAll(async () => {
        ...
        // package's real one; only the type-aware program, which no rule under test needs, is skipped — and with
        // it every rule that REQUIRES that program, which would otherwise fail to load on a virtual path.
        // typescript-eslint's own preset names them, so a type-aware rule added to the shared config later is
        // switched off here too rather than breaking this guard.
