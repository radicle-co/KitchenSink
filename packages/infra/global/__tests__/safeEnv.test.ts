...
/**
 * `.github/scripts/safeEnv.sh` — the ONE way a step in this repository writes to `$GITHUB_ENV` while carrying a
 ...
 * all — no newline.
 *
 * Executed against the REAL script, the way `prScope.test.ts` runs `prScope.sh`.
 */
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/safeEnv.sh', import.meta.url));
...
describe('safeEnv.sh writes only a validated NAME=VALUE line', () => {
    it.each([
        ['ALARMS_ENABLED', 'true'],
...
describe('a step that suppresses `github-env` writes $GITHUB_ENV ONLY through safeEnv.sh', () => {
    ...
    it.each(
        ...
        expect(block).toMatch(/safeEnv\.sh/u);
    });
});
