 ...
 *
 * 1. **The script refuses.** `teardownSandboxPr.sh` runs `pr_scope_is_token` on its argument and exits 2
 ...
 *    the call.
 *
 *    ⚠️ THIS LAYER IS TEXTUAL, AND THAT IS ITS LIMIT. It reads the YAML, so it catches the copy-paste it
 *    was written for and nothing else: a caller that builds its target by concatenation, reads it from a
 *    file, or routes it through `env:` indirection passes, because the literal never appears.
 *    The behavioural form of the question — what value actually ARRIVES at the deletion script — is
 *    answerable only by execution and lives in `tests/teardownTargetIsAToken.integration.test.ts`,
 *    which runs each caller's real `run:` body against a recording stub. This layer is kept because it is
 *    cheap and fails fast on the obvious mistake; it is not the only thing standing behind claim 3.
 *
 ...
 * | mutation | result |
 * |---|---|
 * | `pr_scope_is_token` neutered in `teardownSandboxPr.sh` | 10 failures here |
 * | the digits `case` removed from `sandboxDown.yml` | 9 failures in `tests/sandboxDownReclaims.integration.test.ts` |
 ...
 * file can only read the workflow's shape.
 *
 ...
 * nothing at all rather than failing. Measured while writing this file: `sandboxDown.yml` passed every
...
const TEARDOWN = path.join(repoRoot, '.github/scripts/teardownSandboxPr.sh');
...
describe('layer 2 — no caller can express a target that is not a PR number', () => {
    const callers = (): readonly { readonly workflow: string; readonly name: string; readonly body: string }[] =>
        workflowJobs().filter((job) => job.body.includes('teardownSandboxPr.sh'));
    ...
    it.each(callers().map((job) => [`${job.workflow}:${job.name}`, job] as const))(
        ...
        (_label, job) => {
            ...
            // `/teardownSandboxPr\.sh\s+(\S+)/` spans the newline after the `chmod` line and reports
            ...
            const calls = job.body
                .split('\n')
                .filter((line) => line.includes('teardownSandboxPr.sh') && !/\bchmod\b/u.test(line))
                .flatMap((line) => [...line.matchAll(/teardownSandboxPr\.sh\s+(\S+)/gu)].map((m) => m[1] ?? ''));
...
describe('layer 3 — no tier word appears anywhere on a teardown path', () => {
    ...
    const teardownWorkflows = (): readonly { readonly name: string; readonly text: string }[] =>
        trackedFiles(WORKFLOWS_DIR)
            ...
            .filter((workflow) => workflow.text.includes('teardownSandboxPr.sh'));
        ...
        (name) => {
            ...
            const targets = new Set(
                text
                    ...
                    .flatMap((line) =>
                        [
                            ...line.matchAll(/teardownSandboxPr\.sh\s+"?(?:pr-)?\$\{?([A-Za-z_][A-Za-z0-9_]*)\}?"?/gu),
                        ].map((match) => match[1] ?? ''),
                    ),
            );
