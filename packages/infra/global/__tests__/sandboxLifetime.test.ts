 ...
 * that arithmetic and nothing else repeats it; `sandbox-reap.yml` feeds `expires-at` a stack's own
 ...
 *
 * ⚠️ The instant used to come from a `SandboxExpiresAt` tag stamped at the press of a button. That tag is
 * deleted — `UpdateStack` replaces a stack's tag set, so every deploy could erase it — and CloudFormation's
 * `LastUpdatedTime` replaced it. The functions under test did not change; only what they are fed.
 *
