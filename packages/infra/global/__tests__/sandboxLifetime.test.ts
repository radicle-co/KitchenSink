/**
 * Repo-wide guard: the on-demand sandbox's expiry clock (`.github/scripts/sandboxLifetime.sh`).
 *
 * ## The invariant this protects
 *
 * A preview is torn down at **midnight America/New_York of the day it was LAST DEPLOYED**. This script owns
 * that arithmetic and nothing else repeats it; `sandboxReap.yml` feeds `expires-at` a stack's own
 * `LastUpdatedTime` and asks `is-expired`, so the reaper never does timezone arithmetic of its own and
 * cannot disagree with this file.
 *
 * ⚠️ The instant is CloudFormation's `LastUpdatedTime`, never a stamped tag: `UpdateStack` replaces a
 * stack's tag set, so every deploy could erase one.
 *
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/sandboxLifetime.sh', import.meta.url));
