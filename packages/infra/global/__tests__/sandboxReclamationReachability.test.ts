 ...
 * `.github/scripts/teardownSandboxPr.sh`, which is what keeps them from drifting. On 2026-07-28,
 ...
 * *all* of it — and `teardownSandboxPr.sh` was already written to handle a missing zone correctly
 ...
 *
 * For every job that invokes `teardownSandboxPr.sh`, each EARLIER step's `run:` body must contain no
 ...
 *
 * `.github/scripts/cfnExport.sh` exists because `list-exports … --query "Exports[?Name=='…'].Value | [0]"`
 ...
 *   4. `cfnExport.sh` in the cleanup job's zone step replaced with the bare
...
const TEARDOWN_SCRIPT = 'teardownSandboxPr.sh';

/** The shared, paginated export resolver every lookup must go through. */
const EXPORT_HELPER = 'cfnExport.sh';
 ...
 *
 * There is no exemption, deliberately. The correct form delegates to `cfnExport.sh`, and doing so puts no
 ...
 * through it, because these steps carry a COMMENT pointing at `.github/scripts/cfnExport.sh` to explain
...
describe('per-PR reclamation is reachable', () => {
    ...
    it('has a job that invokes the teardown script at all', () => {
        ...
        // Every reclamation path must exist; without this the two analyzers below could pass vacuously.
        // The set is PINNED rather than merely non-empty because the set is the point — this list is what
        // stops another copy of "what belongs to pr-{N}" being written somewhere nobody is guarding.
        // `sandboxDown.yml#down` is the dispatchable mirror of `Sandbox Up` and the door the scheduled
        // reaper presses; note what it is NOT: it carries no idea of its own about what belongs to a PR,
        // it builds `pr-{N}` from a validated PR number and hands it to the same script. That is the
        // property this pinned set exists to keep true.
        //
        // `sandboxReconcile.yml#reconcile` is ABSENT from this set on purpose. It reaps only the shared
        // tier; reaping per-PR previews there too would make two implementations of "which sandbox is
        // finished" — and the whole reason this set is pinned is that a second such implementation is the
        // thing nobody is guarding.
        //
        // ⚠️ NOT "reaping has one owner" (ADR-0028 §3). Three
        // jobs invoke the teardown and each answers a DIFFERENT question (this PR just closed · is it
        // abandoned · someone asked). What is single is the IMPLEMENTATION they all run, which is the
        // property this set exists to keep true. `sandboxReap.yml` is deliberately absent: it presses
        // `Sandbox Down` rather than running the teardown itself, so it adds no second copy of "what
        // belongs to pr-{N}".
        expect(invoking.sort()).toEqual([
            'sandbox-deploy.yml#cleanup',
            'sandbox-deploy.yml#reap-abandoned',
            'sandboxDown.yml#down',
        ]);
    });
