...
describe('per-PR reclamation is reachable', () => {
    ...
    it('has a job that invokes the teardown script at all', () => {
        ...
        // `sandbox-down.yml#down` is the dispatchable mirror of `Sandbox Up` and the door the scheduled
        ...
        //
        // `sandbox-reconcile.yml#reconcile` is GONE from this set on purpose. It used to reap per-PR
        // previews as well as the shared tier, which made two implementations of "which sandbox is
        // finished" — and the whole reason this set is pinned is that a second such implementation is the
        // thing nobody is guarding.
        //
        // ⚠️ NOT "reaping has one owner" — that phrasing was wrong and is corrected in ADR-0028 §3. Three
        ...
        // property this set exists to keep true. `sandbox-reap.yml` is deliberately absent: it presses
