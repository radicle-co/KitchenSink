 ...
 * (`.github/scripts/sandboxSharedTier.sh`), the on-demand half of ADR-0028.
 *
 * ## Why this guard is stricter than the others
 *
 * Every other teardown in this repository is safe because of `prScope.sh`: it only ever matches `pr-{N}`,
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/sandboxSharedTier.sh', import.meta.url));
...
describe('sandboxSharedTier.sh (ADR-0028 on-demand shared tier)', () => {
