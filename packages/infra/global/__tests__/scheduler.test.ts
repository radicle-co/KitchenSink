...
describe('resource selectors (sandbox-only)', () => {
    ...
    describe('isScheduledCluster — which clusters the nightly window may touch', () => {
        ...
        //
        // ⛔ SELECTED BY TAG, NEVER BY NAME. `ecsQuiesce.sh` states the rule for per-PR ECS discovery:
        ...
        // `teardownSandboxPr.sh` to delete whole stacks, so this widens nothing.
        ...
        it('⚠️ refuses an Environment tag that merely starts with `pr`, or merely ends in `-sandbox`', () => {
            ...
            // most the tier suffix after it — the delimiter-aware discipline `prScope.sh` already enforces
            // for teardown.
            //
