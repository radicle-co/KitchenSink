 ...
 *
 * ADR-0035 makes the migrate INVOKE unconditional, for the reason `runMigrations.sh` records at length: a
...
function workflows(): readonly { readonly file: string; readonly jobs: Record<string, { steps?: Step[] }> }[] {
    // ⚠️ EVERY workflow, not a `*deploy*` glob. The first draft used one and missed `sandboxPreview.yml`
 ...
 * it is an input, supplied once per job in `deployInfra.yml`. These assertions put the literal back under
...
describe('every database-backed package declares its schema stack at the call site', () => {
    const callSites = (): Record<string, Record<string, string>> => {
        const doc = yaml.parse(readFileSync(path.join(REPO_ROOT, '.github/workflows/deployInfra.yml'), 'utf8')) as {
