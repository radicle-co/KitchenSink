 ...
 *
 *  1. `IdentityServiceStack.ts` injected `DELETION_QUEUE_URL` into the task and granted the task role
 ...
 *  3. `WebhooksStack.ts` alarmed on a DIMENSIONLESS `ErasureIncomplete`, while `emitMetric` publishes every
 ...
 * hypothetical in this repo: two gates written for `serviceSecurityInvariants.test.ts` passed against
 * deliberately-broken code because the comment ABOVE the code contained the words the gate searched for. The
 * risk is acute here — `IdentityServiceStack.ts` carries a long comment about the deletion queue, and
 * `RecipeWorkersStack.ts` names `esbuild.mjs` and its handler strings in prose. Every gate below therefore
 ...
 *
 * ⛔ THE UNION IS DECOMPOSED, and it has to be. This used to require EVERY injected queue to hold EVERY grant
 * the package's code needs anywhere — so the moment a service both produced to one queue and merely READ
 * another (plan U12's backstop injects four queue URLs it holds only `GetQueueAttributes` on), the read-only
 * injections were reported as missing `send`. A guard that fires on a correct change is a guard that gets
 * edited until it is quiet, which is how a real one is lost. The two halves of the original claim are asserted
 * separately instead, and together they still catch the recorded defect:
 *
 *  - every injected queue is granted SOMETHING — "the URL was handed over and nobody granted anything" is
 *    exactly the defect this was written for, and it is unchanged;
 *  - every operation the code performs is held by at least one injected queue — so a service that constructs
 *    `SendMessageCommand` and grants send to nothing still fails.
 *
 * ⚠️ RESIDUAL, stated rather than papered over, and now one step wider: this cannot say WHICH injected queue a
 * given command targets, nor that the SPECIFIC role attached to that execution unit holds the grant. Pinning
 * the grantee needs the env-object-to-role association, which survives neither the `commonEnv` spread
 * (identity-webhooks injects one env into seven Lambdas with seven roles) nor an env assembled before its task
 * definition (recipe-service); pinning the queue needs the env-name-to-command association, which crosses
 * modules (recipe-workers names its queue URLs in `queueCheckReads.ts` and issues the command in
 * `queueCheck.ts`). Both precisions belong in each service's own synth test, where the role and the queue are
 * concrete template resources — `identity/infra/__tests__` asserts `sqs:SendMessage` on the identity task
 * role's policy, and `queueProducerRegister.test.ts` holds the whole authority set to a register by exact
 * equality in both directions, for exactly that reason.
 *
 * @param service - The service to check.
 * @returns One finding per ungranted injection, plus one per operation no injected queue can perform.
 */
function queueGrantViolations(service: DiscoveredService): readonly string[] {
    ...
    const grantedAcrossInjections = new Set<QueueGrant>();
    let injections = 0;
    ...
    for (const source of infraSources(service)) {
        ...
        for (const injection of queueUrlInjections(source)) {
            ...
            for (const grant of granted) {
                grantedAcrossInjections.add(grant);
            }

            // Half one: the URL was handed over and NOTHING was granted on it. The original defect.
            if (granted.size === 0) {
                findings.push(
                    `${injection.file}: injects ${injection.envName} but the queue is granted nothing — ` +
                        `the code issues commands requiring [${[...required].sort().join(', ')}]`,
                );
            }
        }
    }

    // Half two: an operation the code performs that NO injected queue can perform. A service that constructs
    // `SendMessageCommand` and grants send to nothing fails here even though each queue is granted something.
    if (injections > 0) {
        const unreachable = [...required].filter((grant) => !grantedAcrossInjections.has(grant));

        if (unreachable.length > 0) {
            findings.push(
                `${service.name}: the code issues commands requiring [${unreachable.sort().join(', ')}] ` +
                    `but no injected queue is granted it (granted across injections: ` +
                    `[${[...grantedAcrossInjections].sort().join(', ') || 'nothing'}])`,
            );
        }
    }
 ...
 * the prose in `RecipeWorkersStack.ts` that quotes these exact strings ("Matches esbuild's outbase:src
 * layout — see esbuild.mjs entryPoints") is invisible.
 *
 * @param source - The infra file.
 * @returns The handler strings, e.g. `handlers/handleSyncWorker.handler`.
 */
 ...
 * @returns The entry-point paths, e.g. `src/handlers/handleSyncWorker.ts`.
 */
 ...
 *
 * `handlers/handleSyncWorker.handler` → `src/handlers/handleSyncWorker.ts`. The exported symbol is
 ...
 *
 * ⚠️ THIS GATE REPLACES TWO THAT FAILED. `RecipeWorkersStack.test.ts` asserted the handler strings with
 * `expect.arrayContaining([…five names…])` — which proves the five are PRESENT and says nothing about a sixth —
 * and `buildInputs.test.ts` iterated the same five names again. Both were green while `handle-sync-worker`
 ...
 * having one. `IdentityServiceStack.ts` records having already been through this once — "A4: alarms previously
 * had no action wired, so they fired silently" — and `WebhooksStack.ts` still had one.
 *
 * @param service - The service to check.
...
/**
 * W6 — the process the stack actually starts loads the service's Sentry instrumentation FIRST.
 *
 * ⛔ THE DEFECT THIS WAS WRITTEN FOR, found in the tree. `recipe-service/Dockerfile` ends
 * `CMD ["node", "--import", "./dist/src/instrument.js", "dist/src/main.js"]`, above a comment explaining that
 * `@sentry/nestjs` instruments modules by patching them AS THEY LOAD, so anything imported before
 * `instrument.js` is never instrumented. `RecipeServiceStack.ts` then set
 * `command: ['node', 'dist/src/main.js']` — and an ECS container `command` OVERRIDES the image's `CMD`. So
 * `Sentry.init` had never run in the deployed recipe API: no issues, no traces, and U17's
 * `Sentry.captureException` in its `ApiExceptionFilter` was a no-op in prod and in every sandbox. Food sets
 * the flag on all three of its tasks; identity sets no `command` at all and correctly inherits its `CMD`.
 *
 * ⛔ AND IT IS SILENT IN BOTH DIRECTIONS. Nothing fails: the template synthesizes, the task starts, the health
 * check passes, the service serves traffic. The only symptom is an absence of telemetry, which is
 * indistinguishable from a quiet service — the same shape as ADR-0042's dead log drain.
 *
 * Both halves are checked because either one alone leaves the other free to drift: a stack that sets no
 * `command` inherits whatever the `CMD` says, so the `CMD` is then the launch, and a stack that sets one makes
 * the `CMD` irrelevant. Whichever line actually starts Node is the line that must carry `--import`.
 *
 * @param service - The service to check.
 * @returns One finding per launch that starts an entry point without preloading instrumentation.
 */
function instrumentPreloadViolations(service: DiscoveredService): readonly string[] {
    const findings = [...containerCommands(service)]
        .filter(([, command]) => launchesEntryPoint(command) && !preloadsInstrumentation(command))
        .map(
            ([where, command]) =>
                `${where}: container command ${JSON.stringify(command)} starts the service without ` +
                `--import of its instrument bundle — an ECS command overrides the image CMD, so Sentry never ` +
                `initialises`,
        );

    const dockerfile = readServiceFile(service.name, 'Dockerfile');
    const cmd = dockerfile === undefined ? undefined : dockerfileCmd(dockerfile);

    if (cmd !== undefined && launchesEntryPoint(cmd) && !preloadsInstrumentation(cmd)) {
        findings.push(
            `packages/services/${service.name}/Dockerfile: CMD ${JSON.stringify(cmd)} starts the service ` +
                `without --import of its instrument bundle`,
        );
    }
...
/**
 * Every `addContainer` call's `command` array, as string elements.
 *
 * ⚠️ AST, not text. `RecipeServiceStack.ts` and both Dockerfiles carry long comments that quote the correct
 * `--import ./dist/src/instrument.js` incantation verbatim — a grep for that string passes against the very
 * file the defect lives in, which is the trap this suite's header records having been sprung twice already.
 *
 * @param service - The service whose infra sources to read.
 * @returns `file + container id` → the command's string elements.
 */
function containerCommands(service: DiscoveredService): ReadonlyMap<string, readonly string[]> {
    ...
    for (const source of infraSources(service)) {
        visit(parse(source), (node) => {
            if (
                !ts.isCallExpression(node) ||
                !ts.isPropertyAccessExpression(node.expression) ||
                node.expression.name.text !== 'addContainer'
            ) {
                return;
            }
            ...
            const command = objectProperties(props).get('command');

            if (command === undefined || !ts.isArrayLiteralExpression(command)) {
                return;
            }

            const id = literalText(node.arguments[0]) ?? 'unnamed';
            const parts = command.elements
                .map((element) => literalText(element))
                .filter((element): element is string => element !== undefined);

            commands.set(`${source.file} (${id})`, parts);
        });
    }
...
/**
 * The `CMD` of a Dockerfile, in its exec (JSON array) form.
 *
 * ⚠️ Only the exec form is read, and that is deliberate rather than a gap: the shell form
 * (`CMD node dist/src/main.js`) runs through `/bin/sh -c`, which does not forward signals, so ECS task
 * shutdown would `SIGKILL` the service after the stop timeout. Every Dockerfile here uses the exec form, and
 * one that did not would be a different and larger defect than a missing `--import`.
 *
 * @param dockerfile - The Dockerfile's contents.
 * @returns The last `CMD`'s arguments, or `undefined` when it declares none in exec form.
 */
function dockerfileCmd(dockerfile: string): readonly string[] | undefined {
    const matches = [...dockerfile.matchAll(/^CMD\s+(\[[^\]]*\])\s*$/gm)];
...
/** Whether a launch starts a compiled service entry point rather than a shell or a health probe. */
function launchesEntryPoint(command: readonly string[]): boolean {
    return command[0] === 'node' && command.some((part) => part.endsWith('main.js'));
}

/** Whether a launch preloads a Sentry instrumentation bundle before the entry point. */
function preloadsInstrumentation(command: readonly string[]): boolean {
    const flag = command.indexOf('--import');

    return flag !== -1 && (command[flag + 1]?.endsWith('instrument.js') ?? false);
}
 ...
 *
 * The prose is not decoration. Two gates in `serviceSecurityInvariants.test.ts` passed against deliberately
    ...
    sources: [
        {
            ...
            contents: `
                ...
                // text-matching gates in serviceSecurityInvariants.test.ts.)
                deletionQueue.grantConsumeMessages(taskRole);

                // Started with --import ./dist/src/instrument.js so Sentry patches modules as they load.
                // (Again a comment, not the code — W6 must read the array, not the prose above it.)
                ...
                // Deploys the sync worker. Bundled by esbuild.mjs as src/handlers/handleSyncWorker.ts.
        ...
        {
            file: 'packages/services/fake-service/src/handlers/handleSyncWorker.ts',
...
const VIOLATING_BUNDLER_CONFIG = `
    // Bundles every handler, including src/handlers/handleSyncWorker.ts.
    const entryPoints = [
...
/**
 * The fake's stack with its container command replaced, for W6's accept/reject cases.
 *
 * @param command - The array elements, as they appear in source.
 * @returns The fake's sources with that command substituted.
 */
function withFakeCommand(command: string): readonly SourceFile[] {
    return VIOLATING_SERVICE.sources.map((source) =>
        isInfraFile(source.file)
            ? { ...source, contents: source.contents.replace("'node', 'dist/src/main.js'", command) }
            : source,
    );
}
...
describe('every deployable service wires its infra to its code', () => {
    ...
    it.each([
        ...
        ['W6 starts every service with its instrumentation preloaded', instrumentPreloadViolations],
    ...
    describe('the gates have real subjects — not green because they found nothing', () => {
        ...
        it('W2 finds deployed handlers and bundled entry points', () => {
            ...
            expect(handlers).toContain('handlers/handleSyncWorker.handler');
            expect(
        ...
        it('W6 finds real container commands AND a real Dockerfile CMD to check', () => {
            const commands = services.flatMap((service) => [...containerCommands(service).values()]);
            const identityCmd = dockerfileCmd(readServiceFile('identity', 'Dockerfile') as string);

            // Food declares three; a gate that found none would be green having examined nothing.
            expect(commands.filter((command) => launchesEntryPoint(command)).length).toBeGreaterThanOrEqual(4);
            expect(identityCmd).toBeDefined();
            expect(launchesEntryPoint(identityCmd as readonly string[])).toBe(true);
        });
    ...
    describe('the gates actually fire — proven against a fake sixth service that violates all of them', () => {
        it('W1 catches a send-only producer holding consume-only permission', () => {
            // ⚠️ This is the SECOND half of W1 since the union was decomposed: the injection IS granted
            // something (consume), so the per-injection half is silent — and the service still fails, because
            // it constructs `SendMessageCommand` and no injected queue can send. Asserting the message rather
            // than only the count is what pins WHICH half fired, so a future edit cannot satisfy this test by
            // reporting the wrong thing.
            expect(queueGrantViolations(VIOLATING_SERVICE)).toEqual([
                'fake-service: the code issues commands requiring [send] but no injected queue is granted it ' +
                    '(granted across injections: [consume])',
            ]);
        });
        ...
        it('W1 does NOT accept the comment that names the missing grant', () => {
            ...
            // ⛔ TWO findings, and both are correct: the injection is granted NOTHING (half one), and no
            // injected queue can send (half two). The prose grants neither, which is the claim.
            expect(queueGrantViolations(proseOnly)).toEqual([
                'packages/services/fake-service/infra/lib/fake-service-stack.ts: injects DELETION_QUEUE_URL ' +
                    'but the queue is granted nothing — the code issues commands requiring [send]',
                'fake-service: the code issues commands requiring [send] but no injected queue is granted it ' +
                    '(granted across injections: [nothing])',
            ]);
        });

        it('W2 catches a deployed handler that the bundler config omits', () => {
            expect(fakeUnbundledHandlers()).toEqual(['handlers/handleSyncWorker.handler']);
        });
        ...
        it('W2 accepts the handler once the bundler config carries its entry point', () => {
            const entries = new Set(
                bundledEntryPoints({
                    file: 'esbuild.mjs',
                    contents: "const entryPoints = ['src/handlers/handleSyncWorker.ts'];",
                }),
            );

            expect(entries.has(entryPointForHandler('handlers/handleSyncWorker.handler') as string)).toBe(true);
        });
        ...
        it('W2 derives the entry point from the handler, whatever the layout', () => {
            ...
            expect(entryPointForHandler('handlers/erasureSweeper.handler')).toBe('src/handlers/erasureSweeper.ts');
        });
        ...
        it('W6 reports a container command that starts the service without the preload', () => {
            expect(instrumentPreloadViolations(VIOLATING_SERVICE)).toEqual([
                'packages/services/fake-service/infra/lib/fake-service-stack.ts (FakeContainer): container ' +
                    'command ["node","dist/src/main.js"] starts the service without --import of its instrument ' +
                    'bundle — an ECS command overrides the image CMD, so Sentry never initialises',
            ]);
        });

        it('⛔ W6 is not satisfied by the comment above the command that names the missing flag', () => {
            const stack = VIOLATING_SERVICE.sources.find((source) => isInfraFile(source.file));

            // The fixture's prose contains the exact incantation. A text gate passes on it; this one must not.
            expect(stack?.contents).toContain('--import ./dist/src/instrument.js');
            expect(instrumentPreloadViolations(VIOLATING_SERVICE)).toHaveLength(1);
        });

        it('W6 accepts a correct command and still rejects one missing only the bundle path', () => {
            const correct = {
                ...VIOLATING_SERVICE,
                sources: withFakeCommand("'node', '--import', './dist/src/instrument.js', 'dist/src/main.js'"),
            };
            const flagOnly = {
                ...VIOLATING_SERVICE,
                sources: withFakeCommand("'node', '--import', './dist/src/warmup.js', 'dist/src/main.js'"),
            };

            expect(instrumentPreloadViolations(correct)).toEqual([]);
            expect(instrumentPreloadViolations(flagOnly)).toHaveLength(1);
        });

        it('W6 ignores a container that starts something other than the service entry point', () => {
            const sidecar = { ...VIOLATING_SERVICE, sources: withFakeCommand("'node', 'dist/src/tools/seed.js'") };

            expect(instrumentPreloadViolations(sidecar)).toEqual([]);
        });
