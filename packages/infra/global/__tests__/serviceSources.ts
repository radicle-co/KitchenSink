 ...
 * can never contribute a finding — the trap documented at length in the code-quality toolchain's boundaries
 * ratchet, where a stray `cdk synth` invented three phantom violations. The `existsSync` filter is not redundant:
 * `ls-files` reports the INDEX, which disagrees with the working tree during an unstaged deletion or a half-finished
...
/**
 * Whether a file is a UNIT test: under a `__tests__/` directory or named `*.test.{ts,tsx,js,jsx,mts,mjs,cts,cjs}`.
 *
 * A DIFFERENT scope from {@link isTestFile} — unit tests only, of any JS/TS extension: it keeps `tests/`, `__fixtures__/`
 * and Playwright specs IN scope, because
 * the guards that use it police tooling that lives there (`tests/e2e/utils/testUser.ts` is exactly what
 * `runtimeUserCreation` must read). The two alternatives are two expressions rather than one `a|b$` pattern, whose
 * anchoring reads as if it bound the whole alternation.
 *
 * @param file - Repo-relative path.
 * @returns `true` for a unit test file.
 */
export function isUnitTestFile(file: string): boolean {
    return /(?:^|\/)__tests__\//u.test(file) || /\.test\.[cm]?[jt]sx?$/u.test(file);
}
...
export function objectProperties(node: ts.ObjectLiteralExpression): ReadonlyMap<string, ts.Expression> {
    ...
    for (const property of node.properties) {
        // ⛔ SHORTHAND COUNTS. `{ logger }` is a `ShorthandPropertyAssignment`, not a `PropertyAssignment`,
        // and skipping it made every gate built on this helper silently blind to a property written that
        // way — `serviceLoggingInvariants`' L1 reported identity's correctly-wired `NestFactory.create`
        // as unwired for exactly that reason. The value is the identifier itself, which is what the
        // expanded form `{ logger: logger }` would have produced anyway.
