...
describe('shared sandbox tier lifecycle (ADR-0028, amended 2026-08-30)', () => {
    describe('sandbox-reconcile.yml destroys the tier in mirror order', () => {
        ...
        it('does NOT tear down a per-PR preview itself', () => {
            // ⚠️ THIS ASSERTION IS INVERTED FROM WHAT IT USED TO SAY, and the inversion is the point.
            //
            // It used to require the tier delete to come AFTER this workflow's own per-PR teardown step —
            // an ordering rule, because the tier had to outlive the previews using it. That teardown step
            // is gone: reaping a preview now has exactly ONE owner (`sandbox-reap.yml`, which presses
            // `Sandbox Down`), and a second implementation of "which sandbox is finished" living here is
            // precisely the drift the pinned invoker set in `sandboxReclamationReachability.test.ts` exists
            // to prevent.
            //
            // The ORDERING CONCERN did not disappear with the step — it is discharged by the condition
            // asserted directly above: the tier is deleted only when `steps.find.outputs.live == ''`, i.e.
            ...
            expect(indexOfRun(steps, 'teardown-sandbox-pr.sh')).toBe(-1);
        });
