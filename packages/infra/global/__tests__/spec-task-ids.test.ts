 ...
 *
 * See the docblock of `./spec-declarations.ts`. The short version, both measured on this repository:
 *
...
import { findDuplicateTaskIds, findTaskIdDefinitions } from './spec-declarations.js';
...
describe('findTaskIdDefinitions — mutation proof', () => {
    ...
    it('PASSES an evidence HEADING for a task the file defines by checkbox (the 003 false positive)', () => {
        ...
        const fine = [
            ...
            '- [x] **T-198** [S] [Test-first: true] SC-007 short-query index bypass — `food-search.dao.ts`',
            '',
