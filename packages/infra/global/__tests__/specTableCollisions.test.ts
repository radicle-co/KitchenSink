 ...
 *
 * See the docblock of `./specDeclarations.ts`. Three measurements from this repository:
 *
...
import {
    ...
    findTableCollisions,
    validateExemptions,
} from './specDeclarations.js';
import type { TableDeclaration, TableExemption } from './specDeclarations.js';
...
describe('table declaration parsers — mutation proof', () => {
    ...
    it("does not count this guard's OWN docblock, which names every table it warns about", () => {
        // The self-referential check. `specDeclarations.ts` and this file both discuss `webhook_events`,
        ...
        const guardSources = ['specDeclarations.ts', 'specTableCollisions.test.ts', 'specTaskIds.test.ts'];
