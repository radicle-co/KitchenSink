 ...
 *     `deployGate.sh evaluate`, which are the only two things in this repository that ask whether a stack is
 *     there. A step is the unit, so a locally-defined `stack_exists()` wrapper is covered without this guard
 *     knowing its name.
 *   - the DECLARATIONS come from `deployGate.sh stacks-for`, EXECUTED rather than re-implemented. That is
...
const GATE = path.join(repoRoot, '.github/scripts/deployGate.sh');

/**
 * The two things in this repository that ask CloudFormation whether a stack is there.
 *
 * `deployGate.sh evaluate` calls `describe-stacks` itself, one file down; naming both spellings is what
 * makes the STEP the unit of discovery rather than the call.
 */
const PROBE_MARKER = /describe-stacks|deployGate\.sh\s+evaluate/u;
...
function declaredFor(entrypoint: string, stage: string): readonly string[] {
    ...
    if (result.status !== 0) {
        throw new Error(`deployGate.sh stacks-for ${entrypoint} ${stage} refused: ${result.stderr.trim()}`);
    }
...
describe('stack-existence probes name every stack the app declares', () => {
    it('discovers real probe steps across the real workflows', () => {
        ...
        expect(new Set(found.map((probe) => probe.workflow))).toContain('sandboxPreview.yml');
                ...
                return missing.length === 0
                    ...
                    : [
                          `${probe.workflow}:${probe.job} — step "${probe.step}" probes ${entrypoint} at stage ` +
                              ...
                              'created it is skipped. Derive the set with `deployGate.sh stacks-for` instead ' +
                              'of listing it.',
                      ];
            });
        });
