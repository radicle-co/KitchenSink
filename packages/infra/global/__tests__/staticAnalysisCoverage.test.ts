...
/** TypeScript sources owned by a standalone package — checked by `deployInfra.yml`, not by a turbo task. */
function standaloneSources(): readonly string[] {
    return allSources.filter((file) => standaloneDirs.some((dir) => file.startsWith(`${dir}/`)));
}

/**
 * The infra directories `deployInfra.yml` covers — read from its JOB LIST, not restated here.
 *
...
function infraCheckSubjects(): readonly string[] {
    const workflow = readFileSync(path.join(repoRoot, '.github/workflows/deployInfra.yml'), 'utf8');
...
describe('static-analysis coverage', () => {
    ...
    it('gives every standalone package its OWN job, since turbo cannot see them', () => {
        ...
        expect(
            unreached,
            'This package is outside the workspace AND has no job in `deployInfra.yml`, so nothing ' +
