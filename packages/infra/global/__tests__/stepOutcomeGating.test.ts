 ...
 *
 * `sandboxReconcile.yml` asks whether any per-PR preview is live, and — when none is — DELETES the shared
 * sandbox ALB and the identity service every preview signs in against. A fail-closed guard on its discovery
 * step — `list-stacks` failing runs `echo '::error::refusing to conclude the shared tier is idle'; exit 1` —
 * exits BEFORE the `$GITHUB_OUTPUT` write, so `live` is unset, so `''`, so two downstream steps carrying
 * `!cancelled()` run anyway, find nothing busy (the normal state), and reclaim the shared tier. One
 * CloudFormation hiccup takes down the tier every open preview depends on, with the run's own logs stating
 * it refused to do so.
 *
