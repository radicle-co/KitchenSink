 ...
 * ## Why this exists
 *
 * `teardownSandboxPr.sh` documents its inputs in its own header — `PREVIEW_ZONE`,
 ...
 *
 * 1. `PREVIEW_HOSTED_ZONE_ID` was never passed, so `sandboxDown.yml` could not delete
 ...
 *
 * ⚠️ A value may be SUPPLIED as job/step `env:` or COMPUTED inside the step body. `sandboxDown.yml`
 * resolves the hosted zone in-step deliberately (a preceding step that can abort would cancel the
 * reclamation), so demanding an `env:` key specifically would push a correct caller into the wrong shape.
 */
