    ...
    return trackedFiles(WORKFLOWS_DIR)
        ...
        .flatMap((file) => {
            ...
            return Object.entries(doc?.jobs ?? {}).flatMap(([name, job]) => {
                ...
                const runs = steps.map((step) => scalarText(step.run));
                const deployedWeb =
...
describe('every job that writes as the fixed test pool holds its lease and resets its data', () => {
    ...
    it.each(resetting.map((subject) => [subject.label, subject] as const))(
        ...
        (_, subject) => {
            ...
            const writes = steps.flatMap((step, index) => (WRITES.test(scalarText(step.run)) ? [index] : []));
            const resets = steps.flatMap((step, index) => {
                const match = RESET.exec(scalarText(step.run));
    ...
    it.each(resetting.map((subject) => [subject.label, subject] as const))(
        ...
        (_, subject) => {
            ...
            const resets = subject.steps.filter((step) => RESET.test(scalarText(step.run)));
