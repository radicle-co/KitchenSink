...
function synthesizePlatform(): { templates: Record<string, Template>; warnings: string[] } {
    ...
    new GlobalStack(app, 'Global-prod', {
        ...
        // Alarms ON: this suite audits the platform's TRANSPORT surface, and the alarm topics it reaches
        // through (`enforceSSL`) only exist when the alarms that publish to them do.
        alarmsEnabled: true,
    });
...
const deniesInsecureTransport = (document: unknown): boolean => {
    ...
    return statements.some(
        (statement) =>
            ...
            scalarText(statement.Condition.Bool['aws:SecureTransport']) === 'false',
    );
};
