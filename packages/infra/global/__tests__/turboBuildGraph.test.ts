...
const WORKSPACE_AREAS: readonly string[] = [
    ...
    'packages/clients',
    'packages/tools',
    // ⚠️ `packages/infra` is NOT here. Its packages left the npm workspace so that `aws-cdk-lib` stops being
    // hoisted into the root tree and shipped inside every service image — which means turbo cannot resolve a
    // build task for them and listing the area would report every one as "declares a build, has no graph
    // entry". They are built by `cdk-checks` in `_ci.yml`, which `staticAnalysisCoverage.test.ts` asserts
    // reaches each of them; this file's subject is turbo's graph, and they are deliberately not in it.
