 ...
 * |---|-----------|---------------------|
 * | 1 | the target is the tiers workflow's REQUIRED `web_origin` input | a host literal is typed in, or the input is made optional so a caller that omits it reads empty |
 * | 2 | `_ci.yml`'s call of the tiers workflow is gated on the preview deploy | the gate is dropped and every shard drives a stage that is not deployed — 8 shards of red that mean nothing |
...
/**
 * Where `e2e-web` is DEFINED. It moved out of `_ci.yml` so the manual door runs the same shards; both callers
 * pass the origin their own resolver produced, which `deployedE2eTiers.test.ts` §2 asserts per caller.
 */
const TIERS_WORKFLOW = fileURLToPath(new URL('../../../../.github/workflows/deployedE2eTiers.yml', import.meta.url));

/** The input that carries the browser's target into the tiers workflow. */
const WEB_ORIGIN_INPUT = 'web_origin';

/** The expression that must supply the browser's target — the caller's resolved origin, never a literal. */
const WEB_ORIGIN_EXPRESSION = `\${{ inputs.${WEB_ORIGIN_INPUT} }}`;
...
interface Document {
    readonly on?: {
        readonly workflow_call?: { readonly inputs?: Readonly<Record<string, { readonly required?: boolean }>> };
    };
...
/** The tiers workflow, parsed. */
function tiersWorkflow(): Document {
    return docOf(readFileSync(TIERS_WORKFLOW, 'utf8'));
}

/**
 * A job by name — `e2e-web` from the tiers workflow, everything else from `_ci.yml` — or a failure that names it
 * rather than a cryptic undefined dereference.
 */
function job(name: string): Job {
    const [file, doc] = name === 'e2e-web' ? ['deployedE2eTiers.yml', tiersWorkflow()] : ['_ci.yml', ciWorkflow()];
    const found = doc.jobs?.[name];

    if (found === undefined) {
        throw new Error(`${file} has no job \`${name}\` — this guard's subject was renamed or removed.`);
    }
...
function auditLocalServing(subject: Job, builtArtifacts: readonly string[]): readonly LocalServingFinding[] {
    ...
    const restored = artifactSteps(subject, 'download').map((step) => scalarText(step.with?.['name']));
...
describe('the web Playwright suite drives the deployed preview', () => {
     ...
     */
    /*
     * REWRITTEN when `e2e-web` moved into `deployedE2eTiers.yml`: the origin now arrives as a REQUIRED input
     * rather than a `needs` output. The empty-string hazard is the same one one hop up — an optional input a
     * caller omits reads empty — so the input must be required; which resolver each caller feeds it from is
     * asserted per caller in `deployedE2eTiers.test.ts` §2.
     */
    it('takes its target from the caller’s resolved origin, and types no host literal', () => {
        expect(deployedTarget(job('e2e-web')), 'e2e-web no longer drives the resolved preview origin').toBe(
            WEB_ORIGIN_EXPRESSION,
        );
        expect(tiersWorkflow().on?.workflow_call?.inputs?.[WEB_ORIGIN_INPUT]?.required).toBe(true);
    });
    ...
    it('skips, rather than running, when nothing is deployed at that stage', () => {
        // The gate sits on the `_ci.yml` job that CALLS the tiers workflow: a skipped `uses:` job runs none of
        // the called jobs, so it gates every shard.
        const calls = Object.values(ciWorkflow().jobs ?? {}).filter(
            (candidate) => (candidate as { uses?: string }).uses === './.github/workflows/deployedE2eTiers.yml',
        );

        expect(calls, '_ci.yml must call the tiers workflow exactly once').toHaveLength(1);
        expect(String(calls[0]?.if ?? ''), 'e2e-web must wait for the preview it tests').toContain(
    ...
    it('serves nothing on this runner — no server, no mode, no build artifact', () => {
        const built = artifactSteps(job('build'), 'upload').map((step) => scalarText(step.with?.['name']));
...
describe('the harness signs into the Clerk instance that serves the preview', () => {
     ...
     * half is kept and re-argued: the preview origin is served by the SANDBOX instance, so the test-pool slot
     * `globalSetup` resolves through Clerk's Backend API has to live in that same tenant — `inputs.stage`
     * would look it up in the prod tenant on `main`, where `resolveSignInTestUser` refuses a slot `poolAdmin`
     * never provisioned there.
     */
