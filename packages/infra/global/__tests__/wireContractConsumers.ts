 ...
 * `packages/schemas/<service>`" — is already guarded: `contractDriftGate`, `generatedSchemaPackages`
 ...
 * `.github/scripts/deployGate.sh`'s pure `decide` + impure `evaluate`.
 *
 ...
 *    `apiError.schema.ts`, while food and identity both do, so its error envelope has no published zod to
 ...
 *
 * `git ls-files` rather than a directory walk, for the reason `appServiceDependency.test.ts` records: a
