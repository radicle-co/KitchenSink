 ...
 *
 *   - `workflow-invariants.test.ts` asks "can this be FALSE for certain under event E?" and answers
 *     `'unknown'` for anything it does not model, so a guess never invents an unreachable job.
 *   - `heavy-e2e-load-tier-gate.test.ts` asks "given this fully-specified event, does the job run?" and
