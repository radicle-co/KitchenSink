 ...
 *
 *   - `prod-deploy-reachability.test.ts` — the flag/paths reachability of `prod-deploy.yml`'s food+recipe
 ...
 *   - `prod-deploy-build-order.test.ts` — `turbo run build` before `npm prune`, in `prod-deploy.yml` only.
 ...
 *     restated here.
 *   - `global-bootstrap-bundle.test.ts` — `bundle:lambda` before `cdk deploy`/prune for the global app. It
 ...
 * guards execute that script (`prod-deploy-reachability.test.ts`, `deploy-gate.test.ts`, `pr-scope.test.ts`)
