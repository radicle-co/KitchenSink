 ...
 * which is what `_sandbox-preview.yml`'s `ecosystem-smoke` deliberately does NOT do: it states
 * `needs.deploy-food.result == 'success'` explicitly, so a skipped or failed deploy means the ecosystem claim
 * is never made (ADR-0036). Both guards that reason about `needs:` edges need this rule, so it lives with the
 * grammar.
 */
