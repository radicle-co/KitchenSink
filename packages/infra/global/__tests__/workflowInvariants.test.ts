...
const ALLOWED_SILENT_SUCCESS: readonly string[] = [
    ...
    // ⚠️ It moved from `_sandbox-preview.yml::deploy-food` when every deploy became one job per
    // `infra/` folder in `deploy-infra.yml`.
    'suppressed-exit deploy-infra.yml::food-service::Smoke — food service live and reachable ×1',
    // Identical shape, identical reason: a retry loop cannot use `set -e` and still retry.
    'suppressed-exit deploy-infra.yml::identity::Smoke — identity service live and reachable ×1',
