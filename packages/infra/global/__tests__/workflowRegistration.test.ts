 ...
 *
 * It caught `sandboxUp.yml` (fixed by the `push:` registration trigger its header documents), it caught
 * `deployedE2e.yml` and `sandboxReconcile.yml`, and then it caught `sandboxReap.yml`, `sandboxDown.yml`
 * and `accountDeploy.yml` on the day they were written — by an author who had read that header earlier the
 * same session. A comment in one file does not generalise; this does.
 *
 * ## The compounding case, which is worse than the first
 *
 * `sandboxReap.yml` dispatches `sandboxDown.yml`. An unregistered CALLEE means the reaper runs, finds the
 ...
 * registration becomes the automatic-deploy posture ADR-0028 removed, or, for `sandboxDown.yml`,
...
import { WORKFLOWS_DIR, foldContinuations, withInvokedScripts, withoutComments } from './cdkApps.js';
...
interface Workflow {
    ...
    /** The raw text plus the scripts it runs, comments stripped, for the dispatch scan. */
...
const workflows = (): readonly Workflow[] =>
    trackedFiles(WORKFLOWS_DIR)
        ...
        .map((file) => {
            ...
            // The scripts a workflow runs are part of its text: `sandboxReap.yml`'s dispatch of
            // `sandboxDown.yml` is issued from `sandboxReap.sh`, and a YAML-only reading drops that edge
            // from every dispatch assertion below without failing any of them.
            return { name: path.basename(file), doc: parse(raw) ?? {}, text: withInvokedScripts(withoutComments(raw)) };
        });
        ...
        try {
            ...
            throw new Error(
                'origin/main is not in this checkout, so whether a workflow is on the default branch cannot be ' +
                    'answered. Check out with `fetch-depth: 0` (deployInfra.yml › global does).',
            );
        }
...
const branchUnreachable = (workflow: Workflow): boolean => {
    ...
    // `branches: [main]` push cannot, and that is not a hypothetical: `accountDeploy.yml` was written with
...
describe('a dispatch-only workflow can be registered from a branch', () => {
    ...
    it.each(subjects().map((workflow) => [workflow.name] as const))(
        ...
        (name) => {
            ...
            // `accountDeploy.yml` looked correct to a check that only asked whether a push trigger existed.
            expect(
    ...
    it.each(subjects().map((workflow) => [workflow.name] as const))(
        ...
        (name) => {
            // ⛔ The registration run must be inert. For `sandboxDown.yml` this is not a tidiness rule —
...
describe('every workflow dispatched by another workflow is itself registrable', () => {
    ...
    const dispatched = (): readonly { readonly caller: string; readonly callee: string }[] =>
        workflows().flatMap((workflow) =>
            [...workflow.text.matchAll(/gh workflow run\s+([A-Za-z0-9._-]+\.yml)/gu)].map((match) => ({
    ...
    it('finds the reaper’s dispatch of Sandbox Down, which is issued from a script rather than the YAML', () => {
        // ⛔ The edge the header calls THE COMPOUNDING CASE. When this population loses it, the per-edge cases
        // below do not fail — they stop existing — so its presence is asserted by name.
        expect(dispatched()).toContainEqual({ caller: 'sandboxReap.yml', callee: 'sandboxDown.yml' });
    });
    ...
    it.each(dispatched().map(({ caller, callee }) => [`${caller} -> ${callee}`, caller] as const))(
        ...
        (_label, caller) => {
            ...
            for (const site of text.matchAll(/gh workflow run\s+[A-Za-z0-9._-]+\.yml[^\n]*/gu)) {
    ...
    it.each(dispatched().map(({ caller, callee }) => [`${caller} -> ${callee}`, callee] as const))(
        ...
        (_label, callee) => {
            ...
            // treats that as fatal it reports success having caused nothing. `sandboxReap.yml` would have
