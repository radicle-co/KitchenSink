...
const DECLARED_IGNORES: Readonly<Record<string, number>> = {
    ...
    // `publishInfraShared.yml` two (its job grant and the publish step); the two callers one each, since a
    ...
    // `sandboxPreview.yml::deploy`. Each restatement is another site, not another decision.
    // ⚠️ `sandboxPreview.yml` dropped from 2 to 1 when its two deploy JOBS became one call to
    // `deployInfra.yml` — the grant is restated once per call site, and there is now one call site.
    ...
    'sandboxPreview.yml:use-trusted-publishing': 1,
    ...
    // `integration-infra`'s "Install the CDK packages": `nagRulesAtZero` synthesizes EVERY app, so the job
    // installs every CDK package with `prod-deploy.yml`'s own loop. Same deferral, same fix.
    ...
    // ⚠️ `sandboxPreview.yml` is GONE from this list, and that is the fix landing rather than a
    // suppression being deleted: it no longer installs a CDK toolchain at all. The deploys moved to
    // `deployInfra.yml`, whose composite action installs per `infra/` directory, and what remains here is
    ...
    // through `.github/scripts/safeEnv.sh` (strict name, loader/runner-name denylist, no-whitespace value
    // charset), and `safeEnv.test.ts` fails if a suppressed step writes the file any other way.
    'actions/infra-package/action.yml:github-env': 5,
    'ci-main.yml:use-trusted-publishing': 1,
    'ci-pr.yml:use-trusted-publishing': 1,
    'publishInfraShared.yml:use-trusted-publishing': 2,
    ...
    // back `failed_retryable` behind a `202` and `recipes/parseIngredients` failed on `2 of 2 lines read`.
...
describe('every zizmor suppression in the repo is declared', () => {
    ...
    it('counts EVERY audit in a comma-separated ignore list', () => {
        ...
        expect(declaredActionIgnores(ACTIONS_DIR)['actions/infra-package/action.yml:github-env']).toBe(5);
    });
