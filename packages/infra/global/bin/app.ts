...
// matcher in `.github/scripts/prScope.sh`. `environmentTagScheme.test.ts` asserts that in both directions.
...
new GlobalStack(app, `Global-${stage}`, {
    alarmsEnabled,
    // ⚠️ OPTIONAL BY DESIGN — see `GlobalStackProps.logForwarderArn`. Empty or unset means the log-drain
    // subscription filters this app owns are not attached on THIS deploy; they attach on the next one, once
    // the webhooks app has published its forwarder ARN. That is what breaks the deploy cycle between the two
    // apps, and it is why this is read rather than imported.
...
// ⚠️ PROD-ONLY, and only for stage-shaped resources. The account's cost guardrails do NOT belong behind this
// `if` — that would make an ACCOUNT-scoped stack reachable only by deploying a STAGE — so they live in
// `bin/account.ts`, which has no stage at all.
if (stage === 'prod') {
