...
export class CostGuardrailsStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props?: CostGuardrailsStackProps) {
        ...
        // ⛔ `global` HERE MEANS ACCOUNT-SCOPED, which is narrower than "persistent, do not reap" (the tier
        // words `sandbox`/`production` carry that). This stack is the account's ONE budget and
        ...
        // once, never per stage" structural rather than conditional. An `if (stage === 'prod')` in the
        // per-stage app would produce the right number of copies by the wrong mechanism and label an
        // account-wide resource `production`.
        //
