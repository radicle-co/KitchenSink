...
export class CostGuardrailsStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props?: CostGuardrailsStackProps) {
        ...
        // ⛔ `global` HERE MEANS ACCOUNT-SCOPED, and that is narrower than the "persistent, do not reap"
        // the word used to carry across every long-lived stack. This stack is the account's ONE budget and
        // anomaly monitor: it has no stage, it watches every stage's spend including sandbox's, and a second
        // copy would register a duplicate account-wide budget.
        //
        // ⚠️ A LITERAL, where every app entrypoint derives the value from its stage — because this stack has
        // no stage to derive from. Its app (`bin/account.ts`) has none either, which is what makes "deployed
        // once, never per stage" structural rather than conditional. It used to be reachable only through
        // `if (stage === 'prod')` in the per-stage app, which produced the right number of copies by the
        // wrong mechanism and labelled an account-wide resource `production`.
        //
        // ⛔ CHANGING THIS VALUE REPLACES THE TWO COST EXPLORER RESOURCES BELOW, and that is a property of
        // the resource types rather than a mistake here: they carry tags through `ResourceTags`, which
        // CloudFormation documents as `Update requires: Replacement` on both `AWS::CE::AnomalyMonitor` and
        // `AWS::CE::AnomalySubscription` (as does every other property of the monitor — it is effectively
        // immutable). The replacement itself is safe: identity is the generated `MonitorArn`, the names are
        // labels with a documented minimum length of 0, `CreateAnomalyMonitor` declares no conflict error,
        // and the only reference to either is the `Fn::GetAtt` between them inside this stack. The cost is
        // DETECTION CONTINUITY — AWS says a new monitor "might start receiving alerts within 24 hours" — so
        // a rename buys a bounded gap in anomaly alerting on the whole account. That is exactly what
        // reverting this word to `global` AVOIDS: the live stack already carries it, so the tag does not
        // change and neither resource is replaced.
        ...
        topicPolicy.document.addStatements(
            ...
            // (ADR-0008) failing silently. `transportSecurity.test.ts` pins both halves: exactly one
