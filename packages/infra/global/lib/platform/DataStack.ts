...
export interface DataStackProps extends StackProps {
     ...
     * see `GlobalStackProps.logForwarderArn` for the cycle that makes absence a supported state.
     */
...
export class DataStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: DataStackProps) {
        ...
        this.database = new rds.DatabaseInstance(this, 'Database', {
            ...
            // `docs/runbooks/pg18Upgrade.md`: it carries the pre-flight checks, the window, and the
        ...
        // to PUBLIC, and the master never on a chain to `rds_iam`. One function for every service, so the
        // bootstrap is never the same code twice.
        //
        ...
        const bootstrapLogGroup = new logs.LogGroup(this, 'DbBootstrapLogGroup', {
            retention: logs.RetentionDays.ONE_MONTH,
            removalPolicy: RemovalPolicy.DESTROY,
        });
        // The persistent stage whose webhook stack owns the log forwarder: prod rides prod, everything
        // else rides sandbox.
        const drainBaseStage = props.stage === 'prod' ? 'prod' : 'sandbox';

        // ⛔ THE PLATFORM DSN, resolved at DEPLOY time from the base stage's parameter, exactly as every
        // other runtime resolves its own. These two functions report ONE thing (plan U18): that per-PR
        // reclamation has silently stopped, which bills for every abandoned preview until somebody
        // notices — and the reason it needs an ISSUE rather than the log line it already writes is that a
        // forwarded log line does not group, carries no fingerprint, and nothing alerts on it.
        //
        // ⚠️ A MISSING PARAMETER FAILS THE DEPLOY, which is the intended behaviour and the same one every
        // other stack has: a deployed function that silently cannot report is the failure this work exists
        // to prevent. Both base stages carry it.
        const platformSentryDsn = ssm.StringParameter.valueForStringParameter(
            this,
            `/kitchensink/${drainBaseStage}/sentry/platform-dsn`,
        );

        // ⛔ ONLY WHEN THE ARN IS KNOWN. This was `Fn.importValue` against the webhooks app's export, which
        // made the two apps mutually dependent: `WebhooksStack` imports this app's `HandleSyncTopicArn`, so
        // neither could go first and every global deploy failed with "No export named … LogForwarderArn
        // found". The ARN now arrives as a CI-resolved input; absent, the filter is skipped and the
        // synth says so.
        //
        // ⚠️ IT ATTACHES ON THE NEXT DEPLOY OF THIS APP, WHICH IS NOT THE NEXT PIPELINE RUN. Nothing
        // arms `deploy_global` when the export APPEARS: it is a `workflow_dispatch`, a change under
        // `packages/infra/global/**`, or — on sandbox only — an absent platform stack. So on a fresh
        // account, where this app necessarily deploys before the webhooks app publishes the ARN, the
        // window stays open until one of those happens. ADR-0042 records it as a residual rather than
        // closing it here, because arming a prod deploy off a probe is a larger change than the gap.
        if (props.logForwarderArn !== undefined) {
            new logs.SubscriptionFilter(this, 'DbBootstrapLogDrain', {
        ...
        // IAM-auth setting applied first. The logical IDs are distinct from `FoodDbBootstrap`/`RecipeDbBootstrap`,
        // whose resources a deploy deletes and whose handler answers that delete as a no-op.
        ...
        if (stageTag !== 'prod') {
            ...
            // and changing a Lambda's name from generated to explicit REPLACES it — a needless replacement of
            // the function that migrates the production database.
            const reaperLogGroup = new logs.LogGroup(this, 'PerPrDatabaseReaperLogGroup', {
                retention: logs.RetentionDays.ONE_MONTH,
                removalPolicy: RemovalPolicy.DESTROY,
            });
            ...
            // closing it here, because arming a prod deploy off a probe is a larger change than the gap.
            if (props.logForwarderArn !== undefined) {
                new logs.SubscriptionFilter(this, 'PerPrDatabaseReaperLogDrain', {
                    logGroup: reaperLogGroup,
                    destination: new logsDestinations.LambdaDestination(
                        lambda.Function.fromFunctionArn(this, 'ImportedLogForwarderForReaper', props.logForwarderArn),
                        { addPermissions: false },
                    ),
                    filterPattern: logs.FilterPattern.literal('-START -END -REPORT -"_aws"'),
                    filterName: 'forward-app-logs',
