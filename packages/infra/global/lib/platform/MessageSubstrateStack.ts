...
import { AcceptedNagFindings, acceptNagFindings, subscribeAlarmEmail } from '@radicle-co/infra-shared/security';
 ...
 * an app-level `Environment` tag naming the persistent TIER (`sandbox` / `production`), and
 * `sandbox-deploy.yml` never deploys the global app with `stage=pr-{N}`. A per-PR table added here would
 * therefore never be created — and if it somehow were, it would carry a tier word no per-PR teardown can
 * claim (ADR-0005). It would leak one table per closed pull request, forever.
 *
 * The producer's stack is already deployed per-PR and already tagged `Environment=pr-{N}-sandbox`, so a table
