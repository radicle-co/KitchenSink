 ...
 *
 * Anchored, and digits-required, for the reason `pr-scope.sh` already enforces on the teardown path — a
 ...
 *     ADR-0005 forbids". That tag is the same authority that licenses `teardown-sandbox-pr.sh` to delete
 *     whole stacks.
 *
 * ⚠️ The name door's justification CHANGED with the tagging scheme and is restated rather than assumed. It
 * used to be "the shared tier's tag is `global`, the same value production carries, so the tag can never
 * select it" — no longer true, since the tiers are now `sandbox` and `production`. The door stays on the
 * name anyway: broadening the tag door to the bare tier word would put every preview AND the shared tier
