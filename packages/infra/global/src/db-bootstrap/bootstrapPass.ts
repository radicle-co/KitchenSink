...
/**
 * A probe failure as an operator reads it: the driver's words, with the SQLSTATE when one came back. Pure.
 *
 * @param failure - The failed probe.
 * @returns E.g. `password authentication failed for user "x" (28P01)`.
 */
function describeProbeFailure(failure: ProbeFailure): string {
    return failure.code === undefined ? failure.message : `${failure.message} (${failure.code})`;
}
...
async function probeOrRollBack(ports: BootstrapPorts, input: BootstrapPassInput, step: string): Promise<void> {
    ...
    // across the whole instance, in response to the thing least likely to mean what it looked like. A single
    // `ECONNREFUSED` read as a lock-out revokes everything and then reports `recoveredAfterRollback: true` —
    // for a master that was fine all along.
    //
    ...
    // ⛔ ONLY A CREDENTIAL REFUSAL IS A LOCK-OUT. Ending a sustained outage in the rollback would revoke
    // `rds_iam` from `<svc>_migrator` and `<svc>_app` — live roles every running service authenticates
    // with — and so manufacture an instance-wide outage out of a network fault. The two are separated by
    // SQLSTATE: `28P01`/`28000` mean the server refused THESE CREDENTIALS, which is what
    ...
    throw new MasterLoginProbeError(
        `${step}: ${describeProbeFailure(failure)}`,
        (await probeMasterLogin(ports)) === undefined,
    );
}
