...
export async function recreateLegacyDatabase(session: CatalogReader, input: RecreateInput): Promise<RecreateReport> {
    ...
    if (input.legacyOwner === master) {
        await session.query(`DROP DATABASE ${pg.escapeIdentifier(database)} WITH (FORCE)`);
    ...
    // Exactly one of the two, so the return below needs no cast — and a rejection whose reason is `undefined` is
    // still a failure rather than a missing report.
    let outcome: { readonly report: RecreateReport } | { readonly failure: unknown };

    try {
        await session.query(
            `GRANT ${pg.escapeIdentifier(app)} TO ${pg.escapeIdentifier(master)} WITH INHERIT TRUE, SET FALSE`,
        );
        await session.query(`DROP DATABASE ${pg.escapeIdentifier(database)} WITH (FORCE)`);
        ...
        if (!input.isProd) {
            ...
            for (const child of children) {
                if (child.datconnlimit === -2) {
                    ...
                    await session.query(
                        `ALTER DATABASE ${pg.escapeIdentifier(child.datname)} OWNER TO ${pg.escapeIdentifier(roles.owner)}`,
                    );
        ...
        outcome = { report: { dropped: database, reowned, skippedDraining } };
    } catch (error) {
    ...
    try {
        ...
        throw 'failure' in outcome
            ? new AggregateError(
                  [releaseError, outcome.failure],
                  `releasing the master failed after the recreate failed: ${releaseError instanceof Error ? releaseError.message : String(releaseError)}`,
              )
            : releaseError;
    }
