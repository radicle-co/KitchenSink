...
export async function revokeMembership(
    ...
    await session.query(`REVOKE ${pg.escapeIdentifier(role)} FROM ${pg.escapeIdentifier(member)}`);
    ...
    for (const { grantor } of survivors) {
        try {
            await session.query(
                `REVOKE ${pg.escapeIdentifier(role)} FROM ${pg.escapeIdentifier(member)} GRANTED BY ${pg.escapeIdentifier(grantor)}`,
            );
        } catch (error) {
            refusals.push(
