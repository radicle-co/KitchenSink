 ...
 *
 * ⚠️ A real lock-out therefore does not self-heal on a probe whose error carries no SQLSTATE. That is
 * accepted: rolling back on such a probe "heals" by causing the outage it is meant to prevent — a single
 * `ECONNREFUSED` produces a full revoke and then reports `recoveredAfterRollback: true`, having recovered
 * only from its own damage.
 */
