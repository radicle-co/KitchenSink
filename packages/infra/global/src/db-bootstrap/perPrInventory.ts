 ...
 * the PR closes. For half of them it never did: `teardownSandboxPr.sh` hardcoded food's migration-runner
 ...
 * left behind, and does the number stop growing?** `docs/runbooks/pg18Upgrade.md` §A4 asks an operator to
 ...
 *
 * @param pool - A connection as the MASTER user (the bootstrap's own session).
 * @param base - The shared base database name, e.g. `kitchensink_food`.
 * @sideEffect Reads `pg_database` and writes to stdout/stderr.
 */
export async function reportPerPrDatabases(pool: CatalogReader, base: string): Promise<void> {
    try {
