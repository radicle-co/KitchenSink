 ...
 * `teardownSandboxPr.sh` used to drop it by invoking that service's own in-VPC migration runner. That door
...
import { ownersTheMasterCannotDrop } from './reclamationAuthority.js';
...
/**
 * The owner roles a per-PR drop depends on the master inheriting.
 *
 * ⚠️ Derived from `DATABASE_ROLES` rather than written out, so a fourth service joining the role model
 * cannot be missed here — a copy of this list would go stale the same way ADR-0004's consumer table did.
 */
const RECLAIMABLE_OWNERS: readonly string[] = Object.values(DATABASE_ROLES).map((roles) => roles.owner);
...
export interface ReapResult {
    ...
    /**
     * The role census (role-split Step 0), read BEFORE any drop so a failed drop still reports the graph that
     * explains it — e.g. the `must be owner` failure a per-PR drop hits on a database the reaper does not own.
     */
    readonly roles: RoleCensus;
}
...
export function reapDropStatement(databaseName: string): string {
    return `DROP DATABASE IF EXISTS ${pg.escapeIdentifier(databaseName)} WITH (FORCE)`;
}
 ...
 */
export async function readPerPrCatalog(pool: CatalogReader): Promise<readonly PerPrDatabaseCatalogRow[]> {
 ...
 * `teardownSandboxPr.sh` applies before deleting a GitHub Environment: the predicate already refused
...
export const handler = async (event: unknown): Promise<ReapResult> => {
    ...
    // ONE held connection for the whole invocation: the advisory lock below is session-level, so it must be taken,
    // used and released on the same session.
    const session = await pool.connect();

    try {
        // ⛔ The role-model bootstrap's lock (ADR-0039): a reap never interleaves with a bootstrap pass, which may be
        // mid-way through joining the master to a legacy app role.
        await session.query('SELECT pg_advisory_lock($1)', [ROLE_CATALOG_LOCK_KEY]);

        const roles = await readRoleCensus(session);

        // Logged NOW, not only in the result: a drop that throws (`must be owner`) never returns one, and that
        // failure is exactly what this census exists to explain.
        console.log(JSON.stringify({ message: 'role census (role-split Step 0)', stage, roles }));

        // ⛔ CAN THIS REAPER STILL DROP? (plan U18). Everything below reports what the reaper DID; nothing
        // reported that it no longer CAN. The bootstrap's master-login rollback releases the master from the
        // owner roles a drop depends on, and from that moment this function runs daily, drops nothing, and
        // returns the same clean census it returns on a quiet day — while every abandoned preview keeps
        // billing. The log line goes to the drain, which is a log line: it does not group, it carries no
        // fingerprint, and nothing alerts on it. So this one becomes an ISSUE.
        //
        // ⚠️ Reported and then CONTINUED, not thrown. The reaper's count action is still useful without drop
        // authority, and a reaper that refused to run would replace a silent no-op with a loud one.
        const blockedOwners = ownersTheMasterCannotDrop(roles.edges, roles.currentUser, RECLAIMABLE_OWNERS);

        if (blockedOwners.length > 0) {
            console.error(JSON.stringify({ message: 'per-PR reclamation is disabled', stage, blockedOwners }));
            // ⛔ The ANNOUNCING form — it reads the reporter's answer and falls back to the drain when the
            // envelope did not land. See `observability/silentNoOp.ts` for why discarding that answer is a
            // silent failure to report a silent failure.
            await announceSilentNoOp({
                service: 'db-reaper',
                stage,
                condition: 'per-pr-reclamation-disabled',
                detail:
                    'the master no longer inherits the owner roles a per-PR DROP DATABASE depends on, so ' +
                    'every abandoned preview keeps billing until the next successful DataStack deploy',
            });
        }

        const plan = planReap(await readPerPrCatalog(session), request);
        const outcome = await executeReap(session, plan, request);
        ...
        await session.query('SELECT pg_advisory_unlock($1)', [ROLE_CATALOG_LOCK_KEY]).catch(() => undefined);
        session.release();
        await pool.end();
    }
};
