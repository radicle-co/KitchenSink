 ...
 *    `bin/app.ts` ({@link DEPLOY_APP}). No deploy runs a compiled `node dist/bin/app.js`: bare `node`
 *    type-strips a workspace package's `index.ts` but cannot resolve its `./x.js` specifiers, and this app
 *    imports a workspace package (`@kitchensink/db-schema-guard`, ADR-0039). That the pipelines name this
 *    runner is pinned by `cdkApps.ts` and `postPruneToolchain`.
 *
