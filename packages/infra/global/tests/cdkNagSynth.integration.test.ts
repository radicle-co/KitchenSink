 ...
 * 2. The synth runs through the exact `--app` every deploy uses — the package's own `tsx` over
 *    `bin/app.ts` ({@link DEPLOY_APP}). Deploys USED to run the compiled `node dist/bin/app.js`, and this
 *    suite carried a regression guard for it: bare `node` type-strips a workspace package's `index.ts` but
 *    cannot resolve its `./x.js` specifiers. 2c9870b5 moved every CDK entrypoint to `tsx` for exactly that
 *    reason, so the compiled path is no longer run anywhere and its guard was deleted once this app took its
 *    first workspace import (`@kitchensink/db-schema-guard`, ADR-0039) — the guard was failing a path that
 *    does not exist. That the pipelines name this runner is pinned by `cdkApps.ts` and `postPruneToolchain`.
 *
 * ## What is real, and what is stubbed
 *
 * - **Real**: the `cdk` CLI as a child process, the real `bin/app.ts` entrypoint under the deploy's own
 *   `tsx`, real synthesis to a real output directory, and the real committed `cdk.context.json`.
...
/**
 * The `--app` every deploy runs: the package's OWN `tsx` over the source entrypoint (2c9870b5). Not `npx tsx`,
 * which falls back to the registry when the bin is missing — the defect that change removed.
 */
const DEPLOY_APP = 'node_modules/.bin/tsx bin/app.ts';

const synth = (stage: string): SynthResult => synthWith(DEPLOY_APP, stage);
...
// A real bundle, plus two real CLI synths of the whole platform app; each synth spawns `tsx` and walks every
// construct twice (tags, then nag), so give them room on a loaded CI runner.
beforeAll(() => {
...
describe('the production CloudFront edge, through the real CLI', () => {
    ...
    it('FAILS the synth when the build-time key is absent, instead of shipping a keyless verifier', () => {
        ...
        const keyless = synthWith(DEPLOY_APP, 'prod', { edgeKey: null });
