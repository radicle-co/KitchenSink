 ...
 *
 * The superuser (`DATABASE_ADMIN_URL`) only builds fixtures and reads results. Every statement under test runs as the
...
/**
 * Resolve once some backend is waiting for an advisory lock it has not been granted, or throw after `timeoutMs`.
 *
 * @param observer - A connection to watch from; it must not be the waiter.
 * @param timeoutMs - How long to wait for the waiter to appear.
 * @sideEffect Polls `pg_locks`.
 */
async function waitForAdvisoryLockWaiter(observer: pg.Client, timeoutMs = 10_000): Promise<void> {
    ...
    throw new Error(`no backend queued on an advisory lock within ${timeoutMs} ms`);
}
...
describe.skipIf(!SERVER_URL)('the role-split bootstrap pass against real PostgreSQL', () => {
    ...
    async function hasMarker(database: string): Promise<boolean> {
        const client = new pg.Client({ connectionString: SERVER_URL?.replace(/\/[^/]*$/u, `/${database}`) });
    ...
    describe('a fresh stage — no roles, no database', () => {
        ...
        it('⛔ SERIALIZES: a second pass waits on the advisory lock the first one holds', async () => {
            ...
            // Wait until the second pass is QUEUED on the lock — observed in `pg_locks`, not assumed from elapsed
            // time — then prove it has not run past it.
            await waitForAdvisoryLockWaiter(holder);
        ...
        it('⛔ a SUSTAINED connectivity failure revokes NOTHING — it is not a lock-out', async () => {
            ...
            // auth in response to a network fault. Re-probing covers a STUMBLE; this covers the server
            // staying unreachable for every attempt.
            //
        ...
        it('⛔ rolls back what it granted the master when a fresh master login fails, then throws', async () => {
            // ⚠️ CARRIES `28P01`, which is what a real `pg` auth failure carries. The pass decides whether to
            // roll back from the SQLSTATE rather than from the message, so a double without one would exercise
            // the UNREACHABLE path instead — and would not be testing what this case is named for.
            ...
            // The operator's only diagnosis of a possible lock-out: the failure's words AND its SQLSTATE, never an
            // interpolated object.
            expect((error as Error).message).toContain('password authentication failed');
            expect((error as Error).message).toContain('28P01');
    ...
    describe('a legacy food database — owned by the old single role, which holds rds_iam', () => {
        ...
        it('⛔ a failed master login right after the role model stops the pass BEFORE the drop, cutting rds_iam', async () => {
            // The probe runs after each role-model step, not only at the END of the pass, so a
            ...
            const failingProbe: BootstrapPorts['connectAsMaster'] = async (database) => {
                if (database === 'postgres') {
                    ...
                    // so a code-less double would take the UNREACHABLE path and revoke nothing.
                    throw Object.assign(new Error('password authentication failed for user "ksb_master" (injected)'), {
                        code: '28P01',
                    });
                }
