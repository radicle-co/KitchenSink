 ...
 *
 * `DATABASE_ADMIN_URL` must point at the maintenance database of a throwaway PostgreSQL as a superuser (CI provides one).
 */
...
/** A connection string for `role` against `database`, on the same server as `DATABASE_ADMIN_URL`. */
...
describe.skipIf(!SERVER_URL)('the role split against real PostgreSQL', () => {
    describe('role split privileges against real PostgreSQL', () => {
        const admin = new pg.Pool({ connectionString: SERVER_URL, max: 1 });
    ...
    describe('applyRoleModel / assertRoleModel as a NOSUPERUSER master', () => {
        const superuser = new pg.Pool({ connectionString: SERVER_URL, max: 1 });
