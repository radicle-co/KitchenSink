/**
 * Integration suite for the ensure-exists deploy gate's IMPURE half — `deployGate.sh evaluate`
...
describe('deployGate.sh evaluate — the gate is wired to the real CloudFormation + HTTP boundaries', () => {
 ...
 *
 * ⛔ The export lookup goes through `.github/scripts/cfnExport.sh` and nowhere else. `list-exports` pages at
...
# Extract the export name out of the JMESPath query, exactly as cfnExport.sh builds it.
query=''
for arg in "\${@}"; do
...
describe('deployGate.sh unmet-imports — the derived graph meets the real account', () => {
    ...
    it('survives the PAGINATED `None` the real CLI emits, rather than reading it as a value', () => {
        // ⛔ The bug `cfnExport.sh` exists for. The stub answers two lines per lookup, as the real CLI does
    ...
    it('REFUSES when the credentials are broken, instead of calling every export absent', () => {
        // ⛔ `cfnExport.sh --optional` cannot tell "the export is absent" from "the CLI failed" — it answers
