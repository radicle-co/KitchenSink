 ...
 * decides whether a deploy is reported as verified. Its first version returned 0 — a PASS — when the AWS
 * call errored, so that a missing `rds:Describe*` grant could not red every deploy. That collapsed
 * "could not check" into "checked and fine", in the one script whose job is telling those apart, and the
 * owner ruled it a hard failure instead (2026-09-10).
 *
