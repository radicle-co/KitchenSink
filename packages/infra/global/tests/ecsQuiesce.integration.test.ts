/**
 * Integration suite for `.github/scripts/ecsQuiesce.sh` — the ordering fix that lets a per-PR stack delete
 * actually complete.
 *
 ...
 *      argument rather than a prefix of it. A `toContain('…Values=pr-73')` would be satisfied by the
 *      single-value filter, staying green across exactly the change it exists to catch.
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/ecsQuiesce.sh', import.meta.url));
...
describe('ecsQuiesce.sh — draining before the stack delete', () => {
    ...
    it('scopes discovery to the exact Environment values this PR owns, and to clusters only', () => {
        ...
        // ⛔ THE WHOLE FILTER, not a substring of it. `toContain('Key=Environment,Values=pr-73')` is
        // satisfied by ANY filter beginning with those characters, so it stays green while the script sends
        // only the bare value and misses every `pr-{N}-sandbox` resource. `pr-73` remains in the list on
        // purpose: resources tagged with the bare value are still this PR's to reclaim.
