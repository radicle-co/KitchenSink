...
describe('the artifact esbuild actually ships to the edge', () => {
    /** Bundle outdirs this suite created, removed in `afterAll` so a FAILING test still cleans up. */
    ...
    async function bundleEdgeVerifier(jwtKey: string): Promise<string> {
        ...
        // Whether Node loads that as CJS depends on the NEAREST package.json, so a temp directory that
        // happens to sit under a `"type": "module"` package makes the require below fail with `module is
        // not defined in ES module scope` — which is exactly how it once shipped red on CI. The marker makes
        // the directory self-describing wherever `TMPDIR` points, rather than relying on where that is.
