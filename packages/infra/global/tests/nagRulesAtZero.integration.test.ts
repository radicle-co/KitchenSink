 ...
 * means; and the build is the same workspace preparation every deploy already runs, read from `deploy-infra.yml`
 ...
 *
 * ⚠️ Each script names the directory it runs in. These used to run as `--workspace=<the app's package>`,
 * which was right while the CDK apps were workspace members and is an npm error ("No workspaces found") now
 * that they are not — and `router:bundle` was never the infra package's script at all, it is `@commise/web`'s.
 * Neither was visible, because the turbo step before them failed first.
 */
...
const DEPLOY_INFRA_WORKFLOW = path.join(repoRoot, '.github/workflows/deploy-infra.yml');
...
/**
 * The workspace preparation the DEPLOY performs per infra directory, read from `deploy-infra.yml`.
 *
 ...
 * that drifts. The previous shape asked turbo to build each app's OWN package, which stopped meaning anything
 * the day the CDK apps left the workspace: turbo answered "No package found" and the whole suite failed in
 * `beforeAll`, on every CI run from 2026-09-08.
 *
...
function deployPreparation(): ReadonlyMap<string, DeployPreparation> {
    ...
    for (const job of Object.values(doc.jobs)) {
        for (const step of job.steps ?? []) {
            ...
            const input = (name: string): string => String(step.with?.[name] ?? '');
