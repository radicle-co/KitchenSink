 ...
 * ## How to run it
 *
 * `DATABASE_ADMIN_URL` must point at the MAINTENANCE database of a throwaway PostgreSQL as a superuser — the
 ...
 *     DATABASE_ADMIN_URL=postgres://postgres:postgres@localhost:5432/postgres npm run test:integration \
...
describe.skipIf(!SERVER_URL)('per-PR database reaper against real PostgreSQL (ADR-0031)', () => {
    const pool = new pg.Pool({ connectionString: SERVER_URL, max: 2 });
    ...
    it('⛔ FORCES a database that still has an open session — the clause the whole drop hangs on', async () => {
        ...
        const squatter = new pg.Pool({
            connectionString: `${SERVER_URL?.replace(/\/[^/]*$/, '')}/kitchensink_food_pr_1`,
            max: 1,
        });
    ...
    describe('role census (role-split Step 0)', () => {
        ...
        it('reports whether the connecting role holds ADMIN on each role the split must grant — measured, not assumed', async () => {
            ...
            const base = SERVER_URL?.replace(/\/\/[^@]*@/u, `//${MASTER}:pw@`) ?? '';
 ...
 */
describe.skipIf(!SERVER_URL)('per-PR database reaper as a NOSUPERUSER master, under the role split', () => {
