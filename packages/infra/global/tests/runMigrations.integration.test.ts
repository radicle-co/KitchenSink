/**
 * Integration suite for the migration safety net's IMPURE half — `runMigrations.sh run`.
 ...
 * | the lookup itself failed | read it as absence → a denied or throttled call skips the migrate and the deploy ships onto an unmigrated schema |
 */
...
const SCRIPT = fileURLToPath(new URL('../../../../.github/scripts/runMigrations.sh', import.meta.url));
...
describe('run — the safety net resolves, invokes, and reads the answer', () => {
    ...
    ])('⛔ FAILS when the stack could not be ASKED about — %s is not an absent stack', (_what, message, needle) => {
        // ⛔ "It does not exist" and "I could not find out" are DIFFERENT answers, and only one of them is
        // safe to skip on. The lookup discarded stderr and treated EVERY non-zero status as absence, so a
        // denied, throttled, or wrong-region call announced "there is no schema of ours to migrate" and
        // returned 0 — and under ADR-0035 the migrate step runs AHEAD of the deploy, so the service then
        // shipped onto an unmigrated schema. That is the same `applied: []`-shaped silence ADR-0035 says
        // hoisting would cause, arriving through the lookup instead of through the runner.
        //
        // The asymmetry that proves it was an oversight: the `aws lambda invoke` twenty lines below ALREADY
        // converts a CLI failure into `InvokeFailed` rather than into success.
        const result = run(['run', 'us-east-1', STACK, OUTPUT, 'food', MIGRATIONS_DIR], {
            STUB_DESCRIBE_ERROR: message,
...
describe('run — the migration manifest travels with the invoke', () => {
    it('sends the digest of the working tree, so a runner holding a different set can refuse', () => {
        ...
        const sent: unknown = JSON.parse(readFileSync(join(fixtures, 'sent-payload'), 'utf8'));

        expect(sent).toEqual({ expectManifestSha: readMigrationManifest(MIGRATIONS_DIR).sha });
    });

    it('sends NOTHING else — every runner parses its event `.strict()`, so an extra key is a refused deploy', () => {
        // ⛔ The runners have ONE action since the role split deleted their `drop` doors (ADR-0039), and each
        // parses `{ expectManifestSha }` `.strict()` so an old `{ "action": "drop" }` is refused rather than
        // silently migrating. This script kept sending `"action":"migrate"`, and every schema step on the
        // first deploy of the split failed with `Unrecognized key: "action"`. Exact equality above is what
        // ties the two sides together; this names the failure it prevents.
        givenStack(STACK, [{ OutputKey: OUTPUT, OutputValue: 'kitchensink-food-migrate-prod' }]);

        run(['run', 'us-east-1', STACK, OUTPUT, 'food', MIGRATIONS_DIR], { STUB_INVOKE_PAYLOAD: '{"applied":[]}' });

        const sent = JSON.parse(readFileSync(join(fixtures, 'sent-payload'), 'utf8')) as Record<string, unknown>;

        expect(Object.keys(sent)).toEqual(['expectManifestSha']);
    });
