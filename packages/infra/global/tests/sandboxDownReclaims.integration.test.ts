 ...
 * ## The failure
 *
 * A hosted-zone lookup ahead of the teardown that fails for a reason unrelated to teardown takes the whole
 * sweep with it: merged PRs are never reclaimed, Fargate tasks keep billing for closed work, and the daily
 * reaper fails at the same step every day. An ordering rule does not prevent it; the SAME outcome is
 * reachable without any ordering violation, by letting the lookup abort the step it lives inside. That is
 * what this asserts.
 *
 * ## How it runs
 *
 * The real `run:` body is lifted out of the workflow YAML, `PATH` is pointed at stubs, and it is executed
 * under `bash -e` exactly as Actions would. The stub for `cfnExport.sh` exits non-zero; the stub for the
...
const WORKFLOW = path.join(repoRoot, '.github/workflows/sandboxDown.yml');
...
function teardownStepBody(): string {
    ...
    const body = Object.values(doc.jobs)
        ...
        .map((step) => step.run ?? '')
        .find((run) => run.includes('teardownSandboxPr.sh'));

    if (body === undefined) {
        throw new Error('sandboxDown.yml no longer has a step running teardownSandboxPr.sh');
    }
...
function runStep(
    ...
    writeFileSync(path.join(scripts, 'cfnExport.sh'), `#!/usr/bin/env bash\nexit ${lookupExit}\n`);
    writeFileSync(
        path.join(scripts, 'teardownSandboxPr.sh'),
        `#!/usr/bin/env bash\nprintf '%s' "$1" > ${JSON.stringify(marker)}\nexit 0\n`,
    );
    chmodSync(path.join(scripts, 'cfnExport.sh'), 0o755);
    chmodSync(path.join(scripts, 'teardownSandboxPr.sh'), 0o755);
...
describe('Sandbox Down reclaims infrastructure even when the DNS lookup fails', () => {
    ...
    it('⛔ still reaches the teardown when the hosted-zone lookup EXITS NON-ZERO', () => {
        // A DNS lookup must never be able to cancel the reclamation of stacks,
...
/**
 * ⛔ LAYER 2 of the three in `sandboxDown.yml`'s header, asserted as BEHAVIOUR rather than as shape.
 *
