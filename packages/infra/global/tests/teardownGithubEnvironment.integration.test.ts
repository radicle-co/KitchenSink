/**
 * Integration suite for section 0b of `.github/scripts/teardownSandboxPr.sh` — reclaiming the per-PR
 * GitHub **Environment**.
 *
 ...
 *
 * - **Real**: the teardown script, executed as `bash` in a child process, sourcing the real `prScope.sh`.
...
describe('teardownSandboxPr.sh §0b — reclaiming the per-PR GitHub Environment', () => {
...
describe('teardownSandboxPr.sh §0b — failure and unconfigured states', () => {
