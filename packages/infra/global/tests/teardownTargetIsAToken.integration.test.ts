 ...
 * `run:` body, under `bash -e` as Actions runs it, with `teardownSandboxPr.sh` replaced by a stub that
 ...
 * | `sandboxDown.yml` › `down` | the `pr` dispatch input | covered in `sandboxDownReclaims` |
 *
...
function teardownStep(workflow: string, job: string): string {
    ...
    // `chmod +x …teardownSandboxPr.sh`, so matching on the mention alone is right, but the earlier
    // version excluded any body whose FIRST line was a chmod — which is all of them.
    const invokes = (run: string): boolean =>
        run.split('\n').some((line) => line.includes('teardownSandboxPr.sh') && !/\bchmod\b/u.test(line));
    const body = (doc.jobs[job]?.steps ?? []).map((step) => step.run ?? '').find(invokes);

    if (body === undefined) {
        throw new Error(`${workflow}:${job} no longer has a step invoking teardownSandboxPr.sh`);
    }
...
function targetsHandedToTeardown(
    ...
    for (const helper of ['prScope.sh', 'cfnRestingStates.sh']) {
...
describe('sandbox-deploy.yml › cleanup — the on-close path', () => {
    ...
    it('⛔ and the REAL script refuses every non-token this job could pass it', () => {
        // The other half, run against the actual `teardownSandboxPr.sh` rather than the stub — because
        // "cleanup does not validate" is only safe if something downstream does.
        for (const hostile of ['pr-prod', 'sandbox', 'pr-91 prod', 'prod']) {
            const result = spawnSync('bash', [path.join(repoRoot, '.github/scripts/teardownSandboxPr.sh'), hostile], {
                encoding: 'utf8',
                timeout: 20_000,
