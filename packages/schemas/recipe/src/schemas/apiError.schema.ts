 ...
 */
// Source: packages/services/recipe-service/src/common/apiError.schema.ts
 ...
 *
 * ⚠️ **`recipe-core`'s `recipeErrorSchema` is NOT the wire shape.** Its `code` is an ENUM of the fifteen
 * recipe-DOMAIN codes: CORRECT for the type guard `ApiExceptionFilter` uses to recognise a thrown domain error, and
 * WRONG as a description of the wire — the generic `500`'s `INTERNAL_ERROR`, the auth middleware's
 * `IDENTITY_SYNC_PENDING` `401`, the erasure service's `ACCOUNT_ALREADY_ERASED` `410`, the validation `400` and the
 * `429` all FAIL it.
 *
 * ── ONE SHAPE, AND THE FILTER IS ITS SOLE AUTHOR ──
 *
 * `ApiExceptionFilter` normalizes EVERY failure into {@link apiErrorSchema} and has **no passthrough branch**.
 * Returning `exception.getResponse()` unchanged would emit FOUR shapes — the envelope, Nest's
 * `{ statusCode, message, error? }`, `nestjs-zod`'s `{ statusCode, message: 'Validation failed', errors }`, and a
 * bare JSON **string** for the `429` — and publishing those as a union makes the inconsistency honest instead of
 * removing it, leaving a client to tell two `409`s apart by a string compare on `body.code` through an unchecked
 * cast. Because the guarantee lives in one class instead of in throw-site discipline, it also covers failures this
 * service never raises — `ClerkAuthService`'s argument-less `401`s, Nest's `404` for an unrouted path, the body
 * parser's `413`, the throttler's string `429`. food-service's filter has the same shape, deliberately, so the
 * services share one dialect.
 *
 ...
 * and it is how a published error type comes to describe a shape the service mostly does not send.
...
export const recipeErrorCodeSchema = z.enum([
    ...
    'FORBIDDEN',
    /**
     * ADR-0040 — a signed test principal attempted a write this stage CONTAINS (publish, rate, clone of another's
     * content, erasure). Its own code rather than `FORBIDDEN`, so a load or e2e harness can tell "the stage
     * contained my test write, as designed" from a scope gate it should treat as a failure.
     */
