/**
 * The food (ingredient) service's own half of its contract configuration — the paths, the import allowlist and
...
/** Absolute path of the food-service package root. */
export const SERVICE_ROOT = resolve(import.meta.dirname, '..');

/** Absolute path of the generated schema package. */
export const SCHEMA_PACKAGE_ROOT = resolve(SERVICE_ROOT, '../../schemas/food');

/** The generated package's npm name. */
export const SCHEMA_PACKAGE_NAME = '@kitchensink/schema-food';

/** The command that regenerates the package, quoted in every generated banner. */
export const REGENERATE_COMMAND = 'npm run contract:generate --workspace=@kitchensink/food-service';

/**
 * Module specifiers an authored `*.schema.ts` may import.
 *
 * ZOD ONLY. This is deliberately narrower than recipe's allowlist, which also admits
 * `@kitchensink/recipe-core` because that package already owns the recipe DOMAIN schemas and re-declaring them
 * would manufacture drift. Food has no equivalent: `@kitchensink/recipe-core` is a RECIPE domain package, and
 * the one-directional rule (recipes reference ingredients by opaque `food_id`; nothing is ever written back into
 * the food DB — feature 001 T150) means the ingredient contract must not acquire a dependency pointing the other
 * way. Before adding an entry, verify the candidate is a leaf whose transitive runtime dependencies are safe to
 * bundle into a React Native app — that property, not convenience, is the admission test.
 */
...
export const SERVICE_PATH_PREFIX = 'packages/services/food-service';

/** Human name of the contract, used in the `CONTRACT_HASH` doc comment. */
export const CONTRACT_DISPLAY_NAME = 'food (ingredient) service';
