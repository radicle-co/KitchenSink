...
describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Five alarms per open pull request: a retry-budget exhaustion detector, a tombstone counter, an
        // API 5xx error-rate expression, a queue-depth backpressure threshold and a pending-age freshness
        // threshold — every one of them watching a catalog nobody is populating.
        synthFoodTemplate('pr-91', 'sandbox', false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    });

    it('creates all five food alarms when the flag is on', () => {
        synthFoodTemplate('pr-91', 'sandbox', true).resourceCountIs('AWS::CloudWatch::Alarm', 5);
    });
