...
describe('In-VPC migration-runner Lambda (T-191) — in its own schema stack', () => {
    ...
    it('grants the migration function rds-db:connect on the food_MIGRATOR db-user, never the service login', () => {
        // ⚠️ REWRITTEN for the role split (`docs/plans/2026-09-11-database-role-split.md`), not relaxed. This
        // asserted `/food_app` while the runner and the service shared one login — which is why the service owned
        // every table. The runner now connects as `food_migrator`; `food_app` is the service's, granted in the
        // service stack, and a schema stack that still granted it would hand the runner a second identity.
