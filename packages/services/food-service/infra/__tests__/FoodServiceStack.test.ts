...
function synthFoodTemplate(stage: string, baseStage: string, alarmsEnabled = true): Template {
    const app = testApp({ context: { ...VPC_LOOKUP_CONTEXT } });
...
const FOOD_METRIC = {
    ...
    failedBacklog: 'food-failed-backlog',
...
describe('Food worker + service wiring', () => {
    ...
    it('⛔ injects NO RECIPE_SERVICE_URL — this service does not call recipe (ADR-0038)', () => {
        // ⚠️ THIS ASSERTION IS INVERTED, deliberately. It used to require the variable, because the
        // authored-food delete asked recipe whether any recipe referenced the food before honouring it.
        // The owner ruled that edge away — "the recipe service needs to handle when it detects that a food
        // item has been deleted; the food service should not be updating recipes" — so the delete is now a
        // local status flip and nothing here has any business knowing recipe's origin.
        //
        // ⛔ The STACK is the right place to assert it. `@kitchensink/recipe-service-client` leaving
        // food's `package.json` proves the code cannot call recipe; this proves the deployed task is not
        // even TOLD where recipe lives, which is what stops the edge being rebuilt by someone wiring a
        // fetch to an env var that happened to still be there.
        ...
        expect(withRecipeUrl).toStrictEqual([]);
    });
...
describe('In-VPC migration-runner Lambda (T-191) — in its own schema stack', () => {
    ...
    it('grants the migration function rds-db:connect on the food_MIGRATOR db-user, never the service login', () => {
        // ⚠️ REWRITTEN for the role split (`docs/plans/2026-09-11-database-role-split.md`), not relaxed. A runner
        // sharing the service's login makes the service own every table. The runner connects as `food_migrator`;
        // `food_app` is the service's, granted in the service stack, and a schema stack that granted it would
        // hand the runner a second identity.
        const json = JSON.stringify(schemaTemplate.toJSON());

        expect(json).toContain('rds-db:connect');
        expect(json).toContain('/food_migrator');
        expect(json).not.toContain('/food_app');
    ...
    it('publishes the migration function name for the deploy-time lambda invoke', () => {
        ...
        // `runMigrations.sh run`, resolves the runner through `describe-stacks --query
...
describe('Change-refresh Fargate scheduled task (T-001c)', () => {
    ...
    /**
     * ⚠️ The command gained `--import ./dist/src/instrument.js` in plan U17. `@sentry/nestjs` instruments
     * modules by patching them as they LOAD, so anything imported before Sentry is never instrumented —
     * importing it first inside `main.ts` still loses everything the framework loaded on the way in. The
     * entrypoint the case is about is unchanged and is still asserted; the loader in front of it is the
     * change.
     */
    it('defines a task definition running the change-refresh entrypoint, with Sentry loaded first', () => {
        serviceTemplate.hasResourceProperties('AWS::ECS::TaskDefinition', {
            ContainerDefinitions: Match.arrayWith([
                Match.objectLike({
                    Command: ['node', '--import', './dist/src/instrument.js', 'dist/src/worker/change-refresh/main.js'],
                }),
            ]),
        });
    });
...
describe('Observability — dashboard, alarms, SNS (T-182/T-183)', () => {
    ...
    /**
     * ⛔ THE ONE ALARM THIS SERVICE DID NOT HAVE, AND THE 14-DAY OUTAGE IT MISSED.
     *
     * `kitchensink-food-service-prod` ran `desiredCount > 0, runningCount = 0` from 2026-09-05 until
     * 2026-09-19 and nothing in this stack could say so. Every alarm above reads an EMF series the WORKER
     * publishes, under `treatMissingData: NOT_BREACHING` — so a service that never starts publishes nothing
     * and every one of them reports a confident `OK`. The five alarms were all in `OK` for the whole outage.
     *
     * ⛔ AND THE SIGNAL HAD TO BE CHOSEN, not assumed. Measured on this account's own target group:
     * `AWS/ApplicationELB HealthyHostCount` returned **zero datapoints** for every hour between 2026-09-06
     * and 2026-09-18 — not zero-valued datapoints, NO datapoints, because the ALB reports the metric only
     * while the target group is non-empty. So `TreatMissingData: breaching` is not a defensive default here,
     * it is the entire mechanism; `notBreaching` or `missing` would have produced the same silent `OK` the
     * EMF alarms did. The two obvious alternatives were rejected on measurement, not taste:
     * `ECS/ContainerInsights RunningTaskCount` does not exist (Container Insights is OFF on both prod
     * clusters — `settings: []` — and enabling it costs several times this account's entire alarm bill), and
     * ALB `HTTPCode_ELB_5XX_Count` is LoadBalancer-dimensioned on a SHARED ALB (ADR-0003), so it cannot say
     * WHICH service is down.
     *
     * ⚠️ It selects the TARGET GROUP dimensions, which are per-stage by construction — unlike the
     * `Commise/Food` alarms above, whose series carry no `Stage` at all (verified against CloudWatch:
     * `list-metrics` returns `Dimensions: []`), so prod's copies read a series every stage writes into.
     * That is pre-existing and deliberately NOT changed here; this alarm simply must not join it.
     */
    it('has a healthy-host alarm treating missing data as breaching (the detector the outage lacked)', () => {
    ...
    /**
     * ⛔ THE HEALTHY-HOST ALARM ABOVE CANNOT SEE THIS SERVICE'S OTHER WORKLOAD, AND IT IS BROKEN RIGHT NOW.
     *
     * `FoodChangeRefresh` is not an ECS *service* — it is a task definition an EventBridge rule runs every
     * six hours. It has no ALB, no target group and no `HealthyHostCount`, so nothing above can observe it,
     * and it emits its EMF metrics only once it is RUNNING, so every `notBreaching` alarm reads a task that
     * never starts as healthy.
     *
     * Measured in production on 2026-09-19, the day the outage was "resolved": CloudFormation drift on
     * `kitchensink-food-service-prod` reports `FoodChangeRefreshLogGroup462CB492` as **DELETED** — three of
     * the four log groups were recreated by hand during recovery and this one was not — and the most recent
     * scheduled task stopped with `stopCode: TaskFailedToStart` and
     * `ResourceInitializationError … ResourceNotFoundException: The specified log group does not exist`.
     * The catalog has not refreshed since, and no alarm in this account says so.
     *
     * ⛔ SO THIS IS AN EVENT RULE, NOT A METRIC ALARM, and the choice is about what CAN be observed. A task
     * that fails to start publishes no metric of any kind; the only artefact it produces is an ECS Task
     * State Change EVENT. Matching `stopCode: TaskFailedToStart` on this cluster catches EVERY such failure
     * — scheduled task, service task, a missing secret, an unpullable image — which is the whole fault
     * CLASS the September outage belonged to, rather than one more symptom of it. It also costs no
     * CloudWatch alarm slot.
     */
    it('publishes every TaskFailedToStart on this cluster to the alarm topic', () => {
        serviceTemplate.hasResourceProperties('AWS::Events::Rule', {
            EventPattern: {
                source: ['aws.ecs'],
    ...
    it('scopes that rule to THIS stage’s cluster — never every cluster in the account', () => {
        const rules = Object.values(serviceTemplate.findResources('AWS::Events::Rule')).filter(
            (rule) => (rule.Properties?.EventPattern as { detail?: Record<string, unknown> })?.detail?.['stopCode'],
        );

        expect(rules).toHaveLength(1);
        // ⛔ The account also hosts an unrelated production application. An unscoped pattern would page on
        // its task failures, which is both wrong and the fastest way to get this rule muted.
        expect(JSON.stringify(rules[0]?.Properties?.EventPattern)).toContain('clusterArn');
    });

    it('creates exactly the six food alarms with the correct thresholds', () => {
        ...
        // count would bury it among the outcomes it does not resemble. Six since the healthy-host detector
        // above, which is the only one of them that can observe the service failing to RUN.
        serviceTemplate.resourceCountIs('AWS::CloudWatch::Alarm', 6);
        ...
        // ⛔ FAILED tombstones only, per REQ-NF-016 ("SHALL fire on FAILED tombstones only"). The alarm read
        // `food-tombstone-count`, which counts `fetch_queue` rows in `tombstone` — a population DSN-9 says is
        // NORMAL for `NOT_FOUND` ("no wired source has this food"). Tombstones persist, so one ordinary
        // NOT_FOUND left the alarm permanently lit and therefore permanently ignorable. The FAILED series is
        // the one that means a food is blackholed until an operator requeues it.
        serviceTemplate.hasResourceProperties('AWS::CloudWatch::Alarm', {
            Namespace: FOOD_METRIC_NAMESPACE,
            MetricName: FOOD_METRIC.failedBacklog,
            Threshold: 0,
            ComparisonOperator: 'GreaterThanThreshold',
        });

        // ...and no alarm watches the all-tombstone series any more. The metric itself stays: the dashboard
        // reads it, and a dashboard is not an alarm slot.
        const alarms = Object.values(serviceTemplate.findResources('AWS::CloudWatch::Alarm'));
        expect(alarms.filter((alarm) => alarm.Properties?.MetricName === FOOD_METRIC.tombstoneCount)).toEqual([]);
        ...
        // ⛔ Oldest-pending age: five minutes of NOT BEING DRAINED, ON TOP OF one full lease recovery.
        //
        // The threshold was a flat 300 and that stopped working when the lease became the DERIVED 250s
        // (U5/R16). A hard-killed worker's row is reverted by the reaper only after its lease lapses, and
        // it re-enters the pending set carrying its ORIGINAL `first_requested` — so a single recovered
        // crash now presents an age of ~250s before the queue has done anything wrong, leaving 50 seconds
        // between normal crash recovery and a page. Deriving the threshold keeps the alarm's meaning
        // ("demand is not being drained") instead of making it a crash detector, and keeps the two numbers
        // from drifting apart again: lengthen the lease and the threshold follows.
        //
        // ⚠️ Read as TEXT, never imported: `infra/tsconfig.json` sets `rootDir: infra`, the same boundary
        // that makes the stack keep its own `FOOD_METRIC` copy. The lease is taken from the ENV SCHEMA's default, which is the value the running container
        // actually gets, and which `src/config/__tests__/env.schema.test.ts` already holds to
        // `leaseWindow.ts`'s derivation. Re-deriving the fan-out arithmetic here would restate USDA's page
        // size and request timeout a third time; reading the one number downstream of them does not.
        const schemaSource = readFileSync(
            resolve(fileURLToPath(import.meta.url), '../../../src/config/env.schema.ts'),
            'utf8',
        );
        const leaseSeconds = Number(/FOOD_LEASE_TIMEOUT_SECONDS: z[^\n]*\.default\((\d+)\)/u.exec(schemaSource)?.[1]);

        expect(Number.isInteger(leaseSeconds)).toBe(true);
        expect(leaseSeconds).toBeGreaterThan(30);
        serviceTemplate.hasResourceProperties('AWS::CloudWatch::Alarm', {
            Namespace: FOOD_METRIC_NAMESPACE,
            MetricName: FOOD_METRIC.pendingAgeSeconds,
            Threshold: leaseSeconds + 300,
        });

        // ...and the threshold must actually clear one whole lease, which a flat 300 no longer did.
        const pendingAgeAlarms = Object.values(serviceTemplate.findResources('AWS::CloudWatch::Alarm')).filter(
            (alarm) => alarm.Properties?.MetricName === FOOD_METRIC.pendingAgeSeconds,
        );
        expect(pendingAgeAlarms).toHaveLength(1);
        expect(pendingAgeAlarms[0]?.Properties?.Threshold).toBeGreaterThan(leaseSeconds);
...
/**
 * This stack no longer OWNS a priority resolver — allocation lives once in `@radicle-co/infra-shared/alb`, whose
 ...
 * with its persistent TIER and sandbox-deploy never runs it with `stage=pr-{N}` — and if it somehow were, it
...
describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Six alarms per open pull request: a retry-budget exhaustion detector, a tombstone counter, an
        // API 5xx error-rate expression, a queue-depth backpressure threshold, a pending-age freshness
        // threshold and a healthy-host detector — every one of them watching a catalog nobody is populating.
        //
        // ⚠️ The healthy-host alarm is gated with the rest ON PURPOSE, and the consequence is stated rather
        // than discovered: its `TreatMissingData: breaching` cannot distinguish "the service failed to
        // start" from "the service was deliberately scaled to zero", and ADR-0007's nightly sandbox shutdown
        // zeroes `desiredCount` from 00:00 to 09:00 ET. A stage that turns this flag on while the scheduler
        // is switching it off will page every night. Prod is not scheduled, so prod is not affected.
        synthFoodTemplate('pr-91', 'sandbox', false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    });

    it('creates all six food alarms when the flag is on', () => {
        synthFoodTemplate('pr-91', 'sandbox', true).resourceCountIs('AWS::CloudWatch::Alarm', 6);
    });

    it('keeps the alarm topic AND the dashboard in both states', () => {
        // ⚠️ The dashboard is deliberately NOT gated by this change. The instruction was alarms, the
        // dashboard is a different line item ($3/month above the first three, not a free-tier alarm slot),
        // and it reads the same EMF metrics whether or not anything is alarming on them. It is called out
        // here rather than left silent so the next reader knows it was decided, not missed.
        for (const flag of [false, true]) {
            const template = synthFoodTemplate('pr-91', 'sandbox', flag);
...
/**
 * ⛔ THE LOG DRAINS ARE CONDITIONAL, AND BOTH POSTURES ARE THE ASSERTION (ADR-0042).
 *
 * The forwarder's ARN used to arrive as `Fn.importValue`, which deadlocked these stacks against the one that
 ...
 * LogForwarderArn found". The ARN is now a CI-resolved input and the filters are skipped when it is absent.
 *
 * ⚠️ THE SOURCE TEXT CANNOT PROVE THIS. Wrapping a filter in an `if` leaves `new
 ...
 * here because these stacks' `aws-cdk-lib` resolves only from this package's own install.
 *
 ...
 */
describe('the log drains are attached only when CI resolved a forwarder ARN', () => {
    const ARN = 'arn:aws:lambda:us-east-1:123456789012:function:kitchensink-log-forwarder-sandbox';

    it.each([
        ['known', 1, ARN],
        ['absent', 0, undefined],
    ])('⛔ FoodSchemaStack: a %s ARN yields %i subscription filter(s)', (_posture, expected, arn) => {
        const template = Template.fromStack(
            new FoodSchemaStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'FoodSchemaStackDrain', {
                env: { account: '123456789012', region: 'us-east-1' },
                stage: 'sandbox',
                baseStage: 'sandbox',
                vpcId: 'vpc-12345678',
    ...
    it.each([
        ['known', 3, ARN],
        ['absent', 0, undefined],
    ])('⛔ FoodServiceStack: a %s ARN yields %i subscription filter(s)', (_posture, expected, arn) => {
        const template = Template.fromStack(
            new FoodServiceStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'FoodServiceStackDrain', {
                env: { account: '123456789012', region: 'us-east-1' },
                stage: 'sandbox',
                baseStage: 'sandbox',
                vpcId: 'vpc-12345678',
