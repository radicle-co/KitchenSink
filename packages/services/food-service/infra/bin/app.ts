...
// was declared perfectly. Every other consumer of this stack — the liveness probe, `runMigrations.sh`,
...
// violation it is not: `deployGate.sh stacks-for`, `prodDeployMigrationOrder` and `stackProbeCoverage`
...
new FoodServiceStack(app, `FoodService-${stage}`, {
    ...
    // ⚠️ `baseStage`, NOT `stage`: every `pr-{N}` preview shares ONE key and ONE quota while each counts only
    // its own calls. Reading it as per-stage is wrong in the REASSURING direction. See the OPEN entry in
