 ...
 *
 * ```sh
 * npx tsx packages/services/food-service/infra/bin/printFoodHost.ts pr-73 commise.app
 ...
 * actually deployed — that is the caller's business (in CI the food and recipe deploys run CONCURRENTLY, and
 * the wiring between them is asserted afterwards by the `ecosystem-smoke` join — ADR-0036). A stage with no
 ...
 */
import { foodServiceOriginForStage } from '../lib/FoodServiceStack.js';
...
if (!stage || !domainName) {
    process.stderr.write(
        'usage: printFoodHost.ts <stage> <domainName>   (or set STAGE and DOMAIN_NAME)\n' +
            'example: printFoodHost.ts pr-73 commise.app\n',
    );
    process.exitCode = 1;
} else {
