...
} from '@radicle-co/infra-shared/messaging';
// ⚠️ From the WORKSPACE messaging package, not the CDK one. The table's shape is shared with the integration
// tier, which cannot import anything from the CDK side of the boundary — see messageTableShape.ts.
import {
 ...
 * `src` (TS6059).
 *
 * ⚠️ NO CDK TEST CROSS-CHECKS THIS AGAINST THE SOURCE CONSTANTS.
 ...
 */
/**
 * The worker lease window in seconds — a copy of `FOOD_LEASE_FLOOR_SECONDS` (`src/worker/leaseWindow.ts`,
 * which is also the default of `FOOD_LEASE_TIMEOUT_SECONDS`), for the same `rootDir` reason as
 * {@link FOOD_METRIC} above. `FoodServiceStack.test.ts` reads that module's SOURCE and derives the value,
 * so the copy cannot drift silently.
 */
const FOOD_LEASE_FLOOR_SECONDS = 250;

/**
 * Seconds of genuinely un-drained demand, on TOP of one lease window, before the freshness alarm fires.
 * This is the "5 minutes" the alarm has always meant; the lease term in front of it is what stops an
 * ordinary crash recovery counting as five minutes of neglect.
 */
const PENDING_AGE_GRACE_SECONDS = 300;
...
export function foodDatabaseNameForStage(stage: string, baseStage: string, importedBaseName: string): string {
    ...
    const suffix = stage
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
        // Single-character and anchored, matching `recipeDatabaseNameForStage` in `@kitchensink/recipe-core`,
        // which owns this rule and states the reasoning: the collapse above turns every maximal run of
        // non-alphanumerics into exactly one `_`, so `+` could never consume a second character, while the
        // `_+$` form it replaces is quadratic on a long underscore run (CodeQL `js/polynomial-redos`).
        // ⚠️ That copy got the fix and this one did not, which is the drift this comment exists to end; the
        // two cannot share a module until a home exists that both a workspace package and a CDK app outside
        // the workspace can import.
        .replace(/^_/, '')
        .replace(/_$/, '');
...
export class FoodServiceStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: FoodServiceStackProps) {
        ...
        // with its persistent TIER (`sandbox`/`production`) and sandbox-deploy never runs it with
        // `stage=pr-{N}` — a per-PR table added there would never be created, and if it were it would carry
        // a tag no per-PR teardown can claim, leaking a table per closed PR forever. This stack is already
        // deployed per-PR and already tagged `Environment=pr-{N}-sandbox`, so a table created here is torn down
        ...
        const messageTable = ownsPerPrSubstrate
            ? new dynamodb.Table(this, 'FoodMessageTable', {
                  ...
                  // ⛔ The shape is declared ONCE, in `@kitchensink/messaging`. The integration tier
                  // stands up a local twin of this table and used to derive it by synthesizing this very
                  // stack — impossible now that a CDK app owns its own aws-cdk-lib — so both sides read the
                  // same constants, and the stack test asserts the synthesized table still matches them.
                  partitionKey: { name: MESSAGE_TABLE_PARTITION_KEY, type: dynamodb.AttributeType.STRING },
                  sortKey: { name: MESSAGE_TABLE_SORT_KEY, type: dynamodb.AttributeType.STRING },
                  ...
                  timeToLiveAttribute: MESSAGE_TABLE_TTL_ATTRIBUTE,
        ...
        // ── Sentry, for the API, the worker and the change-refresh task (plan U17/U22) ──
        //
        // ⛔ Per-STAGE parameter at the BASE stage: a `pr-{N}` preview has no Sentry project of its own and
        // resolves sandbox's, separating itself by the `environment` tag. That is what keeps previews from
        // costing a project each.
        //
        // ⚠️ `SENTRY_DSN` has been DECLARED in `config/env.schema.ts` all along with nothing reading it and
        // nothing supplying it — an optional variable that was never set, so the schema documented an
        // intention rather than a wiring. This is the line that makes it true.
        const sentryEnvironment: Record<string, string> = {
            SENTRY_DSN: ssm.StringParameter.valueForStringParameter(
                this,
                `/kitchensink/${baseStage}/sentry/food-service-dsn`,
            ),
        ...
        const foodDbEnvironment: Record<string, string> = {
            ...
            DB_USERNAME: DATABASE_ROLES.food.app,
            ...
            // ⛔ Clerk auth env — the ONE definition lives in `@radicle-co/infra-shared/security`
        ...
        database.grantConnect(apiTaskRole, DATABASE_ROLES.food.app);
        ...
        // @radicle-co/infra-shared/security.
        ...
        apiTaskDefinition.addContainer('FoodApiContainer', {
            ...
            // ⛔ `--import` loads Sentry BEFORE the app (plan U17). `@sentry/nestjs` instruments modules by
            // patching them as they load, so anything imported first is never instrumented.
            command: ['node', '--import', './dist/src/instrument.js', 'dist/src/main.js'],
        ...
        database.grantConnect(workerTaskRole, DATABASE_ROLES.food.app);
        ...
        workerTaskDefinition.addContainer('FoodWorkerContainer', {
            ...
            // ⛔ `--import` loads Sentry BEFORE the app (plan U17). `@sentry/nestjs` instruments modules by
            // patching them as they load, so anything imported first is never instrumented.
            command: ['node', '--import', './dist/src/instrument.js', 'dist/src/worker/main.js'],
        });
        ...
        database.grantConnect(changeRefreshTaskRole, DATABASE_ROLES.food.app);
        ...
        if (logForwarder !== undefined) {
            new logs.SubscriptionFilter(this, 'FoodApiLogDrain', {
            ...
            new logs.SubscriptionFilter(this, 'FoodWorkerLogDrain', {
            ...
            new logs.SubscriptionFilter(this, 'FoodChangeRefreshLogDrain', {
                logGroup: changeRefreshLogGroup,
                destination: new logsDestinations.LambdaDestination(logForwarder, { addPermissions: false }),
                filterPattern: logs.FilterPattern.literal('-START -END -REPORT -"_aws"'),
                filterName: 'forward-app-logs',
            });
        ...
        // @radicle-co/infra-shared/alb, never spelled here: the rule condition below, the A-record below, and
        ...
        // single HTTPS listener, so they are allocated by @radicle-co/infra-shared/alb and NOT restated here — the
        ...
        //
        // ⚠️ `emfMetric` stays OUTSIDE the gate on purpose: the dashboard below reads the same EMF series,
        // and a dashboard is a different line item from an alarm slot. Only the alarms move.
        if (props.alarmsEnabled) {
            ...
            // ⛔ FAILED TOMBSTONES ONLY — REQ-NF-016 says this alarm "SHALL fire on `FAILED` tombstones
            // only", and until now it read `food-tombstone-count`, which counts every `fetch_queue` row in
            // `tombstone`. DSN-9 calls the `NOT_FOUND` tombstone a NORMAL outcome ("no wired source has this
            // food"), and tombstones PERSIST — so a single ordinary NOT_FOUND left this alarm lit forever,
            // which is an alarm nobody can act on and everybody learns to ignore. The FAILED series is the
            // population that means a food is blackholed until an operator requeues it.
            //
            // ⚠️ It reads the BACKLOG series (`food` rows in `FAILED`), not a queue-row count: the two are the
            // same population by construction — a food is tombstoned `FAILED` only after the retry budget is
            // exhausted — and the backlog query already computed it, so this adds a metric rather than a
            // second source of truth. `FoodRetryBudgetExhaustedAlarm` above stays: that one is the EVENT (a
            // food exhausted its budget just now), this one is the STANDING STATE (foods still blackholed).
            const failedTombstoneAlarm = new cloudwatch.Alarm(this, 'FoodTombstoneAlarm', {
                metric: emfMetric(FOOD_METRIC.failedBacklog, 'max'),
                threshold: 0,
                comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
                evaluationPeriods: 1,
                datapointsToAlarm: 1,
                treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
                alarmDescription:
                    'One or more foods are tombstoned FAILED: blackholed for every user until an operator ' +
                    'requeues them (POST /api/v1/foods/admin/foods/{id}/requeue). REQ-NF-016.',
            });
            failedTombstoneAlarm.addAlarmAction(alarmAction);
            ...
            // Freshness: the oldest pending row is not being drained — five minutes ON TOP OF one full
            // lease recovery.
            //
            // ⛔ DERIVED, not the flat 300 it used to be. A hard-killed worker's row is reverted by the
            // reaper only once its lease lapses, and it re-enters the pending set carrying its ORIGINAL
            // `first_requested` — so under U5's derived lease a single RECOVERED crash presents an age of
            // one whole lease window before the queue has done anything wrong. At a flat 300 against a 250
            // second lease that left fifty seconds between ordinary crash recovery and a page, which turns
            // a freshness alarm into a crash detector and teaches its reader to ignore it. Deriving the
            // threshold keeps the two numbers from drifting apart again: lengthen the lease and this
            // follows.
            // ── the detector this stack did not have, and the 14-day outage that proved it ────────────
            //
            // ⛔ EVERY ALARM ABOVE IS BLIND TO A SERVICE THAT NEVER STARTS. They read EMF series the API and
            // the worker PUBLISH, under `treatMissingData: NOT_BREACHING` — so a task that cannot start
            // publishes nothing and all five report a confident `OK`. `kitchensink-food-service-prod` ran
            // `desiredCount > 0, runningCount = 0` from 2026-09-05 to 2026-09-19 (its task execution role
            // could not create a log stream: the log group had been deleted, and
            // `ResourceInitializationError … The specified log group does not exist` fails a task before any
            // of its metrics exist) and this stack said nothing for fourteen days.
            //
            // ⛔ `TreatMissingData.BREACHING` IS THE WHOLE MECHANISM, not a defensive default. Measured on
            // this account: `HealthyHostCount` on the identity target group returned ZERO DATAPOINTS for
            // every hour of the outage — the ALB reports the metric only while the target group is
            // non-empty, so there is no `0` to compare against and every other treatment yields `OK`. This
            // is the same shape `IdentityServiceStack`'s crash-loop alarm already had, and it is the one
            // alarm in this account that actually detected the outage (`OK -> ALARM` three minutes after
            // the last task died).
            //
            // ⚠️ Rejected, on measurement rather than taste: `ECS/ContainerInsights RunningTaskCount` vs
            // `DesiredTaskCount` would be the most direct statement of "should be running and is not", but
            // Container Insights is DISABLED on both prod clusters and enabling it bills per published
            // metric — several times this account's entire alarm spend — for a signal this one already
            // carries. ALB `HTTPCode_ELB_5XX_Count` (the 503 a target-less ALB returns) is
            // LoadBalancer-dimensioned on a SHARED ALB (ADR-0003), so it cannot name the service that is
            // down; the target-group 5xx the error-rate alarm reads needs a target to produce the 5xx.
            //
            // ⚠️ THREE MINUTES, deliberately: one minute would page on an ordinary rolling deploy's gap.
            // ⚠️ It selects the TARGET GROUP's dimensions, which are per-stage by construction. The
            // `Commise/Food` alarms above select NO dimension because their emitter attaches none — so
            // prod's copies read a series sandbox and every `pr-{N}` also write into. That is pre-existing,
            // is NOT changed here, and is the reason this alarm must not be modelled on them.
            //
            // ACTION WHEN IT FIRES: `aws ecs describe-services` for this stage's cluster. `runningCount = 0`
            // with `desiredCount > 0` means tasks cannot start — read `describe-tasks`' `stoppedReason`
            // (`ResourceInitializationError` names a missing log group or secret; `CannotPullContainerError`
            // names the image). `runningCount > 0` with no healthy host means the container boots and fails
            // its health check: read this stage's container log group.
            const healthyHostAlarm = new cloudwatch.Alarm(this, 'FoodApiHealthyHostAlarm', {
                ...
                alarmDescription:
                    'The food API has no healthy target for three minutes: either its tasks cannot start ' +
                    '(check describe-tasks stoppedReason — a deleted log group fails a task before it runs) ' +
                    'or the container boots and fails its health check.',
            });
            healthyHostAlarm.addAlarmAction(alarmAction);

            // ⛔ AND THE WORKLOAD THE ALARM ABOVE CANNOT SEE. `FoodChangeRefresh` is not a SERVICE — it is a
            // task definition an EventBridge rule runs every six hours. No ALB, no target group, no
            // `HealthyHostCount`; and its EMF metrics only exist once it is RUNNING, so a task that never
            // starts reads as healthy to every `notBreaching` alarm above. Measured in production on
            // 2026-09-19, the day the outage was declared resolved: CloudFormation drift reports
            // `FoodChangeRefreshLogGroup462CB492` as DELETED — three of the four log groups were recreated
            // by hand during recovery and this one was missed — and the latest scheduled task stopped with
            // `stopCode: TaskFailedToStart` and `ResourceInitializationError … ResourceNotFoundException:
            // The specified log group does not exist`. The catalog has not refreshed since, and nothing in
            // this account said so.
            //
            // ⛔ AN EVENT RULE, NOT A METRIC ALARM, and the choice is about what CAN be observed. A task that
            // fails to start publishes no metric of any kind — that is the property that made the whole
            // September outage invisible — so the only artefact is the ECS Task State Change EVENT. This
            // catches the fault CLASS rather than one symptom of it: a deleted log group, an unreadable
            // secret, an unpullable image and an exhausted ENI budget all arrive here. It also consumes no
            // CloudWatch alarm slot.
            //
            // ⚠️ SCOPED TO THIS STAGE'S CLUSTER. The account hosts an unrelated production application, and
            // an unscoped `aws.ecs` pattern would page on its task failures — wrong, and the fastest route
            // to a muted rule.
            //
            // ⚠️ Gated with the alarms rather than always-on, for one reason: this is detection, and the
            // flag is what decides which stages detect. A `pr-{N}` preview would otherwise publish its own
            // routine start failures to a topic nobody reads.
            //
            // ACTION WHEN IT FIRES: the message body is the raw ECS event — read `detail.stoppedReason`.
            // `ResourceInitializationError … log group does not exist` means the task's log group is gone:
            // recreate it with the EXACT name in the task definition's `awslogs-group` (a CDK deploy of
            // this stack also recreates it, because CloudFormation still believes it owns it). Never
            // resolve it by deleting anything.
            const taskStartFailureRule = new events.Rule(this, 'FoodTaskFailedToStartRule', {
                description:
                    'An ECS task in this stage could not start. Read detail.stoppedReason — a missing log ' +
                    'group, an unreadable secret or an unpullable image all stop a task before it runs, ' +
                    'and before any of its metrics exist.',
                eventPattern: {
                    source: ['aws.ecs'],
            ...
            taskStartFailureRule.addTarget(new targets.SnsTopic(alarmTopic));

            const pendingAgeAlarm = new cloudwatch.Alarm(this, 'FoodPendingAgeAlarm', {
                metric: emfMetric(FOOD_METRIC.pendingAgeSeconds, 'max'),
                threshold: FOOD_LEASE_FLOOR_SECONDS + PENDING_AGE_GRACE_SECONDS,
                comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_THRESHOLD,
                evaluationPeriods: 1,
                datapointsToAlarm: 1,
                treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
                alarmDescription: 'Oldest pending fetch_queue row un-drained for 5 minutes beyond one lease',
            });
