...
export interface AuthenticatedPrincipal {
    ...
    /**
     * Whether the verified token marks a member of the fixed Clerk TEST POOL (ADR-0040) — read by
     * `@kitchensink/clerk-verify` ONLY as `public_metadata.testPrincipal === true`, never from a header or an
     * unsigned claim. REQUIRED, so every construction of a principal decides it. Its one consumer here is the
     * authored-food test purge (`foods/domain/testPurgePolicy.ts`); nothing a normal user does reads it.
     */
...
/**
 * Whether a verified `sub` names a service principal (M2M / internal job) rather than a person. Pure.
 *
 * @param sub - The verified Clerk `sub`.
 * @returns `true` for a `svc_*` principal.
 */
export function isServicePrincipalSub(sub: string): boolean {
    return sub.startsWith(SERVICE_PRINCIPAL_PREFIX);
}
 ...
 */
export function resolveRequesterId(principal: Pick<AuthenticatedPrincipal, 'sub' | 'userId'>): RequesterKeyResolution {
    if (isServicePrincipalSub(principal.sub)) {
