 ...
 * - **Identity from the verified token only.** `sub`/`azp`/`scopes`/`permissions`/`testPrincipal` come from the
 *   validated JWT and are attached to `req.user`; nothing else is trusted.
...
import { AuthLoadShedder, type AuthLoadShedderOptions } from './AuthLoadShedder.js';
...
export class FoodAuthGuard implements NestMiddleware {
    ...
    public async use(req: AuthenticatedRequest, _res: Response, next: NextFunction): Promise<void> {
        ...
        req.user = {
            ...
            // ADR-0040: the signed test-pool marker, forwarded verbatim. Only the authored-food test purge reads it.
