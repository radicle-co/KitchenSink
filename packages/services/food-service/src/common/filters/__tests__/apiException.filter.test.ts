...
describe('ApiExceptionFilter (food)', () => {
    ...
    describe('structured logging (T-151)', () => {
        ...
        it("records the throwable's CAUSE chain in `stack`, not just the outer error's", () => {
            // drizzle wraps a driver failure as `cause`; `Error.prototype.stack` omits it, which is how the
            // 2026-09-11 k6 500s logged "Failed query" with nothing under it.
            ...
            expect(records).toHaveLength(1);
            expect(records[0].context['stack']).toContain('Failed query: select 1');
            expect(records[0].context['stack']).toContain('Connection terminated unexpectedly');
            expect(records[0].context['stack']).toContain('57P01');
        });
