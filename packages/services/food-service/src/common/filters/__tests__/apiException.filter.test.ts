...
// ⛔ `getClient` AND `logger` ARE PART OF THE FAKE NOW. The filter's default `WorkerLogger` routes through
// `@kitchensink/service-logging`, which asks the SDK whether a client exists before choosing a destination —
// so a fake declaring only `captureException` no longer describes the module the code under test uses, and
// the sink threw, which the filter correctly reported as `api-error-log-failed`. `getClient` returns
// `undefined` because the stdout branch is what the structured-logging cases below assert; the U17 cases
// assert `captureException`, which is a different product (an ISSUE, not a log) and is unaffected.
...
describe('ApiExceptionFilter (food)', () => {
     ...
     */
    it('does NOT classify a DAO IllegalStatusTransitionError — it stays a 500, logged at error with the throwable', () => {
        const { host, captured } = makeHost();
        ...
        // The throwable itself, not a pre-rendered string: the sink renders it (with its cause chain) and
        // scrubs what it renders, which pre-rendering here skipped.
        expect((records[0]?.context['error'] as Error | undefined)?.name).toBe('IllegalStatusTransitionError');
    });
    ...
    describe('structured logging (T-151)', () => {
        it('logs an unclassified throwable at error with its name, message, throwable, and the request', () => {
            const { host, captured } = makeHost({
                method: 'POST',
            ...
            // The throwable goes over whole and the SINK renders it; `service-logging`'s `logAttributes`
            // suite owns the assertion that the render carries the stack and the cause chain, and its
            // `logSink` suite owns the assertion that the result is scrubbed.
            expect(records[0].context['error']).toBeInstanceOf(TypeError);
            expect((records[0].context['error'] as Error).stack).toContain('apiException.filter.test');
        });

        /**
         * ⛔ REWRITTEN, not edited to compile, and the behaviour it proves has CHANGED. It used to assert
         * that this filter put `renderThrowable(exception)`'s output in a `stack` field. That was the right
         * renderer in the wrong place: a string that arrives at the sink already rendered is free text the
         * sink did not produce, and `@kitchensink/service-logging` scrubs what it renders — so pre-rendering
         * here skipped the scrub and sent an unredacted `util.inspect` dump onto a stream ADR-0042's drain
         * forwards off-host.
         *
         * What this case now proves is the filter's half: the throwable is handed over WHOLE, with its
         * `cause` still attached and reachable. ⚠️ THE COVERAGE FOR THE CHAIN ITSELF MOVED rather than
         * vanished — `service-logging`'s `logAttributes.test.ts` asserts that a wrapped error renders with
         * its cause, and `logSink.test.ts` asserts the rendered text is scrubbed in place on both paths.
         */
        it('hands the throwable over WHOLE, cause attached, for the sink to render', () => {
            const { host } = makeHost({ method: 'GET', originalUrl: '/api/v1/foods/search', headers: {} });
            ...
            expect(records).toHaveLength(1);
            // The Error object itself, not a string: `.stack` alone would have dropped the driver error.
            expect(records[0].context['error']).toBe(wrapped);
            expect((records[0].context['error'] as Error).cause).toBe(driver);
        });
