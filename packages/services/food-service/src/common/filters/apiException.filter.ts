 ...
 *
 * ⚠️ `stack` is `renderThrowable`'s output, not `exception.stack`: the latter describes the outer error alone,
 * so a drizzle "Failed query" logged nothing of the Postgres error wrapped in its `cause`.
 *
 * @param exception - The thrown value.
 * @returns The name, message, and the rendered stack with its cause chain.
 */
function describeThrowable(exception: unknown): { errorName: string; errorMessage: string; stack?: string } {
    if (exception instanceof Error) {
        return { errorName: exception.name, errorMessage: exception.message, stack: renderThrowable(exception) };
    }
