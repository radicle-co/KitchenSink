 ...
 *
 * ⛔ THE THROWABLE IS HANDED OVER WHOLE, not pre-rendered. It used to pass `renderThrowable(exception)`
 * — the right RENDERER, applied in the wrong PLACE. A string that arrives already rendered is free text the
 * sink did not produce, and `@kitchensink/service-logging` owns both halves of that rule: it renders the
 * `Error` (with its `cause` chain, which `exception.stack` drops — the defect `renderThrowable` exists for)
 * AND scrubs the text that produces. Rendering here meant the scrub half was skipped.
 *
 * @param exception - The thrown value.
 * @returns The name, the message, and the throwable itself for the sink to render.
 */
function describeThrowable(exception: unknown): { errorName: string; errorMessage: string; error?: unknown } {
    if (exception instanceof Error) {
        return { errorName: exception.name, errorMessage: exception.message, error: exception };
    }
...
export class ApiExceptionFilter implements ExceptionFilter {
     ...
     *   test can inject a recorder. `RoutedWorkerLogger` resolves `console` per call, so an interception
     *   installed later in the process still sees these lines (the `emfMetrics.ts` `writeLine` lesson).
     */
    public constructor(@Optional() private readonly logger: WorkerLogger = new RoutedWorkerLogger(LOG_COMPONENT)) {}
    ...
    public catch(exception: unknown, host: ArgumentsHost): void {
        ...
        // ⛔ REPORTED, not just logged (plan U17). The line above reaches Sentry as a LOG — since U22a step
        // 2 it goes straight to this service's own project rather than via ADR-0042's drain — and a log is
        // a log either way: it does not group, it carries no stack trace Sentry can symbolicate, and
        // nothing alerts on it. A 5xx is a server fault — precisely the class that should raise an ISSUE.
        //
        // ⚠️ ONLY a 5xx. A 4xx is expected control flow — a 404 for a food that does not exist, a 429 from
        // the flood shed — and capturing those would create an issue per bad request, which is how a Sentry
        // project becomes unreadable and then ignored. The predicate is the same one the log level uses, so
        // the two cannot disagree about what counts as a failure.
        if (logLevelForStatus(resolution.status) === 'error') {
    ...
    private record(exception: unknown, resolution: NormalizedFailure, request: Request | undefined): void {
        ...
        try {
            ...
            const context: LogContext = {
                ...
                // ⚠️ ONLY on an `error`. A 4xx is expected control flow — rendering its cause chain would
                // pay for a multi-kilobyte log line per bad request that tells an operator nothing. The
                // throwable goes over WHOLE and the sink renders it; see `describeThrowable` for why the
                // rendering does not happen here.
                ...(level === 'error' && described.error !== undefined ? { error: described.error } : {}),
            };
