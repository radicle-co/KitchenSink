...
describe('EnvironmentSchema — source-agnostic operational defaults', () => {
    it('applies the documented operational defaults', () => {
        ...
        expect(env.FOOD_QUEUE_DEPTH_ALARM).toBe(10_000);
    ...
    it('rejects a non-positive rate-limit cap and a zero lease timeout (bad values)', () => {
        ...
        expect(EnvironmentSchema.safeParse({ ...VALID_ENV, FOOD_QUEUE_DEPTH_ALARM: 'lots' }).success).toBe(false);
    });
});
...
describe('settingFromEnv', () => {
    ...
    it('covers the whole declared setting surface (a new schema field is readable without new code)', () => {
        expect(SETTING_NAMES).toContain('FOOD_QUEUE_DEPTH_ALARM');
    ...
    it('names the variable AND quotes the offending value, so the operator can find the typo', () => {
        vi.stubEnv('FOOD_QUEUE_DEPTH_ALARM', 'lots');

        expect(() => settingFromEnv('FOOD_QUEUE_DEPTH_ALARM')).toThrow(/FOOD_QUEUE_DEPTH_ALARM/);
        expect(() => settingFromEnv('FOOD_QUEUE_DEPTH_ALARM')).toThrow(/lots/);
    });
    ...
    /**
     * T-199(c) — `FOOD_QUEUE_DEPTH_ALARM` is the FR-046 hard ceiling, the `503` backpressure backstop. `NaN`
     ...
     */
    describe('FOOD_QUEUE_DEPTH_ALARM (FR-046)', () => {
        it('defaults to the schema default when unset — one source of truth, not two literals', () => {
            vi.stubEnv('FOOD_QUEUE_DEPTH_ALARM', undefined);

            expect(settingFromEnv('FOOD_QUEUE_DEPTH_ALARM')).toBe(
                EnvironmentSchema.parse(VALID_ENV).FOOD_QUEUE_DEPTH_ALARM,
            );
        });

        it('returns the operator-configured value (env vars arrive as strings)', () => {
            vi.stubEnv('FOOD_QUEUE_DEPTH_ALARM', '250');

            expect(settingFromEnv('FOOD_QUEUE_DEPTH_ALARM')).toBe(250);
        });

        it.each(['lots', '', '0', '-1', '2.5', 'NaN', 'Infinity'])(
            'throws on the malformed value %o rather than yielding NaN',
            (value) => {
                vi.stubEnv('FOOD_QUEUE_DEPTH_ALARM', value);
