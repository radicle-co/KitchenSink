...
const FoodOperationalConfigSchema = z.object({
    ...
    // The `fetch_queue` depth at which the backlog is ALARMED (FR-046, as amended 2026-09-15). It is no
    // longer an admission gate: nothing fails closed on depth, because the owner ruled a deep queue is
    // drained and reported rather than closed. The value stays configurable so a stage can tune when the
    // depth alarm fires.
    FOOD_QUEUE_DEPTH_ALARM: z.coerce.number().int().positive().default(10_000),
