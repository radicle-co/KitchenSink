/**
 * The boot-time schema-currency check for the food-service.
 *
 * ## Why a boot check when the pipeline already migrates first
 *
 * `kitchensink-food-schema-{stage}` is deployed and invoked ahead of every consumer (ADR-0035), so the
...
/**
 * Check that `kitchensink_food` has applied every migration this release ships.
 *
 ...
 */
export async function verifyFoodSchemaCurrent(db: FoodDrizzle, report: (message: string) => void): Promise<void> {
