...
describe('deleteAuthored — authorization first, then one guarded transition', () => {
    ...
    it('a SECOND delete is 404 — the guarded transition matches no row, which is the honest answer', async () => {
        ...
        //
        // ⚠️ The DAO's REAL error type, never a plain `new Error('illegal transition')`: a 404 for a shape the
        // DAO never throws leaves the real one unproven, and makes a swallow-everything catch look justified.
        const { service } = makeService({
