...
import {
    ...
    withdrawnFoodStatusSchema,
    catalogFoodStatusSchema,
...
describe('terminalFoodStatusSchema', () => {
    ...
    // Together the subsets must tile the lifecycle exactly: every status either answers 202, answers 404,
    // answers 200-and-gone (`WITHDRAWN`), or is `RESOLVED`. A migration that adds a value fails HERE, forcing
    // a decision about which status code it answers with, instead of silently landing in no subset.
    //
    // ⛔ `WITHDRAWN` is its OWN arm and must not be folded into `terminalFoodStatusSchema` to make this pass.
    // Terminal means `404`, and a withdrawal is retained ON PURPOSE so a cook can be told what was removed
    // (owner ruling, 2026-09-07) — `getStatus` answers `200 { status: 'WITHDRAWN' }`, which no other arm can
    // express.
    it('partitions the lifecycle with the pending, terminal and withdrawn arms plus RESOLVED, exhaustively', () => {
        const partitioned = [
            ...pendingFoodStatusSchema.options,
            ...terminalFoodStatusSchema.options,
            ...withdrawnFoodStatusSchema.options,
            'RESOLVED',
        ];
    ...
    it('keeps the arms DISJOINT — a status answering two codes is the bug this partition exists to catch', () => {
        const partitioned = [
            ...pendingFoodStatusSchema.options,
            ...terminalFoodStatusSchema.options,
            ...withdrawnFoodStatusSchema.options,
...
/**
 * ⛔ THE WITHDRAWN ARM (owner rulings 5 + 6, 2026-09-07). A voluntary delete of an authored food is a SOFT
 * delete: the row is retained "so that we can provide information about what was deleted".
 *
 * It is deliberately NOT `terminalFoodStatusSchema` (which means `404` — the row would be unreadable, and
 * then nothing could tell a cook what happened) and deliberately NOT `DELETING` (which is IN-FLIGHT, reverts
 * to `RESOLVED` mid-erasure, and is store-internal). A read that collapsed the two would assert a terminal
 * fact about a state that is about to un-happen — the collapse ADR-0026 §3 refuses one layer down.
 */
describe('withdrawnFoodStatusSchema', () => {
    it('accepts the one status a withdrawal answers with', () => {
        expect(withdrawnFoodStatusSchema.parse('WITHDRAWN')).toBe('WITHDRAWN');
    });

    it('refuses every other lifecycle value', () => {
        for (const value of foodStatusSchema.options.filter((status) => status !== 'WITHDRAWN')) {
            expect(withdrawnFoodStatusSchema.safeParse(value).success).toBe(false);
        }
    });
});

/**
 * ⛔ THE CATALOG SUBSET — the statuses a CATALOG (`user_id IS NULL`) food can report.
 *
 * Only an AUTHORED food can be withdrawn: `deleteAuthored` is gated on `evaluateAuthorship`, which answers
 * `not-editable` for any pipeline row, and the dedup CTE behind `addByName` is `WHERE … user_id IS NULL`. So
 * `WITHDRAWN` is unreachable on the add/batch responses in BOTH directions, and this narrowing is what makes
 * it unrepresentable rather than defended by a branch nothing can execute — the same repair
 * `pendingResponseSchema` and `resolveResponseSchema` already carry.
 */
describe('catalogFoodStatusSchema', () => {
    it('refuses WITHDRAWN — a catalog row cannot be withdrawn', () => {
        expect(catalogFoodStatusSchema.safeParse('WITHDRAWN').success).toBe(false);
    });

    it('accepts every other published status', () => {
        for (const value of foodStatusSchema.options.filter((status) => status !== 'WITHDRAWN')) {
            expect(catalogFoodStatusSchema.safeParse(value).success).toBe(true);
        }
    });

    it('is what the add and batch-add responses publish, so the narrowing cannot be bypassed', () => {
        expect(addResponseSchema.safeParse({ id: 'x', status: 'WITHDRAWN' }).success).toBe(false);
        expect(addResponseSchema.safeParse({ id: 'x', status: 'RESOLVED' }).success).toBe(true);
    });
});
