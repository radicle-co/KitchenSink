...
describe('AWAITING_RETRY on the wire', () => {
    ...
    it('keeps the lifecycle PARTITIONED — every value answers exactly one status code', () => {
        // `RESOLVED` is the 200; the rest split between the 202, 404 and withdrawn sets with no overlap and
        // nothing left over. A value that landed in none would answer no code at all.
        //
        // ⛔ `WITHDRAWN` is the FOURTH class (0016), not a member of the terminal set. Terminal means 404 —
        // an unreadable row — and the withdrawn row is retained precisely so it stays readable and a cook
        // can be told which line lost its food. `getStatus` answers 200 with the status and no `food` body.
        ...
        const withdrawn = new Set<string>(withdrawnFoodStatusSchema.options);

        for (const status of foodStatusSchema.options) {
            const memberships = [
                pending.has(status),
                terminal.has(status),
                withdrawn.has(status),
                status === 'RESOLVED',
