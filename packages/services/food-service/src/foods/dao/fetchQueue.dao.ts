...
import { LeaseLostError, leaseFenceFrom, type LeaseFence, type SettleAuthority } from './leaseFence.js';

/**
 * A row this caller HOLDS, as returned by {@link FetchQueueDao.leaseNext} — the queue row plus the
 * {@link LeaseFence} its claim wrote.
 *
 * The fence is a separate, non-nullable field rather than a narrowing of `leasedAt` because it is not the
 * same fact: `leasedAt` is a parsed `Date` (millisecond-truncated, and so unusable as a fence — see
 * `leaseFence.ts`), while `fence` is the stamp as Postgres holds it. Carrying both would invite a caller
 * to reach for the wrong one, so only the claim carries the fence, and only from the claiming statement.
 */
export interface ClaimedFetchQueueRow extends FetchQueueRow {
    /** The authority every settle of this row must present (R16). */
...
export interface FetchQueueDaoOptions {
     ...
     * Defaults to the configured `FOOD_LEASE_TIMEOUT_SECONDS`, whose own default is DERIVED from the
     * worst case one claim can take (`worker/leaseWindow.ts`) rather than picked.
     *
...
export class FetchQueueDao {
     ...
     * `drainClaimRankingDifferential.integration.test.ts` re-proves that against FR-043's spec-literal
     * ranking over randomised queues, drain order for drain order, and `drainClaimScaling` asserts the
     ...
     * @returns The claimed row plus the fence every settle of it must present, or `undefined` when
     *   nothing is eligible.
     * @sideEffect Updates `fetch_queue` (status/leased_at) — including reverting lapsed leases.
     */
    public async leaseNext(leaseSeconds: number = this.leaseSeconds): Promise<ClaimedFetchQueueRow | undefined> {
        ...
        const result = await this.db.execute<{ food_id: string; fence: string }>(sql`
            ...
            RETURNING food_id, leased_at::text AS fence
        `);
        ...
        // ⛔ The fence comes from the CLAIMING STATEMENT, never from the re-read below. The re-read is a
        // second snapshot: by the time it runs, a reaper tick and another loop could already have taken
        // the row, and a fence built from that read would authorise a settle on somebody else's claim —
        // the exact failure R16 exists to prevent, reintroduced by the convenience of one query.
        const row = await this.getByFoodId(claimed.food_id);

        if (!row) {
            return undefined;
        }

        return { ...row, fence: leaseFenceFrom(claimed.fence) };
    }

    /**
     * Revert orphaned `in_flight` rows whose lease has lapsed back to `pending` (the reaper, FR-018),
     * clearing `leased_at` with them. Does NOT touch `attempts` — a reclaim is not a failure (DSN-5).
     *
     * ⚠️ The clear is what FR-018 always specified and the statement did not do, leaving a reverted row
     * carrying a stamp for a claim nobody held — the one place in the table where `leased_at` did not
     * mean "claimed", while every sibling release clears it. The fence does not DEPEND on the clear (it
     * asks for `status = 'in_flight'` as well, so a reverted row fails either way), but a column whose
     * meaning has an exception is a column the next reader will misread.
     *
     * @param leaseSeconds - Lease window in seconds (defaults to the configured `FOOD_LEASE_TIMEOUT_SECONDS`).
    ...
    public async reapExpiredLeases(leaseSeconds: number = this.leaseSeconds): Promise<number> {
        const result = await this.db.execute(sql`
            UPDATE fetch_queue SET status = 'pending', leased_at = NULL
    ...
    /**
     * The `WHERE` term that binds a settle to the claim that authorised it (R16).
     *
     * ⛔ `leased_at = fence` carries the guarantee and `status = 'in_flight'` is the belt, and the belt is
     * kept deliberately rather than by inertia. Every release path now NULLs the stamp — the reaper, a
     * recorded failure, a deferral, a shutdown release — so a released row already fails the stamp test,
     * and a re-claimed row carries a strictly later `now()` because the reap must commit before the claim
     * that follows it. What the status adds is the only case the stamp cannot rule out on its own: two
     * claims of one row landing on the same microsecond. It costs nothing and it is the cheaper half.
     *
     * @param authority - The claim's fence, or the explicit out-of-band bypass.
     * @returns A predicate fragment for the settle's `WHERE` clause.
     */
    private static fenceTerm(authority: SettleAuthority): ReturnType<typeof sql> {
        if (authority === 'out-of-band') {
            return sql`TRUE`;
        }

        return sql`status = 'in_flight' AND leased_at = ${authority}::timestamptz`;
    }

    /**
     * Refuse a settle that matched no row while holding a fence.
     *
     * ⚠️ Zero rows is NOT an error for an out-of-band settle: that caller holds no claim, and a row that
     * has already gone is the outcome it wanted. Only a fenced settle learns something from zero rows —
     * that the claim it names is no longer the one that owns this food.
     *
     * @param rowCount - Rows the settle statement matched.
     * @param foodId - The food being settled.
     * @param authority - The authority the settle presented.
     * @param operation - The settle's name, for the error.
     * @throws {LeaseLostError} when a fenced settle matched nothing.
     */
    private static refuseWhenLost(
        rowCount: number,
        foodId: string,
        authority: SettleAuthority,
        operation: string,
    ): void {
        if (rowCount === 0 && authority !== 'out-of-band') {
            throw new LeaseLostError(foodId, operation);
        }
    }
     ...
     *
     * @param foodId - Internal food id.
     * @param authority - The claim's fence (R16), or `'out-of-band'` for a caller holding no claim.
     * @throws {LeaseLostError} when the claim was reaped or reclaimed — nothing is written.
     * @sideEffect Deletes from `fetch_requesters` and `fetch_queue` (one transaction).
     */
    public async resolve(foodId: string, authority: SettleAuthority): Promise<void> {
        await this.db.transaction(async (tx) => {
            await tx.execute(sql`DELETE FROM fetch_requesters WHERE food_id = ${foodId}`);
            const result = await tx.execute(
                sql`DELETE FROM fetch_queue WHERE food_id = ${foodId} AND ${FetchQueueDao.fenceTerm(authority)}`,
            );

            // Inside the transaction on purpose: the throw rolls the requester prune back with it, so a
            // refused settle leaves the row exactly as the loop that DOES own it will find it.
            FetchQueueDao.refuseWhenLost(result.rowCount ?? 0, foodId, authority, 'resolve');
        });
    }
     ...
     * @throws {LeaseLostError} when the claim was reaped or reclaimed — the budget is NOT spent.
    ...
    public async recordFailure(
        ...
        const result = await this.db.execute(sql`
            ...
            WHERE food_id = ${foodId} AND ${FetchQueueDao.fenceTerm(authority)}
        `);

        // A stale loop must not spend the retry budget of a food another loop is actively fetching:
        // five such refusals would tombstone it FAILED while it was succeeding elsewhere.
        FetchQueueDao.refuseWhenLost(result.rowCount ?? 0, foodId, authority, 'recordFailure');
     ...
     * @param authority - The claim's fence (R16), or `'out-of-band'` for a caller holding no claim.
     * @throws {LeaseLostError} when the claim was reaped or reclaimed — the row is left alone.
     * @sideEffect Updates `fetch_queue` (status/leased_at/last_requested); `attempts` unchanged.
     */
    public async deferLease(foodId: string, seconds: number, authority: SettleAuthority): Promise<void> {
        const result = await this.db.execute(sql`
            UPDATE fetch_queue SET
                status = 'pending',
                leased_at = NULL,
        ...
        FetchQueueDao.refuseWhenLost(result.rowCount ?? 0, foodId, authority, 'deferLease');
    }
     ...
     * them at once rather than waiting out the reaper window. Does NOT touch `attempts` — a
     * shutdown is not a failure (DSN-5). Safe under the single-drainer invariant (the only `in_flight`
     * rows are this worker's).
     *
     * ⛔ UNFENCED by construction. `WorkerRuntime.stop` halts claiming and waits for the in-flight drain
     * before calling this, so in the common case the leases released are the leases of work that is over.
     *
     * ⚠️ That wait is BOUNDED, and the residual is stated rather than implied: one claim may legitimately
     * run for the whole derived lease window, far longer than the grace between SIGTERM and SIGKILL, so a
     * shutdown mid-fan-out still invalidates that claim's fence. Its settle is then refused — the work is
     * discarded, never mis-applied — and `abandonLostClaim` files it as a shutdown rather than counting it
     * against the `FoodLeaseLost` signal.
     *
     ...
     * @param authority - The claim's fence (R16), or `'out-of-band'` for a caller holding no claim.
     * @throws {LeaseLostError} when the claim was reaped or reclaimed — the food is NOT condemned.
     * @sideEffect Deletes from `fetch_requesters`; updates `fetch_queue` (one transaction).
     */
    public async tombstone(foodId: string, lastError: string | undefined, authority: SettleAuthority): Promise<void> {
        await this.db.transaction(async (tx) => {
            await tx.execute(sql`DELETE FROM fetch_requesters WHERE food_id = ${foodId}`);
            ...
            // The prune is what makes a tombstone unrecoverable by a bare `reactivate`, so a stale loop
            // reaching this must take NOTHING with it — hence the throw inside the transaction.
            FetchQueueDao.refuseWhenLost(result.rowCount ?? 0, foodId, authority, 'tombstone');
        });
    }
}
