    ...
    private relevanceQuery(query: string, callerId: string, limit: number): SQL {
        ...
        return sql`
            ...
            WHERE status = 'RESOLVED'
              ...
              -- and nothing else. An authored food is AUTHOR-ONLY (ADR-0036) — a stranger's search can
              -- never receive another user's authored row, and the predicate is that privacy boundary.
              AND (user_id IS NULL OR user_id = ${callerId})
