...
export class BulkSeedService {
    ...
    public constructor(deps: BulkSeedDeps) {
        ...
        this.logger = deps.logger ?? new RoutedWorkerLogger('food-bulk-seed');
    ...
    private async persist(
        ...
        switch (status) {
            ...
            // Same argument one status over (0016): only an AUTHORED food can be withdrawn — the write path
            // is gated on `evaluateAuthorship`, which refuses any pipeline row — and this seeder touches
            // catalog rows exclusively. Observing it is a defect, not a case to handle.
            case 'WITHDRAWN':
                throw new Error(`bulk seed observed WITHDRAWN on '${foodId}' — a catalog row cannot be withdrawn`);
