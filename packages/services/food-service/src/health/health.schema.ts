 ...
 *
 * It is the SKEW SIGNAL for drift layer 3 (§15.2.5), and it mirrors `recipe-service`'s health payload field
 * for field so the three services agree. The boot assertion in `src/contract/contract-skew.ts` compares the
 * two stamps baked into ONE image; it is structurally blind to the case the layer exists for — a deployed
 * service running ahead of a CONSUMER's pinned schema, which is the live problem for a released mobile binary
 * that cannot be updated in step with a backend deploy. That comparison is cross-process, so the service has
 * to say which contract it is serving.
 *
 * Publishing it leaks nothing: the identical value already ships inside every consumer as `CONTRACT_HASH` in
 * `@kitchensink/schema-food`. What this adds is the OTHER half — and it is on the unauthenticated probes on
...
export const healthStatusSchema = z.object({
    ...
    service: z.string(),
    /**
     * The wire-contract fingerprint this binary was built against — the same value `@kitchensink/schema-food`
     * publishes as `CONTRACT_HASH`. A consumer whose pinned copy differs is talking to a service whose
     * contract has moved, and warns (drift layer 3, §15.2.5).
     */
