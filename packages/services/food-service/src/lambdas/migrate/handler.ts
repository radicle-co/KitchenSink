...
import { applyMigrations, assertBundleMatches, DATABASE_ROLES, isManifestSha } from '@kitchensink/db-schema-guard';
import type { MigrateResult } from '@kitchensink/db-schema-guard';

import { foodPoolConfig } from '../../database/poolConfig.js';
...
export async function ensureDatabaseExists(options: EnsureDatabaseOptions): Promise<EnsureDatabaseResult> {
    ...
    try {
        ...
        await maintenancePool.query(
            `CREATE DATABASE "${databaseName}" TEMPLATE "${BASE_FOOD_DATABASE_NAME}" OWNER "${FOOD_ROLES.owner}"`,
        );
    } catch (err) {
...
export const handler = async (event: unknown = {}): Promise<MigrateResult> => {
    ...
    const withMaintenancePool = async <T>(run: (pool: pg.Pool) => Promise<T>): Promise<T> => {
        const maintenancePool = new Pool({
            ...foodPoolConfig({ host, port, database: 'postgres', username: FOOD_ROLES.migrator }),
            max: 1,
        });
    ...
    const pool = new Pool({
        ...foodPoolConfig({ host, port, database: databaseName, username: FOOD_ROLES.migrator }),
        max: 1,
    });
