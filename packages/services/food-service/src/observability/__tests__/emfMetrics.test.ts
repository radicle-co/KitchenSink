    ...
    describe('FOOD_METRIC names', () => {
        it('exposes the canonical metric-name literals (the dashboard/alarm contract)', () => {
            ...
            expect(FOOD_METRIC).toEqual({
                ...
                // ⛔ The series REQ-NF-016's alarm reads. `tombstoneCount` stays for the dashboard, but it
                // counts `NOT_FOUND` too — a normal outcome under DSN-9 — and tombstones persist, so alarming
                // on it kept the alarm permanently lit.
