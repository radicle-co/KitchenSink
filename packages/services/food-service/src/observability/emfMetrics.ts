 ...
 * `infra/lib/FoodServiceStack.ts`, and in `infra/__tests__/FoodServiceStack.test.ts` — none importing
 ...
 *
 * What DOES protect the contract is `packages/infra/global/__tests__/serviceInfraWiringInvariants.test.ts`
 ...
 * here is a NAME, not an implementation: `__tests__/emfMetrics.test.ts` measures the emitted set and pins the
...
export const FOOD_METRIC = {
    ...
    /** Foods tombstoned `FAILED` — the population REQ-NF-016's alarm watches, EXCLUDING `NOT_FOUND`. */
    ...
    /**
     * Claims whose lease was reaped or reclaimed before their result landed (R16), EXCLUDING the ones a
     * graceful shutdown causes.
     *
     * Zero is the expected steady state: the lease window is DERIVED to exceed the worst case one claim
     * can take (`worker/leaseWindow.ts`), so a sustained non-zero rate means the derivation has stopped
     * matching the work — and each one is a fetch paid for and thrown away.
     *
     * ⛔ The exclusion is what makes that reading true. `WorkerRuntime.stop` waits only a bounded grace for
     * the in-flight drain before releasing leases, so an ordinary deploy of a busy worker loses claims BY
     * DESIGN; `abandonLostClaim` logs those and does not count them. Without that split this series would
     * spike on every release and mean nothing.
     *
     * ⚠️ DASHBOARD-ONLY today — no alarm reads it, deliberately, because there is no operating history to
     * pick a threshold from. Anything added later inherits the exclusion above; do not alarm on a raw count
     * that includes shutdowns.
     */
    leaseLost: 'food-lease-lost',
} as const;
 ...
 * (`packages/infra/global/__tests__/emfIdentifierDimensionRepoGate.test.ts`) is the second line for every
 ...
 * `serviceInfraWiringInvariants.test.ts`'s W4 will require every `Commise/Food` alarm to select it, because
...
export function buildEmf(input: EmfInput): EmfPayload {
    ...
    // ⛔ NULL PROTOTYPE, and `_aws` is attached to it rather than seeding it. Every other write below is
    // `payload[name] = …` with a name that came from the DATA — a dimension key and a metric name — so on an
    // ordinary object literal a name of `__proto__` reaches `Object.prototype`'s inherited setter instead of
    // defining a field: the value is dropped and the record ships one metric short, which is the silent half
    // of the failure `packages/infra/global/__tests__/prototypePollutionSinks.test.ts` guards repo-wide.
    // Seeding the bag with `{ _aws: … }` made this invisible to that guard for a while — its detector wanted
    // a bare `{}` — so the shape is now in its positive table, not just fixed here.
    //
    // ⚠️ The emitted JSON is UNCHANGED, and that is a CloudWatch contract with four production alarms
    // reading it: `JSON.stringify` ignores the prototype, and the insertion order is still `_aws` → the
    // dimension fields → the metric fields (no key here is integer-like, so none is re-ordered).
...
export class FoodMetrics {
    ...
    /**
     * Record the count of foods tombstoned `FAILED` — the population REQ-NF-016's alarm watches.
     *
     * ⛔ Distinct from {@link recordTombstoneCount}, which counts EVERY tombstone including `NOT_FOUND`. DSN-9
     * treats `NOT_FOUND` as a normal outcome, and tombstones persist, so alarming on the combined count kept
     * the alarm permanently lit. This series is the one an operator can act on.
     *
     * @param count - The `FAILED` food count.
     * @sideEffect Emits one EMF line.
     */
    public recordFailedBacklog(count: number): void {
        emitMetric({ metrics: [{ name: FOOD_METRIC.failedBacklog, value: count, unit: 'Count' }] }, this.sink);
    }
    ...
    /**
     * Record one claim abandoned because its lease was reaped or reclaimed mid-work (R16).
     *
     * @sideEffect Emits one EMF line.
     */
    public recordLeaseLost(): void {
        emitMetric({ metrics: [{ name: FOOD_METRIC.leaseLost, value: 1, unit: 'Count' }] }, this.sink);
    }
