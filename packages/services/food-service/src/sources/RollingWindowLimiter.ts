 ...
 * (failsafe back-off).
 *
 * **Two lanes on one budget (F-W1).** Every charge names its {@link SourceCallChannel}, and both lanes are
 * admitted against the SAME 90% ceiling (FR-019, amended 2026-09-15 — owner ruling "Up to 900"). The lane is
 * therefore an ATTRIBUTION, not a privilege: it records who spent, which is what makes the shared budget
 * measurable, while the reserved top 10% of a source's published limit stays unspent by anyone. FR-019's
 * reserve is honoured by the DRAIN yielding — it pauses at the same threshold and defers its row — never by
 * letting a human spend into a third party's final tenth and earn the 429 the reserve exists to avoid.
 *
 * @implements FR-019 FR-020 FR-021 FR-026
 */
import { settingFromEnv } from '../config/env.schema.js';
import type { SourceCallChannel, SourceCallLogDao, WindowCheckResult } from '../foods/dao/sourceCallLog.dao.js';
...
export class RollingWindowLimiter {
    ...
    /**
     * The admission ceiling on the source's ONE window — the SAME 90% for both lanes.
     *
     * ⛔ OWNER RULING 2026-09-15: our usage of a source's published limit stops at 90% of it ("Up to 900" of
     * USDA's 1,000 per hour), and nothing about that number depends on which lane is spending. The
     * interactive lane used to be allowed the whole hard cap, which is FR-019's reserved top 10% read as a
     * licence to spend it: the reserve existed so a waiting human could still be served while the drain
     * paused, but it also meant our own traffic could reach 100% of a third party's limit and take the 429
     * that follows — the failure the reserve was invented to avoid.
     *
     * The reserve still does its job, because the two lanes remain ONE budget and the drain is the side that
     * yields: a paused drain frees the window for interactive calls without either lane crossing 90%.
     *
     * @throws {Error} when no caps are configured for the source.
     */
    private ceilingFor(source: FoodSourceId, _channel: SourceCallChannel): number {
        return this.capFor(source).pauseThreshold;
    }
