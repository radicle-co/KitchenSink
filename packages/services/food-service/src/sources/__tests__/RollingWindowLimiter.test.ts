...
import type {
    ...
    SourceCallLogDao,
    WindowCheckResult,
} from '../../foods/dao/sourceCallLog.dao.js';
...
describe('RollingWindowLimiter — interactive vs worker lanes (F-W1, FR-019)', () => {
    ...
    /**
     * ⚠️ THIS TEST PROVED THE OPPOSITE UNTIL 2026-09-15: that the interactive lane could spend into the top
     * 10% the drain may not touch. The owner ruled our usage of a source's published limit stops at 90% of it
     * ("Up to 900" of USDA's 1,000 per hour) whichever lane is spending — because spending to 100% is how we
     * earn the 429 that FR-019's reserve was invented to avoid.
     *
     * FR-019's reserve is not deleted by that, it is re-read: the two lanes remain ONE budget and the DRAIN
     * is the side that yields, so a paused drain still frees the window for a waiting human. What changed is
     * the ceiling that frees it — 90%, not 100%.
     */
    it('stops BOTH lanes at 90% — our usage of a third party limit never reaches it', async () => {
        ...
        // Nothing was charged: a denied call must not consume the window it was refused from.
        expect(dao.rows.filter((row) => row === 'interactive')).toHaveLength(0);
    });

    it('still serves a waiting human while the drain is paused BELOW the threshold', async () => {
        // The lane split is what this proves now: at 8 of 9 the drain and the human both fit, and the drain
        // yielding at 9 is what keeps the last call available to the human rather than to a backlog.
        const dao = ledgerDao(Array.from({ length: 8 }, () => 'worker' as const));
        const limiter = new RollingWindowLimiter(dao, { caps });
