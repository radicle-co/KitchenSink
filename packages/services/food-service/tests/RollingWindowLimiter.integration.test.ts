...
describe.skipIf(!hasTestDatabase)('RollingWindowLimiter (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
     ...
     * their subject was "the HARD cap", which both lanes are still bounded by. Each still asserts the
     * identical numbers it asserted before F-W1.
     */
    describe('interactive vs worker lanes (F-W1, FR-019)', () => {
        ...
        /**
         * ⛔ REWRITTEN for the FR-019 amendment of 2026-09-15 (owner ruling "Up to 900"). This case used to
         * assert that a human was ADMITTED at the drain's ceiling — the interactive lane spending into the
         * final 10% of the key. That is the behaviour the ruling retired, and for the reason the reserve
         * was invented: spending to 100% of a third party's published limit is how you earn the 429 the
         * reserve exists to avoid. Our usage now stops at 90% whoever is spending.
         *
         * The reserve still does its job through the OTHER half of the mechanism, which is what this case
         * now proves: the two lanes remain ONE budget, and the side that yields is the drain. A cook is
         * served out of the headroom a paused drain leaves behind, never out of the source's last tenth.
         */
        it("holds BOTH lanes at 90% — a human is served from the drain's yield, not the key's last tenth", async () => {
            const limiter = new RollingWindowLimiter(dao, { caps });
            await seedWorkerCalls(8);

            // One slot below the ceiling, either lane may take it — the budget is shared, not split.
            await expect(limiter.tryRecord('usda', 'interactive')).resolves.toMatchObject({ allowed: true });

            // At the ceiling, the human is refused exactly as the drain is, with a tenth of the hard cap
            // deliberately unspent. Restoring the per-lane ceiling makes the first of these `true` again.
            ...
            // ...and the ledger ATTRIBUTES the admitted call, which is the other half: a lane nobody can
            // measure is a claim rather than a guarantee.
            expect(await limiter.count('usda', 'interactive')).toBe(1);
            expect(await limiter.count('usda', 'worker')).toBe(8);
            expect(await limiter.count('usda')).toBe(caps.usda.pauseThreshold);

            // The drain is ALSO paused here, which is what frees the window for the next cook rather than
            // racing them for it.
            expect(await limiter.isPaused('usda')).toBe(true);
        });
