...
describe('ApiExceptionFilter over a booted Nest app', () => {
    ...
    it('logs a real TypeError from a handler as ONE parseable JSON line, and answers 500', async () => {
        ...
        // ⛔ `error`, not `stack`, and the difference is where the RENDER happens. This filter used to call
        // `renderThrowable(exception)` itself and log the result under `stack`; it now hands the throwable
        // over whole and `@kitchensink/service-logging`'s sink renders it — because the sink SCRUBS what it
        // renders, and a string that arrives already rendered was reaching the other scrub path, whose
        // whole-value bearer check would replace an entire trace with `[redacted]`. The rendered text still
        // carries the frame, which is what this case is about; the scrub assertions below are unchanged and
        // are what prove nothing leaked in the move.
        expect(String(parsed[0]['error'])).toContain('ThrowingController');
