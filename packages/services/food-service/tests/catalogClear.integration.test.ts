...
import { provisionRecipeProbeDatabase } from './support/recipeProbeDb.js';
import { FOOD_TEST_DATABASE, foodDb, hasTestDatabase } from './support/roleDb.js';

/**
 * A sibling database on the SAME server, standing in for the recipe service's. The clear reads it through
 * a connection the operator supplies, so the suite supplies one too — provisioned under the RECIPE role
 * model (`tests/support/recipeProbeDb.ts`) and handed over as `recipe_app`, which is the shape of the
 * `RECIPE_DATABASE_URL` a real operator is given.
 */
const PROBE_DB_NAME = 'recipe_probe_u12a_test';

/** A second sibling database with an `ingredients` table that is MISSING a link column. */
const DRIFTED_DB_NAME = 'recipe_drift_u12a_test';

/** A third with no `ingredients` table at all. */
const EMPTY_DB_NAME = 'recipe_empty_u12a_test';
...
describe.skipIf(!hasTestDatabase)('food catalog clear (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    ...
    beforeAll(async () => {
        ...
        // ⛔ The three siblings are provisioned by the harness, not by this suite's own pool: `food_app`
        // has no CREATEDB, which is exactly the privilege the deployed service is not given. Each is a
        // RECIPE-role database, so the URL the CLI is handed below is `recipe_app`'s — the connection an
        // operator would really supply.
        const [probe, drifted, empty] = await Promise.all([
            provisionRecipeProbeDatabase(PROBE_DB_NAME),
            provisionRecipeProbeDatabase(DRIFTED_DB_NAME),
            provisionRecipeProbeDatabase(EMPTY_DB_NAME),
        ]);

        probeUrl = probe.appUrl;
        probePool = new pg.Pool({ connectionString: probe.appUrl });
        driftedPool = new pg.Pool({ connectionString: drifted.appUrl });
        emptyPool = new pg.Pool({ connectionString: empty.appUrl });

        // The columns the probe reads, exactly as `0001_initial.sql` + `0006` shape them in the recipe
        // service. Only these three matter to the probe; nothing else here reads this table. Created as the
        // probe database's OWNER — DDL is not a service role's to do — which also grants `recipe_app` the
        // DML the CLI needs, through the same default-privileges hook a real migration run installs.
        await probe.asOwner(async (client) => {
        ...
        await drifted.asOwner(async (client) => {
            await client.query('DROP TABLE IF EXISTS ingredients');
            await client.query(
        ...
        await empty.asOwner(async (client) => {
            await client.query('DROP TABLE IF EXISTS ingredients');
        });
    ...
    async function setRecipeLinks(count: number): Promise<void> {
        // `DELETE`, not `TRUNCATE`: this connection is `recipe_app`, and a service role holds DML only.
        await probePool.query('DELETE FROM ingredients');
    ...
    describe('the post-condition rolls the transaction back', () => {
         ...
         */
        beforeEach(async () => {
            // AS THE OWNER: a trigger is DDL, and `food_app` holds none.
        ...
        // ⛔ REMOVED AGAIN, every time. The tier no longer rebuilds the schema between suites, so a trigger
        // that re-INSERTS a food row on every DELETE would otherwise follow this file into every later one.
        afterEach(async () => {
            await foodDb().asOwner(async (client) => {
                await client.query('DROP TRIGGER IF EXISTS u12a_smuggle_food_trigger ON food');
                await client.query('DROP FUNCTION IF EXISTS u12a_smuggle_food()');
            });
        });
    ...
    describe("the target it names is the SERVER's own answer", () => {
        it('reports the database and role the connection actually reached, not the connection string', async () => {
            ...
            expect(target).toMatchObject({ database: FOOD_TEST_DATABASE });
