...
import { provisionRecipeProbeDatabase, type RecipeProbeDatabase } from './support/recipeProbeDb.js';
 ...
 *
 * It is not the subject, but it is no longer a superuser-made fixture either: it is provisioned under the
 * RECIPE role model and handed to the CLI as `recipe_app`, because nothing in this tier may connect as a
 * superuser and `food_app` has no CREATEDB to make one with.
 */
const PROBE_DB_NAME = 'recipe_probe_u12b_test';
...
describe.skipIf(!hasTestDatabase)('food catalog reseed (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let probeDb: RecipeProbeDatabase;
    ...
    beforeAll(async () => {
        // The SUBJECT is the tier's own `food_test`, reached ONLY as `food_app` — the role the reseed and
        // clear CLIs connect as (`foodPoolConfigFromEnv`). A superuser would satisfy any missing grant;
        // this proves the DML-only service role suffices.
        pool = makePool();
        db = makeDb(pool);

        probeDb = await provisionRecipeProbeDatabase(PROBE_DB_NAME);
        probeUrl = probeDb.appUrl;
    ...
    beforeEach(async () => {
        ...
        // The probe's table is fixture DDL, so it is made by that database's OWNER — which is also what
        // grants `recipe_app` the DML the CLI performs through it.
        await probeDb.asOwner(async (client) => {
