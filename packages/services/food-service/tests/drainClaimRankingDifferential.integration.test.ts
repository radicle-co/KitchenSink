 ...
 *     `drainClaimScaling.integration.test.ts` is what owns it. Equivalence and cost need separate gates.
 ...
 *     here. `drainClaimScaling.integration.test.ts` asserts the tie-break directly instead.
 *
...
describe.skipIf(!hasTestDatabase)('drain-claim ranking is differentially equivalent to FR-043 (T-201)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    ...
    afterAll(async () => {
        // Hand the database back as the migrations left it. Every other suite now empties DATA rather than
        // rebuilding the schema, so a fixture table left behind here would outlive this file.
        await foodDb().asOwner(async (client) => {
            await client.query('DROP TABLE IF EXISTS queue_snapshot');
        });
        await pool?.end();
    });

    /**
     * (Re)create the empty snapshot table.
     *
     * ⚠️ AS THE OWNER, because `food_app` holds DML and no DDL — which is exactly the privilege set the
     * deployed service has. The table is nonetheless readable and writable by the service role afterwards:
     * the migration engine's `ALTER DEFAULT PRIVILEGES FOR ROLE food_owner … GRANT SELECT, INSERT, UPDATE,
     * DELETE … TO food_app` is what makes anything the owner creates usable by the app, and this fixture
     * exercises that rule rather than working around it.
     *
     * @sideEffect Drops and recreates `queue_snapshot`.
     */
    async function resetSnapshotTable(): Promise<void> {
        await foodDb().asOwner(async (client) => {
            await client.query('DROP TABLE IF EXISTS queue_snapshot');
            // A plain table, not TEMP: statements go out over a pool, so a per-connection temp table would
            // be invisible to half of them. Restoring only the columns a drain mutates is sufficient and
            // exact. (A temp table is also not an option for the subject at all now — `food_app` has no
            // TEMP on the database.)
            await client.query('CREATE TABLE queue_snapshot AS SELECT * FROM fetch_queue WITH NO DATA');
        });
    }

    beforeEach(async () => {
        await foodDb().truncate();
        await resetSnapshotTable();
    });
    ...
    async function seedRandomQueue(
        ...
        // ⚠️ AS THE OWNER: `ANALYZE` issued by `food_app` warns, SKIPS the tables it does not own, and
        // still returns success — so the rankings below would be compared against stale statistics with
        // nothing to show for it.
        await foodDb().asOwner(async (client) => {
            await client.query('ANALYZE food, fetch_queue, fetch_requesters');
        });
    ...
    async function snapshotQueue(): Promise<void> {
        // `DELETE`, not `TRUNCATE`: the service role holds DML and nothing more, and for a fixture table
        // this small the two are indistinguishable in effect.
        await pool.query('DELETE FROM queue_snapshot');
