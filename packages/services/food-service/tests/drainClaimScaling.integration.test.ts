 ...
 *    `SKIP LOCKED` under genuinely concurrent claims. `fairnessDemotion.integration.test.ts` (T-049/
...
describe.skipIf(!hasTestDatabase)('drain-claim cost + ranking equivalence (T-197, DSN-11)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let foods: FoodDao;
    ...
    async function seedDeepQueue(depth: number): Promise<void> {
        ...
        //
        // ⚠️ AS THE OWNER, and that is not ceremony: `ANALYZE` issued by `food_app` — which owns none of
        // these tables — emits `WARNING: permission denied to analyze "food", skipping it` and returns
        // SUCCESS. The statistics would silently stay stale, and every plan assertion below would be made
        // against a plan no stage would ever choose. A skip that reports success is exactly the failure the
        // role split exists to make visible, so the elevation is named here rather than granted quietly.
    ...
    describe('ranking equivalence (FR-043, FR-015)', () => {
        ...
        it('never claims a backed-off row (last_requested in the future) even at the highest demand', async () => {
            ...
            // Out-of-band: this case is about the CLAIM's backoff gate, and the row was never leased.
            await queue.deferLease(backedOff, 3_600, 'out-of-band');
