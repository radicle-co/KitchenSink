...
describe.skipIf(!hasTestDatabase)('real UsdaApiClient + UsdaSourceAdapter over undici MockAgent (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        usda.intercept({ path: (p) => p.startsWith(`/fdc/v1/food/${FOUNDATION_FDC_ID}`), method: 'GET' })
            .reply(200, loadFixture('food1750340Foundation.json'))
            .persist();
        usda.intercept({ path: (p) => p.startsWith(`/fdc/v1/food/${BRANDED_FDC_ID}`), method: 'GET' })
            .reply(200, loadFixture('food2057648Branded.json'))
            .persist();
        ...
        usda.intercept({ path: (p) => p.startsWith('/fdc/v1/foods/search'), method: 'GET' })
            .reply(200, (opts) => {
                ...
                return loadFixture('searchSingleBranded.json');
            })
            .persist();
        ...
        usda.intercept({
            ...
            .reply(200, loadFixture('foodsBatch.json'))
            .persist();
        usda.intercept({
            path: (p) => p.startsWith('/fdc/v1/foods?'),
            method: 'POST',
        ...
        process.env['FOOD_QUEUE_DEPTH_ALARM'] = '25';
