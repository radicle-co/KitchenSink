...
describe.skipIf(!hasTestDatabase)('FoodConsumerService (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let foodDao: FoodDao;
    ...
    /**
     * ⛔ TWO CLAIM LOOPS, ONE ROW, EXACTLY ONE RESULT (U5/R16) — the verification the plan names.
     *
     * The race this proves is NOT two workers: FR-022's advisory lock makes that impossible, and
     * `FOR UPDATE SKIP LOCKED` keeps sibling loops off each other's rows. It is two claim loops of the
     * SAME drainer, and the mechanism is `leaseNext`'s own reaper-on-claim — every claim first reverts
     * lapsed leases, so a loop that is slow enough hands its row to the next loop that asks for work.
     *
     * Before the fence, the slow loop then fetched the food a second time, merged it a second time, and
     * published a second completion for a row it no longer owned. Now its settle is refused, which is
     * what turns "two loops raced" into wasted work rather than a duplicate result.
     */
    it('⛔ a claim reaped mid-fetch loses its settle — two loops on one row produce ONE result (R16)', async () => {
        const id = await enqueueFood('contested food');

        let releaseFirstSearch: (hits: SourceCandidate[]) => void = () => {};

        let announceFirstSearch: () => void = () => {};

        const firstSearchStarted = new Promise<void>((resolve) => {
            announceFirstSearch = resolve;
        });
        const gated = new Promise<SourceCandidate[]>((resolve) => {
            releaseFirstSearch = resolve;
        });

        const hits: SourceCandidate[] = [{ source: 'usda', externalKey: '1', name: 'Contested food' }];
        const adapter = makeFakeUsdaAdapter();
        let searches = 0;
        adapter.searchByName.mockImplementation(async () => {
            searches += 1;
        ...
        adapter.fetchByKeys.mockResolvedValue([makeMergeCandidate('usda', { externalKey: '1', name: 'Contested' })]);
        const { consumer, publisher } = build(adapter);

        // Loop 1 claims the row and parks inside the source call, exactly as a slow fan-out does.
        const slowLoop = consumer.processNext();
        await firstSearchStarted;

        // Its lease lapses while it works. Loop 2's claim reaps the row and takes it.
        await pool.query(`UPDATE fetch_queue SET leased_at = now() - interval '1 hour' WHERE food_id = $1`, [id]);
        expect(await consumer.processNext()).toBe('resolved');

        // Loop 1 now finishes and tries to land its result on a row it no longer holds.
        releaseFirstSearch(hits);

        expect(await slowLoop).toBe('lease_lost');
        expect(publisher.messages.map((put) => put.kind)).toEqual(['FoodFetchCompleted']);
        expect((await foodDao.getById(id))?.status).toBe('RESOLVED');
        expect(await queue.getByFoodId(id)).toBeUndefined();
    });

    /**
     * ⛔ The refusal must not take the DRAINER down with it. `processNext`'s catch handler settles the row
     * too (`recordFailure`), so a claim reaped before an unrelated processing error reaches a settle that
     * ALSO refuses — from inside the handler. Unguarded, that escapes `processNext`, rejects the
     * `Promise.all` in `drain`, and aborts every sibling claim loop: one lost lease would stop the whole
     * drain. The happy-path guard alone does not cover this.
     */
    it('⛔ a lost lease inside the FAILURE handler is contained, not thrown out of the drain', async () => {
        const id = await enqueueFood('exploding food');

        const adapter = makeFakeUsdaAdapter();
        adapter.searchByName.mockImplementation(async () => {
            // Reap the claim, then fail in a way `processRow` does not handle — so the catch path runs
            // while the row is already gone.
            await pool.query(`UPDATE fetch_queue SET status = 'pending', leased_at = NULL WHERE food_id = $1`, [id]);

            throw new TypeError('unexpected processing failure');
        });
        const { consumer } = build(adapter);

        await expect(consumer.processNext()).resolves.toBe('lease_lost');
        // The row survives untouched for whoever claims it next — no attempt was spent on it.
    ...
    /**
     * ⚠️ The fixture ages the claim past the CONFIGURED window rather than a literal. It used to back-date
     * by 120 seconds, which was stale under FR-018's original 30-second lease and is not stale under the
     * derived one (`worker/leaseWindow.ts`) — so the number that made this case work had become a fact
     * about a window nobody was running any more. Reading the window it is testing is what keeps the case
     * about the reaper instead of about an arithmetic coincidence.
     */
    it('reapStaleLeases reverts a stale in_flight lease back to pending (FR-018)', async () => {
        ...
        const staleBy = settingFromEnv('FOOD_LEASE_TIMEOUT_SECONDS') + 1;
        await pool.query(`UPDATE fetch_queue SET leased_at = now() - make_interval(secs => $2) WHERE food_id = $1`, [
            id,
            staleBy,
        ]);
