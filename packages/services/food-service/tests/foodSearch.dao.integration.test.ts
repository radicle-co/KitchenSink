...
describe.skipIf(!hasTestDatabase)('FoodSearchDao ranked full-text search (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
 ...
 */
describe.skipIf(!hasTestDatabase)('FoodSearchDao head-term retrieval (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let dao: FoodSearchDao;
    ...
    it('⛔ retrieves the head noun when the cook supplied a modifier the row does not have', async () => {
        ...
        // Recorded in `docs/reports/2026-08-22/ingredientResolutionMeasurement.md` rather than smuggled
...
describe.skipIf(!hasTestDatabase)('U11/R20 — a private authored food is retrievable ONLY by its author', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let dao: FoodSearchDao;
    ...
    it("⛔ a stranger's search sees CATALOG rows only — never any authored row (ADR-0036)", async () => {
        // ⚠️ REVERSES half of what this case used to assert. It read "sees the catalog and PROMOTED rows",
        // because promotion was the event that made an authored food everybody's. Promotion is removed
        // (owner ruling, 2026-09-07) and an authored food is now AUTHOR-ONLY, so `f_promoted` — which is
        // still seeded carrying the `promoted` VALUE, deliberately, because the value remains admissible on
        // the column — must be as invisible to a stranger as the private one. If a partial revert ever
        // re-adds `OR visibility = 'promoted'` to the predicate, this is the line that catches it.
        const ids = (await dao.search('quinoa', STRANGER)).map((hit) => hit.id);

        expect(ids).toContain('f_catalog');
        expect(ids).not.toContain('f_promoted');
    ...
    it("the author's OWN authored row is the only one carrying an authored visibility", async () => {
        // Replaces "a promoted authored food is flagged 'promoted' for EVERYONE". There is no such food any
        // more, and the surviving question is the one this asks: the DAO carries the RAW column, and the
        // service maps `public` to "no flag on the wire" (`foods.service.test.ts` pins that projection), so
        // the raw value is what is asserted here.
        const strangerHits = await dao.search('quinoa', STRANGER);

        expect(strangerHits.find((hit) => hit.id === 'f_catalog')?.visibility).toBe('public');
        expect(strangerHits.every((hit) => hit.userId === null)).toBe(true);

        const authorHits = await dao.search('quinoa', AUTHOR);

        expect(authorHits.find((hit) => hit.id === 'f_private')?.visibility).toBe('private');
    });
});
