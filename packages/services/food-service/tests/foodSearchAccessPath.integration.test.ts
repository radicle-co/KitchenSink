 ...
 * arithmetic, not the code's behaviour. `drainClaimScaling.integration.test.ts` can assert a plan
...
import { makeDb, makePool } from './support/db.js';
...
describe.skipIf(!hasTestDatabase)('FoodSearchDao relevance access path (T-202, SC-007)', () => {
    let pool: pg.Pool;
    ...
    beforeAll(async () => {
        ...
        //
        // ⚠️ BOTH STATEMENTS GO THROUGH THE OWNER, and they have to: `ALTER TABLE` is DDL the service role
        // does not hold, and `ANALYZE` as `food_app` does something worse than fail — it WARNS, skips the
        // table, and returns success, which would leave every plan assertion below reading statistics that
        // were never gathered.
        await foodDb().asOwner(async (client) => {
            await client.query('ALTER TABLE food ALTER COLUMN name SET STATISTICS 1000');
            await client.query('ALTER TABLE food ALTER COLUMN description SET STATISTICS 1000');
            // `rank_tokens` joined the predicate when the catalog gained head-term retrieval, and a
            // generated column starts with default statistics like any other. Without a matching target the
            // planner's estimate for that branch is the noisiest of the six, and the OR-list is only ever as
            // stable as its worst estimate — which showed up here as the whole statement falling back to a
            // Seq Scan.
            await client.query('ALTER TABLE food ALTER COLUMN rank_tokens SET STATISTICS 1000');
            await client.query('ANALYZE food');
        });
        // 100k rows plus two statistics-1000 ANALYZEs take ~40s on a healthy runner; CI run 34788707047 measured the
        // same unchanged setup past 60s on a runner that was ~2x slow across the whole suite. The limit bounds a hang,
        // not the work, so it carries headroom for a slow runner.
    }, 180_000);

    afterAll(async () => {
        // ⛔ Hand the database back as the migrations left it. The statistics targets above are SCHEMA
        // state, not data, so `truncate()` does not undo them — and this tier does not rebuild the schema
        // per suite, so a raised target would follow every later file and change the plans they measure.
        await foodDb().reset();
        await pool.end();
    });
        ...
        try {
            ...
            const walk = (node: Record<string, unknown> | undefined): void => {
                ...
                if (node['Index Name'] !== undefined) {
                    indexes.push(z.string().parse(node['Index Name']));
                }
