 ...
 *
 * Boot config: `FOOD_QUEUE_DEPTH_ALARM=10`, `FOOD_DEMOTE_THRESHOLD=2` (drain-order demotion),
...
describe.skipIf(!hasTestDatabase)('/api/v1/foods/* HTTP API (booted Nest + real Postgres)', () => {
    ...
    beforeAll(async () => {
        ...
        process.env['FOOD_QUEUE_DEPTH_ALARM'] = '10';
    ...
    describe('intake accepts every request (FR-043b retired)', () => {
        it('accepts an add well past the old depth ceiling', async () => {
            await seedPendingQueue(10, 'user_1'); // once FOOD_MAX_QUEUE_DEPTH=10, now only an alarm threshold
