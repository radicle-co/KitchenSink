...
networklessly against a pinned public PEM and enforces `azp`. So `prepareClerkTokens.ts` generates a
...
- A Postgres holding a **disposable** database with the food schema applied. The scripts REFUSE to run
  ...
  writes ~180,000 rows.
    - The fixture seeder is **additive and never drops the schema** (unlike identity's `prepareDb.ts`),
...
1. **`serviceErasure.load.js` runs FIRST.** Its EdDSA pool lives 120s (the verifier's contract maximum),
   and the fixture seed between the scripts takes ~15s+, so any later slot risks racing the TTL.
2. **`drainDemotion.perf.ts` runs LAST.** It is the only write-heavy step (up to 10,000 queue rows and
...
3. The three SC k6 scripts are read-only and mutually independent, but `localStoreThroughput.load.js` is a
   **separate script** from `localStoreRead.load.js` on purpose: two scenarios hammering the same read path
...
> **RETIRED by 003-FR-010a (plan U37).** The `short` shape no longer exists: a query below three characters
> is answered with no statement and no crosswalk read, so there is nothing left to time. The measurements
> below are kept as the RECORD that produced T-198 — do not read them as a live gate, and do not re-add the
> shape to reinstate one.
...
1. **`FOOD_SEARCH_SHORT_P95_MS` is DELETED.** It defaulted to 200ms, so it never loosened anything by
   ...
   created for no longer exists. All eight shapes now read `SEARCH_P95_MS`. Per-shape attribution is
...
2. **k6 is no longer the primary guard on this path, because it is HEAVY-TIER.** `load-test-food` runs only
   on a PR carrying the `k6` label, or through `deployed-e2e.yml`, so a guard that lives only here does not
   run on an ordinary PR. The routing and the statement shape are therefore pinned by tiers that run on
   EVERY PR — `src/foods/dao/__tests__/foodSearch.dao.test.ts` (unit: the strategy selection AND the
   rendered SQL, asserting the `ILIKE '%q%'` branches are absent at 1–2 characters) and
   `tests/foodSearch.dao.integration.test.ts` (real Postgres: word-initial recall, the stopword traps, the
...
hardware are asserted separately: `tests/drainClaimRankingDifferential.integration.test.ts` proves the
ranking is unchanged against FR-043 stated literally, and `tests/drainClaimScaling.integration.test.ts`
...
pattern, so the index intersection and the per-row `GREATEST(ts_rank(search_vector, …), ts_rank(aliases_search_vector, …), similarity(…))` ranking over every
...
against a pure Seq Scan, and pinned per-PR by `tests/foodSearchAccessPath.integration.test.ts`. Local DB
...
2. **This fixture is not production-shaped, and that is what hides (1).** `perfFoodDescription` writes
   ...
   `usdaBulk.parser.ts`), so the deployed `food_description_trgm_idx` indexes the name's trigrams and
...
(the `k6` PR label, or a `deployed-e2e` dispatch). The job mints BOTH token pools itself — they are gitignored,
