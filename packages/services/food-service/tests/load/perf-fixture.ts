 ...
 * probe set and the disposable-database guard shared by `prepare-perf-fixture.ts` (which seeds Postgres)
 * and `drain-demotion.perf.ts` (which seeds the queue). The k6 scripts never re-derive any of it: they
 * `open()` the JSON `prepare-perf-fixture.ts` emits, so a fixture rule lives here exactly once.
 *
 * Mirrors identity's `tests/load/pool-identities.ts`. It is TypeScript (run under `tsx`), NOT part of the
 ...
 * seeder, exactly as identity's `prepare-db.ts` proves its bulk SQL matches `bulkIdentity(0)`.
 *
...
/** `normalizeName` (src/foods/merge/merge-engine.ts) applied to a fixture name. Pure. */
...
/** Filename `prepare-perf-fixture.ts` writes and the k6 scripts `open()`. Gitignored (per-run derived). */
