...
describe('a real PutItem under the key schema the CDK stack freezes', () => {
    ...
    /**
     * ⛔ THE PRECONDITION the case below depends on, asserted before it rather than discovered through it.
     *
     * That spec constructs the publisher with NO injected client, deliberately — the production constructor
     * is what carries `removeUndefinedValues`, and injecting a correctly-configured client would test this
     * file's configuration instead. The cost is that the client then resolves the full AWS credential chain,
     * and on a developer machine that chain finds `~/.aws/credentials`.
     *
     * ⚠️ It did. Measured 2026-09-08: this suite was resolving a real long-term `AKIA…` key and the DEFAULT
     * AWS endpoint, i.e. issuing `PutItem` against real DynamoDB in the account that also hosts a separate
     * production system. The table does not exist there, so it surfaced as `ResourceNotFoundException`
     * swallowed into `onError` — indistinguishable from the marshalling bug the case is written to catch,
     * which is why it went unexplained.
     *
     * `vitest.integration.config.ts` now pins the tier's AWS environment. This asserts the pin held, so the
     * day someone removes it the failure NAMES the cause instead of looking like a flaky assertion.
     */
    it('⛔ a default-constructed client resolves to LocalStack and dummy credentials, never real AWS', async () => {
        const probe = new DynamoDBClient({});

        // `test` is the dummy pair the tier's config sets, and this half is the one that matters: a real
        // long-term key is 20 chars beginning `AKIA`, and environment credentials are what stop the SDK
        // consulting `~/.aws/credentials` at all.
        expect((await probe.config.credentials()).accessKeyId).toBe('test');

        // ⚠️ The endpoint is asserted BEHAVIOURALLY, not by reading `config.endpoint()` — which is
        // `undefined` here, because `AWS_ENDPOINT_URL` is resolved further down the middleware stack rather
        // than into that field. Listing the table this suite created is a direct proof that a client built
        // the way production builds one reaches the LOCAL substrate: before the tier pinned its environment
        // this same call went to real AWS and could not see it.
        const { TableNames } = await probe.send(new ListTablesCommand({}));

        expect(TableNames).toContain(table.tableName);
    });
