...
import { BASE_MARKER_FOOD, ensureSeededBaseDatabase } from './support/maintenanceDb.js';
...
import type { RdsLikeDatabase } from '@kitchensink/service-test-harness';
...
describe.skipIf(!hasTestDatabase)('per-PR database warm start (U38)', () => {
    /**
     * ⚠️ The clones are made by the production MIGRATOR: cloning the owner's base needs CREATEDB, INHERIT on
     * the template's owner and SET to create `OWNER food_owner` — exactly the grants a stage gives it, and
     * exactly what a superuser run could never have noticed missing.
     *
     * ⛔ The MASTER only cleans up, and it has to be the master: `DROP DATABASE … WITH (FORCE)` terminates
     * the sessions on the target, which needs `pg_signal_backend`. The migrator does not hold it (measured:
     * "permission denied to terminate process"); the stand-in master does, as RDS's master does.
     */
    ...
    async function inClone<T>(database: string, run: (pool: pg.Pool) => Promise<T>): Promise<T> {
        const pool = new pg.Pool({ connectionString: base.urlFor(DATABASE_ROLES.food.migrator, database), max: 1 });
    ...
    async function databaseExists(name: string): Promise<boolean> {
        const found = await master.query('SELECT 1 FROM pg_database WHERE datname = $1', [name]);
    ...
    afterEach(async () => {
        for (const name of [CLONE_TARGET, SECOND_CLONE_TARGET]) {
    ...
    it('FAILS LOUDLY when a session holds the template, and leaves no database behind', async () => {
        const holder = new pg.Client({ connectionString: base.appUrl });
