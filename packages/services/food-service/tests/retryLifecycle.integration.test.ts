 ...
 * was told.
 *
 * ⚠️ Every settle below presents `'out-of-band'` rather than a lease fence (U5/R16), and that is what
 * these cases mean: they drive the DAO directly to observe the `attempts` curve and the tombstone
 * lifecycle, WITHOUT a worker claim behind them. The fence is the drainer's authority, and a suite that
 * minted one here would be asserting something it does not exercise — the fenced path has its own cases
 * in `fetchQueue.dao.integration.test.ts` ("the lease fence") and `foodConsumer.integration.test.ts`.
 */
...
describe.skipIf(!hasTestDatabase)('placeholder retry lifecycle (U9, integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let foods: FoodDao;
    ...
    describe('the AWAITING_RETRY mark reaches the database (R2.1)', () => {
        it('the food_status ENUM carries AWAITING_RETRY — the migration ran, not just the TypeScript union', async () => {
            ...
            // ⚠️ EXACT, not a superset — which is why adding a lifecycle value lands here. `WITHDRAWN`
            // (migration 0016) is the author's own soft delete; a unit test over the TypeScript union cannot
            // see whether the `ALTER TYPE` actually ran, and 0016 is split into its own file precisely
            // because the value must be committed before 0017's index predicate can compare against it.
    ...
    describe('the retry re-enqueues with an exponential backoff that is PERSISTED (R2.3)', () => {
        it('the first failure increments attempts to 1, re-queues pending, clears the lease and records the error', async () => {
            ...
            const row = await queue.recordFailure(id, 'source 503', 'out-of-band');
        ...
        it.each([
            ...
            async (attempt, expectedSeconds) => {
                ...
                for (let n = 0; n < attempt; n += 1) {
                    await queue.recordFailure(id, undefined, 'out-of-band');
                }
        ...
        it('a backed-off row does not block a ready one — the gate is per row, not a global stall', async () => {
            ...
            await queue.recordFailure(gated, undefined, 'out-of-band');
    ...
    describe('the retry budget is capped at five and is spent only by real failures (R2.4, DSN-5)', () => {
        it('five recorded failures reach the budget ceiling; the fourth has not', async () => {
            ...
            for (let n = 0; n < MAX_FAILURE_ATTEMPTS; n += 1) {
                attempts = (await queue.recordFailure(id, undefined, 'out-of-band')).attempts;
        ...
        it('a tombstoned, FAILED food is claimable by nothing — the placeholder reached a terminal state', async () => {
            ...
            await queue.tombstone(id, 'all_sources_errored', 'out-of-band');
    ...
    describe('the operator requeue clears BOTH halves and gives a blackholed food a way back (R2.4)', () => {
        ...
        it.each(['RESOLVED', 'UNRESOLVED'] as const)(
            ...
            async (status) => {
                ...
                await queue.recordFailure(id, 'a real error', 'out-of-band');
