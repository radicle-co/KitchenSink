...
describe.skipIf(!hasTestDatabase)('kitchensink_food schema (integration)', () => {
    let pool: pg.Pool;

    beforeAll(async () => {
        pool = makePool();
        // The schema itself is built once per run by `tests/globalSetup.ts`, with the service's OWN
        // production runner — this suite asserts what that runner produced, and no longer replays the
        // `.sql` files itself (a fourth copy of a runner that skipped the lock, the privilege statements
    ...
    describe('table + enum topology (D-CANDIDATES — 16 tables)', () => {
        it('creates all 16 canonical + operational tables, and the ledger — and nothing else', async () => {
            ...
            // ⚠️ The ledger is the SEVENTEENTH table, and its arrival here is the tier moving onto the real
            // runner: this suite used to replay the `.sql` files itself, which records nothing, so a
            // migrated stage database had one table more than the test ever saw. It is named from the
