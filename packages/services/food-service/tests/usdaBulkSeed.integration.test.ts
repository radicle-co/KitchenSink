...
import { streamBulkCandidates } from '../src/sources/usda/bulk/usdaBulk.reader.js';
...
describe.skipIf(!hasTestDatabase)('USDA bulk seed importer (integration)', () => {
    let pool: pg.Pool;
    let db: TestDb;
    let foods: FoodDao;
    ...
    describe('reader → seeder pipeline over real on-disk bulk CSVs', () => {
        ...
        // ⚠️ afterEACH, not afterAll: `beforeEach` makes a NEW directory per test, so a suite-level teardown
        // removes only the last one and leaks every earlier test's.
        afterEach(() => {
