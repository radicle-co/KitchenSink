 ...
 *
 * `@commise/shared-clerk-verify` is mocked so the auth matrix is deterministic (no real Clerk JWT): a token
 * string maps to a verified principal, an unknown token throws (→ 401). `@commise/clients-usda` is
...
vi.mock('@commise/shared-clerk-verify', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@commise/shared-clerk-verify')>();

    return { ...actual, verifyClerkToken: vi.fn() };
});

vi.mock('@commise/clients-usda', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@commise/clients-usda')>();
