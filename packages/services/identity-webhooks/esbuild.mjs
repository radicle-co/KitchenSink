...
const entryPoints = [
    ...
    // The scheduled queue backstop (plan U12). The Lambda references `handlers/queueCheck.handler`.
    'src/handlers/queueCheck.ts',
];
...
await build({
    ...
    format: 'esm',
    sourcemap: true,
    // The SDK CLIENTS stay external (the runtime provides them). `@aws-sdk/rds-signer` is deliberately NOT: it is
    // a utility, not a client, this repository disagrees with itself about whether the runtime carries it (food's
    // esbuild says no, recipe-workers' says yes), and being wrong is a cold-start ERR_MODULE_NOT_FOUND on every
    // DB-bound Lambda here. Bundling it costs a few hundred KB and removes the question.
    external: ['@aws-sdk/client-*', '@aws-sdk/s3-request-presigner'],
