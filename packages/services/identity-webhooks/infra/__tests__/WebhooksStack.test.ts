...
describe('WebhooksStack (authoritative, consumes the consolidated global exports)', () => {
     ...
     * credential any more. All five used to fetch it at cold start and log in as `identity_app` — the master, a
     * member of `rds_superuser` — so a flaw in any of them reached every database on the instance. They now log
     * in as `identity_service` by RDS IAM. Asserted over the synthesized functions and policies, per function, so a
     * sixth DB-bound Lambda added with the old wiring fails here.
     */
...
describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Three alarms — the GDPR erasure-incompleteness detector and the two webhook-rejection
        // thresholds — on a stage whose webhook Clerk never calls.
        synthWebhooks(false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    }, 120_000);

    it('creates all three webhook alarms when the flag is on', () => {
        synthWebhooks(true).resourceCountIs('AWS::CloudWatch::Alarm', 3);
    }, 120_000);
