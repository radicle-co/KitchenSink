...
/**
 * Synthesize the stack, with the alarm feature flag in a chosen state.
 *
 * Lifted out of the suite's `beforeAll` so the flag's OFF case can be synthesized without a second copy of
 * twenty props — two copies would drift, and a drifted copy would assert the flag against a stack shaped
 * differently from the one every other test reads.
 *
 * @param alarmsEnabled - The synth-time flag (ADR: alarms default OFF, prod opts in per stage).
 * @returns The synthesized template.
 * @sideEffect Reads the bundled Lambda assets under `dist/`.
 */
function synthWebhooks(alarmsEnabled: boolean): Template {
    const app = testApp({
        context: {
    ...
    const stack = new WebhooksStack(app, 'TestWebhooks', {
        ...
        dbResourceId: 'db-ABCDEFGHIJKLMNOPQRSTUVWXYZ',
        deletionQueueArn: 'arn:aws:sqs:us-east-1:123456789012:kitchensink-deletion-test',
        deletionDlqArn: 'arn:aws:sqs:us-east-1:123456789012:kitchensink-deletion-dlq-test',
...
describe('WebhooksStack (authoritative, consumes the consolidated global exports)', () => {
    ...
    /**
     * ⛔ The role split (`docs/plans/2026-09-11-database-role-split.md`): no webhook Lambda reads the RDS master
     * credential. The master, `identity_app`, is a member of `rds_superuser`, so a flaw in any Lambda logged in
     * as it would reach every database on the instance; they log in as `identity_service` by RDS IAM. Asserted
     * over the synthesized functions and policies, per function, so a DB-bound Lambda wired to the master
     * secret fails here.
     */
    describe('⛔ every DB-bound Lambda connects as identity_service by RDS IAM, never as the RDS master', () => {
        const DB_BOUND_HANDLERS = [
            'handlers/identityWebhook.handler',
            'handlers/deletionWorker.handler',
            'handlers/reconciliation.handler',
            'handlers/tombstoneSweep.handler',
            'handlers/erasureReconciliation.handler',
        ];

        type FunctionResource = {
            readonly Properties: {
                readonly Handler: string;
                readonly Role: { readonly 'Fn::GetAtt': readonly [string, string] };
                readonly Environment?: { readonly Variables?: Record<string, unknown> };
            };
        };

        const dbBoundFunctions = (): readonly FunctionResource[] =>
            (Object.values(template.findResources('AWS::Lambda::Function')) as FunctionResource[]).filter((fn) =>
                DB_BOUND_HANDLERS.includes(fn.Properties.Handler),
            );

        /** The `rds-db:connect` resources granted to one role, rendered as JSON. */
        const connectResourcesFor = (roleLogicalId: string): readonly string[] =>
            Object.values(template.findResources('AWS::IAM::Policy'))
                ...
                .filter((statement) =>
                    ([] as string[]).concat(statement['Action'] as string | string[]).includes('rds-db:connect'),
                )
                .map((statement) => JSON.stringify(statement['Resource']));

        it('finds all five — so an empty answer below is a finding, not a filter that matched nothing', () => {
            expect(
                dbBoundFunctions()
                    .map((fn) => fn.Properties.Handler)
                    .sort(),
            ).toEqual([...DB_BOUND_HANDLERS].sort());
        });

        it('gives each one identity_service in DB_USERNAME, the instance coordinates, and no master-secret ARN', () => {
            for (const fn of dbBoundFunctions()) {
                const variables = fn.Properties.Environment?.Variables ?? {};

                expect(variables['DB_USERNAME'], fn.Properties.Handler).toBe('identity_service');
                expect(variables['DB_HOST'], fn.Properties.Handler).toBe('db.example.internal');
                expect(variables['DB_PORT'], fn.Properties.Handler).toBe('5432');
                expect(variables['DB_NAME'], fn.Properties.Handler).toBe('kitchensink_identity');
                expect(variables, fn.Properties.Handler).not.toHaveProperty('DB_SECRET_ARN');
            }
        });

        it('grants each one rds-db:connect as identity_service on this instance — and as nobody else', () => {
            for (const fn of dbBoundFunctions()) {
                const resources = connectResourcesFor(fn.Properties.Role['Fn::GetAtt'][0]);

                expect(resources, fn.Properties.Handler).toHaveLength(1);
                expect(resources[0], fn.Properties.Handler).toContain('db-ABCDEFGHIJKLMNOPQRSTUVWXYZ/identity_service');
            }
        });

        it('leaves nothing in the stack able to read the master secret', () => {
            expect(JSON.stringify(template.toJSON())).not.toContain('secret:db-AbCdEf');
        });
    });
    ...
    describe('alarms watch series the code actually publishes, and page someone', () => {
        ...
        it('⛔ alarms on the deletion DLQ, which two handler comments already promise', () => {
            // `deletionWorker.ts` tells its reader an invalid message "must reach the DLQ and its alarm". No
            // alarm existed: the queue and its DLQ are declared in DataStack, which owns no alarms, and this
            // stack — which owns every other identity alarm — never imported the DLQ. A closure that failed
            // five times therefore left the account tombstoned in the database and unbanned at Clerk, with
            // nothing watching. The alarm reads the QUEUE metric, so its dimension is `QueueName`, not the
            // EMF emitter's pair.
            const [alarm] = alarmsFor('ApproximateNumberOfMessagesVisible');
            const dimensions = dimensionsOf((alarm ?? {}) as Record<string, unknown>);

            expect(alarm, 'the deletion DLQ has no depth alarm').toBeDefined();
            expect(alarm?.['Namespace']).toBe('AWS/SQS');
            expect([...dimensions.keys()]).toEqual(['QueueName']);
            // One message that exhausted its five receives is already a failed ban or unban, so the threshold
            // is one rather than a rate.
...
/**
 * ⛔ Which Lambda environment variables carry a SECRET into the function's configuration.
 *
 * `SecretValue.secretsManager(...).unsafeUnwrap()` in an `environment` map renders a
 * `{{resolve:secretsmanager:…}}` dynamic reference, which CloudFormation resolves at deploy — so the
 * PLAINTEXT ends up in the function's configuration, readable by any principal holding
 * `lambda:GetFunctionConfiguration` on it (and visible in the console). That is a materially wider audience
 * than `secretsmanager:GetSecretValue` on the secret itself, and no IAM policy on the secret constrains it.
 *
 * All three are deliberate — the embed exists so `verifyWebhook`, `identityClient` and `serviceErasureToken`
 * work without a runtime `GetSecretValue` — and all three are reported as an open item rather than fixed
 * here, because moving them to a cold-start fetch changes startup behaviour in three Lambdas and needs new
 * execution-role grants.
 *
 * ⚠️ The register is what this test is FOR, and it proved that on its first run: the list was written with
 * the TWO embeds a review had named, and the guard immediately answered with a third — `IDP_WEBHOOK_SECRET`,
 * the svix signing secret — which nobody had enumerated. A hand-maintained list cannot detect that the list
 * is incomplete; deriving the set from the synthesized template can. An exact set also means a FOURTH cannot
 * join silently: the next `.unsafeUnwrap()` into an environment fails this test and has to be argued, which
 * is the property the ECS side gets from `AcceptedNagFindings.TASK_ENVIRONMENT_HOLDS_NO_SECRET` and the
 * Lambda side had from nothing at all. Shrinking this list is the fix; growing it needs a reason.
 */
const LAMBDA_ENV_SECRET_KEYS: readonly string[] = [
    'IDP_SECRET_KEY',
    'IDP_WEBHOOK_SECRET',
    'SERVICE_ERASURE_SIGNING_KEY',
];

describe('⛔ secrets embedded in Lambda environments', () => {
    it('are exactly the two known, deliberate embeds — a third cannot appear unargued', () => {
        ...
        for (const fn of Object.values(functions)) {
            ...
            for (const [name, value] of Object.entries(variables)) {
                if (JSON.stringify(value).includes('{{resolve:secretsmanager')) {
                    embedded.add(name);
                }
            }
        }

        expect([...embedded].sort()).toEqual([...LAMBDA_ENV_SECRET_KEYS].sort());
    });
});

describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Four alarms — the GDPR erasure-incompleteness detector, the two webhook-rejection thresholds and
        // the deletion DLQ's depth — on a stage whose webhook Clerk never calls.
        synthWebhooks(false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    }, 120_000);

    // ⚠️ THE COUNT WAS THREE. `DeletionDlqDepthAlarm` is the fourth, and it is counted here rather than only
    // asserted by name above so that an alarm added without the flag — the exact regression this describe
    // exists for — still fails. The behaviour proved is unchanged: every alarm this stack owns is gated.
    it('creates every webhook alarm when the flag is on', () => {
        // ⚠️ Five since plan U15 added `LogForwarderFailureAlarm`. The COUNT is the exhaustiveness half —
        // a new alarm arriving unnoticed fails here — and the identities below are what make the count mean
        // something, since a count alone cannot tell an added alarm from a renamed one.
        const template = synthWebhooks(true);

        template.resourceCountIs('AWS::CloudWatch::Alarm', 5);
        expect(
            Object.values(template.findResources('AWS::CloudWatch::Alarm'))
                .map((alarm) => String(alarm.Properties?.AlarmName ?? ''))
                .map((name) => name.replace(/-(prod|sandbox|pr-\d+|test)$/u, ''))
                .sort(),
        ).toEqual([
            'kitchensink-deletion-dlq-depth',
            'kitchensink-erasure-incomplete',
            'kitchensink-log-forwarder-failure',
            'kitchensink-webhook-rejected-shape',
            'kitchensink-webhook-rejected-signature',
        ]);
    }, 120_000);

    it('keeps the alarm topic in both states', () => {
        synthWebhooks(false).resourceCountIs('AWS::SNS::Topic', 1);
        synthWebhooks(true).resourceCountIs('AWS::SNS::Topic', 1);
    }, 120_000);
});

describe('WebhooksStack — the queue backstop holds no queue mutation (plan U12)', () => {
    /**
     * ⛔ U12's VERIFICATION, at the template — and it matters most HERE. The deletion queue carries GDPR
     * Art. 17 erasure, and `queueProducerRegister.test.ts` keeps it consumable by exactly one role. That
     * guard reads source text, and its raw-action reader matches `'sqs:<Word>'` — so a wildcard
     * `actions: ['sqs:*']` matches nothing and would join neither the register nor its read-only rule, while
     * IAM expands it into `ReceiveMessage` at evaluation time. The backstop would then be the erasure
     * queue's second consumer, which is the one thing that guard exists to prevent.
     */
    it('⛔ is granted exactly sqs:GetQueueAttributes, and nothing else on any queue', () => {
        expect(sqsActionsGrantedTo(synthWebhooks(true), 'QueueCheckRole')).toEqual(['sqs:GetQueueAttributes']);
    });

    /**
     * ⛔ A scan that found nothing would satisfy the assertion above vacuously. The deletion worker's role in
     * this same template DOES hold receive and delete, so a reader returning an empty set everywhere fails.
     */
    it('⛔ and the reader is not blind — the deletion worker DOES hold receive and delete', () => {
