...
export class WebhooksStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: WebhooksStackProps) {
        ...
        // ⛔ NOT the master credential. Every DB-bound Lambda here used to read the RDS MASTER secret and log in
        // as `identity_app` — a member of `rds_superuser` — so a flaw in any of them reached every database on the
        // instance. They now log in as `identity_service`, a data-only role, by RDS IAM (the role split,
