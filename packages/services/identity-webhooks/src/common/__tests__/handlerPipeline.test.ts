...
describe('withDb', () => {
    it('calls the inner handler with a resolved db/DAO context and returns the inner result', async () => {
        ...
        // The typed config is validated first, then the warm-cached connection factory is asked for the
        // handle — it takes no credential argument, because there is no credential to pass (RDS IAM).
