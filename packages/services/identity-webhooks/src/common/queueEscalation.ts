/**
 * The backstop's Sentry binding for identity-webhooks (plan U12).
 *
 ...
 *
 * ⛔ This number and the EventBridge rule's rate in `WebhooksStack.ts` are the SAME cadence, held together
 * by `__tests__/queueCheckCadence.test.ts`, which READS both. A monitor expecting a check-in more often
 * than the rule fires reports a missed check-in every interval — a permanently-failing monitor for a check
...
const { escalate, checkInQueueCheck } = createQueueEscalation(
    ...
    { service: 'identity-webhooks', intervalMinutes: QUEUE_CHECK_INTERVAL_MINUTES },
);
