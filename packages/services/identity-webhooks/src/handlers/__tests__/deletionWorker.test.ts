...
const { mockFindByIdentityId, mockEraseIdentityRow, mockRunErasureFanout, mockSettleStatusVersion } = vi.hoisted(
    () => ({
        mockFindByIdentityId: vi.fn(),
        mockEraseIdentityRow: vi.fn(),
        mockRunErasureFanout: vi.fn(),
        // U10: the convergence settles how far the provider got, conditionally on the intent not moving.
        mockSettleStatusVersion: vi.fn().mockResolvedValue(true),
    }),
);
...
vi.mock('@kitchensink/identity-db', () => {
    const UserDAO = vi.fn().mockImplementation(function () {
        return { findByIdentityId: mockFindByIdentityId, settleStatusVersion: mockSettleStatusVersion };
    });
...
describe('deletion-worker handler', () => {
    describe('lifecycle routing (event field)', () => {
        /**
         * ⛔ REWRITTEN for U10 (R26/R27). These two used to assert that the EVENT NAME decides — a `closure`
         * message bans, a `reactivation` message unbans — which is precisely the behaviour that made an
         * unordered queue dangerous. This is an SQS STANDARD queue, so a user who closes their account and
         * is then reactivated by an admin can have the two messages delivered in either order, and the
         * worker applied whichever arrived last. The reachable outcome is an account `active` in identity
         * and BANNED at Clerk: the person cannot sign in, and nothing records that the two disagree.
         *
         * The message is now a TRIGGER — "look at this user" — and the DATABASE decides. So what these
         * cases assert is inverted: the same event name produces a different provider call depending on the
         * stored status, which is the whole point.
         */
        it('⛔ a closure trigger on a TOMBSTONED row bans — the status decides, not the event', async () => {
            mockFindByIdentityId.mockResolvedValue({
                id: 'usr_01',
                identityId: 'user_abc',
                status: 'tombstoned',
                statusVersion: 4,
            ...
            expect(mockSettleStatusVersion).toHaveBeenCalledWith('usr_01', 4);
        });

        /**
         * ⛔ THE OUT-OF-ORDER INTERLEAVE, which is the defect in one case. A stale CLOSURE message is
         * delivered after the account was reactivated. Reading the event name would ban a live account.
         */
        it('⛔ a STALE closure trigger on an ACTIVE row UNBANS — the newer intent wins', async () => {
            mockFindByIdentityId.mockResolvedValue({
                id: 'usr_01',
                identityId: 'user_abc',
                status: 'active',
        ...
        it('a reactivation trigger on an ACTIVE row unbans', async () => {
            mockFindByIdentityId.mockResolvedValue({
                id: 'usr_01',
                identityId: 'user_abc',
                status: 'active',
        ...
        /**
         * ⛔ A version that moved during the provider call means a NEWER intent was recorded while we were
         * away, so this result is already stale. Acknowledging would mark the account converged at a state
         * it is no longer in — the divergence the whole function exists to prevent, arriving by another
         * door. It throws so the redelivery applies the newer one.
         */
        it('⛔ THROWS when the intent moved during the provider call, instead of recording it applied', async () => {
            mockFindByIdentityId.mockResolvedValue({
                id: 'usr_01',
                identityId: 'user_abc',
                status: 'tombstoned',
            ...
            mockSettleStatusVersion.mockResolvedValue(false);

            await expect(
                handler(makeSqsEvent({ identityId: 'user_abc', userId: 'usr_01', event: 'closure' }), makeContext()),
            ).rejects.toThrow(/moved during the provider call/);
        });

        /**
         * ⚠️ An ERASED account is completed without a provider call. Erasure has its own path — it deletes
         * at Clerk and fans out to recipe and food — and re-banning or re-unbanning an erased identity would
         * either fail against a deleted Clerk user or resurrect state the erasure removed.
         */
        it('⛔ an ERASED row is completed with NO provider call and no settle', async () => {
            mockFindByIdentityId.mockResolvedValue({
                id: 'usr_01',
                identityId: 'user_abc',
                status: 'erased',
            ...
            expect(mockBanUser).not.toHaveBeenCalled();
            expect(mockUnbanUser).not.toHaveBeenCalled();
            expect(mockSettleStatusVersion).not.toHaveBeenCalled();
        });
    });
    ...
    describe('config guards', () => {
        it('missing DB_HOST (and no DATABASE_URL) → fails fast before touching the DB', async () => {
