...
import { hasTestDatabase, identityDb, openIntegrationDb, resetIdentityRows } from './integrationDb.js';
import { makeIdentityAccount, makeIdentityProfile, makeIdentityUser } from './__fixtures__/makeIdentityUser.js';

describe.skipIf(!hasTestDatabase)('eraseIdentityRow (integration — real Postgres)', () => {
    let pool: pg.Pool;
    ...
    it('scrubs ONLY the target row — a bystander active user keeps every PII column (FR-002/C-007)', async () => {
        ...
        await eraseIdentityRow(db, { userId: targetId, triggerSource: 'admin', actor: 'clerk-webhook' }, now);
    ...
    it('NEVER hard-deletes the users row — it stays present and resolvable by id (R1)', async () => {
        ...
        await eraseIdentityRow(db, { userId: targetId, triggerSource: 'sweep', actor: null }, new Date());
    ...
    it('appends exactly ONE R8 audit row, carrying the caller’s trigger source and actor', async () => {
        ...
        await eraseIdentityRow(db, { userId: targetId, triggerSource: 'sweep', actor: null }, now);
    ...
    it('is ATOMIC — a failure inside the transaction commits NOTHING (no half-erased row, no orphan audit)', async () => {
        ...
        // ⛔ Through the OWNER, not the subject: under the role split `identity_service` holds DML only, so
        // this fault injection is fixture work, spelled as such rather than smuggled in on a superuser
        // connection that would also have hidden a missing grant anywhere else in this suite.
        await identityDb().asOwner(async (owner) =>
            owner.query(
        ...
        try {
            await expect(
                eraseIdentityRow(db, { userId: targetId, triggerSource: 'admin', actor: 'probe' }, new Date()),
            ).rejects.toThrow();
        } finally {
            await identityDb().asOwner(async (owner) =>
                owner.query('ALTER TABLE lifecycle_events DROP CONSTRAINT lifecycle_events_atomicity_probe'),
            );
        }
