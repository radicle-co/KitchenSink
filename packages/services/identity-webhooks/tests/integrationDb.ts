 ...
 *
 * Connecting as `postgres` and replaying the `.sql` files would give the specs a second migration runner and
 * a subject that can do anything. Under the role split (ADR-0039) the deployed webhook Lambdas hold DML and
 * nothing else, so the tier provisions the production role model and hands the specs the SERVICE role: a
 * privilege these handlers do not have in a stage fails here. Fixture work that legitimately needs more
 * (the erasure suite's constraint probe) goes through `identityDb().asOwner`, named at the call site.
 *
