...
describe.skipIf(!hasTestDatabase)('tombstone-sweep selection predicate (integration — real Postgres)', () => {
    let pool: pg.Pool;
    ...
    beforeEach(async () => {
        ...
        // The handler builds its own config and opens its own pool, so it needs a database location: point it
        // at the SERVICE role's connection, which is the one the deployed Lambda holds.
        identityDb().applySubjectEnv();
