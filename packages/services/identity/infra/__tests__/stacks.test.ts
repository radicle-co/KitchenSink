 ...
 * master any more.
 *
 * The service used to receive `DB_USERNAME`/`DB_PASSWORD` from the master secret — so the ECS task logged in as
 * `identity_app`, a member of `rds_superuser`, and a SQL injection anywhere in the service could drop every
 * database on the instance. It now logs in as `identity_service` by RDS IAM, and the runner as
