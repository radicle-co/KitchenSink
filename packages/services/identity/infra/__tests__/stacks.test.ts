...
beforeAll(() => {
    ...
    // The ONE stage that switches ADR-0040 containment off. Constructed here, before any synth, for the same
    // reason as the prod stack above.
    const sandboxService = new IdentityServiceStack(app, 'SandboxService', {
        env,
        stage: 'sandbox',
        domainName: 'example.com',
        imageTag: 'test',
    ...
    sandboxTemplate = Template.fromStack(sandboxService);
});
...
/**
 * ADR-0040 test-principal containment. Owner ruling: ENFORCE on prod, OFF on sandbox, and the service defaults to
 * `enforce` when the variable is unset. So the switch is emitted as `off` on EXACTLY the `sandbox` stage and nowhere
 * else — a prod task carrying `off` would let a test run delete or ban a shared Clerk pool member, so prod is
 * asserted absent-or-`enforce`, and the non-prod `test` stage is asserted absent to pin that the gate is
 * `sandbox`-exact rather than "not prod".
 */
describe('TEST_PRINCIPAL_CONTAINMENT (ADR-0040)', () => {
    /** Every value the task definition gives the variable (one per container that sets it). */
    const containmentValues = (template: Template): unknown[] =>
        Object.values(template.findResources('AWS::ECS::TaskDefinition')).flatMap((task: any) =>
            (task.Properties?.ContainerDefinitions ?? []).flatMap((container: any) =>
                (container.Environment ?? [])
                    .filter((variable: any) => variable.Name === 'TEST_PRINCIPAL_CONTAINMENT')
                    .map((variable: any) => variable.Value),
            ),
        );

    it('sandbox switches containment OFF', () => {
        expect(containmentValues(sandboxTemplate)).toStrictEqual(['off']);
    });

    it('prod never carries off — the variable is absent (service default enforce) or explicitly enforce', () => {
        const values = containmentValues(prodTemplate);

        expect(values).not.toContain('off');
        expect(values.every((value) => value === 'enforce')).toBe(true);
    });

    it('a non-prod stage other than sandbox does not switch it off either (the gate is sandbox-exact)', () => {
        expect(containmentValues(serviceTemplate)).toStrictEqual([]);
    });
});
...
describe('Alarms notify via SNS (A4)', () => {
    ...
    /**
     * ⛔ THE TEMPLATE HALF of the grant `alarmTopicPublishGrant.test.ts` asserts over the sources.
     *
     * The source gate proves the CALL is written; this proves what the call SYNTHESIZES, and the two answer
     * different questions. CDK creates the `enforceSSL` deny and an `addToResourcePolicy` grant through the
     * SAME lazily-created `Topic/Policy` construct, so the correct output is ONE `AWS::SNS::TopicPolicy`
     * carrying BOTH statements. A second `TopicPolicy` for the same topic would be last-writer-wins on
     * `SetTopicAttributes(Policy=…)` — the failure `CostGuardrailsStack` documents and `transportSecurity`
     * already pins there — and would silently drop whichever document lost.
     *
     * The deny is asserted alongside the allow deliberately: a future "fix" that swapped `enforceSSL` for a
     * hand-built `sns.TopicPolicy` would keep the allow, lose the deny, and pass a test that only looked for
     * the grant this outage was about.
     */
    it('synthesizes ONE topic policy carrying both the CloudWatch grant and the TLS deny', () => {
        serviceTemplate.resourceCountIs('AWS::SNS::TopicPolicy', 1);

        const [policy] = Object.values(serviceTemplate.findResources('AWS::SNS::TopicPolicy'));
        const statements = policy?.Properties?.PolicyDocument?.Statement as readonly Record<string, unknown>[];

        const grant = statements.find((statement) => statement['Sid'] === 'AllowCloudWatchAlarmPublish');
        ...
        // The confused-deputy guard: another account's CloudWatch must not be able to publish here.
        expect(JSON.stringify(grant)).toContain('aws:SourceAccount');

        expect(statements.some((statement) => statement['Effect'] === 'Deny')).toBe(true);
    });
});
...
describe('Shared ALB topology (no per-service ALB)', () => {
     ...
     * from `@radicle-co/infra-shared/alb`'s registry (note this stack imports `kitchensink-alb-${stage}`, not
...
const identityTemplate = (stage: string, alarmsEnabled = true): Template => {
    const app = testApp({ context: { ...VPC_LOOKUP_CONTEXT } });
...
describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // A `pr-{N}` preview of this service carried three production alarms — an ALB 5xx threshold, an ECS
        // CPU high-water mark and a boot crash-loop detector — all watching a stage nobody drives, all
        // publishing to a topic with no subscriber.
        identityTemplate('pr-91', false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    });

    it('creates all three service alarms when the flag is on', () => {
        // Non-vacuity for the case above, and the count is the point: this is a switch, not a deletion.
        identityTemplate('prod', true).resourceCountIs('AWS::CloudWatch::Alarm', 3);
    });

    it('keeps the alarm topic in both states', () => {
        identityTemplate('pr-91', false).resourceCountIs('AWS::SNS::Topic', 1);
        identityTemplate('prod', true).resourceCountIs('AWS::SNS::Topic', 1);
    });
});

/**
 * ⛔ The role split (`docs/plans/2026-09-11-database-role-split.md`): nothing identity DEPLOYS connects as the RDS
 * master.
 *
 * A task given `DB_USERNAME`/`DB_PASSWORD` from the master secret logs in as `identity_app`, a member of
 * `rds_superuser`, and a SQL injection anywhere in the service could drop every database on the instance.
 * The service logs in as `identity_service` by RDS IAM, and the runner as
 * `identity_migrator`. These assertions read the synthesized IAM and container definition, so a stack that keeps
 * the secret "just in case" fails here rather than in review.
 */
describe('⛔ identity connects by RDS IAM as its own roles, never as the RDS master', () => {
    type Statement = { readonly Action?: string | readonly string[]; readonly Resource?: unknown };

    /** Every `rds-db:connect` resource the template grants, rendered as JSON. */
    const connectResources = (template: Template): readonly string[] =>
        Object.values(template.findResources('AWS::IAM::Policy'))
            .flatMap((policy) => (policy.Properties?.PolicyDocument?.Statement ?? []) as readonly Statement[])
            .filter((statement) => ([] as string[]).concat(statement.Action ?? []).includes('rds-db:connect'))
            .map((statement) => JSON.stringify(statement.Resource));

    /** The service container's definition. */
    const serviceContainer = (template: Template): Record<string, unknown> => {
        const taskDefinitions = Object.values(template.findResources('AWS::ECS::TaskDefinition'));
        const containers = (taskDefinitions[0]?.Properties?.ContainerDefinitions ?? []) as Record<string, unknown>[];
    ...
    const namesIn = (entries: unknown): readonly string[] =>
        ((entries ?? []) as readonly { readonly Name: string }[]).map((entry) => entry.Name);

    it.each([
        ['test', () => serviceTemplate],
        ['prod', () => prodTemplate],
    ])('%s: the task role may connect as identity_service — and only as identity_service', (_stage, template) => {
        const resources = connectResources(template());

        expect(resources).toHaveLength(1);
        expect(resources[0]).toContain('/identity_service"');
        expect(resources[0]).toContain('DatabaseResourceId');
    });
    ...
    it.each([
        ...
        '%s: the container names identity_service and receives no credential from the master secret',
        (_stage, template) => {
            const container = serviceContainer(template());
            const environment = (container['Environment'] ?? []) as readonly { Name: string; Value: unknown }[];

            expect(environment.find((entry) => entry.Name === 'DB_USERNAME')?.Value).toBe('identity_service');
            expect(namesIn(container['Secrets'])).not.toContain('DB_USERNAME');
            expect(namesIn(container['Secrets'])).not.toContain('DB_PASSWORD');
        },
    );
    ...
    it.each([
        ['test', () => serviceTemplate],
        ['prod', () => prodTemplate],
    ])('%s: nothing in the service stack can read the master secret', (_stage, template) => {
        expect(JSON.stringify(template().toJSON())).not.toContain('DatabaseSecretArn');
    });

    it.each(['test', 'prod'])(
        '%s: the runner may connect as identity_migrator — and only as identity_migrator',
        (stage) => {
            const template = identitySchemaTemplate(stage);
            const resources = connectResources(template);

            expect(resources).toHaveLength(1);
            expect(resources[0]).toContain('/identity_migrator"');
            expect(JSON.stringify(template.toJSON())).not.toContain('DatabaseSecretArn');
        },
    );
});
...
describe('the log drain is attached only when CI resolved a forwarder ARN', () => {
    ...
    it.each([
        ...
        const template = Template.fromStack(
            new IdentitySchemaStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'IdentitySchemaStackDrain', {
                env,
                stage: 'sandbox',
                vpcId: 'vpc-12345678',
