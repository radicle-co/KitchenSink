 ...
 * that, and would need its own `verifyDeployment.sh` invocation to stay covered.
 */
export class IdentitySchemaStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: IdentitySchemaStackProps) {
        ...
        // ⛔ NOT the master credential (`DatabaseSecretArn` — the RDS MASTER, `rds_superuser`). The runner
        // connects as `identity_migrator` by RDS IAM (the
        ...
        const migrationLogGroup = new logs.LogGroup(this, 'IdentityMigrationLogGroup', {
        ...
        if (props.logForwarderArn !== undefined) {
            new logs.SubscriptionFilter(this, 'IdentityMigrationLogDrain', {
