...
export class IdentityServiceStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: IdentityServiceStackProps) {
        ...
        // ⛔ The service no longer receives the RDS MASTER credential. It used to log in as `identity_app` — the
        // master, a member of `rds_superuser` — so any SQL injection in the service reached every database on the
        // instance. It now logs in as `identity_service`, a data-only role, by RDS IAM (the role split,
