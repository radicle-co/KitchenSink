...
export class IdentityServiceStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: IdentityServiceStackProps) {
        ...
        // per-base-stage service (ADR-0005 — tagged with its persistent TIER, never `pr-{N}-…`), so its base platform
        ...
        // ⛔ The service does NOT receive the RDS MASTER credential. The master, `identity_app`, is a member of
        // `rds_superuser`, so any SQL injection in a service logged in as it would reach every database on the
        // instance. It logs in as `identity_service`, a data-only role, by RDS IAM (the role split,
        // `docs/plans/2026-09-11-database-role-split.md`); the resource id is imported so `grantConnect` scopes
        // `rds-db:connect` to exactly that db-user.
        ...
        // needs to read the secrets referenced by the container `secrets:` block (the auth publishable
        // key), and `ecs.Secret.fromSecretsManager` already grants the execution role
        ...
        database.grantConnect(taskRole, DATABASE_ROLES.identity.app);
        ...
        taskDefinition.addContainer('IdentityServiceContainer', {
            ...
            environment: {
                ...
                // ⛔ ADR-0040 test-principal containment. The service defaults to `enforce` when this is unset, which
                // refuses a Clerk test-pool member's self-service erasure and closure (either would delete or ban a
                // shared pool user for good). Owner ruling: OFF on sandbox, where those flows must stay testable, and
                // ENFORCE everywhere else — so `off` is emitted on EXACTLY `sandbox` and prod synthesizes the
                // variable ABSENT. Gated on equality, never on "not prod", so a new stage inherits the safe default.
                ...(stage === 'sandbox' ? { TEST_PRINCIPAL_CONTAINMENT: 'off' } : {}),
        ...
        // single HTTPS listener, owned by @radicle-co/infra-shared/alb (ADR-0003). Identity reads its BASE priority
        ...
        // ⛔ THE ALARMS THEMSELVES ARE GATED; THE TOPIC ABOVE IS NOT. SNS bills per publish and per
        // notification, so a topic with no alarm publishing to it and (outside prod) no subscriber costs
        // nothing at all — while CloudWatch bills per alarm above a 10-alarm free tier, which this account
        // was already over. Gating the topic would also mean threading an `SnsAction | undefined` through
        // every alarm below for a saving of zero. See `packages/infra/global/__tests__/alarmFeatureFlag.test.ts`.
