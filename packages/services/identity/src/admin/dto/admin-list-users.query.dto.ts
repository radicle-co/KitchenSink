/**
 * Query DTO for `GET /api/v1/admin/users`.
 *
 * A thin `nestjs-zod` adapter over the AUTHORED contract in `../admin.schema.ts` (CODING_STANDARDS §15.2).
 *
 ...
 * in the path.
 *
 * ⚠️ A `createZodDto` class carries NO `class-validator` metadata, so this DTO validates NOTHING under Nest's
 * own `ValidationPipe` — it must be reached by `nestjs-zod`'s. This service binds exactly one pipe, globally, as
 * `APP_PIPE` in `app.module.ts`, and `tests/app-validation.test.ts` asserts it is a `ZodValidationPipe` and NOT
 * a `ValidationPipe`. Do not add a route- or controller-scoped `ValidationPipe` here; it would silently disable
 * this validation while leaving every visible sign of it in place.
 */
