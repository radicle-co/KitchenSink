/**
 * Path-parameter DTO for the five admin action routes.
 *
 * A thin `nestjs-zod` adapter over the AUTHORED contract in `../admin.schema.ts` (CODING_STANDARDS §15.2), and
 * the same pattern as `admin-list-users.query.dto.ts`.
 *
 ...
 * `String`. That is precisely how five admin-privileged routes came to hand an unchecked caller-supplied
 * string into a database predicate while reading as validated.
 *
 * ⚠️ A `createZodDto` class carries NO `class-validator` metadata, so this DTO validates NOTHING under Nest's
 * own `ValidationPipe`. The service binds exactly one pipe, globally, as `APP_PIPE` in `app.module.ts`;
