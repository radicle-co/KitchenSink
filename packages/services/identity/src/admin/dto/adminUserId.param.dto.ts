 ...
 * `../admin.schema.ts` (CODING_STANDARDS §15.2), the same pattern as `adminListUsers.query.dto.ts`.
 *
 ...
 * `tests/appValidation.test.ts` asserts it is `nestjs-zod`'s and not Nest's, and
 * `tests/adminParamValidation.test.ts` asserts a known-bad id really is rejected on every route.
 */
