...
export class ClerkAuthService {
    ...
    async verify(token: string): Promise<VerifiedClerkClaims> {
        ...
        // `@clerk/backend`'s `verifyToken` THROWS on failure (caught above) and resolves the verified
        // token on success. Its declared return type is the legacy `{ data, errors }` envelope, but at
        // runtime (>= 1.34) it resolves the BARE JWT payload — the declared types lag the runtime. Accept
        // either a legacy `{ data }` envelope OR the payload itself; still reject a legacy failure (`errors`).
