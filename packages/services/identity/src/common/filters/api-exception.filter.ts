 ...
 * identity's published `openapi.yaml` and `api-error.schema.ts` describe the `400` that way today, and changing it
 ...
 * that is behind again. Regression pinned in `tests/api-exception.filter.test.ts`.
 */
@Catch()
export class ApiExceptionFilter implements ExceptionFilter {
    public catch(exception: unknown, host: ArgumentsHost): void {
        ...
        if (exception instanceof HttpException) {
            ...
            // `packages/infra/global/__tests__/error-envelope-parity.test.ts`, which parses a mechanism envelope
