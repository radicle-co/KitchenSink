...
describe('main.ts boot order', () => {
    ...
    it('binds SCHEMA_PACKAGE_CONTRACT_HASH to the generated leaf package, not to a local constant', () => {
        ...
        expect(source).toContain("from './contract/contractHash.js'");
    });

    // Sentry's instrumentation patches modules on load, so it has to be linked before anything it instruments.
    // It is also why a contract-skew crash is REPORTED rather than dying silently.
    it('keeps ./instrument.js as the very first import', () => {
        expect(source.trimStart().startsWith('/**')).toBe(true);
        const firstImport = /^import .*$/mu.exec(source)?.[0];

        expect(firstImport).toBe("import './instrument.js';");
    });
});
