 ...
 * serving a shape NEITHER stamp describes. The check compares the two stamps to each other, not either of them to
 * the authored corpus, so a hand-built container, a bypassed gate or a partially-applied hotfix is caught here
 * ONLY if it mixed the two stamps, not merely because it skipped generation.
 *
    ...
    public constructor(serviceHash: string, schemaPackageHash: string) {
        super(
            'CONTRACT SKEW — refusing to start. The wire-contract fingerprint compiled into this service does ' +
                'not match the one published by @kitchensink/schema-identity, so the contract every client ' +
