...
describe('identityPoolConfig — who the identity service connects as', () => {
    ...
    it('⛔ connects as identity_service by RDS IAM — never as the master it used to run as', () => {
        // Never the RDS MASTER's password (`identity_app`, rds_superuser) behind a public ALB. The role split
        // gives the service its own login with data privileges only.
