/**
 * The boot-time schema-currency check for the identity.
 *
 * ## Why a boot check when the pipeline already migrates first
 *
 * `kitchensink-identity-schema-{stage}` is deployed and invoked ahead of every consumer (ADR-0035), so the
 ...
 * `migrations/` rather than anywhere more natural. The build mirrors `src/` into `dist/src/`, and the
 * Dockerfile copies the `.sql` to the matching place, so ONE path is correct both from source and inside the
 * image — no probing, and no branch that is only ever exercised in one of the two.
 */
...
import type { IdentityDrizzle } from './database.module.js';
...
/**
 * Check that `the identity database` has applied every migration this release ships.
 *
 ...
 */
export async function verifyIdentitySchemaCurrent(
    db: IdentityDrizzle,
