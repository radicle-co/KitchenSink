 ...
 * which `runMigrations.sh` classifies as a failed run and which fails the deploy. A migration runner that
...
import { applyMigrations, DATABASE_ROLES, parseMigrateEvent } from '@kitchensink/db-schema-guard';
...
export const handler = async (event: unknown = {}): Promise<MigrateResult> => {
    const parsedEvent = parseMigrateEvent(event, 'Identity');
    ...
    try {
        return await runMigrations({
            ...
            expectManifestSha: parsedEvent.expectManifestSha,
            database,
        });
    } finally {
        await pool.end();
