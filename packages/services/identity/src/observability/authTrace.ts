 ...
 *
 * ⛔ NOT ROUTED THROUGH `@kitchensink/service-logging`, AND THE EXEMPTION IS NAMED RATHER THAN IMPLICIT.
 * This is not application logging: it is a flag-gated diagnostic with its own on/off switch (`DEBUG_AUTH`),
 * whose entire value is that a deployed operator can filter one signup's whole flow by `sub` IN SENTRY. The
 * shared rule sends `info` to stdout, which would put this trace in the drain project mixed with every other
 * line — exactly what the flag exists to avoid. So it keeps its own `Sentry.logger.info` sink, and
 * `serviceLoggingInvariants.test.ts` exempts this file BY NAME with this reason attached.
 *
 * ⚠️ ITS OWN `PII_DENYLIST_SUBSTRINGS` IS A SIXTH COPY of the "what may not leave a host" list and is owed a
 * merge into `@kitchensink/observability-scrubbers`. It is not a straight duplicate — it matches key
 * SUBSTRINGS rather than exact names and deliberately keeps booleans and numbers (`emailIsReal: true`) —
 * so folding it in is a behaviour decision, not a move, and it is not this change's business.
 *
 * ⚠️ THE ORIGINAL REASON GIVEN FOR INLINING IS FALSE and is corrected here rather than left to mislead: it
 * said "a shared workspace package that exports raw `.ts` cannot be loaded by the compiled Docker runtime
 * (`node dist/main.js`) — it triggered an ECS boot crash-loop". This service already imports
 * `@kitchensink/observability-scrubbers` and `@kitchensink/service-logging`, both raw-`.ts` workspace
 * packages, in the same image. What actually solves it is the `prod.package.json` + Dockerfile `COPY`
 * convention every shared package here follows.
 */
...
export const scrubAuthAttributes = (attributes: Attributes): Attributes => {
    const out: Attributes = Object.create(null) as Attributes;
 ...
 *
 * The `debug`-drop stays: it exists for framework noise (`NestRoutedLogger` routes Nest's own
