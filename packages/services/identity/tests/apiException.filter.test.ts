 ...
 *   published contract makes (`contract/openapi.ts`, `src/common/apiError.schema.ts`).
 *
...
describe('ApiExceptionFilter (identity)', () => {
    ...
    // `undefined` → `{}` (see `tests/e2e/usersValidation.e2e.test.ts` for the recorded decision).
