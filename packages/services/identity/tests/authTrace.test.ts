/**
 * `observability/authTrace.ts` — the `DEBUG_AUTH` trace of the sign-up → auth → DB flow.
 *
...
import { scrubLog } from '../src/observability/sentryScrubbers.js';
...
/** Import `authTrace` FRESH: it reads `STAGE`/`DEBUG_AUTH` once, at module evaluation. */
async function loadTrace(
    env: Record<string, string | undefined>,
): Promise<typeof import('../src/observability/authTrace.js')> {
    ...
    return import('../src/observability/authTrace.js');
}
