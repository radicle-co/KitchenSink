 ...
 * the bare `/{version}/` path it originally shipped on. `tests/api-route-paths.test.ts` pins that at the
...
// Mirrors users-validation.e2e.test.ts: DatabaseModule builds a `pg` Pool at init and QueueModule an SQS
