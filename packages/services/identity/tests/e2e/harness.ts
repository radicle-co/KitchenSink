 ...
 *
 * Thin wrapper over the generic {@link bootServiceApp} that supplies the identity `AppModule` loader and
 * the env its config schema + database module require. Auth: passing `devAuthUserId` sets
 * `IDENTITY_DEV_AUTH_USER_ID`, so protected routes resolve to that fixed synthetic principal with NO
 * Clerk token and NO DB read-through (see the dev bypass in `auth.middleware.ts`).
 *
 ...
 * @module
 */
import { bootServiceApp, type BootedServiceApp } from './service-harness.js';
