 ...
 *
 * Two prepare steps need the same ids from opposite directions — `prepareDb.ts` SEEDS the warm rows and
 * `prepareClerkTokens.ts` MINTS tokens whose `sub` must match them — so the mapping
 ...
 * in an admin listing.
 *
 * NOT the `adminUserSearch.load.js` `?name=` needle, deliberately: it matches EVERY warm principal —
...
/**
 * Field fragments the BULK filler users are composed from. `prepareDb.ts` inserts those users with one
 ...
 * definitions, and `prepareDb.ts` asserts the rendered row matches {@link bulkIdentity} before running.
 */
 ...
 * (see the exhaustion guard in `provisioningAndRename.load.js`).
 *
 * @param index - Zero-based pool index.
...
/**
 * The single ADMIN principal for `adminUserSearch.load.js`. Seeded like a warm user; its token carries
