 ...
 *     (the same shape as recipe/food's `prepareErasureTokens.ts`).
 ...
 *   npx tsx tests/load/prepareClerkTokens.ts
 *   IDENTITY_COLD_POOL_SIZE=20000 npx tsx tests/load/prepareClerkTokens.ts
 *
...
import { ADMIN_SCOPE, adminIdentity, coldIdentity, warmIdentity } from './poolIdentities.js';
    ...
    JSON.stringify(
        {
            ...
            admin: adminToken,
            reject,
            // Search terms for `adminUserSearch.load.js`. Emitted as DATA so the k6 script never
