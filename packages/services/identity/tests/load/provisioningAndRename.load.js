...
//
// @loadTier substrate-bound — it needs 6,000 never-seen Clerk subs presentable exactly once, which only a runner
//   holding the signing key can mint
//
...
//   k6 run -e IDENTITY_API_BASE_URL=http://localhost:3001 tests/load/provisioningAndRename.load.js
//
 ...
 *
 * WHY. A cold token is only "cold" against a database that has never seen its `sub`. `prepareDb.ts`
