 ...
 * `docs/plans/2026-09-11-database-role-split.md`) — never as a superuser, and no longer as the RDS master it
 * used to run as. A dedicated database, so the suite never touches the one the other suites share; named
 * `…_test` rather than `kitchensink_…`, which is the shape the per-PR reaper's census counts.
 */
const MIGRATE_IT_DATABASE = 'identity_migrate_test';

describe.skipIf(!hasTestDatabase)('identity migration runner (integration)', () => {
    let pool: pg.Pool;
    ...
    beforeEach(async () => {
        ...
        await pool.query(`SET ROLE "${DATABASE_ROLES.identity.owner}"`);
