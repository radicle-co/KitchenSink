...
import { provisionRoleDatabase, roleDatabase } from '@kitchensink/service-test-harness';

import { hasTestDatabase, migrationsDir } from './support/roleDb.js';
 ...
 *
 * Runs against a real Postgres named by `DATABASE_ADMIN_URL`; skips cleanly when none is configured.
 *
 * Applies ONLY the 0005 SQL file to a blank schema — not the full migration chain — because later
 * migrations change the shape this suite asserts on (0009 replaces the plain `users_email_unique` index
 * with a partial one scoped to `deleted_at IS NULL`); running the full chain would pin 0009's contract,
 * not 0005's.
 *
 * ## ⛔ Its own database, and the OWNER's session
 *
 * Two consequences of the role split (ADR-0039), neither optional:
 *
 * 1. It has its OWN database. The other suites share one that `globalSetup` migrates once; a suite that
 *    drops the schema and applies a single migration to it would leave that shared schema at 0005 for
 *    everything that runs after.
 * 2. Its subject is the MIGRATION, so it runs as the database's OWNER — `identity_service` may not create
 *    a table, and `information_schema.constraint_column_usage` returns ZERO ROWS to a non-owner, which
 *    would make the two foreign-key assertions below pass vacuously.
 */

const MIGRATION_PATH = join(migrationsDir, '0005_identity_reset.sql');

/** This suite's own database: it rebuilds the schema from ONE migration and must not disturb the shared one. */
const DATABASE = 'identity_0005_test';

describe.skipIf(!hasTestDatabase)('0005_identity_reset migration (integration)', () => {
    // `migrate` is a no-op: `reset()` must leave an EMPTY schema here, because applying 0005 to it is the
    // subject.
    const spec = {
        roles: DATABASE_ROLES.identity,
        database: DATABASE,
        migrationsDir,
        migrate: async (): Promise<void> => undefined,
    };

    /**
     * The migrator's pool, acting as the OWNER on every connection it opens. `pg` queues queries issued
     * from a `connect` handler ahead of the caller's, so each client is already `SET ROLE`d when it is
     * handed out — the pool-wide form of the `asOwner` bracket the fixture exposes for one-off work.
     */
    let pool: pg.Pool;
    ...
    beforeAll(async () => {
        ...
        pool = new pg.Pool({ connectionString: roleDatabase(spec).migratorUrl, max: 1 });
        pool.on('connect', (client) => {
            void client.query(`SET ROLE "${DATABASE_ROLES.identity.owner}"`);
        });
    }, 120_000);
