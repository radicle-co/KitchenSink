 ...
 * override.
 *
 * Runs against a real Postgres named by DATABASE_ADMIN_URL (a CI service container, or a local
 * throwaway); skips cleanly when none is
 * configured — see `packages/services/identity/infra/docker/compose.yml` to run locally.
 */
...
describe.skipIf(!hasTestDatabase)('user.updated A->B->A profile sync — coherent users/profiles (integration)', () => {
    let pool: pg.Pool;
    ...
    it('logs a warning (does not throw) when the profiles row is missing — an invariant violation', async () => {
        ...
        const warn = vi.fn();
        const daoWithLogger = new UserDAO(db, { warn });
