 ...
 * (the d59e11c fix needed 8/8). Runs against a real Postgres named by DATABASE_ADMIN_URL (a CI service
 * container, or a local throwaway); skips cleanly when none is configured.
 */
...
/** Concurrent attempts per case — the d59e11c fix needed 8/8 to hold. */
const ITERATIONS = 8;

describe.skipIf(!hasTestDatabase)('provisionCompleteUser — race-safety under concurrency (integration)', () => {
    let pool: pg.Pool;
