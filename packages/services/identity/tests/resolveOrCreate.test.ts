...
describe('UsersService.resolveOrCreateFromClaims', () => {
    ...
    beforeEach(async () => {
        ...
        // (`packages/utils/identity/src/provisioning.ts`, proven in `provisioningRace.integration.test.ts`).
    ...
    // ADR-0040: the containment Specification reads `ctx.testPrincipal`, so the context must carry the SIGNED
    // claim through. A context that hard-coded `false` would silently exempt every pool member from containment.
    it.each([true, false])(
        'carries the verified testPrincipal claim (%s) into the authorizer context',
        async (flag) => {
            const userRow = { id: '01EXISTINGUSER000000000000', email: 'a@b.com' };
            mockDb.select = vi
                .fn()
            ...
            expect(ctx.testPrincipal).toBe(flag);
        },
    );
