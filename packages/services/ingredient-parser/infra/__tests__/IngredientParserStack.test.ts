...
import { AdvisoryAwsSolutionsChecks, PYTHON_LAMBDA_RUNTIME } from '@radicle-co/infra-shared/security';
...
function stagedAsset(): string {
    const directory = scratchDirectory('ingredient-parser-asset-');
...
function templateFor(assetDirectory: string): Template {
    const app = new App({ outdir: scratchDirectory('cdk-synth-') });
...
describe('IngredientParserStack', () => {
    ...
    it('refuses to synthesize against an empty staging directory', () => {
        const empty = scratchDirectory('ingredient-parser-empty-');
    ...
    it('tags nothing per-PR at the stack level, leaving the Environment tag to the app', () => {
        ...
        const stack = Stack.of(
            new IngredientParserStack(new App({ outdir: scratchDirectory('cdk-synth-') }), 'IngredientParser-test', {
                env,
                stage: 'pr-1',
                assetDirectory: stagedAsset(),
            }),
...
describe('the log drain is attached only when CI resolved a forwarder ARN', () => {
    ...
    it.each([
        ...
        const template = Template.fromStack(
            new IngredientParserStack(
                new App({ outdir: scratchDirectory('cdk-synth-') }),
                'IngredientParserStackDrain',
                {
                    env,
                    stage: 'pr-1',
                    assetDirectory: stagedAsset(),
