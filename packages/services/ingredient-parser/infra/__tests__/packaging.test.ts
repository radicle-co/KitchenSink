...
describe('distInfoDirectory', () => {
    ...
    /**
     * ⛔ EVERY pinned requirement, not just the first (plan U21). The build read
     * `const [engine] = requirements` and checked that distribution alone — so the moment a second
     * requirement was added, its wheel was installed into the asset and verified by NOTHING. A missing or
     * truncated `sentry-sdk` would then surface as an `ImportError` on the first real invocation, in the
     * one module whose whole job is to report failures.
     */
    it('⛔ resolves a dist-info directory for EVERY pinned requirement in the real file', () => {
        const requirements = readRequirements(path.join(packageRoot, 'requirements.txt'));

        expect(requirements.length).toBeGreaterThan(1);

        const directories = requirements.map((requirement) => distInfoDirectory(requirement));

        expect(new Set(directories).size).toBe(requirements.length);
        for (const directory of directories) {
            expect(directory).toMatch(/^[a-z0-9_]+-[0-9][^/]*\.dist-info$/u);
        }
    });
});
