 ...
 *
 * @param requirements - EVERY pinned requirement. ⛔ Not just the engine's: the build used to read
 *   `const [engine] = requirements` and check that distribution alone, so the moment a second requirement
 *   was added (plan U21's `sentry-sdk`) its wheel was installed into the asset and verified by nothing.
...
function expectationFor(
...
): Parameters<typeof assetViolations>[1] {
    ...
    return {
        ...
        engineRecord: readInstalledRecords(assetDirectory, requirements),
...
function main(): void {
    ...
    const violations = assetViolations(readStagedAsset(assetDirectory), expectationFor(requirements, requested));
