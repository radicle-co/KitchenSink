...
/**
 * pip's `RECORD` rows for EVERY pinned requirement, concatenated.
 *
 * ⛔ A LIST, because `requirements.txt` stopped being one line (plan U21). The build read
 * `const [engine] = requirements` and checked that distribution alone — so the moment a second requirement
 * was added, its wheel was installed into the asset and verified by nothing. A missing or truncated
 * `sentry-sdk` would then surface as an `ImportError` on the first real invocation, in the one module whose
 * job is to report failures.
 *
 * ⚠️ Concatenating is right rather than lazy: `manifestViolations` asks "is every file some upstream
 * authority declared present, and whole?", and that question does not care which distribution a row came
 * from. Each `RECORD`'s paths are already relative to the same install root.
 *
 * @param directory - The staging root.
 * @param requirements - Every pinned requirement line.
 * @returns One entry per recorded file, across all distributions.
 * @sideEffect Reads the files.
 */
export function readInstalledRecords(directory: string, requirements: readonly string[]): readonly RecordedFile[] {
    return requirements.flatMap((requirement) => readEngineRecord(directory, requirement));
}
