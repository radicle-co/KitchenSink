...
import { parseEngineResponse } from '@kitchensink/ingredient-parser';
import { ASSET_DIRECTORY, NLTK_DATA_DIRECTORY } from '../lib/packaging.js';

// ⚠️ `../..`, NOT `..`. This suite moved from the service's own `tests/` into the CDK package's when
// CDK left the npm workspace, and the Python it exercises did NOT move with it: `src/handler.py` and
// the staged `build/asset` tree both still live at the SERVICE root, one level above this package.
// With `..` every path resolved inside `infra/`, and the tier failed with a bare
// `Command failed: python3 -c import json, sys` — the interpreter could not find the module at all.
...
function withUnwritableHome(run: (home: string) => void): void {
    ...
    try {
        ...
        // Restored before removal: `rmSync` cannot unlink through a directory it may not write.
...
describe.skipIf(!engineInstalled())('the staged NLTK corpus, on a filesystem the engine cannot write to', () => {
    beforeAll(() => {
        ...
        if (!existsSync(stagedCorpus)) {
            // The infra directory's own `tsx`, by absolute path — see `packaging.integration.test.ts`.
            execFileSync(path.join(packageRoot, 'infra/node_modules/.bin/tsx'), ['infra/bin/buildAsset.ts'], {
                cwd: packageRoot,
                stdio: 'inherit',
            });
        }
