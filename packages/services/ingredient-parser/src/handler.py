...
# ⛔ Initialised at IMPORT, once, with the engine version as the release — for this service that pin is the
# thing that actually changes behaviour, so a traceback is attributable to the model that produced it.
# Inert without a DSN (see `observability.py`), so every local run and every test is unchanged.
observability.init(ENGINE_VERSION)
...
def parse_line(line: str) -> dict[str, Any]:
    ...
    try:
        ...
        # ⛔ REPORTED as well as returned (plan U21). The per-line `status` is what the CALLER needs, and it
        # deliberately carries only the exception's class name — but that means nobody ever saw the
        # traceback, which is the only thing that names the cause. Reporting it makes the engine's own
        # failures visible as issues, grouped by where they happen rather than by which recipe hit them.
        #
        # ⚠️ Identifiers only, never the sentence: the phrase is a cook's own words, and the point of
        # reporting here is to name the failure without copying them anywhere new.
        observability.report(failure, engine_version=ENGINE_VERSION)
