 ...
 * 3. **THE CAPTURE-POINT MUTATION TEST.** A rating write and both version reads must emit ZERO
 *    `recipe_viewed` events. The rating write still returns the recipe detail through
 *    `RecipesService.getById`, so it is the scenario that fails if anyone "helpfully" moves capture from
 *    the controller handler into the service — that placement would permanently inflate every lifetime
 *    count 015 consumes. The version reads now authorize through the row-only `findReadableRecipe` and never
 *    reach `getById`; they stay as the guard against capture being added to THAT door instead.
 */
