...
describe.skipIf(!hasDatabaseUrl)('recipe read shape — client ↔ server contract conformance', () => {
    ...
    it('returns the cookable content (ingredients + steps) as a typed RecipeDetail (#6b)', async () => {
        ...
        // REWRITTEN to carry `freshness`: the detail now states how current its food data is (KTD-3b), and a
        // user-entered line drew on no food data, so it is `fresh` whatever the food service is doing.
