...
const TEST_DATABASE = 'recipe_droppool_test';

/**
 * ⚠️ The maintenance side is the stand-in RDS MASTER, not a superuser and not the migrator: terminating
 * another role's backend needs `pg_signal_backend`, which the migrator does not hold (ADR-0039) — and
 * terminating a backend is the whole subject here.
 */
describe.skipIf(!hasTestDatabase)('a pool whose database will be dropped', () => {
    let maintenancePool: pg.Pool;
    let urlFor: (database: string) => string;
    ...
    beforeAll(async () => {
        ...
        urlFor = (database: string): string => provisioned.urlFor(DATABASE_ROLES.recipe.app, database);
        maintenancePool = new pg.Pool({ connectionString: provisioned.masterUrl });
        await maintenancePool.query(`DROP DATABASE IF EXISTS "${TEST_DATABASE}" WITH (FORCE)`);
        await maintenancePool.query(`CREATE DATABASE "${TEST_DATABASE}" OWNER "${DATABASE_ROLES.recipe.owner}"`);
    });
