 ...
 * executed against a migrated recipe database, and neither had ADR-0006's `ensureDatabaseExists` per-PR path.
 * (The runner's own `dropDatabase` door is gone: the per-PR reaper is the one drop authority, ADR-0031.)
 *
...
import { hasTestDatabase } from '../../../tests/support/roleDb.js';
 ...
 */
// ⚠️ `kitchensink_recipes_…`, not `…_test`: the runner REFUSES to create a database whose name is not a
// valid recipe database name (`isValidRecipeDatabaseName`, ADR-0006), and creating one through the runner's
// own path is this suite's subject. The suite drops it in `afterAll`.
...
describe.skipIf(!hasDatabaseUrl)('recipe migration runner (ADR-0022, ADR-0006)', () => {
    /**
     * ⚠️ As the production principals (role split, `docs/plans/2026-09-11-database-role-split.md`): the MIGRATOR
     * creates the per-PR database through the runner's own path — `OWNER recipe_owner` — and runs every migration
     * acting as the owner. The stand-in MASTER only drops the database around the suite, because a FORCE drop
     * must terminate the migrator's own backends and only the master holds `pg_signal_backend`; dropping a per-PR
     * database for real is the reaper's job (ADR-0031), proven in its own integration suite.
     */
    let maintenancePool: pg.Pool;
    let pool: pg.Pool;
    let provisioned: RdsLikeDatabase;
    let master: pg.Pool;
    const migratorUrl = (): string => provisioned.urlFor(DATABASE_ROLES.recipe.migrator, TEST_DATABASE);
    ...
    beforeAll(async () => {
        ...
        master = new pg.Pool({ connectionString: provisioned.masterUrl, max: 1 });
        await master.query(`DROP DATABASE IF EXISTS "${TEST_DATABASE}" WITH (FORCE)`);
        maintenancePool = new pg.Pool({ connectionString: provisioned.migratorMaintenanceUrl, max: 1 });
        await ensureDatabaseExists({ maintenancePool, databaseName: TEST_DATABASE });
        pool = poolForDroppableDatabase(migratorUrl());
    });
    ...
    beforeEach(async () => {
        ...
        await pool.query(`SET ROLE "${DATABASE_ROLES.recipe.owner}"`);
        await pool.query('DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;');
        await pool.query('RESET ROLE');
    });

    it('created the per-PR database OWNED BY THE OWNER, not the migrator that ran the CREATE', async () => {
        const owner = await master.query<{ owner: string }>(
            'SELECT pg_get_userbyid(datdba) AS owner FROM pg_database WHERE datname = $1',
            [TEST_DATABASE],
        );

        expect(owner.rows[0]?.owner).toBe(DATABASE_ROLES.recipe.owner);
    });
    ...
    it('rolls a failing migration back and leaves it UNRECORDED, so a half-applied schema never reads as done', async () => {
        ...
        const scratch = scratchDirectory('recipe-migrations-');
        ...
        await expect(
            runMigrations({
                ...
                expectManifestSha: readMigrationManifest(scratch).sha,
                database: TEST_DATABASE,
            }),
    ...
    it('⛔ SERIALIZES two runners racing the same database — check-then-apply is not atomic on its own', async () => {
        ...
        const other = poolForDroppableDatabase(migratorUrl());
