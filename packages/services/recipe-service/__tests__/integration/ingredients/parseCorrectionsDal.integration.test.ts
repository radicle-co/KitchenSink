...
describe.skipIf(!hasDatabaseUrl)('ParseCorrectionsDal', () => {
    ...
    afterEach(async () => {
        ...
        await pool.query('DELETE FROM test_principals WHERE user_id IN ($1, $2, $3)', [COOK_A, COOK_B, COOK_C]);
    });
     ...
     * @param excludeTestPrincipals - Whether the stage contains test principals (ADR-0040); production's `true`.
    ...
    async function correct(
        ...
        return dal.runInTransaction(async (tx) => {
            const writeFacts = await dal.findWriteFacts(k, userId, facts, { excludeTestPrincipals }, tx);
    ...
    describe('findWriteFacts — ADR-0040: a registered test principal never corroborates a real correction', () => {
        // ⛔ The twin of `resolutionMappingsDal.integration.test.ts`'s case. The scope is a REQUIRED parameter so the
        // route that makes this tier live cannot compile without deciding; these rows prove the decision it is handed
        // is carried out by the statement, against the real registry.
        it('⛔ excludes a registered test principal’s row, so a real cook’s matching correction does NOT promote', async () => {
            await pool.query('INSERT INTO test_principals (user_id) VALUES ($1)', [COOK_B]);
            await correct('test pair line', COOK_B, FLOUR);

            const result = await correct('test pair line', COOK_A, FLOUR);

            // Without the exclusion the real cook's correction would cite the test row and bind the line globally —
            // a promotion the test purge's cascade would later destroy under every real cook relying on it.
            expect(result.written && result.promotion).toBeUndefined();
            const global = await pool.query(
                `SELECT 1 FROM ingredient_parse_corrections WHERE normalized_key = $1 AND scope = 'global'`,
                [key('test pair line')],
            );
            expect(global.rows).toHaveLength(0);
        });

        it('still counts the SAME row where containment is off (sandbox and pr-{N}) — the exclusion is the stage’s', async () => {
            await pool.query('INSERT INTO test_principals (user_id) VALUES ($1)', [COOK_B]);
            await correct('test pair line off', COOK_B, FLOUR, [], false);

            const result = await correct('test pair line off', COOK_A, FLOUR, [], false);

            expect(result.written && result.promotion).toBeDefined();
        });

        it('counts an UNREGISTERED cook exactly as before, with the exclusion on', async () => {
            await correct('real pair line', COOK_B, FLOUR);

            const result = await correct('real pair line', COOK_A, FLOUR);
    ...
    describe('two FIRST-TIME cooks correcting one line (no live row to lock)', () => {
        ...
        it('serialises them so the second sees the first and PROMOTES — the corroboration is never lost', async () => {
            ...
            const writeAs = async (
                ...
                const writeFacts = await dal.findWriteFacts(k, userId, FLOUR, { excludeTestPrincipals: true }, tx);
                const decision = evaluateParseCorrectionWrite({
                    principalKind: 'real',
                    containment: 'enforce',
                    correctedAnswer: writeFacts.canonicalAnswer,
