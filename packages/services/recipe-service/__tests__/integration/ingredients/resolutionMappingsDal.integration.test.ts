...
describe.skipIf(!hasDatabaseUrl)('ResolutionMappingsDal', () => {
    ...
    afterEach(async () => {
        ...
        await pool.query('DELETE FROM test_principals WHERE user_id IN ($1, $2)', [AUTHOR_A, AUTHOR_B]);
    });
    ...
    async function correct(
        ...
        const k = key(phrase);
        const facts = await dal.findWriteFacts(k, userId, foodId, { excludeTestPrincipals });
        // A REAL principal on an enforcing stage — the production case (ADR-0040).
        const decision = evaluateMappingWrite({
            principalKind: 'real',
            containment: 'enforce',
            correctedFoodId: foodId,
            grantedScopes,
            ...facts,
        });
    ...
    describe('findWriteFacts — ADR-0040: a registered test principal never corroborates a real correction', () => {
        it('⛔ excludes a registered test principal’s row from the corroborator set, so a real correction does NOT promote', async () => {
            // AUTHOR_B is a signed test principal the registry knows; its correction is an ordinary author row.
            await pool.query('INSERT INTO test_principals (user_id) VALUES ($1)', [AUTHOR_B]);
            await correct('test pair phrase', AUTHOR_B, FOOD_A);

            const result = await correct('test pair phrase', AUTHOR_A, FOOD_A);

            // Without the exclusion the real user's correction would cite the test row and bind the phrase
            // globally for every real user — a promotion no cleanup can undo.
            expect(result.written && result.promotion).toBeUndefined();
            const global = await pool.query(
                `SELECT 1 FROM ingredient_resolution_mappings WHERE normalized_key = $1 AND scope = 'global'`,
                [key('test pair phrase')],
            );
        ...
        it('still counts the SAME row where containment is off (sandbox and pr-{N}) — the exclusion is the stage’s', async () => {
            await pool.query('INSERT INTO test_principals (user_id) VALUES ($1)', [AUTHOR_B]);
            await correct('test pair phrase off', AUTHOR_B, FOOD_A, [], false);

            const result = await correct('test pair phrase off', AUTHOR_A, FOOD_A, [], false);

            expect(result.written && result.promotion).toBeDefined();
        });

        it('counts an UNREGISTERED author exactly as before, with the exclusion on', async () => {
            await correct('real pair phrase', AUTHOR_B, FOOD_A);

            const result = await correct('real pair phrase', AUTHOR_A, FOOD_A);
