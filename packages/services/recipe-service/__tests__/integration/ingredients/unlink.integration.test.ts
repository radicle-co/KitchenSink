...
describe.skipIf(!hasDatabaseUrl)('ingredient unlink (integration)', () => {
    ...
    describe("the target it names is the SERVER's own answer", () => {
        it('reports the database and role the connection actually reached, not the connection string', async () => {
            ...
            expect(target).toMatchObject({ database: roleDb.database });
    ...
    describe('the post-condition rolls the transaction back', () => {
        ...
        beforeEach(async () => {
            // ⛔ Fixture DDL, so it runs as the OWNER: `recipe_app` may not create a function or a trigger.
        ...
        afterEach(async () => {
            await roleDb.asOwner(async (owner) => {
                await owner.query('DROP TRIGGER IF EXISTS u12a_smuggle_link_trigger ON ingredients');
                await owner.query('DROP FUNCTION IF EXISTS u12a_smuggle_link()');
            });
