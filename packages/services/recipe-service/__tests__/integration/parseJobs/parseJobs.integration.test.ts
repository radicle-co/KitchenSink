...
describe.skipIf(!hasDatabaseUrl)('recipe parse jobs (integration)', () => {
    ...
    /**
     * ⛔ U6: THE RETRY MUST CLEAR THE CLAIM COUNTERS, or it is a button that does nothing.
     *
     * A line becomes `failed_retryable` precisely BY exhausting its delivery allowance, so it re-enters the
     * queue with `attempts` already at the limit. Reset only the status and the first delivery of the retry
     * claims, sees itself over budget and marks the line retryable again — a loop the cook can press
     * forever, each press costing a queue round trip and changing nothing.
     *
     * The lease clears for a second reason: a retry enqueued within the claim-lease window would be refused
     * as a duplicate of the delivery that just failed. And `failure_code` described the previous attempt, so
     * it must not outlive it.
     */
    it('⛔ retry clears attempts, the claim lease and the failure code — not just the status (U6)', async () => {
        const job = await createJob('1 cup exhausted milk');
        await drainQueue(1);
        await pool.query(
            `UPDATE recipe_parse_job_lines
                SET status = 'failed_retryable',
                    ...
                    failure_code = 'delivery_allowance_exhausted'
              WHERE job_id = $1`,
            [job.id],
        );
        ...
        const after = await pool.query(
            'SELECT status, attempts, last_received_at, failure_code FROM recipe_parse_job_lines WHERE job_id = $1',
            [job.id],
        );
    ...
    /**
     * ⛔ U6, the other re-enqueue door. The stale digest already stops the OLD message landing, so this is
     * not about the edited line's own answer — it is about the NEW message claiming against `attempts`
     * inherited from a phrase it has nothing to do with. Without the reset, a cook who edits a line after it
     * exhausted its allowance is refused on the first delivery: fixing the thing we asked them to fix, and
     * being ignored for it.
     */
    it('⛔ editing a line clears the claim counters too, so the new phrase starts from zero (U6)', async () => {
        const job = await createJob('1 cup mystery');
        await drainQueue(1);
        await pool.query(
            `UPDATE recipe_parse_job_lines
                SET attempts = 20, last_received_at = now(), failure_code = 'delivery_allowance_exhausted'
              WHERE job_id = $1`,
            [job.id],
        );
        ...
        const edit = await fetch(`${baseUrl}/api/v1/recipe-parse-jobs/${job.id}/lines/0`, {
            method: 'PATCH',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ sourceLine: '1 cup buttermilk' }),
        });

        expect(edit.status).toBe(202);

        const after = await pool.query(
            'SELECT attempts, last_received_at, failure_code FROM recipe_parse_job_lines WHERE job_id = $1',
            [job.id],
        );

        expect(after.rows[0]).toEqual({ attempts: 0, last_received_at: null, failure_code: null });
    });
