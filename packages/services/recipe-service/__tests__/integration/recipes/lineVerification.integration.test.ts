 ...
 * tier can produce a withdrawal: `ingredients.food_resolution_status` is a persisted MIRROR that no
 * withdrawal refreshes, which is precisely why the presence overlay reads the live value instead.
 */
let liveFoodStatus = 'RESOLVED';

/** A stub standing in for food's `GET /api/v1/foods/nutrition`, answering with real nutrition. */
...
describe.skipIf(!hasDatabaseUrl)('U14 — a verification verdict reaches the cook (integration)', () => {
    ...
    /**
     * ⛔ THE PRIVACY ANSWER OUTRANKS THE FACTUAL ONE, and until this test nothing asserted it at any tier.
     *
     ...
     *
     * ⚠️ All three functions were unit-tested individually, and `viewerLineStatus`-last was pinned relative
     * to `resolveLineStatus` — but no test at any tier drove a LIVE `WITHDRAWN` status together with the
     * privacy fact, so the presence-vs-viewer half of the composition was guarded by two docstrings and
     * nothing else. Inverting the two would leak, to a stranger, that a private food they cannot see once
     * existed and has been withdrawn.
     *
     * ⛔ Verified to FAIL when the composition is inverted — swap `viewerLineStatus` and
     * `foodPresenceStatus` in `recipeDetail.assembler.ts` and this returns `FOOD_REMOVED`.
     */
