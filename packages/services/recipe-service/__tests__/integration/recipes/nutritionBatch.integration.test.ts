...
async function startFoodStub(): Promise<{ server: Server; origin: string }> {
    const server = createServer((req, res) => {
        ...
        if (url.startsWith('/api/v1/foods/nutrition') && foodOutage) {
...
describe.skipIf(!hasDatabaseUrl)('POST /api/v1/recipes/nutrition-batch (integration)', () => {
    ...
    it('⛔ marks the figure STALE when food is unreachable and it was served from the recipe service’s cache', async () => {
        // KTD-3b: serve stale, MARKED. Warm the cache HERE rather than lean on the previous case; with food down,
        // the same figure comes back from cache — and says so, rather than reading as a fresh fetch. Its own
        // recipe id: re-seeding one another case created would add a second ingredient line and double it.
        const id = recipeId(90);
        await seedRecipe({ id });
        expect(((await (await askFor([id])).json()) as NutritionBody).nutrition[id]).toMatchObject({
            freshness: 'fresh',
        });
        foodOutage = true;
