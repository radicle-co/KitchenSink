 ...
 * and covered here — is retention (DB row AND S3 object), owner-list exclusion, re-delete idempotency, and
 * the row-only read the authorization checks use applying the same tombstone filter.
 *
...
import type { RecipesDal } from '../../../src/recipes/dal/recipes.dal.js';
import { RECIPES_DAL } from '../../../src/recipes/recipes.tokens.js';
...
describe.skipIf(!hasDatabaseUrl)('recipe soft-delete tombstones (C-007 integration)', () => {
    ...
    it('findRowById reads the active ROW alone, and hides a tombstone exactly as findById does', async () => {
        // The authorization-only reads (photos, versions, ratings) load this row instead of the whole
        // aggregate, so it must apply the SAME tombstone filter — a row read that returned a deleted recipe
        // would re-open every route those checks guard.
        const dal = booted.app.get<RecipesDal>(RECIPES_DAL);
        const id = await createRecipe('Row Read');

        expect(await dal.findRowById(id)).toMatchObject({ id, ownerId: OWNER });

        expect((await fetch(`${baseUrl}/api/v1/recipes/${id}`, { method: 'DELETE' })).status).toBe(204);
        expect(await dal.findRowById(id)).toBeUndefined();
        expect(await dal.findRowById('00000000-0000-4000-8000-00000000dead')).toBeUndefined();
    });
});
