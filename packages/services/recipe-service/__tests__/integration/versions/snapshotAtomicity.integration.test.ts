...
describe.skipIf(!hasDatabaseUrl)('a recipe save and its version row are atomic (integration)', () => {
    ...
    afterEach(async () => {
        await roleDb.asOwner(async (owner) => {
            await owner.query('DROP TRIGGER IF EXISTS atomicity_probe ON recipe_versions');
            await owner.query('DROP TRIGGER IF EXISTS atomicity_probe_outbox ON recipe_version_pending_archives');
        });
    });
    ...
    afterAll(async () => {
        await roleDb.asOwner(async (owner) => {
            await owner.query('DROP TRIGGER IF EXISTS atomicity_probe ON recipe_versions');
            await owner.query('DROP FUNCTION IF EXISTS atomicity_probe_raise()');
        });
        await booted.close();
    });
     ...
     *
     * ⛔ Through the OWNER, never the subject's own handle. This suite used to issue its fixture DDL on the
     * app's drizzle connection, which made the subject an administrator of its own schema — and under the
     * role split `recipe_app` cannot create a function at all (`permission denied for schema public`).
     */
    async function breakVersionInserts(): Promise<void> {
        await roleDb.asOwner(async (owner) => {
            await owner.query(`CREATE OR REPLACE FUNCTION atomicity_probe_raise() RETURNS trigger AS $$
                BEGIN RAISE EXCEPTION 'atomicity probe: recipe_versions insert refused'; END;
                $$ LANGUAGE plpgsql`);
            await owner.query(`CREATE TRIGGER atomicity_probe BEFORE INSERT ON recipe_versions
    ...
    it('⛔ persists NO recipe when the ARCHIVE OUTBOX row cannot be written', async () => {
        ...
        await roleDb.asOwner(async (owner) => {
            await owner.query(`CREATE OR REPLACE FUNCTION atomicity_probe_raise() RETURNS trigger AS $$
                ...
                $$ LANGUAGE plpgsql`);
            await owner.query(`CREATE TRIGGER atomicity_probe_outbox BEFORE INSERT ON recipe_version_pending_archives
                FOR EACH ROW EXECUTE FUNCTION atomicity_probe_raise()`);
        });
        ...
        try {
            ...
            await roleDb.asOwner(async (owner) =>
                owner.query('DROP TRIGGER IF EXISTS atomicity_probe_outbox ON recipe_version_pending_archives'),
            );
