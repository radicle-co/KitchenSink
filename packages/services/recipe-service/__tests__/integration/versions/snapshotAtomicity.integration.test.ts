...
describe.skipIf(!hasDatabaseUrl)('a recipe save and its version row are atomic (integration)', () => {
     ...
     *
     * ⛔ Through the OWNER, never the subject's own handle. Fixture DDL on the app's drizzle connection would
     * make the subject an administrator of its own schema — and under the role split `recipe_app` cannot
     * create a function at all (`permission denied for schema public`).
     */
