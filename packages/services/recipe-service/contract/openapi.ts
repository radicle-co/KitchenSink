...
import { testResetAcceptedResponseSchema, testResetJobResponseSchema } from '../src/account/testReset.schema.js';
import {
...
export const openApiComponents = {
    ...
    TestResetAcceptedResponse: testResetAcceptedResponseSchema,
    TestResetJobResponse: testResetJobResponseSchema,
} as const;
...
/**
 * The `403` a CONTAINED write answers (ADR-0040): a signed test principal attempted, on a stage that contains test
 * principals, a write whose effect would reach real users' data. Its own code, so a harness can tell "contained, as
 * designed" from a scope gate.
 */
const TEST_PRINCIPAL_CONTAINED =
    'A signed test principal attempted a write this stage contains (ADR-0040) — `code: TEST_PRINCIPAL_CONTAINED`.';

/** A `403` response documenting ONLY the containment refusal. */
const CONTAINED_403: OpenApiResponse<ComponentName> = {
    description: TEST_PRINCIPAL_CONTAINED,
    schema: 'ErrorResponse',
};
...
const paths: Readonly<Record<string, Partial<Record<HttpMethod, Operation>>>> = {
    ...
    '/api/v1/recipes/{id}/visibility': {
        patch: {
            ...
            responses: {
                '200': { description: 'The updated recipe.', schema: 'RecipeDetail' },
                '403': {
                    description: `The caller does not own the recipe, or: ${TEST_PRINCIPAL_CONTAINED}`,
                    schema: 'ErrorResponse',
                },
                '404': RECIPE_NOT_FOUND,
                ...sharedErrorResponses(),
    ...
    '/api/v1/recipes/{id}/rating': {
        put: {
            ...
            responses: {
                ...
                '403': {
                    description: `A caller may not rate their own recipe, or: ${TEST_PRINCIPAL_CONTAINED}`,
                    schema: 'ErrorResponse',
                },
                '404': RECIPE_NOT_FOUND,
                ...sharedErrorResponses(),
    ...
    '/api/v1/collections/{id}': {
        ...
        patch: {
            ...
            responses: {
                ...
                '403': {
                    description: `The caller does not own the collection, or: ${TEST_PRINCIPAL_CONTAINED}`,
                    schema: 'ErrorResponse',
                },
                '404': { description: 'No such collection.', schema: 'ErrorResponse' },
                ...sharedErrorResponses(),
    ...
    '/api/v1/account/test-reset': {
        post: {
            operationId: 'requestTestReset',
            summary: 'Purge everything the calling TEST PRINCIPAL owns (a fixture door — ADR-0040).',
            description:
                'NOT a product surface. Only a caller whose verified token carries `public_metadata.testPrincipal === true` AND whom the service’s own registry also records may use it; every other caller receives the `404 NOT_FOUND` a path this service does not route answers. There is NO request body: the principal is the token, and a principal can purge only itself. The purge is asynchronous, total (no public carve-out, no pseudonymization) and REPEATABLE — unlike erasure there is no `410`. An in-flight job is returned rather than a second one started, and while it runs the principal’s writes answer `423`. Poll `getTestReset` until `completed`.',
            responses: {
                '202': {
                    description: 'The queued or already-running reset job.',
                    schema: 'TestResetAcceptedResponse',
                },
                '404': {
                    description: 'Not found (the caller is not a registered test principal).',
                    schema: 'ErrorResponse',
                },
                ...sharedErrorResponses(),
            },
    ...
    '/api/v1/account/test-reset/{jobId}': {
        get: {
            operationId: 'getTestReset',
            summary: 'Read where one of the calling test principal’s own reset jobs stands.',
            description:
                'The same gate as `requestTestReset`: anyone else — and any id that is not one of the caller’s own jobs, including a malformed one — receives `404 NOT_FOUND`. The id is deliberately not validated by the request parser, so a `400` cannot reveal that the route exists.',
            parameters: [
                { name: 'jobId', in: 'path', required: true, description: 'The reset job id.', schema: z.string() },
            ],
            responses: {
                '200': { description: 'The job’s status.', schema: 'TestResetJobResponse' },
