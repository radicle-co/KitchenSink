...
/**
 * `CallerToken` is BUILT to be interpolated: its `toString` (and `toJSON`, and `inspect.custom`) render
 * `[REDACTED]`, which is exactly what makes a log line or an error message that mentions one safe — and
 * `CallerToken.test.ts` asserts that template route. So the shared "no object in a template literal" rule admits
 * this one type by name, beside the platform types it already admits (`Error`, `URL`, `URLSearchParams`).
 */
const callerTokenIsTemplateSafe = {
    files: ['src/**/*.ts'],
    rules: {
        '@typescript-eslint/restrict-template-expressions': [
            'error',
            {
                allow: [
                    ...
                    { from: 'file', name: 'CallerToken', path: 'src/auth/CallerToken.ts' },
                ],
            },
        ],
    },
