...
import {
    ...
    RecipeServiceStack,
    testPrincipalContainmentEnvironment,
} from '../lib/RecipeServiceStack.js';
 ...
 * once in `@radicle-co/infra-shared/alb`, whose suite proves disjointness exhaustively; what belongs here is the
...
describe('recipe database name (ADR-0006)', () => {
    ...
    it('lands the derived per-PR database on the API task definition AND the migration runner', () => {
        ...
        // ⛔ THE API READS, THE SCHEMA STACK DERIVES. This container's `DB_NAME` is no longer a synth-time
        // literal: `RecipeSchemaStack` creates the database, publishes the name it used to SSM, and this
        // stack resolves that parameter at deploy time — which is what makes agreement structural rather
        // than two derivations that happen to match. So the assertion resolves the `Ref` back to the
        // parameter path and checks it is THIS stage's, while the producer below is still checked against
        // the literal name, because it is the one place the name is actually decided.
        const json = template.toJSON() as {
            Parameters?: Record<string, { Default?: string }>;
            Resources: Record<string, { Type: string; Properties?: Record<string, unknown> }>;
        };
        const containers = Object.values(json.Resources)
        ...
        const dbName = containers
            ...
            .find((entry) => entry.Name === 'DB_NAME')?.Value;
        const referenced = (dbName as { Ref?: string } | undefined)?.Ref;

        expect(referenced, 'DB_NAME should resolve an SSM parameter, not a literal').toBeDefined();
        expect(json.Parameters?.[referenced ?? '']?.Default).toBe(recipeDatabaseNameParameter('pr-73'));
...
describe('the API container starts with Sentry loaded first (U22a step 2)', () => {
    /**
     * ⛔ THE STACK'S `command` OVERRIDES THE IMAGE'S `CMD`, AND THAT IS WHY THIS ASSERTION EXISTS ON THE
     * TEMPLATE RATHER THAN ON THE DOCKERFILE. The Dockerfile has carried
     * `CMD ["node", "--import", "./dist/src/instrument.js", "dist/src/main.js"]` since U17, under a comment
     * explaining that `@sentry/nestjs` patches modules AS THEY LOAD so anything imported before
     * `instrument.js` is never instrumented. The stack then set `command: ['node', 'dist/src/main.js']`,
     * which discarded it — so `Sentry.init` had never run in the deployed API, in prod or in any sandbox.
     * U17's `Sentry.captureException` in `ApiExceptionFilter` was a no-op the whole time, and nothing failed:
     * the template synthesized, the task started and the health check passed. An absence of telemetry looks
     * exactly like a quiet service.
     *
     * The repo-wide half of this — every service, including ones that do not exist yet — is W6 in
     * `serviceInfraWiringInvariants.test.ts`, which reads the AST and also checks the `CMD` of any service
     * whose stack sets no command at all. This case pins the SYNTHESIZED value for this stack, because that
     * is the artifact CloudFormation acts on.
     */
    it('preloads the instrument bundle in the synthesized container command', () => {
        synthTemplate('pr-73', 'sandbox').hasResourceProperties('AWS::ECS::TaskDefinition', {
...
describe('TEST_PRINCIPAL_CONTAINMENT per stage (ADR-0040, owner ruling 2026-09-13)', () => {
    /** The value the service container is given, or `undefined` when the stack does not set it. */
    function containmentOf(template: Template): unknown {
        const json = template.toJSON() as {
            Resources: Record<string, { Type: string; Properties?: Record<string, unknown> }>;
        };

        return Object.values(json.Resources)
            .filter((resource) => resource.Type === 'AWS::ECS::TaskDefinition')
            .flatMap(
                (resource) =>
                    (resource.Properties?.['ContainerDefinitions'] ?? []) as {
            ...
            .flatMap((container) => container.Environment ?? [])
            .find((entry) => entry.Name === 'TEST_PRINCIPAL_CONTAINMENT')?.Value;
    }

    it('⛔ leaves PROD unset, so the service parses the fail-closed default `enforce` — never `off`', () => {
        expect(containmentOf(synthTemplate('prod', 'prod'))).toBeUndefined();
    });

    it('turns containment OFF on the shared sandbox, where every row is non-production', () => {
        expect(containmentOf(synthTemplate('sandbox', 'sandbox'))).toBe('off');
    });

    it('turns containment OFF on a pr-{N} preview', () => {
        expect(containmentOf(synthTemplate('pr-73', 'sandbox'))).toBe('off');
    });

    it.each(['prod', 'dev', 'test', 'local', 'staging', 'pr-73-food', 'sandbox-2', 'pr-'])(
        '⛔ is an ALLOWLIST — `%s` is not switched off, so a stage named later cannot opt out by its spelling',
        (stage) => {
            expect(testPrincipalContainmentEnvironment(stage)).toEqual({});
        },
    );
});
...
describe('Shared ALB topology (no per-service ALB)', () => {
    ...
    it.each(['prod', 'sandbox', 'pr-91'])(
        '⛔ NEVER ships RECIPE_DEV_AUTH_USER_ID on a deployed stage (%s)',
        (stage) => {
            // The unauthenticated dev bypass is enabled by the PRESENCE of this variable. `auth.middleware.ts`
            // now also requires `NODE_ENV` to be `development`/`test`, which no deployed stage sets — but that
            // is one gate, in another package, and this stack is what decides whether the variable is shipped
            // at all. Two independent reasons the bypass cannot reach a deployed task, so neither one being
            // wrong is sufficient on its own.
            //
            // ⚠️ Deliberately checked on PROD **and** on the non-prod stages: recipe is the one service whose
            // `NODE_ENV` is not hardcoded to `production` (`stage === 'prod' ? 'production' : 'staging'`), so
            // sandbox and every `pr-{N}` are exactly where a shipped bypass variable would have been live.
            const template = synthTemplate(stage, stage === 'prod' ? 'prod' : 'sandbox');
            const envAll = Object.values(template.findResources('AWS::ECS::TaskDefinition')).flatMap((t) =>
                (
                    (t as { Properties?: { ContainerDefinitions?: { Environment?: { Name: string }[] }[] } }).Properties
                        ?.ContainerDefinitions ?? []
            ...
            expect(envAll).not.toContain('RECIPE_DEV_AUTH_USER_ID');
        },
    );
});
...
describe('the schema deploys and migrates AHEAD of the service, in its own stack', () => {
    ...
    it('the schema stack ships exactly one Lambda — the runner — and publishes its name', () => {
        ...
        // ⚠️ An OUTPUT, not an EXPORT: `runMigrations.sh run` reads `describe-stacks --query
...
describe('the log drains are attached only when CI resolved a forwarder ARN', () => {
    ...
    it.each([
        ['known', 1, ARN],
        ['absent', 0, undefined],
    ])('⛔ RecipeSchemaStack: a %s ARN yields %i subscription filter(s)', (_posture, expected, arn) => {
        const template = Template.fromStack(
            new RecipeSchemaStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'RecipeSchemaStackDrain', {
                env: { account: '123456789012', region: 'us-east-1' },
                stage: 'sandbox',
                baseStage: 'sandbox',
                vpcId: 'vpc-12345678',
    ...
    it.each([
        ['known', 1, ARN],
        ['absent', 0, undefined],
    ])('⛔ RecipeServiceStack: a %s ARN yields %i subscription filter(s)', (_posture, expected, arn) => {
        const template = Template.fromStack(
            new RecipeServiceStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'RecipeServiceStackDrain', {
                env: { account: '123456789012', region: 'us-east-1' },
                stage: 'sandbox',
                baseStage: 'sandbox',
                vpcId: 'vpc-12345678',
                ...
                desiredCount: 1,
                cloudfrontUrl: 'https://cdn.example.com',
                foodServiceUrl: 'https://food-sandbox.example.com',
                ...(arn === undefined ? {} : { logForwarderArn: arn }),
            }),
        );
