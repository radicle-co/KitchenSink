...
export class RecipeSchemaStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: RecipeSchemaStackProps) {
        ...
        // NO RDS created here — owned by the global DataStack. `recipe_migrator` connects via RDS IAM auth, so
        ...
        const migrationLogGroup = new logs.LogGroup(this, 'RecipeMigrationLogGroup', {
        ...
        if (props.logForwarderArn !== undefined) {
            new logs.SubscriptionFilter(this, 'RecipeMigrationLogDrain', {
        ...
        const migrationFn = new lambda.Function(this, 'RecipeMigrationFunction', {
            ...
            environment: {
                ...
                DB_USERNAME: DATABASE_ROLES.recipe.migrator,
            },
            vpc,
        ...
        database.grantConnect(migrationFn, DATABASE_ROLES.recipe.migrator);
        ...
        // ⛔ THE NAME THIS STACK ACTUALLY USED, PUBLISHED FOR THE STACKS THAT READ IT. `RecipeServiceStack`
        // and `RecipeWorkersStack` used to re-derive it, each calling `recipeDatabaseNameForStage` with its
        // own arguments — which is how #119 put the API on `kitchensink_recipes_pr_73` while six worker
        // Lambdas, three of them scheduled and destructive, ran against the shared sandbox database.
        //
        // A shared FUNCTION makes agreement likely; a shared LOOKUP makes it structural. This stack creates
        // the database, so it is the authority, and the value below is the one it really used.
        new ssm.StringParameter(this, 'RecipeDatabaseNameParameter', {
            parameterName: recipeDatabaseNameParameter(stage),
            stringValue: recipeDatabaseName,
            description: 'The logical recipe database this stage migrates and reads (ADR-0006, #119).',
        });
