...
import { ErasureLockGuard } from '../erasureLock.guard.js';
...
function contextFor(
    method: string,
    principal: { userId: string; principalKind?: 'real' | 'test' } | undefined,
...
beforeEach(() => {
    dal = makeDal();
    resets = makeDal();
    guard = new ErasureLockGuard(
        dal as unknown as ErasureJobsDal,
        resets as unknown as TestResetJobsDal,
        new Reflector(),
    );
});
...
describe('ADR-0040 — an active TEST RESET locks the test principal’s writes too', () => {
    it('⛔ rejects a test principal’s mutation with ERASURE_IN_PROGRESS while its reset job is in flight', async () => {
        resets.findActiveJob.mockResolvedValue({ id: 'reset-1', status: 'running' });
        const ctx = contextFor('POST', { userId: OWNER, principalKind: 'test' });

        // A write landing mid-purge would re-create data the worker is deleting — the HAZ-052 race, for a reset.
        await expect(guard.canActivate(ctx)).rejects.toSatisfy(
            (err: unknown) => isRecipeDomainError(err) && err.code === RecipeErrorCode.ERASURE_IN_PROGRESS,
        );
        expect(resets.findActiveJob).toHaveBeenCalledWith(OWNER);
    });

    it('allows a test principal’s mutation when no reset is in flight', async () => {
        await expect(guard.canActivate(contextFor('PUT', { userId: OWNER, principalKind: 'test' }))).resolves.toBe(
            true,
        );
    });

    it('⛔ never reads the reset table for a REAL principal — real users pay no extra round trip for a fixture door', async () => {
        resets.findActiveJob.mockResolvedValue({ id: 'reset-1', status: 'running' });

        await expect(guard.canActivate(contextFor('POST', { userId: OWNER, principalKind: 'real' }))).resolves.toBe(
            true,
        );
        expect(resets.findActiveJob).not.toHaveBeenCalled();
    });

    it('lets the exempted reset request itself through while its own job is running', async () => {
        resets.findActiveJob.mockResolvedValue({ id: 'reset-1', status: 'running' });
        const ctx = contextFor(
            'POST',
            { userId: OWNER, principalKind: 'test' },
            ExemptController.prototype.handler,
            ExemptController,
        );
