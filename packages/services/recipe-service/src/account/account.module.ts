...
@Module({
    ...
    imports: [AuthModule],
    controllers: [AccountController, ServiceErasureController, TestResetController],
    providers: [
        ...
        ErasureJobsDal,
        ErasureService,
        // ADR-0040 — the test principal's self-purge. Shares the erasure queue (the worker dispatches on the
        // message kind) and reads the registry `AuthModule` provides.
        TestResetJobsDal,
        TestResetService,
        AccountExportDal,
        {
    ...
    exports: [ErasureService, ErasureJobsDal, TestResetJobsDal],
})
