...
import { ErasureJobsDal } from '../erasureJobs.dal.js';
...
describe('ErasureJobsDal.insertQueuedJob', () => {
    it('inserts a queued job for the owner and returns the new job id', async () => {
        ...
        // `!`, not `?.`: the assertion above has already established `calls[1]`, and short-circuiting to
        // `undefined` here would throw a TypeError out of the property access instead of failing the
        // expectation — an unsafe optional chain, which the commit-time oxlint pass flags.
