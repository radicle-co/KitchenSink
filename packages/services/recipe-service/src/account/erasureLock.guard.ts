 ...
 *
 * **An active TEST RESET locks the same way (ADR-0040).** A test principal's self-purge is the same hard delete run
 * by the same worker module, so a write landing mid-purge is the same race. The reset table is read ONLY for a
 * principal whose signed claim marks it `test` — a real user's mutation pays no extra round trip for a fixture door.
 *
...
import { erasureInProgressError } from './erasureLock.error.js';
...
export class ErasureLockGuard implements CanActivate {
    public constructor(
        ...
        private readonly resets: TestResetJobsDal,
     ...
     * @sideEffect Reads `account_erasure_jobs` via the DAL, ONLY for mutating, non-exempt requests; and
     *   `test_reset_jobs` ONLY when the principal is a signed test principal (ADR-0040).
     */
    public async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        // ADR-0040 — gated on the signed claim, so the reset table is never read for a real user.
        if (request.principal?.principalKind === 'test' && (await this.resets.findActiveJob(ownerId)) !== undefined) {
            throw erasureInProgressError(ownerId);
        }
