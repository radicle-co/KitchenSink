...
/** A REAL actor on an enforcing stage — the production case, which containment must not move (ADR-0040). */
const ACTOR = makeActingPrincipal(OWNER);
/** A signed test principal on an enforcing stage. */
const CONTAINED = makeActingPrincipal(OWNER, { principalKind: 'test' });
...
describe('AnalyticsService.capture (U3 — fire-and-forget, two-tier shed)', () => {
    ...
    it('returns void synchronously — the caller never awaits analytics', async () => {
        const result = service.capture({ type: 'recipe_saved', actor: ACTOR, recipeId: RECIPE });
    ...
    it('inserts the server-door row shape: type, actor, subject, server-stamped occurred_at, NO event_id', async () => {
        service.capture({ type: 'recipe_saved', actor: ACTOR, recipeId: RECIPE });
        await fake.settle();
    ...
    it('a rejected insert is swallowed and warned — never thrown to the caller', async () => {
        ...
        expect(() => {
            service.capture({ type: 'recipe_viewed', actor: ACTOR, recipeId: RECIPE });
        }).not.toThrow();
        await fake.settle();
    ...
    it('a SYNCHRONOUS throw from the db layer is swallowed too', async () => {
        ...
        expect(() => {
            brokenService.capture({ type: 'recipe_saved', actor: ACTOR, recipeId: RECIPE });
        }).not.toThrow();
        await fake.settle();
    ...
    it('sheds CLIENT-DOOR events at the lower threshold while server-door events still land (KTD4)', async () => {
        ...
        service.capture({ type: 'query_outcome', actor: ACTOR, queryText: 'salt', eventId: crypto.randomUUID() });
    ...
    it('ingestBatch lands a multi-row INSERT spelling the PARTIAL ON CONFLICT predicate, and counts RETURNING', async () => {
        ...
        const result = await service.ingestBatch(ACTOR, batch);
        ...
        expect(result).toEqual({ landed: 0, shed: false, contained: false });
    });
    ...
    it('ingestBatch SHEDS the whole batch at the client-door threshold — the ingest door is client-door load', async () => {
        ...
        const saturated = fake.statements().length;
        const result = await service.ingestBatch(ACTOR, [
            {
                type: 'query_outcome' as const,
        ...
        expect(result).toEqual({ landed: 0, shed: true, contained: false });
        expect(fake.statements()).toHaveLength(saturated);
    ...
    // ── ADR-0040: a contained test principal's activity never reaches the store ──────────────────
    it.each(['recipe_saved', 'recipe_viewed'] as const)(
        '⛔ issues NO insert for a contained test principal’s %s — impact counts never decrement, so only prevention works',
        async (type) => {
            service.capture({ type, actor: CONTAINED, recipeId: RECIPE });
            await fake.settle();
    ...
    it('records a test principal’s event where containment is off (sandbox and pr-{N})', async () => {
        service.capture({
            type: 'recipe_saved',
            actor: makeActingPrincipal(OWNER, { principalKind: 'test', containment: 'off' }),
            recipeId: RECIPE,
        });
        await fake.settle();
    ...
    it('⛔ lands NOTHING from a contained test principal’s ingest batch, and reports it as contained, not shed', async () => {
        const result = await service.ingestBatch(CONTAINED, [
            {
                type: 'query_outcome' as const,
                eventId: '99999999-9999-4999-8999-000000000d09',
                occurredAt: '2026-09-01T12:00:00.000Z',
                query: 'load probe',
                served: [],
                outcome: { kind: 'no_pick' as const },
        ...
        expect(result).toEqual({ landed: 0, shed: false, contained: true });
        expect(fake.statements()).toHaveLength(0);
    });
