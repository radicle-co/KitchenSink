...
const OWNER_ID = '01JU4INGESTCONTROLLERUSER0';
/** The verified principal the decorator resolves; the controller hands its acting slice to the service. */
const OWNER = {
    userId: OWNER_ID,
    sub: 'user_clerk',
    scopes: [],
...
function fakeAnalytics(landed = 1): { ingestBatch: ReturnType<typeof vi.fn> } {
    return { ingestBatch: vi.fn().mockResolvedValue({ landed, shed: false, contained: false }) };
}
...
describe('AnalyticsIngestController (U4)', () => {
    it('lands a well-formed batch under the TOKEN principal and reports accepted vs landed', async () => {
        ...
        const [actor, events] = analytics.ingestBatch.mock.calls[0] as [unknown, unknown[]];
        // The verified identity's acting slice — its kind and the stage's containment ride along (ADR-0040).
        expect(actor).toEqual({ userId: OWNER_ID, principalKind: 'real', containment: 'enforce' });
