...
import { isContained } from '../common/containmentPolicy.js';
...
export interface CapturedEvent {
    ...
    /**
     * The verified actor — its owner key, plus ADR-0040's containment facts. The WHOLE acting slice rather than a bare
     * ULID, so containment is decided at this ONE seam: a capture site added tomorrow cannot forget it.
     */
    readonly actor: ActingPrincipal;
...
export class AnalyticsService {
    ...
    public capture(event: CapturedEvent): void {
        // ADR-0040 — FIRST, before the shed accounting: a contained test principal's view or save is never recorded.
        // `recipe_impact_signals` has no DELETE trigger (ADR-0030 §1), so a test view of a real recipe would be a
        // permanent count no purge can take back; prevention is the only answer. Not a drop, so not counted as one.
        if (isContained(event.actor)) {
            return;
        }
    ...
    private async insert(event: CapturedEvent): Promise<void> {
        ...
        if (event.eventId === undefined) {
            await this.db.execute(sql`
                ...
                VALUES (${event.type}, ${event.actor.userId}, ${event.recipeId ?? null}, ${event.queryText ?? null},
        ...
        await this.db.execute(sql`
            ...
            VALUES (${event.eventId}, ${event.type}, ${event.actor.userId}, ${event.recipeId ?? null},
     ...
     *
     * @param actor - The VERIFIED principal every event in the batch is attributed to.
     * @param events - Schema-validated client events (the controller already dropped invalid ones).
     * @returns How many rows landed, whether the batch was shed instead of attempted, and whether it was refused as a
     *   contained test principal's (ADR-0040) — distinct from a shed, which is load, and from a duplicate.
    ...
    public async ingestBatch(
    ...
    ): Promise<{ landed: number; shed: boolean; contained: boolean }> {
        if (events.length === 0) {
            return { landed: 0, shed: false, contained: false };
        }

        // ADR-0040: a contained test principal's query text ("load probe …") would pollute search-outcome analysis.
        if (isContained(actor)) {
            return { landed: 0, shed: false, contained: true };
        }
        ...
        if (this.inFlight >= ANALYTICS_CLIENT_SHED_AT) {
            ...
            return { landed: 0, shed: true, contained: false };
        }
        ...
        try {
            ...
            return { landed: result.rows.length, shed: false, contained: false };
        } finally {
            this.inFlight -= 1;
        }
    }
