...
export class AnalyticsIngestController {
     ...
     *
     * @param principal - The verified caller — the ONLY actor any event is attributed to.
    ...
    public async ingest(
        ...
        const { landed, shed, contained } = await this.analytics.ingestBatch(actingPrincipalOf(principal), accepted);

        // A contained test principal's batch lands nothing BY DESIGN (ADR-0040) — not a dedup gap worth an alarm.
        if (!shed && !contained && landed < accepted.length) {
