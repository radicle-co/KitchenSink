 ...
 *
 * Companion to `@OwnerId()` / `@CurrentPrincipal()` (`currentPrincipal.decorator.ts`), and deliberately
 ...
 * is the authentication assertion. This decorator only supplies the credential a route forwards onward — the
 * ingredients vertical's catalog calls, and every recipes/ratings/versions route that answers with a recipe
 * detail, whose nutrition the food service authorizes AS this caller.
 */
import { createParamDecorator, type ExecutionContext } from '@nestjs/common';

import { CallerToken } from './CallerToken.js';
