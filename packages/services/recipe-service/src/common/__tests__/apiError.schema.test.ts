 ...
 *
 * ⚠️ Nothing here asserts a multi-shape union (Nest's `{ statusCode, message, error? }`, `nestjs-zod`'s
 * validation body, a bare string `429`) on purpose: the filter is the sole author of the envelope, so those shapes
 * are not on the wire, and `apiError.schema.ts` says why a union describing them is refused.
 */
