...
describe('the EMF envelope has exactly one owner in this service', () => {
    it('⚠️ scanned a realistic tree — non-vacuity before any absence claim', () => {
        // ⚠️ The floor exists to catch a scanner that walks NOTHING — a broken `readSourceFiles`, a moved
        // `SRC_ROOT` — because an absence claim over an empty set is free. It is NOT a census of this
        // service, and it must not become a tripwire on every legitimate deletion: it was `> 200` against a
        // tree of 202 and tripped at exactly 200 when two re-export barrels were removed under
        // `CODING_STANDARDS §4`. Slack, not precision, is what this assertion wants — a broken scan returns
        // single digits, not 180.
        expect(sourceFiles.length).toBeGreaterThan(180);
