...
import { recipeErrorCodeSchema, type RecipeErrorCode } from '../apiError.schema.js';
import { RECIPE_ERROR_STATUS, RECIPE_STATUS_CODE } from '../apiError.js';

/**
 * The published failure codes, by name. AUTHORED in `../apiError.schema.ts` (`recipeErrorCodeSchema`) and
 ...
 * prevent. The two shapes are asserted equivalent in `packages/infra/global/__tests__/errorEnvelopeParity.test.ts`
 ...
 * `../__tests__/apiError.schema.test.ts` rather than trusted. The status is NOT decided here: it comes from
 ...
 *
 * ⚠️ `RECIPE_ERROR_STATUS` MOVED OUT of this file to `../apiError.ts` and is re-exported below, because the table
...
export class ApiExceptionFilter implements ExceptionFilter {
    ...
    public catch(exception: unknown, host: ArgumentsHost): void {
        ...
        if (status >= HttpStatus.INTERNAL_SERVER_ERROR) {
            // ⛔ REPORTED, not just logged (plan U17). The line below reaches Sentry as a LOG — since U22a step 2
            // it goes straight to this service's own project rather than via ADR-0042's drain — and a log
            // is a log either way: it does not group, it carries no stack trace Sentry can symbolicate,
            // and nothing alerts on it. A 5xx is a server fault, which is precisely the class that should
            // raise an ISSUE.
            //
            // ⚠️ ONLY a 5xx. A 4xx is expected control flow — a 404 for a recipe that does not exist, a 401
            // for an expired token — and capturing those would create an issue per bad request, which is how
            // a Sentry project becomes unreadable and then ignored.
            Sentry.captureException(exception);

            // ⛔ THE THROWABLE WHOLE, not pre-rendered. A 5xx must log the full error INCLUDING its `cause`
            // chain — `.stack` alone dropped drizzle's wrapped Postgres error, which is why `renderThrowable`
            // exists — but rendering it HERE produced free text the sink had not made, and the sink scrubs
            // what it renders. `@kitchensink/service-logging` owns both halves: it renders the cause chain
            // and scrubs the result. The response still deliberately carries none of it.
            this.logger.error(`${route} -> ${status} ${body.code}: ${body.message}`, { error: exception });
        } else {
...
export { RECIPE_ERROR_STATUS } from '../apiError.js';
export { apiErrorSchema, type ApiErrorBody } from '../apiError.schema.js';
