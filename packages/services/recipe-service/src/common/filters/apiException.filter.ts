...
import { normalizeHttpException, renderThrowable } from '@kitchensink/nest-error-envelope';
...
export class ApiExceptionFilter implements ExceptionFilter {
    ...
    public catch(exception: unknown, host: ArgumentsHost): void {
        ...
        if (status >= HttpStatus.INTERNAL_SERVER_ERROR) {
            // A 5xx is a server fault: log the full throwable INCLUDING its stack AND its `cause` chain, which is
            // the only record of what actually failed — the response deliberately carries none of it. `.stack`
            // alone dropped drizzle's wrapped Postgres error (see `renderThrowable`).
            this.logger.error(`${route} -> ${status} ${body.code}: ${body.message}`, renderThrowable(exception));
        } else {
