 ...
 * @param index - Position within the batch.
 * @returns The entry. Pure.
 */
function entryFor<T>(tenantOf: TenantOf<T> | undefined, message: T, index: number): SendMessageBatchRequestEntry {
        ...
        if (sendable.length > 0) {
            ...
            const results = await Promise.allSettled(
                batched.map(async (batch) =>
                    send({
                        QueueUrl: queueUrl,
                        Entries: batch.map((message, index) => entryFor(tenantOf, message, index)),
                    }),
                ),
            );
