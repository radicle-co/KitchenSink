...
/**
 * How old a previously landed answer may be and still be copied forward, when nothing configures it.
 *
 * ⚠️ SEVEN DAYS, chosen against a hard bound rather than by feel: `expireParseJobs` DELETES a job seven days
 * past its 24-hour expiry, cascading to its lines, so no candidate can ever be older than ~8 days whatever
 * this says. A larger default would be fiction.
 *
 * ⛔ Declared HERE rather than beside the service that reads it, because config owns defaults (as it does
 * for `PORT`) and the reverse import would pull Nest and the whole recipes module into the config leaf.
 */
export const DEFAULT_COPY_FORWARD_MAX_AGE_HOURS = 168;

/**
 * The parse-job hand-off's config contract (plan U9, origin D9/R13) — the queue this service produces onto,
 * and the copy-forward window it applies before producing at all.
 */
export const parseJobConfigSchema = z.object({
    /**
     * URL of the `recipe-parse-line` SQS queue this service enqueues one message per pasted line onto,
     * drained by `parseLine` in `@kitchensink/recipe-workers` (plan U9, origin D9/R13).
     *
    ...
    /**
     * How old a previously landed answer may be and still be COPIED FORWARD instead of re-queued.
     *
     * ⚠️ Defaults to {@link DEFAULT_COPY_FORWARD_MAX_AGE_HOURS}, which carries the derivation — the number
     * is argued there rather than restated here.
     *
     * ⛔ It is a PROXY for version-awareness, never the check — `recipe_parse_job_lines` carries no engine
     * version, so a model pin changed inside the window still serves the old answer (see
     * `recipes/domain/copyForwardPolicy.ts`). `0` disables copy-forward entirely, which is the kill switch
     * for the corrections watermark's install-wide blast radius.
     *
     * ⛔ `z.coerce` because an env var is a STRING: `config.get<number>` would otherwise hand the policy a
     * string that multiplies correctly and compares wrongly — a type lie the schema exists to stop.
     */
    PARSE_COPY_FORWARD_MAX_AGE_HOURS: z.coerce.number().int().min(0).default(DEFAULT_COPY_FORWARD_MAX_AGE_HOURS),
});
...
export const parseJobConfigMeta: Record<keyof ParseJobConfig, ConfigFieldMeta> = {
    ...
    PARSE_COPY_FORWARD_MAX_AGE_HOURS: {
        secret: false,
        description: 'how old a landed parse may be and still be copied forward, in hours (0 disables)',
    },
};
