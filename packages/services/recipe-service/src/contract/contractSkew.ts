 ...
 * (`./contractHash.ts`) and the generated leaf package `@kitchensink/schema-recipe`. This module asserts, at boot, that the image
 ...
 * deploy. That comparison is inherently cross-process: the device's copy of `@kitchensink/schema-recipe` was pinned at ITS build
 ...
 * service; `@kitchensink/schema-recipe` is consumed for types), so nothing in-repo compares this value yet —
    ...
    public constructor(serviceHash: string, schemaPackageHash: string) {
        super(
            'CONTRACT SKEW — refusing to start. The wire-contract fingerprint compiled into this service does ' +
                'not match the one published by @kitchensink/schema-recipe, so the contract every client compiles against does not ' +
