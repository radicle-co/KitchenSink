...
const accounts: readonly ColumnAccount[] = [
    ...
    // ── test principals (ADR-0040, 0044 + 0045) ───────────────────────────────────────────────────
    // Neither table takes a client-supplied value: the registry row and the reset job are keyed on the VERIFIED
    // principal, and the reset request has no body at all.
    { table: 'test_principals', column: 'user_id', why: PRINCIPAL_DERIVED },
    { table: 'test_reset_jobs', column: 'user_id', why: PRINCIPAL_DERIVED },
    {
        table: 'test_reset_jobs',
        column: 'attempts',
        why: 'Server-incremented retry counter; no wire field sets it.',
    },
    ...
    {
        table: 'recipe_verification_attempts',
        column: 'attempts',
        why: "Server-incremented by the U7 verification claim, once per delivery that reaches the database, and the gate STOPS at the stack's CLAIM_ATTEMPT_ALLOWANCE by recording a failure code and letting the line publish unverified. No wire field sets it.",
    },
    {
        table: 'recipe_parse_job_lines',
        column: 'attempts',
        why: "Server-incremented by the U6 parse claim, once per delivery that reaches the database, and the claim STOPS at the stack's CLAIM_ATTEMPT_ALLOWANCE by marking the line failed_retryable; a retry or an edit resets it to 0. No wire field sets it.",
    },
