...
-- columns, no index, no change to any existing column.
--
-- ⚠️ It is the exact MIRROR of food-service's `0008_food_rank_terms.sql`, over `ingredients.name` instead of
-- `food.name`. The two are deliberately separate files: the plan's U5 rule is "shared rule, never shared
-- SQL", the two services own their own schemas, and nothing in this repository can apply one service's
-- migration to the other's database. What keeps them honest is that BOTH are asserted against the SAME
-- TypeScript reference (`@kitchensink/recipe-core/resolution/ranking-terms`) by each service's own
-- integration suite, so a change to the rule that lands in only one mirror fails on the other.
--
-- ## ⛔ WHY THIS EXISTS: the ladder is unaffordable computed per row, and that was MEASURED
--
-- U5 orders `ingredients` by a tier ladder above `word_similarity` (see `ingredients/dal/ingredientRelevance.ts`).
-- The measurements below were taken on food-service's catalog, whose statement is the same shape; the ladder
-- needs
-- the same three things per candidate row: the folded name, its token array, and its head term. Computing them inside
...
-- rewritten and the lock is held for the duration.
...
--
-- No index is added. These columns are a SORT KEY, never a predicate; `idx_ingredients_name_trgm` and
-- `idx_ingredients_search_vector` are untouched, and U5's brief is explicit that the trigram indexes are
-- load-bearing and must not be changed.
--
-- ⚠️ `ingredients.search_vector` is maintained by the DAL on every write, not by a trigger (see
-- `dal/ingredients.dal.ts`'s header). These two columns are the OPPOSITE: Postgres computes them, so no
-- writer can forget them and `updateResolution`'s rename recomputes them for free along with the name.
