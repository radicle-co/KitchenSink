 ...
 *
 * ⛔ This alias is NOT `ParsedLine`, and must not become it. `ParsedLine.raw` is the cook's line BYTE-IDENTICAL
 * (HAZ-041), while `line_digest` below is "the ONLY representation of the cook's line that is stored anywhere in
 * this table". The digest wins, because KTD-14's whole erasure argument depends on it: this table has no owner
 * column and is absent from the account-erasure sweep precisely because it carries no person-to-row link.
 * `CorrectedParse` rules the same way for the sibling table. The writer's half of the rule is enforced by
