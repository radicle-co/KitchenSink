...
/** A factory over the fixed origin with deliberately tiny typeahead, post-commit and read deadlines. */
function clients(
    typeaheadTimeoutMs = 25,
    postCommitNutritionTimeoutMs = 60,
    readNutritionDeadlineMs = 300,
): FoodServiceClients {
    return new FoodServiceClients({
        baseUrl: FOOD_ORIGIN,
        typeaheadTimeoutMs,
...
describe('FoodServiceClients — the three deadlines stay distinct', () => {
    ...
    // ⛔ THE THIRD CONTRACT: a response sent AFTER a write has committed. Its figure is extra information on a
    // result that already succeeded, and a slow food service must not hold that success past the recipe
    // client's own 10 s deadline — so it aborts early and the gateway degrades honestly (stale-marked).
    it('aborts the post-commit nutrition client at ITS deadline', async () => {
        slowFetch(400);

        await expect(clients(25, 60).postCommitNutrition(caller()).search('chicken')).rejects.toMatchObject({
            status: 503,
        });
    });

    it('does NOT abort the READ nutrition client at the post-commit deadline (GET detail keeps its budget)', async () => {
        slowFetch(150);

        await expect(clients(25, 60).nutrition(caller()).search('chicken')).resolves.toEqual({ results: [] });
    });

    // ⛔ THE READ IS BOUNDED TOO, and by its own deadline rather than the client's 8 s default. The GET detail
    // used to take that default per request and issue up to two in series, so a slow food service held a
    // READ for 16 s — past the recipe client's 10 s, with a timeout reported for a read that would have
    // answered. One request may never outlive the deadline of the lookup it belongs to.
    it('aborts the READ nutrition client at the read deadline, not at the 8 s client default', async () => {
        slowFetch(400);

        await expect(clients(25, 60, 100).nutrition(caller()).search('chicken')).rejects.toMatchObject({
            status: 503,
        });
    });
});
...
/**
 * ⛔ THE LOOKUP DEADLINE — one bound over every request a nutrition lookup issues (chunks, waves, authored call).
 *
 * The factory owns the milliseconds of every latency contract, so it answers the call-level deadline too. A read
 * takes its configured deadline; a post-commit response keeps the bound its per-request timeout was derived
 * against — two requests — now ENFORCED rather than merely argued.
 */
describe('FoodServiceClients — the nutrition lookup deadline per budget', () => {
    it("'read' is the configured read deadline", () => {
        expect(clients(25, 60, 3_000).nutritionLookupDeadlineMs('read')).toBe(3_000);
    });

    it("'postCommit' is twice the post-commit per-request timeout — the two-request bound it was sized against", () => {
        expect(clients(25, 1_500, 3_000).nutritionLookupDeadlineMs('postCommit')).toBe(3_000);
        expect(clients(25, 700, 3_000).nutritionLookupDeadlineMs('postCommit')).toBe(1_400);
    });
});
