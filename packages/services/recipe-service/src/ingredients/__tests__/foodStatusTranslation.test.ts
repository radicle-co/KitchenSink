...
import { licensesStatusWrite, toResolutionStatus } from '../foodStatusTranslation.js';

/**
 * ⛔ THE LICENCE QUESTION, asked BEFORE the translation (0016).
 *
 * `WITHDRAWN` is the first status food publishes that recipe's shared catalog row must not record AT ALL —
 * and it is neither a catalog value nor a line value. The four line-only members (`NEEDS_REVIEW`,
 * `PENDING_VERIFICATION`, `AMBIGUOUS`, `RESOLVED_UNAVAILABLE`) are values recipe DERIVES; `WITHDRAWN` is a
 * fact food OWNS, which recipe reads live and never writes down. A withdrawal is presumptively restorable,
 * so a persisted mirror of it goes stale in the WORSE direction — claiming "removed" about a food that came
 * back.
 *
 * So the pair is: this predicate answers whether the status licenses a catalog write, and
 * {@link toResolutionStatus} — narrowed to the statuses that do — answers what to write.
 */
describe('licensesStatusWrite', () => {
    it('refuses WITHDRAWN, the one status the shared catalog row must never record', () => {
        expect(licensesStatusWrite('WITHDRAWN')).toBe(false);
    });

    it('licenses every other status food publishes', () => {
        for (const status of foodStatusSchema.options.filter((value) => value !== 'WITHDRAWN')) {
            expect(licensesStatusWrite(status), `'${status}' should license a catalog write`).toBe(true);
        }
    });

    it('⛔ is exhaustive over food`s published union — a new status is licensed until someone rules', () => {
        // Stated so the DEFAULT is visible rather than implied: an eighth food status passes this predicate
        // and then fails to compile inside `toResolutionStatus`, which is where the decision belongs. The
        // predicate is `!== 'WITHDRAWN'` and NOT a `Record<FoodStatus, boolean>` on purpose — a second total
        // list over the same union is a copy of a list, and a copy cannot detect that the list is incomplete.
        for (const status of foodStatusSchema.options) {
            expect(typeof licensesStatusWrite(status)).toBe('boolean');
        }
    });
});

describe('toResolutionStatus', () => {
    it('⛔ maps EVERY status it accepts to one recipe can persist', () => {
        // The assertion that would have caught the shipped defect. It enumerates food's union from food's
        // OWN published schema rather than a local list, so food adding a member reds here instead of at an
        // INSERT in production — minus the statuses that license no write at all, which is what the
        // predicate above is for.
        for (const status of foodStatusSchema.options) {
            if (!licensesStatusWrite(status)) {
                continue;
            }
    ...
    it('is total over every status it accepts — no status falls through to undefined', () => {
        // ⛔ TOTAL AND NON-NULLABLE, deliberately. Widening the return to `… | undefined` to express "leave
        // the mirror alone" would have downgraded the U9 tripwire from TS2366 ("function lacks ending return
        // statement and return type does not include 'undefined'" — a property of the TYPE) to TS7030
        // ("not all code paths return a value" — a property of the `noImplicitReturns` COMPILER FLAG, which
        // no guard in this repo pins). The refusal lives in the narrowed PARAMETER instead.
