 ...
 * `recipe_ingredients`). ⚠️ It is NOT free of an ownership predicate: `food_owner_id` (migration
 * `0040_private_food_scoping.sql`, plan U11 / R20) is the admitting AUTHOR's ULID when the row points at that
 * author's PRIVATE authored food, and {@link IngredientsDal.search} wraps every retrieval branch in
 * `food_owner_id IS NULL OR food_owner_id = :caller`. A catalog row is ownerless; a row standing for somebody's
 * private food is visible only to them.
 *
...
export function rowToIngredient(row: RawIngredientRow): Ingredient {
    return {
        ...
        //
        // ⚠️ The column is NOT unconstrained: `0001_initial.sql`'s `ingredients_food_resolution_status_check`
        // covers exactly the five catalog values (mirrored in the drizzle schema, dropped by no migration), so
        // the database is what forbids the mistaken `NEEDS_REVIEW` 0023 rules out, not this parse. The parse
        // earns its place anyway: the CHECK is invisible to `tsc`, and widening it is a migration this build
        // would not learn about.
...
export class IngredientsDal {
     ...
     *
     * ⚠️ It returns NO nutrition: `RETURNING` carries no nutrition column, because migration 0019 dropped them
     * all (see that constant's own note).
     *
