 ...
 *
 * ⚠️ The same TRANSACTION and the same cast, NOT the same statement — it is four statements: the `FOR UPDATE`
 * lock read, a standalone `SELECT :proposal::jsonb::text`, and one read
...
import type { CorroboratorScope } from '../../common/containmentPolicy.js';
...
export class ParseCorrectionsDal {
     ...
     * `pg_advisory_xact_lock(ADVISORY_LOCK_CLASSES.recipeParseCorrection, hashtext(key))` exists whether or not any row does; it is taken FIRST, and the
     ...
     * @param corroborators - Whether a registered test principal's row may count (ADR-0040). REQUIRED, and before the
     *   optional `writer`, so the route that makes this tier live cannot compile without deciding — the twin of
     *   `ResolutionMappingsDal.findWriteFacts`'s parameter, built by `corroboratorScopeFor`.
    ...
    public async findWriteFacts(
        ...
        corroborators: CorroboratorScope,
        ...
        await writer.execute(
            sql`SELECT pg_advisory_xact_lock(${ADVISORY_LOCK_CLASSES.recipeParseCorrection}, hashtext(${normalizedKey}))`,
        );
        ...
        //
        // ⛔ ADR-0040: on a containing stage a REGISTERED test principal's row is not a corroborator, for every caller —
        // the same exclusion `resolutionMappingsDal.findWriteFacts` carries, for its reason: a test pool is a
        // sock-puppet farm, and a promotion that cites a test row is destroyed by the test purge's cascade under every
        // real cook relying on it.
        const excludeTestPrincipals = corroborators.excludeTestPrincipals
            ? sql`AND NOT EXISTS (SELECT 1 FROM test_principals tp WHERE tp.user_id = ingredient_parse_corrections.user_id)`
            : sql``;
     ...
     * (`owner_id` → `user_id`, index `idx_parse_corrections_live_owner` → `…_live_user`). ⚠️ NOT `0029`'s
     * literal text for the author target: against that it would infer no index at all.
     *
