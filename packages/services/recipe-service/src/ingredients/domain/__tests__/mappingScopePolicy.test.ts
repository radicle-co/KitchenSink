...
function input(overrides: Partial<MappingWriteInput> = {}): MappingWriteInput {
    return {
        ...
        // ADR-0040: every row above is the production case for a REAL principal — containment on, changing nothing.
        principalKind: 'real',
        containment: 'enforce',
        ...overrides,
    };
...
describe('evaluateMappingWrite — ADR-0040: a contained test principal’s correction binds ONLY itself', () => {
    const CONTAINED = { principalKind: 'test', containment: 'enforce' } as const;

    it('⛔ a curator GRANT buys a contained test principal no global reach', () => {
        const decision = evaluateMappingWrite(input({ ...CONTAINED, grantedScopes: [CURATOR_MAPPING_SCOPE] }));

        // A prod test-pool slot holding a curator grant must not be able to bind a phrase for every real user.
        expectWrite(decision, 'author');
    });

    it('⛔ a corroborating author earns a contained test principal NO promotion', () => {
        const decision = evaluateMappingWrite(input({ ...CONTAINED, corroboratorsForSameFood: ONE_CORROBORATOR }));

        // A pool of test accounts is a sock-puppet farm: two slots agreeing would otherwise promote a phrase
        // globally and fire the U19 corroborated-completion call at food.
        expectWrite(decision, 'author');
        expect(decision.write === 'author' && decision.promotion).toBeUndefined();
    });

    it('still lets a contained test principal supersede its OWN earlier correction', () => {
        const decision = evaluateMappingWrite(input({ ...CONTAINED, liveOwn: { id: OWN_ID, foodId: OTHER_FOOD } }));

        expect(decision.write === 'author' && decision.supersedes).toBe(OWN_ID);
    });

    it('promotes a test principal’s corroborated correction exactly as before where containment is off', () => {
        const decision = evaluateMappingWrite(
            input({ principalKind: 'test', containment: 'off', corroboratorsForSameFood: ONE_CORROBORATOR }),
        );
