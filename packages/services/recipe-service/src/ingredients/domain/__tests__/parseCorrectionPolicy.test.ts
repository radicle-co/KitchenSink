...
function input(
    ...
    return {
        ...
        // ADR-0040: a REAL principal on an enforcing stage — the production case, which containment must not move.
        principalKind: 'real',
        containment: 'enforce',
        ...overrides,
    };
...
describe('evaluateParseCorrectionWrite — ADR-0040: containment reaches the PARSE tier through the shared rule', () => {
    it('⛔ a contained test principal’s parse correction is neither curated globally nor promoted', () => {
        const decision = evaluateParseCorrectionWrite(
            input({
                principalKind: 'test',
                containment: 'enforce',
                grantedScopes: [CURATOR_PARSE_SCOPE],
                corroboratorsForSameAnswer: [{ id: 'corroborator-row', userId: '01JU21COOK0000000000000B' }],
            }),
        );

        // The rule lives in `evaluateCorrectionScope`, so the parse tier is contained by the SAME edit that
        // contains the mapping tier — a second copy would have left this subject silently uncontained.
        expect(decision.write).toBe('author');
