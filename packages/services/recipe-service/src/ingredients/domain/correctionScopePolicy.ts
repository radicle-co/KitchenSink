 ...
 * did.
 */
import { evaluateContainment, type ContainmentSubject } from '../../common/containmentPolicy.js';
 ...
 *
 * ⛔ ADR-0040 — `principalKind` + `containment` (from {@link ContainmentSubject}) are REQUIRED, and composed HERE, in
 * the shared rule, rather than in either specialization: both the mapping tier and the parse tier are contained by
 * this one edit, and a copy in one adapter would have left the other subject silently uncontained.
 */
export interface CorrectionScopeInput extends ContainmentSubject {
    /**
...
export function evaluateCorrectionScope(input: CorrectionScopeInput): CorrectionScopeDecision {
    const { correctedAnswer, requiredGrant, liveGlobal, liveOwn } = input;
    // ⛔ ADR-0040 — a CONTAINED test principal's correction reaches only itself. Spelled exactly like U11/R20's
    // private-food rule in `mappingScopePolicy`: "the grant does not apply and the corroborators do not count" — one
    // subject-level fact and zero new branches, so idempotence and own-row supersession behave as they always did.
    // A pool of test accounts is a sock-puppet farm; promotion binds a phrase for every real user and can complete a
    // PENDING catalog food, and none of that can be cleaned up afterwards.
    const promotion = evaluateContainment({
        principalKind: input.principalKind,
        containment: input.containment,
        action: 'promoteCorrection',
    });
    const grantedScopes = promotion.allowed ? input.grantedScopes : [];
    const corroboratorsForSameAnswer = promotion.allowed ? input.corroboratorsForSameAnswer : [];
