 ...
 * branch — and all three non-test call sites do: `ingredients.controller.ts`, `foodStatusTranslation.ts`, and
 * `recipes.service.ts`'s U11 clone path.
 *
 * ⚠️ "Ownerless" above is about the CATALOG row: `ingredients.food_owner_id` (migration 0040) means a row
 * standing for somebody's PRIVATE authored food is retrieved only for its author.
