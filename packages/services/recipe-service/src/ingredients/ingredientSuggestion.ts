...
export interface CatalogHit {
    /**
     * U11 (R20): present ONLY on the CALLER's own authored hits — the lexical tier's author-augmentation
     * signal. An authored food is AUTHOR-ONLY (ADR-0036), so a stranger never receives one at all and
     * `private` is the only value food publishes.
     */
 ...
 * ruling, 2026-08-19), not left for provenance badges to distinguish: without it, importing 338 recipes
 * through the app's own path produces 268 lines with no food record, ALL of them pre-existing freeform rows
 * shadowing a catalog-backed twin, permanently. Matching is exact on a normalized
