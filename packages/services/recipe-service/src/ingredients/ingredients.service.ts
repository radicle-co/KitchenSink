...
import type { CandidateView, StatusResponse } from '@kitchensink/food-service-client';
...
import { canonicalNameFrom, licensesStatusWrite, toResolutionStatus } from './foodStatusTranslation.js';
...
export class IngredientsService {
    ...
    public constructor(
        ...
        // ⛔ REQUIRED, all three. "No cascade" is `[]` — a state this service supports and the code below
        // still checks for — but it must be SAID, not defaulted: a collaborator that defaults to absent is
        // how a feature ships silently disabled past a green suite, which is the failure U11 shipped and the
        // verification queue's own docstring refuses. A fixture that does not care passes a double.
    ...
    public async addByFoodId(
        ...
        if (name === undefined) {
            ...
            if (existing !== undefined) {
                // ⛔ A status that licenses no catalog write leaves the row EXACTLY as it stands (0016).
                // `WITHDRAWN` is a live fact food owns, derived into the line's treatment at read time;
                // recording it here would strand a stale "removed" on a food that was restored, and on a
                // row shared by every recipe that binds this food.
                if (!licensesStatusWrite(status.status)) {
                    return existing;
                }
    ...
    public async refreshStatus(
        ...
        try {
            ...
            // ⛔ SKIP THE WHOLE WRITE for a status that licenses none (0016) — not just its status field.
            // Verified this loses nothing: for `WITHDRAWN`, `getStatus` returns no `food` body at all, so
            // `canonicalNameFrom` is already `undefined`, `priorFraction` is already absent, and
            // `foodOwnerId` is already left untouched. Every other field on this write is a no-op, and the
            // row's live treatment is derived at READ from the same status this refresh just observed.
            if (!licensesStatusWrite(status.status)) {
                return ingredient;
            }
