...
function principal(
    scopes: string[] = [],
    permissions: string[] = [],
    subject: Pick<Principal, 'principalKind' | 'containment'> = { principalKind: 'real', containment: 'enforce' },
): Principal {
    return { userId: AUTHOR, sub: 'user_clerk', scopes, permissions, ...subject };
}
...
describe('ResolutionMappingsService — ADR-0040 test-principal containment', () => {
    it('⛔ a contained test principal holding the curator grant binds only itself', async () => {
        const { service, applyWrite } = build();

        await service.recordCorrection({
            principal: principal([CURATOR_MAPPING_SCOPE], [], { principalKind: 'test', containment: 'enforce' }),
            phrase: 'Plain Flour',
            foodId: FOOD_A,
            surfacing: 'picker_correction',
        });
    ...
    it('⛔ a contained test principal earns no promotion and fires NO corroborated-completion call at food', async () => {
        const { service, applyWrite, corroborateFood } = build({
            facts: { ...NO_HISTORY, corroboratorsForSameFood: [{ id: 'row-b', userId: CORROBORATOR }] },
        });

        await service.recordCorrection({
            principal: principal([], [], { principalKind: 'test', containment: 'enforce' }),
            phrase: 'Plain Flour',
            foodId: FOOD_A,
            surfacing: 'picker_correction',
            caller: 'caller-token' as never,
        });

        const decision = applyWrite.mock.calls[0]![0].decision;
    ...
    it.each([
        ...
        'asks the reader to exclude registered test principals exactly when the stage is `%s`-contained',
        async (containment, excludeTestPrincipals) => {
            const { service, findWriteFacts } = build();

            // A REAL caller: the exclusion is the stage's, so a real user's correction on production must not count
            // a test row as its corroborator either.
            await service.recordCorrection({
                principal: principal([], [], { principalKind: 'real', containment }),
                phrase: 'Plain Flour',
                foodId: FOOD_A,
                surfacing: 'picker_correction',
            });

            expect(findWriteFacts.mock.calls[0]![3]).toEqual({ excludeTestPrincipals });
        },
    );
});
...
describe('ResolutionMappingsService — the phrase is PARSED at the boundary', () => {
    it('keys the correction on the normalized phrase, and persists the raw one beside it', async () => {
        ...
        expect(findWriteFacts).toHaveBeenCalledWith(
            'plain flour',
            AUTHOR,
            FOOD_A,
