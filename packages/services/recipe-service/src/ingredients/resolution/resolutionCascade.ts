 ...
 *
 * ## ⛔ THE VERIFICATION GATE IS NOT A TIER OF THIS CHAIN
 *
 * There is no tier 4 that ENQUEUEs instead of answering, and {@link TierOutcome} has no `deferred` member for
 * one. Building either would leave a named shape that lies.
 *
 ...
 * different answer.
 */
const RESOLUTION_EVIDENCE_CLASSES = ['curated-mapping', 'remembered-verification', 'catalog-ranking'] as const;
 ...
 * already uses in `deployGate.sh` and in `evaluateProvenance` vs `RecipesService.create`), so the rule is
