...
export class ResolutionMappingsDal {
     ...
     *
     * `pg_advisory_xact_lock(ADVISORY_LOCK_CLASSES.recipeResolutionMapping, hashtext(key))` closes it: the lock exists whether or not any row does. It is the
     ...
     * @param corroborators - Whether a registered test principal's row may count (ADR-0040).
    ...
    public async findWriteFacts(
        ...
        await writer.execute(
            sql`SELECT pg_advisory_xact_lock(${ADVISORY_LOCK_CLASSES.recipeResolutionMapping}, hashtext(${normalizedKey}))`,
        );
        ...
        //
        // ⛔ ADR-0040: on a containing stage a REGISTERED test principal's row is not a corroborator. A test pool is
        // a sock-puppet farm; without this, two pool slots — or one slot and a real user — promote a phrase globally,
        // and a promotion cannot be cleaned up (the purge cannot un-bind what real users have since resolved by).
        const excludeTestPrincipals = corroborators.excludeTestPrincipals
            ? sql`AND NOT EXISTS (SELECT 1 FROM test_principals tp WHERE tp.user_id = ingredient_resolution_mappings.user_id)`
            : sql``;
