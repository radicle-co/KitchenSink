...
import { corroboratorScopeFor } from '../../common/containmentPolicy.js';
...
export class ResolutionMappingsService {
    ...
    public async recordCorrection(input: RecordCorrectionInput): Promise<RecordCorrectionResult> {
        ...
        const { result, decidedScope, corroboratingAuthorIds } = await this.dal.runInTransaction(async (tx) => {
            // ADR-0040: on a containing stage no registered test principal's row counts as a corroborator — for ANY
            // caller, which is why this keys on the stage and not on the caller's kind.
            const facts = await this.dal.findWriteFacts(
                normalizedKey,
                userId,
                input.foodId,
                corroboratorScopeFor(input.principal),
                tx,
            );
            const decision = evaluateMappingWrite({
                principalKind: input.principal.principalKind,
                containment: input.principal.containment,
                correctedFoodId: input.foodId,
                grantedScopes,
                ...facts,
            });
