 ...
 * `.github/scripts/deployGate.sh` uses (pure `decide`, impure `evaluate`, ADR-0010), and it is what lets
 ...
 *
 * ⚠️ What it returns is a GUARD'S INPUT, not a courtesy printed for the operator: {@link refuseUnboundTarget}
 * consumes this descriptor and refuses on it, so `--stage` IS bound to the database this process actually
 * opened. Do not describe `--stage` as an unchecked declaration — that is the sentence a reader would use to
 * conclude that nothing checks the target.
 *
