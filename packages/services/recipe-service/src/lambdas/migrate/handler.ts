 ...
 * `rdsPoolConfig` (`@kitchensink/rds-iam-auth`). The lambda role holds `rds-db:connect` on the `recipe_migrator`
 * db-user (granted in the recipe schema stack). Every DDL statement runs as `recipe_owner` (the role split,
...
import { applyMigrations, assertBundleMatches, parseMigrateEvent } from '@kitchensink/db-schema-guard';
...
export async function ensureDatabaseExists(options: EnsureDatabaseOptions): Promise<EnsureDatabaseResult> {
    ...
    try {
        ...
        await maintenancePool.query(
            `CREATE DATABASE ${pg.escapeIdentifier(databaseName)} OWNER ${pg.escapeIdentifier(RECIPE_ROLES.owner)}`,
        );
    } catch (err) {
...
export const handler = async (event: unknown = {}): Promise<MigrateResult> => {
    const parsed = parseMigrateEvent(event, 'Recipe');
    ...
    const withMaintenancePool = async <T>(run: (pool: pg.Pool) => Promise<T>): Promise<T> => {
        const maintenancePool = new Pool({
            ...rdsPoolConfig({ host, port, database: 'postgres', username: RECIPE_ROLES.migrator }),
            max: 1,
        });
    ...
    const pool = new Pool({
        ...rdsPoolConfig({ host, port, database: databaseName, username: RECIPE_ROLES.migrator }),
        max: 1,
    });
