...
function makeDal(overrides: Partial<MockedDal> = {}): MockedDal {
    return {
        ...
        // Copy-forward's two reads, answering "nothing to copy" — so every pre-existing assertion in this
        // file still describes the same behaviour it always did (every line enqueued). The dedup rules have
        // their own suite, `parseJobCopyForward.service.test.ts`.
        findCopyForwardCandidates: vi.fn<ParseJobsDalPort['findCopyForwardCandidates']>(async () => []),
...
describe('create', () => {
    /**
     * ⚠️ REWRITTEN for the copy-forward contract (plan 2026-09-19-001). Each stored line now carries its
     * DECISION alongside its text and digest, so the DAL can insert a copied line terminal in the same
     * transaction. The rule this test has always owned — one splitter, one digest, one message per line —
     * is UNCHANGED and still asserted; what moved is the shape `createJob` receives. The decision RULES are
     * covered by `parseJobCopyForward.service.test.ts`; here every line is `enqueue` because this file's
     * fake reports no candidates.
     */
        ...
        expect(dal.createJob).toHaveBeenCalledWith(
            ...
            [
                ...
                },
                {
                    ...
                    lineDigest: lineDigest('1 tsp salt', sha256Hex),
                    decision: { kind: 'enqueue' },
                },
            ],
            expect.any(Date),
