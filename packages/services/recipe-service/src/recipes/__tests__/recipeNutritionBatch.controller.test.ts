...
describe('RecipesController.getNutritionBatch', () => {
    it('delegates the owner key, the requested ids and the caller bearer, and returns the result verbatim', async () => {
        ...
        // ⚠️ THE DOUBLE MOVED, not the assertion. `getNutritionForRecipes` is the READ projection and now
        // lives on `RecipeDetailAssembler`; the controller delegates to it directly. Every expectation below
        // is unchanged — this test still pins that the owner key, the ids and the caller bearer are passed
        // through and the result returned verbatim.
        const controller = new RecipesController(
            {} as unknown as RecipesService,
            { getNutritionForRecipes } as unknown as RecipeDetailAssembler,
