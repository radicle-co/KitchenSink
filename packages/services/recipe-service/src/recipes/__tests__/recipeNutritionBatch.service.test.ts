...
function newService(
    ...
    // ⚠️ THE SUBJECT IS THE ASSEMBLER. `getNutritionForRecipes` is a READ projection and lives on
    // `RecipeDetailAssembler`; every expectation in this file — one food lookup per batch, the deduplicated id
    // list, authorization-by-absence, the verdict read ordering — pins it there. The collaborators narrow to
    // the assembler's seven because the write-side ones (versions, the verification queue, the bands, the
    // redrive) are not reachable from this path.
