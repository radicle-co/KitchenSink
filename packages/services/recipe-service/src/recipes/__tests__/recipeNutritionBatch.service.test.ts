/**
 * Unit tests for `RecipeDetailAssembler.getNutritionForRecipes` — the deferred calorie lookup's
 * orchestration. It moved off `RecipesService` with the read-side extraction; every assertion here is the
 * one it always was.
 *
...
function newService(
    ...
    // ⚠️ THE SUBJECT MOVED, not the assertions. `getNutritionForRecipes` is a READ projection and now lives on
    // `RecipeDetailAssembler`; every expectation in this file — one food lookup per batch, the deduplicated id
    // list, authorization-by-absence, the verdict read ordering — is unchanged and still pins exactly what it
    // pinned on the service. The collaborators narrow to the assembler's seven because the write-side ones
    // (versions, the verification queue, the bands, the redrive) were never reachable from this path.
    return makeRecipeDetailAssembler({
        dal: dal,
        ingredientsDal: fakeIngredientsDal(),
        foodNutrition: { lookup } as never,
        lineVerificationsDal: fakeLineVerificationsDal(verdicts),
    });
}

describe('RecipeDetailAssembler.getNutritionForRecipes', () => {
    ...
    it('⛔ does NOT read verdicts at all when no line was transcribed — there is nothing to judge', async () => {
        ...
        const service = makeRecipeDetailAssembler({
            dal: dal,
            ingredientsDal: fakeIngredientsDal(),
            foodNutrition: { lookup } as never,
            lineVerificationsDal: verificationsDal,
        });

        await service.getNutritionForRecipes(VIEWER, ['r-1'], undefined);
...
async function this_getNutrition(service: RecipeDetailAssembler, recipeIds: readonly string[]) {
    return service.getNutritionForRecipes(VIEWER, recipeIds, undefined);
}
