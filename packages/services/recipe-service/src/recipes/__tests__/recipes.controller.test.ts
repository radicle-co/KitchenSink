...
function makeController(
    ...
    // ⚠️ The assembler is a stub here on purpose: none of these cases drives the nutrition-batch route, which
    // is the only one that reaches it. `recipeNutritionBatch.controller.test.ts` is where that route is pinned.
    return new RecipesController(
        service,
        {} as unknown as RecipeDetailAssembler,
...
describe('RecipesController', () => {
    ...
    it('getById delegates the owner key + id', async () => {
        ...
        const result = await controller.getById(PRINCIPAL, 'r-1', CALLER);

        // A READ, so the full food-latency budget: a slightly slower right answer beats a degraded one.
        expect(getById).toHaveBeenCalledWith({ viewerId: OWNER, id: 'r-1', caller: CALLER, budget: 'read' });
        expect(result).toBe(RESPONSE);
    });

    // ⛔ REWRITTEN (not relaxed) for the three write routes below: each also asserts the caller's bearer is
    // forwarded. Without it `update`, `setVisibility` and `clone` answer with nutrition from the in-process
    // cache alone — absent on a cold cache, indistinguishable from a food outage.
    // `nutrition.integration.test.ts` proves the outcome.
    it('update delegates the verified principal, caller, id, and body', async () => {
        ...
        await controller.update(PRINCIPAL, CALLER, 'r-1', body);

        expect(update).toHaveBeenCalledWith(PRINCIPAL, 'r-1', body, CALLER);
    });
    ...
    it('clone delegates the verified principal, id and caller, and returns the service result', async () => {
        const clone = vi.fn().mockResolvedValue(RESPONSE);
        const controller = makeController(fakeService({ clone }));

        const result = await controller.clone(PRINCIPAL, CALLER, 'r-1', {} as CloneRecipeDto);

        expect(clone).toHaveBeenCalledWith(PRINCIPAL, 'r-1', CALLER);
        expect(result).toBe(RESPONSE);
    });

    it('setVisibility delegates the principal, id, requested visibility, and caller', async () => {
        const setVisibility = vi.fn().mockResolvedValue(RESPONSE);
        const controller = makeController(fakeService({ setVisibility }));

        const result = await controller.setVisibility(PRINCIPAL, CALLER, 'r-1', {
            visibility: 'private',
        } as SetVisibilityDto);

        expect(setVisibility).toHaveBeenCalledWith(PRINCIPAL, 'r-1', 'private', CALLER);
        expect(result).toBe(RESPONSE);
    });

    // ── U3: view capture at the DETAIL handler, deliberately not RecipesService.getById ──────────
    // The service method is also reached without a view taking place (a rating write returns the rated
    // recipe's detail through it); capturing there would count those as views, permanently inflating the
    // lifetime counts 015 will consume. The controller handler observes exactly the
    ...
    it('getById captures ONE recipe_viewed event for the verified caller after a successful read (U3)', async () => {
        ...
        await controller.getById(PRINCIPAL, 'r-1', CALLER);

        expect(analytics.capture).toHaveBeenCalledTimes(1);
        // The ACTING slice of the verified principal — containment is decided at the analytics seam (ADR-0040).
        expect(analytics.capture).toHaveBeenCalledWith({
            type: 'recipe_viewed',
            actor: { userId: OWNER, principalKind: 'real', containment: 'enforce' },
            recipeId: 'r-1',
        });
    });
    ...
    it('getById captures NOTHING when the read fails — a 404/403 is not a view', async () => {
        ...
        await expect(controller.getById(PRINCIPAL, 'gone', CALLER)).rejects.toThrow();
    ...
    it('no OTHER handler captures a view — create, list, update, delete, clone are not reads of a detail', async () => {
        ...
        await controller.update(PRINCIPAL, CALLER, 'r-1', { expectedVersion: 1 } as UpdateRecipeDto);
        await controller.remove(OWNER, 'r-1');
        await controller.clone(PRINCIPAL, CALLER, 'r-1', {} as CloneRecipeDto);
