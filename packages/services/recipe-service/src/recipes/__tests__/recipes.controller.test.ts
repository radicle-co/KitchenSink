...
describe('RecipesController', () => {
    ...
    // ⛔ REWRITTEN (not relaxed) for the three write routes below: each now also asserts the caller's bearer
    // is forwarded. Before, `update`/`setVisibility` took no credential and the clone route dropped the one
    // `clone` accepted, so all three answered with nutrition from the in-process cache alone — absent on a
    // cold cache, indistinguishable from a food outage. `nutrition.integration.test.ts` proves the outcome.
