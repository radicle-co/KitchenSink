 ...
 *  2. **There is ONE representation of each body.** `recipe-core` keeps no parallel interfaces held in step by
 *     mutual assignability: assignability is not identity, a spread is exempt from excess-property checking,
 *     and a passing bidirectional check is evidence that a second declaration exists.
