...
function makeService(
    ...
    // U14 — the verdict READER is the fixture's default, "the gate has judged nothing", which migration 0023
    // defines as PUBLISH, so these producer cases exercise the pre-gate read behaviour they were written against.
...
describe('RecipesService.create — the verification producer', () => {
    it('U11/R20: stamps privateFood on the message when the line is backed by a PRIVATE food', async () => {
        ...
        await makeService(fakeRecipesDal(), queue, ingredientsDal).create(OWNER_PRINCIPAL, createDto(), undefined);
    ...
    describe('ADR-0040 — a contained test principal asks the gate NOTHING', () => {
        ...
        it('enqueues nothing, and reads nothing for it, for a TEST principal on an ENFORCING stage', async () => {
            ...
            const response = await makeService(fakeRecipesDal(), queue, ingredientsDal).create(
            ...
            expect(response.id).toBe(RECIPE_ID);
            expect(queue.enqueue).not.toHaveBeenCalled();
            // The early return is at the TOP: no provenance or privacy read is paid for a request never sent.
            expect(ingredientsDal.privateFoodIngredientIds).not.toHaveBeenCalled();
        });
