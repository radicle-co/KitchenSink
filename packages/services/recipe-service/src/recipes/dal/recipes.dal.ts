...
export class RecipesDal {
    ...
    /**
     * Load one active (non-tombstoned) recipe ROW — no steps, no ingredients.
     *
     * The read an authorization check needs: visibility and ownership are both columns of this row, so a
     * caller that only asks "may this viewer act on this recipe?" pays one primary-key read instead of the
     * aggregate. {@link findById} is built on it, so the two cannot disagree about what a tombstone is.
     *
     * @param id - The recipe id.
     * @returns The row, or `undefined` when no active recipe has that id.
     * @sideEffect Reads `recipes`.
     */
    public async findRowById(id: string): Promise<RecipeRow | undefined> {
    ...
    /**
     * Load one active (non-tombstoned) recipe + its steps and ingredients.
     *
     * @returns The aggregate, or `undefined` when no active recipe has that id.
     * @sideEffect Reads `recipes`, `recipe_steps` and `recipe_ingredients`.
     */
    public async findById(id: string): Promise<RecipeAggregate | undefined> {
        const recipe = await this.findRowById(id);
