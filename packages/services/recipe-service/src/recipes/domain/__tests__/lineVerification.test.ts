...
/**
 * `foodPresenceStatus` — the WITHDRAWAL overlay (owner rulings 3 + 4, 2026-09-07).
 *
 * ## Its whole reason for existing: the persisted mirror is stale by construction
 *
 * A line's `ingredients.food_resolution_status` is a MIRROR of food's lifecycle, written on whatever write
 * path last touched the row. Nothing refreshes it when a food is withdrawn — recipe deliberately does not
 * write that value at all — so a food withdrawn yesterday still reads `RESOLVED` there forever. Detection
 * is LAZY ("the recipe only finds out when the recipe is fetched"), which means it has to be derived from
 * the LIVE answer at read time, and that is this function.
 *
 * ## ⛔ Where it sits in the overlay chain, and why that is load-bearing
 *
 *     resolveLineStatus(…) → foodPresenceStatus(…) → viewerLineStatus(…)   // viewer stays LAST
 *
 * The viewer overlay must remain last. If presence ran after it, a line that is `RESOLVED_UNAVAILABLE` to a
 * stranger — a PRIVACY answer, meaning "this viewer is not served the details" — could be rewritten into
 * `FOOD_REMOVED`, a FACTUAL claim that the food is gone. That would tell a stranger something true about
 * another user's catalog that they are not entitled to know, and would tell them it about a food that may
 * be perfectly present. The final test in this block is what pins the order.
 */
describe('foodPresenceStatus — a withdrawn food, detected at read', () => {
    it('reports FOOD_REMOVED when the live status is WITHDRAWN', () => {
        expect(foodPresenceStatus(FoodResolutionStatus.RESOLVED, 'WITHDRAWN')).toBe(FoodResolutionStatus.FOOD_REMOVED);
    });

    it('⛔ overrides whatever the STALE persisted mirror said, which is the point', () => {
        // Every one of these is a value the mirror can legitimately hold while the food is already gone.
        for (const persisted of [
            ...
            expect(foodPresenceStatus(persisted, 'WITHDRAWN')).toBe(FoodResolutionStatus.FOOD_REMOVED);
        }
    });

    it('⛔ leaves the status UNCHANGED when food could not be asked — the single most important case', () => {
        // An absent live status means the lookup DEGRADED (food unreachable, the chunk failed, the id was
        // disowned). ⛔ Absence is not a withdrawal: reporting one here would turn a transient outage into
        // a permanent-looking claim that the cook's ingredient was deleted, on every line of every recipe,
        // for the duration of the outage. Silence is not dissent (ADR-0026 §3), one service over.
        for (const persisted of [
            FoodResolutionStatus.RESOLVED,
            FoodResolutionStatus.NEEDS_REVIEW,
            FoodResolutionStatus.PENDING,
            ...
            expect(foodPresenceStatus(persisted, undefined)).toBe(persisted);
        }
    });

    it('leaves the status unchanged for every OTHER live status food can publish', () => {
        for (const live of ['PENDING', 'UNRESOLVED', 'RESOLVED', 'NOT_FOUND', 'FAILED', 'AWAITING_RETRY'] as const) {
            expect(foodPresenceStatus(FoodResolutionStatus.NEEDS_REVIEW, live)).toBe(FoodResolutionStatus.NEEDS_REVIEW);
        }
    });

    it('reports FOOD_REMOVED even when the line had NO persisted status at all', () => {
        expect(foodPresenceStatus(undefined, 'WITHDRAWN')).toBe(FoodResolutionStatus.FOOD_REMOVED);
    });
});

describe('⛔ the overlay ORDER — a privacy answer must never become a factual one', () => {
    it('a stranger on a private food still sees RESOLVED_UNAVAILABLE, not FOOD_REMOVED', () => {
        // The composition the read path performs, in order. `viewerLineStatus` runs LAST and wins.
        const status = viewerLineStatus(
            foodPresenceStatus(FoodResolutionStatus.RESOLVED, 'WITHDRAWN'),
            'author-1',
            'someone-else',
        );

        expect(status).toBe(FoodResolutionStatus.RESOLVED_UNAVAILABLE);
    });

    it('the food`s OWN author still learns it was withdrawn', () => {
        const status = viewerLineStatus(
            foodPresenceStatus(FoodResolutionStatus.RESOLVED, 'WITHDRAWN'),
            'author-1',
            'author-1',
        ...
        expect(status).toBe(FoodResolutionStatus.FOOD_REMOVED);
    });
});
