...
describe('toRecipeNutritionState — known', () => {
    ...
    it('⛔ marks a reading STALE when a line THIS recipe counted was served from cache (KTD-3b)', () => {
        // REWRITTEN: staleness rides on the LINE (`catalogFreshness`), not a recipe-wide count, so the rule
        // can ask whether the stale food is one that actually contributed.
        // Still per recipe, not the batch-level `degraded` flag: a recipe whose own lines all came back
        // fresh must not be caveated because a sibling recipe's chunk failed.
        const state = toRecipeNutritionState(
            {
                lines: [{ ...catalogLine, catalogFreshness: 'stale' }],
                referencedFoodCount: 1,
                resolvedFoodCount: 1,
                withheldLineCount: 0,
                pendingLineCount: 0,
    ...
    it('⛔ stays FRESH when the only stale food sits behind a line that contributed no catalog figure', () => {
        // The over-marking a recipe-wide rule produces: one food is stale, but its line cannot turn the
        // catalog figure into a contribution (a unit with no mass), and a DIFFERENT, fresh catalog line supplies
        // the whole figure. "Any stale food referenced, and any catalog line at all" would mark this reading
        // stale. (A WITHHELD line cannot carry the marker at all: `assembleLines` gives it no catalog.)
        const unconvertibleStale: NutritionLine = {
            ...catalogLine,
        ...
        const state = toRecipeNutritionState(
            {
                lines: [{ ...catalogLine, catalogFreshness: 'fresh' }, unconvertibleStale],
        ...
        expect(state).toMatchObject({ state: 'known', freshness: 'fresh' });
    });
