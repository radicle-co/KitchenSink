...
/**
 * The WITHDRAWAL overlay (owner rulings 3 + 4, 2026-09-07): prefer the LIVE food status over the persisted
 * mirror, without writing anything. Pure and total.
 *
 * ## Why this cannot read the persisted status alone
 *
 * `ingredients.food_resolution_status` is a MIRROR of food's lifecycle, written by whichever write path
 * last touched the shared row. Nothing refreshes it when a food is withdrawn — recipe deliberately records
 * no value for that status at all (`foodStatusTranslation.ts`'s `licensesStatusWrite`) — so a food
 * withdrawn yesterday reads `RESOLVED` there forever. The ruling is that detection is LAZY: "the recipe
 * only finds out when the recipe is fetched". Deriving it here, from the live answer the nutrition lookup
 * already carries, is what makes that true without a second write path.
 *
 * ## ⛔ An ABSENT live status is not a withdrawal
 *
 * `undefined` means the lookup could not answer for this food — food unreachable, the chunk failed, or the
 * id was disowned. Reporting `FOOD_REMOVED` for it would convert a transient outage into a permanent-
 * looking claim that a cook's ingredient was deleted, on every line of every recipe, for the duration.
 * Absence is not dissent (ADR-0026 §3, one service over), so the persisted status is returned unchanged.
 *
 * ## ⛔ Order: this runs BEFORE {@link viewerLineStatus}, which must stay LAST
 *
 *     resolveLineStatus(…) → foodPresenceStatus(…) → viewerLineStatus(…)
 *
 * If presence ran after the viewer overlay, a line that is `RESOLVED_UNAVAILABLE` to a stranger — a PRIVACY
 * answer meaning "this viewer is not served the details" — could be rewritten into `FOOD_REMOVED`, a
 * FACTUAL claim about another user's catalog that this viewer is not entitled to and that may not even be
 * true of a food they can see. A privacy answer must never degrade into a factual one.
 *
 * @param status - The line's status so far ({@link resolveLineStatus}'s answer).
 * @param liveFoodStatus - What food published for this line's food on THIS read, or `undefined` when the
 *   lookup could not answer for it.
 * @returns `FOOD_REMOVED` when the food was withdrawn, else the status unchanged. Pure.
 */
export function foodPresenceStatus(
    status: LineResolutionStatus | undefined,
    liveFoodStatus: FoodStatus | undefined,
): LineResolutionStatus | undefined {
    return liveFoodStatus === 'WITHDRAWN' ? FoodResolutionStatus.FOOD_REMOVED : status;
}
