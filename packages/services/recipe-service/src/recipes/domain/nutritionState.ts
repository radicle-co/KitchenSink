 ...
 *   and had nothing" from "food could not be reached"; staleness comes from each LINE's `catalogFreshness`
 *   (see `nutritionFreshness`), never from this flag.
...
export function toRecipeNutritionState(
    ...
    if (sources.some((source) => source !== null)) {
        ...
        return {
            ...
            // KTD-3b is "serve stale, MARKED" — and equally, do not mark what is not stale. Decided PER LINE
            // over the lines that counted, by the ONE rule the detail read also uses: a stale food behind a
            // user override, an unconvertible unit, or a withheld line contributed no catalog figure, so it
            // cannot age this reading. Per recipe, too — never the batch-level `degraded` flag, since one
            // request mixes fresh and stale chunks.
            freshness: nutritionFreshness(accounting.lines),
        };
    }
