...
interface LineCatalog {
     ...
     */
    readonly byIngredientId: ReadonlyMap<
        string,
...
export class RecipeDetailAssembler {
    ...
    public async toDetailResponse(
        ...
        // ⛔ `budget` is required for the same reason: whether this response follows a COMMITTED write decides how
        // long it may wait on food, and a path that has not said so must not compile.
        options: {
    ...
    /*
     * ⛔ NO single-recipe shell over `loadLineCatalog` (plan U14). `computeDetailNutrition` needs the LOADED
     * CATALOG itself (for each line's food id, to derive the verdict key, and for the catalog's own
     * resolution status), so a shell that returned only the assembled lines and discarded the catalog cannot
     * serve it, and a second catalog load per detail read is the exact fan-out `loadLineCatalog`'s own
     * docstring exists to prevent. Load-then-assemble lives inline in `computeDetailNutrition`.
     */
    ...
    private async computeDetailNutrition(
        ...
        viewerId?: string,
    ): Promise<{ nutrition: RecipeDetailNutrition; lineStatuses: ReadonlyMap<string, LineResolutionStatus> }> {
     ...
     * list), never one per ingredient. Skipping the lookup makes every recipe report `calories: 0,
     * isComplete: false` while every unit test stays green, because they mock the food client and none of them
     * exercises this wiring. `nutrition.integration.test.ts` is what catches it, and it is why that tier is not
     * optional.
     *
     ...
     * batch endpoint fans one of these out across up to `MAX_NUTRITION_RECIPE_IDS` recipes; assembling and
     * looking up together would issue one lookup per recipe with every answer still correct.
     *
    ...
    private async loadLineCatalog(
        ...
        const lookup = await this.foodNutrition.lookup(caller, foodIds, budget);
     ...
     * @returns Pending state by line id.
