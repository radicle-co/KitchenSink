...
export class RecipesController {
    public constructor(
        ...
        // ⛔ The nutrition batch is a READ projection, so it goes through the assembler rather than the
        // orchestrator. Kept on THIS controller rather than a new one: a second controller would drag in
        // `throttleWiring.test.ts`'s method-to-rate-limit census for no gain.
    ...
    public async getNutritionBatch(
        ...
        return this.detailAssembler.getNutritionForRecipes(ownerId, body.recipeIds, caller);
    }
    ...
    public async getById(
        ...
        // reason capture is (below): the service method is also reached by non-view routes that must not
        // pay — or serve — analytics.
        const ownerId = principal.userId;
        const [response, impact] = await Promise.all([
            // A READ, so the standard food deadline — a slightly slower right answer beats a degraded one.
            this.recipesService.getById({ viewerId: ownerId, id: id, caller: caller, budget: 'read' }),
        ...
        // plan U3): the service method is also reached without a view taking place — a rating write returns
        // the rated recipe's detail through it — so capturing there would count those as views, permanently
        // inflating the lifetime counts 015 consumes. (Photos and versions authorize through the row-only
        // `findReadableRecipe`/`findOwnedRecipe` and never reach it.)
        // After the await, so a refused read (404/403) is never a view. Fire-and-forget: never awaited.
        this.analytics.capture({ type: 'recipe_viewed', actor: actingPrincipalOf(principal), recipeId: id });
    ...
    public async update(
        ...
        return this.recipesService.update(principal, id, body, caller);
    }
    ...
    public async clone(
        ...
        return this.recipesService.clone(principal, id, caller);
    }
    ...
    public async setVisibility(
        ...
        return this.recipesService.setVisibility(principal, id, body.visibility, caller);
    }
}
