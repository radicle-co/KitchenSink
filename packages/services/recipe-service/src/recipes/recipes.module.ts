...
import { ParseJobsService, PARSE_COPY_FORWARD_MAX_AGE_HOURS, PARSE_JOBS_DAL } from './parseJobs.service.js';
...
@Module({
    ...
    providers: [
        ...
        {
            // The copy-forward window, in hours. ⚠️ Bounded in practice by `expireParseJobs`' purge (~8
            // days), so a larger value is fiction; `0` disables copy-forward outright.
            provide: PARSE_COPY_FORWARD_MAX_AGE_HOURS,
            inject: [ConfigService],
            useFactory: (config: ConfigService): number =>
                config.get<number>('PARSE_COPY_FORWARD_MAX_AGE_HOURS') ?? DEFAULT_COPY_FORWARD_MAX_AGE_HOURS,
        },
        {
        ...
        // ⛔ THE READ-SIDE COLLABORATOR (Nest resolves it by class token; array order is immaterial). Every detail
        // projection that needs the food service lives here behind three public methods; `RecipesService`
        // keeps the orchestration and the writes. It is also injected into `RecipesController` directly for
        // the nutrition-batch route, which is a read and has no business going through the orchestrator.
