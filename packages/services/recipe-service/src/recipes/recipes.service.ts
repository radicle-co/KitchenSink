...
export class RecipesService {
    ...
    public async create(
        ...
        await this.requestVerification(principal, aggregate.recipe.id, aggregate.ingredients, catalog, []);
     ...
     *    return is the FIRST statement, before the provenance and privacy reads, so a request never sent pays for
     *    nothing. `subject` is REQUIRED so a new caller cannot reach the queue without deciding.
     *
     ...
     * loads photos, the viewer's rating, the ingredient catalog, a food lookup and the verification verdicts.
    ...
    public async update(
        ...
        if (resolved !== undefined) {
            ...
            await this.requestVerification(principal, id, updated.ingredients, resolved.catalog, existing.ingredients);
        }
