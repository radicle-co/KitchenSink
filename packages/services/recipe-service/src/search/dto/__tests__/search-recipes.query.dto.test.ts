 ...
 *     `BAD_REQUEST` — a code that is deliberately NOT a member of `recipeErrorCodeSchema` (see `api-error.ts`), so
 ...
 * test at this unit's boundary; `tests/api-exception.filter.test.ts` owns the other half.
 */
