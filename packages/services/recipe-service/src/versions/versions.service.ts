...
import { recipeNotFound } from '../recipes/recipe.error.js';
...
export class VersionsService {
    public constructor(
        ...
        private readonly recipes: Pick<RecipesService, 'findReadableRecipe' | 'findOwnedRecipe' | 'update'>,
    ...
    public async list(ownerId: string, recipeId: string): Promise<RecipeVersion[]> {
        // The recipe read authorization, from ONE row — RECIPE_NOT_FOUND for a recipe this viewer cannot see.
    ...
    public async get(ownerId: string, recipeId: string, versionNumber: number): Promise<RecipeVersion> {
        // The row carries the owner id the archive key needs.
        const recipe = await this.recipes.findReadableRecipe(ownerId, recipeId);
     ...
     * triggers retention).
     *
     * @param principal - The verified caller; the restore is owner-only.
     * @param recipeId - The recipe to restore.
     * @param versionNumber - The 1-based version to restore FROM.
     * @param caller - The caller's bearer, forwarded to `update` because the envelope's `recipe` is a full
     *   detail whose nutrition the food service authorizes AS this caller; `undefined` only when the request
     *   carried none.
    ...
    public async restore(
        ...
        // Owner-only, from ONE row: RECIPE_NOT_FOUND when unseeable, NOT_OWNER when visible but not theirs. The
        // row also carries `currentVersion`, the optimistic-concurrency token the update below is issued against.
        const current = await this.recipes.findOwnedRecipe(ownerId, recipeId);
