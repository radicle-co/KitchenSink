...
export class VersionsService {
    ...
    public async list(ownerId: string, recipeId: string): Promise<RecipeVersion[]> {
        // Reuses the recipe read authorization (throws RECIPE_NOT_FOUND / NOT_OWNER as appropriate). No caller:
        // the detail body is discarded, so there is no nutrition anyone reads.
    ...
    public async get(ownerId: string, recipeId: string, versionNumber: number): Promise<RecipeVersion> {
        // No caller: only the owner id is read from this body (for the archive key), never its nutrition.
    ...
    public async restore(
        ...
        // No caller: an authorization + concurrency read whose body is discarded. The caller goes to the
        // `update` below, whose body IS returned.
        const current = await this.recipes.getById(ownerId, recipeId, undefined);
