...
/**
 * Whether a test database is configured — suites use this to `describe.skipIf(!hasDatabaseUrl)`.
 *
 * The name is kept (26 suites read it) while its meaning moved: the tier is gated on an admin SERVER now,
 * and the app is booted against the SERVICE role's connection, never a superuser's (ADR-0039).
 */
export const hasDatabaseUrl = hasTestDatabase;

/** Options for {@link bootRecipeApp}. */
export interface BootRecipeAppOptions {
    /**
     * The SERVICE-role connection the app is booted against — `recipeDb().appUrl` from the integration tier,
     * `recipeE2eDb().appUrl` from the e2e tier.
     *
     * ⛔ REQUIRED, and required for a reason that was measured: this module is shared by both tiers, which own
     * SEPARATE databases. While it defaulted, converting the e2e tier silently re-pointed 26 integration suites
     * at the e2e database — 89 failures, and the same class of decoupling (gate says one thing, connection says
     * another) that the conversion existed to remove. A default here is a tier chosen by whoever edited last.
     */
 ...
 * @throws {NonDisposableAdminServerError} when no admin server is configured — guard suites with
