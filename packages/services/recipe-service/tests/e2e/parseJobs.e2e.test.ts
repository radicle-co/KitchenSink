...
describe.skipIf(!hasDatabaseUrl)('parse jobs (e2e)', () => {
    ...
    /**
     * Copy-forward dedup, through the PUBLIC surface only.
     *
     * ⛔ WHAT THIS TIER CAN AND CANNOT SAY. No worker consumes the queue here and this tier makes no direct
     * database writes, so nothing ever LANDS — which means there is never an answer to copy. That makes the
     * positive case (a repeat is served from a prior landing) structurally unreachable at this tier; it is
     * proven in `__tests__/integration/parseJobs/parseJobCopyForward.integration.test.ts`, which stands in
     * for the worker and reads the real queue.
     *
     * What IS worth asserting here, and is not asserted anywhere else through the public API, is the
     * NEGATIVE: copy-forward must not fire on lines nobody has answered. A dedup that over-triggers would
     * return a second paste already `parsed` with no parse having happened — silently publishing an empty
     * proposal — and this is the cheapest place to notice that.
     */
    it('does not short-circuit a repeated paste whose lines have never been answered', async () => {
        const text = '4 cups e2e-dedup flour\n2 tsp e2e-dedup salt';

        const first = await fetch(`${baseUrl}/api/v1/recipe-parse-jobs`, {
            method: 'POST',
        ...
        const second = await fetch(`${baseUrl}/api/v1/recipe-parse-jobs`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ text }),
        });
        ...
        const job = (await second.json()) as JobView;

        expect(job.status).toBe('running');
        expect(job.lines.map((line) => line.status)).toEqual(['pending', 'pending']);
        expect(job.lines.every((line) => line.proposal === null)).toBe(true);
    });
