...
describe.skipIf(!hasDatabaseUrl)('version editor_handle provenance (e2e, real DB)', () => {
    ...
    it('stamps the editor handle on the UPDATE-created version (the main W6 defect)', async () => {
        const created = await recipes.create(EDITOR, CREATE_DTO, undefined);

        await recipes.update(EDITOR, created.id, { expectedVersion: 1, title: 'Renamed' }, undefined);
    ...
    it('stamps the cloner handle on the CLONE-created version', async () => {
        const source = await recipes.create(EDITOR, CREATE_DTO, undefined);

        const clone = await recipes.clone(EDITOR, source.id, undefined);
    ...
    it('stamps the restorer handle on the RESTORE-created version', async () => {
        ...
        const restored = await versions.restore(EDITOR, created.id, 1, undefined);
