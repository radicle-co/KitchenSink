 ...
 *   3. Provision the test Postgres under the production role model (ADR-0039) and migrate it with the
 *      service's OWN runner as `recipe_migrator`, so the tier's schema is built the way a stage's is.
 ...
 *
 * Idempotent: the whole setup can run repeatedly and always lands the same end state (step 3 rebuilds
 * `public`; steps 1, 2 and 4 use existence guards). It is a NO-OP when `DATABASE_ADMIN_URL` is unset, so a
 * machine without the harness up simply skips DB work rather than failing.
 *
...
import { hasTestDatabase, recipeDbSpec } from './support/roleDb.js';
...
export async function prepareHarness(spec: RoleDatabaseSpec): Promise<void> {
    // `DATABASE_ADMIN_URL` is the master switch for "the harness is expected to be up". When it is unset
    ...
    // The production role model + the service's OWN runner, as `recipe_migrator` (ADR-0039). The seed then
    // runs as `recipe_app` — DML is all a seed has ever needed, and running it as the subject proves that.
...
/**
 * The INTEGRATION tier's setup: the shared harness, on the integration database.
 *
 * @sideEffect Network + database I/O as described above.
 */
