...
//
// That is why this file exists as its own scenario instead of a case inside `searchLatency.load.js`: the
...
export function suggestPath() {
    ...
    // typeahead traffic instead of a synthetic hammer — on the calibrated profile. On a deployed one the per-user
    // limit sets the pace (`iterationPause`).
    sleep(iterationPause(0.3));
}
