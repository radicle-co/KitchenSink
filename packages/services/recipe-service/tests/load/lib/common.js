 ...
 * requests per minute per user. At 3s that is 20/min against the tightest default limit the deployed tier
 * reaches — `RATE_LIMIT_ANALYTICS`, 60 — a third of the budget, because a ramp concentrates requests and a
 * token bucket refills on a schedule this run does not control. (This used to cite `RATE_LIMIT_WRITE` at 30;
 * that default is 300 now, and `k6DeployedLoadShape.test.ts` does the arithmetic against the service's own
 * `throttleDefaults.ts` so the justification cannot go stale silently again.)
 *
...
/**
 * The pause between iterations for a scenario with its OWN cadence: that cadence on the calibrated profile, the
 * rate-limit pace on a deployed one.
 *
 * ⛔ A scenario may not sleep a literal on the deployed tier. `ingredientSuggestLatency` slept 0.3s — about 200
 * req/min/user — and lost 861 of 2061 requests to the limiter, succeeding exactly 1,200 times on both runs.
 * On a deployed stage the per-user limit decides the pace, whatever the scenario would prefer; asserted by
 * `k6DeployedLoadShape.test.ts`.
 *
 * @param {number} substrateSeconds - The scenario's natural cadence, used only on the calibrated profile.
 * @returns {number} Seconds to sleep.
 */
export function iterationPause(substrateSeconds) {
    return LOAD_PROFILE === 'substrate' ? substrateSeconds : DEPLOYED_PACE_SECONDS;
}
...
/**
 * Resolve the baseline ingredients this suite's payloads reference, creating them if absent.
 *
 * ⛔ CALL THIS FROM `setup()`, NOT PER-ITERATION. It is get-or-create, so it is safe to repeat, but the
 * ids are constant for a run and resolving them per iteration would measure the catalog instead of the
 * path under test.
 *
 * ⚠️ THIS REPLACED TWO HARDCODED UUIDs (`…aa` / `…bb`), and the reason is worth keeping. Those matched
 * `tests/globalSetup.ts`'s seed, which is TRUE of the runner-local substrate and FALSE of every deployed
 * stage: a `pr-{N}` preview's catalog is seeded by nothing. Since T043b create validates every line's
 * `ingredientId` against the catalog, so on a deployed run every write answered
 * `400 UNKNOWN_INGREDIENT` — measured on run 34045472743, where all three write scenarios failed
 * (`createRecipe 201` at 0 of 265) while the five read scenarios were clean.
 *
 * `POST /api/v1/ingredients` is get-or-create and ownerless, which is the same door
 * `packages/tools/e2e-seed/src/recipeWorld.ts` uses to build its world — so this needs no seed step, no
 * admin scope and no per-stage fixture, and it behaves identically on both profiles. One code path
 * instead of a substrate/deployed divergence is the point: the divergence is what hid this.
 *
 * @returns `{ flour, sugar }` — catalog ids that exist on THIS stage.
 * @sideEffect Creates the ingredient rows when they are absent.
 */
export function resolveSeedIngredients() {
    const resolve = (name) => {
        const res = http.post(`${BASE_URL}/api/v1/ingredients`, JSON.stringify({ name }), {
            headers: jsonHeaders(),
            tags: { operation: 'resolveIngredient' },
        });

        if (res.status !== 200 && res.status !== 201) {
            // Loud: every write payload in the run depends on this, so a silent fallback to a
            // non-existent id reproduces exactly the 400-storm this function exists to end.
            throw new Error(
                `common.js: could not resolve the '${name}' ingredient against ${BASE_URL} ` +
                    `(status ${res.status}). Every write payload needs a catalog id that exists on THIS stage.`,
            );
        }
    ...
    return { flour: resolve('flour'), sugar: resolve('sugar') };
}
...
/**
 * A valid CreateRecipeRequest body (matches packages/schemas/recipe/openapi.yaml, derived from the
 * service's authored zod).
 *
 * ⛔ `ingredients` IS REQUIRED, not defaulted. A default would be a hardcoded catalog id — the exact
 * shape that made every deployed write 400 — and it would let a new caller inherit that silently rather
 * than being told to resolve real ids. Pass {@link resolveSeedIngredients}'s result through `setup()`.
 *
 * ⛔ `visibility` IS `private`, and a scenario must not override it (`k6WriteContainment.test.ts`). A public
 * recipe is served to real users' discovery and search the moment it lands — before any reset can run — so a
 * load run's writes stay inside its own pool users' libraries (owner ruling 2026-09-13: test data must be
 * scoped "such that it won't conflict with real data"). A private `user_created` create is admitted only for
 * a PREMIUM caller (`visibilityPolicy.ts`), which is why every k6 VU slot in the test-pool roster carries
 * `premium` — a pool `poolAdmin` has not provisioned answers these creates with 403.
 *
 * @param label - Distinguishes this VU/iteration's recipe.
 * @param ingredients - `{ flour, sugar }` catalog ids for the stage under test.
 * @returns The request body.
 */
export function makeRecipePayload(label, ingredients) {
    return {
        ...
        visibility: 'private',
        ingredients: [
            { ingredientId: ingredients.flour, name: 'flour', quantity: { kind: 'exact', value: 2 }, unit: 'cups' },
            { ingredientId: ingredients.sugar, name: 'sugar', quantity: { kind: 'exact', value: 1 }, unit: 'cup' },
        ],
        steps: [
