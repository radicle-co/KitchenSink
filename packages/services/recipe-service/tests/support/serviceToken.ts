/**
 * Hermetic minting of internal service-principal account-erasure tokens (CR-002 / U4a) for the recipe
 * service's unit + integration auth suites.
 *
 * The PRODUCTION signer is the identity deletion-worker Lambda (U4b, a different package); this is its
 * test-only stand-in. It generates a throwaway Ed25519 keypair and signs tokens with `jose` against the
 * SHARED wire contract in `@kitchensink/recipe-core` (issuer/audience/algorithm/claim names), so a test
 * token is byte-for-byte what the real worker will mint — driving recipe-service's REAL
 * `ServiceErasureAuthService.verify` end to end, no mocks. Every knob (issuer, audience, algorithm, iat,
 * expiry, omitted claims) is overridable so the negative-path suite can forge each rejection reason.
 */
...
/** A throwaway Ed25519 keypair: the SPKI public PEM (the verification key) + its PKCS#8 private PEM. */
export interface ServiceKeypair {
    /** SPKI public-key PEM — set as `RECIPE_SERVICE_PRINCIPAL_JWT_KEY` so the verifier trusts this signer. */
...
/** Options for {@link signServiceErasureToken} — sane defaults, each overridable to forge a rejection reason. */
export interface SignServiceTokenOptions {
    ...
    /** Issuer override (defaults to the contract issuer) — set to something else to forge a wrong-iss token. */
    readonly issuer?: string;
    /** Audience override (defaults to the contract audience) — set to forge a wrong-aud token. */
    readonly audience?: string;
    /** JWS algorithm override (defaults to the contract algorithm). */
    readonly alg?: string;
    /** `iat` in epoch seconds (defaults to now). Move it back to forge an over-max-age / over-window token. */
    readonly issuedAtSeconds?: number;
    /** Seconds from `iat` to `exp` (defaults to 60). Negative ⇒ already expired; large ⇒ over-window. */
    readonly expiresInSeconds?: number;
    /** Custom claims to OMIT from the payload (e.g. `['evt']`) — to test a verified-but-malformed token. */
...
export async function signServiceErasureToken(
    ...
    return new SignJWT(payload)
        ...
        .setAudience(options.audience ?? SERVICE_ERASURE_TOKEN_AUDIENCE)
        .setIssuedAt(iat)
