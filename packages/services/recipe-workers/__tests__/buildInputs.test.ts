...
describe('production build inputs', () => {
     ...
     *
     * `handleSyncWorker.ts` was the sixth handler the stack deployed, and it appeared in neither this list nor
     * `esbuild.mjs`'s `entryPoints`. Both this test and `RecipeWorkersStack.test.ts` stayed green while the
     ...
     * rather than from anything anyone maintains: `serviceInfraWiringInvariants.test.ts` (W2) requires an
     * `esbuild.mjs` entry point for every `lambda.Function` handler string in the stack, and
     * `RecipeWorkersStack.test.ts` asserts the synthesized template's handlers each resolve to a BUNDLED
