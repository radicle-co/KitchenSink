 ...
 * `s3Erasure.integration.test.ts` and `accountErasure.integration.test.ts`, this cannot boot the
 ...
 *      `s3Erasure.integration.test.ts`); this test's own assertion is that the objects are REALLY gone.
 *   2. `createCloudFrontInvalidation` (the real adapter, `@aws-sdk/client-cloudfront` MOCKED — LocalStack
 *      Community has no CloudFront support, `compose.test.yml`'s `SERVICES` is `s3,sqs` only)
 ...
 * failure) is pinned at the unit tier (`src/handlers/__tests__/accountErasureWorker.test.ts`, which
 * mocks `common/cdnInvalidation.js` and asserts ordering via a rendered timeline) — this tier's job is
...
import { createCloudFrontInvalidation } from '../../../src/common/cdnInvalidation.js';
