 ...
 *
 * ⚠️ THE #119 SEAM: the workers do not derive their database name from an env var. `RecipeSchemaStack`
 * creates the database and publishes the name it used to SSM, and every DB-bound Lambda here resolves
 * `/kitchensink/{stage}/recipe/database-name` at DEPLOY time. This suite asserts that contract: the parameter
 * is THIS stage's, it is a strict SSM type (so a missing schema stack fails the deploy rather than falling
 * back), and the stage flows from `STAGE` rather than a constant. What a synth cannot see is the concrete
 * name — that trade is `498c6be7`'s, stated there.
 */
...
describe('recipe-workers CDK app — deploy input contract', () => {
    it("points every DB-bound Lambda at THIS preview's published database name (#119)", () => {
        ...
        //
        // ⚠️ ELEVEN since the queue backstop (plan U12, `QueueCheckFunction`) landed. It is inside the #119
        // guarantee for the sharpest version of the same reason: the backstop READS the claim and attempt
        // columns to decide whether work is stuck, so one pointed at the base database would report another
        // stage's backlog as this preview's — and it reports through a cron monitor, so the wrong answer
        // would arrive as a confident green rather than as an error.
    ...
    it('resolves the name at DEPLOY time through a strict SSM parameter keyed on STAGE — no fallback', () => {
        // ⚠️ REWRITTEN, not relaxed. The guarantee is that #119 cannot silently recur — a deploy must fail
        // loudly rather than aim workers at another stage's data. It has two halves, both asserted here:
        //
        ...
        const migrationRunners = Object.entries(all)
            .filter(
                ([, resource]) =>
                    ...
                    typeof resource.Properties?.['Handler'] === 'string' &&
                    resource.Properties['Handler'].includes('migrate'),
            )
            .map(([id]) => id);
