...
import { UNWARRANTED_MODEL_ID, UNWARRANTED_REGISTRY } from '../../../src/parsing/__tests__/unwarrantedModel.js';
...
describe.skipIf(!canRun)('a CRF outage never lands (integration)', () => {
    let pool: pg.Pool;
    let converse: Mock<GatedLlmDeps['bedrock']['converse']>;
    ...
    function deps(crf: ParseEnginePort<'crf'>): ParseLineDeps {
        ...
        // ⛔ TYPED, not `as unknown as ParseLineDeps`. The cast this replaces is what deferred a missing
        // REQUIRED member to call time — `deps.emit is not a function` at runtime instead of an error here —
        // and it accepted a `gated` literal that omitted `deployRegion` outright.
        const deps: ParseLineDeps = {
            ...
            gated: {
                ...
                // ⛔ THROWS RATHER THAN RESOLVING, because this file's whole subject is that an absent CRF
                // stops the line BEFORE the gated leg.
                //
                // ⚠️ It buys a LEGIBLE FAILURE, not detection a bare `vi.fn()` would have missed — every
                // test here that could reach this port asserts `toBeInstanceOf(Error)`, and an undefined
                // `resolve` would throw a TypeError on `settings.modelId` that the same assertion accepts.
                // The difference is that a regression now fails naming the invariant it broke instead of
                // reading as an unrelated property access.
                settings: {
                    resolve: () => {
                        throw new Error('the gated leg must not be reached when the CRF is absent');
                    },
                },
...
describe.skipIf(!canRun)('a residency-refused parse model never lands (integration)', () => {
     ...
     *
     * ⛔ `UNWARRANTED_MODEL_ID` is a FIXTURE rather than fabricated: it is a real registry entry, profile-only
     * over three regions and carrying no warrant. A fixture id would test a shape the registry does not have.
     *
     * ⚠️ NOT `NOVA_2_LITE_MODEL_ID`, which is WARRANTED (owner ruling 2026-09-12) — so it is residency-clear
     * and would exercise the opposite branch. Haiku 4.5 is deliberately left unwarranted
     * (see its registry entry), which is what keeps a real refusal available to test against.
     */
    function deps(): ParseLineDeps {
        ...
        return {
            ...
            gated: {
                ...
                registry: UNWARRANTED_REGISTRY,
            },
            crf: crfPort(crfParse),
            pool: { query: async (text: string, params: unknown[]) => pool.query(text, params) },
            digest,
            parseModelId: UNWARRANTED_MODEL_ID,
            sleep: instantSleep,
            claimLeaseSeconds: 150,
            deliveryAllowance: 20,
            emit: vi.fn(),
