 ...
 *
 *  - **No `DATABASE_ADMIN_URL` / no LocalStack** — the whole file skips. CI provides both
 ...
 *
 * ## ⛔ An absent CRF engine is NOT survivable — it is not ADR-0026 §3's `single-engine`
 *
 * The line does NOT still land on the LLM leg alone. `handlers/parseLine.ts` pushes everything an ENGINE throws into
 ...
 */
const canRun = hasTestDatabase && Boolean(SQS_ENDPOINT) && crfEngineAvailable();
