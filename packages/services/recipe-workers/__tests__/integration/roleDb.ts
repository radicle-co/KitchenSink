 ...
 *
 * A pool on a SUPERUSER URL satisfies every grant, so a privilege the deployed workers do NOT hold would
 * pass the whole tier and fail only in a stage. Under the role split these Lambdas hold `recipe_app`: DML and nothing
 ...
 * silently.
 *
 * ## Where the disposable-database rules live
 *
 * The loopback Specification — "a URL being set is not permission to destroy what it points at" — lives in
 * the harness, as `decideAdminServerUrl` in `packages/tools/service-test-harness/src/adminServer.ts`
 * (covered by that package's `src/__tests__/adminServer.test.ts`). Its second half, "the database name must
 * end in `_test`", is a precondition on every database the fixture provisions (`roleDatabase`'s
 * `assertDisposable`, covered by `src/__tests__/roleDatabase.test.ts`). Neither rule is re-implemented here,
 * and there is one variable — `DATABASE_ADMIN_URL` — so no precedence to order.
 *
