...
describe.skipIf(!canRun)('createVerdictStore (integration)', () => {
    ...
    it('⛔ writes NOTHING for a phrase with no visible content — a branch, not a failure', async () => {
        // `normalizedIngredientKey` is TOTAL: a phrase that sanitizes to nothing has no match grain, so
        // `rememberAgreement` returns without writing rather than throwing. Untested until now, and the two
        // ways to get it wrong are both silent — throwing would fail a billed handler on junk input, while
        // writing anyway would put a memo under an empty key that every later phrase collides with.
        const blank = '   ';

        // The premise, asserted rather than assumed: if this ever stops being `undefined` the test below is
        // proving nothing, and it should say so here rather than passing vacuously.
        expect(normalizedIngredientKey(blank)).toBeUndefined();

        const before = await pool.query(`SELECT count(*)::int AS n FROM ingredient_resolution_memos`);

        await expect(
            store.rememberAgreement({ phrase: blank, foodId: '01M13XBBPQQNBDNTMH0BQ4BYJ2', modelId: 'model-v1' }),
        ).resolves.toBeUndefined();

        const after = await pool.query(`SELECT count(*)::int AS n FROM ingredient_resolution_memos`);
