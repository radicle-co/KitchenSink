...
describe.skipIf(!canRun)('verification spend ledger (real Postgres)', () => {
    ...
    beforeAll(() => {
        // ⛔ No DROP and no hand re-apply of `0022_verification_spend.sql`. `globalSetup.ts` migrates recipe's
        // directory with the production engine, so the table is already the deployed one; re-applying the
        // file is `42P07 relation "verification_spend" already exists`, which throws out of `beforeAll` and
        // reports all 13 tests as SKIPPED behind a green run. And `recipe_app` holds DML and no DDL, so a
        // DROP would be `permission denied` — which is exactly the point of the tier.
