 ...
 *     cross-package seam where "the SQL was filed in the wrong package" hides. The fixture migrates recipe's
 *     own directory with the production engine, so the table under test IS the one the deploy ships.
 *
 * Runs against the role-split fixture (`../roleDb.js`); skipped in lockstep when no admin server is
 * configured, and run in CI.
 */
...
describe.skipIf(!canRun)('verification spend ledger (real Postgres)', () => {
    ...
    beforeAll(() => {
        // ⛔ This suite used to DROP `verification_spend` and re-apply `0022_verification_spend.sql` by hand,
        // because CI applied the schema with a psql loop and re-applying the file was
        // `42P07 relation "verification_spend" already exists` — which threw out of `beforeAll` and reported
        // all 13 tests as SKIPPED, so the ledger deciding whether the $100 ceiling holds had NO executed
        // coverage behind a green run. `globalSetup.ts` now migrates recipe's directory with the production
        // engine, so the table is already the deployed one and that apparatus is dead. It could not survive
        // the role split in any case: `recipe_app` holds DML and no DDL, so the DROP would be
        // `permission denied` — which is exactly the point of the tier.
