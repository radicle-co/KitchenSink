...
describe('RecipeWorkersStack — the bedrock:InvokeModel resource scope', () => {
    ...
    it('names a region outside the deploy region ONLY for the warranted profile, and never the unwarranted one', () => {
        // ⚠️ REWRITTEN by the Nova 2 Lite warrant (owner ruling 2026-09-12, ADR-0024 §4b). This asserted that
        // NO grant left us-east-1, which was true while nothing was cleared. Cross-region ARNs are the
        // warrant's visible cost, so the claim becomes a CONFINEMENT: they exist, and ONLY for the entry the
        ...
        //
        // ⚠️ This subsumes the old "and never the unwarranted profile" line: any unwarranted entry's fan-out
        // would BE a cross-region ARN, and every one of those is asserted to belong to the warranted model.
