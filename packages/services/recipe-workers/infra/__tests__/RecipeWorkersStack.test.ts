...
describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Seventeen alarms in ONE stack, the largest concentration in the repository: archive backlog and
        // age, three DLQ depths, erasure age, orphan resurrection, and nine on the LLM verification gate.
        // Every one of them was provisioned for every open pull request's preview.
        synth('pr-91', 'sandbox', false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    });

    it('creates all seventeen worker alarms when the flag is on', () => {
        // Non-vacuity, and the count is load-bearing: this is a switch, and nothing was deleted to build it.
        synth('pr-91', 'sandbox', true).resourceCountIs('AWS::CloudWatch::Alarm', 17);
    });
