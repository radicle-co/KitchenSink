...
function testApp(props: AppProps = {}): App {
    const outdir = mkdtempSync(path.join(tmpdir(), 'cdk-synth-'));
 ...
 * and `pg-native` is an optional peer `pg` only requires when `Client.native` is touched. Builtins are handled
 * separately, by asking Node.
 */
...
function unresolvableImports(file: string): readonly string[] {
    ...
    // ⛔ `isBuiltin`, NOT a `node:` PREFIX TEST. The prefix rule admitted `node:util` and reported bare
    // `util`, `url`, `worker_threads`, `diagnostics_channel`, `module` and `perf_hooks` as unresolvable — all
    // six of which Node resolves perfectly well, because the prefix is optional for every builtin except
    // `node:test`. A bundled dependency emitting the un-prefixed form (which `@sentry/*` and `pg` both do) was
    // therefore reported as a broken artifact, and the fix a reader would reach for is to add the six names to
    // `RUNTIME_PROVIDED` — a hand-kept list of builtins, which is the copy-of-a-fact this guard exists to
    // avoid. Asking Node what it considers builtin is the derivation the docstring above already claims.
 ...
 */
function synth(stage = 'sandbox', baseStage = 'sandbox', alarmsEnabled = true): Template {
...
/**
 * Every SQS action the SYNTHESIZED template grants to a named role.
 *
 * ⛔ READ FROM THE TEMPLATE, not from the stack's source. `queueProducerRegister.test.ts` reads source text
 * and holds the whole authority set to a register — which is the right shape for "who may talk to which
 * queue" across two CDK apps, and is BLIND to one thing: its raw-action reader matches `'sqs:<Word>'`, so a
 * wildcard `actions: ['sqs:*']` matches nothing at all and would join neither the register nor its
 * read-only rule. A wildcard is exactly how a role quietly acquires `ReceiveMessage` and `DeleteMessage`,
 * and IAM expands it at evaluation time whatever the source said. The template is what IAM gets.
 *
 * ⚠️ Filtered to SQS on purpose. `logGroup.grantWrite` and the VPC managed policy put `logs:*` and
 * `ec2:*` on the same role, and asserting the whole document would make every unrelated grant this role
 * legitimately needs a failure of a queue rule.
 *
 * @param template - The synthesized template.
 * @param roleId - The role's construct id, e.g. `QueueCheckRole`.
 * @returns The distinct `sqs:` actions, sorted.
 */
function sqsActionsGrantedTo(template: Template, roleId: string): readonly string[] {
    const actions = new Set<string>();

    for (const policy of Object.values(template.findResources('AWS::IAM::Policy'))) {
        const roles: unknown = policy.Properties?.Roles;
        const targetsRole =
            Array.isArray(roles) && roles.some((role: unknown) => JSON.stringify(role).includes(roleId));
        ...
        for (const statement of statements as { Action?: unknown }[]) {
            const declared = Array.isArray(statement.Action) ? statement.Action : [statement.Action];

            for (const action of declared) {
                if (typeof action === 'string' && action.startsWith('sqs:')) {
                    actions.add(action);
                }
            }
        }
...
function databaseBoundFunctions(template: Template): ReadonlyMap<string, string> {
    ...
    // ⚠️ THE NAME IS NO LONGER A LITERAL. `RecipeSchemaStack` publishes the database name to SSM and this
    // stack READS it, so the env var synthesizes as `{ Ref: <SSM parameter> }` and CloudFormation resolves it
    // at deploy time. Requiring `typeof name === 'string'` therefore matched NOTHING and reported zero
    // database-bound Lambdas — a guard going quiet, not red.
    //
    // ⛔ Resolving the Ref back to the parameter PATH keeps the #119 assertion intact and arguably sharpens
    // it: what mattered was never the literal text, it was that every one of these Lambdas points at the SAME
    // database as the API. Comparing the parameter they read proves exactly that. The trade is stated
    // honestly — the concrete name is now decided at deploy time and is no longer visible to a synth-time
    // test, which is the price of making agreement structural instead of derived.
    const parameters = (template.toJSON() as { Parameters?: Record<string, { Default?: string }> }).Parameters ?? {};
    ...
    const resolve = (value: unknown): string | undefined => {
        ...
        return ref === undefined ? undefined : (parameters[ref]?.Default ?? ref);
    };
    ...
    for (const [logicalId, fn] of Object.entries(template.findResources('AWS::Lambda::Function'))) {
        ...
        const name = resolve(variables?.Variables?.['RECIPE_DB_NAME'] ?? variables?.Variables?.['DB_NAME']);
...
describe('RecipeWorkersStack', () => {
    ...
    it('VPC-attaches every Lambda that reads the database (ADR-0004)', () => {
        ...
        // `kitchensink-recipe-schema-{stage}`, and the number falling is the change rather than a Lambda
        // quietly losing its database env — which the assertions below still catch.
        // ⚠️ AND UP BY ONE (plan U12) when the scheduled queue backstop landed: ELEVEN. It opens a READ
        // ONLY transaction over the owed-work tables, so it is database-bound by construction — and it is
        // the one Lambda here that must NEVER be able to write, which its own role and that transaction
        // enforce rather than this count.
        expect([...bound.keys()], 'the eleven database-bound workers').toHaveLength(11);
...
describe('RecipeWorkersStack — RDS IAM auth + per-stage database', () => {
    ...
    it('points every Lambda at the PER-PR logical database, never the shared base one', () => {
        ...
        // ⚠️ THE CLAIM MOVED WITH THE DESIGN, and it still catches #119. This stack no longer decides the
        // name — `RecipeSchemaStack` creates the database, publishes the name it used, and every consumer
        // reads that parameter. So the thing to assert is that all eleven Lambdas read the parameter for THIS
        // stage rather than the base stage's: a preview whose workers read `/kitchensink/sandbox/...` is
        // precisely the cross-stage data-loss path #119 was.
        //
        // ⚠️ What is no longer visible here is the concrete database name, because it is now chosen at deploy
        // time. That is the price of making agreement structural rather than derived, and it is the right
        // trade: the old assertion could prove they agreed with EACH OTHER, never that they agreed with
        // the stack that actually created the database.
        expect(names).toHaveLength(11);
        expect(new Set(names)).toEqual(new Set([recipeDatabaseNameParameter('pr-73')]));
    });

    it('reads the BASE stage parameter on a base platform stage', () => {
        const template = synth('prod', 'prod');

        expect(new Set(recipeDbNames(template))).toEqual(new Set([recipeDatabaseNameParameter('prod')]));
    });

    it('⛔ a preview and its base stage read DIFFERENT parameters — the #119 separation, stated directly', () => {
        expect(recipeDatabaseNameParameter('pr-73')).not.toBe(recipeDatabaseNameParameter('sandbox'));
    });
});
...
describe('RecipeWorkersStack — account erasure', () => {
    ...
    it('publishes NO migration-runner output — one runner per database means one output', () => {
        ...
        // `teardownSandboxPr.sh` §1 discovered doors across a PR's own stacks by the shape
        ...
        //
        // What remains of the output is its MIGRATION role: `runMigrations.sh run` resolves the runner
...
/**
 * The fewest breaching datapoints an alarm on OUR OWN published gauges may require.
 *
 * ⛔ Two, because tolerance is `datapointsToAlarm - 1` and one survivable missed publish is the minimum
 * that makes a self-published series alarmable at all: the emitters here are allowed to miss a tick (an
 * unaligned `Schedule.rate`, a throttle, a swallowed read), and under `BREACHING` a missed tick is a breach.
 */
const MINIMUM_DATAPOINTS_TO_ALARM = 2;

/**
 * Missed publishes an alarm survives: CloudWatch fires on M breaching datapoints, and under `BREACHING` an
 * absent datapoint IS a breach, so M-1 absences are survivable. `evaluationPeriods` only widens the window
 * they are counted over and does not appear here. Pure.
 *
 * @param datapointsToAlarm - The alarm's M.
 * @returns How many absent publishes it tolerates.
 */
const missedPublishesTolerated = (datapointsToAlarm: number): number => datapointsToAlarm - 1;

/**
 * Whether an alarm shape over one of OUR OWN published gauges may ship.
 *
 * ⛔ ONE HOME, called from both the fixture table and the template loop. An earlier version asserted the
 * rule in the template loop and RETYPED it in the table, so the table could not detect a change to the loop
 * — it was arithmetic over literals that happened to agree. That is the same defect (a rule restated rather
 * than shared) that the rule itself was written to catch. Pure.
 *
 * @param datapointsToAlarm - The alarm's M.
 * @returns Whether it tolerates at least one missed publish.
 */
const admitsAlarmShape = (datapointsToAlarm: number): boolean =>
    missedPublishesTolerated(datapointsToAlarm) >= MINIMUM_DATAPOINTS_TO_ALARM - 1;
...
describe('RecipeWorkersStack — alarm notifications', () => {
    ...
    /**
     * ⛔ THE ALARM'S THRESHOLD IS THE DECLARED CONSTANT, NOT A LITERAL THAT HAPPENS TO MATCH IT.
     *
     * `PARSE_JOB_STALL_THRESHOLD_SECONDS` carries a derivation — an hour is three orders of magnitude past
     * a happy path measured in seconds, and twenty-three hours short of the TTL that would discard the job
     * unannounced. A hand-typed `3600` at the alarm would satisfy every other assertion in this file while
     * silently decoupling the number from the reasoning that chose it, and the next person to revise the
     * derivation would change the constant and not the alarm.
     *
     * ⚠️ There used to be a SECOND declaration of this value in `src/parsing/parseJobExpiry.ts`, under a
     * `MUST EQUAL` comment no guard enforced. It is gone rather than guarded: nothing in the runtime read
     * it, because a threshold is an alarm concern — the gauge publishes the age, the alarm decides what is
     * too old. This case is what remains once the duplication is removed instead of policed.
     */
    it('⛔ alarms the stall gauge on the declared threshold constant', () => {
        const source = readFileSync(
            path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../lib/RecipeWorkersStack.ts'),
            'utf8',
        );
        const declared = /\bPARSE_JOB_STALL_THRESHOLD_SECONDS\s*=\s*(\d+)/u.exec(source)?.[1];

        expect(declared, 'PARSE_JOB_STALL_THRESHOLD_SECONDS is no longer declared in the stack').toBeDefined();

        template.hasResourceProperties('AWS::CloudWatch::Alarm', {
            MetricName: 'OldestRunningParseJobAgeSeconds',
            Threshold: Number(declared),
        });
    });

    /**
     * ⛔ THE DETECTOR IS TESTED AGAINST SHAPES THE TREE DOES NOT CONTAIN, which is the step whose absence
     * let the inverted formula through. A guard run only against a tree that satisfies it cannot
     * distinguish "the rule holds" from "the rule is wrong in a direction this tree never exercises" — and
     * reverting the shipped alarm to 1-of-1 does not separate them either, because 1-of-1 fails the correct
     * predicate AND the inverted one. Only a shape the tree has never held does: 1-of-5 (tolerance 0, which
     * the inverted form admitted) and 3-of-3 (tolerance 2, which it rejected).
     */
    it.each([
        { m: 1, n: 1, tolerated: 0, admitted: false },
        { m: 1, n: 5, tolerated: 0, admitted: false },
        { m: 2, n: 3, tolerated: 1, admitted: true },
        { m: 3, n: 3, tolerated: 2, admitted: true },
        { m: 2, n: 2, tolerated: 1, admitted: true },
    ])('rates $m-of-$n as tolerating $tolerated missed publishes', ({ m, tolerated, admitted }) => {
        expect(missedPublishesTolerated(m)).toBe(tolerated);
        expect(admitsAlarmShape(m)).toBe(admitted);
    });

    /**
     * ⛔ A SELF-PUBLISHED SERIES UNDER `BREACHING` MAY NOT PAGE ON ONE ABSENT DATAPOINT.
     *
     * `BREACHING` makes a missing window count as a breach. That is correct for these gauges — they publish
     * every tick including `0`, so an absence means the emitter stopped rather than that the system is
     * quiet. But it also means the alarm's tolerance for a missed publish is exactly
     * `datapointsToAlarm - 1` windows.
     *
     * ⛔ THAT IS `datapointsToAlarm`, NOT A FUNCTION OF `evaluationPeriods`, and an earlier version of this
     * guard got it backwards — it asserted `evaluationPeriods - datapointsToAlarm + 1 >= 2`, which reduces
     * to `M <= N - 1`. CloudWatch fires when M of the last N datapoints breach, and under `BREACHING` an
     * absent datapoint IS a breach, so M breaches are needed and M-1 absences are survivable; N only widens
     * the window they are counted over. The inverted form ADMITTED 1-of-5 (tolerance 0 — the precise defect
     * this guard exists to stop) and REJECTED 3-of-3 (tolerance 2 — the safest shape there is, and the one
     * `IdentityServiceStack`'s crash-loop alarm uses). It passed against the shipped 2-of-3 and against the
     * 1-of-1 mutant, so neither the suite nor a revert-and-check could tell it was wrong.
     *
     * At 1-of-1 the tolerance is ZERO: one jittered `Schedule.rate` tick, one Lambda throttle, or one
     * deliberately swallowed read (see `publishParseStallGauge`) becomes a page on a healthy system.
     *
     * ⚠️ Asserted as a RULE over every BREACHING `Commise/RecipeParse` alarm rather than against the one
     * that had the defect, so a future alarm on this namespace inherits it instead of repeating it. An
     * AWS-published series (`IdentityServiceStack`'s 1-minute crash-loop alarm) is a different case and is
     * deliberately out of scope here — this namespace is ours and its cadence is ours to miss.
     */
    it('⛔ no BREACHING alarm on our own parse gauges fires on a single absent datapoint', () => {
        const alarms = Object.entries(template.findResources('AWS::CloudWatch::Alarm')).filter(
            ([, alarm]) =>
                (alarm as { Properties?: { Namespace?: string; TreatMissingData?: string } }).Properties?.Namespace ===
                    'Commise/RecipeParse' &&
                (alarm as { Properties?: { TreatMissingData?: string } }).Properties?.TreatMissingData === 'breaching',
        );

        expect(alarms.length).toBeGreaterThan(0);

        for (const [logicalId, alarm] of alarms) {
            const properties = (alarm as { Properties: { DatapointsToAlarm?: number; EvaluationPeriods: number } })
                .Properties;
            const datapoints = properties.DatapointsToAlarm ?? properties.EvaluationPeriods;

            expect(
                admitsAlarmShape(datapoints),
                `${logicalId} tolerates no missed publish: ${datapoints} of ${properties.EvaluationPeriods}`,
            ).toBe(true);
        }
    });
    ...
    it('routes EVERY alarm to the SNS topic — no alarm may page nobody (the QE-001 defect)', () => {
        ...
        // ⚠️ It was 11, then 15, then 16, then 17, and is now 19: the parse leg's CRF-availability, DLQ and
        // throttle alarms, the handle-sync DLQ alarm the derived "every DLQ has a depth alarm" guard found
        // missing, ADR-0024 layer 1's input-bound alarm, §4b's residency-refusal alarm, and the two
        // parse-pipeline detectors (the landing-discard RATE and the job-stall gauge). The earlier revision
...
describe('RecipeWorkersStack — the schema barrier lives in its own stack now', () => {
    ...
    it('⛔ publishes no migration-function output — one runner per database means one output', () => {
        // The reader is `runMigrations.sh run`, which resolves the runner from the stack's outputs. Two
    ...
    it('still points every database-bound Lambda at the PER-PR logical database', () => {
        ...
        // The name is resolved from SSM at deploy time now, so what a synth can see — and what actually
        // matters — is that every one of them reads THIS stage's parameter rather than the base stage's.
        for (const name of names) {
            expect(name).toBe(recipeDatabaseNameParameter('pr-7'));
        }
    });
});
...
describe('RecipeWorkersStack — the bedrock:InvokeModel resource scope', () => {
    ...
    it('names a region outside the deploy region ONLY for the warranted profile, and never the unwarranted one', () => {
        // ⚠️ REWRITTEN for the Nova 2 Lite warrant (owner ruling 2026-09-12, ADR-0024 §4b). Cross-region ARNs
        // are the warrant's visible cost, so the claim is a CONFINEMENT: they exist, and ONLY for the entry the
        // ruling named. `nova-2-lite` is written out rather than derived — every other assertion in this file
        // reads `residencyClearance`, the same predicate the implementation calls, so one literal that cannot
        // move with the predicate is what survives a mutation disabling the filter everywhere.
        //
        // ⚠️ This covers "and never the unwarranted profile": any unwarranted entry's fan-out would BE a
        // cross-region ARN, and every one of those is asserted to belong to the warranted model.
        ...
        for (const arn of arns) {
            const outsideDeployRegion = !arn.includes('arn:aws:bedrock:us-east-1:');

            if (outsideDeployRegion) {
                expect(arn, `${arn} leaves the deploy region for a model with no warrant`).toContain('nova-2-lite');
            }
        }
    });
     ...
     */
    it('grants every residency-CLEARED profile its ARN and a conditioned fan-out, and an unwarranted one nothing', () => {
        const statements = bedrockStatements(template);

        // ⛔ ITS OWN NON-VACUITY FLOOR. Assertions over extracted collections are all satisfied by an
        // extractor that stopped matching anything — the failure mode that turns a guard into a green no-op.
        // A floor in the sibling test does not protect this one.
        expect(statements.length, 'no bedrock statement was discovered — this proves nothing').toBeGreaterThan(0);

        const profiles = Object.entries(BEDROCK_MODEL_REGISTRY).filter(
            ([modelId, entry]) => entry.invocation.invocationId !== modelId,
        );
        const cleared = profiles.filter(([, entry]) => residencyClearance(entry, 'us-east-1') !== 'unapproved');
        const unapproved = profiles.filter(([, entry]) => residencyClearance(entry, 'us-east-1') === 'unapproved');

        // ⛔ BOTH ARMS MUST BE INHABITED or one half of this test proves nothing. The day every profile is
        // warranted, the negative arm goes vacuous and this floor says so instead of passing quietly.
        expect(cleared.length, 'no profile is cleared — the positive assertions prove nothing').toBeGreaterThan(0);
        expect(unapproved.length, 'every profile is warranted — the refusal arm is unexercised').toBeGreaterThan(0);

        const arns = statements.flatMap(({ resources }) => resources);

        // RESTORED (this test's previous form asked for exactly this the day a profile was cleared): the
        // profile ARN, and a fan-out carrying its `bedrock:InferenceProfileArn` condition.
        for (const [modelId, entry] of cleared) {
            expect(
                arns.some((arn) => arn.includes(`:inference-profile/${entry.invocation.invocationId}`)),
                `${modelId} is cleared but has no inference-profile grant`,
            ).toBe(true);
        }

        expect(
            statements.filter(({ condition }) => condition !== undefined).length,
            'a cleared profile fans out with no InferenceProfileArn condition',
        ).toBeGreaterThan(0);

        // …and the refusal still holds for anything unwarranted.
        for (const [modelId, entry] of unapproved) {
            expect(
                arns.filter((arn) => arn.includes(entry.invocation.invocationId)),
                `${modelId} carries no warrant but is granted`,
            ).toEqual([]);
        }
    });
 ...
 */
/**
 * ⛔ A DEAD-LETTER QUEUE OUTLIVES THE QUEUE THAT FEEDS IT — derived from the redrive graph, never enumerated.
 *
 * On a STANDARD queue a message keeps its ORIGINAL enqueue timestamp when SQS moves it to a DLQ: "expiration
 * of a message is always based on its original enqueue timestamp" (SQS dead-letter-queue guide). So a DLQ
 * whose retention is SHORTER than its source's deletes late arrivals the moment they land — the message
 * exhausted its receives on day 3 of a 4-day source queue, arrived already older than a 3-day retention, and
 * was gone before any alarm could read its depth. That is the failure this asserts away, and it is invisible
 * to the depth alarm above: a queue nothing can reach reports zero, exactly like a healthy one.
 *
 * Both sides are DISCOVERED from the synthesized redrive graph for the reason recorded on the alarm test: a
 * hand-kept list of pairs cannot notice a pair it never knew about.
 */
describe('RecipeWorkersStack — a DLQ retains longer than the queue that feeds it', () => {
    const template = synth('sandbox');

    it('⛔ gives every dead-letter queue a retention above its source — derived, never enumerated', () => {
        const queues = template.findResources('AWS::SQS::Queue');

        /** Source-to-DLQ pairs, read out of each queue's own redrive policy. */
        const pairs = Object.entries(queues).flatMap(([sourceId, queue]) => {
            const target = (queue.Properties?.RedrivePolicy as { deadLetterTargetArn?: unknown } | undefined)
                ?.deadLetterTargetArn;
            const attribute = (target as { 'Fn::GetAtt'?: [string, string] } | undefined)?.['Fn::GetAtt'];

            return attribute === undefined ? [] : [{ sourceId, dlqId: attribute[0] }];
        });

        // Non-vacuity: this stack redrives archive, erasure, handle-sync, verification and parse. A reader
        // that found fewer is reading a broken template, and the loop below would pass by never running.
        expect(pairs.length).toBeGreaterThanOrEqual(5);

        for (const { sourceId, dlqId } of pairs) {
            // REQUIRED on both sides: an absent retention is the SQS default of 4 days arrived at by
            // omission, and comparing against `undefined` would pass vacuously rather than fail.
            const sourceRetention = requireSeconds(queues[sourceId]?.Properties?.MessageRetentionPeriod, sourceId);
            const dlqRetention = requireSeconds(queues[dlqId]?.Properties?.MessageRetentionPeriod, dlqId);

            expect(
                dlqRetention,
                `${dlqId} must retain at least as long as ${sourceId}, or a late dead-letter expires on arrival`,
            ).toBeGreaterThanOrEqual(sourceRetention);
        }
    });
});
...
/**
 * ⛔ THE TWO PARSE-PIPELINE DETECTORS — a rate that needs a denominator, and a stall with no event.
 *
 * Both close silences that were MEASURED, not imagined:
 *
 *  1. `parse landing discarded` ran at ~99.9% of landings for three days (2026-09-11 → 09-13, ~780/hour;
 *     4,647 / 29,085 / 16,969 a day against a near-zero baseline) with no metric and no alarm — only a
 *     `logger.info`. The user-visible consequence is an import that never completes.
 *  2. A job whose lines never land stays `running` for the full 24-hour TTL and is then discarded, with no
 *     error and nothing to click.
 */
describe('RecipeWorkersStack — the parse-pipeline detectors', () => {
    let template: Template;
    ...
    /**
     * ⛔ A RATE, NOT A COUNT, AND THE VOLUME FLOOR IS HALF THE DESIGN.
     *
     * The discard is a NORMAL outcome — R17 guards the landing on the stored digest, so every edited line
     * legitimately discards the message its edit superseded, and it ran at roughly a tenth of landings
     * before the incident. A count threshold would therefore have to sit above normal BUSY traffic, which
     * makes it blind at low volume; a bare ratio fires on 1-of-1 on a quiet queue. The expression answers
     * both: `IF(attempts >= 20, 100 * discards / attempts, 0)`.
     *
     * The three-arg `IF` is deliberate over a two-arg one. Below the floor it yields a definite `0` — an
     * explicit "not enough evidence to judge", which reads the same as healthy and is never a page; with no
     * traffic at all `attempts` has no datapoint, the expression has none, and `notBreaching` covers it.
     * Fail-safe in both directions, and neither depends on how CloudWatch handles a missing else-branch.
     */
    it('alarms on the DISCARD RATE with a volume floor, not on a discard count', () => {
        const alarm = Object.values(template.findResources('AWS::CloudWatch::Alarm')).find(
            (candidate) => candidate.Properties?.AlarmName === 'kitchensink-recipe-parse-landing-discarded-sandbox',
        );

        expect(alarm, 'the discard-rate alarm must exist').toBeDefined();
        expect(alarm?.Properties?.Threshold).toBe(50);
        expect(alarm?.Properties?.ComparisonOperator).toBe('GreaterThanThreshold');
        expect(alarm?.Properties?.TreatMissingData).toBe('notBreaching');

        // Three consecutive five-minute periods: the incident sustained ~65 attempts per period for three
        // days, so fifteen minutes is detection within the first hour with no chance of a one-window blip.
        expect(alarm?.Properties?.EvaluationPeriods).toBe(3);
        expect(alarm?.Properties?.DatapointsToAlarm).toBe(3);

        const queries = alarm?.Properties?.Metrics as readonly Record<string, never>[];
        const expression = queries.map((query) => query['Expression']).find(Boolean) as string | undefined;

        expect(expression).toContain('20');
        expect(expression).toContain('100');

        // ⛔ BOTH STATISTICS OF ONE SERIES. `Sum` is the discards, `SampleCount` is the landings attempted —
        // which is only true because the emitter publishes a datapoint on BOTH outcomes. An alarm that read
        // `Sum` alone would be the count this design exists to avoid.
        const stats = queries.flatMap((query) =>
            query['MetricStat'] === undefined ? [] : [(query['MetricStat'] as Record<string, unknown>)['Stat']],
        );

        expect(new Set(stats)).toEqual(new Set(['Sum', 'SampleCount']));
    });

    it('reads the discard series under the parse namespace, dimensioned by stage', () => {
        const alarm = Object.values(template.findResources('AWS::CloudWatch::Alarm')).find(
            (candidate) => candidate.Properties?.AlarmName === 'kitchensink-recipe-parse-landing-discarded-sandbox',
        );
        const queries = alarm?.Properties?.Metrics as readonly Record<string, never>[];
        ...
        for (const query of queries) {
            if (query['MetricStat'] === undefined) {
                continue;
            }

            const metric = (query['MetricStat'] as Record<string, Record<string, unknown>>)['Metric'];

            expect(metric?.['Namespace']).toBe('Commise/RecipeParse');
            expect(metric?.['MetricName']).toBe('ParseLandingDiscarded');
            expect(metric?.['Dimensions']).toEqual([{ Name: 'Stage', Value: 'sandbox' }]);
        }
    });

    /**
     * ⛔ `BREACHING`, and it is the opposite treatment to its neighbour one screen up — on purpose.
     *
     * The discard RATE is a ratio over traffic, so no traffic means no judgement and missing data must not
     * page. The stall gauge is read on a FIXED 15-minute tick and publishes `0` when nothing is stalled, so
     * its series is continuous by construction and an absence means the sweeper itself stopped running —
     * which is the failure that hides every other parse failure behind it.
     */
    it('treats a MISSING stall gauge as breaching, because the gauge publishes 0 when idle', () => {
        template.hasResourceProperties('AWS::CloudWatch::Alarm', {
            AlarmName: 'kitchensink-recipe-parse-job-stall-sandbox',
            MetricName: 'OldestRunningParseJobAgeSeconds',
            Namespace: 'Commise/RecipeParse',
            Threshold: 3600,
            ComparisonOperator: 'GreaterThanThreshold',
    ...
    it('routes both to the stack SNS topic — an alarm with no action is a widget', () => {
        for (const name of [
            'kitchensink-recipe-parse-landing-discarded-sandbox',
            'kitchensink-recipe-parse-job-stall-sandbox',
        ]) {
            const alarm = Object.values(template.findResources('AWS::CloudWatch::Alarm')).find(
                (candidate) => candidate.Properties?.AlarmName === name,
            );

            expect(alarm?.Properties?.AlarmActions, name).toHaveLength(1);
        }
    });
});

describe('⛔ the alarm feature flag (default OFF)', () => {
    it('creates NO alarm when the flag is off', () => {
        // Nineteen alarms in ONE stack, the largest concentration in the repository: archive backlog and
        // age, three DLQ depths, erasure age, orphan resurrection, nine on the LLM verification gate, and
        // the two parse-pipeline detectors. Every one of them was provisioned for every open PR's preview.
        synth('pr-91', 'sandbox', false).resourceCountIs('AWS::CloudWatch::Alarm', 0);
    });

    it('creates all nineteen worker alarms when the flag is on', () => {
        // Non-vacuity, and the count is load-bearing: this is a switch, and nothing was deleted to build it.
        synth('pr-91', 'sandbox', true).resourceCountIs('AWS::CloudWatch::Alarm', 19);
    });

    it('keeps the alarm topic in both states', () => {
        synth('pr-91', 'sandbox', false).resourceCountIs('AWS::SNS::Topic', 1);
        synth('pr-91', 'sandbox', true).resourceCountIs('AWS::SNS::Topic', 1);
    });
});

/**
 * U8 — PROD SCALES, NON-PROD DOES NOT (R1, R2, R4).
 *
 * ⛔ Asserted on the SYNTHESIZED template rather than on the source, because the two numbers live on
 * different resources — `reservedConcurrentExecutions` on the function, `maxConcurrency` on the event source
 * mapping — and the whole hazard is that they can disagree. Reading the source would show the intent; only
 * the template shows what is deployed together.
 */
describe('RecipeWorkersStack — consumer concurrency (U8)', () => {
    /** Reservation per Lambda logical id, from the synthesized template. */
    function reservations(template: Template): Map<string, number | undefined> {
        return new Map(
            Object.entries(template.findResources('AWS::Lambda::Function')).map(([id, fn]) => [
                id,
                fn.Properties?.ReservedConcurrentExecutions as number | undefined,
            ]),
        );
    }

    /** Event source mappings keyed by the queue-consuming function they attach to. */
    function mappings(template: Template): { scaling: unknown; fn: string }[] {
        return Object.values(template.findResources('AWS::Lambda::EventSourceMapping')).map((mapping) => ({
            scaling: mapping.Properties?.ScalingConfig,
            fn: JSON.stringify(mapping.Properties?.FunctionName ?? ''),
        }));
    }

    /** The two consumers U8 scales, by the substring their logical ids carry. */
    const CONSUMERS = ['IngredientVerificationFunction', 'RecipeParseLineFunction'];

    it('⛔ at PROD, both consumers reserve N and their pollers start at most N — the SAME N, N ≥ 2', () => {
        const template = synth('prod', 'prod');
        const reserved = reservations(template);

        const values = CONSUMERS.map((name) => {
            const entry = [...reserved].find(([id]) => id.includes(name));
            expect(entry, `${name} must declare a reservation`).toBeDefined();

            return entry?.[1];
        });

        // Both consumers, the same value, above one — which is what R1 asks for and what a mapping
        // concurrency of 2 is the AWS floor for.
        expect(new Set(values).size).toBe(1);
        expect(values[0]).toBeGreaterThanOrEqual(2);

        // ...and every queue-consuming mapping carries that same ceiling. A reservation raised without this
        // changes nothing; a mapping raised without the reservation throttles healthy messages (R2).
        const scaling = mappings(template)
            .map((mapping) => (mapping.scaling as { MaximumConcurrency?: number } | undefined)?.MaximumConcurrency)
            .filter((value) => value !== undefined);

        expect(scaling.length).toBeGreaterThanOrEqual(CONSUMERS.length);

        for (const value of scaling) {
            expect(value).toBe(values[0]);
        }
    });

    it('⛔ at a PREVIEW, both consumers reserve ONE and NO scaling config is emitted (ADR-0024 layer 2)', () => {
        const template = synth('pr-1');
        const reserved = reservations(template);

        for (const name of CONSUMERS) {
            const entry = [...reserved].find(([id]) => id.includes(name));
            expect(entry?.[1], `${name} must stay at one in a preview`).toBe(1);
        }

        // ⚠️ ABSENT, not `MaximumConcurrency: 1` — AWS refuses a value below 2, so "one at a time" can only
        // be expressed by leaving the mapping unconfigured. An empty `ScalingConfig` would also be a diff on
        // every non-prod stack for no behaviour.
        for (const mapping of mappings(template)) {
            expect(mapping.scaling, `${mapping.fn} must carry no scaling config`).toBeUndefined();
        }
    });

    /**
     * ⛔ R2's other half. A non-prod consumer's database is deliberately stopped for nine hours every night
     * (ADR-0007), and a consumer that cannot reach it fails every delivery — each failure spending a receive.
     * Below `window / visibilityTimeout`, a perfectly healthy message dead-letters because the stage was
     * asleep, and the DLQ depth ADR-0024 layer 4 alarms on fills with work nothing was wrong with.
     */
    it('⛔ every consumer queue tolerates a full nightly stop before dead-lettering', () => {
        const template = synth('sandbox');
        const nightlyStopSeconds = 9 * 60 * 60;
        const queues = template.findResources('AWS::SQS::Queue');

        const consumerQueues = Object.entries(queues).filter(
            ([id]) => id.includes('RecipeParseQueue') || id.includes('IngredientVerificationQueue'),
        );

        expect(consumerQueues.length).toBe(2);

        for (const [id, queue] of consumerQueues) {
            const visibility = queue.Properties?.VisibilityTimeout as number;
            const maxReceives = (queue.Properties?.RedrivePolicy as { maxReceiveCount?: number } | undefined)
                ?.maxReceiveCount;

            expect(visibility, `${id} must declare a visibility timeout`).toBeGreaterThan(0);
            expect(
                maxReceives,
                `${id} must outlast ${String(nightlyStopSeconds)}s of stopped database`,
            ).toBeGreaterThan(nightlyStopSeconds / visibility);
        }
    });
});

/**
 * U16 — every recipe Lambda can report to Sentry.
 *
 * ⛔ Asserted on the SYNTHESIZED template because the failure mode is a function that was MISSED. Ten
 * functions are declared across 1,400 lines of stack, each with its own environment block, and the one
 * nobody wired is the one whose errors stay invisible — which is indistinguishable from it working.
 */
describe('RecipeWorkersStack — Sentry wiring (U16)', () => {
    it('⛔ gives EVERY function a DSN, a release and a sample rate — derived, never enumerated', () => {
        const template = synth('sandbox');
        const functions = Object.entries(template.findResources('AWS::Lambda::Function'));

        // Exclude CDK's own provider framework functions, which are not ours to instrument. The guard this
        // stack already uses for the same exclusion reads `Handler`, so this matches it.
        const ours = functions.filter(([, fn]) => !String(fn.Properties?.Handler ?? '').startsWith('framework'));

        expect(ours.length).toBeGreaterThanOrEqual(10);

        for (const [id, fn] of ours) {
            const env = (fn.Properties?.Environment as { Variables?: Record<string, unknown> } | undefined)?.Variables;

            expect(env, `${id} must declare an environment`).toBeDefined();
            expect(env, `${id} must receive a Sentry DSN`).toHaveProperty('SENTRY_DSN');
            expect(env, `${id} must receive a release`).toHaveProperty('SENTRY_RELEASE');
            expect(env, `${id} must receive a trace sample rate`).toHaveProperty('SENTRY_TRACES_SAMPLE_RATE');
        }
    });

    it('samples traces down in prod and fully elsewhere', () => {
        const envOf = (template: Template): Record<string, unknown> => {
            const found = Object.entries(template.findResources('AWS::Lambda::Function')).find(
                ([, candidate]) => !String(candidate.Properties?.Handler ?? '').startsWith('framework'),
            );

            expect(found, 'the stack must declare at least one non-provider function').toBeDefined();

            const properties = found?.[1].Properties as { Environment?: { Variables?: Record<string, unknown> } };

            return properties.Environment?.Variables ?? {};
        };

        expect(envOf(synth('prod', 'prod'))['SENTRY_TRACES_SAMPLE_RATE']).toBe('0.1');
        expect(envOf(synth('pr-1'))['SENTRY_TRACES_SAMPLE_RATE']).toBe('1.0');
    });
});

describe('RecipeWorkersStack — the queue backstop holds no queue mutation (plan U12)', () => {
    /**
     * ⛔ U12's VERIFICATION, at the template. The plan's wording is "the guard fails if the check role is
     * granted any queue mutation", and the source-text register cannot see the one spelling that matters
     * most: `sqs:*`. This asserts the EXPANDED set IAM will evaluate.
     *
     * A backstop that could receive would take a message from the consumer it is watching; one that could
     * delete or purge could destroy the evidence it exists to report — and both would still look like an
     * ordinary consumer to every other assertion in this file.
     */
    it('⛔ is granted exactly sqs:GetQueueAttributes, and nothing else on any queue', () => {
        expect(sqsActionsGrantedTo(synth('pr-73', 'sandbox'), 'QueueCheckRole')).toEqual(['sqs:GetQueueAttributes']);
    });

    /**
     * ⛔ A scan that found nothing would satisfy the assertion above vacuously, which is how a guard stops
     * guarding without anyone noticing. The consuming roles in this same template DO hold receive and
     * delete, so a reader that returned an empty set everywhere is caught here.
     */
    it('⛔ and the reader is not blind — the real consumers DO hold receive and delete', () => {
        const consumers = Object.values(synth('pr-73', 'sandbox').findResources('AWS::IAM::Policy')).flatMap(
            (policy) => {
                const statements: unknown = policy.Properties?.PolicyDocument?.Statement;

                return Array.isArray(statements)
                    ? (statements as { Action?: unknown }[]).flatMap((statement) =>
        ...
        expect(consumers).toContain('sqs:ReceiveMessage');
        expect(consumers).toContain('sqs:DeleteMessage');
    });
});

/**
 * ⛔ THE LOG DRAIN IS CONDITIONAL, AND BOTH POSTURES ARE THE ASSERTION (ADR-0042).
 *
 * The forwarder's ARN used to arrive as `Fn.importValue`, which deadlocked this stack against the one that
 * publishes the export — `WebhooksStack` imports the global app's `HandleSyncTopicArn`, and ADR-0035 deploys
 * each schema stack ahead of everything that reads its database, the webhooks stack included.
 * `kitchensink-identity-schema-sandbox` died in `UPDATE_ROLLBACK_IN_PROGRESS` on "No export named …
 * LogForwarderArn found". The ARN is now a CI-resolved input and the filter is skipped when it is absent.
 *
 * ⚠️ THE SOURCE TEXT CANNOT PROVE THIS. Wrapping the filter in an `if` leaves `new
 * logs.SubscriptionFilter(` exactly where it was, so `logDrainRegister.test.ts`'s repo-wide count answers the
 * same number either way — it asserts the SHAPE and the workflow wiring, and this asserts the BEHAVIOUR,
 * here because this stack's `aws-cdk-lib` resolves only from this package's own install.
 *
 * ⚠️ The absent case is a SUPPORTED state, not a failure: a fresh account has no forwarder to point at.
 * Asserted so that "no filters" stays a deliberate posture rather than somewhere a later edit arrives.
 */
describe('the log drain is attached only when CI resolved a forwarder ARN', () => {
    ...
    it.each([
        ...
        const template = Template.fromStack(
            new RecipeWorkersStack(testApp({ context: { ...VPC_LOOKUP_CONTEXT } }), 'RecipeWorkersStackDrain', {
                env: { account: '123456789012', region: 'us-east-1' },
                stackName: 'kitchensink-recipe-workers-sandbox',
                stage: 'sandbox',
                baseStage: 'sandbox',
                vpcId: 'vpc-12345678',
                lambdaSecurityGroupId: 'sg-12345678',
                ...
                handleSyncTopicArn: 'arn:aws:sns:us-east-1:123456789012:kitchensink-handle-sync-sandbox',
                alarmsEnabled: true,
                ...(arn === undefined ? {} : { logForwarderArn: arn }),
            }),
        );
