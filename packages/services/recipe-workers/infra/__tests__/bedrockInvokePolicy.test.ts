 ...
 *
 * ⛔ A FIXTURE, and deliberately NOT the shipped warrant. `spendArithmetic.test.ts` holds the register of
 * which real entries are approved and with what date and reference (today: Nova 2 Lite, owner ruling
 * 2026-09-12) — approving a real model must show up as its own diff there, never as a test constant leaking
 * into the table. Its `reference` says "(fixture)" so a grep for the real warrant cannot land here.
 */
...
describe('bedrockInvokeStatements', () => {
     ...
     */
    it('grants NOTHING for a profile that carries no warrant, and grants the one that does', () => {
        // ⚠️ Driven from a two-entry FIXTURE rather than the shipped table, so the pair holds whatever the
        // registry decides next. The literals below are the fixture's own ids — hardcoded on purpose, because
        // every OTHER assertion in this file derives its expectation from `residencyClearance`, the same
        // predicate the implementation calls, so a mutation disabling the filter in both would be invisible.
        // Two ids pulling in OPPOSITE directions is the guard: it fails if the filter is turned off (the
        // unwarranted one appears) AND if it is stuck on (the warranted one vanishes).
        const registry = {
            ...throughProfile(
                'vendor.warranted-v1:0',
                'us.vendor.warranted-v1:0',
                [DEPLOY_REGION, 'us-west-2'],
                APPROVAL,
            ),
            ...throughProfile(
                'vendor.unwarranted-v1:0',
                'us.vendor.unwarranted-v1:0',
                [DEPLOY_REGION, 'us-west-2'],
                undefined,
            ),
        };
        const granted = bedrockInvokeStatements(registry, formatArn, DEPLOY_REGION).flatMap(
            ({ resources, throughInferenceProfileArns }) => [...resources, ...(throughInferenceProfileArns ?? [])],
        );

        // Unwarranted: named nowhere at all.
        expect(granted.filter((arn) => arn.includes('unwarranted'))).toEqual([]);
        // Warranted: reachable, or a leg whose model was warranted could not run.
        expect(
            granted.filter((arn) => arn.includes('us.vendor.warranted-v1:0')).length,
            'the warranted profile is not granted',
        ).toBeGreaterThan(0);
    });

    it('names a region outside the deploy region ONLY for a profile that carries a warrant', () => {
        // ⛔ THE WARRANT'S BLAST RADIUS, asserted rather than assumed. Cross-region ARNs are exactly what a
        // residency refusal eliminates, so their presence is the visible cost of the ruling — and it must be
        // CONFINED to the entry that was warranted. An unwarranted model's destination regions appearing here
        // would mean the filter leaked, which is the failure ADR-0024 §4b's "one change" rule exists to stop.
        const granted = bedrockInvokeStatements(BEDROCK_MODEL_REGISTRY, formatArn, DEPLOY_REGION).flatMap(
            ({ resources, throughInferenceProfileArns }) => [...resources, ...(throughInferenceProfileArns ?? [])],
        );

        const crossRegion = granted.filter((arn) => arn.includes(':us-east-2:') || arn.includes(':us-west-2:'));

        expect(
            crossRegion.length,
            'the warranted fan-out reaches no other region — is the warrant wired?',
        ).toBeGreaterThan(0);

        for (const arn of crossRegion) {
            expect(arn, `${arn} names a destination region for a model with no warrant`).toContain('nova-2-lite');
        }
    });
