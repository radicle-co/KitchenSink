...
export class RecipeWorkersStack extends Stack {
    ...
    public constructor(scope: Construct, id: string, props: RecipeWorkersStackProps) {
        ...
        //
        // ⚠️ #121 IS WHY THIS IS `grantConnect` AND NOT A HAND-BUILT ARN. This stack used to build the ARN itself
        // with `Stack.formatArn`, whose default `SLASH_RESOURCE_NAME` emits `…:dbuser/{resourceId}/{dbUser}` — an
        // ARN matching no real resource, so `rds-db:connect` was implicitly DENIED and RDS reported it as
        // `PAM authentication failed for user "recipe_app"` (SQLSTATE 28000). Every invocation of all six workers
        // failed on the live pr-73 preview (540/540 archive sweeps in three hours). The required shape is
