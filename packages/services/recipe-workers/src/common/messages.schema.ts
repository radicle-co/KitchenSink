 ...
 *
 * ⛔ `VerifyIngredientLineMessage` USED TO LIVE HERE AND DOES NOT ANY MORE — do not move it back. Its
 * producer is `recipe-service`, which carries this package as a **devDependency** only (for the infra
 * tests) and does not ship it in its image, so a runtime import of this module from the API resolves in the
 * workspace and throws `ERR_MODULE_NOT_FOUND` in the container. It now lives at
 * `@kitchensink/recipe-core/resolution/verification-message`, beside the gate policy that is run at both
 * ends for the identical reason. See that file's docstring.
 */
import { z } from 'zod';
 ...
 */
// ⛔ NOT DEFINED HERE ANY MORE. The producers (identity's PATCH route and the Clerk webhook) must validate
// against the same shape this consumer enforces, and they cannot import a deployable's module — so the
// contract lives in `@kitchensink/identity-core`, which both sides already depend on, and is re-exported
// here so this module stays the one place a recipe-workers reader looks for queue contracts.
export { handleSyncMessageSchema, MAX_DISPLAY_NAME_LENGTH } from '@kitchensink/identity-core';

/** The author-handle rename event's contract. */
export type { HandleSyncMessage } from '@kitchensink/identity-core';
