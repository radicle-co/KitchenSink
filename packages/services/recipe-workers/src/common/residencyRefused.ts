 ...
 *
 * ⚠️ The error does NOT cross a bundle boundary (the pipeline never re-creates it), and the name check is not a
 * house convention: the neighbouring guards (`crfInvoke.ts`, `@kitchensink/bedrock-client`'s `errors.ts`,
 * `verificationPrompt.ts`) are all bare `instanceof`. Do not copy the fallback as if either were true.
 *
