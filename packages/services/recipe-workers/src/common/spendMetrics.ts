 ...
 *
 * ⛔ Layer 4 detects counter BUGS, not a bypass. This metric is emitted BY the gated paths, so a caller that skipped the gate emits nothing. The
 ...
 *
 * ⛔ THE EMIT IN `gatedLlm.ts`'s `settleQuietly` IS WHAT MAKES THAT TRUE — do not remove it as redundant.
 * Without it `verifyLine.ts` is the ONLY emitter and the gated path's settle failures publish nothing. Because
 * ADR-0024's settle is never retried, each one leaves a worst-case reservation standing against the
 * $100/month ceiling for good, draining headroom with no signal until a denial.
 *
