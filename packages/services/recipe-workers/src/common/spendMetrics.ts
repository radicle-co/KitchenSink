 ...
 * so the alarm already deployed on this name (`RecipeWorkersStack.ts`) covers both legs, rather than waiting
 * for a second alarm nobody has written yet.
 *
 * ⛔ THAT WAS A FALSE CLAIM UNTIL 2026-09-12, and the emit in `gatedLlm.ts`'s `settleQuietly` is what made it
 * true — do not remove it as redundant. This paragraph asserted the parse leg was covered "from its first
 * invocation" and spoke of "the two emitters" while `verifyLine.ts` was the ONLY one: the gated path logged
 * its settle failures and published nothing. Because ADR-0024's settle is never retried, each one left a
 * worst-case reservation standing against the $100/month ceiling for good, draining headroom with no signal
 * until a denial. A docstring claiming coverage is worse than none, because it tells the next reader there is
 * nothing to add.
 *
