...
/**
 * ⚠️ REAL-SHAPED IDENTIFIERS. These were `'ver-1'`, `'rec-1'` and `'own-1'` — placeholders the archive
 * worker's own schema refuses (UUID version and recipe ids, a ULID owner). That was harmless while the
 * sweeper sent whatever the row held; it now validates the message it built before putting it on the queue
 * (GR-018 §18-b at the producer), so a fixture the consumer would reject would exercise the refusal path
 * instead of the dispatch path these tests were written for.
 */
const VERSION_1 = '4f2504e0-4f89-11d3-9a0c-0305e82c3301';
const VERSION_2 = '5f2504e0-4f89-11d3-9a0c-0305e82c3302';
const RECIPE_ID = '3f2504e0-4f89-11d3-9a0c-0305e82c3300';
const OWNER_ID = '01JQ8Z3K9PABCDEFGHJKMNPQRS';

function makePendingRow(overrides: Partial<PendingArchiveRow> = {}): PendingArchiveRow {
    return {
        id: 'pa-1',
        recipe_version_id: VERSION_1,
        recipe_id: RECIPE_ID,
        version_number: 3,
        created_by: OWNER_ID,
        ...overrides,
...
describe('archive-sweeper handler', () => {
    it('dispatches one SQS message per claimable outbox row', async () => {
        vi.mocked(getRecipeDbMock).mockReturnValue(
            dbWithPending([
                makePendingRow({ recipe_version_id: VERSION_1, version_number: 1 }),
                makePendingRow({ id: 'pa-2', recipe_version_id: VERSION_2, version_number: 2 }),
            ]) as never,
        );
        ...
        expect(first.versionId).toBe(VERSION_1);
    ...
    it('keeps sweeping the remaining rows when one send fails', async () => {
        vi.mocked(getRecipeDbMock).mockReturnValue(
            dbWithPending([
                makePendingRow({ recipe_version_id: VERSION_1 }),
                makePendingRow({ id: 'pa-2', recipe_version_id: VERSION_2 }),
            ]) as never,
        );
