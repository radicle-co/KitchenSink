 ...
 *
 *  1. **Archive bucket.** `versionArchiveWorker.ownerErasureRequested` narrows, but cannot close, the
...
// how `accountErasureWorker.test.ts` mocks the same `common/cdnInvalidation.js` boundary — the
// adapter's own real-vs-no-op branching is unit-tested in `common/__tests__/cdnInvalidation.test.ts`;
...
import { handler, readRecentlyCompletedOwners, SWEEP_BATCH_SIZE } from '../erasureOrphanSweeper.js';
