...
describe('handler batch isolation', () => {
    // The handler stamps the stage on the metric it records for a refused payload (GR-018 §18-b), and a
    // deployed Lambda always carries STAGE. Setting it here keeps the suite honest about that dependency
    // rather than letting the handler fall back to a placeholder in production.
    beforeEach(() => {
        process.env['STAGE'] = 'pr-91';
    });

    it('reports a failing record in batchItemFailures and RECORDS an unparseable one', async () => {
