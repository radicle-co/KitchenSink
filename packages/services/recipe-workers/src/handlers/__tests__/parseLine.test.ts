...
function build(
    overrides: Partial<ParseLineDeps> & { cacheRows?: unknown[]; llm?: unknown; claimAttempt?: number | null } = {},
) {
 ...
 */
function buildUnwarranted() {
    const built = build({ parseModelId: UNWARRANTED_MODEL_ID });
