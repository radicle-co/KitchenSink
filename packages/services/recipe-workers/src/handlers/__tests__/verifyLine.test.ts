...
// `initObservability` has run, so an assertion made without this would pass against a no-op — which is
// exactly how `captureHandled` came to ship with ZERO callers and nobody noticed.
...
vi.mock('@sentry/aws-serverless', () => ({
    ...
    withScope: vi.fn((run: (scope: { setContext: () => void }) => void) => {
...
function deps(overrides: Partial<VerificationDeps> = {}): VerificationDeps & {
    readonly spies: {
        ...
        claimVerification: ReturnType<typeof vi.fn>;
    ...
    const spies = {
        ...
        // ⛔ The U7 CLAIM is the default these cases assume: no verdict exists yet and this delivery owns
        // the attempt. A fake that omitted it would stop every case at the claim, which is exactly what the
        // claim is for — so the default has to say "first delivery of an unjudged line" out loud.
        claimVerification: vi.fn().mockResolvedValue({ kind: 'claimed', attempts: 1 }),
    ...
    return {
        ...
        store: {
            claimVerification: spies.claimVerification,
            recordAttemptFailure: spies.recordAttemptFailure,
            recordVerdict: spies.recordVerdict,
            rememberAgreement: spies.rememberAgreement,
        },
...
/**
 * ⛔ U7 — THE GATE CHECKS FOR A VERDICT BEFORE IT SPENDS.
 *
 * These cases are about the ORDER of the handler's own steps, which is what a store-level test cannot see:
 * that no reservation is made and no model is called on a duplicate, and that the claim happens after the
 * residency and pricing checks rather than before them.
 */
describe('the verification claim (U7)', () => {
    it('⛔ a duplicate with a stored verdict reserves NOTHING and calls no model', async () => {
        const d = deps();
        d.spies.claimVerification.mockResolvedValue({ kind: 'already-verified' });
    ...
    it('⛔ a delivery whose claim another holds reserves NOTHING and calls no model', async () => {
        const d = deps();
        d.spies.claimVerification.mockResolvedValue({ kind: 'held' });
    ...
    it('claims against the model the reservation was PLANNED for, not the raw setting', async () => {
        const d = deps();

        await processVerification(d, MESSAGE);

        expect(d.spies.claimVerification).toHaveBeenCalledWith(expect.any(String), SETTINGS.modelId, 60);
    });

    /**
     * Past the allowance the gate stops and the line PUBLISHES — which is what an absent verdict has always
     * meant, so nothing downstream changes except that the giving-up is now visible. Completing also keeps
     * the message off the DLQ, whose depth is the one signal ADR-0024 layer 4 alarms on.
     */
    it('⛔ past the attempt allowance it records the reason, spends nothing, and lets the line publish', async () => {
        const d = deps();
        d.spies.claimVerification.mockResolvedValue({ kind: 'claimed', attempts: 21 });
        ...
        expect(d.spies.converse).not.toHaveBeenCalled();
        expect(d.spies.recordVerdict).not.toHaveBeenCalled();
        expect(d.spies.recordAttemptFailure).toHaveBeenCalledWith(
            expect.any(String),
            SETTINGS.modelId,
    ...
    /**
     * ⛔ The claim sits AFTER the residency and pricing checks. Those decide whether this model may be called
     * at all — deterministically in (model, region), never per line — so claiming first would write an
     * attempt row for a delivery that was never going to spend, inflating a counter whose only job is to
     * bound real attempts.
     */
    it('⛔ a residency-refused model is refused BEFORE any claim is written', async () => {
        const d = deps({ registry: UNWARRANTED_REGISTRY });
        d.settings.resolve = vi.fn().mockResolvedValue({ ...SETTINGS, modelId: UNWARRANTED_MODEL_ID });

        await processVerification(d, MESSAGE);

        expect(d.spies.claimVerification).not.toHaveBeenCalled();
        expect(d.spies.reserve).not.toHaveBeenCalled();
    });
});
...
describe('the invocation id, and the identity it is not', () => {
    ...
    const depsForModel = (modelId: string, registry?: Readonly<Record<string, unknown>>): ReturnType<typeof deps> =>
        deps({
            settings: { resolve: vi.fn().mockResolvedValue({ ...SETTINGS, modelId }) },
    ...
    it('refuses ANY unwarranted model the SSM parameter names — it is the same table, judged the same way', async () => {
        // ⛔ The property is that the gate judges whatever it is POINTED AT, not a specific model: the
        // parameter is operator-editable mid-incident, so an unwarranted id can arrive without a deploy.
        // Claude Haiku 4.5 is the exemplar because it is `INFERENCE_PROFILE`-only over three regions and
        // carries no `residencyApproval`.
        //
        // ⚠️ NOT Nova 2 Lite, which is WARRANTED (owner ruling 2026-09-12) — so it is residency-clear and
        // would prove nothing here. Haiku 4.5 is deliberately left
        // unwarranted (see the registry entry), which is what keeps this refusal exercised at all.
        const d = depsForModel(UNWARRANTED_MODEL_ID, UNWARRANTED_REGISTRY);
...
/**
 * U22a step 3 — THE TWO SWALLOWS THAT LEAVE A DURABLE CONSEQUENCE RAISE AN ISSUE.
 *
 * ⛔ THE DISCRIMINATOR, stated so the next reader does not apply this mechanically. This handler catches in
 * many places and most of those catches are correct to stay quiet: they rethrow, or they have a defined
 * benign fallback, or their failure is already the subject of a metric an alarm reads. Two are different,
 * and what separates them is not severity but ACTIONABILITY — the failure leaves a durable consequence, and
 * acting on it requires the error's IDENTITY (its type, its constraint, its cause chain) rather than the
 * fact that it happened:
 *
 *  - a failed SETTLE strands a worst-case reservation against ADR-0024's one $100/month ceiling, and settle
 *    is never retried, so the headroom is gone permanently;
 *  - a failed VERDICT WRITE is safe for the request — absence of a verdict means publish — but it means the
 *    gate silently stopped gating, and nothing downstream can distinguish a line nobody verified from a
 *    line whose verdict was lost.
 *
 * ⚠️ Both already emit a metric and an error log. Neither carries the exception, and a counter cannot be
 * debugged. That is the whole of what these two cases assert.
 */
describe('the swallows that leave a durable consequence raise an issue', () => {
    beforeEach(async () => {
        vi.clearAllMocks();
    ...
    it('⛔ reports the error itself when the settle is refused — the reservation is stranded', async () => {
        const d = deps();
        const failure = new Error('deadlock detected');

        d.spies.settle.mockRejectedValue(failure);
        await processVerification(d, MESSAGE);

        expect(mockCaptureException).toHaveBeenCalledWith(failure);
    });

    it('⛔ reports the error itself when the verdict write fails — the gate stopped gating', async () => {
        const d = deps();
        const failure = new Error('relation does not exist');

        d.spies.recordVerdict.mockRejectedValue(failure);
        await processVerification(d, MESSAGE);

        expect(mockCaptureException).toHaveBeenCalledWith(failure);
    });

    it('⚠️ stays silent on the happy path — an issue per verification is how a project becomes ignored', async () => {
        const d = deps();
