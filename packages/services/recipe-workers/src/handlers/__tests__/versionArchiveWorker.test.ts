...
describe('handler', () => {
    ...
    it('touches NEITHER the database nor S3 when the message is shape-invalid', async () => {
        ...
        // boundary parse is removed, and it is deliberately asserted on the SINKS, which is the half that
        // matters and the half that survived GR-018 §18-b. The refusal no longer REJECTS — it is recorded and
        // completed, so the message is not redelivered — and the sinks are still untouched either way.
        await expect(runHandler(makeArchiveEvent({ ownerId: 'not-a-ulid' }))).resolves.not.toThrow();
    ...
    // ⚠️ THIS TEST PROVED THE OPPOSITE UNTIL 2026-09-15, and the reversal is GR-018 §18-b rather than a
    // change of mind about poison: "A queue/event consumer that rejects a payload on shape does not return it
    // to the queue for redrive. It records the rejection and completes the message." Propagating the
    // SyntaxError redelivered a body that can never become valid — five times here, twenty on the parse and
    // verification queues — and buried the producer bug under the retries. The coverage did not go away: the
    // refusal is now asserted in `invalidPayloadCompletion.test.ts` (one case per consumer, as AC-018-b
    // requires) and `parseArchiveMessage`'s own throw is still asserted above.
    it('completes a poison message once and records it, instead of redriving it', async () => {
        const event: SQSEvent = { Records: [makeSqsRecord('{broken')] };

        await expect(runHandler(event)).resolves.not.toThrow();
        // Nothing was written: a refused payload never reaches S3 or the database.
        expect(s3Send).not.toHaveBeenCalled();
    });
