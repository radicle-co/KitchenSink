...
/**
 * Raised when THIS database holds no record authorizing the destructive work a message asks for — the interlock
 * refusing to act. Matching guard: {@link isMisroutedErasureMessageError}.
 *
 * `missingRecord` names what was absent, because the queue carries two kinds of work (ADR-0040) and a human reading
 * the DLQ must know which record to go and look for: no `account_erasure_jobs` row for an erasure, no
 * `test_reset_jobs` row for a reset, or no `test_principals` registry row for a reset whose job exists — the
 * worker's own re-check of the registry the purge requires (derived from the claim, so not an independent witness). It is REQUIRED rather than defaulted, so a new
 * call site has to say which authorization it found missing.
 *
...
export class MisroutedErasureMessageError extends Error {
    ...
    constructor(ownerId: string, missingRecord: 'account_erasure_jobs' | 'test_reset_jobs' | 'test_principals') {
        super(
            `account-erasure-worker: no ${missingRecord} row for owner ${ownerId} in this database — refusing to ` +
                'act, and failing the delivery so the request is redelivered and surfaced rather than lost',
        );
