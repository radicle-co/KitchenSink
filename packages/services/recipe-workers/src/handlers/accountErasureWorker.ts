 ...
 *
 * This is the ONLY MODULE permitted to issue `DELETE FROM recipes` — every other "delete" in the system sets
 * `deleted_at` instead — and correspondingly the only module exempt from the `activeRecipes()` /
 * `no-raw-recipes-select` read-path rule (data-model.md §Hard purge). Exactly two functions here use that
 * exemption: {@link eraseRecipeRows} (GDPR erasure — SCOPED, keeps truly-public recipes pseudonymized) and
 * {@link purgeTestPrincipalRows} (ADR-0040's test-principal reset — TOTAL, keeps nothing). The reset lives in this
 * module, and rides this worker's queue, precisely BECAUSE a hard purge must read past tombstones and delete what
 * the read-path rule hides: a second module holding that exemption would be a second place a hard delete could
 * drift from the cascade graph and the FK-ordering rules recorded below, where one module keeps them in one place.
 * See {@link eraseRecipeRows} for why the exemption is load-bearing rather than a convenience.
 *
 ...
 *
 * **Two kinds of work, one queue (ADR-0040).** Each record is dispatched on `erasureQueueMessageKind` — a body with
 * no `kind` is an erasure (every pre-ADR-0040 message), `testPrincipalReset` is the reset, and ANY other value fails
 * the delivery. The reset keeps the erasure path's "no false success" ordering — claim before destroying, row-level
 * interlock, rows then both buckets then the CDN, `completed` last, errors recorded non-terminally — and differs in
 * two deliberate ways: it sweeps the WHOLE owner prefix (nothing is kept, so there is no per-recipe scoping to
 * preserve), and a redelivery that finds no ACTIVE job is acknowledged without purging (see
 * {@link resetTestPrincipal} for why an erasure's idempotent replay would be destructive there).
 */
...
export const parseErasureMessage = (record: SQSRecord): AccountErasureMessage =>
    readAccountErasureMessage(JSON.parse(record.body));

/**
 * Shape an already-parsed body into a typed erasure message — {@link parseErasureMessage} without the JSON step, so
 * the dispatcher parses each record ONCE. Same contract, same refusals.
 *
 * @param parsed - The parsed JSON body of one SQS record.
 * @returns The typed message. Pure.
 * @throws {InvalidErasureMessageError} When `ownerId` is not a valid ULID.
 */
const readAccountErasureMessage = (parsed: unknown): AccountErasureMessage => {
    const body = (typeof parsed === 'object' && parsed !== null ? parsed : {}) as Partial<AccountErasureMessage>;
...
/**
 * Shape an already-parsed body into a typed test-principal reset message (ADR-0040).
 *
 * The owner is validated EXACTLY as {@link readAccountErasureMessage} validates it, and for a sharper reason: the reset
 * sweeps the principal's WHOLE owner prefix, so a malformed id is the one input that could widen that prefix. The
 * caller has already established the kind; this re-states it on the returned value rather than trusting the cast.
 *
 * @param parsed - The parsed JSON body of one SQS record whose kind is `testPrincipalReset`.
 * @returns The typed message. Pure.
 * @throws {InvalidErasureMessageError} When `ownerId` is not a valid ULID.
 */
const readTestPrincipalResetMessage = (parsed: unknown): TestPrincipalResetMessage => {
    const body = (typeof parsed === 'object' && parsed !== null ? parsed : {}) as Partial<TestPrincipalResetMessage>;

    if (!isAppUserUlid(body.ownerId)) {
        throw new InvalidErasureMessageError(`ownerId must be a valid ULID, received ${JSON.stringify(body.ownerId)}`);
    }

    return { kind: 'testPrincipalReset', ownerId: body.ownerId, requestedAt: String(body.requestedAt) };
};
...
export const recordErasureJobError = async (
    ...
    await db.execute(sql`
        UPDATE account_erasure_jobs
        SET last_error = ${lastErrorText(error)}, updated_at = now()
        WHERE id = ${jobId}
    `);
};

/**
 * The text persisted to a job's `last_error` — the thrown message, stringified when a non-`Error` was thrown, capped
 * at {@link MAX_LAST_ERROR_LENGTH}. One definition for both job tables, so the two annotations cannot drift.
 *
 * @param error - The thrown value.
 * @returns The bounded annotation. Pure.
 */
const lastErrorText = (error: unknown): string =>
    (error instanceof Error ? error.message : String(error)).slice(0, MAX_LAST_ERROR_LENGTH);

/**
 * The claimed `test_reset_jobs` row a reset invocation is accountable for, and the attempt its claim counted. A `type`
 * for Drizzle's `execute<T>`.
 *
 * `attempts` is the claim's identity: only a claim increments it, so a completion or a release guarded on it touches
 * the row only while THIS invocation still holds it.
 */
export type TestResetJobClaim = {
    readonly id: string;
    readonly attempts: number;
};

/**
 * Claim the principal's QUEUED reset job, marking it `running` and counting the attempt (ADR-0040).
 *
 * The counterpart of {@link claimErasureJob}, resolved by `user_id` for the same reason:
 * `idx_test_reset_jobs_active_user` makes "the active reset job for this user" unique, so the message needs no job
 * id and cannot name a job that is no longer the active one.
 *
 * ⛔ **`queued` ONLY — the second place this path departs from erasure (LOW-5).** An erasure claim also takes a
 * `running` job, because re-running its idempotent statements over data that is gone for good is harmless. A reset's
 * principal is live again the moment its reset completes, so two deliveries purging at once is not harmless: the
 * later purge's READ COMMITTED statements and its owner-prefix sweep reach writes a test run made after the first
 * purge completed. `running` therefore means "an invocation owns this job", and a second delivery claims nothing. A
 * failed invocation hands its own claim back ({@link recordTestResetJobError}); a DEAD one (a Lambda timeout, which
 * records nothing) is handed back by the erasure sweeper once the row is idle past its staleness window — longer
 * than the worker's timeout — at the cost of recovery waiting for that window instead of the next SQS redelivery.
 *
 * @param db - The recipe database handle.
 * @param ownerId - The test principal being reset.
 * @returns The claim, or `undefined` when the principal has no queued reset (none at all, one already running, or one
 *   finished).
 * @sideEffect Updates `test_reset_jobs`.
 */
export const claimTestResetJob = async (
    db: NodePgDatabase<Record<string, never>>,
    ownerId: string,
): Promise<TestResetJobClaim | undefined> => {
    const result = await db.execute<TestResetJobClaim>(sql`
        UPDATE test_reset_jobs
        SET status = 'running', attempts = attempts + 1, updated_at = now()
        WHERE user_id = ${ownerId} AND status = 'queued'
...
/**
 * Does THIS database hold any `test_reset_jobs` row for the principal, in any status? The reset path's half of the
 * interlock {@link erasureJobExistsForOwner} documents: it tells a finished reset's duplicate delivery (a row exists)
 * from a misrouted message (no row at all).
 *
 * @param db - The recipe database handle.
 * @param ownerId - The principal the message names.
 * @returns True when a row exists.
 * @sideEffect Reads `test_reset_jobs`.
 */
export const testResetJobExistsForUser = async (
    db: NodePgDatabase<Record<string, never>>,
    ownerId: string,
): Promise<boolean> => {
    const result = await db.execute(sql`SELECT 1 FROM test_reset_jobs WHERE user_id = ${ownerId} LIMIT 1`);

    return result.rows.length > 0;
};

/**
 * Is the principal in THIS database's test-principal registry (`test_principals`, ADR-0040)?
 *
 * ⛔ The worker's own re-check of the registry. The route admitted the reset only because the signed `testPrincipal`
 * claim AND this row agreed; the worker cannot see the claim, but it can refuse to purge a user the registry does not
 * name. That turns "a producer bug enqueued a reset for a real user" from total data loss into a refused, surfaced
 * delivery. It is NOT an independent witness against a mis-marked Clerk user — the row is written from that same
 * claim (ADR-0040 §4).
 *
 * @param db - The recipe database handle.
 * @param ownerId - The principal the message names.
 ...
 */
export const isRegisteredTestPrincipal = async (
    db: NodePgDatabase<Record<string, never>>,
    ownerId: string,
): Promise<boolean> => {
    const result = await db.execute(sql`SELECT 1 FROM test_principals WHERE user_id = ${ownerId} LIMIT 1`);

    return result.rows.length > 0;
};

/**
 * Mark a claimed reset job `completed`, clearing a stale `last_error` — {@link markErasureJobCompleted}'s counterpart,
 * guarded on the claim.
 *
 * ⛔ `status = 'running' AND attempts = claim.attempts`, so a completion can only close the claim it holds. The row
 * can have moved on only one way: the sweeper judged this invocation dead, handed the job back to `queued`, and a newer
 * delivery claimed it. That delivery owns the job and will complete it, so a `false` here is reported, not thrown —
 * throwing would redeliver a message whose work is already owned.
 *
 * @param db - The recipe database handle.
 * @param claim - The claim this invocation made.
 * @returns `true` when this claim completed the job; `false` when a newer claim owns it.
 * @sideEffect Updates `test_reset_jobs`.
 */
export const markTestResetJobCompleted = async (
    db: NodePgDatabase<Record<string, never>>,
    claim: TestResetJobClaim,
): Promise<boolean> => {
    const result = await db.execute<{ id: string }>(sql`
        UPDATE test_reset_jobs SET status = 'completed', last_error = NULL, updated_at = now()
        WHERE id = ${claim.id} AND status = 'running' AND attempts = ${claim.attempts}
        RETURNING id
    `);
...
/**
 * Annotate a reset job with its latest attempt's failure and RELEASE the claim back to `queued` — never terminal.
 *
 * ⛔ Non-terminal for {@link recordErasureJobError}'s two reasons, not re-argued here: writing `failed` would drop the
 * job out of the set `erasureSweeper` re-drains, and it would free `idx_test_reset_jobs_active_user` so a fresh
 * request's row collides with this message's redelivery. The give-up decision stays the sweeper's.
 *
 * ⛔ And RELEASED, unlike erasure's annotation, because a delivery claims only `queued` ({@link claimTestResetJob}):
 * left `running`, every SQS redelivery of a failed purge would claim nothing, acknowledge itself as a duplicate, and
 * the DLQ would never fill. Releasing is safe because a failed purge re-run is idempotent and the recipe service's
 * 423 write lock holds the principal still while the job is active. Guarded on the claim, so a late failure never
 * releases a newer invocation's claim.
 *
 * @param db - The recipe database handle.
 * @param claim - The claim this invocation made.
 * @param error - The thrown value.
 * @sideEffect Updates `test_reset_jobs`.
 */
export const recordTestResetJobError = async (
    db: NodePgDatabase<Record<string, never>>,
    claim: TestResetJobClaim,
    error: unknown,
): Promise<void> => {
    await db.execute(sql`
        UPDATE test_reset_jobs SET status = 'queued', last_error = ${lastErrorText(error)}, updated_at = now()
        WHERE id = ${claim.id} AND status = 'running' AND attempts = ${claim.attempts}
    `);
};
...
export const eraseRecipeRows = async (
    ...
    return db.transaction(async (tx) => {
        ...
        // 15. Test-principal bookkeeping (ADR-0040, migrations 0044/0045) — DELETED, every status. Both tables hold
        //     an opaque ULID and bookkeeping and nothing else, so there is nothing to keep and a row with a NULLed id
        //     would mean nothing. An erasure leaves no row naming the person, test principal or not. (Containment
        //     refuses a test principal's own erasure request at the route, so for a real user both deletes match
        //     nothing; a service-triggered erasure of a pool member still reaches them.)
        await tx.execute(sql`DELETE FROM test_principals WHERE user_id = ${ownerId}`);
        await tx.execute(sql`DELETE FROM test_reset_jobs WHERE user_id = ${ownerId}`);
...
/**
 * Purge EVERYTHING a registered test principal owns in the recipe database, atomically (ADR-0040). No public
 * carve-out, no donate election, no pseudonymization: a test-pool slot must start its next run from an empty world,
 * so every user-keyed row is DELETED. The caller has already claimed the `test_reset_jobs` row and re-checked the
 * `test_principals` registry; this function only removes rows.
 *
 * ⛔ Keep this a `const` bound to an arrow function. `testResetSweepCoverage.test.ts` reads the purge's statement set
 * from the template literals inside THIS `VariableDeclaration`, and a `function` declaration would hide every one.
 *
 * The statements run in ONE transaction, in this order — the order is load-bearing, and every step was checked
 * against the migrations that define the constraint it has to satisfy:
 *
 *  1. **Ratings on ANY recipe** — `DELETE FROM recipe_ratings WHERE user_id`. The ones on the principal's own recipes
 *     would cascade with them; the ones on other users' recipes would not. The STATEMENT-level aggregate trigger
 *     (0010) re-derives every affected survivor's `average_rating`/`rating_count`, so a test principal's stars leave
 *     a real recipe's public average. Never written by hand.
 *  2. **Detach OTHER owners' clones** — `recipes.cloned_from_id` is `NO ACTION` and not deferrable (0001), so another
 *     user's clone pointing at one of the principal's recipes makes step 3 throw. Every one of the principal's
 *     recipes is deleted, so the survivors are exactly `owner_id <> principal` — unlike erasure, which has to guard
 *     on the removed SET because it keeps some of the owner's own recipes.
 *  3. **Every recipe** — `DELETE FROM recipes WHERE owner_id`: public and private, published and draft, tombstoned
 *     and live. Cascades (0001/0004/0010) to steps, ingredient lines, photos, versions (so `recipe_versions.created_by`
 *     goes with them — mutations are owner-only), collection memberships — including in OTHER users' collections —
 *     pending archives and the ratings on those recipes.
 *  4. **Collections** — all of them, cascading to memberships; other users' cloned collections have
 *     `source_collection_id` SET NULL by the FK (0003).
 *  5. **`author_handles`** — user-keyed, no FK.
 *  6. **Parse jobs** (0039) — cascading to their lines.
 *  7. **Private authored foods** (0040) that no `recipe_ingredients` row still references. AFTER step 3, so the
 *     principal's own lines are gone and only OTHER users' references can keep a row. A referenced row is RETAINED,
 *     `food_owner_id` intact: `recipe_ingredients.ingredient_id` is `NO ACTION`, so deleting it would fail the
 *     transaction, and nulling another user's line would corrupt their recipe.
 *  8. **Analytics events** — DELETED by `user_id` (erasure anonymizes; a purge keeps nothing). Safe for the folded
 *     counts: 0043 ships exactly ONE trigger, `AFTER INSERT`, so a DELETE cannot decrement `recipe_impact_signals`.
 *     ⚠️ Those counts are RECIPE-keyed and carry no user column, so they are outside this purge by key — events other
 *     users generated about the principal's recipes, and the counts they folded, stay. Keeping test traffic out of
 *     real counts is containment's job upstream (ADR-0040), not something a reset can undo after the fact.
 *  9. **The two correction tiers** (0021 `ingredient_resolution_mappings`, 0029 `ingredient_parse_corrections`) —
 *     hard-deleted by `user_id` under the 2026-09-13 owner ruling amending ADR-0027 for a principal the claim AND the
 *     registry both name. (Erasure still does NOT sweep these tables.) Both carry three `NO ACTION` self-FKs, so the
 *     delete is two statements per table over one DOOMED set:
 *       - DOOMED = the principal's rows, closed transitively over `corroborated_a`/`corroborated_b`. A
 *         `corroboration` binding citing a doomed row must itself be doomed, never detached:
 *         `…_corroboration_cites_both` makes both citations non-NULL on a binding. Deleting that binding un-promotes a
 *         correction the OTHER cook half-earned; their own author row survives, and a second real corroborator can
 *         promote it again. That is the intended containment outcome — a promotion a test principal contributed to
 *         was never a real consensus.
 *       - First NULL `superseded_by` on every row pointing INTO the set. `…_supersession_coherent` allows
 *         `superseded_by NULL` with `superseded_at` set, so a real user's row a doomed row superseded stays RETIRED,
 *         with no successor — deliberately not revived, because reviving a global row can collide with
 *         `idx_*_live_global` and would re-assert a ruling someone chose to replace.
 *       - Then delete the set. Doomed rows pointing at each other go in one statement: `NO ACTION` is checked at the
 *         end of the statement, when both ends are gone — the same property step 2 relies on.
 *     The set is recomputed by the second statement rather than bound as an array; a concurrent transaction that
 *     cites a doomed row between the two makes the delete fail and the transaction roll back — a refused, retried
 *     reset, never a partial one.
 *
 * ⛔ DELIBERATELY UNTOUCHED — each with its reason in `testResetSweepCoverage.test.ts`'s `EXEMPT_FROM_TEST_RESET`:
 * `account_erasure_jobs` (the erasure legal record), `test_principals` (the registry must keep agreeing or the next
 * reset of the slot is refused), `test_reset_jobs` (the job this purge runs under), `recipe_versions.created_by`
 * (reached by step 3's cascade).
 *
 * ⚠️ Residual, stated rather than hidden: `collections.source_owner_handle` on ANOTHER user's clone of the principal's
 * collection keeps the principal's display handle — this function deletes and never pseudonymizes, and that row is
 * somebody else's. Containment is what keeps real users from cloning a test principal's content.
 *
 * @param db - The recipe database handle.
 * @param ownerId - The registered test principal being purged.
 * @sideEffect Deletes/updates rows in RDS (cascading).
 */
export const purgeTestPrincipalRows = async (
    db: NodePgDatabase<Record<string, never>>,
    ownerId: string,
): Promise<void> => {
    // Named once per tier so the pointer-nulling UPDATE and the DELETE close over the IDENTICAL set; the table name
    // stays a literal in each so `testResetSweepCoverage.test.ts` can read which tables the purge mutates.
    const doomedMappings = sql`
        WITH RECURSIVE doomed(id) AS (
            SELECT id FROM ingredient_resolution_mappings WHERE user_id = ${ownerId}
            UNION
            SELECT m.id FROM ingredient_resolution_mappings m, doomed
             WHERE m.corroborated_a = doomed.id OR m.corroborated_b = doomed.id
        )
    `;
    const doomedCorrections = sql`
        WITH RECURSIVE doomed(id) AS (
            SELECT id FROM ingredient_parse_corrections WHERE user_id = ${ownerId}
            UNION
            SELECT c.id FROM ingredient_parse_corrections c, doomed
             WHERE c.corroborated_a = doomed.id OR c.corroborated_b = doomed.id
        )
    `;

    await db.transaction(async (tx) => {
        // 1. Ratings on anyone's recipes; the statement trigger re-derives every survivor's aggregate.
        await tx.execute(sql`DELETE FROM recipe_ratings WHERE user_id = ${ownerId}`);

        // 2. Detach other owners' clones of the principal's recipes (NO ACTION FK), before the delete.
        await tx.execute(sql`
            UPDATE recipes
            SET cloned_from_id = NULL, updated_at = now()
        ...
        // 3. Every recipe — no visibility/status carve-out, no tombstone filter. Cascades the recipe graph.
        await tx.execute(sql`DELETE FROM recipes WHERE owner_id = ${ownerId}`);

        // 4-6. Collections (cascading memberships), the handle read-model row, parse jobs (cascading lines).
        await tx.execute(sql`DELETE FROM collections WHERE owner_id = ${ownerId}`);
        await tx.execute(sql`DELETE FROM author_handles WHERE user_id = ${ownerId}`);
        await tx.execute(sql`DELETE FROM recipe_parse_jobs WHERE owner_id = ${ownerId}`);

        // 7. Private authored foods nothing still lines against — after step 3, so only OTHER users' lines count.
        await tx.execute(sql`
            DELETE FROM ingredients i
             WHERE i.food_owner_id = ${ownerId}
               AND NOT EXISTS (SELECT 1 FROM recipe_ingredients ri WHERE ri.ingredient_id = i.id)
        ...
        // 8. Analytics events — DELETE is count-safe: 0043 has no DELETE trigger, so recipe_impact_signals never moves.
        await tx.execute(sql`DELETE FROM analytics_events WHERE user_id = ${ownerId}`);

        // 9. The correction tiers: null successor pointers into the doomed set, then delete the set.
        await tx.execute(sql`
            ${doomedMappings}
            UPDATE ingredient_resolution_mappings SET superseded_by = NULL
             WHERE superseded_by IN (SELECT id FROM doomed)
        `);
        await tx.execute(sql`
            ${doomedMappings}
            DELETE FROM ingredient_resolution_mappings WHERE id IN (SELECT id FROM doomed)
        `);
        await tx.execute(sql`
            ${doomedCorrections}
            UPDATE ingredient_parse_corrections SET superseded_by = NULL
             WHERE superseded_by IN (SELECT id FROM doomed)
        `);
        await tx.execute(sql`
            ${doomedCorrections}
            DELETE FROM ingredient_parse_corrections WHERE id IN (SELECT id FROM doomed)
        `);
    });
};
...
export const eraseRecipeObjects = async (
    ...
    return deleteObjectsUnderPrefix(client, bucket, recipeMediaPrefix(ownerId, recipeId));
};

/**
 * Delete EVERY object under a test principal's whole owner prefix in one bucket (ADR-0040). Idempotent.
 *
 * **Owner-wide, deliberately — the opposite of {@link eraseRecipeObjects}' choice, for the opposite reason.** Erasure
 * keeps truly-public recipes, so it must not touch the owner prefix. A test-principal purge keeps NOTHING, and an
 * owner-prefix sweep also reaches what a per-recipe sweep cannot: an object orphaned under the owner by a late
 * write whose recipe row is already gone.
 *
 * ⛔ The owner must be a strict ULID, not merely non-blank. This is the widest prefix any worker deletes under, so
 * the guard is the one thing between a malformed id and `recipes/` or a sibling owner's subtree: `recipes` alone
 * would yield `recipes/recipes/`, and a partial id is a prefix of every owner that shares it — which a ULID's fixed
 * 26 characters make impossible.
 *
 * @param client - The S3 client.
 * @param bucket - The bucket to sweep.
 * @param ownerId - The test principal whose objects are purged.
 * @returns The number of objects deleted.
 * @throws {InvalidErasureMessageError} When `ownerId` is not a valid ULID.
 * @throws When S3 reports a per-key delete failure.
 * @sideEffect Deletes objects from S3.
 */
export const eraseOwnerObjects = async (client: S3Client, bucket: string, ownerId: string): Promise<number> => {
    if (!isAppUserUlid(ownerId)) {
        throw new InvalidErasureMessageError(`ownerId must be a valid ULID to sweep the owner prefix in ${bucket}`);
    }

    return deleteObjectsUnderPrefix(client, bucket, ownerMediaPrefix(ownerId));
};

/**
 * List and batch-delete every object under one prefix, page by page — the loop both sweeps share, so the "a 200 with
 * `Errors` is not success" rule lives once. Callers own the prefix's safety; this function trusts it.
 *
 * A page is always a safe batch: `ListObjectsV2` returns at most 1000 keys and `DeleteObjects` accepts at most 1000.
 *
 * @param client - The S3 client.
 * @param bucket - The bucket to sweep.
 * @param prefix - The already-validated key prefix.
 * @returns The number of objects deleted.
 * @throws When S3 reports a per-key delete failure.
 * @sideEffect Deletes objects from S3.
 */
const deleteObjectsUnderPrefix = async (client: S3Client, bucket: string, prefix: string): Promise<number> => {
    let deleted = 0;
...
/**
 * Dispatch one record on the kind of work it asks for (ADR-0040). The body is parsed ONCE.
 *
 * DESIGN PATTERN: Visitor — an exhaustive `switch` over the `ErasureQueueMessageKind` union, with `never` in the
 * default so a kind added to `@kitchensink/recipe-core` without a branch here is a compile error.
 *
 * @param record - The raw SQS record.
 * @param buckets - Both object buckets.
 * @param cdn - The CDN invalidation port.
 * @throws {SyntaxError} When the body is not JSON.
 * @throws {InvalidErasureMessageError} When the kind is unrecognised or the owner is malformed — never acked.
 * @sideEffect Whatever the dispatched path does.
 */
const processRecord = async (record: SQSRecord, buckets: ErasureBuckets, cdn: CdnInvalidationPort): Promise<void> => {
    ...
    const kind = erasureQueueMessageKind(body);

    switch (kind) {
        case undefined: {
            // ⛔ FAIL, never ack and never default: a kind this consumer does not understand must not run the most
            // destructive work in the system under a name it was never given, and must not vanish either.
            const stated = typeof body === 'object' && body !== null && 'kind' in body ? body.kind : body;

            throw new InvalidErasureMessageError(`unrecognised message kind ${JSON.stringify(stated)}`);
        }

        case 'accountErasure':
            await eraseAccount(readAccountErasureMessage(body), buckets, cdn);

            return;
        case 'testPrincipalReset':
            await resetTestPrincipal(readTestPrincipalResetMessage(body), buckets, cdn);
        ...
        default: {
            ...
            throw new InvalidErasureMessageError(`unhandled message kind ${String(unhandled)}`);
        }
    }
};

/**
 * The GDPR erasure path — unchanged by ADR-0040 apart from receiving its message from the dispatcher.
 *
 * @param message - The validated erasure message.
 * @param buckets - Both object buckets.
 * @param cdn - The CDN invalidation port.
 * @sideEffect Deletes/updates RDS rows and S3 objects, invalidates the CDN, updates `account_erasure_jobs`.
 */
const eraseAccount = async (
    { ownerId }: AccountErasureMessage,
    buckets: ErasureBuckets,
    cdn: CdnInvalidationPort,
): Promise<void> => {
    ...
    if (!job && !(await erasureJobExistsForOwner(db, ownerId))) {
        ...
        throw new MisroutedErasureMessageError(ownerId, 'account_erasure_jobs');
    }
...
/**
 * The test-principal reset path (ADR-0040): claim → registry re-check → purge rows → sweep BOTH buckets' owner
 * prefix → CDN purge → `completed`. The erasure path's "no false success" ordering, for the same reason: a
 * `completed` reset job releases the principal's write lock, so it must only ever mean the world is really empty.
 *
 * ⛔ **Only a QUEUED job authorizes a purge — where this path deliberately departs from erasure.** An
 * erasure replay that finds a `completed` row re-runs its idempotent statements, harmlessly: that owner's data is
 * gone for good. A test principal is live again the moment its reset completes — the next test run writes fixtures
 * straight away — so re-running the purge on a stale duplicate delivery would delete a RUNNING test's data out from
 * under it. A delivery that claims nothing but finds a row (completed, or abandoned to `failed` by the sweeper) is
 * therefore acknowledged as a no-op; one that finds no row at all is misrouted and FAILS, exactly as erasure's does.
 * A delivery that finds the job RUNNING is acknowledged the same way, for the same reason: another invocation owns the
 * purge, and a second one running beside it would reach the writes of a test run that starts after the first ends.
 *
 * @param message - The validated reset message.
 * @param buckets - Both object buckets.
 * @param cdn - The CDN invalidation port.
 * @throws {MisroutedErasureMessageError} When no reset job row exists for the principal in this database, or the
 *   registry does not name it — both refusals fail the delivery.
 * @sideEffect Deletes/updates RDS rows and S3 objects, invalidates the CDN, updates `test_reset_jobs`.
 */
const resetTestPrincipal = async (
    { ownerId }: TestPrincipalResetMessage,
    buckets: ErasureBuckets,
    cdn: CdnInvalidationPort,
): Promise<void> => {
    const db = getRecipeDb();
    const job = await claimTestResetJob(db, ownerId);

    if (!job) {
        if (await testResetJobExistsForUser(db, ownerId)) {
            logger.info(
                'account-erasure-worker: no QUEUED test reset for principal (running elsewhere, or finished) — ' +
        ...
        logger.warn('account-erasure-worker: no test reset job for principal in this database — refusing to purge', {
            ownerId,
        });

        throw new MisroutedErasureMessageError(ownerId, 'test_reset_jobs');
    }

    try {
        // Defence in depth: the route checked the signed claim AND the registry; the worker re-checks the registry
        // it can see. Inside the try, so the refusal is recorded on the claimed job and retried to the DLQ.
        if (!(await isRegisteredTestPrincipal(db, ownerId))) {
            throw new MisroutedErasureMessageError(ownerId, 'test_principals');
        }

        await purgeTestPrincipalRows(db, ownerId);

        // Rows first, then BOTH buckets' whole owner prefix, then the CDN — for the reasons `eraseAccount` records:
        // real deletion never waits on an edge-cache purge.
        const deletedMedia = await eraseOwnerObjects(s3, buckets.media, ownerId);
        const deletedArchives = await eraseOwnerObjects(s3, buckets.archive, ownerId);

        await cdn.invalidate([cdnOwnerPrefixInvalidationPath(ownerId)]);

        if (!(await markTestResetJobCompleted(db, job))) {
            // Not an error to record or a delivery to fail: the sweeper handed this claim to a newer delivery, which
            // owns the job now. Releasing or annotating it here would reach into that invocation's claim.
            logger.warn('account-erasure-worker: test reset purged, but a newer claim owns the job — not completing', {
                ownerId,
                jobId: job.id,
        ...
        logger.info('test principal reset', { ownerId, jobId: job.id, deletedMedia, deletedArchives });
    } catch (error) {
        try {
            await recordTestResetJobError(db, job, error);
        } catch (annotationError) {
            logger.error('account-erasure-worker: could not record test reset job error', {
                ownerId,
                jobId: job.id,
                error: annotationError instanceof Error ? annotationError.message : String(annotationError),
            });
