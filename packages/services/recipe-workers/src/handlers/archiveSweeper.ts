...
const rawHandler = async (): Promise<void> => {
    ...
    for (const row of rows) {
        const message = toArchiveMessage(row, requestedAt);

        // ⛔ REFUSED BEFORE THE QUEUE. The worker parses this body against the same schema and would throw,
        // spending five receives and a DLQ slot to learn what `safeParse` answers here for nothing. A row
        // that cannot produce a valid message is a DATA defect — an owner id that is not a ULID, a version
        // number that is not a number — and re-sending it every tick would never fix it. The row is left
        // alone: it keeps its outbox record, so the archive is still owed and still visible in the backlog
        // metric, rather than being silently dropped.
        if (!recipeVersionArchiveMessageSchema.safeParse(message).success) {
            logger.error('archive-sweeper refused a row: it cannot produce a valid archive message', {
