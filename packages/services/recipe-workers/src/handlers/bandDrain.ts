...
import { planReservation, type ModelRegistryEntry } from '@kitchensink/recipe-core/spend/spend-arithmetic';
...
import { PARSE_METRIC_NAMESPACE } from '../parsing/crfInvoke.js';
import {
...
/**
 * The gauge name `RecipeParseJobStallAlarm` watches.
 *
 * ⛔ MUST EQUAL the `metricName` of that alarm in `infra/lib/RecipeWorkersStack.ts`. The alarm extracts by
 * exact namespace, dimension and metric name, so a divergence fails nothing at deploy time — it leaves the
 * alarm watching a metric nobody publishes. `serviceInfraWiringInvariants` W3 asserts the pairing from the
 * alarm's side; the namespace is imported from its owner rather than restated.
 */
export const PARSE_JOB_STALL_METRIC_NAME = 'OldestRunningParseJobAgeSeconds';
...
export async function drainRevokedBands(deps: BandDrainDeps): Promise<{ sent: number; budget: number }> {
    ...
    for (const skip of skips) {
        // ⛔ REFUSED BEFORE SENT, never after: the stored message is replayed from a row written by an earlier
        // release, so a contract that has moved on since would otherwise put a message the consumer must
        // reject onto the queue — where it costs 20 receives and a DLQ slot to discover. A refused skip is
        // MARKED DRAINED rather than left: re-sending it every 15 minutes forever is the retry GR-018 §18-b
        // refuses, and the row remains as the record of what was dropped.
        if (!verifyIngredientLineMessageSchema.safeParse(skip.message).success) {
            logger.error('band drain refused a stored skip: it no longer matches the verification contract', {
                skipId: skip.id,
            });
    ...
    if (remaining > 0) {
        ...
        for (const redrive of redrives) {
            // Same rule as the skips above: refuse before the queue, mark so it cannot loop.
            if (!verifyIngredientLineMessageSchema.safeParse(redrive.message).success) {
                logger.error('band drain refused a stored re-drive: it no longer matches the contract', {
...
function productionDeps(stage: string, region: string, queueUrl: string): BandDrainDeps {
    ...
    cachedDeps = {
        ...
        async send(message: unknown): Promise<void> {
            // ⛔ NO `MessageGroupId`, and that is a DECISION rather than an omission (U8, owner ruling
            // 2026-09-16). SQS fair queues label a message's tenant with a group id, and U8 gives the PARSE
            // queue one because a bulk import is a parse job — 200 messages from one cook at once. The
            // VERIFICATION queue this drain feeds is deliberately left unlabelled: its message carries no
            // submitter, and `verificationMessage.ts` records the owner's standing ruling that `ownerId`
            // was removed and must not be re-added, on the ground that a DLQ is a copy of personal data
            // outside every erasure path. Adding a tenant here would mean putting one back.
            //
            // ⚠️ Un-grouped is not un-fair by default: AWS treats every message WITHOUT a group as its own
            // distinct tenant. What is genuinely given up is stated rather than hidden — if a bulk import
            // ever does flood the verification queue, one submitter can still starve others there.
...
/**
 * ⛔ THE STALL GAUGE — publish how long the oldest still-running parse job has been running.
 *
 * `PARSE_JOB_TTL_HOURS` is 24. A job whose lines never land stays `running` for that whole day and is then
 * silently discarded; the cook sees an import that never completes — no error, no failed state, nothing to
 * click. The stall has no EVENT to log, because it IS the absence of one, so the only way to see it is to
 * read the backlog on a tick. It rides this drain's 15-minute schedule for the same reason the TTL sweep
 * does: a Lambda of its own would be a new ADR-0004 NAT consumer and a new migration-barrier entry for one
 * indexed SELECT.
 *
 * ⛔ IT PUBLISHES EVERY TICK, ZERO INCLUDED. A gauge that only spoke up when something was wrong would
 * leave `RecipeParseJobStallAlarm` unable to tell "nothing is stalled" from "the emitter is gone" — and
 * treating a MISSING series as a fault is precisely what that alarm does, so a gappy series would page on
 * every idle window.
 *
 * ⛔ IT SWALLOWS ITS OWN FAILURE. This is observation riding on somebody else's invocation: a database blip
 * while reading a gauge must never fail the drain that just sent real work, and must never cost that work a
 * redelivery. The failure is logged and the tick publishes nothing.
 *
 * ⚠️ THAT SWALLOW IS WHY THE ALARM IS 2-OF-3, and the two decisions have to be read together. Under
 * `BREACHING` an absent window counts AS a breach, so at 1-of-1 — which is what that alarm was — a single
 * swallowed read WAS the page, not a lost datapoint. Two of three 15-minute windows must now be absent or
 * over threshold, which is what makes this swallow cost a datapoint rather than a false alarm, while a
 * SUSTAINED read failure still shows up within 45 minutes. Do not narrow that alarm back to a single
 * datapoint without removing this swallow first.
 *
 * @param deps - The recipe pool, this deploy's stage, and the metric sink.
 * @sideEffect Reads `recipe_parse_jobs` and writes one EMF line.
 */
export async function publishParseStallGauge(deps: {
    readonly pool: ExpiryQueryable;
    readonly stage: string;
    readonly emit: (metric: EmfMetric) => void;
}): Promise<void> {
    try {
        const ageSeconds = await oldestRunningParseJobAgeSeconds(deps.pool);

        deps.emit({
            namespace: PARSE_METRIC_NAMESPACE,
            name: PARSE_JOB_STALL_METRIC_NAME,
            unit: 'Seconds',
            stage: deps.stage,
            value: ageSeconds,
        });
    } catch (error) {
        logger.warn('parse-job stall gauge could not be read this tick', {
            error: error instanceof Error ? error.message : String(error),
        });
    }
}
...
const rawHandler = async (): Promise<void> => {
    ...
    // ⚠️ AFTER the sweep, deliberately: the sweep has just retired every job past its TTL, so what the
    // gauge reads is the backlog of jobs still genuinely owed an answer rather than a pile the sweep was
    // about to clear. Reading first would report a stall that no longer exists.
    await publishParseStallGauge({ pool: getRecipePool(), stage, emit: emitMetric });
};
