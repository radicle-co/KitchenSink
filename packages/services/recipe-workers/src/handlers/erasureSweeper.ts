...
import type { AccountErasureMessage, TestPrincipalResetMessage } from '@kitchensink/recipe-core';
 ...
 *
 * **It backstops BOTH kinds of work the erasure queue carries (ADR-0040).** A `test_reset_jobs` row is claimed,
 * errored and completed by the same worker under the same "never terminal" rule, so it has the same failure a
 * backstop exists for — a lost eager send leaves it `queued` forever, and a dead invocation leaves it `running`
 * where no delivery may claim it ({@link requeueStaleTestResetJob}) — with a sharper operational cost: while a reset
 * is active the recipe service answers the principal's writes with `423`, so a stuck reset wedges a whole test-pool
 * slot. Stuck resets are re-sent as `testPrincipalReset` messages and given up on under the SAME policy
 * ({@link ERASURE_GIVE_UP_ATTEMPTS} + {@link ERASURE_GIVE_UP_AGE_SECONDS}), because the policy is arithmetic about
 * the queue's `maxReceiveCount` and visibility timeout, and both kinds ride that one queue. The age METRIC stays
 * erasure-only: it drives a compliance alarm, and a stuck test reset is not a compliance incident.
 */
...
/** The `test_reset_jobs` columns one re-drain needs (ADR-0040). */
export type StaleTestResetJobRow = {
    readonly id: string;
    /** The test principal's app-user ULID — the entire reset message contract. */
    readonly user_id: string;
    /** Claims so far; incremented by the worker on every claim. */
    readonly attempts: number;
    /** Seconds since THIS job row was created — the give-up age floor, as for erasure. */
    readonly age_seconds: number;
};
...
export function toErasureMessage(row: StaleErasureJobRow, requestedAt: string): AccountErasureMessage {
    ...
    //
    // `kind` is stated explicitly (ADR-0040). The worker honours an absent kind as an erasure, but only for messages
    // produced before the queue carried a second kind; a producer leaning on that default is one refactor from a
    // reset that reads as an erasure.
    return {
        kind: 'accountErasure',
        ownerId: row.owner_id,
        requestedAt,
        publishRecipeIds: row.publish_recipe_ids ?? [],
    };
}

/**
 * Shape a stuck reset job into the worker's `testPrincipalReset` contract (ADR-0040). Owner-scoped with no job id,
 * for {@link toErasureMessage}'s reason: `idx_test_reset_jobs_active_user` makes the active job for a user unique.
 *
 * @param row - The stale reset job to re-dispatch.
 * @param requestedAt - ISO 8601 dispatch time.
 * @returns The message body. Pure.
 */
export function toTestPrincipalResetMessage(row: StaleTestResetJobRow, requestedAt: string): TestPrincipalResetMessage {
    return { kind: 'testPrincipalReset', ownerId: row.user_id, requestedAt };
}
...
/**
 * Read the in-flight test-reset jobs that have stopped making progress — {@link claimStaleErasureJobs} over
 * `test_reset_jobs` (ADR-0040), with the same staleness window, ordering and batch cap.
 *
 * @param db - The recipe database handle.
 * @returns The stale in-flight reset jobs, oldest first.
 * @sideEffect Reads `test_reset_jobs`.
 */
export async function claimStaleTestResetJobs(
    db: NodePgDatabase<Record<string, never>>,
): Promise<StaleTestResetJobRow[]> {
    const result = await db.execute<StaleTestResetJobRow>(sql`
 ...
 * `accountErasureWorker.recordErasureJobError` deliberately never writes `failed`: one attempt failing
...
/**
 * Mark a stuck reset job `failed` — {@link abandonExhaustedJob}'s decision for `test_reset_jobs` (ADR-0040), with the
 * same in-flight guard against the read→update window. Writing `failed` frees `idx_test_reset_jobs_active_user`,
 * which releases the principal's write lock and lets a fresh reset be requested.
 *
 * @param db - The recipe database handle.
 * @param job - The exhausted reset job.
 * @sideEffect Updates `test_reset_jobs`.
 */
export async function abandonExhaustedTestResetJob(
    db: NodePgDatabase<Record<string, never>>,
    job: StaleTestResetJobRow,
): Promise<void> {
    await db.execute(sql`
...
/**
 * Hand a stale reset job back to `queued` so the message the sweeper is about to re-send has a job a delivery may
 * claim (ADR-0040, LOW-5).
 *
 * ⛔ Required, not tidying: a reset delivery claims ONLY a `queued` job (`claimTestResetJob`), so a job left `running`
 * by a DEAD invocation — a Lambda timeout records nothing — would have its re-sent message acknowledged as a duplicate
 * forever, holding the principal's 423 write lock. A `running` row idle past {@link STALE_AFTER} is dead, not slow:
 * every claim bumps `updated_at` and the worker's timeout is shorter than the window.
 *
 * Guarded on the ATTEMPT the sweeper read as well as on being in flight: only a claim increments `attempts`, so a job a
 * live delivery claimed between the read and this update is left to that delivery, and a job that finished stays
 * finished. `updated_at` is deliberately left alone, so a job whose re-sent message is lost is re-sent on the next tick
 * exactly as a stale `queued` job always was.
 *
 * @param db - The recipe database handle.
 * @param job - The stale reset job, as read.
 * @returns `true` when the job is `queued` and may be re-dispatched; `false` when it moved on and must not be.
 * @sideEffect Updates `test_reset_jobs`.
 */
export async function requeueStaleTestResetJob(
    db: NodePgDatabase<Record<string, never>>,
    job: StaleTestResetJobRow,
): Promise<boolean> {
    const result = await db.execute<{ id: string }>(sql`
        UPDATE test_reset_jobs SET status = 'queued'
        WHERE id = ${job.id} AND status IN ('queued', 'running') AND attempts = ${job.attempts}
        RETURNING id
    `);
...
/** How one kind of stuck job is recovered — what {@link redrainStuckJobs} needs to know about it. */
interface StuckJobKind<Job extends { readonly id: string; readonly attempts: number; readonly age_seconds: number }> {
    /** Log label for the kind of work. */
    readonly label: string;
    /** The owner id to log. */
    readonly ownerOf: (job: Job) => string;
    /** Write the give-up transition. */
    readonly abandon: (job: Job) => Promise<void>;
    /**
     * Make the job claimable by the message about to be re-sent. `false` means it moved on in between and must not be
     * re-dispatched.
     */
    readonly prepareRedispatch: (job: Job) => Promise<boolean>;
    /** The message that re-dispatches the job. */
    readonly toMessage: (job: Job, requestedAt: string) => AccountErasureMessage | TestPrincipalResetMessage;
}

/**
 * Re-dispatch or abandon each stuck job of one kind. The give-up rule and the per-job isolation live here ONCE, so the
 * two kinds of work on the erasure queue cannot drift onto different policies.
 *
 * @param jobs - The stale jobs.
 * @param kind - How this kind of job is recovered.
 * @param queueUrl - The erasure queue.
 * @param requestedAt - ISO 8601 dispatch time.
 * @returns How many were re-dispatched and how many abandoned.
 * @sideEffect Sends SQS messages and writes give-up transitions.
 */
async function redrainStuckJobs<
    Job extends { readonly id: string; readonly attempts: number; readonly age_seconds: number },
>(
    jobs: readonly Job[],
    kind: StuckJobKind<Job>,
    queueUrl: string,
    requestedAt: string,
): Promise<{ readonly redispatched: number; readonly abandoned: number }> {
    let redispatched = 0;
    let abandoned = 0;

    for (const job of jobs) {
        // Per-job try/catch, never per-batch: one owner's throttled send or deadlocked update must not
        // strand another owner's request. A job that errors here keeps its row and the next tick picks it
        // up again — the row-as-truth design is what makes that safe.
        try {
            ...
            // re-requested job whose counter was inflated by a prior cycle's stale messages is re-dispatched to
            ...
            if (job.attempts >= ERASURE_GIVE_UP_ATTEMPTS && job.age_seconds >= ERASURE_GIVE_UP_AGE_SECONDS) {
                await kind.abandon(job);
                abandoned += 1;
                logger.error(`erasure-sweeper: abandoned a ${kind.label} job after exhausting its attempts`, {
                    jobId: job.id,
                    ownerId: kind.ownerOf(job),
                    attempts: job.attempts,
                });
                continue;
            }

            if (!(await kind.prepareRedispatch(job))) {
                continue;
            }
            ...
            await sqs.send(
                new SendMessageCommand({
                    QueueUrl: queueUrl,
                    MessageBody: JSON.stringify(kind.toMessage(job, requestedAt)),
                }),
            );
            redispatched += 1;
        } catch (error) {
            logger.error(`erasure-sweeper: could not process a stuck ${kind.label} job`, {
                jobId: job.id,
                ownerId: kind.ownerOf(job),
...
/**
 * Re-drain every stuck erasure AND test-reset job, and abandon the ones that have exhausted their attempts.
 *
...
const rawHandler = async (): Promise<void> => {
    ...
    const resets = await claimStaleTestResetJobs(db);

    if (jobs.length === 0 && resets.length === 0) {
        return;
    }

    const requestedAt = new Date().toISOString();
    const erasures = await redrainStuckJobs(
        jobs,
        {
            label: 'erasure',
            ownerOf: (job) => job.owner_id,
            abandon: (job) => abandonExhaustedJob(db, job),
            // An erasure delivery claims a `running` job too (its replay is idempotent over data gone for good), so a
            // stale erasure is claimable as it stands.
            prepareRedispatch: async () => true,
            toMessage: toErasureMessage,
        },
        queueUrl,
        requestedAt,
    );
    const testResets = await redrainStuckJobs(
        resets,
        {
            label: 'test reset',
            ownerOf: (job) => job.user_id,
            abandon: (job) => abandonExhaustedTestResetJob(db, job),
            prepareRedispatch: (job) => requeueStaleTestResetJob(db, job),
            toMessage: toTestPrincipalResetMessage,
        },
        queueUrl,
        requestedAt,
    );

    logger.info('erasure-sweeper swept', {
        stale: jobs.length,
        redispatched: erasures.redispatched,
        abandoned: erasures.abandoned,
        staleTestResets: resets.length,
        redispatchedTestResets: testResets.redispatched,
        abandonedTestResets: testResets.abandoned,
