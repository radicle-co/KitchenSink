...
import { captureHandled, initObservability, withObservability } from '../common/observability.js';
 ...
 */
const VERIFICATION_CALL_SITE: SpendCallSite = VERIFICATION_GATE_CALL_SITE;
 ...
 */
const CACHE_TOKENS_METRIC_NAME = 'VerificationCacheTokensObserved';
...
export interface VerificationDeps {
    ...
    readonly now: () => Date;
    /**
     * The model table to price and judge residency against — defaults to the shipped registry.
     * See `GatedLlmDeps.registry` for why this seam exists (fixture-driven residency proof).
     */
    readonly registry?: Readonly<Record<string, ModelRegistryEntry>>;
    /**
     * How long a verification claim holds, in seconds (U7) — the handler's own timeout, for the reason the
     * parse leg's claim lease is (`parseLine.ts`): a duplicate delivered mid-attempt is refused, while a
     * genuine redelivery, which SQS only makes after the queue's longer visibility timeout, is admitted.
     */
    readonly claimLeaseSeconds: number;
    /**
     * Deliveries one line may CLAIM under one model before the gate gives up and lets it publish unverified.
     *
     * ⛔ NOT the queue's `maxReceiveCount`, which U8 raised to outlast a nine-hour nightly database stop. A
     * receive is spent whenever SQS hands the message over, including when this gate cannot reach the
     * database; an attempt is recorded only by a successful claim, which needs it. An outage therefore
     * spends receives and never a line's allowance.
     */
    readonly attemptAllowance: number;
}

/** Why the gate stopped trying (U7). The line then publishes unverified, as an unjudged line always has. */
export const VERIFICATION_ATTEMPTS_EXHAUSTED = 'attempt_allowance_exhausted';

/** A duplicate delivery answered from the stored verdict — the spend U7 stops paying twice. */
const VERIFICATION_ALREADY_VERIFIED_METRIC_NAME = 'VerificationAlreadyVerified';

/** A line the gate gave up on. Non-zero means lines are publishing unchecked. */
const VERIFICATION_ATTEMPTS_EXHAUSTED_METRIC_NAME = 'VerificationAttemptsExhausted';

/**
 * The verification key for a message — the content hash a verdict lands under.
 *
 * ⛔ EVERY INPUT COMES FROM THE MESSAGE, which is what makes U7 possible at all: the key was computed after
 * the model had answered, purely because that is where it was first needed, so a duplicate delivery paid
 * Bedrock to reach a judgement already stored under the very key it could have computed for free.
 *
 * @param message - The delivered line.
 * @returns The key.
 */
function keyFor(message: VerifyIngredientLineMessage): string {
...
async function settleQuietly(deps: VerificationDeps, plan: PricedReservation, actualMicros: number): Promise<void> {
    try {
        ...
        // ⛔ AN ISSUE TOO, not only the metric and the log (plan U22a step 3). The metric counts and the log
        // names the leg; neither carries the error's IDENTITY — the exception type, the constraint it
        // violated, the cause chain — which is the only thing an operator can act on. A counter cannot be
        // debugged, and ADR-0024's settle is NEVER retried, so the stranded reservation is permanent.
        captureHandled(error, { callSite: 'verification', period: plan.period });
    }
}
...
async function recordQuietly(deps: VerificationDeps, row: Parameters<VerdictStore['recordVerdict']>[0]): Promise<void> {
    try {
        ...
        // ⛔ AN ISSUE, because the swallow is safe and the CAUSE is not. "Absence of a verdict means
        // publish" makes losing this row safe for the request — it degrades to the pre-gate behaviour —
        // but it also means the gate silently stops gating, and nothing downstream can tell the difference
        // between a line nobody verified and a line whose verdict was lost. The error's identity is the
        // only evidence of which one is happening.
        captureHandled(error, { callSite: 'verification-verdict', band: row.band });
    }
}
...
export async function processVerification(deps: VerificationDeps, message: VerifyIngredientLineMessage): Promise<void> {
    ...
    const plan = planReservation({
        // Optional model table — production omits it and gets the shipped registry. See `GatedLlmDeps.registry`.
    ...
    // 3b. ⛔ CLAIM THE VERIFICATION, ahead of every reservation and every model call (U7). The key is
    //     computable from the message alone, so a duplicate delivery of a line already judged under this
    //     model costs one indexed read instead of a reservation and a Bedrock call. The claim also refuses a
    //     concurrent duplicate, which on a standard queue is an ordinary event rather than an exotic one.
    //
    //     ⚠️ AFTER the residency and pricing checks on purpose: those decide whether this model may be
    //     called AT ALL, and claiming first would write an attempt row for a delivery that was never going
    //     to spend — inflating a counter whose only job is to bound real attempts, and doing so on the two
    //     branches (unpriced, residency-refused) that are deterministic in the model rather than the line.
    const key = keyFor(message);
    const claim = await deps.store.claimVerification(key, plan.modelId, deps.claimLeaseSeconds);

    if (claim.kind === 'already-verified') {
        logger.info('verification already recorded for this key and model; nothing was spent', {
            foodId: message.foodId,
        });
        publish(deps, VERIFICATION_ALREADY_VERIFIED_METRIC_NAME, 1);

        return;
    }

    if (claim.kind === 'held') {
        logger.info('another delivery holds this verification claim; nothing was spent', {
            foodId: message.foodId,
        });
    ...
    if (claim.attempts > deps.attemptAllowance) {
        // ⛔ STOP, and say why — then let the line PUBLISH. An absent verdict has always meant publish, so
        // this changes nothing downstream except that an operator can now see the gate gave up rather than
        // inferring it from a line that quietly never got checked. Completing also keeps the message off the
        // DLQ, where its depth is the one signal ADR-0024 layer 4 alarms on.
        await deps.store.recordAttemptFailure(key, plan.modelId, VERIFICATION_ATTEMPTS_EXHAUSTED);
        logger.warn('verification exhausted its attempt allowance; the line publishes unverified', {
            foodId: message.foodId,
            attempts: claim.attempts,
        });
        publish(deps, VERIFICATION_ATTEMPTS_EXHAUSTED_METRIC_NAME, 1);
...
const rawHandler: SQSHandler = async (event) => {
    ...
    // Lazy for the reason given on the parse leg: an all-junk batch builds no clients.
    let deps: VerificationDeps | undefined;

    for (const record of event.Records) {
        // GR-018 §18-b, as on the parse leg: a shape rejection is recorded once and COMPLETED. Redelivering
        // it would spend 20 receives (this queue's `maxReceiveCount`) on a message that can never land, and
        // on this queue each delivery also passes through the ADR-0024 spend gate. A transient failure —
        // ceiling denial, Bedrock error, database — still throws.
        let message: VerifyIngredientLineMessage;
        ...
        try {
            ...
            recordInvalidPayload({ stage, queue: 'verification', messageId: record.messageId, error });
            continue;
        }

        deps ??= productionDeps(stage, region);
        await processVerification(deps, message);
    }
};
...
function productionDeps(stage: string, region: string): VerificationDeps {
    ...
    cachedDeps = {
        ...
        deployRegion: region,
        // From the STACK, which owns the function and the queue these describe (U7). No default here: a
        // fallback would let the gate run against a queue whose real numbers it had never been told.
        claimLeaseSeconds: requirePositiveEnv('VERIFICATION_CLAIM_LEASE_SECONDS'),
        attemptAllowance: requirePositiveEnv('VERIFICATION_ATTEMPT_ALLOWANCE'),
