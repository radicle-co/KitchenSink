...
const rawHandler: SQSHandler = async (event) => {
    ...
    for (const record of event.Records) {
        // GR-018 §18-b. `processRecord` parses the body itself, so the refusal is caught here rather than
        // around a separate parse call: an unparseable archive message is recorded once and completed, while
        // an S3 or database failure still throws and redelivers. The outbox row survives either way — it is
        // deleted only after S3 confirms — so completing a junk message loses no archive.
        try {
            ...
            recordInvalidPayload({ stage, queue: 'archive', messageId: record.messageId, error });
        }
    }
};
