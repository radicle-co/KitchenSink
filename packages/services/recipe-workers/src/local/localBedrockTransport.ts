 ...
 * local run — the loop would never reject, and a line the judges refuse would never be seen locally.
 * ⚠️ It no longer follows that `unparseable` is unreachable without it: `landingOf` was rekeyed on ZERO
 * FOODS (or the `name_is_source_line` placeholder) rather than on a `not_a_food` branch, which no longer
 * exists there — so a foodless answer lands `unparseable` whatever the validators said. What this fake
 * still governs is whether a REJECTION is ever exercised locally, which is the reason it must keep refusing
 * something. `namesNoFood` is the vocabulary
