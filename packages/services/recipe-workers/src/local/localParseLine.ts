...
import { realSleep } from '../parsing/parseLease.js';
 ...
 */
const LOCAL_INERT_CEILING_MICROS = 100_000_000;
...
export function createLocalParseLineDeps(config: LocalParseLineConfig, pool: Pool): ParseLineDeps {
    return {
        ...
        // The SAME sink the gated leg and the CRF port already use above — one EMF line to stdout. Locally
        // nothing extracts it into a metric, which is exactly the point: this module runs the SHIPPED code,
        // and a local no-op here would be the one call site where the detector does not exist.
        emit: emitMetric,
        parseModelId: PARSE_LEG_MODEL_ID,
        // The deployed stack's numbers (`RecipeWorkersStack`'s parse handler timeout and the parse queue's
        // `maxReceiveCount`). Stated rather than imported because this package's infra is a separate CDK
        // app with its own `rootDir`; `parseClaimBounds.test.ts` reads that stack and holds the two equal.
