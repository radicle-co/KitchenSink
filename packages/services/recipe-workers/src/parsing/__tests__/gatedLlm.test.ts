...
// ⛔ THE REAL SDK IS FAKED SO THE ISSUE PATH IS OBSERVABLE. `captureHandled` is inert until
// `initObservability` has run, so without this the assertion below would pass against a no-op — the exact
// shape of test theatre that let `captureHandled` ship with ZERO callers in the first place.
...
import { UNWARRANTED_MODEL_ID, UNWARRANTED_REACHED_REGIONS, UNWARRANTED_REGISTRY } from './unwarrantedModel.js';
import {
...
describe('⛔ the no-poisoning rule, pinned on the carrier a CRF reading would arrive through', () => {
    it('exposes no slot on the deps through which an engine reading could arrive', () => {
        ...
        // deploy region, a settings resolver, the spend ledger, the Bedrock adapter, a metric sink, a clock,
        // and the model rate table. None of them can hold a parse.
        //
        // ⚠️ `registry` was ADDED deliberately, and the guard failing first is the process working — a new
        // member is supposed to cost somebody an argument. Here is the argument: a rate table holds prices
        // and addresses, is consulted BEFORE any model call, and never sees a `ParsedLine`, so it cannot be
        // the channel ADR-0026 §1 is about. What it buys is the ability to prove the residency refusal
        // against a FIXTURE entry, so that proof does not depend on some real model staying unwarranted —
        // the day the last one is warranted, the coverage would otherwise disappear in silence. If a future
        // member cannot survive this paragraph, it does not belong here.
        const depsAreOnlyInfrastructure: Exact<
            keyof GatedLlmDeps,
            'stage' | 'deployRegion' | 'settings' | 'ledger' | 'bedrock' | 'emit' | 'now' | 'registry'
        > = true;
...
describe('gatedConverse — what the alarmed spend series carries', () => {
    it('⛔ publishes the PERIOD TOTAL on the gated path, the quantity ADR-0024 layer 4 alarms on', async () => {
        // ⛔ Two emitters, one metric, two different quantities. `SPEND_METRIC_NAME` is
        // `VerificationSpendMicros` in `Commise/RecipeVerification` — the exact series
        // `VerificationSpendAlarm` watches at `Stage` alone, `Maximum`, threshold HALF the monthly ceiling,
        // `treatMissingData: NOT_BREACHING`. `verifyLine.publishSpend` sends `reservation.reservedMicros`,
        // whose own type says "the period's total AFTER this charge — the value the EMF dollar metric
        // reports". This leg sent ONE CALL'S COST instead and threw the reservation away.
        //
        // The consequence is not noise, it is under-reporting: the parse leg's calls RAISE the shared period
        // counter and never publish the raised total, so a month whose spend is dominated by parsing shows
        // the alarm only whatever the verification leg last published — or, if that leg is idle, nothing at
        // all, which `NOT_BREACHING` reads as fine. The counter reports green precisely when it matters.
        const { deps, emit } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 });

        await createGatedFoodnessValidator(deps).judge('flour');

        const spend = emit.mock.calls
            .map((call) => call[0] as { name: string; value: number })
            .filter((metric) => metric.name === SPEND_METRIC_NAME);

        expect(spend).toHaveLength(1);
        // `reservedMicros: 100` from the ledger double — NOT the handful of micros this call itself cost.
        expect(spend[0]?.value).toBe(100);
    });
});
...
describe('gatedConverse — a residency-unapproved model', () => {
    it('refuses BEFORE reserving or calling anything', async () => {
        const { deps, reserve, converse } = makeDeps(
            { inputTokens: 10, outputTokens: 5, totalTokens: 15 },
            UNWARRANTED_REGISTRY,
        );

        await expect(
            createGatedMeasurementValidator(deps, UNWARRANTED_MODEL_ID).judge('2 cups flour', PARSE),
    ...
    it('carries the whole judgement on the error, so the handler needs no second registry read', async () => {
        const { deps } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 }, UNWARRANTED_REGISTRY);
        const caught = await createGatedLlmEngine(deps, UNWARRANTED_MODEL_ID)
            .parse(['2 cups flour'])
        ...
        expect(isResidencyRefusedError(caught) ? caught.refusal : undefined).toEqual({
            ...
            modelId: UNWARRANTED_MODEL_ID,
            deployRegion: 'us-east-1',
            reachedRegions: UNWARRANTED_REACHED_REGIONS,
        });
    });
    ...
    it('lets the refusal OUT of every port — none of them converts it into absence', async () => {
        ...
        const ports: readonly (() => Promise<unknown>)[] = [
            () => createGatedLlmEngine(deps, UNWARRANTED_MODEL_ID).parse(['2 cups flour']),
            () =>
                createGatedRetryPort(deps, UNWARRANTED_MODEL_ID).parse('2 cups flour', [
    ...
    it('EMITS the refusal, attributed to the leg that was silenced', async () => {
        const { deps, emit } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 }, UNWARRANTED_REGISTRY);

        await createGatedMeasurementValidator(deps, UNWARRANTED_MODEL_ID)
...
/**
 * A FAILED SETTLE ON THIS LEG MUST REACH THE ALARM.
 *
 * ⛔ THE DEFECT THIS CLOSES, and it was a false claim rather than an omission. `spendMetrics.ts` says of
 * `SETTLE_FAILURE_METRIC_NAME`: *"SHARED BY BOTH CONSUMERS … the alarm already deployed on this name
 * covers the parse leg from its first invocation"*, and speaks of *"the two emitters' log lines"*. There was
 * exactly ONE emitter — `verifyLine.ts` — so every settle failure on THIS leg was logged by `settleQuietly`
 * and reached no metric, no alarm, and nobody.
 *
 * ⚠️ Why that is not cosmetic: ADR-0024's settle is NEVER retried, so a failed settle leaves the worst-case
 * reservation standing against the ONE $100/month ceiling permanently. The counter drifts toward the cap
 * with real headroom silently consumed, and the operator's first signal would be a denial. The docstring
 * was actively harmful — it told the next reader the alarm was already covered.
 *
 * ⚠️ It publishes the SAME metric name rather than a second one, which is what that docstring argues for and
 * is why no `CallSite` dimension is attached: a worst-case charge stands against the shared pool, and the
 * operator's response does not depend on which leg took it. The log line still names the leg.
 */
describe('a settle failure on the parse leg reaches the shared alarm', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });

    it('⛔ emits the settle-failure metric when the ledger refuses the settle', async () => {
        const { deps, emit } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 });
        const settle = vi.fn().mockRejectedValue(new Error('check constraint violated'));

        await createGatedFoodnessValidator({ ...deps, ledger: { ...deps.ledger, settle } } as never).judge('flour');
    ...
    /**
     * ⛔ AN ISSUE, NOT ONLY A METRIC AND A LOG. The metric says a settle failed; the log names the leg.
     * Neither carries WHY — which driver error, which constraint, which cause in the chain — and that is
     * the only thing an operator can act on, because ADR-0024's settle is NEVER retried and the stranded
     * reservation stands against the one $100/month ceiling permanently. A counter cannot be debugged.
     */
    it('⛔ raises an ISSUE carrying the error itself, which the metric and the log line cannot', async () => {
        const { initObservability } = await import('../../common/observability.js');

        initObservability({ SENTRY_DSN: 'https://k@o1.ingest.sentry.io/1', STAGE: 'test' } as NodeJS.ProcessEnv);

        const { deps } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 });
        const failure = new Error('check constraint violated');
        const settle = vi.fn().mockRejectedValue(failure);
    ...
    it('stays silent on a settle that succeeds — the metric is a failure count, not a settle count', async () => {
        const { deps, emit } = makeDeps({ inputTokens: 10, outputTokens: 5, totalTokens: 15 });

        await createGatedFoodnessValidator(deps).judge('flour');

        expect(emit).not.toHaveBeenCalledWith(expect.objectContaining({ name: SETTLE_FAILURE_METRIC_NAME }));
    });
});
