...
import { captureHandled } from '../common/observability.js';
...
export interface GatedLlmDeps {
    ...
    readonly now: () => Date;
    /**
     * The model table to price and judge residency against. Defaults to the shipped
     * `BEDROCK_MODEL_REGISTRY`, which is what production wants and gets by omitting it.
     *
     * ⛔ INFRASTRUCTURE, not a channel for an engine reading — see the no-poisoning pin in this module's
     * suite, whose exact key set includes this member deliberately. A rate table cannot carry a parse: it
     * holds prices and addresses, is consulted BEFORE any call, and never sees a `ParsedLine`. What it makes
     * possible is proving the residency refusal against a FIXTURE entry, so that coverage stops depending on
     * some real model staying unwarranted — the day the last one is warranted, the proof would vanish.
     *
     * ⚠️ `bedrockInvokeStatements` already takes its registry as a parameter for the same reason, so this is
     * the established shape here rather than a test-only hatch.
     */
...
export async function gatedConverse(deps: GatedLlmDeps, call: GatedCall): Promise<GatedOutcome> {
    ...
    // The period's total after this leg's charge, on the gated path only — see the emit below.
    let periodMicros: number | undefined;
    ...
    if (gated) {
        ...
        periodMicros = reservation.reservedMicros;
    }
    ...
    // ⛔ THE PERIOD TOTAL WHEN GATED, not this call's cost. This is the series `VerificationSpendAlarm`
    // watches (`Commise/RecipeVerification` / `VerificationSpendMicros`, `Stage` alone, `Maximum`, threshold
    // at HALF the monthly ceiling, `treatMissingData: NOT_BREACHING`) and `verifyLine.publishSpend` already
    // sends `reservation.reservedMicros` into it — the value that type's own docstring calls "the period's
    // total AFTER this charge". Publishing a per-call figure here made one metric mean two things, and the
    // damage is UNDER-REPORTING rather than noise: this leg's calls raise the shared counter without ever
    // publishing the raised total, so a month whose spend is dominated by parsing shows the alarm only what
    // the verification leg last published — nothing at all when that leg is idle, which `NOT_BREACHING`
    // reads as healthy.
    //
    // ⚠️ Ungated stages (sandbox, every `pr-{N}`) take no reservation and have no period total to report, so
    // there the per-call cost remains the only honest value — and no ceiling is being tracked to mislead.
    deps.emit({
        namespace: SPEND_METRIC_NAMESPACE,
        name: SPEND_METRIC_NAME,
        unit: 'None',
        stage: deps.stage,
        value: periodMicros ?? (usage === undefined ? plan.worstMicros : actualCostMicros(plan.rate, usage)),
        dimensions: { CallSite: call.callSite },
    });
...
async function settleQuietly(
    ...
    try {
        ...
        // ⛔ QUIET TO THE CALLER, NOT TO THE OPERATOR. Swallowing the throw is deliberate — a bookkeeping
        // failure must not fail a line the model already answered — but ADR-0024's settle is NEVER retried,
        // so the worst-case reservation stands against the ONE $100/month ceiling permanently. A log line is
        // not enough: this emit is what lets `spendMetrics.ts`'s alarm cover the parse leg. Without it,
        // headroom drains silently and the operator's first signal is a DENIAL.
        //
        // ⚠️ The SAME metric name as the verification gate, with no `CallSite` dimension, exactly as that
        // docstring argues: the charge stands against the shared pool and the operator's response does not
        // depend on which leg took it. The log line below is what names the leg.
        deps.emit({
            namespace: SPEND_METRIC_NAMESPACE,
        ...
        captureHandled(error, { callSite: 'parse-leg', period: plan.period, stage: deps.stage });
    }
}
