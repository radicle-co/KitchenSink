...
async function settleQuietly(
    ...
    try {
        ...
        // so the worst-case reservation now stands against the ONE $100/month ceiling permanently. Logging
        // it was not enough: `spendMetrics.ts` claims this name's alarm "covers the parse leg from its first
        // invocation" and speaks of "the two emitters", while `verifyLine.ts` was the only one. Until this
        // emit existed, headroom drained silently and the operator's first signal was a DENIAL.
        //
