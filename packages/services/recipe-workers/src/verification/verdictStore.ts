...
/** What a claim of one verification found. */
export type VerificationClaim =
    /** A verdict for this key under this model already exists — nothing to reserve, nothing to call. */
    | { readonly kind: 'already-verified' }
    /** Another delivery of this line holds the claim — this one completes, having spent nothing. */
    | { readonly kind: 'held' }
    /** This delivery owns the attempt; `attempts` is the post-increment count the allowance is judged on. */
    | { readonly kind: 'claimed'; readonly attempts: number };

/** The gate's entire read and write surface. */
export interface VerdictStore {
    /**
     * Claim one verification ahead of any spend (U7): refuse when a verdict already exists for this key
     * under this model, refuse when another delivery holds the lease, otherwise record this attempt.
     *
     * ⛔ THE VERDICT READ IS KEYED ON THE MODEL, and that is the supersede rule one table over: a judgement
     * made by an older model must NOT suppress re-verification under the current one, which is precisely
     * what `recordVerdict`'s `DO UPDATE` exists to allow.
     *
     * @param key - The verification key.
     * @param modelId - The model this delivery would call.
     * @param leaseSeconds - How long a claim holds, before another delivery may take it.
     * @returns What this delivery found.
     * @sideEffect Upserts `recipe_verification_attempts`; reads `recipe_ingredient_verifications`.
     */
    claimVerification(key: string, modelId: string, leaseSeconds: number): Promise<VerificationClaim>;

    /**
     * Stop trying: record why, and leave the line to publish unverified.
     *
     * @param key - The verification key.
     * @param modelId - The model the attempts were made against.
     * @param failureCode - Why the gate gave up.
     * @sideEffect Updates `recipe_verification_attempts`.
     */
    recordAttemptFailure(key: string, modelId: string, failureCode: string): Promise<void>;

    /**
     * Record what the gate concluded about one line, and retire its attempt bookkeeping with it.
     *
     * @param row - The verdict.
     * @sideEffect Upserts `recipe_ingredient_verifications`; deletes from `recipe_verification_attempts`.
     */
...
export function createVerdictStore(db: NodePgDatabase<Record<string, never>>): VerdictStore {
    return {
        async claimVerification(key: string, modelId: string, leaseSeconds: number): Promise<VerificationClaim> {
            // ⛔ THE VERDICT READ COMES FIRST AND IS FREE. Every input to `key` comes from the MESSAGE, so it
            // has always been computable before the reservation — it simply was not computed until after the
            // model had answered, which is why a duplicate delivery paid Bedrock to reach a judgement already
            // stored under the same key.
            const verdict = await db.execute(sql`
                SELECT 1 FROM recipe_ingredient_verifications
                 WHERE verification_key = ${key} AND model_id = ${modelId}
                 LIMIT 1
            `);

            if ((verdict.rows.length as number) > 0) {
                return { kind: 'already-verified' };
            }

            // The claim. `ON CONFLICT … WHERE` makes the lease part of the WRITE rather than a check before
            // it: two concurrent duplicates serialize on the row and exactly one comes back with a row.
            const claimed = await db.execute<{ attempts: number }>(sql`
                INSERT INTO recipe_verification_attempts (verification_key, model_id, attempts, last_received_at)
                VALUES (${key}, ${modelId}, 1, now())
                ON CONFLICT (verification_key, model_id) DO UPDATE
                   SET attempts = recipe_verification_attempts.attempts + 1,
                       last_received_at = now()
                 WHERE recipe_verification_attempts.last_received_at
            ...
            return attempts === undefined ? { kind: 'held' } : { kind: 'claimed', attempts };
        },

        async recordAttemptFailure(key: string, modelId: string, failureCode: string): Promise<void> {
            await db.execute(sql`
                UPDATE recipe_verification_attempts SET failure_code = ${failureCode}
                 WHERE verification_key = ${key} AND model_id = ${modelId}
            `);
        },
        ...
        async recordVerdict(row: VerdictRow): Promise<void> {
            ...
            // ⛔ The bookkeeping retires WITH the judgement. A judged line carries no attempt row, so the
            // counter can never outlive what it was counting — and the next delivery of this line reads the
            // verdict and stops, which is the whole point of the claim.
            //
            // ⚠️ Separate statement, same call, deliberately NOT a transaction: `recordVerdict` is invoked
            // through `recordQuietly`, which swallows failures because the model call is already billed and a
            // throw would redeliver a paid message. A leftover attempt row is harmless (the next delivery
            // reads the verdict first); a verdict rolled back because its cleanup failed would not be.
            await db.execute(sql`
                DELETE FROM recipe_verification_attempts
                 WHERE verification_key = ${row.verificationKey} AND model_id = ${row.modelId}
            `);
        },
