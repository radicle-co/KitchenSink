 ...
 * `deployGate.sh` already uses. It answers ONE question — "for this line, which aspects must be checked?" —
 ...
 *
 * Stated as a union rather than derived from a `const` array: nothing iterates the aspects at runtime, so
 * the array was a second representation of these two words and no more.
 */
export type VerificationAspect = 'identity' | 'quantity';
...
export type IdentityEvidence =
    ...
    | {
           ...
           *
           * The producer (`lexicalTier.ts`, with `RANKER_VERSION`/`rankingTiers.ts` beside it) sends real
           * candidates. An empty shortlist means the tier ran and offered nothing.
           */
    ...
    | {
          /**
           * ⛔ NO PROVENANCE WAS RECORDED FOR THIS LINE — not a placeholder, and not the ordinary case.
           *
           * ⚠️ Provenance IS recorded: `0035_ingredient_resolutions.sql` is the event store,
           * `resolveThroughCascade` writes to it through `IngredientResolutionsDal.record`, and the producer's
           * `evidenceFor` (`recipes/domain/verificationRequests.ts`) reads it back as `curated-exact`,
           * `remembered` or `ranked`. What lands here is a line with NO usable event: the write is deliberately
