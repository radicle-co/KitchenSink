...
describe('the model registry — invocation id and reach', () => {
    ...
    it('addresses a profile-only model through its inference profile, not its bare id', () => {
        ...
        // ships on a profile — the same property previously asserted on a roster model we do not call.
...
describe('residencyClearance', () => {
     ...
     *
     * ⚠️ REWRITTEN from "ships no residency-approved entry", which was true until the owner warranted Nova 2
     * Lite for the parse leg (2026-09-12, ADR-0024 §4b). The guard did its job — the approval could not land
     * quietly — so what changes is the SHAPE, not the strictness: an exact register of which entries are
     * approved, each with its date and reference, rather than a blanket "none". A second approval still
     * fails here until it is added deliberately, and a warrant REMOVED from an entry fails here too, which
     * "none" could never catch.
     */
...
describe('Nova 2 Lite — the shipped parse model', () => {
    ...
    it('is CALLABLE only because a warrant is recorded — the profile still leaves the deploy region', () => {
        // ⚠️ The reach is UNCHANGED and still cross-region: nothing about the model moved, and the warrant is
        // not a claim that it did. What changed is that the owner accepted the consequence (2026-09-12,
        // ADR-0024 §4b), so `residencyClearance` answers `approved` and the reservation prices it. Asserting
    ...
    it('costs more in every token class than the Nova Lite v1 it replaced — the warrant is not free', () => {
        // ⚠️ REFRAMED, not weakened. This asserted why Lite v1 was SHIPPED; the same inequality now records
        // what returning to Nova 2 Lite COSTS, so the ADR-0024 pool stretches less far than it did. On the
        // same external gold set Lite v1 scores 73/41 static against Nova 2 Lite's 84/53 — the accuracy the
        // residency refusal used to cost, now bought back at these rates.
