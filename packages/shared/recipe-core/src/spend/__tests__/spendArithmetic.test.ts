...
/**
 * A cross-region entry that exists ONLY here, so the residency branches can be driven without depending
 * on which real model happens to be unwarranted this week.
 *
 * ⛔ The two differ in exactly ONE field — the warrant — which is what makes the pair a mutation guard
 * rather than two unrelated assertions. The region list is deliberately NOT a real profile's: nothing
 * about these tests should read as a claim about where a shipped model routes.
 */
const FIXTURE_MODEL_ID = 'vendor.fixture-model-v1:0';
const FIXTURE_RATE = {
...
const fixtureWith = (residencyApproval?: ResidencyApproval) => ({
    [FIXTURE_MODEL_ID]: {
        rate: FIXTURE_RATE,
        invocation: {
            invocationId: `us.${FIXTURE_MODEL_ID}`,
            reach: {
...
const UNWARRANTED_FIXTURE = fixtureWith();
const WARRANTED_FIXTURE = fixtureWith({ approvedOn: '2026-09-12', reference: 'ADR-0024 §4b (fixture)' });
...
describe('planReservation', () => {
    ...
    it('refuses an id that names an Object.prototype member, which a bare index would resolve', () => {
        // `table['constructor']` is `Object`, not `undefined`: without an own-property check an id read from SSM
        // would reach a "priced" plan with no rate behind it, on either the shipped or an injected registry.
        for (const modelId of ['constructor', '__proto__', 'toString']) {
            expect(plan(modelId)).toEqual({ kind: 'unpriced', modelId });
            expect(rateFor(modelId)).toBeUndefined();
            expect(registryEntryFor(modelId)).toBeUndefined();
        }
    });
    ...
    it('refuses a profile-addressed model that reaches beyond the deploy region without a warrant', () => {
        // Driven from a FIXTURE rather than whichever shipped model is unwarranted today — see
        // `UNWARRANTED_FIXTURE`. The property is about the SHAPE of an entry, not about any real model.
        const entry = UNWARRANTED_FIXTURE[FIXTURE_MODEL_ID]!;

        expect(residencyClearance(entry, 'us-east-1')).toBe('unapproved');
        expect(
            planReservation({
                registry: UNWARRANTED_FIXTURE,
            ...
            modelId: FIXTURE_MODEL_ID,
            deployRegion: 'us-east-1',
            reachedRegions: ['us-east-1', 'eu-west-1'],
        });
    });
    ...
    it('names the residency refusal apart from the unpriced one, and says where the call would have gone', () => {
        // Driven from `UNWARRANTED_FIXTURE`: the discriminant's meaning is a property of the refusal, not of
        // any particular model, and tying it to a shipped entry is what made this coverage vanish the moment
        // one was warranted.
        const refusal = planReservation({
            registry: UNWARRANTED_FIXTURE,
            modelId: FIXTURE_MODEL_ID,
            ceilingMicros: DEFAULT_MONTHLY_CEILING_MICROS,
            maxInputTokens: 1_000,
        ...
        expect(refusal.kind === 'residency-unapproved' && refusal.reachedRegions).toEqual(['us-east-1', 'eu-west-1']);
    });
...
describe('the model registry — invocation id and reach', () => {
    ...
    it('addresses a profile-only model through its inference profile, not its bare id', () => {
        // ⛔ The bare id is REFUSED by Bedrock for such a model: "Invocation of model ID … with on-demand
        // throughput isn't supported". Asserted on **Nova 2 Lite**, which is the model this repo actually
        // ships on a profile, not on a roster model we do not call.
        const entry = registryEntryFor(NOVA_2_LITE_MODEL_ID);

        expect(entry?.invocation.invocationId).toBe(`us.${NOVA_2_LITE_MODEL_ID}`);
...
describe('residencyClearance', () => {
    ...
    if (novaEntry === undefined) {
        throw new Error('the registry must carry the on-demand model this arm is about');
    }

    // ⛔ The cross-region arm is a FIXTURE, not a shipped model. Tying it to whichever real entry lacked a
    // warrant made the coverage hostage to a product decision — warranting that model deleted the proof that
    // the predicate refuses anything. A fixture cannot be warranted out from under the test.
    const crossRegionEntry = UNWARRANTED_FIXTURE[FIXTURE_MODEL_ID]!;
    ...
    it('refuses a profile that reaches beyond the deploy region without a warrant', () => {
        expect(residencyClearance(crossRegionEntry, 'us-east-1')).toBe('unapproved');
    });
    ...
    /**
     * ⛔ R9 — APPROVING AN ENTRY IS A DELIBERATE ACT, and this is the assertion that forces it to look like
     * one. Flipping a marker fails this test until someone edits it in the same commit, so the diff carries
     * both the approval and its warrant together.
     *
     * ⚠️ REWRITTEN for the Nova 2 Lite warrant (2026-09-12, ADR-0024 §4b): an exact register of which entries
     * are approved, each with its date and reference, rather than a blanket "none". A second approval fails
     * here until it is added deliberately, and a warrant REMOVED from an entry fails here too, which "none"
     * could never catch.
     */
    it('approves exactly the entries the owner has warranted, each carrying its date and reference', () => {
        const approved = Object.entries(BEDROCK_MODEL_REGISTRY).flatMap(([modelId, entry]) => {
            const { reach } = entry.invocation;
            const approval = reach.kind === 'regions' ? reach.residencyApproval : undefined;

            return approval === undefined ? [] : [[modelId, approval] as const];
        });

        expect(approved.map(([modelId]) => modelId)).toEqual([NOVA_2_LITE_MODEL_ID]);
        expect(approved[0]?.[1]).toEqual({ approvedOn: '2026-09-12', reference: 'ADR-0024 §4b' });
    });

    /**
     * ⛔ THE REFUSAL BRANCH IS PROVEN AGAINST A FIXTURE, NOT A SHIPPED MODEL — and that is the point.
     *
     * Tying this to whichever real entry happened to be unwarranted made the coverage hostage to a product
     * decision: warranting that model silently deleted the only proof that `planReservation` refuses
     * anything, which is exactly the control a warrant most needs. A fixture cannot be warranted out from
     * under the test, so the branch stays exercised no matter what the shipped table decides.
     */
    it('refuses an unwarranted cross-region entry — proven against an injected fixture', () => {
        const refusal = planReservation({
            registry: UNWARRANTED_FIXTURE,
            modelId: FIXTURE_MODEL_ID,
            ceilingMicros: DEFAULT_MONTHLY_CEILING_MICROS,
            maxInputTokens: 1_000,
            maxOutputTokens: 200,
            nowUtc: new Date('2026-09-12T00:00:00.000Z'),
            deployRegion: 'us-east-1',
        });
    ...
    it('prices that same fixture once a warrant is recorded on it — the warrant is the only difference', () => {
        // The mutation guard for the pair: a refusal that ignored the warrant, or a price that ignored the
        // reach, would pass one of these two and fail the other.
        const priced = planReservation({
            registry: WARRANTED_FIXTURE,
            modelId: FIXTURE_MODEL_ID,
            ceilingMicros: DEFAULT_MONTHLY_CEILING_MICROS,
            maxInputTokens: 1_000,
            maxOutputTokens: 200,
...
describe('Nova 2 Lite — the shipped parse model', () => {
     ...
     */
    it('is CALLABLE only because a warrant is recorded — the profile still leaves the deploy region', () => {
        // ⚠️ The reach is cross-region: the warrant is not a claim that the model is anything else. The owner
        // accepted the consequence (2026-09-12, ADR-0024 §4b), so `residencyClearance` answers `approved` and
        // the reservation prices it. Asserting
        // the reach alongside the clearance is what keeps those two facts from being confused — a future
        // reader must not conclude the profile became single-region.
        const entry = BEDROCK_MODEL_REGISTRY[NOVA_2_LITE_MODEL_ID]!;

        expect(entry.invocation.reach.kind).toBe('regions');
        expect(residencyClearance(entry, 'us-east-1')).toBe('approved');
        expect(
            planReservation({
    ...
    it('costs more in every token class than the Nova Lite v1 it replaced — the warrant is not free', () => {
        // ⚠️ REFRAMED, not weakened. The inequality records what Nova 2 Lite COSTS over the Lite v1 fallback,
        // so the ADR-0024 pool stretches less far. On the same external gold set Lite v1 scores 73/41 static
        // against Nova 2 Lite's 84/53 — the accuracy a residency refusal costs, bought at these rates.
