 ...
 *
 * ⛔ IT WAS THE SHIPPED PARSE MODEL between 2026-09-04 and 2026-09-12, which is not what this entry was added
 * for: it was priced so a family sweep could be costed without inventing a rate. It ran the parse leg while
 * {@link NOVA_2_LITE_MODEL_ID} — the accuracy winner at 84%/53% against this model's 73/41 static — was
 * residency-refused, and the leg returned to Nova 2 Lite once the owner warranted it (ADR-0024 §4b). This
 * entry stays priced because it is the option the leg falls back TO if that warrant is ever revoked.
 *
 ...
 * twenty points where eight rounds of prompt revision moved three. ⚠️ That measurement never established
 * that the model could be REACHED, which is a separate question and was the binding one for a week.
 *
...
export const BEDROCK_MODEL_REGISTRY: Readonly<Record<string, ModelRegistryEntry>> = Object.freeze({
    ...
    [NOVA_2_LITE_MODEL_ID]: Object.freeze({
        ...
        // consequence the refusal existed to prevent, and the ruling accepts it rather than reinterpreting
...
export interface ReservationRequest {
     ...
     * model is refused" used to require some real entry to be unwarranted, which made that coverage hostage
     * to a product decision: warranting the last one silently deleted the only proof the refusal works —
     * precisely the control a warrant most needs. `bedrockInvokeStatements` already takes its registry as a
     * parameter for the same reason, and this closes the asymmetry between the IAM half and the runtime half.
     *
...
export function planReservation(request: ReservationRequest): ReservationPlan {
    ...
    const table = request.registry ?? BEDROCK_MODEL_REGISTRY;
    const entry = Object.hasOwn(table, request.modelId) ? table[request.modelId] : undefined;
