 ...
 * the same split `deployGate.sh` uses — a pure `decide` (here: the whole rate table, the worst case, the
...
/**
 * Amazon Nova Lite — the next rung of the family for the bake-off's purposes, and the parse leg's
 * residency-clear FALLBACK.
 *
 * ⛔ PRICED so a family sweep can be costed without inventing a rate, and because it is the option the parse
 * leg falls back TO if the {@link NOVA_2_LITE_MODEL_ID} warrant (ADR-0024 §4b) is ever revoked — Nova 2 Lite
 * is the accuracy winner at 84%/53% against this model's 73/41 static.
 *
...
/**
 * Amazon Nova 2 Lite — the ACCURACY winner, and **the model the SHIPPED parse leg calls**
 * (`PARSE_LEG_MODEL_ID`).
 *
 * ⛔ SELECTED on an external held-out gold set (144 ingredient + 210 instruction lines, human-adjudicated):
 * 84% / 53% exact against Nova Micro's 64% / 30% on the same prompt. Model choice moved accuracy roughly
 * twenty points where eight rounds of prompt revision moved three. ⚠️ That measurement does not establish
 * that the model can be REACHED, which is a separate question.
 *
 * ⛔ IT IS CALLABLE ONLY BECAUSE RESIDENCY IS WARRANTED (ADR-0024 §4b, owner ruling 2026-09-12). Every
 * inference profile that exists for it leaves us-east-1 and AWS stores prompts and outputs in destination
 * Regions for abuse detection, so calling it means **user recipe text rests in us-east-2 and us-west-2**. The
 * owner accepted that; `residencyClearance` answers `approved` only because a `residencyApproval` sits on
 * this entry, and `planReservation` plus the IAM policy both read that one field. Revoke it and the leg falls
 * back to {@link NOVA_LITE_MODEL_ID}. See this entry in {@link BEDROCK_MODEL_REGISTRY} for the whole record.
 */
...
export const BEDROCK_MODEL_REGISTRY: Readonly<Record<string, ModelRegistryEntry>> = Object.freeze({
    ...
    [NOVA_2_LITE_MODEL_ID]: Object.freeze({
        ...
        //
        // ⛔ RESIDENCY IS WARRANTED — owner ruling, 2026-09-12, recorded in ADR-0024 §4b. What the warrant
        // ADMITS, stated plainly because a marker is easy to read past: user recipe text is processed in and
        // stored at rest by AWS in **us-east-2 and us-west-2** for abuse detection. That is the exact
        // consequence a residency refusal prevents, and the ruling accepts it rather than reinterpreting
        // it. `residencyClearance` therefore answers `approved`, `planReservation` prices this entry, and
        // `bedrockInvokePolicy.ts` emits its statement — the three move together by construction, because
        // all of them ask this one field.
        //
        // ⚠️ Claude Haiku 4.5 is deliberately NOT warranted by the same ruling. It reaches the same three
        // regions, so the data-protection argument would carry — but it is not a model we ship, the ruling
        // was about the parse leg, and an approval nobody needs is an approval nobody reviewed. It also
        // keeps `spendArithmetic.test.ts`'s non-vacuity floor honest ("the registry refuses nothing — the
        // residency branch is unexercised"), which REQUIRES an unapproved member to exist or the refusal
        // branch goes untested. Approving the last unapproved entry would silently delete that coverage.
        //
        // ⚠️ The warrant is NOT a statement about 016. When the legal-compliance framework is built it owns
        // this determination and may narrow or revoke it; this records that the owner made the call ahead of
        // it, knowingly, so the review has something explicit to find rather than a silent default.
        invocation: Object.freeze({
            ...
            reach: Object.freeze({
                ...
                regions: Object.freeze(['us-east-1', 'us-east-2', 'us-west-2']),
                readOn: '2026-08-27',
                residencyApproval: Object.freeze({
                    approvedOn: '2026-09-12',
                    reference: 'ADR-0024 §4b',
                } as const),
            } as const),
        }),
    }),
...
export interface TokenUsage {
    ...
    /**
     * Optional because Bedrock declares it `Required: No` — NOT because it is absent here. The shipped
     * verification prompt is ~5,025 tokens, past Haiku 4.5's 4,096-token caching minimum, so a warm call
     * reports a non-zero value as the NORMAL case (ADR-0024 §5a). A sustained zero is the anomaly.
     * See {@link ModelRate.cacheReadMicrosPerMillionTokens}.
     */
    readonly cacheReadInputTokens?: number | undefined;
    /** Same: optional by the API's contract, and non-zero on a warm call. See {@link ModelRate.cacheWriteMicrosPerMillionTokens}. */
...
export interface ReservationRequest {
    ...
    /**
     * The model table to price against. Defaults to {@link BEDROCK_MODEL_REGISTRY}, which is what every
     * production caller wants and what all three of them get by omitting it.
     *
     * ⛔ IT EXISTS SO THE RESIDENCY BRANCHES CAN BE DRIVEN WITHOUT A SHIPPED MODEL. Proving "an unwarranted
     * model is refused" against a real entry makes that coverage hostage to a product decision: warranting
     * the last one would silently delete the only proof the refusal works — precisely the control a warrant
     * most needs. `bedrockInvokeStatements` takes its registry as a parameter for the same reason, so the
     * IAM half and the runtime half are symmetric.
     *
     * ⚠️ OPTIONAL, unlike `deployRegion` directly above, and the difference is real rather than lazy. A
     * default region would silently assert a POSITION no caller had thought about; the default here is the
     * one table production could possibly mean, so omitting it cannot express a wrong intention — only the
     * right one, more briefly.
     */
...
export function rateFor(modelId: string): ModelRate | undefined {
    return registryEntryFor(modelId)?.rate;
}
 ...
 *
 * `Object.hasOwn`, not a bare index: an id such as `constructor` or `__proto__` must not resolve through the
 * prototype chain.
 *
 * @param modelId - A Bedrock model id, as read from SSM.
 * @param registry - The registry to read; the shipped one unless a caller injects a table.
 * @returns The registry entry, or `undefined`.
 */
export function registryEntryFor(
    modelId: string,
    registry: Readonly<Record<string, ModelRegistryEntry>> = BEDROCK_MODEL_REGISTRY,
): ModelRegistryEntry | undefined {
    return Object.hasOwn(registry, modelId) ? registry[modelId] : undefined;
}
...
export function planReservation(request: ReservationRequest): ReservationPlan {
    ...
    const entry = registryEntryFor(request.modelId, request.registry);
