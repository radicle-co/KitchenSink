...
const CORRUPTS_STATED_VALUE = {
    ...
    // U7: the validator's terminal not-a-food records a DISPUTED NAME — no stated number was altered.
    not_a_food: false,
    // ⛔ 2026-09-19, the token split. This one is SILENCE — the parse returned no name at all — and the
    // values kept are exactly what that same reader stated, with no validator questioning them. This set's
    // rule is "a stated number would be wrong", not "something is missing", so it is `not_a_food`'s answer
    // for `not_a_food`'s reason and the split changes nothing here. ⚠️ The consequence decides it a SIXTH
    // time: membership would make `cookbook-import` discard the whole clause rather than surface it.
    no_food_returned: false,
    ...
    // ⛔ R5 (2026-09-19). The quantity and unit are exactly what the readers read, and a placeholder NAME
    // states no number at all — this set's membership rule is "a stated number would be wrong", not
    // "something is missing". The consequence decides it a FIFTH time: `cookbook-import` drops a clause
    // whose reading corrupts a value, so membership would discard the very lines the placeholder exists to
    // preserve. ⚠️ The line is still gated — `landingOf` reads this reason as `unparseable`, so nothing is
    // bound; that is a different mechanism from this set, and the two must not be confused.
    name_is_source_line: false,
