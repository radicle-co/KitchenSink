...
/**
 * ⛔ R1 — AN EMPTY FOOD LIST IS ABSENCE, NOT A READING (owner ruling 2026-09-19).
 *
 * `DEFAULT_WINNERS.foods = 'llm'` took the LLM's list unconditionally, so an LLM that named NO food beat a
 * CRF that named one and the merged line carried `foods: []` — a correct result discarded. That is ADR-0026
 * §3's "absence is silence" applied one field over, exactly as U36 applied it to the unit and U38 to the
 * amount, and it is scoped to the EMPTY case alone: where both engines named a food the LLM still wins, on
 * the measured strength KTD-11 records.
 *
 * | mutation | tests it fails |
 * | -------- | -------------- |
 * | revert the rescue (`foods: 'llm'` unconditionally) | "⛔ keeps the food the CRF found where the LLM returned none", "⛔ the rescue is what stops the raw-line placeholder…" |
 * | widen it to "the LLM's list is SHORTER" | "⛔ and NOTHING more — the LLM still wins when BOTH engines named a food", "⛔ does not fire when the LLM named FEWER foods…" |
 * | fire it in the mirror direction (an empty CRF list takes the LLM's) | "⛔ does not fire in the mirror direction — an empty CRF list is silence too" |
 * | ALSO silence `foods` in the report | "⚠️ only what is STORED moves — the disagreement is still reported" |
 * | keep the LLM's stale `no_food_returned` | "⛔ drops the `no_food_returned` the merge has just answered" |
 * | ALSO drop a `not_a_food` a validator earned | "⛔ KEEPS a `not_a_food` a validator earned — a rescue answers silence, never a verdict" |
 * | drop the loser's OTHER reasons with it | "keeps every other reason the silent engine raised" |
 */
describe('compareParses — R1, an empty LLM food list never beats a CRF food', () => {
    it('⛔ keeps the food the CRF found where the LLM returned none', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [] }),
        });

        expect(result.merged?.foods).toEqual([{ name: 'flour', prep: null }]);
        expect(result.merged?.provenance.foods).toBe('crf');
    });

    it('⛔ and NOTHING more — the LLM still wins when BOTH engines named a food', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [{ name: 'white flour', prep: null }] }),
        });

        expect(result.merged?.foods).toEqual([{ name: 'white flour', prep: null }]);
        expect(result.merged?.provenance.foods).toBe('llm');
    });

    it('⛔ does not fire when the LLM named FEWER foods — only an EMPTY list is silence', () => {
        const result = compareParses({
            crf: crfParse({
                foods: [
                    { name: 'salt', prep: null },
                    { name: 'pepper', prep: null },
                ],
            }),
            llm: llmParse({ foods: [{ name: 'salt and pepper', prep: null }] }),
        });

        expect(result.merged?.foods).toEqual([{ name: 'salt and pepper', prep: null }]);
        expect(result.merged?.provenance.foods).toBe('llm');
    });

    it('⛔ does not fire in the mirror direction — an empty CRF list is silence too', () => {
        const result = compareParses({
            crf: crfParse({ foods: [] }),
            llm: llmParse({ foods: [{ name: 'flour', prep: null }] }),
        });
    ...
    it('⚠️ only what is STORED moves — the disagreement is still reported', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [] }),
        });
    ...
    /**
     * ⛔ R1 CREATES THIS, so it is decided here rather than left to a reader. The validator loop records
     * `no_food_returned` on an LLM answer that bound nothing; the merge then binds the CRF's food, and the
     * chip a cook reads would say "we found nothing on this line" over a line showing `flour`.
     *
     * `unionReasons`' rule — never drop the loser's signal — is overridden ONLY here and only for this one
     * reason, because it is not a signal about the line: it is a terminal-state record of one engine's
     * SILENCE, and the merge has just filled that silence in.
     *
     * ⚠️ REWRITTEN 2026-09-19, and the rewrite is the whole point rather than a rename. This test was
     * written against `not_a_food`, which at the time carried BOTH terminal facts — silence and a
     * validator's verdict — so the filter it certified discarded a verdict as well: a CRF naming
     * `frying-pan` against an LLM whose every name a validator disputed merged to `foods:
     * [{name:'frying-pan'}]`, `reviewReasons: []`, and landed `parsed` with `needsReview: false`. The
     * behaviour under test is unchanged (silence the merge has answered is dropped); what changed is that
     * silence now has a token of its own, and the sibling test below pins the verdict SURVIVING.
     */
    it('⛔ drops the `no_food_returned` the merge has just answered', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [], reviewReasons: ['no_food_returned'] }),
        });

        expect(result.merged?.reviewReasons).not.toContain('no_food_returned');
    });

    /**
     * ⛔ THE OTHER ROUTE TO AN EMPTY FOOD LIST, and it must SURVIVE the rescue.
     *
     * `validatedEngine`'s `unParseable` is reached when a validator disputed EVERY name the model offered.
     * That is evidence about the line, not silence about it — the shape is the literal terminal record:
     * every stated value blanked, `foods: []`, `not_a_food`. The CRF naming a vessel (`a large frying-pan`
     * → `1 large :: frying-pan`, the reading CLAUDE.md records it produces) then rescues the food list, and
     * dropping the verdict with it would publish a name a judge rejected under `status: 'parsed'`,
     * `needsReview: false` — the hole R1 exists to close, one door over.
     */
    it('⛔ KEEPS a `not_a_food` a validator earned — a rescue answers silence, never a verdict', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'a large frying-pan', foods: [{ name: 'frying-pan', prep: null }] }),
            llm: llmParse({
        ...
        expect(result.merged?.foods).toEqual([{ name: 'frying-pan', prep: null }]);
        expect(result.merged?.reviewReasons).toContain('not_a_food');
    });

    it('keeps every other reason the silent engine raised', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [], reviewReasons: ['no_food_returned', 'measurement_unverified'] }),
        });

        expect(result.merged?.reviewReasons).toEqual(['measurement_unverified']);
    });

    it('⛔ keeps a `not_a_food` raised beside foods that SURVIVED — mixed exhaustion is untouched', () => {
        const result = compareParses({
            crf: crfParse({ foods: [{ name: 'flour', prep: null }] }),
            llm: llmParse({ foods: [{ name: 'eggs', prep: null }], reviewReasons: ['not_a_food'] }),
        });

        expect(result.merged?.foods).toEqual([{ name: 'eggs', prep: null }]);
        expect(result.merged?.reviewReasons).toContain('not_a_food');
    });
});

/**
 * ⛔ R5 — THE SOURCE LINE IS THE PLACEHOLDER OF LAST RESORT (owner ruling 2026-09-19).
 *
 * When no reader found a food, the line's own text becomes the food name so the words are never lost. It is
 * a PLACEHOLDER, not a parse: `name_is_source_line` says so, and `landingOf` reads that reason as
 * `unparseable`, so nothing is ever bound to it.
 *
 * ⛔ THE ORDER IS THE RULE: the LLM's food, else the CRF's (R1), else the raw line. The placeholder is
 * reached only when the merged line has no food from either engine.
 *
 * | mutation | tests it fails |
 * | -------- | -------------- |
 * | apply the placeholder before R1 | "⛔ the rescue is what stops the raw-line placeholder — R1 outranks R5" |
 * | apply it to a line that HAS a food | "⛔ never replaces a food an engine did find" |
 * | omit the review reason | "⛔ flags the placeholder — an unread line must never land clean" |
 * | skip it on a single-engine merge | "⛔ places the line on a SINGLE-engine merge too" |
 * | fabricate a name from a blank line | "does not manufacture a name out of nothing" |
 * | store the untrimmed `raw` as the name | "⚠️ trims the placeholder — the padding is not part of what the cook wrote" |
 * | trim `raw` itself on the way through | "⚠️ trims the placeholder…" (its second assertion) |
 */
describe('compareParses — R5, the raw line is the placeholder of last resort', () => {
    it('⛔ names the food after the source line when NEITHER engine found one', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'spoonfuls', foods: [] }),
            llm: llmParse({ raw: 'spoonfuls', foods: [] }),
        });

        expect(result.merged?.foods).toEqual([{ name: 'spoonfuls', prep: null }]);
    });

    it('⛔ flags the placeholder — an unread line must never land clean', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'spoonfuls', foods: [] }),
            llm: llmParse({ raw: 'spoonfuls', foods: [] }),
        });

        expect(result.merged?.reviewReasons).toContain('name_is_source_line');
    });

    it('⛔ the rescue is what stops the raw-line placeholder — R1 outranks R5', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'three quarts of cold water', foods: [{ name: 'cold water', prep: null }] }),
            llm: llmParse({ raw: 'three quarts of cold water', foods: [] }),
        });

        // ⚠️ `cold` moves to `prep` on the way through: a rescued food is canonicalised like any other
        // winner (KTD-11b, temperature is preparation), which is the property that keeps R1 from becoming a
        // second, unplaced path into the merged line.
        expect(result.merged?.foods).toEqual([{ name: 'water', prep: 'cold' }]);
        expect(result.merged?.reviewReasons).not.toContain('name_is_source_line');
    });

    it('⛔ never replaces a food an engine did find', () => {
        const result = compareParses({ crf: crfParse(), llm: llmParse() });
    ...
    it('⛔ places the line on a SINGLE-engine merge too', () => {
        const result = compareParses({
            crf: { unavailable: true },
            llm: llmParse({ raw: 'a baking-board', foods: [] }),
        });

        expect(result.agreement).toEqual({ kind: 'single-engine', engine: 'llm' });
        expect(result.merged?.foods).toEqual([{ name: 'a baking-board', prep: null }]);
        expect(result.merged?.reviewReasons).toContain('name_is_source_line');
    });

    /**
     * ⚠️ THE SHAPE A REAL CEILING DENIAL PRODUCES, and it never passes through the R1 filter at all —
     * `fromOneEngine` copies the answering engine's reasons verbatim and `withSourceLineFallback`
     * appends to them, so `mergedReasons` is not on this path. That is the correct outcome and the reason
     * it is asserted: nothing answered the silence here, so the record is still true of the line, and a
     * filter written one branch too wide would strip it with nothing to show a reader that it had.
     */
    it('⚠️ a SINGLE-engine placeholder keeps the silent leg’s own terminal record beside it', () => {
        const result = compareParses({
            crf: { unavailable: true },
            llm: llmParse({ raw: 'a baking-board', foods: [], reviewReasons: ['no_food_returned'] }),
        });

        expect(result.merged?.reviewReasons).toEqual(['no_food_returned', 'name_is_source_line']);
    });

    /**
     * ⚠️ Every other food in a merge goes through `canonicaliseFood`, which trims; the placeholder is the
     * one food that does not, so it was the one food that could carry padding into a stored proposal and
     * into the catalog lookup a reviewer's edit feeds. The SAME string that decides "is there any text
     * here" must be the string that is stored.
     */
    it('⚠️ trims the placeholder — the padding is not part of what the cook wrote', () => {
        const result = compareParses({
            crf: crfParse({ raw: '  spoonfuls  ', foods: [] }),
            llm: llmParse({ raw: '  spoonfuls  ', foods: [] }),
        });
        ...
        expect(result.merged?.foods).toEqual([{ name: 'spoonfuls', prep: null }]);
        // ⛔ And `raw` is untouched: HAZ-041's "never discard the original" is a different obligation from
        // what the placeholder NAMES, and trimming the stored source line would breach it.
        expect(result.merged?.raw).toBe('  spoonfuls  ');
    });

    it('does not manufacture a name out of nothing', () => {
        const result = compareParses({
            crf: crfParse({ raw: '   ', foods: [] }),
            llm: llmParse({ raw: '   ', foods: [] }),
        });

        expect(result.merged?.foods).toEqual([]);
        expect(result.merged?.reviewReasons).not.toContain('name_is_source_line');
    });

    it('⚠️ keeps the terminal `not_a_food` beside the placeholder — both are true of this line', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'spoonfuls', foods: [] }),
            llm: llmParse({ raw: 'spoonfuls', foods: [], reviewReasons: ['not_a_food'] }),
        });

        expect(result.merged?.reviewReasons).toEqual(['not_a_food', 'name_is_source_line']);
    });

    /**
     * ⚠️ AND SILENCE IS KEPT HERE TOO, which is the other half of R1's filter rather than a contradiction
     * of it. That filter drops `no_food_returned` only when the CRF's food is TAKEN — the merge having
     * answered the silence. Here nobody answered it: neither engine named a food, so the record is still
     * true of the line and travels beside the placeholder it explains.
     */
    it('⚠️ keeps `no_food_returned` beside the placeholder — nothing answered the silence', () => {
        const result = compareParses({
            crf: crfParse({ raw: 'spoonfuls', foods: [] }),
            llm: llmParse({ raw: 'spoonfuls', foods: [], reviewReasons: ['no_food_returned'] }),
        });
