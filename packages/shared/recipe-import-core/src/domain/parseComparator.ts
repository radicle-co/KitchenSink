 ...
 *     {@link llmRescuedTheAmount}). An LLM food list that is EMPTY loses to a CRF that named one (R1 — see
 *     {@link crfRescuedTheFoods}), and where NOBODY named a food the source line itself becomes the
 *     placeholder name (R5 — see {@link withSourceLineFallback}). The merged line's
 *     {@link ParsedLine.provenance} IS that rule, evaluated — the merge reads the winner out of the
 ...
 */
export function canonicaliseFood(food: ParsedFood): ParsedFood {
...
/**
 * R1 — whether the LLM returned NO FOOD where the CRF returned one (owner ruling 2026-09-19).
 *
 * ## ⛔ §3's PRINCIPLE, ONE FIELD OVER AGAIN
 *
 * {@link DEFAULT_WINNERS} gives `foods` to the LLM unconditionally, so an LLM that named NOTHING beat a CRF
 * that named `flour` and the merged line carried `foods: []` — a correct result discarded, which is the one
 * thing the owner's ruling forbids outright. An empty list is not a competing reading of what the line
 * names, any more than an absent unit was a reading of the unit (U36) or an absent amount of the amount
 * (U38). ABSENCE IS SILENCE, and this is the third field it has had to be said about.
 *
 * ⛔ IT REACHES ONLY THE EMPTY CASE, and the narrowness is the whole of the ruling. Two engines that each
 * named a food and named different ones is KTD-11's column, where the LLM's measured strength on multi-food
 * lines and on pulling a unit out of a name governs — including where the LLM named FEWER foods than the
 * CRF, which is `modelSplitsFoods`' mirror and a genuine disagreement about how many foods the text holds.
 * {@link DEFAULT_WINNERS}' own docstring warns that its `foods` entry must not be read as evidence the LLM
 * files modifiers better; this rule must not be read as the beginning of re-deciding it.
 *
 * ⛔ IT IS A MERGE-TIME RULE AND NOTHING ELSE. The CRF's answer never reaches a prompt — `buildParsePrompt`
 * takes the line and nothing else, pinned in invariant position — so a foodless LLM answer is repaired by
 * choosing what to STORE, never by telling the model what the other engine said.
 *
 * ⚠️ The mirror direction needs no rule: `DEFAULT_WINNERS` already gives the foods to the LLM, so a CRF
 * that named nothing against an LLM that did is already resolved the same way, by silence losing.
 *
 * @param crf - The CRF's parse.
 * @param llm - The LLM's parse.
 * @returns `true` when the LLM named no food at all and the CRF named at least one. Pure.
 */
function crfRescuedTheFoods(crf: ParsedLine, llm: ParsedLine): boolean {
    return llm.foods.length === 0 && crf.foods.length > 0;
}
 ...
 *
 * ⚠️ ITS ONE CALLER OVERRIDES THAT RULE, for exactly one reason — see {@link mergedReasons}. The override
 * is argued there rather than here because the rule above is still the default and still the reason every
 * OTHER reason survives; this pointer exists so the absolute wording is not read as the whole story.
 *
...
/**
 * The merged line's reasons — the union, minus the one record R1 has just answered.
 *
 * ⛔ AN OVERRIDE OF {@link unionReasons}' RULE, narrow and argued rather than assumed. That function is
 * right that a reason means the same thing whichever reader raised it — but `no_food_returned` is not a
 * reading of the line at all: it is the validator loop's TERMINAL RECORD that the LLM leg returned nothing
 * (see its own docstring). When {@link crfRescuedTheFoods} fires, the merge binds the CRF's food, and the
 * record is now false about the line it travels on — a cook would be shown `flour` under a chip saying
 * nothing was found. R1 created that, so R1 clears it.
 *
 * ## ⛔ SILENCE IS DROPPED; A VERDICT IS NOT — and the two used to be one token
 *
 * `not_a_food` is NOT filtered here, and that is the correction rather than an omission. It means a
 * validator DISPUTED a name, which is evidence about the line and survives any rescue. Until 2026-09-19 it
 * also carried the silence case, so filtering it discarded the verdict too: measured against this very
 * function, a CRF reading `a large frying-pan` as `1 large :: frying-pan` (the reading CLAUDE.md records
 * it produces) against an LLM whose every name a judge rejected merged to `foods: [{name:'frying-pan'}]`
 * with `reviewReasons: []`, and `landingOf` then called it `parsed`, `needsReview: false` — a name a judge
 * had rejected, published with nobody sent to look. That is the hole R1 exists to close, one door over.
 *
 * ⛔ ONLY THAT REASON, and only on a rescue. Everything else the silent engine raised is kept, including
 * `measurement_unverified`, which is about the measure and is untouched by which engine named the food.
 *
 * ⚠️ The route cannot be inferred from the parse's SHAPE — a Route-B answer on a measureless line has
 * `statedMeasure === null` exactly as the blanked Route-A record does — which is why the distinction has
 * to be a token the producer writes, and why this function reads nothing but the token.
 *
 * @param crf - The CRF's parse.
 * @param llm - The LLM's parse.
 * @param foodsRescued - Whether the CRF's foods were taken because the LLM named none.
 * @returns The reasons the merged line carries. Pure.
 */
function mergedReasons(crf: ParsedLine, llm: ParsedLine, foodsRescued: boolean): readonly IngredientReviewReason[] {
    const union = unionReasons(crf.reviewReasons, llm.reviewReasons);

    return foodsRescued ? union.filter((reason) => reason !== 'no_food_returned') : union;
}

/**
 * R5 — the source line as the food of LAST RESORT (owner ruling 2026-09-19).
 *
 * When no reader found a food, the line's own text becomes the food's name so the words are never lost: a
 * cook reviewing the job is shown what they pasted rather than a blank name.
 *
 * ## ⛔ IT IS A PLACEHOLDER, AND IT SAYS SO
 *
 * `name_is_source_line` rides with it, and `landingOf` reads that reason as `unparseable` — the proposal is
 * kept and the line stays editable, but nothing is bound and no food entity is created. Letting a raw line
 * land as a clean `parsed` name is the defect this whole change exists to close; a name nobody read must
 * never be presented as one that was.
 *
 * ## ⛔ LAST, BEHIND R1 — the order IS the rule
 *
 * LLM food → else CRF food ({@link crfRescuedTheFoods}) → else the line. This function sees the merged
 * foods, so it can only fire once both of those have declined, which is what makes the precedence
 * structural rather than a comment. Applied to a single-engine merge for the same reason: "no food from
 * anywhere" includes an engine that was never asked.
 *
 * ⚠️ A BLANK line gets nothing. There is no text to preserve, `foods: []` is already the honest answer, and
 * a food named `''` would be a placeholder that looks like a parse.
 *
 * ## ⚠️ TRIMMED, AND DELIBERATELY NOT CANONICALISED
 *
 * The name is `raw.trim()`, so the same string decides "is there any text" and "what is the text" — a
 * placeholder reading `'  spoonfuls  '` carried padding no other food in the merge can, because every
 * other food goes through `canonicaliseFood` and this one does not.
 *
 * ⛔ And it must not: `canonicaliseFood` MOVES words between `name` and `prep` (`boiling water` becomes
 * `water` + `boiling`). Doing that to a line nobody read would split the cook's own sentence across two
 * fields and dress a placeholder up as a parse — contradicting both the reason that rides with it and the
 * copy a cook is shown ("we kept this line as you wrote it"). Trimming deletes nothing and states nothing;
 * placement asserts a reading. Only the first is honest here.
 *
 * ⚠️ `provenance.foods` is left as the winner rule computed it, and that is an accepted inaccuracy rather
 * than an oversight: {@link ParseProvenance}'s value set is `ParseFactSource`, which has an inhabitant for
 * each engine and one for a cook, and none for "we supplied this". Widening a contract that reaches stored
 * proposals, for a line that never binds, costs more than the reason already records.
 *
 * @param merged - The merged line, however it was merged.
 * @returns It unchanged when any reader named a food; otherwise with the source line as its one food and
 *   the placeholder reason recorded. Pure.
 */
function withSourceLineFallback(merged: ParsedLine): ParsedLine {
    const text = merged.raw.trim();

    if (merged.foods.length > 0 || text === '') {
        return merged;
    }

    return {
        ...merged,
        foods: [{ name: text, prep: null }],
        reviewReasons: [...merged.reviewReasons, 'name_is_source_line'],
    };
}
...
export function compareParses(answers: EngineAnswers): ParseComparison {
    ...
    if (crf === null) {
        return llm === null
            ...
            : {
                  agreement: { kind: 'single-engine', engine: 'llm' },
                  merged: withSourceLineFallback(fromOneEngine(llm, 'llm')),
              };
    }

    if (llm === null) {
        return {
            agreement: { kind: 'single-engine', engine: 'crf' },
            merged: withSourceLineFallback(fromOneEngine(crf, 'crf')),
        };
    }
    ...
    const measureWinners = rescued ? rescuedWinners(llm) : winnersWithoutAMeasureRescue(crf, llm);
    // R1, applied LAST and to the `foods` key alone: both measure rules spread `DEFAULT_WINNERS`, so the
    // food rescue has to override whichever of them ran rather than living inside one of them. The two
    // concern disjoint facts, so there is no precedence question to answer between them.
    const foodsRescued = crfRescuedTheFoods(crf, llm);
    const provenance: ParseProvenance = foodsRescued ? { ...measureWinners, foods: 'crf' } : measureWinners;
    ...
    const merged: ParsedLine = {
        ...
        reviewReasons: mergedReasons(crf, llm, foodsRescued),
        provenance,
    ...
    // ⛔ R5 LAST, over the finished merge — see `withSourceLineFallback` on why the order is the rule
    // rather than a note about one. ⚠️ It touches only what is STORED: `fields` is computed from the two
    // ENGINES' answers above, so a placeholder can never make a line look like agreement, and R1 cannot
    // either. The `foods` disagreement a rescued line carries is reported exactly as U36a reports the
    // amount it moves.
    const answer = withSourceLineFallback(merged);

    return fields.length === 0
        ? { agreement: { kind: 'agree' }, merged: answer }
        : { agreement: { kind: 'differ', fields }, merged: answer };
}
