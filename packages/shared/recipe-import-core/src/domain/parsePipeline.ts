...
import { bindsNothing, type ParsedFacts, type ParsedLine, type ParseEngine } from '../parsedLine.js';
...
/**
 * Which digests were READ, and so may be remembered.
 *
 * ⛔ A LINE NOBODY COULD READ IS NEVER REMEMBERED (owner ruling, 2026-09-19) — and the subject of that
 * ruling is the LINE. It named the two statuses `landingOf` assigns to a MERGED reading, so this asks
 * {@link bindsNothing} about the merge and never about one engine's answer: an engine that found no food
 * on a line the other engine read is SILENT, its silence is what `compareParses` merges, and dropping it
 * would make the next submission re-ask and read a failed re-ask as ABSENCE instead (ADR-0026 §3's
 * distinction, one layer down).
 *
 * ⚠️ The accepted cost is a line no reader could read at all: it is re-asked on every submission, for as
 * long as it keeps failing. `unavailable` needs no rule here — it never reaches `candidates`, which is the
 * `failed_retryable` half of the same ruling.
 *
 * @param candidates - Every freshly-read answer, with the digest it answers for.
 * @param answerFor - This run's lookup of one engine's answer for a digest.
 * @returns One verdict per distinct digest. Pure with respect to `answerFor`, which reads only this run's
 *   own maps.
 */
function readableDigests(
    candidates: readonly { digest: LineDigest; parse: ParsedLine }[],
    answerFor: (digest: LineDigest, engine: ParseEngine, raw: string) => EngineAnswer,
): Map<LineDigest, boolean> {
    const readable = new Map<LineDigest, boolean>();

    for (const { digest, parse } of candidates) {
        if (!readable.has(digest)) {
            const { merged } = compareParses({
                crf: answerFor(digest, 'crf', parse.raw),
                llm: answerFor(digest, 'llm', parse.raw),
            });

            readable.set(digest, merged !== null && !bindsNothing(merged));
        }
    }
...
async function consultEngines(
    ...
    // ⚠️ A `Map`, not a record written through `answers[engine] = …`. The repo-wide guard
    // (`packages/infra/global/__tests__/prototypePollutionSinks.test.ts`) reads an accumulator indexed by a
    // VARIABLE as data-keyed, and it is deliberately over-broad because its remedy is never wrong. The
    // narrower remedy — `Object.create(null)` — would cost the exhaustiveness the return literal below
    // carries: a third `ParseEngine` fails to compile there, which is the check worth keeping.
    const answered = new Map<ParseEngine, readonly EngineAnswer[]>();
    ...
    settled.forEach((outcome, index) => {
        ...
        answered.set(engine, outcome.value);
    });
    ...
    // An engine that was not asked, that was rejected, or that mispaired has NO entry — and an empty array is
    // what every one of those meant before, unchanged: absence, never an empty parse (ADR-0026 §3).
    return { crf: answered.get('crf') ?? [], llm: answered.get('llm') ?? [] };
}
...
export async function runParsePipeline(
    ...
    const candidates: { digest: LineDigest; engine: ParseEngine; parse: ParsedLine }[] = [];
    ...
    for (const engine of ENGINES) {
        missingByEngine[engine].forEach((digest, position) => {
            ...
            candidates.push({ digest, engine, parse: answer });
        });
    }

    /**
     * One engine's answer for a digest: a cached row rehydrated, a fresh reading, or absence.
     *
     * ⚠️ `raw` is a PARAMETER because two positions sharing a digest were asked about ONCE, under
     * whichever spelling came first; the digest says they are the same LINE, not the same STRING, and
     * HAZ-041 is about the string.
     */
    const answerFor = (digest: LineDigest, engine: ParseEngine, raw: string): EngineAnswer => {
        const hit = cached.get(cacheSlot(digest, engine));

        if (hit !== undefined) {
            return rehydrateEngineParse(hit, raw, engine);
        }

        const answered = fresh.get(cacheSlot(digest, engine));

        return answered === undefined ? UNAVAILABLE : { ...answered, raw };
    };

    const readable = readableDigests(candidates, answerFor);
    const writes = candidates.filter(({ digest }) => readable.get(digest) === true);
    ...
    return lines.map((line, index) => {
        ...
        /** This position's own reading, through the shared lookup, recording which engines the cache served. */
        const atThisPosition = (engine: ParseEngine): EngineAnswer => {
            if (cached.has(cacheSlot(digest, engine))) {
                fromCache.push(engine);
            }

            return answerFor(digest, engine, line);
        };
        ...
        const comparison = compareParses({ crf: atThisPosition('crf'), llm: atThisPosition('llm') });
