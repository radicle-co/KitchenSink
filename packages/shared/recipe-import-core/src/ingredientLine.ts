...
export type IngredientReviewReason =
     ...
     *
     * ⚠️ WHERE THE DROPPED TEXT ACTUALLY IS, stated precisely because it is easy to get wrong.
     * `ClauseSegment.trailingInstruction` carries it out of the segmenter, and `cookbook-import`
    ...
    /**
     * A VALIDATOR DISPUTED A NAME the parse offered (plan U7, R6) — and only that. The line is SAVED, the
     * disputed names are dropped, and this reason records the drop. Where EVERY name was disputed there is
     * nothing left to bind, so `foods: []` and the stated values are blanked with them (the equipment /
     * heading case); where some survived, they are kept and this reason records the ones that did not.
     * ⚠️ Not value-corrupting: no stated number was altered.
     *
     * ⛔ IT IS EVIDENCE ABOUT THE LINE, NOT SILENCE ABOUT IT, and the split from
     * {@link IngredientReviewReason} `no_food_returned` is what makes that readable (2026-09-19). This
     * reason used to carry BOTH terminal facts, so the merge — which is right to drop silence an engine's
     * partner has just filled in — dropped a validator's verdict along with it: a CRF naming `frying-pan`
     * against an LLM whose every name was rejected merged to a clean `parsed` line with no reasons at all.
     * A verdict SURVIVES the merge; only silence is dropped.
     */
    | 'not_a_food'
    /**
     * The parse returned NO FOOD NAME AT ALL, at any attempt, and the loop ended (plan U7, R6 / R2).
     *
     * ⛔ THIS IS SILENCE, and its separation from {@link IngredientReviewReason} `not_a_food` is the whole
     * of its reason to exist. No validator was ever offered a name to dispute — the foodness judge iterates
     * the food list and an empty list judges nothing — so nothing here is a finding ABOUT the line. It is a
     * record that one reader produced nothing for it.
     *
     * ⛔ IT IS A RECORD ABOUT ONE READER'S LOOP, NOT A FACT ABOUT THE MERGED LINE, and the merge is where
     * the difference bites (R1, 2026-09-19). An LLM that returned nothing raises this; if the CRF named a
     * food the merge then keeps, the line a cook sees HAS an ingredient and this record is false about it —
     * so `compareParses` drops THIS reason in exactly that case, and only there. It still travels beside
     * `name_is_source_line`, where both are true.
     *
     * ⚠️ The measure the parse DID state is KEPT (amended 2026-08-31's principle, one field over): no
     * validator questioned it, the judge may have passed it outright, and a reviewer shown `three` can
     * correct the line where a reviewer shown "No amount given" cannot. ⚠️ Not value-corrupting: nothing
     * was altered, and a name nobody read states no number.
     */
    | 'no_food_returned'
    /**
    ...
    /**
     * No reader found a food on this line, so the food's name IS the source line, verbatim (R5, owner
     * ruling 2026-09-19). The text is never lost — a cook is shown the words they pasted rather than an
     * empty name — and this reason is what says those words are a PLACEHOLDER rather than a reading.
     *
     * ⛔ IT MUST NEVER LAND AS A CLEAN PARSE. `landingOf` reads this reason as `unparseable`, which keeps
     * the proposal and leaves the line editable while binding nothing: a line whose name is its own text
     * has not been read, and letting it land `parsed` — with a name the catalog would then try to resolve —
     * is the whole defect R5 exists inside. ⚠️ It is therefore the one reason whose presence changes a
     * landing status, which is why it is a reason of its own rather than a second meaning for
     * `not_a_food`: that one is raised beside SURVIVING foods too (mixed exhaustion), so it cannot carry
     * this consequence without taking those lines down with it.
     *
     * ⚠️ Not value-corrupting: the quantity and unit are exactly what the readers read, and a placeholder
     * name states no number at all. Membership would make `cookbook-import` discard the very lines this
     * reason exists to preserve — `additional_foods_dropped`'s argument, for the fifth time.
     */
    | 'name_is_source_line';
