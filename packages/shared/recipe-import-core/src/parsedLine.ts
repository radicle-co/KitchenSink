 ...
 *
 * ⚠️ `statedMeasure` is NOT dropped because "its READING survives as `quantity` + `unit`": that is only true of
 * a measure a number can hold. For the class
...
/**
 * Whether a merged reading BINDS NOTHING — the one condition under which a line was not read.
 *
 * ⛔ THE SUBJECT IS THE MERGED LINE, NEVER ONE ENGINE'S ANSWER. An engine that named no food on a line
 * the other engine read is silent, not wrong, and `compareParses` takes the other engine's foods; judging
 * its answer by this predicate would call that line a failure.
 *
 * It lives here, in the package both readers may import, because it decides two things that must never
 * disagree: which lines land `unparseable` (`recipe-workers/handlers/parseLine.ts`'s `landingOf`) and
 * which readings the parse cache may remember (`domain/parsePipeline.ts`). Stating it twice is what lets
 * them drift, and a deployable cannot be imported back into this package (ADR-0026 §6) — so the rule is
 * declared once, here, and both call it.
 *
 * ⛔ READ THE SECOND DISJUNCT FIRST. Since a line no reader could read is given the SOURCE TEXT as a
 * placeholder food, the readings this catches in the parse pipeline arrive with exactly one food and are
 * caught by `name_is_source_line`; without it a PLACEHOLDER would land `parsed`, with a whole ingredient
 * line handed to the catalog as a name to resolve. It must be that reason and not `not_a_food`, which a
 * MIXED exhaustion raises BESIDE foods that survived — keying on that one would take those lines down
 * with it.
 *
 * ⚠️ The zero-foods disjunct is no longer the common route but still holds the blank-`raw` case and the
 * correction tier: a reading that binds nothing has not been read, whatever its reasons say. It once
 * required `not_a_food` as well, which only a validator DISPUTING a name could satisfy — so a parse that
 * returned no name disputed nothing and fell through with an empty name and `needsReview: false`,
 * measured on 28 of 1,265 lines.
 *
 * @param parsed - A merged reading of one line.
 * @returns Whether it binds no food: none at all, or one that is only the cook's own line handed back.
 *   Pure.
 */
export function bindsNothing(parsed: ParsedLine): boolean {
    return parsed.foods.length === 0 || parsed.reviewReasons.includes('name_is_source_line');
}
