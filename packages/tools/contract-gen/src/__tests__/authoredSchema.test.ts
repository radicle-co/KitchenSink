...
import type { ComposedSource } from '../composedSources.js';
...
async function makeServiceTree(files: Readonly<Record<string, string>>): Promise<string> {
    const root = await scratchDirectory('contract-gen-');
...
describe('computeContractHash', () => {
     ...
     * `./composedSources.test.ts` against real reachability, which is where they belong.
     */
