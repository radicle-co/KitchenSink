...
function buildComponentSchemas(components: Readonly<Record<string, ZodType>>): Record<string, Record<string, unknown>> {
    ...
    const schemas: Record<string, Record<string, unknown>> = Object.create(null) as Record<
        string,
...
export function buildOpenApiDocument<TComponents extends Readonly<Record<string, ZodType>>>(
    ...
    for (const [path, methods] of Object.entries(spec.paths)) {
        // ⛔ NULL-PROTOTYPE, because the keys come from the spec being generated. The exposure here is
        // build-time and our own input, not a request — but an operation or path named `__proto__` would be
        // silently ABSENT from the generated document rather than failing, and the remedy costs nothing.
        const pathItem: Record<string, unknown> = Object.create(null) as Record<string, unknown>;
