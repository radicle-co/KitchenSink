 ...
 * 5. **This measures a PARSE, not a verification.** `docs/reports/2026-08-23/verificationBakeOff.md`
 ...
 * ## The prompt arm
 *
 * `--variant` selects which wording is measured and defaults to `v1`. ⛔ **`v1` IS NOT THE SHIPPED PROMPT
 * and has not been since 2026-08-27**, when `PARSE_SYSTEM_PROMPT` moved to the 19,925-byte relational
 * document. Measured byte lengths: v1 511, v2 561, v3 763, v4 733, v5 810, v6 3,656 — no CANDIDATE arm
 * matches production. **`--variant shipped` is the arm that does**, by reference, with the production
 * reader; use it for anything that informs a decision about the leg that runs. The default is left on v1
 * because it is the arm every recorded figure was measured against and moving it would silently re-point
 * an existing command line at a different experiment.
 *
 * Each run measures exactly ONE arm — a bake-off is N invocations over the same corpus, never one
 * invocation that interleaves them, because a shared run would have to keep N contract censuses apart
 * inside one reducer for no gain.
 *
 ...
 * `docs/reports/2026-08-23/verificationBakeOff.md` §1. It is unmeasured, not rejected.
 *
...
// ⚠️ `NOVA_2_LITE_MODEL_ID` is deliberately NOT in the default roster: it is the model the parse leg ships
// on, it is ~5.5x Nova Lite on input and ~11.5x on output, and a run that acquired it by forgetting a flag
// would be a surprise on the bill. Name it with `--models` when it is what you meant to measure.
