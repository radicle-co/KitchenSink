 ...
 * the two terminal records — `not_a_food` (a judge rejected every name) and `no_food_returned` (the model
 * returned none) — are listed separately and neither is hidden), name/measure changes, retry-rate, and the
 * validator verdict census.
 *
...
interface DiffRow {
    ...
    /**
     * ⛔ `not-a-food` and `no-food-returned` are SEPARATE counters, not one (2026-09-19).
     *
     * The two review reasons they read were one token until the merge was found dropping a validator's
     * verdict along with an engine's silence. Folding them back together here would report a finer fact
     * coarsely; leaving only the old branch would silently reclassify every Route-B line as
     * `retried-changed`, and a census that moves without anyone touching the census is how this repo's
     * §8b divergence happened. `not-a-food` is "a judge rejected every name"; `no-food-returned` is "the
     * model returned none".
     */
    readonly kind: 'unchanged' | 'retried-changed' | 'food-loss' | 'not-a-food' | 'no-food-returned' | 'unavailable';
}
...
const rows: DiffRow[] = corpus.map((line, index) => {
    ...
    if (final === undefined || 'unavailable' in final) {
        ...
        kind = 'not-a-food';
    } else if (final.reviewReasons.includes('no_food_returned')) {
        kind = 'no-food-returned';
