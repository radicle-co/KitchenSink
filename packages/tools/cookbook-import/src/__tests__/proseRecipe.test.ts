...
import type { IngredientReviewReason, ParsedFacts, ParsedLine } from '@kitchensink/recipe-import-core';
 ...
 * a clause whose own reading misstates a value — a judgement about the SPAN. The pipeline decides what an
 * ACCEPTED line says, never which span is a line, and its own `reviewReasons` are never merged into the
 * stored line's.
 *
 * ⚠️ THE FIXTURES WIDENED 2026-09-19, from `ParsedFacts` to the `ParsedLine` the pipeline always actually
 * returned. That is not bookkeeping: the substitution now reads `reviewReasons` for ONE question — whether
 * the name it is about to take is R5's placeholder rather than a reading — and under the narrow type that
 * question could not be asked at all. No assertion was weakened to make them compile; every case below
 * states the reasons it means, and the placeholder case is new.
 */
describe('pipeline readings substituted for the library parse', () => {
    ...
    /**
     * A `ParsedLine` around the facts one case is about — DAMP, so each test states its own reasons.
     *
     * `reviewReasons: []` is the ordinary case: an engine read this line and said nothing was amiss.
     */
    const reading = (
        raw: string,
        facts: ParsedFacts,
        reviewReasons: readonly IngredientReviewReason[] = [],
    ): ParsedLine => ({
        raw,
        ...facts,
        reviewReasons,
    ...
    /**
     * ⛔ THE PLACEHOLDER IS NOT A READING, and after R5 it is the ONLY way the zero-food decline above can
     * be reached at all — R5 makes an empty food list unrepresentable for a non-blank line, so without this
     * veto the whole source clause replaces the library parser's name in a published recipe.
     *
     * ⚠️ The shape here is R5's own: the food's name IS the line, and `name_is_source_line` is what says so.
     * The integration tier drives the real pipeline to prove this is the shape it actually produces; this
     * tier proves the substitution refuses it.
     */
    it('⛔ declines R5’s PLACEHOLDER, whose one “food” is the source line itself', () => {
        const readings = new Map<string, ParsedLine>([
            [
                'one pound of butter',
                reading(
                    'one pound of butter',
                    {
                        ...
                        quantity: amount(9),
                        unit: 'quart',
                        foods: [{ name: 'one pound of butter', prep: null }],
                    },
                    ['name_is_source_line'],
                ),
            ],
        ...
        const names = outcome.recipe.ingredients.map((ingredient) => ingredient.name);

        expect(names).toContain('butter');
        expect(names).not.toContain('one pound of butter');
        // ⛔ Declined WHOLE: the measure must not ride in behind a name that was refused.
        expect(outcome.recipe.ingredients.some((ingredient) => ingredient.unit === 'quart')).toBe(false);
    });
