/**
 * The HARNESS's own assertions about the parse prompt it measured — read straight from the authority,
 * `@kitchensink/recipe-core/parsing/parse-prompt`.
 *
 * ⚠️ This is deliberately a SECOND reader of that text, not a duplicate of recipe-core's own suite: every
 * figure in `docs/reports/2026-08-23-002-ingredient-parse-model-comparison.md` is denominated in these
 * exact bytes, so if the shipped prompt moves, the harness must go red and be re-briefed rather than keep
 * publishing numbers for a prompt nothing sends any more.
 *
 * It used to import a local `parseComparison/parsePrompt.ts` re-export, deleted under
 * `CODING_STANDARDS §4`; the alias bought nothing the subpath does not, and its docstring restated the
