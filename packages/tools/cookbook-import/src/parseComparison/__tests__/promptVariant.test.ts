...
describe('the arm registry', () => {
    ...
    it('lets NO arm name a vessel word, so the headline is not measuring an enumeration', () => {
        ...
        // ⛔ REWRITTEN AGAIN, not relaxed, for `shipped` — and its break is the LOUDEST finding in the file.
        // The prompt production sends enumerates the detector's own vocabulary wholesale: rule 10's
        // equipment definition and eleven worked examples name 22 vessel tokens (9 distinct). So the shipped
        // arm's vessel-in-`foods` figure is partly a measurement of that enumeration and is NOT comparable
        // with v1-v5's, which were built to name none. It IS comparable ACROSS MODELS on this arm — every
        // model sees the same words — which is the comparison a model bake-off actually asks for.
        // ⛔ Pinned token-for-token, in order, exactly as v6's is: a 23rd vessel word is a new fact and must
        // fail here rather than quietly widen what the census is measuring.
        const ENUMERATING_ARMS: Readonly<Record<string, readonly string[]>> = {
            v6: ["'skillet'", "'bowl')"],
            shipped: [
                'skillet',
                'oven',
    ...
    it('sends the arm’s system prompt with the SHIPPED delimiter, byte for byte', () => {
        for (const id of PARSE_VARIANT_IDS) {
            ...
            if (variant.buildUserTurn === undefined) {
                // ⛔ ASKED OF `buildParsePrompt`, never transcribed. This branch was DEAD — every arm of the
                // day declared its own `<ingredient_line>` turn — and it held a literal that the shipped
                // module stopped emitting on 2026-08-27 when the delimiter moved to `<input>`. The `shipped`
                // arm is the first to route through the default path, and it would have been failed by a
                // stale copy of the thing it exists to reuse. Deriving it is the same discipline the comment
                // below already applies to an arm's own turn.
                expect(prompt.userMessage).toBe(buildParsePrompt('one cup of flour').userMessage);
            } else {
...
describe('shipped — the arm that measures the prompt production actually sends', () => {
    /**
     * ⛔ THE ARM THIS FILE EXISTED FOR AND DID NOT HAVE. Every other arm is a CANDIDATE wording; none of them
     * is what `gatedLlm.ts` sends. v1 was documented as "the shipped prompt" and stopped being it on
     * 2026-08-27, when `PARSE_SYSTEM_PROMPT` moved to the 19,925-byte relational document — so a model
     * comparison run on any existing arm measures a prompt we do not ship, which is the one way to spend the
     * money and learn nothing.
     */
    it('IS the shipped constant, by reference and not by transcription', () => {
        expect(PARSE_VARIANT_SHIPPED.systemPrompt).toBe(PARSE_SYSTEM_PROMPT);
        expect(createHash('sha256').update(PARSE_VARIANT_SHIPPED.systemPrompt, 'utf8').digest('hex')).toBe(
            PARSE_PROMPT_SHA256,
        );
    });

    it('inherits the SHIPPED delimiter rather than declaring one, so the user turn is production\u2019s', () => {
        // ⛔ `buildUserTurn` is deliberately ABSENT. v1-v5 pin `<ingredient_line>` because their own text
        // names it; the shipped prompt names `<input>` and `buildParsePrompt` is its single authority. An
        // arm that re-spelled the tag here would be measuring a turn production never sends.
        expect(PARSE_VARIANT_SHIPPED.buildUserTurn).toBeUndefined();
        expect(buildVariantPrompt(PARSE_VARIANT_SHIPPED, 'two eggs').userMessage).toBe(
            buildParsePrompt('two eggs').userMessage,
        );
    });

    it('inherits the shipped output budget and the shipped prompt cap', () => {
        // ⚠️ Both ABSENT, not restated. A restated constant is a second authority that can drift away from
        // the one the worker enforces, and the bound this run is judged against would then be the harness's.
        expect(PARSE_VARIANT_SHIPPED.maxOutputTokens).toBeUndefined();
        expect(PARSE_VARIANT_SHIPPED.promptCharCap).toBeUndefined();
    });

    it('asks for the prompt cache, because the shipped call does', () => {
        // ⛔ `gatedLlm.ts:433` sets `cachePrompt: true`. Caching cannot change the ANSWER, so no accuracy
        // figure moves — but the cost column would be 4x production's on the input term without it, and the
        // cost column is what a model switch is being decided on.
        expect(PARSE_VARIANT_SHIPPED.cachePrompt).toBe(true);
    });

    it('reads a production-shaped answer through the PRODUCTION normalizer', () => {
        ...
        const reading = classifyParseResponse(answer, 'end_turn', PARSE_VARIANT_SHIPPED.readAnswer);

        expect(reading.kind).toBe('valid');
        expect(reading.kind === 'valid' && reading.parse).toEqual({
            measure: '2',
            statedUnit: 'cups',
    ...
    it('carries EACH record\u2019s own preparations, never the first record\u2019s', () => {
        // ⛔ Rule 8 of the shipped prompt SPLITS foods whose preparation differs. Replicating one record's
        // prep across all of them would erase the distinction the document is shaped to draw, and the prep
        // column would measure the projection rather than the model.
        const answer = JSON.stringify([
            { food_items: ['cream cheese'], measurement: null, preparations: ['Mix'], equipment: null },
            { food_items: ['butter'], measurement: null, preparations: ['Mix', 'melted'], equipment: null },
        ]);
        const reading = classifyParseResponse(answer, 'end_turn', PARSE_VARIANT_SHIPPED.readAnswer);

        expect(reading.kind === 'valid' && reading.parse.foods).toEqual([
            { name: 'cream cheese', prep: 'Mix' },
            { name: 'butter', prep: 'Mix, melted' },
        ]);
    });

    it('takes the amount VERBATIM and never joins the unit into it', () => {
        // ⛔ The join `normalizeParseAnswer` performs for `statedMeasure` is safe only because it was
        // measured at 0/146 restatements on ONE model. A model that answers
        // `quantity: "two tablespoons", unit: "tablespoons"` — gen0 did — would be folded to
        // `two tablespoons tablespoons`, whose leftover word `judgeMeasure` reads as a SECOND amount and
        // reports as `amountCountDiffers`: a disagreement about nothing, manufactured by the projection and
        // charged to whichever candidate restates. The arm takes `statedQuantity` and lets `statedUnit`
        // carry the unit, exactly as v3, v5 and v6 do.
        const answer = JSON.stringify([
            {
                food_items: ['sugar'],
                measurement: { quantity: 'two tablespoons', unit: 'tablespoons', unit_type: 'VOLUME' },
        ...
        expect(reading.kind === 'valid' && reading.parse.measure).toBe('two tablespoons');
        expect(reading.kind === 'valid' && reading.parse.statedUnit).toBe('tablespoons');
    });

    it('accepts `measure: null` and the EMPTY document, which are both correct answers', () => {
        // ⚠️ `parseAnswer.ts`: a null measurement is the single largest measured "failure" of both candidate
        // models and is not one — it fires on lines that state no measure. The empty array is named by the
        // prompt itself ("No culinary content -> []").
        const nullMeasure = classifyParseResponse(
            JSON.stringify([{ food_items: ['salt'], measurement: null, preparations: null, equipment: null }]),
            'end_turn',
            PARSE_VARIANT_SHIPPED.readAnswer,
        );

        expect(nullMeasure.kind).toBe('valid');
        expect(nullMeasure.kind === 'valid' && nullMeasure.parse.measure).toBe('');

        const empty = classifyParseResponse('[]', 'end_turn', PARSE_VARIANT_SHIPPED.readAnswer);

        expect(empty.kind).toBe('valid');
        expect(empty.kind === 'valid' && empty.parse.foods).toEqual([]);
    });

    it('REFUSES v1\u2019s document, and v1 refuses its own', () => {
        // ⛔ The artefact that would make the whole run meaningless in the other direction: a reader that
        // accepted both shapes could not tell a production answer from a bake-off one.
        const shipped = JSON.stringify([
            { food_items: ['flour'], measurement: null, preparations: null, equipment: null },
        ]);
        const v1Document = '{"measure":"one cup","foods":[{"name":"flour","prep":null}]}';

        expect(classifyParseResponse(v1Document, 'end_turn', PARSE_VARIANT_SHIPPED.readAnswer).kind).toBe('wrongShape');
        expect(classifyParseResponse(shipped, 'end_turn', PARSE_VARIANT_V1.readAnswer).kind).toBe('wrongShape');
    });

    it('REFUSES an extra key, because the prompt names the WHOLE document', () => {
        ...
        expect(classifyParseResponse(extra, 'end_turn', PARSE_VARIANT_SHIPPED.readAnswer).kind).toBe('wrongShape');
    });

    it('states that its unit is the MODEL\u2019s, so no table can blend it with a derived one', () => {
        expect(PARSE_VARIANT_SHIPPED.unitSource).toBe('model-stated');
    });

    it('is registered, so the runner can name it', () => {
        expect(PARSE_VARIANT_IDS).toContain('shipped');
        expect(resolveParseVariant('shipped')).toBe(PARSE_VARIANT_SHIPPED);
    });
});
