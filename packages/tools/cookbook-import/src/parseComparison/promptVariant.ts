...
export type ParseVariantId = 'v1' | 'v2' | 'v3' | 'v4' | 'v5' | 'v6' | 'shipped';

/** Every arm, so a roster cannot silently omit one. */
export const PARSE_VARIANT_IDS = [
    'v1',
    'v2',
    'v3',
    'v4',
    ...
    'v6',
    'shipped',
] as const satisfies readonly ParseVariantId[];
...
export interface ParseVariant {
    ...
    /**
     * Place a cache checkpoint after the system prompt, as the SHIPPED call does (`gatedLlm.ts`).
     *
     * ⛔ Absent on every candidate arm and that is not an oversight. Caching cannot change what a model
     * ANSWERS, so no accuracy figure in this harness moves either way — what it changes is the bill, and a
     * candidate whose wording is being judged has no production call whose billing it should imitate. The
     * shipped arm sets it because its cost column is read as production's, and at a 5,025-token prompt the
     * cached read is 25% of the fresh rate while Nova bills the write at zero.
     */
 ...
 */
export const OWNER_PROMPT_PATH = new URL('./prompts/v6Owner.txt', import.meta.url).pathname;
...
/**
 * shipped — THE PROMPT PRODUCTION ACTUALLY SENDS, by reference.
 *
 * ⛔ **THE ARM A MODEL COMPARISON MUST USE, AND THE ONE THIS FILE DID NOT HAVE.** v1 was authored as "the
 * shipped prompt" and STOPPED being it on 2026-08-27, when `PARSE_SYSTEM_PROMPT` moved to the 19,925-byte
 * relational document. Measured: v1 is 511 bytes, v2 561, v3 763, v4 733, v5 810, v6 3,656 — no candidate
 * arm matches. A billed run on any of them measures a wording we do not ship, so the numbers cannot inform
 * a decision about the leg that runs in production.
 *
 * ⚠️ **It is NOT a seventh candidate wording** and it must never be edited to become one. Every other arm is
 * harness-local text; this one is the constant `gatedLlm.ts` sends, held BY REFERENCE, and its reader is the
 * production reader. `PARSE_VARIANT_V1`'s docstring rule still binds: shipping a winner is a separate change
 * that owes `PARSE_SYSTEM_PROMPT`, `PARSE_PROMPT_SHA256`, `PARSE_PROMPT_VERSION` and the normalizer.
 *
 * ⚠️ **Its unit is MODEL-STATED, like v3/v5/v6 and unlike v1/v2/v4.** The shipped document has a
 * `measurement.unit` slot, so the unit is the model's claim and {@link statedUnitOf} takes it at face value.
 * No table may put this arm's unit column beside a derived one without saying so.
 *
 * ⛔ **It ENUMERATES vessel words, which the candidate arms were deliberately built not to do.** Rule 10 and
 * the examples name `bowl`, `pan`, `skillet`, `saucepan`, `oven`, `stove`, `dish`, `jar` and `boiler` — 22
 * tokens that `notAFoodLexicon` scores against. So this arm's vessel-in-`foods` figure is partly a
 * measurement of that enumeration and is NOT comparable with v1-v5's. It is comparable ACROSS MODELS on this
 * arm, which is what a model bake-off asks, because every model sees the same words.
 *
 * ⛔ **It declares no `buildUserTurn`, on purpose.** v1-v5 pin `<ingredient_line>` because their own text
 * names it; the shipped prompt names `<input>` and `buildParsePrompt` is its one authority. That also means
 * this arm's user turn differs from every other arm's, so an arm-vs-arm table across this boundary would be
 * comparing two things at once.
 */
export const PARSE_VARIANT_SHIPPED: ParseVariant = Object.freeze({
    id: 'shipped',
    systemPrompt: PARSE_SYSTEM_PROMPT,
    readAnswer: readShippedAnswer,
    unitSource: 'model-stated',
    summary: 'the SHIPPED prompt and the SHIPPED reader, by reference: a root array of relational records',
    cachePrompt: true,
});
...
/**
 * Read a shipped-shape answer through the PRODUCTION boundary and project it.
 *
 * ⛔ `modelParseAnswerSchema` and `normalizeParseAnswer` are the SHIPPED pair, imported rather than
 * restated. A local re-spelling of the document would make this arm a seventh candidate that merely looked
 * like production — and the two most load-bearing decisions in that boundary (`measure: null` is a correct
 * answer, `null` and `''` collapse to one absent value) would have to be re-derived here to be measured.
 *
 * ⛔ `measure` IS `statedQuantity` AND NEVER `statedMeasure`. `normalizeParseAnswer` also publishes a JOINED
 * phrase, and joining is safe only because it measured 0 of 146 restatements — on ONE model, on a
 * held-out set. Two of the three models this arm exists to compare have never been measured on this prompt,
 * and gen0 DID restate (`quantity: "two tablespoons", unit: "tablespoons"`): that folds to
 * `two tablespoons tablespoons`, whose leftover word `judgeMeasure` reads as a SECOND amount and reports as
 * `amountCountDiffers`. The error is asymmetric in exactly the wrong direction — it penalises whichever
 * candidate restates — so the amount slot supplies `measure` and the unit slot supplies `statedUnit`, which
 * is what v3, v5 and v6 already do and what `withStatedUnit` was built for.
 *
 * ⚠️ `statedQuantity` and `statedUnit` are `string | null | undefined` on `LlmParse` and `undefined` is
 * unreachable here: `normalizeParseAnswer` always writes both. They are folded through the same
 * `null -> ''` collapse {@link statedUnitOf} applies, because in THIS vocabulary `undefined` means "this arm
 * has no unit slot" and the shipped document has one.
 *
 * @param value - The JSON document the model returned.
 * @returns The projection, or the shape failure. Pure.
 */
function readShippedAnswer(value: unknown): ReturnType<AnswerReader> {
    ...
    const reading = normalizeParseAnswer(parsed.data);

    return {
        ok: true,
        parse: {
            // ⚠️ `normalizeParseAnswer`'s own `stated()` already trims and collapses `''` to `null`, so a
            // nullish coalesce is the whole fold. `statedUnitOf` is NOT reused here: it is the UNIT fold and
            // borrowing it for an amount would put a second meaning on the one function that documents why
            // `undefined` and `''` must stay different answers.
            measure: reading.statedQuantity ?? '',
            foods: reading.foods.map((food) => ({ name: food.name, prep: food.prep })),
            statedUnit: statedUnitOf(reading.statedUnit ?? null),
        },
    };
}
