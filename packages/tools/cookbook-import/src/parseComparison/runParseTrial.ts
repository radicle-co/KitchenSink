...
import { PARSE_MAX_OUTPUT_TOKENS } from '@kitchensink/recipe-core/parsing/parse-prompt';
...
export async function runParseTrial(request: ParseTrialRequest): Promise<ParseTrialRecord> {
    ...
    try {
        outcome = await client.converse({
            ...
            // ⛔ THE ARM'S choice, and absent for every candidate. Caching cannot change what a model
            // ANSWERS, so no accuracy figure here moves either way — but the SHIPPED call sets it
            // (`gatedLlm.ts`), and an arm whose cost column is read as production's must be billed the way
            // production is. Leaving it unset for the candidates keeps every previously recorded figure
            // reproducible: `undefined` is what they were measured with.
            cachePrompt: variant.cachePrompt,
            temperature: 0,
        });
    } catch (error) {
