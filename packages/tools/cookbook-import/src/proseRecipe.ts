...
import type { MeasureSystem, ParsedLine } from '@kitchensink/recipe-import-core';
 ...
 *
 * ## ⛔ AND ONLY WHEN THAT FOOD IS A READING — `name_is_source_line` IS A VETO
 *
 * R5 (2026-09-19) made the zero-food case UNREPRESENTABLE for a non-blank line: where no reader found a
 * food, `compareParses` names the food after the source line itself so the cook's words are never lost,
 * and records `name_is_source_line` to say it is a PLACEHOLDER. The clause above then counted one food and
 * accepted it — so the whole source clause replaced the library parser's name, carrying the merged
 * quantity and unit with it. Measured through the real pipeline: `one pound of butter` became an
 * ingredient literally NAMED `one pound of butter`, with `quantity: absent`, `unit: null`,
 * `needsReview: false`, in a published `imported_public` recipe. The only nets downstream are a length
 * slice and the `NOT_AN_INGREDIENT` set, and neither can see it.
 *
 * ⛔ The veto is that reason and NOTHING more. A food only the CRF named (R1's rescue) is a real reading
 * from a real engine and is still taken; narrowing this to "the LLM found nothing" would decline it.
 *
 * ⚠️ `needsReview` / `reviewReasons` are carried through UNCHANGED from the library parse. They are the
 * segmentation judgement (R39), and the pipeline's own reasons are read here for ONE question only —
 * whether its name is a placeholder — never merged into the line's. The pipeline decides what an accepted
 * line SAYS; it never decides which span is a line, and it may not clear a flag the scan raised.
 *
...
function applyPipelineReading<T extends ParsedIngredientLine & { readonly sourceText: string }>(
    parsed: T,
    reading: ParsedLine | undefined,
): T {
    const food = reading?.foods.length === 1 ? reading.foods[0] : undefined;

    if (reading === undefined || food === undefined || reading.reviewReasons.includes('name_is_source_line')) {
        return parsed;
    }
 ...
 *   refuses a clause whose own reading misstates a value, which is a judgement about the SPAN; the
 *   pipeline decides what an ACCEPTED line says, never which span is a line, and its own `reviewReasons`
 *   are never merged into the stored line's.
 *
 *   ⚠️ A `ParsedLine`, not the narrower `ParsedFacts` it used to be — the value the pipeline hands back
 *   always was one. The widening is not cosmetic: {@link applyPipelineReading} must be able to tell a
 *   READING from R5's placeholder, and `ParsedFacts` cannot express the difference, so under it the safety
 *   decision was unrepresentable rather than merely unwritten.
...
export function toCandidateRecipe(
    block: CookbookBlock,
    book?: Cookbook,
    readings?: ReadonlyMap<string, ParsedLine>,
