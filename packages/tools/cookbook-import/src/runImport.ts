 ...
 *
 *   ⚠️ `ParsedLine`, not the narrower `ParsedFacts` this used to be typed as — `outcome.parsed` always WAS
 *   one, and the narrowing hid the field `applyPipelineReading` needs in order to tell a reading from R5's
 *   placeholder.
...
): Promise<{ readonly data: ParseObservationData; readonly readings: ReadonlyMap<string, ParsedLine> } | undefined> {
