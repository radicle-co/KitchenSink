 ...
 * tier exists to remove. Deciding whether anything is deployed is the CALLER's job — `deployedE2e.yml`
 * asks `deployGate.sh` and skips the job — so by the time these specs run, absence IS a failure.
 */
