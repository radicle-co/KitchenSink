 ...
 *
 * A `use[A-Z]…` module is a HOOK by React's own naming convention — the one the rules-of-hooks lint keys on — so it
 * is excluded even when it is `.tsx` because it returns a render callback.
 *
