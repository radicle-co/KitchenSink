...
export function buildDesignTokens(repoRoot: string = REPO_ROOT): DesignTokenDocument {
    ...
    for (const groupSource of GROUP_SOURCES) {
        ...
        if (groupSource.id === 'native') {
            // ⛔ NULL-PROTOTYPE, because the keys are token names read off the design-system source. A token
            // called `__proto__` would vanish from the generated register instead of being emitted.
            const scalars: Record<string, unknown> = Object.create(null) as Record<string, unknown>;
