...
- **The boundaries ratchet lists `.docusaurus/` files after a local build.** `turbo boundaries` walks gitignored
  output, and `.docusaurus/` is thousands of generated modules. The ratchet excludes every file `git check-ignore`
  matches and lists them as not counted, so they no longer fail the run or reach `--update`. Do **not** declare or
  baseline anything they import. (The ratchet lives in the staff-code-quality agent's toolchain skill, not in this
  repository: `~/.claude/skills/code-quality-toolchain/scripts/boundariesRatchet.mjs`.)
...
- **Vitest warns that `vitest.config.ts` uses ESM syntax in a CommonJS file.** Cosmetic — vitest
  ...
  `packages/infra/global/__tests__/staticAnalysisCoverage.test.ts` resolves each package's config by
  that exact name and would go blind to any other one. (It used to cite `vitestTempRoot.test.ts`,
  which was deleted with the temp-root globalSetup — the constraint outlived the guard that stated it.)
