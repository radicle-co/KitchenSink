/**
 * PER-RUN Clerk fixture identity — the run key, the ONE identity still minted per run, and the cleanup rules.
 *
 * ⛔ SIGN-IN IDENTITIES ARE NO LONGER DERIVED HERE (owner ruling 2026-09-13: "We should have a pool of test users
 * for clerk so that we don't need to create ones"). The web sign-in fixture and the Maestro signer, co-author and
 * erasure subject used to be minted from the run key below, one `createUser` each; every one of them is now a
 * FIXED slot of the test pool (`testPool.ts`), provisioned only by `poolAdmin`. What remains here is the web
 * sign-UP spec's address — registration is the thing that spec tests, so it is the one named carve-out that still
 * creates a user per run — the run key that scopes it (and the seeded world's titles), and the age-gated sweep of
 * sign-up users a crashed run left behind. That sweep's predicates are asserted NEVER to match a pool slot.
 *
 * The history below is why run scoping exists at all, and it is also why the pool is leased by a static slot plus a
 * concurrency group rather than by hashing a run onto a slot.
 *
...
/**
 * Max run-key length. Budget (RFC 5321 caps an email local part at 64 chars): `commise-e2e-signup-` (19) +
 * key (20) + `-` + unique (<=9) + `+clerk_test` (11) = 60, which fits. The key also suffixes the seeded world's
 * recipe titles.
 */
...
function slug(raw: string): string {
    const collapsed = raw.toLowerCase().replace(/[^a-z0-9]+/gu, '-');

    // ⛔ TRIMMED BY INDEX, NOT BY `/^-+|-+$/`. That alternation is quadratic when it does not match — the
    // engine retries every position in a run of separators and fails at the anchor each time
    // (`js/polynomial-redos`) — and this runs over a name a test fixture was handed.
    let start = 0;
 ...
 *
 * `signin` stays in the shape although nothing mints it any more: pre-cutover run-minted sign-in fixtures still sit
 * on the shared instance, and this is what lets the age-gated sweep reclaim them. No pool slot has this shape.
 */
const RUN_SCOPED_EMAIL = /^commise-e2e-(?:signin|signup)-.+\+clerk_test@example\.com$/;

/** True when the address has the run-scoped shape — minted by {@link signUpEmail}, or a pre-cutover sign-in fixture. */
export function isRunScopedE2EEmail(email: string): boolean {
    return RUN_SCOPED_EMAIL.test(email.toLowerCase());
}

/**
 * True when the address is a sign-up user THIS run minted. Delimiter-aware, like the ADR-0005 `pr-{N}` match: the
 * key is followed by `-`, so run key `abc` never claims run key `abcd`'s users.
 */
export function isThisRunE2EEmail(email: string, runKey: string): boolean {
    return email.toLowerCase().startsWith(`commise-e2e-signup-${runKey}-`);
}
...
export interface CleanupPlan {
    /** Sign-up users created by THIS run — deleted unconditionally. */
 ...
 *   - own: a sign-up address carrying THIS run's key → this run created it → always safe to delete.
 ...
 * Anything else — a test-pool slot, a human's account, an unrelated `commise-e2e`-ish name, a concurrent run's
 * FRESH sign-up user — is never returned.
 */
