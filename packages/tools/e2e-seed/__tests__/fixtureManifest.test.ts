...
describe('deriveFixtureManifest', () => {
    ...
    it('signs in as the maestro tier’s fixed pool slots, the same on every run', () => {
        ...
        expect(manifest.signInEmail).toBe(slotFor('maestro', 'signer').email);
        expect(manifest.coAuthorEmail).toBe(slotFor('maestro', 'coauthor').email);
        expect(deriveFixtureManifest('gh99-1-other').signInEmail).toBe(manifest.signInEmail);
    ...
    it('types the password the pool provisions its password-bearing slots with', () => {
        expect(deriveFixtureManifest(RUN).password).toBe(POOL_PASSWORD);
    });
...
describe('manifestToEnvLines', () => {
    it('emits exactly the registry, one KEY=VALUE per line, nothing else', () => {
        const lines = manifestToEnvLines(deriveFixtureManifest(RUN), ERASURE);
    ...
    it('carries the LEASED erasure subject, not a derived one', () => {
        expect(manifestToEnvLines(deriveFixtureManifest(RUN), ERASURE)).toContain(
    ...
    it('refuses an erasure subject that is not a consumable pool slot — the flow really erases it', () => {
        expect(() => manifestToEnvLines(deriveFixtureManifest(RUN), slotFor('maestro', 'signer').email)).toThrow(
            /consumable/u,
        );
        expect(() => manifestToEnvLines(deriveFixtureManifest(RUN), 'someone@example.com')).toThrow(/consumable/u);
    });

    it('never emits an empty value — a blank renders as the literal ${VAR} on screen', () => {
        expect(manifestToEnvLines(deriveFixtureManifest(RUN), ERASURE).every((line) => line.split('=')[1] !== '')).toBe(
            true,
        );
    });
});
