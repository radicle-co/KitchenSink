...
export function readSeedEnvironment(env: Readonly<Record<string, string | undefined>>): SeedEnvironment {
    ...
    return {
        ...
        recipeOrigin: withoutTrailingSlashes(env[REQUIRED.recipeOrigin] ?? ''),
        webOrigin: withoutTrailingSlashes(env[REQUIRED.webOrigin] ?? ''),
    };
}
 ...
 *
 * ⛔ A LOOP, NOT `/\/+$/`. That pattern is LINEAR when it matches and QUADRATIC when it does not: with no
 * match the engine retries every position in the run, matching all the remaining slashes and then failing
 * at `$` each time (`'/'.repeat(n) + 'x'` is the shape). CodeQL files it as `js/polynomial-redos`, and it
 * is right — these values come from the environment a job was handed.
 *
 * @param value - The raw origin.
 * @returns The origin with every trailing `/` removed. Pure.
 */
...
export function readFoodOrigin(env: Readonly<Record<string, string | undefined>>): string {
    const origin = withoutTrailingSlashes((env[FOOD_ORIGIN] ?? '').trim());
