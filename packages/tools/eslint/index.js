...
const PACKAGE_INTERNALS_PATTERN = {
    ...
    group: [
        ...
        // `e2e-fixtures`' fixed Clerk test pool (owner ruling 2026-09-13): `testPool` is the roster every tier
        // leases from and `lease` the Facade that signs a slot in. Separate entry points rather than additions to
        // the package barrel, which is an `export *` barrel this rule's own house style forbids extending.
        '!@kitchensink/*/testPool',
        '!@kitchensink/*/lease',
        // The three PURE entries, whose entire reason for existing is that a consumer takes the rule
        // WITHOUT the runtime the barrel drags in. `observability-scrubbers/denylist` imports nothing at
        // all, so React Native can load it; `observability-scrubbers/core` is the same package's scrubbing
        // ENGINE with no Node built-ins, which is what finally let `@commise/web` and `@commise/mobile`
        // stop carrying their own copies (the barrel imports `node:crypto` for `pseudonymizeId`, and that
        // one import is what had excluded every browser and Hermes bundle); `service-logging/logAttributes`
        // has no `@sentry/nestjs`, so an esbuild-bundled Lambda can take the Error-rendering rule without
        // the Nest SDK. Forcing any of them through its barrel would defeat the split and re-create the
        // copy each one deleted.
        '!@kitchensink/*/denylist',
        '!@kitchensink/*/core',
        '!@kitchensink/*/logAttributes',
        ...
        // `@kitchensink/typescript/fixDeclarationPaths`). There is no "internal"
    ...
    message:
        "Import a package's barrel '@kitchensink/<package>' or one of its declared subpath exports (database/*, database-name, scaling, external-url, food-name, infra, types/*, hooks, testing, users/handle-sync-publisher, denylist, core, logAttributes, or any subpath of the shared tooling packages) — don't reach into other internals.",
};
...
export function createConfig(tsconfigPath = './tsconfig.json', tsconfigRootDir = process.cwd()) {
    return [
        ...
        {
            /**
             * Interpolating a value whose string form is not its meaning. A template literal calls `String()` on
             * whatever it is given, and for an object that is `[object Object]` — which is how a PostgreSQL error
             * reached a deploy log as `master login probe failed: [object Object]`. Both rules need type
             * information, so they apply to TypeScript only (the block above gives plain JS no project).
             */
        ...
        {
            /**
             * A promise whose rejection nobody observes. `no-floating-promises` reports a call whose promise is
             * dropped; `no-misused-promises` reports an async function handed to a slot that expects a void
             * return (an `onClick`, a `forEach` callback), where the rejection is dropped by the CALLER instead;
             * `await-thenable` reports an `await` that waits for nothing. All three need type information, so they
             * apply to TypeScript only.
             *
             * Taken individually rather than as `recommendedTypeChecked`: that preset also enables `no-unsafe-*`
             * and `require-await`, and `require-await` alone measured 503 reports across the tree — nearly all an
             * `async` method satisfying a Promise-returning interface — which would have buried the defects these
             * three find. Every report these three raised when first enabled was fixed, so they land with no
             * baseline.
             */
            files: ['**/*.{ts,tsx,mts,cts}'],
            rules: {
        ...
        {
            /**
             * React's own rules: the Rules of Hooks, exhaustive effect dependencies, and the React Compiler's
             * purity checks — among them `refs` (a ref read or written during render, i.e. state React cannot see),
             * `set-state-in-render`, `set-state-in-effect`, `static-components` (a component type created during
             * render, which remounts its subtree every time) and `globals` (render reassigning an outer binding).
             *
             * Before this block the resolved config for every React package held ZERO `react-hooks` rules, so the
             * repo's "refs are near-forbidden" rule (CLAUDE.md, design-pattern-first §3) was enforced by review
             * alone, and `ed273e20` had to fix render-mutated refs by hand in six components.
             *
             * Applied to `.ts` as well as `.tsx`: a custom hook in a `.ts` file is where a ref-as-state most often
             * hides. The rules key on component and hook NAMES, so non-React packages report nothing.
             *
             * Every rule the preset ships is raised to `error`. The preset leaves three at `warn`
             * (`exhaustive-deps` among them), and every package lints with a bare `eslint .`, so a warning fails
             * nothing and becomes a census nobody gates on.
             */
            files: ['**/*.{ts,tsx,jsx}'],
        ...
        },
        {
            ...
            //
            // This is the AST-level half of a pair: `turbo boundaries` (the code-quality toolchain's ratchet,
            // run only by the local staff-code-quality GATE — NOT in CI) catches a bare specifier that the
            // manifest does not declare, and this catches the other
