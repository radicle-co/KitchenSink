...
/**
 * ⛔ THE GRAMMAR ITSELF, which nothing pinned until this suite's sibling parser was compared against it.
 * `excludedTargetsOf` has had synthetic cases since it was written; `tierOf` was covered only by the real
 * corpus — which happens to be well-formed, so every statement about what the marker REQUIRES was resting
 * on the absence of a counter-example rather than on a test.
 *
 * ⚠️ It matters because this file's delta CHANGED that grammar. The reason capture went from `(.+)` to
 * `(\S.*)` to remove an ambiguity CodeQL flagged, which also made a whitespace-only reason stop matching.
 * That shift was caught incidentally — the corpus check moved it from one failure mode to another — while
 * the contract it implements ("a reason that is entirely whitespace is not a reason") was asserted nowhere.
 */
describe('the @loadTier marker grammar', () => {
    it('reads a declared tier and its reason, in either comment style', () => {
        expect(tierOf('// @loadTier deployed-capable — hits a real origin\nimport http from "k6/http";')).toEqual({
            tier: 'deployed-capable',
            reason: 'hits a real origin',
        });
        expect(tierOf('/**\n * @loadTier substrate-bound — needs a local database\n */')).toEqual({
            tier: 'substrate-bound',
            reason: 'needs a local database',
        });
    });

    it('reads no tier from a script that declares none', () => {
        expect(tierOf('// @loadExcludeTarget prod — a reason\n')).toBeUndefined();
    });

    /**
     * ⛔ The whitespace-only reason is the case this delta changed, and the one the contract is about. An
     * unexplained verdict is a guess that survives review by looking like a decision, so a marker whose
     * reason is only spaces must read as "declares no tier" rather than as a tier with an empty reason.
     */
    it.each([
        ['a whitespace-only reason', '// @loadTier deployed-capable —   \n'],
        ['no reason at all', '// @loadTier deployed-capable\n'],
        ['a hyphen instead of the em dash', '// @loadTier deployed-capable - a reason\n'],
        ['an unknown tier', '// @loadTier occasionally-deployed — a reason\n'],
    ])('⛔ reads a malformed marker (%s) as NO tier, never as a tier with an empty reason', (_label, source) => {
        expect(tierOf(source)).toBeUndefined();
    });
});
