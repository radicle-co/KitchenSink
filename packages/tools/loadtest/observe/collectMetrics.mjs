 ...
 * endpoints, as the pool's admin slot leased by `provisionPool.ts`) and best-effort CloudWatch data points
 ...
 *
 * Env: ADMIN_FILE (admin.json from provisionPool.ts), FOOD_BASE_URL, DURATION_S, INTERVAL_S, OUT_FILE,
...
// The Frontend API the admin slot's session lives on, and the Origin that stamps `azp` — the same two values
// `journey.js` refreshes with, and the ones `provisionPool.ts` signed in against.
...
async function freshAdminToken() {
    ...
    // ⛔ THE FRONTEND API SESSION ENDPOINT, from the handle `provisionPool.ts` wrote. It is the endpoint that is not
    // per-IP throttled, and the only mint whose token carries the `azp` the deployed services require — the old
    // Backend API branch minted azp-less tokens, which answer 401 on every stage running the pattern guard.
    const res = await fetch(
        `${FAPI}/v1/client/sessions/${admin.sessionId}/tokens?__clerk_db_jwt=${encodeURIComponent(admin.devJwt)}`,
        { method: 'POST', headers: { Origin: ORIGIN } },
    );
    const body = res.ok ? await res.json().catch(() => null) : null;

    if (!body?.jwt) {
        // Never the body: a Frontend API answer piggybacks the client, whose sessions carry live JWTs.
        throw new Error(`admin-token re-mint ${res.status} — is admin.json from this stage's provisionPool run?`);
    }
...
async function main() {
    ...
    try {
        ...
        throw new Error(
            `Cannot read observer handle ${ADMIN_FILE} (run 'npm run provision:pool' first): ${err?.message ?? err}`,
        );
    }
