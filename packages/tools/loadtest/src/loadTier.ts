 ...
 */
// ⛔ EVERY RUN IN THIS PATTERN IS FOLLOWED BY A LITERAL OR A DISJOINT CLASS, and `[ \t]` never crosses a
// line. Both halves are load-bearing against `js/polynomial-redos`: under the `m` flag `\s` also matches a
// NEWLINE, so `^\s*\*` beside `\s*` gives two adjacent runs that can each claim the same whitespace; and a
// trailing `(.+)` beside `[ \t]+` is that same ambiguity one token over, since `.` matches a tab too.
// `(\S.*)` pins the capture's first character, which makes the split unique and matches `EXCLUDE_MARKER`
// below. It also says the true thing — a reason that is entirely whitespace is not a reason.
const TIER_MARKER = /^(?:\/\/|[ \t]*\*)[ \t]*@loadTier[ \t]+(deployed-capable|substrate-bound)[ \t]+—[ \t]+(\S.*)$/mu;

/** The environments a deployed k6 run can target — `_ci-heavy.yml`'s `load_test_target` choices. */
export const LOAD_TARGETS = ['sandbox', 'prod'] as const;

/** One of {@link LOAD_TARGETS}. */
export type LoadTarget = (typeof LOAD_TARGETS)[number];

/**
 * A declared exclusion, in either comment style:
 *
 *     // @loadExcludeTarget prod — ADR-0040 contains a test principal's analytics on an enforcing stage
 *
 * For a scenario whose assertion cannot hold against a target BY DESIGN — a fact about the scenario, so it lives in
 * its header rather than in a job's `if:` that a later change lifts. The reason is required, as for `@loadTier`.
 */
const EXCLUDE_MARKER = /^(?:\/\/|\s*\*)\s*@loadExcludeTarget\s+(sandbox|prod)\s+—\s+(\S.*)$/u;

/** Any line that MENTIONS the marker, well-formed or not. */
const EXCLUDE_MENTION = /@loadExcludeTarget\b/u;
...
/** A scenario's declared exclusion from one target. */
...
/**
 * Read a scenario's declared target exclusions out of its source.
 *
 * @param source - The script's full text.
 * @returns Every exclusion it declares; empty when it declares none.
 * @throws {Error} When a line mentions `@loadExcludeTarget` but is not a well-formed declaration — reading it as "no
 *   exclusion" would put the scenario back on the target it says it cannot run against.
 */
export function excludedTargetsOf(source: string): readonly TargetExclusion[] {
    return source
        .split('\n')
        .filter((line) => EXCLUDE_MENTION.test(line))
        .map((line) => {
            const match = EXCLUDE_MARKER.exec(line);

            if (!match?.[1] || !match[2]) {
                throw new Error(
                    `malformed @loadExcludeTarget declaration: '${line.trim()}' — expected ` +
                        `'@loadExcludeTarget <${LOAD_TARGETS.join('|')}> — <reason>'`,
                );
            }

            return { target: match[1] as LoadTarget, reason: match[2].trim() };
        });
}
...
/**
 * The scenarios declaring `tier` — and, when a target is given, not excluding it.
 *
 * @param tier - The tier to select.
 * @param target - The environment the run targets; omitted, no exclusion applies (the question "which scenarios are
 *   in this tier at all", which the wiring guards ask).
 * @returns Their paths, in discovery order.
 * @sideEffect Runs git and reads each scenario.
 */
export function scriptsInTier(tier: LoadTier, target?: LoadTarget): readonly string[] {
    return allLoadScripts().filter((script) => {
        const source = sourceOf(script);

        return (
            tierOf(source)?.tier === tier &&
            (target === undefined || !excludedTargetsOf(source).some((exclusion) => exclusion.target === target))
        );
    });
}
...
/**
 * The package directory a load script belongs to — everything before its `tests/load/` segment.
 *
 * ⛔ `indexOf`, NOT `/\/tests\/load\/.*$/`. The regex scans forward from every position looking for a
 * literal that may not be there, which is quadratic on a long path (`js/polynomial-redos`); a single
 * substring search is linear and says the same thing more plainly.
 *
 * @param script - A repo-relative load-script path.
 * @returns The owning package directory, or the path unchanged when it has no `tests/load/` segment. Pure.
 */
function packageDirOf(script: string): string {
    const at = script.indexOf('/tests/load/');

    return at === -1 ? script : script.slice(0, at);
}
...
export function orderedByDeclaration(scripts: readonly string[]): readonly string[] {
    const packageDirs = [...new Set(scripts.map((script) => packageDirOf(script)))];
...
/** What `printLoadTier.ts` was asked to do: print one tier for one target, or explain its usage. */
export type PrintLoadTierRequest =
    | {
          readonly kind: 'print';
          readonly tier: LoadTier;
...
const PRINTABLE_TIERS: readonly LoadTier[] = ['deployed-capable', 'substrate-bound'];

/**
 * Read `printLoadTier.ts`'s arguments: `<tier> [<path-fragment>] --target <target>`, with `--target` anywhere. Pure.
 *
 * ⛔ `--target` is REQUIRED: a scenario can declare that it cannot run against a target (`@loadExcludeTarget`), and an
 * invocation that could omit it would run that scenario there anyway.
 */
export function parsePrintLoadTierArgs(argv: readonly string[]): PrintLoadTierRequest {
    const targetIndex = argv.indexOf('--target');
    const target = targetIndex === -1 ? undefined : argv[targetIndex + 1];
    const [tier, filter] =
        targetIndex === -1 ? argv : argv.filter((_, index) => index !== targetIndex && index !== targetIndex + 1);
    const isTier = (value: string | undefined): value is LoadTier => PRINTABLE_TIERS.some((known) => known === value);
    const isTarget = (value: string | undefined): value is LoadTarget => LOAD_TARGETS.some((known) => known === value);

    if (!isTier(tier) || !isTarget(target)) {
        return {
            kind: 'usage',
            message: `usage: printLoadTier.ts <${PRINTABLE_TIERS.join('|')}> [<path-fragment>] --target <${LOAD_TARGETS.join('|')}>`,
        };
    }

    return { kind: 'print', tier, filter, target };
}
