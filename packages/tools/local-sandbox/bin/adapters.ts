 ...
 * the same split `.github/scripts/deployGate.sh` uses (pure `decide`, impure `evaluate`).
 */
...
export async function runCdkSynth(request: SynthRequest): Promise<SynthResult> {
    ...
    // ⛔ RUN IN THE PACKAGE'S DIRECTORY, NEVER `--workspace=<name>`. Not one CDK app is a declared npm
    // workspace: the root globs are `packages/services/*` (which matches `identity`, NOT `identity/infra`),
    // and nothing covers `packages/infra/*` or `packages/apps/commise/web/infra` at all. So every synth
    // exited with `npm error No workspaces found: --workspace=@kitchensink/identity-infra`, and because that
    // hit EVERY app, `local:up` and `local:audit` could not stand up anything — local parity with the
    // sandbox was impossible rather than merely incomplete.
    //
    // ⚠️ THE FIX IS NOT TO ADD THE GLOBS. Declaring eight new workspaces changes install topology, hoisting,
    // and every tool that enumerates workspaces (turbo, knip, sherif, the repo guards) — a large blast
    // radius to make one tool's shell-out resolve. `discoverApps` already carries `packageDir`, documented
    // as "the directory the synth command must run in", so running there is both smaller and the shape the
    // data was designed for.
    const args = ['run', app.script, '--', '--output', outDir];
