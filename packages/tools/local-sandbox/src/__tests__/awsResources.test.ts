...
describe('localExportMap', () => {
    it('maps an export to the Ref value of the resource it names', () => {
        ...
        // ⚠️ `toEqual`, not `toStrictEqual`, and the difference is asserted rather than lost: the map is
        // built on a NULL PROTOTYPE because its keys are export names read out of a template, and
        // `toStrictEqual` compares prototypes, so it would fail on the property the next assertion exists
        // to require.
        expect(localExportMap([{ stack: 'S', template }], LOCALSTACK)).toEqual({
            'kitchensink-data-dev:MediaBucketName': 'media',
        });
    });

    /**
     * ⛔ THE KEYS ARE DATA — export names read out of a synthesized template — so the accumulator must not
     * expose `Object.prototype`'s inherited `__proto__` setter. On an object literal an export named
     * `__proto__` is silently DROPPED rather than mapped, which here would mean a local stand-in resolving
     * to nothing with no error. Asserted because the previous `toStrictEqual` made the prototype an
     * accident of the comparison rather than a property of the function.
     */
    it('⛔ builds the map with a null prototype, so an export named `__proto__` is data', () => {
        const template = {
            Resources: { MediaBucket: { Type: 'AWS::S3::Bucket', Properties: { BucketName: 'media' } } },
            Outputs: { Out: { Value: { Ref: 'MediaBucket' }, Export: { Name: '__proto__' } } },
        };

        const map = localExportMap([{ stack: 'S', template }], LOCALSTACK);
    ...
    it('skips an export whose value references something not created locally', () => {
        ...
        // ⚠️ KEYS, not `toEqual({})`, and the difference is the point. `toStrictEqual` used to fail here on
        // an `undefined`-VALUED key; `toEqual` ignores one. So a regression from SKIPPING an export to
        // writing `map[name] = undefined` would have passed unnoticed — asserting the key count keeps what
        // the stricter matcher proved, now that the prototype makes `toStrictEqual` unusable.
        expect(Object.keys(localExportMap([{ stack: 'S', template }], LOCALSTACK))).toHaveLength(0);
    });
});
