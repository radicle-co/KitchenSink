...
describe('partitionForLocalSynth', () => {
    ...
    it('⛔ the WORKING TREE declares at most the parser — a skip must be argued, not accumulated', () => {
        ...
        // ⚠️ The declaration lives on the INFRA package now, beside the `synth` script it describes.
        // CDK left the npm workspace, so the script moved out of the service manifest and the skip
        // had to move with it — a reason stranded on a manifest that no longer synthesises anything
        // is a skip nothing reads.
        expect(declared).toEqual(['@kitchensink/ingredient-parser-infra']);
    });
});
