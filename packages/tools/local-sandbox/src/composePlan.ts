 ...
 *
 * ⛔ REPLACES TWO HAND-MAINTAINED FILES THAT HAD ALREADY DRIFTED. `compose.yml` ran
 * `localstack/localstack:3` with `SERVICES: s3`; `infra/localstack/compose.yml` ran `4.4.0` with
...
const LOCAL_DB = Object.freeze({ user: 'postgres', password: 'postgres', bootstrapDatabase: 'postgres' });
