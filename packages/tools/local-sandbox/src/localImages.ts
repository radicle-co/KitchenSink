...
export function localContainerEnv(keys: readonly string[], context: LocalEnvContext): Readonly<Record<string, string>> {
    const env: Record<string, string> = {
        ...
        // (`@kitchensink/rds-iam-auth`). Identity used to append `?sslmode=no-verify` to its discrete form
        // unconditionally, so `STAGE` alone left it answering `503 NOT_READY: Database not reachable` while the
        // other two were ready (`server does not support SSL, but SSL was required`); a URL with no `sslmode`
        // is what every service can use here.
