...
export function localContainerEnv(keys: readonly string[], context: LocalEnvContext): Readonly<Record<string, string>> {
    // ⛔ NULL PROTOTYPE, because the loop below both WRITES and probes this bag by a name that comes from the
    // data — a task definition's declared variable names and the SSM keys in `context.resolved`. On an
    // ordinary literal a variable named `__proto__` assigns through `Object.prototype`'s inherited setter and
    // vanishes, and `key in env` answers TRUE for `constructor`/`toString`/`valueOf`, so a service declaring
    // one of those would be skipped and shipped with no value at all. Both are silent; the second is the one
    // that would surface as a container that boots and misbehaves. Same class as `scripts/
    // prepareProdManifest.mjs`, guarded by `packages/infra/global/__tests__/prototypePollutionSinks.test.ts`.
    const env: Record<string, string> = Object.assign(Object.create(null) as Record<string, string>, {
        ...LOCAL_INFRA,
        PORT: String(context.port),
        // ⛔ NO `sslmode`, ON PURPOSE. The postgres container speaks no TLS, and every service prefers this
        // variable over the discrete `DB_*` set — whose deployed form is RDS IAM over TLS
        // (`@kitchensink/rds-iam-auth`). A discrete form that requires SSL answers `503 NOT_READY: Database not
        // reachable` against this container (`server does not support SSL, but SSL was required`); a URL with
        // no `sslmode` is what every service can use here.
