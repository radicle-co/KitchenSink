 ...
 *
 * Integration suites recreate schemas, drop tables and delete rows — that is what the tier is for. But
 * `DATABASE_URL` is the APPLICATION's variable: `.env.development` sets it to the local sandbox's live recipe
 * database, and a tunnel sets it to a shared RDS. A suite reading it, in a shell that had sourced either,
 * would drop `verification_spend` — the live monthly spend ledger — and nothing in the suites could tell.
 *
 * So the harness reads its own variable, `DATABASE_ADMIN_URL`, pointing at the SERVER's maintenance
 * database, and admits it only when the host is loopback. The other half of the rule — the database name
 * must end in `_test` — is a precondition on every database the fixture provisions (`roleDatabase`),
 * because the admin URL names the maintenance database and the disposable names are the ones the fixture
 * creates.
 *
...
/**
 * Whether an admin server is configured at all — the flag every integration suite skips on (directly, or
 * re-exported by a package's test support as `hasTestDatabase`), so a machine with no PostgreSQL skips rather
 * than fails.
 *
