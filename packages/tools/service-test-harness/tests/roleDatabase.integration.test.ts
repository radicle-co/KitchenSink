 ...
 *
 * A superuser satisfies every grant, so under a tier connected as `postgres` a missing privilege, an object
 * owned by the wrong role, or a runner that forgot `SET ROLE` passes the whole tier and fails only in a
 * deployed stage — the defect class the role split (ADR-0039) exists to surface.
