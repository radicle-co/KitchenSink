 ...
 * exactly the field `teardownSandboxPr.sh` already reads for `Environment`. One knowledge, two modules;
 ...
 * for an app nobody read — the vacuity failure `verifyDeployment.sh` guards against in three places.
 *
    ...
    lines.push(
        hasDriftFindings(stacks)
            ? 'Deploy the affected stage, or record why it is deliberately behind. A stack whose commit is ' +
                  'unknowable is how `docs/architecture/2026-08-28/ingredientPipelineState.md` came to ' +
...
function main() {
    ...
    if (summary !== undefined && summary !== '') {
        // ⛔ APPENDED DIRECTLY, NOT THROUGH `bash -c`. The old form spawned a shell to run `cat >> "$1"`
        // with the path as a positional argument — correctly quoted, so not exploitable, but it hands an
        // environment-supplied string to a shell for no reason and CodeQL files it as
        // `js/indirect-command-line-injection`. `appendFileSync` does the same job with no shell, no
        // subprocess and no quoting to get right.
