 ...
 *
 * `docs/architecture/2026-08-28/ingredientPipelineState.md` §1 was a hand-maintained table headed "What
 ...
 *
 * `crossAppImports.tsv` projects the one thing CloudFormation cannot order for itself: a `Fn.importValue`
 ...
 * `kitchensink-service-logs-prod` did not exist while two apps imported from it. `.github/scripts/deployGate.sh`
 ...
 * `.github/scripts/deployGate.sh` reads to close the deploy graph.
 */
 ...
 *
 * ⚠️ Not a convenience. `.github/scripts/deployGate.sh` runs before `npm ci` has restored `typescript`, and
 ...
 * stack.
 */
export const MANIFEST_DEPLOY_EDGES = `${MANIFEST_DIR}/crossAppImports.tsv`;
 ...
 *
 * The same predicate `packages/infra/global/__tests__/cdkApps.ts` uses — a tracked file under a `bin/`
 * directory that constructs a CDK `App`, whatever it is named. Deliberately not imported from there: that module is a test helper in a workspace,
...
export function discoverCdkApps() {
    // ⚠️ ANY tracked `bin/*.ts` that constructs an `App`. Filtering on the NAME drops
    // `packages/infra/global/bin/account.ts` silently, and the symptom is not an error — the account's cost
    // guardrails simply DISAPPEAR from the manifest, as though the stack had been deleted. A missing app
    // reads exactly like a removed one.
...
export function renderDeployEdges(manifest) {
    const lines = [
        ...
        '# (ADR-0022 §5). `.github/scripts/deployGate.sh close` reads this to force a producer leg whose',
...
export function renderManifestMarkdown(manifest) {
    ...
    if (edges.length > 0 || unresolved.length > 0) {
        lines.push(
            ...
            '`.github/scripts/deployGate.sh close` reads to force a producer leg.',
            '',
