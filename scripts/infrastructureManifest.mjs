...
export function discoverCdkApps() {
    // ⚠️ ANY tracked `bin/*.ts` that constructs an `App`. Filtering on the NAME dropped
    // `packages/infra/global/bin/account.ts` silently, and the symptom was not an error — the account's cost
    // guardrails simply DISAPPEARED from the manifest, as though the stack had been deleted. A missing app
