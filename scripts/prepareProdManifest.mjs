 ...
 * repo's other cross-cutting scripts (`contractGenerate.mjs`, `contractDriftGate.mjs`, `prScope.sh`).
 *
...
export function rewriteExports(exports) {
    ...
    // ⛔ `Object.fromEntries`, NOT `rewritten[key] = …` on an object literal. The keys come from a parsed
    // `package.json`, so a subpath named `__proto__` reached `Object.prototype`'s inherited setter and was
    // silently DROPPED from the production manifest — a rewriter that deletes an export rather than
    // rewriting it, and one whose only symptom is an import failing at runtime in the deployed image.
    // ⚠️ `Object.create(null)` is NOT the equivalent fix: it still assigns with `[[Set]]`, and CodeQL's
    // `js/remote-property-injection` does not accept it as a sanitizer. `fromEntries` uses
    // `CreateDataProperty` and never `[[Set]]`. `Object.assign` is the same sink and is not an alternative.
