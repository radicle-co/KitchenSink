...
// (AwsSolutions-L1) and `__tests__/lambdaRuntime.test.ts`, which keeps this pin from falling behind.
//
// THE Node runtime every Lambda in this repository runs on. One value, one place.
//
// ## Why this is a security concern, and therefore lives in @radicle-co/infra-shared/security
//
...
// Node runtime therefore fails `lambdaRuntime.test.ts` instead of silently changing production, which turns
 ...
 * `__tests__/lambdaRuntime.test.ts`.
 */
