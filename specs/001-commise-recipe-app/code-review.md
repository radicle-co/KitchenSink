...
- **Throttle store is in-memory** — the per-user _key_ is correct, but each Fargate task holds its own counter, so the effective per-user cap is ×(task count). Fine as abuse-dampening; if throttling must be a hard security control, move to a shared store (Redis/ElastiCache). _(Pre-existing infra choice; carried to release-readiness.)_
