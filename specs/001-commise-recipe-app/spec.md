...
| [002-user-auth](../002-user-auth/spec.md)       | Provides authentication required by FR-045; owns the user profile/preferences where Home layout persists as JSON (`profiles.preferences.homeLayout`, via the existing `/v1/profiles/me`; consumed by US-0 / FR-046); provides the app-ULID→Clerk-`external_id` sync (**shipped**); 001 (T000-prereq) emits that ULID as a session-token claim + extends `@kitchensink/clerk-verify` to surface it as `userId` — the prerequisite for recipe `owner_id` |
