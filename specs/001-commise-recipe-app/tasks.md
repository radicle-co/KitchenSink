...
- [x] T051 [US2] Add integration test for clone visibility + attribution + substantive-edit unlock rules in `packages/services/recipe-service/__tests__/integration/recipes/clone-visibility.integration.test.ts`
- [x] T103 [US2] Add integration test for collection cloning (public collection clone excludes private recipes) in `packages/services/recipe-service/__tests__/integration/collections/clone-collection.integration.test.ts`
...
- [x] T128 [US2] Add `CloneCollectionRequest` DTO + controller endpoints `POST /api/v1/collections/{id}/clone` and `POST /api/v1/collections/{id}/pull-from-source` in `packages/services/recipe-service/src/collections/{dto/clone-collection.dto.ts,collections.controller.ts}`
- [x] T129 [US2] Update existing T103 collection-clone integration test to assert `source_collection_id` and `added_via=clone_seed` are set, and add a follow-up `pull-from-source` integration test in `packages/services/recipe-service/__tests__/integration/collections/pull-from-source.integration.test.ts`
...
- [x] T156 Add an integration test for the aggregate trigger against real Postgres: two concurrent raters of the same recipe both count (the `FOR UPDATE` lost-update guard); removing a rating re-derives the average; the last rating removed yields `count=0` + `average=NULL` (never `0`); deleting a rated recipe cascades and the trigger is a silent no-op; a bulk `DELETE … WHERE user_id = :u` fires the trigger once (statement-level) — in `packages/services/recipe-service/__tests__/integration/ratings/aggregate-trigger.integration.test.ts`
...
> `storage-capacity.test.ts` in all three services, so T189 is "copy the existing pattern and complete the
...
- [x] T188 Convert recipe-service's **ONE remaining `class-validator` importer** to the single `nestjs-zod` mechanism — `packages/services/recipe-service/src/search/dto/search-recipes.query.dto.ts`
    ...
    - ⚠️ **CORRECTION to the "19 files" figure this task previously carried — it was a MENTION count, not an importer count.** Re-measured 2026-08-12 with `grep -rn "class-validator|class-transformer"` restricted to **import statements**: exactly **ONE** file imports either package — `src/search/dto/search-recipes.query.dto.ts`, which imports `Transform, Type` from `class-transformer` and `IsArray, IsIn, IsInt, IsOptional, IsString, Max, Min` from `class-validator`. The other 18 files mention the string once or twice in **JSDoc about having migrated away from it**. So this is a **one-file** task, not a nineteen-file project — and the difference is exactly the kind of figure that decides whether the work gets scheduled. **Count importers, not mentions.** The same overstatement appears in ADR-0015, `docs/CODING_STANDARDS.md` §15.4 and earlier revisions of GR-015/GR-016; all four now carry the correction.
    ...
    - **Tests**: unit (per-converted-DTO accept/reject, unknown-key rejection, and the retained numeric bounds) **AND** integration (a known-bad body posted to **each** converted route on a booted app returns `400` naming the field, modelled on `packages/services/identity/tests/app-validation.test.ts`) **AND** e2e (`packages/services/recipe-service/tests/e2e/` — existing suites must stay green, proving the error contract did not change shape) **AND** k6 (`packages/tools/loadtest/` — T081's p95 ≤ 500ms budget must hold after the pipe change).

- [ ] T189 Complete recipe-service's storage-capacity mapping using the EXISTING gate — `packages/services/recipe-service/src/database/__tests__/storage-capacity.test.ts`
    - **Depends on**: T188
    - ⛔ **DO NOT BUILD A NEW GATE — the mechanism already exists and this task is to finish USING it.** `@kitchensink/contract-gen` exports `auditStorageCapacity` / `collectBoundedColumns` / `formatStorageCapacityFindings` (`packages/tools/contract-gen/src/storage-capacity.ts`), and a `storage-capacity.test.ts` already wires it in **all three** services — recipe at `src/database/__tests__/`, food at `src/db/schema/__tests__/`, identity at `src/types/schema/__tests__/`. A previous revision of this task said "no bidirectional completeness assertion exists anywhere in the repo"; that was **wrong** — `auditStorageCapacity` is already exhaustive over columns and already emits `stale-account` and `duplicate-account` findings, which IS the bidirectional check. The remaining work is the **mapping**: every bounded column bound to the wire fields that write it, or declared not-client-writable **with a reason**.
