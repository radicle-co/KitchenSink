...
> mapping", not "invent the gate"), and recipe-service's **missing** contract test tier (T190).
...
> `packages/schemas/recipe/openapi.yaml` is **4,945 lines covering 34 paths**, against the hand-written
> `specs/001-commise-recipe-app/contracts/api.openapi.yaml`'s **2,827 lines / 32 paths** (measured 2026-08-12).
...
- [ ] T186 Repoint every source citation of the hand-written recipe contract at the derived document — `packages/services/recipe-service/src/`, `packages/clients/recipe-service/src/`, `packages/shared/recipe-core/src/`, `packages/apps/commise/web/tests/e2e/utils/recipeApi.ts`
    - **Verified inventory (2026-08-12)**: counted over **`git ls-files`**, `api.openapi.yaml` is cited by **12 files under `packages/`** (11 `.ts` sources plus one k6 `.js`), 26 under `specs/` and 5 under `docs/`. ⚠️ **A worktree glob reports 31 under `packages/` — the extra 19 are `dist/` and `.next/` build output that disappears on the next build and must NOT be hand-edited.** Counting build output as work to do is how this task gets estimated wrong, and it is the same error that produced the "57 source files" figure in ADR-0014 and in this feature's own plan. **Count with `git ls-files | xargs grep -l`.**
    - The 11 `.ts` sources: `packages/clients/recipe-service/src/errors.ts`; `packages/services/recipe-service/src/{collections/collections.service.ts, common/filters/api-exception.filter.ts, recipes/dto/recipe-response.dto.ts}`; `packages/services/recipe-service/src/account/__tests__/{account.controller.test.ts, export.service.test.ts}`; `packages/services/recipe-service/src/account/domain/__tests__/erasure-status.test.ts`; `packages/services/recipe-service/__tests__/integration/account/erasure.integration.test.ts`; `packages/shared/recipe-core/src/{accountErasure.ts, recipe.types.ts}`; `packages/apps/commise/web/tests/e2e/utils/recipeApi.ts`.
...
- [ ] T187 Mark the hand-written contract superseded and correct its stale status header — `specs/001-commise-recipe-app/contracts/api.openapi.yaml`
    - **Depends on**: T186
    - **⚠️ The file's own header is now WRONG and actively misleading.** Its `STATUS (2026-08-11)` block states "the replacement has **NOT** been generated yet — recipe's `openapi.yaml` does not exist, and these citations have NOT been repointed." The first half is **false as of 2026-08-12** (`packages/schemas/recipe/openapi.yaml`, 4,945 lines, 34 paths); only the second half was true, and T186 closes it. A header that under-claims the replacement's existence is what keeps contributors extending the wrong document.
    - **Acceptance**: The header records that the derived document **exists and is normative**, names it, drops the stale "not generated yet" claim, and keeps the two standing rules: **where this file and the service's `*.schema.ts` disagree, the service's zod wins**, and **do not extend this file with a new endpoint** — author the zod in the service instead. The file is **retained, not deleted**, as the historical record until the ~24 remaining citations under `specs/` and 5 under `docs/` are repointed by their own owners.
...
- [ ] T188 Convert recipe-service's **ONE remaining `class-validator` importer** to the single `nestjs-zod` mechanism — `packages/services/recipe-service/src/search/dto/search-recipes.query.dto.ts`
    - **⛔ Two validation mechanisms in one service is a GR-016 §16-a.2 violation**, and recipe-service is still in that state: **18 `ZodValidationPipe` / 26 `createZodDto` sites alongside one file that still imports `class-validator`**. Two mechanisms means two error contracts, two sets of edge cases, and a mechanism-per-route that is a **per-file accident** rather than a decision.
...
- [ ] T190 Add the missing contract test tier for recipe-service — `packages/services/recipe-service/contract/__tests__/contract.test.ts`
    - **⚠️ Verified gap (GR-017 enforcement table, 2026-08-12)**: `packages/services/{identity,food-service}/contract/__tests__/contract.test.ts` both exist; **`recipe-service` has no such tier** — and it is the service with the largest contract surface (34 paths, 4,945 derived OpenAPI lines).
    - **Acceptance**: A contract tier matching the two existing ones in shape, with its own `vitest.*.config.ts`, its own `package.json` script, an exclude keeping it out of the default `test` glob, **and a CI step** — CI invokes these per-workspace by name in `.github/workflows/_ci.yml`, so a new script that no job calls is a test that never runs (§7 "Integration and E2E MUST NOT run in the default `test` task").
    - **⚠️ Name it `*.test.ts`, never bare `*.spec.ts`** — that suffix is reserved for Playwright, whose default `testMatch` would otherwise collect vitest files and crash the run on their `vitest` imports (§7).
    - **Tests**: this task **is** a test tier; it must be able to fail — assert it goes red against a deliberately skewed schema package rather than only green against a correct one.
