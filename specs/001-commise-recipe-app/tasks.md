...
| ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Non-UI code — services, DALs, domain evaluators, **controllers**, workers, type guards, config/env loaders, DTO validation, libraries | **Unit** (mocks + fixtures) **AND Integration** (real PostgreSQL + LocalStack)                                                                                                                                | `src/**/__tests__/*.test.ts` + `__tests__/integration/**/*.integration.test.ts` |
...
- [x] T006a [P] Create `infra/docker/postgresInit.sql` with `CREATE EXTENSION IF NOT EXISTS pg_trgm;` and `CREATE EXTENSION IF NOT EXISTS pgcrypto;` — referenced by `compose.yml` init volume mount
- [x] T006b [P] Create local dev `compose.yml` at monorepo root (PostgreSQL 16 + LocalStack S3) per quickstart.md specification — distinct from `compose.test.yml` (T088) which is CI-specific
...
- [x] T086 [P] Configure Playwright project with `globalSetup.ts` (run migrations + seed, start API server) in `packages/apps/commise/web/playwright.config.ts` and `packages/apps/commise/web/tests/e2e/globalSetup.ts`
...
- [x] T088 [P] Create `compose.test.yml` for CI test infrastructure (PostgreSQL 16 + LocalStack S3 with bucket auto-provisioning) at monorepo root
- [x] T089 [P] Implement test `globalSetup.ts` for integration tests: start LocalStack, provision S3 buckets, run Drizzle migrations, seed test data in `packages/services/recipe-service/tests/globalSetup.ts`
...
- [x] T017-test Write unit tests for environment loader (valid env, missing required vars, SSM fallback) in `packages/services/recipe-service/src/config/__tests__/loadConfig.test.ts`
...
- [x] T020-test Write unit tests for API exception filter (RecipeError mapping, unknown error fallback, HTTP status codes) in `packages/services/recipe-service/src/common/filters/__tests__/apiException.filter.test.ts`
...
- [x] T017 Implement environment loader with Zod validation and optional SSM fallback in `packages/services/recipe-service/src/config/loadConfig.ts`
...
- [x] T020 Implement shared API exception filter and recipe-domain error mapping in `packages/services/recipe-service/src/common/filters/apiException.filter.ts`
...
- [x] T023 [P] [US1] Define create/update/list recipe DTOs in `packages/services/recipe-service/src/recipes/dto/{createRecipe.dto.ts,updateRecipe.dto.ts,listRecipes.query.dto.ts}`
...
- [x] T026 [US1] Implement recipes controller endpoints for `/api/v1/recipes` and `/api/v1/recipes/{id}` in `packages/services/recipe-service/src/recipes/recipes.controller.ts`
...
- [x] T029 [US1] Implement ingredients controller endpoints for `/api/v1/ingredients/search` and `/api/v1/ingredients` in `packages/services/recipe-service/src/ingredients/ingredients.controller.ts`
...
- [x] T045 [US1] Add integration test for optimistic conflict detection returning HTTP 409 with version metadata in `packages/services/recipe-service/__tests__/integration/recipes/conflict.integration.test.ts`
- [x] T098 [US1] Add integration test for recipe CRUD lifecycle (create → get → update → list → delete) against real PostgreSQL in `packages/services/recipe-service/__tests__/integration/recipes/crud.integration.test.ts`
- [x] T099 [US1] Add integration test for ingredient search (pg_trgm fuzzy + FTS exact) against real PostgreSQL in `packages/services/recipe-service/__tests__/integration/ingredients/search.integration.test.ts`
- [x] T100 [US1] Add integration test for photo upload flow (presigned URL → S3 upload via LocalStack → confirm validates magic bytes + S3 HEAD size ≤ 5 MB → `recipe_photos` row with single `s3_key`; served as-is, no processing) in `packages/services/recipe-service/__tests__/integration/photos/upload.integration.test.ts`
- [x] T101 [US1] Add integration test for collections CRUD + membership (add/remove recipes, no-cascade delete) in `packages/services/recipe-service/__tests__/integration/collections/crud.integration.test.ts`
- [x] T102 [US1] Add integration test for search endpoint (FTS + facets + pagination) in `packages/services/recipe-service/__tests__/integration/search/search.integration.test.ts`
...
- [x] T048-test Write unit tests for visibility policy evaluator (all C-004 scenarios: user-created, imported-public, imported-physical, paid-source, tier transitions, substantive edit unlock) in `packages/services/recipe-service/src/recipes/domain/__tests__/visibilityPolicy.test.ts`
...
- [x] T049-test Write unit tests for substantive edit detection (ingredient change = substantive, title change = not substantive) in `packages/services/recipe-service/src/recipes/__tests__/substantiveEdit.service.test.ts`
- [x] T139-test Write unit tests for substantive-edit detection on **imported** recipes (FR-005 + C-004): editing ingredients/steps on an imported recipe MUST set `hasSubstantiveEdit=true` and unlock private-visibility transition for premium users per C-004; editing only metadata (title, description, tags, photos) MUST NOT in `packages/services/recipe-service/src/recipes/__tests__/substantiveEditImported.test.ts`

### Implementation (TDD Green)

- [x] T046 [P] [US2] Add clone and visibility DTOs in `packages/services/recipe-service/src/recipes/dto/{cloneRecipe.dto.ts,setVisibility.dto.ts}`
...
- [x] T048 [US2] Implement C-004 visibility policy evaluator for source type, tier, and substantive edit state in `packages/services/recipe-service/src/recipes/domain/visibilityPolicy.ts`
...
- [x] T050 [US2] Implement `/api/v1/recipes/{id}/clone` and `/api/v1/recipes/{id}/visibility` endpoints in `packages/services/recipe-service/src/recipes/recipes.controller.ts`

### Integration Tests

- [x] T051 [US2] Add integration test for clone visibility + attribution + substantive-edit unlock rules in `packages/services/recipe-service/__tests__/integration/recipes/cloneVisibility.integration.test.ts`
- [x] T103 [US2] Add integration test for collection cloning (public collection clone excludes private recipes) in `packages/services/recipe-service/__tests__/integration/collections/cloneCollection.integration.test.ts`
...
- [x] T126 [US1] Add integration test asserting `DELETE /api/v1/recipes/{id}` returns 204, row remains with `deleted_at` set, and recipe is excluded from list/search/get/collection responses in `packages/services/recipe-service/__tests__/integration/recipes/softDelete.integration.test.ts`. Covers the previously-unpinned C-007 retention guarantee (the row SURVIVES — a hard `DELETE FROM recipes` passed the whole suite before this) plus owner-list exclusion and idempotent re-delete. The search / collection / GET-404 clauses were already owned by `search/search.integration.test.ts`, `collections/crud.integration.test.ts`, and `recipes/crud.integration.test.ts` respectively, so they are referenced rather than duplicated.

### Collection Clone & Pull-from-Source (FR-011)

- [x] T127-test Write unit tests for collection clone service (creates new collection with `source_collection_id`, copies memberships with `added_via=clone_seed`, owner reassignment) in `packages/services/recipe-service/src/collections/__tests__/cloneCollection.service.test.ts`
- [x] T128-test Write unit tests for pull-from-source service (additive only, `added_via=pull`, no-op when no new recipes, 400 when collection has no source) in `packages/services/recipe-service/src/collections/__tests__/pullFromSource.service.test.ts`
...
- [x] T128 [US2] Add `CloneCollectionRequest` DTO + controller endpoints `POST /api/v1/collections/{id}/clone` and `POST /api/v1/collections/{id}/pull-from-source` in `packages/services/recipe-service/src/collections/{dto/cloneCollection.dto.ts,collections.controller.ts}`
- [x] T129 [US2] Update existing T103 collection-clone integration test to assert `source_collection_id` and `added_via=clone_seed` are set, and add a follow-up `pull-from-source` integration test in `packages/services/recipe-service/__tests__/integration/collections/pullFromSource.integration.test.ts`

### Async Version Archive Worker (FR-007b-i)

- [x] T130-test Write unit tests for pending-archive enqueue — insert one idempotent row (`UNIQUE(recipe_version_id)` + `ON CONFLICT DO NOTHING`) when a version goes over-retention — in `packages/services/recipe-service/src/versions/dal/__tests__/pendingArchives.dal.test.ts`. Retries are NOT tracked in-DB: the shipped design re-drives a failed archive via SQS redelivery and, on exhaustion, the DLQ (worker handler tested in recipe-workers `versionArchiveWorker.test.ts`: success → DELETE outbox row, failure → throw → SQS redelivery → DLQ). No `attempts`/`last_error` DAL mutation path — the outbox row is the source of truth, the SQS message derived
...
- [x] T133 Add integration test for the full async archive path (enqueue → worker drains → S3 object exists, pending row gone) using LocalStack SQS + S3 in `packages/services/recipe-workers/__tests__/integration/archive.integration.test.ts`
...
- [x] T135 Implement `POST /api/v1/account/erasure` controller in `packages/services/recipe-service/src/account/account.controller.ts`
...
- [x] T057 Verify integration tests include requirement traceability comments and avoid prohibited test patterns in `packages/services/*/tests/**/*.integration.test.ts` (Principle IV)
- [x] T058 Verify workspace governance entries and task pipelines remain correct in `package.json` and `turbo.json` (Principle V)
- [x] T059 Run and validate `turbo run typecheck lint format:check test` from `<repo root>` with all exit codes 0 (Principle VI)
- [x] T060 Verify platform parity (FR-044 / FR-044a) across all Phase 5 frontend tasks: (a) confirm every implementation task covers both web and mobile explicitly, has a paired task, or carries a `[PARITY-EXCEPTION]` note; (b) confirm the Home widget surface (T104-web + T104-mobile) is present and tested on both platforms, with only the recipe widget live and gated widgets absent; (c) confirm no user-facing screen exists on one platform without a corresponding screen on the other, and every **live** widget ships on both platforms behind one id (`.native.tsx` split); (d) confirm the Home widget TS contract lives in `@commise/features-core` (the `HomeWidgetId` / `HomeWidgetDescriptor` / `curateHomeWidgets` types — Commise home-surface infra, not the platform `@kitchensink/recipe-core`), and that per-user layout persistence uses `PATCH /api/v1/users/me` which is **owned by the identity service (002) and consumed here** (there is deliberately **no** `/api/v1/profiles/me` route — clients historically drifted to it and two guard tests now assert against it) — it is deliberately **NOT** required in 001's `contracts/api.openapi.yaml` (Principle VII)
...
- [x] T152-int Add an integration test asserting `POST /api/v1/recipes` with and without difficulty, and `PATCH` clearing it via explicit `null`, round-trip correctly against real Postgres in `packages/services/recipe-service/__tests__/integration/recipes/difficulty.integration.test.ts`
...
- [x] T155 Implement the `RatingsModule` — `PUT`/`DELETE /api/v1/recipes/{id}/rating` controller + `RatingsService` (upsert `ON CONFLICT (recipe_id, user_id) DO UPDATE`; delete; visibility-derived authz returning 404-not-403 for unreadable, 403 only for own-recipe) + `SetRecipeRatingInput` DTO in `packages/services/recipe-service/src/ratings/{ratings.module.ts,ratings.controller.ts,ratings.service.ts,dto/setRating.dto.ts}`. Returns the updated `RecipeDetail` with the trigger-refreshed aggregate
- [x] T156 Add an integration test for the aggregate trigger against real Postgres: two concurrent raters of the same recipe both count (the `FOR UPDATE` lost-update guard); removing a rating re-derives the average; the last rating removed yields `count=0` + `average=NULL` (never `0`); deleting a rated recipe cascades and the trigger is a silent no-op; a bulk `DELETE … WHERE user_id = :u` fires the trigger once (statement-level) — in `packages/services/recipe-service/__tests__/integration/ratings/aggregateTrigger.integration.test.ts`
- [x] T157 Add service e2e + k6 for the rating endpoints: drive `PUT`/`DELETE /api/v1/recipes/{id}/rating` over HTTP against the booted Nest app (own-recipe 403, unreadable 404, idempotent replace, aggregate visible on the returned detail) in `packages/services/recipe-service/tests/e2e/ratings.e2e.ts`, and a k6 script exercising the read-heavy list + rating write mix under the SC-009 SLO in `packages/services/recipe-service/tests/load/ratings.load.ts`
...
- [x] T162-test Write the integration test for cover-photo resolution: the list projection carries `coverPhotoUrl` = the photo with the lowest `sort_order` (deterministic tiebreak `created_at`, then `id`), the field is **absent** (not null, not a placeholder URL) for a photoless recipe, and a page of N recipes resolves in a single query (no N+1) in `packages/services/recipe-service/__tests__/integration/recipes/cover-photo.integration.test.ts`
...
- [x] T169 Extend the erasure worker to `DELETE FROM recipe_ratings WHERE user_id = :ownerId` (the third owner-scoped root, using `idx_recipe_ratings_user_id`) as part of the rows-first phase, relying on the statement-level aggregate trigger to repair surviving recipes — in `packages/services/recipe-workers/src/handlers/accountErasureWorker.ts` (extends T136; do not start before T136's worker body exists)
- [x] T169-int Extend the end-to-end erasure integration test: seed the erasing user's ratings on **other** users' recipes → erase → assert the user's ratings are gone, the other recipes survive, and their aggregates are re-derived to exclude the erased ratings in `packages/services/recipe-service/__tests__/integration/account/erasure.integration.test.ts` (extends T137)
...
> Raw checkbox count and real task count were both **187** at that recount.

> **Update (2026-08-12) — contract convergence appended.** The table above covers Phases 1–6 and is unchanged.
> Two later programs sit outside it and are counted separately, so the phase rows are not silently wrong:
>
> | Program                                                    | Tasks     | Count   |
> | ---------------------------------------------------------- | --------- | ------- |
> | Phases 1–6 (recounted 2026-07-15)                          | see table | **187** |
> | Reconciliation & remediation program (appended 2026-07-18) | T171–T185 | **15**  |
> | Contract convergence (appended 2026-08-12, GR-015/016/017) | T186–T190 | **5**   |
> | **Total**                                                  |           | **207** |
>
> T186–T190 close what GR-017 §17-e.12 measures: the citation repointing (T186), the supersession of the
> hand-written document (T187), the removal of recipe-service's **second** validation mechanism (T188 — **one**
> remaining `class-validator` importer; the "19 files" figure quoted elsewhere is a **mention** count and
> overstates the work ~19×), the storage-floor parity assertion (T189 — ⚠️ note that
> `@kitchensink/contract-gen`'s `auditStorageCapacity` **already exists** and is already wired by a
> `storageCapacity.test.ts` in all three services, so T189 is "copy the existing pattern and complete the
> mapping", not "invent the gate"), and recipe-service's contract test tier (T190).
>
> ⚠️ **Re-measured 2026-08-12 — two of these five are DONE, and one was never owed.** **T188 has shipped**: the
> single `class-validator` importer is converged and both `class-validator` and `class-transformer` are removed
> from recipe-service's `package.json` and `prod.package.json`, so no importer survives anywhere under
> `packages/services`. **T190 was never a gap at all** — recipe-service already has a contract tier, and the
> acceptance it carried actively contradicted the shipped design (see the task). The rows are **retained and
> ticked with the correction in place**, not deleted, so a reader can see what was measured, what it turned out
> to be, and why the row is closed.
...
## Contract convergence (T186+, appended 2026-08-12) — GR-015 / GR-016 / GR-017

> ⚠️ **Recipe's derived contract document NOW EXISTS**, which changes what is owed here.
> `packages/schemas/recipe/openapi.yaml` is **5,700 lines covering 34 paths**, against the hand-written
> `specs/001-commise-recipe-app/contracts/api.openapi.yaml`'s **2,840 lines / 32 paths** (re-measured 2026-08-12
> — this block previously said **4,945** and **2,827**, which were the 2026-08-11 figures; the derived document
> grew as the `versions` and `api-error` schema copies landed, and the hand-written file grew **by header
> rewrites, not by body** — its 2,810 body lines are unchanged. **Re-measure with `wc -l`; do not quote these
> back.** The **path** counts, 34 vs 32, are unchanged).
> The hand-written document is therefore **genuinely superseded**, not merely superseded "in principle" — but its
> citations have **not** been repointed, so **two OpenAPI documents describe the recipe service and only one of
> them is verified.** That is the state CODING_STANDARDS §15.2(6) exists to end.
>
> ⛔ **Do NOT "correct" any of these three** while doing this work: the schema package is a literal file **COPY**
> (zod are runtime values, so they cannot be derived from themselves); turbo wires the copy with `$TURBO_ROOT$`
> **`inputs`**, **NOT** `dependsOn` (that edge closes the cycle `client → schema → service → client`, because
> `recipe-service` devDepends on its own client for the contract tier); and `openapi.yaml` is **DERIVED** output
> for `oasdiff`/docs/integrators and is **NEVER a codegen input**.

- [x] T186 Repoint every source citation of the hand-written recipe contract at the derived document — `packages/services/recipe-service/src/`, `packages/clients/recipe-service/src/`, `packages/shared/recipe-core/src/`, `packages/apps/commise/web/tests/e2e/utils/recipeApi.ts`
    - **Inventory RE-MEASURED 2026-08-12 — most of this task has already landed.** Counted over **`git ls-files`**, `api.openapi.yaml` is now cited by **5 files under `packages/`** (4 `.ts` sources plus one k6 `.js`), **20 under `specs/`** and **5 under `docs/`**. ⚠️ **This bullet previously recorded 12 under `packages/` and 26 under `specs/`**, and both were correct when measured — seven of the `packages/` citations have since been repointed. Re-run the count before starting; do not work the stale list.
    - The **4 surviving `.ts` sources** (plus `packages/services/recipe-service/tests/load/lib/common.js`): `packages/clients/recipe-service/src/errors.ts`; `packages/services/recipe-service/src/collections/collections.service.ts`; `packages/services/recipe-service/src/recipes/dto/recipeResponse.dto.ts`; `packages/apps/commise/web/tests/e2e/utils/recipeApi.ts`. ⚠️ **Seven files this task used to list are already repointed** and must not be re-edited: `src/common/filters/apiException.filter.ts`, `src/account/__tests__/{account.controller.test.ts, export.service.test.ts}`, `src/account/domain/__tests__/erasureStatus.test.ts`, `__tests__/integration/account/erasure.integration.test.ts`, and `packages/shared/recipe-core/src/{accountErasure.ts, recipe.types.ts}`.
    - ⚠️ **A worktree glob still over-reports** — it counts `dist/` and `.next/` build output that disappears on the next build and must NOT be hand-edited. Counting build output as work to do is how this task gets estimated wrong, and it is the same error that produced the "57 source files" figure in ADR-0014 and in this feature's own plan. **Count with `git ls-files | xargs grep -l`.**
    - **Acceptance**: Each citation names **`packages/schemas/recipe/openapi.yaml`** (or the authoring zod at `packages/services/recipe-service/src/**/*.schema.ts`, where the reference is to a shape rather than a document) as its authority. **No file under `packages/` cites `contracts/api.openapi.yaml` as authority afterwards.** Behaviour is unchanged — this is a comment/reference correction, not a contract change, so `npm run contract:verify` must show **no diff** when it lands.
    - **Tests**: unit (a `git ls-files`-based assertion that no non-generated file under `packages/` cites `contracts/api.openapi.yaml` as authority, so the repointing cannot silently regress — the same discovery-based shape as `packages/infra/global/__tests__/appServiceDependency.test.ts`, and **not** a hardcoded file list, which is the defect GR-017 names) **AND** integration (`npm run typecheck` across `packages/{services,clients,apps,shared}/**` plus the regenerate-and-diff gate, both clean).

- [x] T187 Mark the hand-written contract superseded and correct its stale status header — `specs/001-commise-recipe-app/contracts/api.openapi.yaml`
    - **Depends on**: T186
    - **⚠️ The header has since been REWRITTEN — corrected 2026-08-12.** This bullet used to say the file's own `STATUS (2026-08-11)` block claimed "the replacement has **NOT** been generated yet". That block is gone: the header now carries a `STATUS (2026-08-12)` block which records that the replacement **exists**, names it, and explicitly corrects the note it replaced. Its **figures have also been re-measured in place** (2026-08-12): the header quoted the derived document at 4,945 lines and 12/26/5 citations, and now records **5,700 lines** and **5 / 20 / 5**. So what remains owed is far narrower than this bullet implies — the header is correct; the **citations** are not yet fully repointed. A header that under-claims the replacement's existence is what keeps contributors extending the wrong document, which is why the numbers were fixed at the site rather than only noted here.
    - **Acceptance**: The header records that the derived document **exists and is normative**, names it, drops the stale "not generated yet" claim, and keeps the two standing rules: **where this file and the service's `*.schema.ts` disagree, the service's zod wins**, and **do not extend this file with a new endpoint** — author the zod in the service instead. The file is **retained, not deleted**, as the historical record until the remaining citations — ⚠️ **re-measured 2026-08-12: 20 under `specs/` and 5 under `docs/`**, correcting the "~24 / 5" this line carried — are repointed by their own owners.
    - **⛔ One contract document per service (§15.2(6) / GR-015 §15-a.7 / AC-015-f).** A generated document living alongside a hand-maintained one **makes the problem worse**, because two artifacts both claim to be normative and only one is verified. This task's whole purpose is to make which-one-wins unambiguous from the file itself.
    - **Tests**: integration (the regenerate-and-diff gate proves the derived document is reproducible from the service's `*.schema.ts` — the property the hand-written file has never had; a header edit must not change it).

- [x] T188 Convert recipe-service's **ONE remaining `class-validator` importer** to the single `nestjs-zod` mechanism — `packages/services/recipe-service/src/search/dto/searchRecipes.query.dto.ts`
    - **⚠️ DONE — verified 2026-08-12, and the row is kept rather than deleted so the measurement history stays legible.** `searchRecipes.query.dto.ts` is converged onto `createZodDto` + **`nestjs-zod`'s** `ZodValidationPipe`, and `class-validator` / `class-transformer` are **removed from `packages/services/recipe-service/package.json` AND `prod.package.json`** — so the second mechanism cannot creep back in by an import that still resolves. Verified with `grep -rn "from 'class-validator'" packages --include="*.ts"` (minus `node_modules`/`dist`): **no importer anywhere under `packages/services`**; the single surviving hit is a synthetic fixture string inside the repo-wide AST gate.
    - **⚠️ The enforcement moved from prose to a gate, which is the part that outlives this task.** **G5** in `packages/infra/global/__tests__/serviceSecurityInvariants.test.ts` — discovery-based over the TypeScript AST — requires every HTTP controller in **every discovered deployable** to be covered by `ZodValidationPipe`, and it now runs with **NO exception list at all**: the `UNCONVERGED_CONTROLLERS` ratchet was **deleted** because its one entry, search's controller, is this task. ⛔ Do not re-add a ratchet entry to "temporarily" land an unvalidated controller.
    - **⛔ Two validation mechanisms in one service is a GR-016 §16-a.2 violation.** ⚠️ **The "18 `ZodValidationPipe` / 26 `createZodDto` sites" pair this bullet used to assert was UNREPRODUCIBLE BY ANY PARSE and is withdrawn.** Re-measured 2026-08-12, recipe-service has **23 `ZodValidationPipe` references** and **22 classes extending `createZodDto`** across **26** files. The point the numbers were making survives them: two mechanisms means two error contracts, two sets of edge cases, and a mechanism-per-route that is a **per-file accident** rather than a decision.
    - ⚠️ **CORRECTION to the "19 files" figure this task previously carried — it was a MENTION count, not an importer count.** Re-measured 2026-08-12 with `grep -rn "class-validator|class-transformer"` restricted to **import statements**: exactly **ONE** file imports either package — `src/search/dto/searchRecipes.query.dto.ts`, which imports `Transform, Type` from `class-transformer` and `IsArray, IsIn, IsInt, IsOptional, IsString, Max, Min` from `class-validator`. The other 18 files mention the string once or twice in **JSDoc about having migrated away from it**. So this is a **one-file** task, not a nineteen-file project — and the difference is exactly the kind of figure that decides whether the work gets scheduled. **Count importers, not mentions.** The same overstatement appears in ADR-0015, `docs/CODING_STANDARDS.md` §15.4 and earlier revisions of GR-015/GR-016; all four now carry the correction.
    - **Acceptance**: Every one of those routes takes a `createZodDto` DTO derived from the service's authored `*.schema.ts` zod, served by **`nestjs-zod`'s** `ZodValidationPipe`; **zero** `class-validator` and `class-transformer` imports remain in `packages/services/recipe-service/src/`; validation failure has **ONE** path producing a `400` that names the offending field(s). Every **mutating** body uses **`z.strictObject()`** (GR-017 §17-c) — `z.object()` strips unknown keys silently, so a misspelled field yields a `200` and a partial write the caller was told succeeded. Contract output is unchanged: `npm run contract:verify` shows **no diff**, since the zod is already the authored source.
    - **⚠️ Two hazards that are invisible by construction.** **(1)** Under Nest's **own** `ValidationPipe` a `createZodDto` DTO **validates nothing while looking correctly wired** — schema present, DTO referenced, route reads as validated, no input checked. This already bit identity's `PATCH /users/me`, a route that writes user data. The **only** way to observe it is a test that posts a known-bad body to a **real** route. **(2)** As each DTO converts, the **storage floor must not regress** (GR-016 §16-d): the five int-backed fields `servings`, `prepTimeMinutes`, `cookTimeMinutes`, `totalTimeMinutes`, `timerSeconds` are now bounded by `positiveInt4()` in `packages/shared/recipe-core/src/recipeRequestBounds.ts`, and `packages/shared/recipe-core/src/__tests__/recipeRequestBounds.test.ts` asserts accept-at-ceiling / reject-above. Do not drop those bounds while moving the field.
    - **⛔ Do NOT add server-side response validation while converting.** GR-016 §16-g **defers** a producing service parsing what it **emits** — an owner decision, not an unfinished task. Only **input** validation is in scope (GR-017 §17-f).
    - **Tests**: unit (per-converted-DTO accept/reject, unknown-key rejection, and the retained numeric bounds) **AND** integration (a known-bad body posted to **each** converted route on a booted app returns `400` naming the field, modelled on `packages/services/identity/tests/appValidation.test.ts`) **AND** e2e (`packages/services/recipe-service/tests/e2e/` — existing suites must stay green, proving the error contract did not change shape) **AND** k6 (`packages/tools/loadtest/` — T081's p95 ≤ 500ms budget must hold after the pipe change).

- [x] T189 Complete recipe-service's storage-capacity mapping using the EXISTING gate — `packages/services/recipe-service/src/database/__tests__/storageCapacity.test.ts`
    - **⚠️ VERIFIED COMPLETE 2026-09-17, and the verification is the reason this is checked rather than the
      absence of a failure.** `src/database/__tests__/storageCapacity.test.ts` passes 14/14 and asserts the
      mapping in BOTH directions by exact set equality (`accounted.sort()` === `bounded.sort()`), so a new
      bounded column fails on arrival and a stale entry fails too. All 14 bounded columns are principal-
      derived ids (`owner_id`, `user_id`, `created_by`, `actor`) carrying an explicit `PRINCIPAL_DERIVED`
      reason — correct, because they are not client-writable. There are NO client-writable bounded columns
      to map: recipe's text columns are `text()`, which this task already records as unbounded in Postgres.
      The int-backed wire fields are bounded by `positiveInt4()` and asserted accept-at-ceiling /
      reject-above by `packages/shared/recipe-core/src/__tests__/recipeRequestBounds.test.ts` (50 passing).
    - **Depends on**: T188
    - ⛔ **DO NOT BUILD A NEW GATE — the mechanism already exists and this task is to finish USING it.** `@kitchensink/contract-gen` exports `auditStorageCapacity` / `collectBoundedColumns` / `formatStorageCapacityFindings` (`packages/tools/contract-gen/src/storageCapacity.ts`), and a `storageCapacity.test.ts` already wires it in **all three** services — recipe at `src/database/__tests__/`, food at `src/db/schema/__tests__/`, identity at `src/types/schema/__tests__/`. A previous revision of this task said "no bidirectional completeness assertion exists anywhere in the repo"; that was **wrong** — `auditStorageCapacity` is already exhaustive over columns and already emits `stale-account` and `duplicate-account` findings, which IS the bidirectional check. The remaining work is the **mapping**: every bounded column bound to the wire fields that write it, or declared not-client-writable **with a reason**.
    - **Acceptance**: One test living **in the service** (never in the schema package, never in a wire schema) that imports **both** the drizzle schema and the authored zod — a test is **not** a wire schema, so §16-d's ban on the _production_ coupling is not weakened — **derives** its enumeration of bounded columns from the drizzle schema rather than typing it out, and asserts for each writing wire field that the zod **rejects** a value the column cannot hold. The field→column mapping is authored and its completeness is asserted in **BOTH** directions: every bounded column has a mapping entry or an **explicit, reasoned exemption**, and every mapping entry names a column that **exists**.
    - **⛔ Asserted, never derived.** No zod generated from drizzle, and **no storage type imported into a `*.schema.ts`** — that coupling is exactly what ADR-0014 removed when `RecipeSearchResponse.facets` took its wire type from `../dal/search.dal.js`. Satisfying the floor by deriving zod from drizzle is a violation of **both** GR-016 §16-d and GR-015 §15-a.5.
    - **⚠️ The floor is a floor, not a target.** Recipe's text columns are `text()` — **unbounded in PostgreSQL** — so a character limit on a title, a step or a note is a **product decision with no storage floor to derive from**. "The column allows it" is not an argument for accepting it, and those limits must not be dropped because the parity test cannot derive them.
    - **⚠️ Stated limitation**: this proves the floor only for the columns it maps. Only the "every bounded column has an entry" direction can catch a **new** column, and only if the enumeration is derived from the drizzle schema. Derive it.
    - **Tests**: unit (the parity assertions, including both completeness directions — a deliberately unmapped bounded column must **fail** the test, and a mapping entry naming a nonexistent column must **fail** it too; a test that cannot fail is coverage theater) **AND** integration (a ceiling+1 value posted to a real route yields `400`, not the `22003`/`22001` from a failed `INSERT` surfacing as a `500` — the defect class recipe shipped on five int-backed fields).

- [x] T190 Contract test tier for recipe-service — `packages/services/recipe-service/contract/__tests__/{contract,openapi}.test.ts`
    - **⚠️ DONE — and the "gap" this task recorded NEVER EXISTED. Corrected 2026-08-12; the row is retained, not deleted, because both halves of its premise were wrong and a future reader needs to know that.**
    - **⚠️ Correction 1 — the tier is present, in ALL THREE services.** The previous text read: _"⚠️ Verified gap (GR-017 enforcement table, 2026-08-12): `packages/services/{identity,food-service}/contract/__tests__/contract.test.ts` both exist; **`recipe-service` has no such tier**."_ It has **two** files, both **tracked** — `packages/services/recipe-service/contract/__tests__/contract.test.ts` and `.../openapi.test.ts` — alongside the tier's `config.ts`, `generate.ts` and `openapi.ts`. Verify with `git ls-files packages/services/recipe-service/contract`, not a memory of the enforcement table.
    - **⚠️ Correction 2 — the tier RUNS IN CI ALREADY, because it is INSIDE the default `test` glob on purpose.** `packages/services/recipe-service/vitest.config.ts` includes `'contract/**/*.test.ts'`, so the files are collected by a plain `npm run test` and therefore by CI's default per-workspace test step. Identity's and food's configs carry the same include with the reason written at the site: _"Run in the DEFAULT tier on purpose: they need no database and no network, and a gate that lives in its own tier is a gate someone has to remember to add to CI."_
    - **⛔ Correction 3 — the acceptance this task carried CONTRADICTED THE SHIPPED DESIGN, and must not be "restored".** It demanded the tier get _"its own `vitest.*.config.ts`, its own `package.json` script, an exclude keeping it out of the default `test` glob, and a CI step"_, citing §7's _"Integration and E2E MUST NOT run in the default `test` task"_. That rule governs tiers needing a **database, a network or a booted app**; the contract gates need **none of the three** — they compare committed generated files against a regeneration, in-process. Separating them would have bought nothing and cost the exact failure mode §7 exists to prevent: a script no CI job invokes. **A deliberate decision, not an oversight — do not split these into their own tier.**
    - **⚠️ Name it `*.test.ts`, never bare `*.spec.ts`** — that suffix is reserved for Playwright, whose default `testMatch` would otherwise collect vitest files and crash the run on their `vitest` imports (§7). This still holds and the shipped files comply.
    - **What the shipped tier actually asserts** (read before assuming it is thin): three drift layers — the authored zod's import allowlist, the committed package **matching a fresh generation verbatim** including `openapi.yaml`, and the `CONTRACT_HASH` being **stamped identically in the schema package and in the SERVICE**, which is what makes a skew check possible at all — plus a **discovery-based** GR-017 §17-c check that every **mutating** request body rejects unknown keys and that it claims rejection on _exactly_ those bodies (no response body, no read query). The regenerate-and-diff layer is inherently red-against-skew: skew the package and the comparison fails.
