...
- [ ] T019-test Write unit tests for the Clerk AuthMiddleware (valid session token, expired token, missing token, **absent `external_id`/`userId` claim → fail closed with 401, never falls back to `sub`**, dev bypass) in `packages/services/recipe-service/src/auth/__tests__/auth.middleware.test.ts`
...
- [ ] T018 Wire global DB provider module and injection token in `packages/services/recipe-service/src/database/database.module.ts`
- [ ] T000-prereq **[BLOCKER — 001; Clerk session-token config + first-token sync-race handling]** The shared-verifier surfacing is **DONE (PR #60)**: `@kitchensink/clerk-verify` (`VerifiedClerkClaims` + `ClerkAuthService`) already reads the Clerk `external_id` (app-user ULID) claim and sets it as `userId` on the verified Principal. This is a `@kitchensink` **platform** package (not `@commise`). When the claim is absent the shipped verifier leaves `userId` **undefined** and delegates the fail-closed decision to **per-service policy** — it does **NOT** itself fail verification (that assertion lives in the recipe AuthMiddleware, T019). This task therefore covers only the genuinely-remaining work: **(a)** add the app-user ULID to the Clerk **session-token customization** on **both** Clerk instances (prod + sandbox) — a **manual** Clerk-dashboard change so the token actually carries `external_id`; **(b)** handle the **first-token sync race** — a just-created user whose `external_id` is not yet backfilled to Clerk — so a token missing the claim is handled (retry/refresh) rather than silently resolving to a `sub`-shaped owner key. `sub` is retained for trace/audit only, NEVER for ownership
