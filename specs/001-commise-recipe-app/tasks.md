...
- [ ] T100 [US1] Add integration test for photo upload flow (presigned URL → S3 upload via LocalStack → confirm validates magic bytes + S3 HEAD size ≤ 5 MB → `recipe_photos` row with single `s3_key`; served as-is, no processing) in `packages/services/recipe-service/__tests__/integration/photos/upload.integration.spec.ts`
