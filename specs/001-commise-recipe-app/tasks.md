...
- [x] T152-int Add an integration test asserting `POST /v1/recipes` with and without difficulty, and `PATCH` clearing it via explicit `null`, round-trip correctly against real Postgres in `packages/services/recipe-service/__tests__/integration/recipes/difficulty.integration.test.ts`
