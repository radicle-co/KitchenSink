...
## 9a. On-Device Assistive-Technology Checks (manual, before release)

The component suites pin every accessibility **call and attribute** below on both platforms: the live-region
politeness, the iOS announcement and its `queue` flag, the `aria-hidden` projection, and each screen-reader
focus event and its target. They cannot pin what a screen reader **says**, whether VoiceOver honours
`accessibilityElementsHidden`, how two utterances in one frame interact, or whether a browser raises a soft
keyboard. react-native-web renders all of it in jsdom, so only a person with a device can check these.

**Devices.** One iPhone on a current iOS with VoiceOver, one Android phone on a current release with TalkBack
(both running the mobile app), and Chrome on that Android phone for the web rows. Use a recipe with at least 4
servings, and in the editor a query that returns a catalog ingredient (for example `bas` → Basil).

| ID     | Platform / AT                      | Steps                                                                                                     | Pass when                                                                                                                                                                                     | Automated counterpart                                                                            |
| ------ | ---------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| DEV-01 | iOS app, VoiceOver                 | Open recipe detail. Do not touch the stepper.                                                             | Nothing about servings is spoken on open.                                                                                                                                                     | `ServingScaleControl.native.test.tsx` (silent on open)                                           |
| DEV-02 | iOS app, VoiceOver                 | Double-tap **More servings** once.                                                                        | "5 servings" (the new count) is spoken after the button's own label.                                                                                                                          | `ServingScaleControl.native.test.tsx`, `LiveRegion.native.test.tsx`                              |
| DEV-03 | iOS app, VoiceOver                 | Double-tap **More servings** five times as fast as possible.                                              | Only the last count is spoken in full; no backlog of earlier counts is read after the taps stop.                                                                                              | `LiveRegion.native.test.tsx` (a region replacing its own message sends `queue: false`)           |
| DEV-04 | iOS app, VoiceOver                 | After DEV-02, swipe right through the stepper from **Fewer servings** to **More servings** and beyond.    | The swipe order is Fewer servings, the visible count, More servings. There is **no** extra invisible stop reading the count a second time (the hidden announcement node).                     | `LiveRegion.native.test.tsx` (`aria-hidden="true"` on iOS only)                                  |
| DEV-05 | iOS app, VoiceOver                 | Step down to the minimum, then double-tap **Fewer servings** again.                                       | Reaching it speaks "1 serving, minimum". The refused press speaks no count.                                                                                                                   | `ServingScaleControl.native.test.tsx` (limit named, refused press silent)                        |
| DEV-06 | Android app, TalkBack              | Repeat DEV-01 to DEV-05.                                                                                  | The same speech. On Android the hidden node **is** the live region, so one extra swipe stop reading the count is expected and is not a failure.                                               | `LiveRegion.native.test.tsx` (node stays exposed on Android; no imperative announcement)         |
| DEV-07 | iOS app, VoiceOver                 | Recipe editor: type `bas` in **Search ingredients**, double-tap **Basil**.                                | The cursor lands on **Search ingredients** and "Added Basil" is spoken. Both must be heard in full: neither utterance may cut the other off. The on-screen keyboard does **not** open.        | `IngredientPicker.native.test.tsx` (focus event to the search box, polite region, no text focus) |
| DEV-08 | Android app, TalkBack              | Repeat DEV-07.                                                                                            | Same as DEV-07.                                                                                                                                                                               | as DEV-07                                                                                        |
| DEV-09 | iOS and Android app                | After DEV-07, type one more character, clear it, and add Basil again.                                     | "Added Basil" is spoken again for the second add.                                                                                                                                             | `IngredientPicker.native.test.tsx` (region clears on the next keystroke)                         |
| DEV-10 | iOS and Android app                | Type a query, double-tap **Create your own food**.                                                        | The keyboard closes and the cursor lands on the form heading ("Create “…”"). Double-tapping **Cancel** returns the cursor to **Create your own food**.                                        | `IngredientPickerCreateFood.native.test.tsx`                                                     |
| DEV-11 | iOS and Android app, airplane mode | Type a query, double-tap **Search USDA for “…”**, wait for the failure, double-tap **Try again**.         | The failure sentence is spoken when it appears. After **Try again** the cursor is on **Search USDA for “…”**, not lost to the top of the screen.                                              | `IngredientPickerLiveSearch.native.test.tsx`                                                     |
| DEV-12 | iOS and Android app                | Open the recipe list with data, go offline, pull to refresh, then go online and double-tap **Try again**. | "We couldn’t refresh your recipes." is spoken once. After a successful retry the cursor moves to the screen heading.                                                                          | `RefreshNotice.native.test.tsx`, `useScreenReaderFocusOnSignal.native.test.tsx`                  |
| DEV-13 | Android Chrome (web), no AT        | Recipe editor: type `bas`, tap **Basil**.                                                                 | Focus returns to **Search ingredients** and "Added Basil" is visible. Record whether the soft keyboard opens. An opening keyboard that covers the ingredient just added is a defect to raise. | `web/tests/components/recipes/IngredientPicker.test.tsx` (focus returns; `focusIfLost`)          |
| DEV-14 | Android Chrome (web), TalkBack     | Recipe detail: activate **More servings** once, then type `12` into the count field and move out of it.   | Each step speaks the new count. Typing speaks nothing until the field is left, then "12 servings" is spoken once.                                                                             | `ServingScaleControl.test.tsx` (web: silent while typing, announces on blur)                     |

Record the device model, OS version and app build beside each row. A failure is raised as a P2 bug against the
row ID. It is a P1 when it silences a failure or progress message or strands the screen-reader cursor.
