...
- **PostgreSQL Docker container**: sibling service in same `compose.yml` network
...
- ⚠️ [`contracts/identityApi.openapi.json`](./contracts/identityApi.openapi.json) and the hand-written TS
  contract files in [`contracts/`](./contracts/) (`user.ts`, `authSession.ts`, `authorizer.ts`,
  `deletion.ts`, `errors.ts`, `postReg.ts`, `reconciliation.ts`) are **hand-maintained and verified by
