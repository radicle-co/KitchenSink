...
EventBridge (scheduled only): change-driven refresh cron → enqueue;  FoodDataReceived completion
```
...
5. **Scheduled path**: EventBridge cron → change-driven refresh → re-enqueues only fields whose
   originating external item changed upstream (FR-031/FR-032).
...
CREATE TABLE food_nutrients (
  ...
  source_id   text NOT NULL REFERENCES food_sources(id),  -- per-value provenance (R5/FR-028)
...
CREATE TABLE food_portions (
  ...
  source_id   text NOT NULL REFERENCES food_sources(id) -- per-value provenance (R9)
);
...
CREATE TABLE food_category_assignment (
  ...
  source_id   text REFERENCES food_sources(id),      -- which source asserted the classification
...
`source_sync_metadata`. **Removed** from the prior design: `foods` (denormalized), `usda_sync_metadata`,
`usda_call_log`, `rate_limiter_state`, `user_fetch_quota`, `global_fetch_quota`, and all
denormalized-nutrient / `raw_json` / `fetch_status` columns.
...
- **Identity from the verified token only** (FR-038): `sub` (+ `azp`, `public_metadata`) come from the
  validated JWT; no client-suppliable identity header is ever trusted.
...
quota** — the food service only calls a source on a cache miss, and the per-source rolling-60-min-window
limiter (§4, FR-019) already guarantees the system never exceeds each source's budget (USDA: 1,000
req/hr). Within that budget, fairness is by **demotion at drain time**, not by a per-`sub` enqueue quota:
...
- **Fairness by demotion** (FR-043): a `sub` with **more than 50 items currently pending** in the
  ...
  **at drain time** from the requester's _current_ per-`sub` pending count (derived from `fetch_queue` +
  `fetch_requesters`, DEMOTE_THRESHOLD = 50), so items auto re-promote the moment the `sub` drops below 50. The scheme is **work-conserving**: a demoted user still drains on spare capacity, and **no enqueue
  is ever rejected with `429`** for a personal quota.
- **Distinct-requester demand** (FR-044): `request_count` (FR-015 priority) counts **distinct `sub`s**
  ...
  contribution capped; ordering aged so no food `id` is pinned to the front. This table also drives
  WebSocket targeting (§2A.5) **and** supplies the per-`sub` pending count used by demotion.
- **Max batch size** (FR-045): recipe-import name sets and `POST /v1/foods/batch` ≤ 100 names/`id`s; over
  → `400`. A mixed cached+miss batch returns a **per-item partial response** — cached foods inline (with
  `id`s) and a `PENDING` entry per miss (each row created + enqueued) in one response, so the caller gets
  available data immediately and polls only the pending `id`s. (USDA's 20-key/call cap, FR-023, stays an
...
- `FoodDataReceived` pushes resolve recipients from `fetch_requesters` (the requester `sub`→food `id` set)
  — **no broadcast** to non-requesting connections. _(Targeting re-keyed from `fdcId` to the food `id`.)_
...
| POST   | `/v1/foods/batch`           | U or M    | Batch add-by-name; ≤100 names (`400` over); per-item partial (cached inline + `PENDING` per miss) (FR-045)    |
...
> Postgres `fetch_queue` enqueue**, NOT an EventBridge event (matches spec.md `FoodDataEvent` +
...
> change-driven-refresh schedule + the `FoodDataReceived` completion rule; there is no demand-event rule.
> All payloads now carry the food **`id`**, not `fdcId`.
...
```typescript
// Scheduled producer — change-driven refresh cron enqueues low-priority fetch_queue rows (FR-031/FR-032)
IngestionScheduled {
  eventId: string,
  timestamp: ISO8601,
...
```sql
-- See §2 for the full DDL. Idempotent enqueue (FR-014):
...
**Lease timeout**: Rows stuck in `status='in_flight'` for >30s are reverted to `pending` by a watchdog
query (consumer start + every minute), recovering from consumer crashes (FR-018).

**No DLQ infrastructure**: Tombstone rows (`status='tombstone'`) are the audit trail — queryable via SQL,
alertable via CloudWatch metric, reprocessable by setting `status='pending'`. `NOT_FOUND` tombstones carry
a TTL (default 30 days, `food.tombstoned_at`); `FAILED` is reached after the FR-016 retry budget and is
re-fetchable.
...
- **Fan-out + merge (per row)** — the core re-baselined logic:
    1. Read the food's `name` (the add-by-name query).
    ...
    6. Set `food.status`: `RESOLVED` (confident single merge), `UNRESOLVED` (multiple non-collapsible
       candidates → surface via `/candidates`), `NOT_FOUND` (no source has it → tombstone + TTL), or
       `FAILED` (a source fetch errored after bounded retries). Emit `FoodDataReceived`.
...
- **Rate limiting**: per-source rolling 60-min window (FR-019); atomic check-and-record against
  ...
  back off (FR-026).
- **Error handling**: 5 attempts with exponential backoff (`last_requested = now() + interval '2^attempts seconds'`)
...
- **Lease recovery**: watchdog reverts `in_flight` rows older than 30s to `pending` (FR-018).
...
### change-refresh consumer (EventBridge scheduled)

- Triggered by `IngestionScheduled` (cron). For `RESOLVED` foods, re-fetches each backing source item via
  its adapter and compares an item version/etag/hash (`food_sources.item_version`); a field is re-pulled
  **only** when its originating external item changed upstream (FR-031/FR-032). Unchanged fields — incl.
  user-resolved ones — are left intact. Re-pulled values pass adapter validation and update their
  `source_id` provenance. Enqueued as low-priority `fetch_queue` work (deduped via `ON CONFLICT`).
...
- With `pg_trgm` GIN indexes on `food.name`/`food.description`, no separate indexer is strictly required
  for fuzzy search. If ranked full-text is later added (generated `tsvector` + GIN), a `FoodDataReceived`-
...
CREATE TABLE fetch_queue ( ... );                -- demand-weighted Postgres-as-queue (food_id PK)
...
- `food-tombstone-count` — `FAILED`/`NOT_FOUND` tombstone rows
- `food-cache-hit-rate` — local store hit rate (in-process LRU; Postgres fallback)
...
## 9. Planning Decisions & Open Questions

The spec/brainstorm explicitly defer several decisions. Recommended resolutions (call them out at the gate):

1. **Auto-`RESOLVED` confidence threshold** _(spec brainstorm: "resolve before planning")._ What makes a
   fan-out result "confident enough" to auto-`RESOLVED` vs hand off as `UNRESOLVED`.
   **Recommendation:** auto-`RESOLVED` iff **exactly one surviving candidate after pre-merge dedup**
   (single source hit, or multiple source hits that collapse to one via normalized-name exact match +
   nutrient agreement within a tolerance, e.g. ±10% on energy/protein). Two or more non-collapsible
   survivors → `UNRESOLVED`. This keeps the matcher simple (the human is the safety net, FR-RES-3) and is
   tunable via config without a schema change. **Needs your judgment** — the tolerance is a product call.

2. **`UNRESOLVED` TTL** _(deferred to planning)._ What happens to an `UNRESOLVED` food nobody ever picks.
   **Recommendation:** give `UNRESOLVED` a soft TTL (default 30 days) tracked via `food.updated_at`; an
   expired `UNRESOLVED` is swept to `NOT_FOUND` (re-addable) by the change-refresh cron, so abandoned
   disambiguations don't accumulate. Low-risk; reuses the tombstone TTL machinery. **Confirm the default.**

3. **Sync vs async candidate search** _(deferred to planning)._ Whether the external candidate search is
   synchronous on `POST` or itself async (`202` + poll).
   **Recommendation:** **async** (the locked architecture). `POST /v1/foods` returns `202` + `id`
   immediately and the worker fans out off the queue — this is the only option consistent with the
   per-source rolling-window limiter and the rate budget (a synchronous search could not be throttled
   without blocking the request and exposing a denial-of-wallet surface). No change needed; recorded here
   to close the question.

4. **Change-detection mechanism on refresh** _(deferred)._ How "the external item changed upstream" is
   detected without stored raw payload. **Recommendation:** persist a per-item `food_sources.item_version`
   (etag / source version / content hash of the mapped candidate) and compare on re-fetch (FR-032). Cheap,
   payload-free, already in the §2 DDL.

5. **Source priority ranking** _(implied by FR-MRG-2)._ USDA is the hard-coded default highest priority.
   **Recommendation:** keep a static config-ordered list now (`['usda']`); promote to a DB-backed ranking
   only when a second source is wired. No schema change required at launch.
