...
**Docs touched (clause-level only):** `spec.md` FR-MRG-3; `decision-register.md` §3.17 (D-PERSERVING); `v-model/module-design.md` MOD-008/MOD-017. **Code:** `src/sources/usda/usda.adapter.ts`, `src/foods/merge/mergeEngine.ts`, with unit + integration coverage. Approval status unchanged: **LOCKED**.
