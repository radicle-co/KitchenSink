...
- **SC-014** _(first-time NEW-food resolution rate)_: The first-time resolution of **NEW** foods (those not yet in the local store, requiring a source fetch) is **bounded by the per-source budget (SC-002)**. A NEW food costs **two** charged source requests under USDA's ≤1,000-calls/rolling-60-min cap — the non-batchable name search, and the batch fetch of the keys it returned — so the realistic first-time resolution rate is **~250–450 NEW foods/hour**. The system MUST NOT claim a flat ≥5,000 NEW-food resolution rate (physically impossible under SC-002); batch ID fetches (FR-023) accelerate only the fetch-by-key leg, not the per-name search leg.
    - ⚠️ **This figure was HALVED (from ~500–900) as an accounting CORRECTION, not a regression, and the distinction is the whole point.** The old range was derived from "~1 non-batchable source call per NEW food", which described the name search alone — but a single admission was being charged for the entire fan-out, the search AND every batch chunk AND up to twenty per-key recoveries. The ledger the cap is enforced against could therefore read 900 while roughly 1,800 requests had actually gone to USDA (`foodConsumer.service.ts`, "ONE REQUEST, ONE ADMISSION"). **Nothing about the system's real throughput changed**: it was always this, and the number here was measuring the ledger rather than the world. What changed is that the ledger stopped under-counting, which is also what stops the cap being silently overrun. Do not "restore" the old range by re-merging the two legs onto one admission — FR-018 counts before every source API call, and FR-023's "a batch counts as 1" is batch-versus-twenty, never search-plus-batch.
