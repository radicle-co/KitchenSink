...
- [x] **T-190** [M] [Test-first: false] E2E harness — booted food-service (real `FoodAuthGuard`, real minted RS256 JWT) + Docker Postgres + manually-driven `FoodConsumerService.drain()` with a programmable stub `FoodSourceAdapter`; scenarios add-by-name `202`→worker fan-out/merge→`200`, dedup, UNRESOLVED+candidates/PATCH-resolve, NOT_FOUND, FAILED+`FetchFailed`, batch partial, search (fuzzy/external_key/barcode crosswalk), backpressure `503`, `FoodFetchCompleted` emission — `packages/services/food-service/tests/e2e/foodsApi.e2e.test.ts` + `tests/support/{jwt,stub-source-adapter}.ts` (15 specs, FR-005, FR-011, FR-013, FR-014, FR-024, FR-MRG-1, FR-RES-2 — E2E). EventBridge completion captured via an in-memory `EventBus` (no real AWS); LocalStack not required by these flows.
      **(reuse/extend: foundation in place — `infra/localstack/compose.yml` (`localstack:4.4.0` + `postgres:16`), `e2e-food` CI job, `test:e2e` script, `health.e2e.test.ts` boots the real Nest app + migrates Docker Postgres. Extend with the re-baselined id-keyed flows once Phases 3–6 land. Community tier — no auth token.)**
...
## ✅ BUILT — SC-007's load fixture vs. the head-term retrieval branch (raised 2026-08-23, fixture rebuilt 2026-08-25)

**Status: the FIXTURE is built (plan U30). The BREACH was closed by owner ruling 2026-08-24 (SC-007 → 500ms
p95, flat); the INSTRUMENT is now heavy-tailed to match the real catalog. ⚠️ The k6 re-baseline is still
OWED IN CI — see "Re-baseline" below for why the local runs do not discharge it.**

### The fixture, rebuilt (U30, 2026-08-25)

The either/or the fork below poses ("the fixture is lying" vs. "the query is too broad") is malformed: the
two are sequential, not alternative. The fixture is a **measurement instrument**, and an instrument that
does not model the population is wrong whatever it reads — so it was fixed first, and `8c70d742`'s
`rank_tokens @> ARRAY[head]` branch was left exactly as written (head-term retrieval is a relevance
feature; removing it is a product decision, not a benchmarking one).

`tests/load/headTermSelectivity.ts` (new) draws every head-bearing word from a **Zipf ladder** — 36 ranks,
exponent 0.68, the pair solved against BOTH measured anchors simultaneously. `PREPARATIONS` stays uniform
at seven, because no probe shape ever heads on a preparation word.

**Why the vocabularies had to GROW (23/11/17 → 36/36/36).** Each row draws exactly one word per axis, so an
axis's weights sum to 1 and its MEAN selectivity is `1 / n`. With 11 cuts the mean is already 9.09%, and a
1.89% median would require the top six cuts to sum to more than six times the permitted 13.75% maximum —
the old shape was unreachable at ANY exponent, not merely unchosen. `zipfWeights` now refuses a rank count
outside `[13, 52]` for exactly that reason.

**Generated head-term selectivity, before and after** (the same 50,000-row corpus, measured through
`rankingTokens` — the fold `food.rank_tokens` is generated by):

| axis         | probe shapes      | before (uniform)      | after: tail | after: p50 | after: floor | tail/p50          |
| ------------ | ----------------- | --------------------- | ----------- | ---------- | ------------ | ----------------- |
| `ingredient` | `broad`, `phrase` | 4.35% flat (23 terms) | 13.64%      | 1.85%      | 1.20%        | 1.00x → **7.35x** |
| `cut`        | `narrow`          | 9.09% flat (11 terms) | 13.68%      | 1.88%      | 1.18%        | 1.00x → **7.29x** |
| `brand`      | `brand`, `alias`  | 5.88% flat (17 terms) | 13.72%      | 1.88%      | 1.18%        | 1.00x → **7.31x** |

Catalog anchors: tail **13.75%**, p50 **1.89%**, ratio **7.28x**. The corpus still reaches **50,000 distinct
`normalized_name`s** (verified by generating all 50,000 and by re-running the real seeder, which inserted
50,000 RESOLVED rows with no `ON CONFLICT` loss).

**Two decisions inside the fixture that are not obvious.**

1. **Three DISTINCT PRIME cycles (997 / 1009 / 1013) with a per-axis STRIDE (617 / 631 / 647).** All three
   axes are driven by the same row index. A shared cycle makes one axis a function of another; blocked
   expansion over two nearby cycles correlates them along the diagonal where both residues stay small —
   measured at ~6.7x the independent joint count, which would have thinned the `phrase` and `narrow`
   conjunctions onto that diagonal. The independence is asserted, not argued.
2. **The probe walk gained a stride of 23.** The vocabularies are ordered by selectivity, so the old
   `index % length` walk sampled only the broad end for small probe counts — every probe expensive, none
   typical, the mirror of the defect being fixed. 23 is prime and therefore coprime to 36, 13 and 7, so
   three probes already span all three selectivity regimes.

**FR-010a was already honoured and now is asserted over the DATA.** The two-character `short` shape was
removed by U37; the new suite additionally asserts that EVERY emitted probe of EVERY shape clears
`MIN_SEARCH_QUERY_LENGTH`, so a short vocabulary word cannot reintroduce it through the back door.

### Re-baseline — ⚠️ OWED IN CI, and the local runs do NOT discharge it

k6 IS runnable locally (service booted under `vite-node` against Docker Postgres 18, the real 50,000-row
fixture, 50 VUs, the real `search.load.js`, both corpora seeded into their own databases). **Five runs were
taken and NONE of them is usable**, which is itself the finding:

| run                         | worst shape       | `miss` p95 | `barcode` p95 | 1-min load avg | verdict           |
| --------------------------- | ----------------- | ---------- | ------------- | -------------- | ----------------- |
| BEFORE #1 (uniform fixture) | `narrow` 228.56ms | 50.91ms    | 57.07ms       | (not sampled)  | PASS at 500ms     |
| AFTER #1 (Zipf fixture)     | `narrow` 323.32ms | 174.19ms   | 152.63ms      | (not sampled)  | PASS at 500ms     |
| AFTER #2 (Zipf fixture)     | `alias` 4.11s     | 3.76s      | 3.68s         | **33.03**      | BREACH, discarded |
| AFTER #3 (Zipf fixture)     | `brand` 80.40ms   | 27.86ms    | 22.00ms       | 12.94 → 4.90   | PASS at 500ms     |
| BEFORE #2 (uniform fixture) | `narrow` 247.74ms | 122.35ms   | 92.11ms       | 4.02 → 5.84    | PASS at 500ms     |

⛔ **The control shapes disqualify every one of them.** `miss` (`zqxjkvwf<n>`) and `barcode` are
BYTE-IDENTICAL across the two fixtures and their cost barely depends on the corpus — yet they range from
**22ms to 3.76s**, a factor of **170**, across the five runs. That is a host signature, not a fixture
signature: this workstation was shared with other agents running full monorepo builds and CDK synths, and
the AFTER #2 run landed at load average 33 with 0.86% of requests failing outright. Normalising each run's
worst shape by its own control does not rescue the comparison either — under contention every shape
converges on the queue (`narrow`/`miss` collapses from 4.49x to 1.05x), so the noise is not a constant
factor that divides out.

⚠️ Note especially that the two runs on the QUIETEST box invert the expected ordering — the corrected
(broader) fixture read **76.09ms** on `narrow` while the old uniform one read **247.74ms**. That is not
evidence the new fixture is cheaper; it is evidence that on this host the fixture is not what the
measurement is measuring.

What the local runs DO establish: the corrected fixture is exercisable end to end, the real seeder's own
assertions (`assertRenderingsAgree` x3, `assertGoldenRecordDepth`, `assertProbeSelectivity`) pass against
it, 50,000 RESOLVED rows seed with no `ON CONFLICT` loss, and every shape returns the rows it is supposed
to (`checks` **100.00%** on all four uncontended runs — so no probe went silently empty under the new
vocabulary). What they do NOT establish is a p95.

**The SC-007 re-baseline is owed on the `heavy-e2e` CI runner**, which is the only arbiter this gate has
ever had — see T-202's before/after table above, taken on GitHub-hosted runners for exactly this reason,
and the standing warning in the original entry below: _"Do not re-run and call it green."_

⚠️ Expect the corrected instrument to read SLOWER than the old one at the top of the ladder — that is the
point. The old fixture's broadest head term matched 9.09% of 50,000 rows (4,545); the new one's matches
13.7% (≈6,850), which is the worst realistic real query the old fixture could not produce. A breach at
500ms under the new distribution would be a real finding about the head-term branch and must be reported
as one, not repaired by re-flattening the fixture.

**Original status line, kept:** _RESOLVED in direction, not yet built. The BREACH is closed by owner ruling
2026-08-24 (SC-007 → 500ms p95, flat). The FIXTURE is to be made realistically skewed — owner ruling
2026-08-25 — which is work, not a decision._
