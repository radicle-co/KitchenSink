...
- [x] **T-203** [S] [Test-first: false] **The drain probe's own p95 estimator — a 5.4× "regression" that did not exist. DONE 2026-08-12; NO SQL CHANGED.** `packages/services/food-service/tests/load/drain-demotion.perf.ts` (SC-003, FR-046, DSN-11)
      ...
      **NOT DONE, held for an owner ruling:** a re-measure-before-failing gate (on breach, re-measure that series once; fail only if it breaches again; report a non-reproducing breach as CONTAMINATED, never as PASS). It is the only mechanism that separates a host stall from a real regression, since raising `n` reduces the estimator's variance but cannot help when a contamination episode reaches 5% of a series. It changes the gate's SEMANTICS, and SC-007's own text requires "a new owner ruling plus measurement" for exactly that class of change, so it is not implemented and no stub or flag for it was left behind.
