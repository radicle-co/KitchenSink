...
> `FoodDataReceived`). The canonical store is now **13 tables** (adds **`food_candidates`** backing
> `UNRESOLVED`, SYS-007/SYS-016); `fetch_queue` gains the **`leased_at`** lease column + reaper (SYS-003/
> SYS-005); provenance integrity is structural (`UNIQUE(food_id, id)` + composite `(food_id, source_id)`
> FKs, `ON DELETE NO ACTION`). The fan-out outcome is decided by **survivor count after normalized-name
> exact match** (REQ-050a) and follows the **legal lifecycle transition set** (REQ-028a); the
> `UNRESOLVED` candidate set has a 30-day TTL (REQ-025a). `source_call_log` is pruned to the trailing
> 60-min window (SYS-006). Cache-hit/`429`-quota framing is purged from the read/demand paths (cache
> vocabulary remains only for the deferred Redis variant, SYS-008); the forgeable `x-debug-sub` identity
> path is removed (SYS-013).
...
producers (change-driven refresh) and the `FoodFetchCompleted` completion event — never the demand-path
...
| SYS-002 | EventBridgeBus | Event bus for **scheduled producers only** (change-driven refresh, `IngestionScheduled`), the `FoodFetchCompleted` completion event, and `FetchFailed` (emitted on a `FAILED` tombstone **only**, not `NOT_FOUND` — DSN-9; drives the failure alarm). **Not** on the demand-path enqueue — add-by-name enqueues are `INSERT … ON CONFLICT (food_id)` into `fetch_queue` + `pg_notify` directly from SYS-001. Decouples scheduled producers and completion consumers from the API service. | REQ-032, REQ-IF-005 | Component |
| SYS-003 | FetchQueue | The single Postgres `fetch_queue` (Postgres-as-queue), **keyed on the food `id`** (`food_id PRIMARY KEY REFERENCES food(id)`). Every fetch unit of work — demand-path add-by-name, batch/recipe import, and scheduled refresh — is one row in this one table. **No** static high/low partition: ordering is demand-weighted at drain time, `ORDER BY <effective_priority> DESC, first_requested ASC`, where `effective_priority` is the **distinct-`sub`** `request_count` (via `fetch_requesters`, FR-044) with aging — a sub counts at most once (`PRIORITY_CAP=1` per sub is **structural** via the `fetch_requesters` PK; the total is the **uncapped** distinct count, never a raw `+1` or a `LEAST(…)` ceiling, DSN-3) — and a `sub` holding >50 pending rows is demoted to the back (dynamic, at drain time; a food with multiple requesters is demoted only when **all** its requesters are over the threshold — there is no stored demotion-tier column). Rows carry a single `status` of `pending \| in_flight \| tombstone` plus a **`leased_at`** lease-stamp column (the 30s `in_flight` lease; a reaper reverts rows with `leased_at < now() - 30s` for crash recovery, REQ-017), served by a partial index `(leased_at) WHERE status='in_flight'` (DB-8). `attempts` is the **failure counter**, incremented only on a real source error — never on a claim, reaper reclaim, or back-pressure deferral (DSN-5); a TTL-expired terminal food is **reactivated** by reviving its tombstone row to `pending` (DSN-1); a worker `resolve`/`tombstone` prunes the food's `fetch_requesters` rows so that set stays bounded (DSN-10). Drained by the Fargate fan-out/merge worker via `LISTEN/NOTIFY`. | REQ-011, REQ-012, REQ-013, REQ-014, REQ-015, REQ-017, REQ-018, REQ-039, REQ-IF-005 | Component |
| SYS-004 | FetchRequesters | Postgres `fetch_requesters` table modeling **distinct-requester demand** (FR-044): `(food_id, sub)` upserted `ON CONFLICT DO NOTHING` per request so a single `sub`'s repeats never re-count. Supplies the **distinct-`sub`** `request_count` on each `fetch_queue` row (a sub counts at most once — `PRIORITY_CAP = 1` per sub is **structural** via the PK; the total is the **uncapped** distinct count, DSN-3), each `sub`'s live pending count for SYS-013 demotion, and the authoritative subscription set the WebSocket notifier (SYS-010) uses for per-recipient targeting. Keyed on the food `id`, not a source key. Not a separate queue — a demand index over the single `fetch_queue`. | REQ-014, REQ-039 | Component |
| SYS-005 | FoodFanOutMergeWorker | Rate-limited Fargate consumer worker (single instance via advisory lock) that drains the single `fetch_queue` via `LISTEN/NOTIFY` in demand-weighted order (dynamic >50-pending demotion; no high/low split). Claims one row under a single **30s** `in_flight` lease stamped on `leased_at` (a reaper reverts rows with `leased_at < now() - 30s` for crash recovery). For each row it **fans out across every wired source adapter** (SYS-014) by name, fetches from each source that has the item (per-source rolling-window-limited, SYS-006), invokes adapter validation (SYS-020), pre-merges/dedups candidates, then decides the outcome by **survivor count after normalized-name exact match** (1 → `RESOLVED`; >1 → `UNRESOLVED`, persisting the surviving set to `food_candidates`; 0 → `NOT_FOUND`; no nutrient tolerance, REQ-050a), drives the golden-record merge (SYS-015), persists via the DAO layer (SYS-018) with provenance (SYS-017), and sets `food.status` along the legal transition set (REQ-028a; refresh never clobbers a manual pick). A **RESOLVED** food only reaches the drainer because change-refresh (SYS-019) re-enqueued it (a fresh add never re-enqueues a RESOLVED food), so it takes a **selective in-place re-pull branch** — re-fetching its `food_sources` backing items by `external_key`, re-merging only those changed upstream (`item_version`), never re-fanning-out by name and never clobbering a manual pick (DSN-4). `attempts` counts **real source failures only** (5xx/timeout) — back-pressure/rate-limit deferrals and reaper reclaims never increment it — so a food is tombstoned `FAILED` only after 5 genuine errors (REQ-016/REQ-027, DSN-5); emits `FoodFetchCompleted` (and `FetchFailed` on a `FAILED` tombstone **only** — `NOT_FOUND` is normal and raises no failure alarm, DSN-9). Validates async-producer provenance from `fetch_requesters` (FR-048, DSN-2). | REQ-015, REQ-016, REQ-017, REQ-024, REQ-025, REQ-027, REQ-028a, REQ-042, REQ-050, REQ-050a | Component |
| SYS-006 | PerSourceRollingWindowLimiter | **Per-source** rolling-60-minute-window limiter holding the worker to ≤ each source's hourly cap in any trailing 60 minutes (USDA ≤1,000; the worker pauses draining work that needs a given source at 90% — USDA 900 — and resumes as that source's earlier calls age out). State = recent source-call timestamps in the Postgres `source_call_log` keyed by `source` (lean default); a per-source Redis sorted set is the deferred variant. Check-and-record (count trailing-60-min calls + record the new call) is atomic per source and, because exactly one consumer drains under the advisory lock (SYS-005/REQ-022), effectively serial — this is what makes "zero `429` in any window" safe. `source_call_log` rows older than the trailing 60-minute window are pruned on a periodic sweep (REQ-020). On a source `429`, backs off treating that source's window as full. Each additional wired source gets its own window sized to that source's limit. | REQ-019, REQ-020, REQ-021, REQ-026 | Component |
| SYS-007 | FoodCanonicalPostgresStore | PostgreSQL-backed canonical, normalized, provenance-bearing store on `kitchensink_food` (shared instance). Holds the source-agnostic schema: `food` (golden scalar fields — internal `id` PK, `name`, `normalized_name` dedup key, `description`, `kind` (`generic\|branded`), brand attributes, barcode, lifecycle `status`, `tombstoned_at`, timestamps), `food_sources` (crosswalk, `UNIQUE(source, external_key)`, `item_version`, **no verbatim payload**), `nutrient` (dictionary; units live here), `food_nutrients` (`amount`, `basis`, `source_id`), `food_portions` (`gram_weight`, `source_id`), `food_field_provenance` (`food_id`, `field`, `source_id`), `food_category`(+assignment), and **`food_candidates`** (`id` PK, `food_id`, `source`, `external_key`, `name`, `summary`, `created_at`, `UNIQUE(food_id, source, external_key)`) backing the `UNRESOLVED` set — **13 tables** total. Provenance integrity is structural: `UNIQUE(food_id, id)` on `food_sources` + composite `(food_id, source_id)` FKs on `food_nutrients`/`food_portions`/`food_field_provenance`/`food_category_assignment` (`ON DELETE NO ACTION`, no cascade-delete of golden/manual values). Data-integrity constraints: the `nutrient` dictionary has a stable dedup key (`UNIQUE(external_code)` plus `UNIQUE(COALESCE(external_code, lower(name)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                       | '         | '   |     | unit))`) so a tagname-less source nutrient does not split `nutrient_id`(DB-5);`food_nutrients.amount`has`CHECK (amount >= 0)`and`food_portions.gram_weight`has`CHECK (gram_weight > 0)`(numeric precision intentionally omitted for fidelity, DB-6);`food_sources.fetch_state`is`text`+`CHECK (fetch_state IN ('fetched','error'))`— operational state columns use text+CHECK while controlled schema sets use`pgEnum`(DB-7). Indexed for`id`PK,`status`, the unique `normalized_name`, `food_sources(source, external_key)`, `food_nutrients(food_id)`/`(source_id)`, `food_candidates(food_id)`, the partial `fetch_queue (leased_at) WHERE status='in_flight'`reaper index (DB-8), and trigram GIN on`name`/`description`. No `fdcId`, no denormalized nutrient columns, no `fetch_status`, no EAV. | REQ-001, REQ-002, REQ-003, REQ-004, REQ-008, REQ-010, REQ-028, REQ-029, REQ-NF-018 | Component |
...
| SYS-015 | GoldenRecordMergeEngine | **(New.)** Field-level cross-source merge that assembles the golden record after candidates are normalized: **presence beats absence**; identity/short fields (`name`, `brand`) take the **higher-priority source** (NOT longest); free-text (`description`, `ingredients`) **longer-wins**; nutrients normalized to a common basis (per-100g) before any blend, conflicts resolved to the higher-priority source with `food_nutrients.source_id` recording the winner. Deterministic and auditable. Drives the outcome by **survivor count after normalized-name exact match** (1 → `RESOLVED`; >1 → `UNRESOLVED`; 0 → `NOT_FOUND`; no nutrient tolerance, bias to `UNRESOLVED`, REQ-050a). | REQ-050, REQ-050a, REQ-051 | Component |
| SYS-016 | CandidateResolutionService | **(New.)** Cross-source candidate disambiguation: `GET /v1/foods/{id}/candidates` returns the candidate list for an `UNRESOLVED` food (each candidate carries its `source` and that source's item key); `PATCH /v1/foods/{id}` resolves from the user's pick — each chosen candidate **validated to belong to that food's own candidate set** — then **re-fetches each picked candidate's full payload from its source by `external_key`** (a budgeted per-source call via SYS-014/SYS-006, since `food_candidates` stores only metadata, not nutrients/portions), drives the merge (SYS-015) over the re-fetched payload, and moves the food to `RESOLVED`. An out-of-set pick is rejected (`CandidateMismatchError`, `400`/`409`) with `status` unchanged; a re-fetch failure aborts the resolve (`SourceApiError`) leaving the food `UNRESOLVED`. `PATCH`-resolve is exempt from flood-shed and never `429`'d, but its re-fetch call is **counted** against the per-source rolling window (never an unrecorded call → never breaches the cap, DSN-6); in the rare case the hard cap is momentarily reached it **waits** briefly for headroom, then aborts retryably (`503`) leaving the food `UNRESOLVED`. Concurrent `PATCH`es on the same food are serialized by a food-row lock so the budgeted re-fetch + status flip happen once (DSN-8). It is `UNRESOLVED`-only and idempotent (a `PATCH` on an already-`RESOLVED` food is a no-op `200`, REQ-028a). The candidate set is the **`food_candidates`** rows persisted for the `UNRESOLVED` food; it expires 30 days after `created_at`, after which the next request re-fans-out (REQ-025a). The user's pick is stored as ordinary provenance (SYS-017). The human is the final arbiter, so pre-merge dedup need not be perfect. | REQ-025a, REQ-028a, REQ-048, REQ-049, REQ-IF-010, REQ-IF-011 | Component |
...
| SYS-019 | ChangeDrivenRefresh | **(New.)** Change-driven refresh: once a food is populated our stored values stand; a scheduled `IngestionScheduled` rule (SYS-002) triggers periodic checks that **re-enqueue** a changed `RESOLVED` food as a low-demand `fetch_queue` row via the ordinary `enqueue(food_id, 'svc_change_refresh')` path (deduped via `ON CONFLICT`). The **selective per-item re-pull + in-place re-merge is the executable home in SYS-005's refresh branch** (DSN-4): when the worker drains that row the food is still `RESOLVED`, so it re-fetches each backing item by `external_key`, re-pulls a field **only** when its originating external item changed upstream (per-item version/etag/hash in `food_sources.item_version`, not stored payload), never blindly re-blends, and **never** overwrites a user's manual resolution (it is just a stored value, REQ-028a). Re-pulled values pass adapter validation (SYS-020) and update their `source_id` provenance. No max-staleness cutoff withholds an already-held record. | REQ-031, REQ-032, REQ-053 | Component |
...
| SYS-005 | SYS-002 | Publishes     | The worker emits `FoodFetchCompleted` on completion; failure is fire-and-forget (polling remains the primary notification)                                                                                            |
...
Client → ALB → ECS/Fargate NestJS service → SYS-001 (FoodApiController)
              ...
              SYS-003 (single fetch_queue; status pending|in_flight|tombstone; keyed on food id) ──┐
                   ...
                   SYS-002 (EventBridge: scheduled + FoodFetchCompleted) ── triggers ─→ SYS-019 (ChangeDrivenRefresh)
...
| WebSocket API (optional, deferred) | Outbound  | Real-time `FoodFetchCompleted` push (carrying the food `id`) to connected clients, per-recipient                                                                                                         |
...
### Path 3: Add By Name (Add-by-name miss → Async Resolution)
...
Postgres LISTEN/NOTIFY (fetch_queued) → Fargate FoodFanOutMergeWorker (SYS-005)
  ...
  → UPDATE fetch_queue SET status='in_flight', leased_at=now()        (single 30s in_flight lease on leased_at; reaper reverts rows with leased_at < now() - 30s)
  ...
  → GoldenRecordMergeEngine.merge(candidates) (SYS-015) — outcome by survivor count after normalized-name exact match (REQ-050a)
       ├─ exactly one survivor          → assemble golden record → status='RESOLVED'
       ├─ >1 survivors                  → status='UNRESOLVED'; persist surviving candidates to food_candidates; surface via /candidates (SYS-016)
  ...
  → EventBridge Publish FoodFetchCompleted { id, status }
```
...
Client → PATCH /v1/foods/{id} { "candidateIds": ["c1"] }
  → SYS-016 validate each candidateId ∈ this food's own candidate set
       ...
       └─ valid → re-fetch each pick from its source by external_key (SYS-014 adapter; budgeted
                  rolling-window call SYS-006 — food_candidates holds only metadata, no nutrients;
                  resolve is never shed/429 but consumes budget)
             ├─ re-fetch fails → SourceApiError (502/503); status stays 'UNRESOLVED'
             └─ ok → GoldenRecordMergeEngine.merge (SYS-015) over the re-fetched payload
                  → record user pick as ordinary provenance (SYS-017)
...
| EventBridge       | Default event bus                                          | us-east-1 | Scheduled producers (change-refresh `IngestionScheduled`) + `FoodFetchCompleted` only; not on the demand path                                                                                            |
| FetchQueue        | Postgres `fetch_queue` table                               | us-east-1 | SYS-003; single demand-weighted queue keyed on food `id`; `status pending\|in_flight\|tombstone`; 30s `in_flight` lease on `leased_at` (reaper-reclaimed); tombstone on terminal (30d TTL for NOT_FOUND) |
...
| CanonicalStore    | `kitchensink_food` DB on shared `kitchensink-data-{stage}` | us-east-1 | SYS-007; 13-table source-agnostic schema (incl. `food_candidates`); logical DB on the shared instance; no new RDS, no cluster                                                                            |
...
**Parent Requirements**: REQ-015, REQ-016, REQ-017, REQ-024, REQ-025, REQ-027, REQ-028a, REQ-042, REQ-050, REQ-050a

**Traceability Rationale**: SYS-005 drains the single `fetch_queue` in demand-weighted order with dynamic demotion (REQ-015), claims one row under a single **30s** `in_flight` lease stamped on `leased_at` and the reaper reverts stale rows (`leased_at < now() - 30s`) for crash recovery (REQ-017), **fans out across every wired adapter** and merges into the golden record (REQ-050), decides the outcome by survivor count after normalized-name exact match — 1 → `RESOLVED`, >1 → `UNRESOLVED` (persisting the surviving set to `food_candidates`), 0 → `NOT_FOUND` (REQ-050a) — along the legal transition set without clobbering a manual pick (REQ-028a), maps a source `200` into the canonical model with the crosswalk row and emits `FoodFetchCompleted` (REQ-024), sets `NOT_FOUND` + tombstone when no source has the item (REQ-025), retries transient errors up to 5 attempts then sets `FAILED` + tombstone (REQ-016/REQ-027), and validates async-producer provenance before processing (REQ-042/FR-048). The merge rules and provenance are delegated to SYS-015/SYS-017; the per-source throttle to SYS-006.

### Component: SYS-006 (PerSourceRollingWindowLimiter)

**Parent Requirements**: REQ-019, REQ-020, REQ-021, REQ-026

**Traceability Rationale**: SYS-006 enforces the **per-source** rolling-60-minute window (USDA ≤1,000; pause at 90% / 900, REQ-019), performs the atomic per-source check-and-record on `source_call_log` (REQ-020) — atomic and effectively serial because exactly one consumer drains under the advisory lock (REQ-022), which is what makes "zero `429` in any window" safe — holds the worker below the cap by re-deferring the `fetch_queue` row lease when a source is at cap (REQ-021), prunes `source_call_log` rows older than the trailing 60-minute window on a periodic sweep (REQ-020), and backs off treating a source's window as full on a `429` (REQ-026). Each additional wired source gets its own window. Tombstone-TTL retention (REQ-018) belongs to SYS-003.
...
**Traceability Rationale**: SYS-007 is the normalized, provenance-bearing canonical store (REQ-028) with the access-path indexes for sub-200ms search, lifecycle/refresh queries, and single-query provenance (REQ-029/REQ-010). It backs the lifecycle reads (REQ-002/REQ-003/REQ-004), local-only search incl. barcode/`external_key` via the `food_sources` crosswalk (REQ-008), and faithful nutrient values after per-100g basis normalization (REQ-NF-018). The 13-table schema includes `food_candidates` (backing `UNRESOLVED`), and enforces same-food provenance structurally via `UNIQUE(food_id, id)` on `food_sources` + composite `(food_id, source_id)` FKs (`ON DELETE NO ACTION`). No `fdcId`, no denormalized nutrient columns, no `fetch_status`, no EAV.
...
**Parent Requirements**: REQ-050, REQ-050a, REQ-051

**Traceability Rationale**: SYS-015 performs the field-level cross-source merge that assembles the golden record (REQ-050) with the normative rules (REQ-051): presence beats absence; identity/short fields take the higher-priority source (NOT longest); free-text longer-wins; nutrients normalized to per-100g before any blend, conflicts resolved to the higher-priority source with `food_nutrients.source_id` recording the winner. It drives the outcome by survivor count after normalized-name exact match — 1 → `RESOLVED`, >1 → `UNRESOLVED`, 0 → `NOT_FOUND`, no nutrient tolerance, biasing to `UNRESOLVED` (REQ-050a) — and records winners via SYS-017.

### Component: SYS-016 (CandidateResolutionService) — NEW

**Parent Requirements**: REQ-025a, REQ-028a, REQ-048, REQ-049, REQ-IF-010, REQ-IF-011

**Traceability Rationale**: SYS-016 exposes the cross-source candidate set — the **`food_candidates`** rows persisted for an `UNRESOLVED` food (`GET /candidates`, each candidate carrying its `source` and item key, REQ-048/REQ-IF-010) — and resolves from the user's pick (`PATCH /{id}`, REQ-049/REQ-IF-011), validating each chosen candidate belongs to that food's own candidate set (out-of-set → `CandidateMismatchError`, `400`/`409`, status unchanged). `PATCH`-resolve is `UNRESOLVED`-only and idempotent (a `PATCH` on an already-`RESOLVED` food is a no-op `200`, REQ-028a); the candidate set expires 30 days after `created_at`, after which the next request re-fans-out (REQ-025a). Because `food_candidates` holds only metadata, resolve **re-fetches each picked candidate's full payload from its source by `external_key`** (a budgeted per-source call; never shed/`429` but consumes budget) before it drives the merge (SYS-015) and stores the pick as ordinary provenance (SYS-017). The human is the final arbiter, keeping pre-merge dedup safe.
...
| Retry / tombstone / lease / TTL (REQ-016..018, 025, 025a, 027) | SYS-003, SYS-005, SYS-006                            |
...
| Lifecycle transition set (REQ-028a)                            | SYS-001, SYS-005, SYS-016                            |
...
| Auto-resolve boundary (REQ-050a)                               | SYS-005, SYS-015                                     |
