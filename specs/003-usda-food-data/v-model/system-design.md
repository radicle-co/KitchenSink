...
Client → ALB → ECS/Fargate NestJS service → SYS-001 (FoodApiController)
              ...
              └────── SYS-004 (fetch_queue: low demand) ───┘        ↓ calls                                    ↓ writes
                        ↑ INSERT (scheduled only)                SYS-006 (RollingWindowLimiter)
...
| SYS-005 → SYS-006 | Atomic check-and-record on the Postgres `usda_call_log` (count trailing-60-min calls + insert the new call in one transaction; Redis sorted-set Lua variant deferred); returns `{ allowed: bool, trailingCount: number }` |
...
| IC-003      | SYS-005    | SYS-006    | CheckAndRecordCall | none                                                                  | `{ allowed: boolean, trailingCount: number }`                                                                                            |
...
Postgres LISTEN/NOTIFY (fetch_queued) → Fargate ConsumerWorker (SYS-005)
  → SELECT … FROM fetch_queue WHERE status='pending' ORDER BY (requester pending-count > 50) ASC, request_count DESC, first_requested ASC FOR UPDATE SKIP LOCKED LIMIT 1 (demand order + dynamic demotion, row lease, FR-018)
...
Postgres LISTEN/NOTIFY (fetch_queued) → Fargate ConsumerWorker (SYS-005)
  ...
  → release row lease (no status change); pause draining / back off
