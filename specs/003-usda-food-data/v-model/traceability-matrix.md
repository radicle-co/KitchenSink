...
> New rows were added for all new elements: REQ-045..055 / REQ-IF-009..012 / REQ-CN-007;
> SYS-014..020; ARCH-013..019; MOD-015..021; HAZ-042..047; and the new tests (UTP-015..021 +
> UTP-014, ITP-013..019, STP-014..020, AT-046..055 / AT-048..052 + AT-005-B/008-B/049-B/050-B/051-B).
...
| ---------------- | ------------------------------------------------------------------- | ----- | ----------------------------------------------- |
| Spec FR          | FR-001..053 + FR-IDN-1..3 + FR-RES-1..3 + FR-MRG-1..4 + FR-ADP-1..3 | 65    | FR-IDN/RES/MRG/ADP families, FR-043..053        |
| REQ (functional) | REQ-001..055                                                        | 55    | REQ-045..055                                    |
| REQ-IF           | REQ-IF-001..012                                                     | 12    | REQ-IF-009..012                                 |
| REQ-NF           | REQ-NF-001..018                                                     | 18    | —                                               |
...
| AT / ATP         | 51 cases (42 AT + ATP-008-A..I)                                     | 51    | AT-046..055, AT-048..052, \*-B variants         |
| ATS              | 74 scenarios                                                        | 74    | ATS-036..044 (auth), candidate/merge/provenance |
| HAZ              | HAZ-001..047                                                        | 47    | HAZ-042..047                                    |
...
inverse view.) **Result: 65 / 65 FRs traced — no FR gap.** `fdcId` appears only at FR-023 / FR-IDN-2
...
**Coverage: 65 / 65 FR → ≥1 REQ (100%).** No spec FR is unanchored.
...
| **REQ-017**                                  | ⚠️ GAP — no AT (STP-003-A, STP-005 only)                     | —                                              | ⬜ Untested |
...
| **REQ-NF-001..006, 008..010, 014, 015, 017** | (Inspection / Analysis per Verification Method — no AT)      | —                                              | ⬜ Untested |
...
| **REQ-NF-001..006, 008..010, 014, 015, 017** | (Inspection/Analysis — no SYS test)                   | —                                                                           | ⬜ Untested |
...
HAZ-042..047 (merge/provenance/dedup/refresh) and the auth slice HAZ-036..041 are fully connected.
...
| **HAZ-026** | `FoodDataReceived` push routed to wrong connection              | REQ-043; MOD-009 (+ SYS-004 subscription set)                                  | UTP-009-B, ITP-009-A, STP-010-B, ATP-008-E                                                                              | ⬜ Untested |
...
| **HAZ-043** | Wrong-merge — distinct foods unified / out-of-set pick          | REQ-048, REQ-049, REQ-050, REQ-051; MOD-017, MOD-018                           | UTP-017-D, UTP-018-B, ITP-016-B, STP-016-B, AT-049-B                                                                    | ⬜ Untested |
...
| ------ | ----------------------- | -------------------- | ------- | ---- | ------------------------- |
| **Ø**  | FR → REQ                | 65 FR                | 65      | 100% | none                      |
| **A**  | REQ → AT/ATP            | 67 functional+IF REQ | 59      | 88%  | 8 acceptance gaps (below) |
| **B**  | REQ → SYS               | 67 functional+IF REQ | 67      | 100% | none                      |
| **B**  | REQ → STP               | 67 functional+IF REQ | 67      | 100% | none                      |
| **C**  | ARCH → SYS              | 19 ARCH              | 19      | 100% | none                      |
| **C**  | ARCH → ITP              | 19 ARCH              | 19      | 100% | none                      |
| **D**  | MOD → ARCH              | 21 MOD               | 21      | 100% | none                      |
| **D**  | MOD → UTP               | 21 MOD               | 21      | 100% | none                      |
| **H**  | HAZ → mitigation        | 47 HAZ               | 47      | 100% | none                      |
| **H**  | HAZ → verification test | 47 HAZ               | 47      | 100% | none                      |

### Chain-completeness checks (per task acceptance criteria)

| Check                                        | Result                                                                                                           |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Every spec FR → ≥1 REQ                       | ✅ 65/65                                                                                                         |
...
| Every functional REQ → ≥1 ATP                | ⚠️ 8 acceptance gaps (below) — all have STP coverage                                                             |
| Every HAZ → mitigation AND verification test | ✅ 47/47                                                                                                         |
...
| AT / ATP (acceptance)      | 51    | 59/67 functional+IF REQ |
| ATS (acceptance scenarios) | 74    | —                       |
...
2. **REQ-017** (single 30s `in_flight` lease + stale-revert) — no `AT-*`. Verification Method is
   _Inspection_; STP-003-A/STP-005-A, UTP-003-D cover it. _Gap: acceptance (low risk — Inspection REQ)._
3. **REQ-018** (tombstone / `NOT_FOUND` 30-day TTL retention) — no standalone `AT-*`; touched only inside
   ATS-025-A2. Verification Method _Inspection_; STP-003-C covers it. _Gap: acceptance._
4. **REQ-028** (canonical normalized provenance-bearing schema) — no `AT-*`. Verification Method
   _Inspection_; STP-007-A/017-A/018-B, UTP-006-_, UTP-016-_, UTP-019-* cover it. *Gap: acceptance.\*
5. **REQ-038a** (all authenticated users may read shared food data) — no `AT-*`; ATP-008-G validates only
   REQ-038b–c. STP-013-D covers the read-authz path. _Gap: acceptance (auth slice — should add an AT)._
6. **REQ-042** (async-producer provenance / named IAM principals) — no `AT-*`. STP-005-G, UTP-014-C,
   ITP-012-E cover it. _Gap: acceptance._
7. **REQ-044b/c/d** (load-shed latency under flood / non-secret config / named component) — no `AT-*`;
...
8. **REQ-IF-006** (per-source API key in Secrets Manager, never exposed) — no `AT-*`. Verification Method
...
- **REQ-NF-001..006, 008..010, 014, 015, 017 and REQ-CN-001..006** — Verification Method _Inspection_ or
