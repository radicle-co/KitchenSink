...
- **CI**: a `deploy-meal-plan` job in `.github/workflows/sandbox-deploy.yml` gated **ensure-exists** through
  `.github/scripts/deployGate.sh` (ADR-0010), not on changed paths. Post-deploy smoke asserts the ecosystem: the
  ...
  `teardownSandboxPr.sh`; 006 adds no new public DNS of its own beyond the service subdomain.
