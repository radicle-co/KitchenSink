...
| ARCH-017 | `ApiExceptionFilter` + error classes                       | Single error envelope      | `common/apiException.filter.ts` plus `*Error` classes with `Object.setPrototypeOf` and `is*` guards. **One** `{code,message,details?}` shape for the whole service.                                                                                 | SYS-008      |
