...
- `packages/services/` is workspace-registered and ships five services (`identity`, `identity-webhooks`, `food-service`, `recipe-service`, `recipe-workers`); ~~007 **plans to add** `grocery-service`, which does not exist yet~~ — **SUPERSEDED by [ADR-0017](../../docs/architecture/decisions/0017-service-ownership-for-features-006-007-009-010.md) (2026-08-12): 007 adds NO service.** It lands in the existing `packages/services/recipe-service` (a `GroceryListsModule` beside `RecipesModule`, sharing `@kitchensink/schema-recipe`), with the retailer adapters and order-status polling in `packages/services/recipe-workers`. The empty leftover is `packages/api/`, which is **not** a workspace root.
...
- ⚠️ **SUPERSEDED (2026-08-12) — ~~RESOLVED (2026-08-02)~~**: the 2026-08-02 ruling ("007's backend is `packages/services/grocery-service/`, matching the shipped `packages/services/<domain>-service` convention and 005's `ai-service`") is **no longer the answer.** [ADR-0017](../../docs/architecture/decisions/0017-service-ownership-for-features-006-007-009-010.md) rules 007 into the **existing `packages/services/recipe-service`** — no new deployable, and no `@kitchensink/schema-grocery` (a schema package is per **service**, not per feature). Still true from 2026-08-02: `packages/api/` is an empty leftover and is **not** a workspace root.
    - ⚠️ The 2026-08-02 claim that "all paths in `plan.md`, `tasks.md`, and `research/` were updated accordingly" was **false for `research/`**: `research/codebase-analysis.md` still prescribed `packages/apps/commise/api/src/grocery-lists/` — a path that has never existed — until 2026-08-12. `plan.md`, `spec.md` and `tasks.md` were repointed to the recipe service by commit `b9221bb3` (2026-08-12); `research/codebase-analysis.md` was corrected separately, same day.
...
4. ~~**Monorepo path drift**~~ — ~~**CLOSED (2026-08-02)**: canonical API location is `packages/services/grocery-service/`~~ → **SUPERSEDED (2026-08-12) by [ADR-0017](../../docs/architecture/decisions/0017-service-ownership-for-features-006-007-009-010.md)**: the canonical location is **`packages/services/recipe-service/src/grocery-lists/`**, with retailer adapters in `packages/services/recipe-workers/src/grocery/`. `packages/api/` remains an empty leftover, not a workspace root.

---

_Report generated on 2026-06-02. READ-ONLY scan. No files modified._

_Amended in place twice since: 2026-08-02 (path-drift ruling recorded) and 2026-08-12 (that ruling marked
**superseded** by ADR-0017 — 007 ships in `packages/services/recipe-service`). Superseded findings are struck
through, never deleted, so the audit trail survives._
