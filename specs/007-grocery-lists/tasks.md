...
- [ ] **T-001** [P1] [US-001] Create DB migration for grocery_lists, grocery_list_items, user_pantry_items, grocery_product_map — `packages/services/grocery-service/src/database/migrations/007_grocery_lists.sql`
...
- [ ] **T-002** [P1] [US-001] Define Drizzle ORM schemas for all four tables — `packages/services/grocery-service/src/database/schema/grocery-lists.ts`
    - Depends on: T-001
    - Implements: FR-028
...
- [ ] **T-004** [P1] [US-001] Implement IngredientAggregatorService (aggregate, normalizeUnit, deduplicate) — `packages/services/grocery-service/src/ingredient-aggregator.service.ts`
    - Depends on: T-003
    - Implements: FR-028
    - Acceptance: "2 cups flour" + "100g flour" = ~315g flour in test; deduplication collapses same fdc_id to single line.

- [ ] **T-005** [P1] [US-001] Implement PantryService (add/remove/get/subtractFromList/pruneExpired) — `packages/services/grocery-service/src/pantry.service.ts`
    - Depends on: T-002
...
- [ ] **T-006** [P1] [US-001] POST /api/v1/grocery-lists endpoint — generate list from meal plan — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-004, T-005
    - Implements: FR-028
    - Acceptance: Scenario 1 from spec passes; empty meal plan returns 200 with empty items array; SC-004 timing target.

- [ ] **T-007** [P1] [US-001] GET /api/v1/grocery-lists endpoint — list user's grocery lists — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-028
    - Acceptance: Returns only authenticated user's lists; cursor-based pagination works.

- [ ] **T-008** [P1] [US-001] GET /api/v1/grocery-lists/:id endpoint — get list with items — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-028
...
- [ ] **T-004** [P1] [US-002] _(shared with US-001)_ IngredientAggregatorService deduplication — `packages/services/grocery-service/src/ingredient-aggregator.service.ts`
    - Depends on: T-003
    - Implements: FR-028
...
- [ ] **T-009** [P1] [US-003] PUT /api/v1/grocery-lists/:id endpoint — batch update items (toggle is_pantry, is_ordered, sort_order) — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-029
    - Acceptance: Scenario 3 from spec passes; pantry items excluded from "to order" count; 403 if wrong user.

- [ ] **T-011** [P1] [US-003] POST /api/v1/grocery-lists/:id/items/:itemId/pantry endpoint — mark as pantry — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-029
    - Acceptance: 200 on success; 404 if item missing; pantry flag persisted.

- [ ] **T-012** [P1] [US-003] DELETE /api/v1/grocery-lists/:id/items/:itemId/pantry endpoint — remove pantry flag — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-029
...
- [ ] **T-013** [P2] [US-005] Walmart adapter: searchByIngredient + createCart + checkout — `packages/services/grocery-service/src/adapters/walmart.adapter.ts`
    - Depends on: T-004
    - Implements: FR-030
    - Acceptance: Adapter unit tests pass with mocks; 10s timeout configured; circuit breaker after 5 failures.

- [ ] **T-014** [P2] [US-005] ProductMappingService: USDA fdc_id → store SKU lookup + cache — `packages/services/grocery-service/src/product-mapping.service.ts`
    - Depends on: T-013
    - Implements: FR-030
    - Acceptance: Returns walmart_sku + price for known fdc_id; null for unmapped items; JSONB store_sku persisted.

- [ ] **T-016** [P2] [US-005] Instacart adapter: OAuth authorize + searchProducts + createOrder — `packages/services/grocery-service/src/adapters/instacart.adapter.ts`
    - Depends on: T-014
    - Implements: FR-030
...
- [ ] **T-015** [P2] [US-007] Walmart adapter: checkout handoff URL generation — `packages/services/grocery-service/src/adapters/walmart.adapter.ts`
    - Depends on: T-013
    - Implements: FR-031
    - Acceptance: Returns valid checkoutUrl for cart of mapped items; unmapped items excluded from cart.

- [ ] **T-017** [P2] [US-007] Instacart adapter: order creation + checkout handoff — `packages/services/grocery-service/src/adapters/instacart.adapter.ts`
    - Depends on: T-016
    - Implements: FR-031
...
- [ ] **T-018** [P2] [US-007] StoreMappingRegistry: adapter selection (walmart | instacart | null) — `packages/services/grocery-service/src/adapters/registry.ts`
...
- [ ] **T-019** [P2] [US-007] Order status polling service (polls store API, updates list status: pending | ready | unavailable) — `packages/services/grocery-service/src/order-status.service.ts`
    - Depends on: T-018
    - Implements: FR-031
    - Acceptance: Polling runs on interval; status transitions recorded; unavailable shown on store API failure.

- [ ] **T-020** [P2] [US-007] PremiumOrderingGuard: gate order routes behind 010 subscription check — `packages/services/grocery-service/src/common/guards/premium-ordering.guard.ts`
...
- [ ] **T-021** [P1] [all] GroceryListsModule wiring with imports (Drizzle, Pantry, Aggregator, ProductMapping, Adapters) — `packages/services/grocery-service/src/grocery-lists/grocery-lists.module.ts`
...
- [ ] **T-022** [P1] [all] GroceryListsController: register all 9 endpoints with AuthMiddleware and PremiumOrderingGuard — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-021
    - Implements: FR-028, FR-029, FR-030, FR-031
    - Acceptance: All routes registered; Swagger docs generated; guards applied to correct routes.

- [ ] **T-023** [P1] [all] GroceryListsService: thin orchestration layer (aggregator + pantry + mapping + Drizzle CRUD) — `packages/services/grocery-service/src/grocery-lists/grocery-lists.service.ts`
...
- [ ] **T-024** [P1] [all] DTOs: CreateGroceryListDto, UpdateGroceryListItemsDto, GroceryListResponseDto, GroceryListItemDto, OrderResponseDto — `packages/services/grocery-service/src/grocery-lists/dto/`
    - Depends on: T-002
    - Implements: FR-028, FR-029, FR-030, FR-031
...
- [ ] **T-030** [P1] [all] Unit tests: IngredientAggregatorService — `packages/services/grocery-service/src/__tests__/ingredient-aggregator.service.test.ts`
    - Depends on: T-004
    - Acceptance: Coverage for deduplication, unit normalization, empty input, unknown density fallback.

- [ ] **T-031** [P1] [all] Unit tests: PantryService — `packages/services/grocery-service/src/__tests__/pantry.service.test.ts`
    - Depends on: T-005
    - Acceptance: Coverage for add/remove/get/subtractFromList/pruneExpired; expired items pruned correctly.

- [ ] **T-032** [P1] [all] Unit tests: ProductMappingService + Walmart adapter — `packages/services/grocery-service/src/adapters/__tests__/walmart.adapter.test.ts`
    - Depends on: T-014
    - Acceptance: Coverage for search, cart creation, checkout; circuit breaker behavior verified.

- [ ] **T-033** [P1] [all] Unit tests: GroceryListsController — `packages/services/grocery-service/src/grocery-lists/__tests__/grocery-lists.controller.test.ts`
    - Depends on: T-022
    - Acceptance: All 9 endpoints tested; auth guards mocked; 404/403 cases covered.

- [ ] **T-034** [P1] [all] Unit tests: GroceryListsService — `packages/services/grocery-service/src/grocery-lists/__tests__/grocery-lists.service.test.ts`
    - Depends on: T-023
...
- [ ] **T-010** [P2] [US-001] DELETE /api/v1/grocery-lists/:id — delete list — `packages/services/grocery-service/src/grocery-lists/grocery-lists.controller.ts`
    - Depends on: T-002
    - Implements: FR-028
...
| ----- | -------- | ---------- | ---------- | ------------------------------------------------------------ | --------------------------------- |
| T-001 | P1       | US-001     | FR-028     | `packages/services/grocery-service/src/database/migrations/` | 006 migration                     |
| T-002 | P1       | US-001     | FR-028     | `packages/services/grocery-service/src/database/schema/`     | T-001                             |
| T-003 | P1       | US-001     | FR-028     | `packages/shared/`                                           | T-002                             |
| T-004 | P1       | US-001/002 | FR-028     | `packages/services/grocery-service/src/`                     | T-003                             |
| T-005 | P1       | US-001/003 | FR-028/029 | `packages/services/grocery-service/src/`                     | T-002                             |
| T-006 | P1       | US-001     | FR-028     | `packages/services/grocery-service/src/grocery-lists/`       | T-004, T-005                      |
| T-007 | P1       | US-001     | FR-028     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-008 | P1       | US-001     | FR-028     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-009 | P1       | US-003     | FR-029     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-010 | P2       | US-001     | FR-028     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-011 | P1       | US-003     | FR-029     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-012 | P1       | US-003     | FR-029     | `packages/services/grocery-service/src/grocery-lists/`       | T-002                             |
| T-013 | P2       | US-005     | FR-030     | `packages/services/grocery-service/src/adapters/`            | T-004                             |
| T-014 | P2       | US-005     | FR-030     | `packages/services/grocery-service/src/`                     | T-013                             |
| T-015 | P2       | US-007     | FR-031     | `packages/services/grocery-service/src/adapters/`            | T-013                             |
| T-016 | P2       | US-005     | FR-030     | `packages/services/grocery-service/src/adapters/`            | T-014                             |
| T-017 | P2       | US-007     | FR-031     | `packages/services/grocery-service/src/adapters/`            | T-016                             |
| T-018 | P2       | US-007     | FR-030/031 | `packages/services/grocery-service/src/adapters/`            | T-015, T-017                      |
| T-019 | P2       | US-007     | FR-031     | `packages/services/grocery-service/src/`                     | T-018                             |
| T-020 | P2       | US-007     | FR-031     | `packages/services/grocery-service/src/common/guards/`       | 010-subscriptions                 |
| T-021 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/`       | T-002, T-004, T-005, T-014        |
| T-022 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/`       | T-021                             |
| T-023 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/`       | T-004, T-005, T-014               |
| T-024 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/dto/`   | T-002                             |
...
| T-030 | P1       | all        | all        | `packages/services/grocery-service/src/`                     | T-004                             |
| T-031 | P1       | all        | all        | `packages/services/grocery-service/src/`                     | T-005                             |
| T-032 | P1       | all        | all        | `packages/services/grocery-service/src/adapters/`            | T-014                             |
| T-033 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/`       | T-022                             |
| T-034 | P1       | all        | all        | `packages/services/grocery-service/src/grocery-lists/`       | T-023                             |
