...
Cooking Mode is decomposed into nine system components: a Step Display subsystem responsible for rendering recipe instructions one step at a time, a Step Navigation module managing forward/backward traversal, a Timer Engine service handling countdown logic and audio alerts, a Screen Wake Lock utility preventing device sleep, an Offline Cache module ensuring recipe data availability without connectivity, a Recipe Data Adapter, an Auth Guard, and a Session Extras module owning ingredient checkoff and yield scaling (added 2026-08-07). Cross-cutting concerns (type safety, accessibility, documentation) are addressed by a Quality & Accessibility component. The system is a read-only consumer of Recipe data from feature 001 and requires an authenticated session from feature 002.
...
| SYS-003 | Timer Engine            | Reads the structured `RecipeStep.timerSeconds` from recipe data (no free-text duration parsing), manages countdown state (start/pause/reset), emits audible alerts on completion, and exposes timer status to the display layer.                                                                                     | REQ-004, REQ-005, REQ-006                                     | Service   |
| SYS-004 | Screen Wake Lock        | Acquires the platform wake lock when Cooking Mode is entered and releases it on exit via a disposer. Handles platform API differences (Web Wake Lock API / Expo `activateKeepAwakeAsync`).                                                                                                                           | REQ-007, REQ-CN-002                                           | Utility   |
...
| SYS-009 | Session Extras          | Owns the two session-scoped, display-only augmentations added 2026-08-07: per-ingredient checkoff state (FR-032a) and the yield scale factor with its quantity recalculation and not-scaled advisory (FR-034a). Read-only with respect to the Recipe entity, and explicitly forbidden from touching timer durations. | REQ-012, REQ-013, REQ-014, REQ-015                            | Module    |
...
| --------- | -------------- | ---------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------ |
| SYS-007   | External       | Clerk session token    | Derived — supports cross-cutting implementation constraints for traced parent system behavior | `{ authenticated: boolean, userId: string }` (Derived)  | Redirect to login on invalid/expired token                   |
