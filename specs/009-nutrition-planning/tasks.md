...
- [ ] **T-016** [P3] [US-001] Author 009's wire shapes as zod in the recipe service and regenerate the existing schema package — `packages/services/recipe-service/src/nutrition-plans/*.schema.ts` → `packages/schemas/recipe`  
       ...
       **Tests**: unit (each schema accepts a valid fixture and rejects every malformed variant) **AND** integration (regenerate-and-diff runs clean; `src/__tests__/buildInputs.test.ts` still covers the new files)

- [ ] **T-017** [P3] [US-001] Add the storage-floor boundary-parity test for 009's bounded columns — `packages/services/recipe-service/src/nutrition-plans/__tests__/storageCapacity.test.ts`  
