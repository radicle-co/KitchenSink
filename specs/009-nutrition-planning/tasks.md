 ...
 **Tests**: unit (each schema accepts a valid fixture and rejects every malformed variant) **AND** integration (regenerate-and-diff runs clean; `src/**tests**/buildInputs.test.ts` still covers the new files)
