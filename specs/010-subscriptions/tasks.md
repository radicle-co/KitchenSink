...
- [ ] **T-003** [P0] [US-001] Create the `stripe_webhook_events` idempotency table (name collision RESOLVED — ADR-0018)  
       ...
       **✅ The collision with the SHIPPED `webhook_events` table is RESOLVED (2026-08-12) — [ADR-0018](../../docs/architecture/decisions/0018-per-sender-webhook-dedup-tables.md).** `plan.md` §2 used to declare `webhook_events`, which already ships at `packages/shared/identity-db/src/schema/webhookEvents.ts` keyed `svix_id text PRIMARY KEY` with `identity_id text NOT NULL` and no `stripe_event_id`, `status`, `error` or `processed_at` — **two different tables under one name**, in one database, written by one deployable. The ruling is **one dedup table per sender**: Stripe gets `stripe_webhook_events`, the shipped table is untouched, and its `identity_id text NOT NULL` survives (which is what keeps GR-019 schema-enforced for the Clerk path). `plan.md` §2 now carries the renamed DDL, so the plan and this task agree. ⛔ Do **not** "simplify" this by adding a `source` discriminator to the shipped table: that requires dropping its PK and relaxing that NOT NULL on a live table on the user-provisioning path, and it lets a billing-side dedup or retention defect evict Clerk's rows. ADR-0018 records the flip condition.  
...
- [ ] **T-014** [P0] [US-003] Implement the Stripe webhook handler in `identity-webhooks`, with ONE rejection path whose status comes from a complete `reason → status` lookup (`shape → 200`, `signature → 401`)  
       ...
       **⚠️ Three corrections to what this task used to say, all of which would have shipped a defect.**  
       **(1) Wrong deployable.** It named `packages/services/identity/src/billing/webhook/webhook.controller.ts` — the **ECS API**. [ADR-0017](../../docs/architecture/decisions/0017-service-ownership-for-features-006-007-009-010.md) and `plan.md` L145-160 put the Stripe webhook in **`@kitchensink/identity-webhooks`**, for four reasons that package already satisfies for Clerk's svix callback: it is an **unauthenticated third-party callback** that must not sit behind Clerk's `AuthMiddleware` (which protects every identity-service route but `/health`); it needs the **raw body**, so hosting it on the API forces `rawBody: true` globally for one route; it must **answer while the API is scaled down** (non-prod runs `FARGATE_SPOT` + a nightly shutdown, and Stripe retries for 72h against whatever is or is not up); and its idempotency rows belong in **that** database (in its own `stripe_webhook_events` table — ADR-0018 — not the shipped svix one).  
       **(2) ⛔ "Invalid signatures return `400` for a SHAPE failure" is the inversion [GR-018 §18-c](../governance-rules.md#gr-018-one-rejection-path-and-invalid-input-is-never-retried) forbids** — and it is dangerous precisely because it **looks more correct than the rule**. **Stripe retries on ANY non-2xx for 72 hours.** A `400` for a body that can never parse therefore _requests_ a three-day retry storm of a payload that **can never succeed**, converting a producer's bug into sustained load and burying the signal that would have found it. It also split **signature** failure from **shape** failure into two **code paths**, violating §18-a.  
       **(3) ⛔ CORRECTED 2026-08-12 — correction (2) was itself overshot into a NEW defect, and this is the one to read carefully.** The revision that fixed the `400` replaced it with _"An invalid payload — **either cause** — is answered **`2xx`**"_, and required the test to assert only that an invalid body yields `2xx`. **That inverts the second row of §18-a's table and contradicts the SHIPPED code.** `packages/services/identity-webhooks/src/common/handlerPipeline.ts` defines `WEBHOOK_REJECTION_STATUS` as a **complete `Record`** mapping **`shape → 200`** and **`signature → 401`**, with the reasoning written at the site: a shape failure behind a **valid** signature parses identically forever, so retrying replays a permanent failure — but a signature failure may be **OUR stale signing secret**, which is **transient and operator-fixable**, and the sender's retry window is the mechanism that rescues it. Answering `2xx` there says "delivered" and **discards every real event during a rotation, behind a green check**; on a public unauthenticated endpoint it also **tells a forger their forgery was accepted**. §18-c names this the _more expensive_ of the two inversions, and the incident is on record — a dropped `user.created` left Clerk holding a user the database did not have. **"One rejection path" means one PATH and one shape, NOT one status**; the status comes from the single complete lookup, so adding a reason cannot compile until its retry disposition is decided.  
       **Acceptance**: `POST /api/v1/billing/webhook` registered on the webhooks deployable; **no JWT auth**. Signature is verified **first**, the body is parsed against the T-029 Stripe boundary zod **second** — ⚠️ a signature proves **origin, not shape**, so both controls are required, in that order, never one instead of the other. A **signature failure and a shape failure are EQUALLY invalid**: they take **ONE** rejection path producing **ONE** structured shape differing **only** in a **`reason`** field, and the status is derived from that `reason` by **one complete `Record`** — modelled on `WEBHOOK_REJECTION_STATUS` in `handlerPipeline.ts`, never a second `if`. **Three dispositions, per GR-018 §18-a's table and §18-c:**  
       &nbsp;&nbsp;&nbsp;&nbsp;**(a)** a **shape** failure behind a **valid** signature ⇒ **`2xx` (200)** — no redelivery can parse it, so acknowledging takes it off Stripe's 72-hour schedule;  
       &nbsp;&nbsp;&nbsp;&nbsp;**(b)** a **signature** failure ⇒ **non-2xx (`401`)** — it is an **authentication** failure, and if the cause is our own stale secret the retry window is the recovery;  
       &nbsp;&nbsp;&nbsp;&nbsp;**(c)** a **valid** body ⇒ its normal success.  
       Every rejection — **either** reason — is recorded in **(1)** the response body (so Stripe's dashboard shows what was wrong), **(2)** structured logs carrying the `reason`, **(3)** a **per-`reason` counter**, and **(4)** an **alarm** on that counter (AC-018-f), because a rejection nobody sees is indistinguishable from success. **Reject the content, accept the delivery — but never accept an unauthenticated delivery.**  
       **⚠️ The alarm half is a SHIPPED pattern to copy, not new ground** (re-measured 2026-08-12, correcting an earlier claim that `IdentityWebhookRejected` had no alarm): `WebhookShapeRejectionAlarm` and `WebhookSignatureRejectionAlarm` both exist in `packages/services/identity-webhooks/infra/lib/WebhooksStack.ts`, dimensioned **per `reason`** exactly as AC-018-f requires. Stripe's counter gets the same treatment — two alarms, one per reason, not one dimensionless alarm.  
       **⛔ The rejected event is NOT recorded as a row** (GR-018 §18-d + GR-019). An invalid payload has **no trustworthy identifier** — a shape that failed to parse has no `stripe_event_id` worth keying a dedup row on — so "just record it" forces the writer to invent `'unknown'`, the sentinel GR-019 forbids outright. ⚠️ The Clerk half of this database makes the same point structurally: the shipped `webhook_events.identity_id` is `text NOT NULL` (`packages/shared/identity-db/src/schema/webhookEvents.ts`), so a rejected svix payload cannot be recorded there either. A sentinel here fuses every unattributable billing event into one fictitious account, cannot be told apart from a real id afterwards, and would put an **authorization** decision in the hands of a string literal. **The log line and the counter are load-bearing, not a consolation prize.**  
       ...
       **Tests**: unit (a bad signature and a malformed body produce the **same** shape differing only in `reason`; the status for each comes from the complete `Record` and **differs** — `200` vs `401`; no row is written for either) **AND** integration — **ALL THREE dispositions, per AC-018-c**, because ⚠️ **any two of them pass on a handler that always returns the same thing** (⛔ the earlier version of this line asked for only "invalid ⇒ `2xx`" plus "valid ⇒ success", which is exactly a two-of-three test and would have **passed a handler that `2xx`-ed a forged signature**): **(a)** a **shape** failure behind a **valid** signature yields **`2xx`** + the rejection in the body + a `reason` in the log + the shape counter incremented + **no** `stripe_webhook_events` row; **(b)** a **signature** failure yields **non-2xx (`401`)** with the **same shape** and the signature counter incremented + no row; **(c)** a **valid** body still yields its normal success.
...
- [ ] **T-015** [P0] [US-003] Implement Stripe idempotency in the webhooks deployable  
       ...
       **✅ The table-name collision is RESOLVED — this writes `stripe_webhook_events`, keyed `stripe_event_id`.** See T-003 and [ADR-0018](../../docs/architecture/decisions/0018-per-sender-webhook-dedup-tables.md): the shipped `webhook_events` (svix, `svix_id text PRIMARY KEY`, `identity_id text NOT NULL`) is a **different table** and is not written by this code path at all.  
       **⚠️ `stripe_webhook_events` has NO `identity_id` column, deliberately.** A Stripe event arrives attributed to a `stripe_customer_id`, not to an app identity, and a `NOT NULL` identity column would force the writer to invent one for an event whose customer we never recorded — the sentinel GR-019 forbids. Attribution is a lookup against `accounts.stripe_customer_id`; an unresolvable account is a **rejection** (T-014), never `'unknown'`, `''` or `0`, including as a metrics dimension. The dedup row itself does not depend on that lookup succeeding, which is precisely why it can be keyed on Stripe's own id.  
...
- [ ] **T-020** [P1] [US-003] Integration tests for all webhook handlers  
       ...
       **Acceptance**: All state transitions verified (free→premium, active→past_due, active→canceled); idempotency confirmed under concurrent duplicate delivery; data retention on cancellation confirmed. **Plus all THREE GR-018 dispositions** — ⚠️ **corrected 2026-08-12; this line previously asked for "both halves" (invalid ⇒ `2xx`, valid ⇒ success), and AC-018-c requires three because any TWO of them pass on a handler that always returns the same thing**: a **shape** failure behind a valid signature yields **`2xx`** + a recorded rejection + the shape counter + **no** row; a **signature** failure yields **non-2xx (`401`)** with the same shape + the signature counter + no row; a **valid** body yields its normal success. The two-of-three version would have passed a handler that `2xx`-ed a **forged** signature.  
...
- [ ] **T-029** [P0] [US-002] Author billing's wire shapes as zod in the identity service and extend the existing schema package  
       ...
       **Acceptance**: Checkout, portal and subscription-status request/response shapes — plus the **webhook endpoint's own response** shape — authored as zod **beside `BillingController`** under `src/billing/`, importing **only `zod` and other `*.schema.ts` files**. `npm run contract:verify` regenerates `packages/schemas/identity` (`schemas.ts`, `types.ts`, `contractHash.ts`, barrel, **derived** `openapi.yaml`) with no diff. **Add to the existing `@kitchensink/schema-identity` — never fork it**, and never hand-edit the generated package. Schemas live at `src/billing/*.schema.ts` beside the controller, **never in a `dto/` directory** (§15.2).  
       ...
       **Tests**: unit (each schema accepts a valid fixture and rejects every malformed variant; the enum rejects an unknown status) **AND** integration (regenerate-and-diff runs clean; `packages/services/identity/src/__tests__/buildInputs.test.ts` covers the new files; the `CONTRACT_HASH` boot assertion still holds).
...
- [ ] **T-030** [P0] [US-002] Register billing routes under **`nestjs-zod`'s** `ZodValidationPipe`, with `z.strictObject()` on mutating bodies  
       ...
       **⚠️ Billing lands inside the ONE service where this has already gone wrong.** Identity is where a `createZodDto` DTO was served by Nest's **own** `ValidationPipe` and therefore **validated nothing while looking correctly wired** — `PATCH /users/me`, a route that writes user data. Billing routes must not inherit that wiring. Identity registers `nestjs-zod`'s pipe in `app.module.ts` today — ⚠️ **re-measured 2026-08-12: 11 `ZodValidationPipe` references and 10 classes extending `createZodDto` across 6 files**, correcting the "**6** / **6** … up from 3/4" this line carried. That figure has now been wrong twice in two days, so **count it rather than quoting it**; the durable statement is repo-wide gate **G5** in `packages/infra/global/__tests__/serviceSecurityInvariants.test.ts`, which requires the pipe over every controller in every deployable and runs with no exception list.  
...
- [ ] **T-032** [P0] [US-003] ⛔ Boundary-validate Stripe's inbound shapes — and NEVER converge them  
       ...
       **Acceptance**: `checkout.session.completed`, `invoice.paid`, `invoice.payment_failed`, `customer.subscription.updated`, `customer.subscription.deleted` and `customer.subscription.trial_will_end` each have a zod schema validating the **raw upstream shape at the boundary**, applied **after** signature verification and **before any field drives a `subscriptions` or `stripe_webhook_events` write**. This adapter **MAY declare its own types**; Stripe's types stay on **Stripe's side of the adapter**; **NO OpenAPI document is written for Stripe**; and rules 17-b.1–17-b.5 do not apply to it. Only unknown/unregistered event types are ignored — and ignored **loudly**, with a counter, never silently.  
...
| T-003 | Create `stripe_webhook_events` table (ADR-0018)            | US-001 | T-001               |
