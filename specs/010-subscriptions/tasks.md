...
- [ ] **T-014** [P0] [US-003] Implement the Stripe webhook handler in `identity-webhooks`, with ONE rejection path whose status comes from a complete `reason → status` lookup (`shape → 200`, `signature → 401`)  
       ...
       **⚠️ The alarm half is a SHIPPED pattern to copy, not new ground** (re-measured 2026-08-12, correcting an earlier claim that `IdentityWebhookRejected` had no alarm): `WebhookShapeRejectionAlarm` and `WebhookSignatureRejectionAlarm` both exist in `packages/services/identity-webhooks/infra/lib/webhooks-stack.ts`, dimensioned **per `reason`** exactly as AC-018-f requires. Stripe's counter gets the same treatment — two alarms, one per reason, not one dimensionless alarm.  
