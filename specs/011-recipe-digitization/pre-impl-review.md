...
| `packages/apps/commise/ui` shared components      | Web correction screen, audience picker, Circle UI  |   ⚠️ Implicit; not explicitly enumerated in tasks. **D-004**   |
...
| D-004 | WARNING  | Reuse of `packages/apps/commise/ui` shared components is implicit, not enumerated. Risk of duplicating components inside the digitization feature.      | In T057–T067, require each task author to first check `packages/apps/commise/ui` for existing form, button, and modal primitives. Document any new primitives introduced and propose promotion to `packages/apps/commise/ui`. |
...
| Dependency direction correct (no circular deps)         |   ✅   | `@kitchensink/audience` is a leaf consumed by 001/006/007/011 APIs; OCR Lambda writes to DB via internal client, not reverse-imported by API. |
...
| `@kitchensink/audience` consumed by 001/006/007                 | ✅ Covered (T019–T025); contract published before consumers depend on it |     M      |
...
| ID    | Severity | Finding                                                                                                                                                                                                                                      | Recommendation                                                                                                                                                                                                                                |
| ----- | :------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A-001 | WARNING  | Workspace globs (`packages/services/*`, `packages/shared/*`) and TS project references for the four new packages are deferred to T001 / T006. If any earlier task is mistakenly merged before T001, the monorepo state will be inconsistent. | Enforce strict task ordering: T001 must land in its own commit/PR before any T002–T005 scaffold PR. Add a CI guard that fails if a `packages/services/*` package is added without a matching workspace entry.                                 |
...
| R-003 | Technical   | Schema regret on `recipes.audience` JSONB contract once consumed by 001/006/007 — breaking change is expensive across four packages. |     M      |   H    |   **High**   | Publish `@kitchensink/audience` v1 contract behind explicit version export; consumer features pin via TS path alias; document migration path in `shared-audience/README.md` before T021 lands.                                            |
...
2. **R-003** — publish `@kitchensink/audience` v1 contract surface in T019/T021 with an explicit version export and a `README.md` migration note before any consumer (001/006/007) imports it.
...
- [x] **C-D-004** Resolved by **T097** (PR-template note + `packages/apps/commise/ui` INDEX.md requirement on T057–T067). _(Mitigates D-004.)_
