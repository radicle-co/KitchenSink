...
| [003-usda-food-data](../003-usda-food-data/spec.md)         | **Downstream (launch consumer)** — a food resolution reaches the requesting user through this service. ⚠️ **Cited requirements do not exist and the two specs contradict each other** (found 2026-08-10): there is no `US-005` / `FR-NOTIF` in 003, and no `food.backfill.completed` / `food.fetch.failed` — 003's real event is `FoodFetchCompleted` on EventBridge. 003 instead specs its OWN API Gateway WebSocket (US-9, T-185, T-186, and the WebSocket halves of FR-041/049/050), which contradicts GR-011's rule that this feature owns transport. Resolving that is an amendment to 003, pending. Until it lands, **SC-001 is unachievable as written**. |
