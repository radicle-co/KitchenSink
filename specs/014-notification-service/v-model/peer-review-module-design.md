...
| PRF-MOD-007 | Major       | **`invalid_input` was emitted by MOD-068 but absent from MOD-071's closed dead-letter vocabulary** — so a malformed event-path envelope (mistyped `schemaVersion`, oversize `payload`, a by-kind `id` violation) would have been rejected by the validator and then refused by the dead-letter entry point, producing exactly the silent drop FR-028 exists to prevent. Fixed on 2026-08-10: `invalid_input` is now a member of `REASON_CODES`, and MOD-071's own internal error for an out-of-vocabulary code is renamed `unknown_reason_code`, because one identifier must not mean both "this envelope is malformed" and "this reason code does not exist". |
