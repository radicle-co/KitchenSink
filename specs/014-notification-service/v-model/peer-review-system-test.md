...
| PRF-SYSTEST-006 | Major       | **Three of the original cases are now actively misleading, not merely empty.** STP-002 frames producer auth as the undecided "mechanism aligned with feature 002" that REQ-040 has resolved. STP-008 asserts no ordering behaviour at all, so it cannot detect the event-path FIFO defect REQ-037 describes. STP-018 treats `idempotencyKey` as optional with no derivation constraint, which REQ-034 and REQ-038 have narrowed. STP-037, STP-038 and STP-040 cover those cases correctly, but the stale entries remain in the same document and a reader cannot tell which pair is current. |
