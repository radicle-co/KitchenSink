...
- ✅ **Both gates exist and are wired**: `packages/infra/global/__tests__/spec-task-ids.test.ts` and
  `packages/infra/global/__tests__/spec-table-collisions.test.ts`, over the shared parser
  `packages/infra/global/__tests__/spec-declarations.ts`. They run in that workspace's default `test` task, so CI
