...
**Version**: 3.5.0
**Ratified**: 2026-05-10 | **Last amended**: 2026-08-22
**Authority**: Senior Product Owner, cross-feature governance
**Scope**: All features 001–017 and any future feature in this portfolio
...
21. [One Declarer Per Table Name, and One Definition Per Task ID (GR-021)](#gr-021-one-declarer-per-table-name-and-one-definition-per-task-id)
22. [Governance Amendment Process](#governance-amendment-process)
...
- Feature 002: `contracts/identityApi.openapi.json` server base path is `/api/v1` — ✅.
...
**Amended 2026-08-22 (v3.5.0).** Article 50 became **applicable on 2 August 2026** — it is live, not
forthcoming. It carries **two** obligations where this rule originally captured one:

- **Art. 50(1) — interaction disclosure.** A person interacting directly with an AI system is informed, unless
  it is obvious from the context.
- **Art. 50(2) — synthetic-content marking.** Content generated or materially altered by AI is marked in a
  **machine-readable** format and detectable as AI-generated. This is a _generation-time_ obligation:
  a marking cannot be retrofitted onto content that was already produced without it, which is why it must be
  built before any AI feature ships rather than added at the display layer later.

The legal-surface requirements are specified in
[`016-legal-compliance-framework`](./016-legal-compliance-framework/spec.md) `016-FR-045`–`016-FR-047`;
Feature 005 remains the owner of the shared disclosure component.
...
- **AC-010-e**: Content generated or materially altered by AI carries a **machine-readable marking at rest**
  and a **human-legible label** where displayed. The label reflects the content's **current** state after any
  human editing, and is never conveyed by colour alone (`001-NFR-004`).
- **AC-010-f**: A feature that consumes AI cites both this rule and `016-FR-045`–`016-FR-047`; it does not
  restate the obligation or build a second disclosure component.
...
- ⚠️ **Article 50 is now LIVE** (applicable 2 August 2026). AC-010-d's deadline has passed; the rule is no
  longer a forward-looking one.
- A transitional allowance under the AI Omnibus provisional agreement (May 2026) gives generative systems
  **already on the market** before 2 August 2026 until **2 December 2026** for the Art. 50(2) machine-readable
  marking. **We do not rely on it** — we are not yet on the market, so we owe the marking at launch.
- Penalties for non-compliance reach **€15 million or 3% of worldwide annual turnover**.
- Feature 005: EU AI Act compliance defined in `spec.md:18` — predates the 50(1)/50(2) split and covers the
  human-facing disclosure only. **`AC-010-e` is not yet reflected in 005.**
- Features 006, 009: still no EU AI Act mention. **Correction required before implementation** (unchanged
  since 2026-05-10).
- Feature 016 records the obligation, the applicability date and the transitional allowance.
...
**Amended 2026-08-02** (v3.0.0) and **2026-08-22** (v3.5.0, AC-014-g/h/i) — see [Change Log](#change-log).
...
- **AC-014-e**: **Ingestion provenance sets the initial scope, and there is exactly ONE implementation of
  ...
  `private`, and the system MUST NOT auto-publish it.
    ...
    (`packages/services/recipe-service/src/recipes/domain/visibilityPolicy.ts`), keyed on the shipped
...
- **AC-014-g**: **A clone MUST be modified before it may be published.** A copy of another user's recipe
  carries a **substantive edit** — a modification to ingredients or instructions; title, description, tags and
  photos do not qualify — before it may hold the `public` scope. Until then it is not publishable, whatever
  scope it would otherwise default to. The substantive edit gates **publication**, not privacy: the premium
  gate previously placed on making a clone private is **removed**, consistent with v3.0.0's withdrawal of the
  premium-recipe model.

    ⚠️ Requiring a modification MUST NOT be presented, internally or externally, as a **copyright defence**. A
    modified copy is a **derivative work**, and a derivative of a protected work still infringes. What makes
    cloning defensible is AC-014-e's provenance rule and the user content licence (`016-FR-010`); AC-014-g is
    a corpus-quality rule with a secondary evidentiary benefit and nothing more.

- **AC-014-h**: **A `private` recipe is copyable only by its owner.** No user may clone another user's private
  recipe; the `circle` scope grants **read** access, never a copy. An owner's copy of their own private recipe
  is created `private`, and publishing it later runs AC-014-e.

- **AC-014-i**: **A clone survives the original's deletion; a read-only share does not.** Deleting a recipe
  tombstones it and removes it from every surface **including `circle` shares** (`001-FR-002`), so a person who
  could see it only through a share loses access — while a clone in another user's collection is that user's
  own modified work and persists. Account **erasure** MUST strip the erased user's identifying attribution from
  surviving clones while retaining a **non-identifying** provenance marker: the recipe text is generally not
  personal data, the attribution is.
...
Publishing an unmodified clone, cloning another user's private recipe, destroying a clone because the original
was deleted, or leaving an erased user's name on a surviving clone are each violations of AC-014-g/h/i — as is
citing the modification requirement as a copyright defence.

### Current State (2026-08-22) — AC-014-g/h/i added

- **AC-014-g/h/i come from `016` C-016-003** (owner ruling, 2026-08-22). `001-FR-005` was rewritten into
  `001-FR-005` plus `001-FR-005a`–`001-FR-005d`, and the C-004 matrix row for `imported_public` in
  `001/data-model.md` was updated to match.
- ⛔ **The code still implements the pre-amendment rule.** `evaluateVisibility` and `defaultCloneVisibility`
  (`packages/services/recipe-service/src/recipes/domain/visibilityPolicy.ts`) are unchanged; both deltas are
  written into `001/data-model.md` and are implementation work requiring failing tests first.
- ⚠️ **Two open items block implementation of AC-014-g.** (1) `001-FR-005b` is unrepresentable for a free-tier
  user while `001-FR-003` says free-tier recipes are _always public_ — it needs D4a (free-tier privacy, per
  `015` C-015-001) or a distinct **unpublished-draft** state that is not `visibility = private`. (2)
  `001-FR-003a`'s PRO-badge derivation infers "uses a premium capability" from _private AND
  `user_created`/`imported_public`_; once a clone can be private without premium that inference is wrong, and
  it must be fixed in the **same** change.
- **AC-014-e already covered provenance-gated visibility** — a licence forbidding redistribution **or
  derivatives** is named in it. `001-FR-005a` extends the same rule from _initial scope_ to _cloneability_; it
  is not a new concept, and the residual gap is narrow: a source that is public and freely available but whose
...
3. A generated, **committed** package `packages/schemas/<service>` (`@kitchensink/schema-<service>`) exports
   the zod (`schemas.ts`), the `z.infer` types (`types.ts`), a `contractHash.ts`, a barrel (`index.ts`), and
...
- ✅ **Three schema packages exist**, each with `schemas.ts`, `types.ts`, `contractHash.ts`, a barrel **and a
...
search DAL, and the `erasureSweeper.ts` / `erasureOrphanSweeper.ts` workers), all three taking a config value
...
`packages/infra/global/__tests__/serviceSecurityInvariants.test.ts`. That matters more than the count: the
...
- **Zero `class-validator` / `class-transformer` importers anywhere in `packages/services/**`.** The last one
  was `recipe-service/src/search/dto/searchRecipes.query.dto.ts`, converged onto `createZodDto`over the
...
- **Every HTTP controller in every discovered deployable is covered by `ZodValidationPipe`, with no exception
  list.** `packages/infra/global/__tests__/serviceSecurityInvariants.test.ts` gate **G5** discovers services
...
- ✅ **16-d's five int-backed recipe fields are bounded** by `positiveInt4()`
  (`packages/shared/recipe-core/src/recipeRequestBounds.ts`), and `storageCapacity.test.ts` asserts the floor
...
- ✅ **OPEN-GR-016-A — the storage floor is enforced by a per-service parity TEST**, not a review-checklist
  ...
  already BUILT**: `@kitchensink/contract-gen`'s `auditStorageCapacity` is wired by a `storageCapacity.test.ts`
...
8. Wires the **storage-capacity audit** (`@kitchensink/contract-gen`'s `auditStorageCapacity`) in its own
   `storageCapacity.test.ts`, copying the pattern the three existing services already use — see 17-d. This is
...
`formatStorageCapacityFindings` (`packages/tools/contract-gen/src/storageCapacity.ts`) are wired by a
`storageCapacity.test.ts` in **all three** services:
...
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Schema package shape, scripts, lint/format posture (17-a.3) | `packages/infra/global/__tests__/generatedSchemaPackages.test.ts`                                                                                                                                                  | ✅ reads `packages/schemas/*`                                                                                      | **exists**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Regenerate-and-diff drift gate (GR-015 §15-c layer 2)       | `scripts/contractDriftGate.mjs` + `packages/infra/global/__tests__/contractDriftGate.test.ts`                                                                                                                      | ✅                                                                                                                 | **exists**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Every contract owner is actually regenerated (17-a.2)       | `scripts/contractOwners.mjs` `discoverContractOwners` + `contractGenerationRunner.test.ts`                                                                                                                         | ✅ reads `packages/services/*/package.json`                                                                        | **exists**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| No app depends on a service package                         | `packages/infra/global/__tests__/appServiceDependency.test.ts`                                                                                                                                                     | ✅ discovers services + `git ls-files`                                                                             | **exists**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Turbo `inputs` wiring for the copy (GR-015 §15-c layer 1)   | `packages/infra/global/__tests__/turboBuildGraph.test.ts`, `packages/services/*/src/__tests__/buildInputs.test.ts`                                                                                                 | ✅ / per-service                                                                                                   | **exists**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `CONTRACT_HASH` boot assertion (17-a.4)                     | `packages/services/*/src/__tests__/mainBootOrder.test.ts`; consumer side `packages/clients/{food-service,recipe-service}/src/contractSkew.ts` **and** `packages/apps/commise/features/account/src/contractSkew.ts` | per-service / per-consumer                                                                                         | **partial** — the SERVICE half is present for all three. The CONSUMER half now covers all three too: identity's was the gap (there is no `packages/clients/identity-service`; web, mobile and `features/account` import `@kitchensink/schema-identity` directly) and it lives in `@commise/features-account`, fired from `ProfileServiceClient`'s transport funnel. ⚠️ Still **not** a repo-wide discovery gate, and mobile's `src/services/api.ts` + `useAvatarUpload.ts` reach identity WITHOUT going through that client — see 17-b.5 below |
| `nestjs-zod` pipe registered, bad body ⇒ `400` (17-a.5)     | `packages/infra/global/__tests__/serviceSecurityInvariants.test.ts` gate **G5**, plus `packages/services/identity/tests/appValidation.test.ts`                                                                     | ✅ discovers deployables, parses sources as TypeScript                                                             | ✅ **exists, repo-wide.** ⚠️ This row said "per-service only, **no repo-wide gate**" — stale. G5 requires EVERY HTTP controller in EVERY discovered deployable to be covered by a `ZodValidationPipe` (global `APP_PIPE` or `@UsePipes`), catches Nest's own `ValidationPipe` standing in over a `createZodDto` class (16-e.1) and the bare-class-token bug, and now runs with **no exception list** — its `UNCONVERGED_CONTROLLERS` ratchet was deleted when its last entry converged                                                         |
...
| Clients declare no wire types (17-b.1)                      | `packages/infra/global/__tests__/wireContractConsumers.{ts,test.ts}`                                                                                                                                               | ✅ AST-based                                                                                                       | ✅ **exists.** ⚠️ This row said "UNTRACKED … do not cite it as coverage" — stale: it was committed in `df21cbed` with 12 violation fixtures and is green. Cite it                                                                                                                                                                                                                                                                                                                                                                              |
| `z.strictObject()` on mutating bodies (17-c)                | —                                                                                                                                                                                                                  | —                                                                                                                  | ❌ **no gate yet** — greppable, so cheap to add                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Storage-floor parity (17-d)                                 | `@kitchensink/contract-gen` `auditStorageCapacity` + `storageCapacity.test.ts` in **all three** services                                                                                                           | per-service, but **exhaustive over columns** within each — a new bounded column fails the gate the day it is added | ✅ **exists, and is the strongest gate in this table.** Bidirectional bookkeeping via `stale-account` / `duplicate-account` findings                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- ✅ **Three schema packages exist and are complete in shape**: `packages/schemas/{recipe,food,identity}`, each
  with `schemas.ts`, `types.ts`, `contractHash.ts`, a barrel — **and `openapi.yaml`, which now exists for all
...
- ✅ **`@kitchensink/recipe-service` has ONE validation mechanism.** Its last `class-validator` **importer**,
  `src/search/dto/searchRecipes.query.dto.ts`, is converged onto `createZodDto` over the authored
...
- ✅ **17-d is already fully enforced** — `@kitchensink/contract-gen`'s `auditStorageCapacity` is wired by a
  `storageCapacity.test.ts` in **all three** services, exhaustive over bounded columns in both directions. The
  ruling that closed OPEN-GR-016-A describes shipped code, not a plan.
- ✅ **The gate for 17-b.1 has LANDED**: `packages/infra/global/__tests__/wireContractConsumers.{ts,test.ts}`
...
**Reference implementation:** `packages/services/identity-webhooks/src/common/handlerPipeline.ts` —
...
- ✅ **The reference implementation exists and is better than the rule's first draft.**
  `packages/services/identity-webhooks/src/common/handlerPipeline.ts` verifies the svix signature **and** then
...
- ✅ **`IdentityWebhookRejected` IS alarmed, per REASON.** ⚠️ This bullet previously read "has NO ALARM …
  `WebhooksStack.ts` alarms only on `ErasureIncomplete`", and AC-018-f was recorded as unmet. Stale: landed in
  `34d9e8d0`. `identity-webhooks/infra/lib/WebhooksStack.ts` now creates **`WebhookShapeRejectionAlarm`**
...
## GR-021: One Declarer Per Table Name, and One Definition Per Task ID

**Severity**: CRITICAL
**Ratified**: 2026-08-12
**Normative source**: [`docs/CODING_STANDARDS.md` §16](../docs/CODING_STANDARDS.md).
**Relates to**: [GR-004](#gr-004-data-model-naming-convention) (how a table is named),
[GR-007](#gr-007-shared-type-library-ownership) and [GR-015](#gr-015-api-contract-ownership) (the same
single-owner principle applied to types and wire contracts),
[ADR-0017](../docs/architecture/decisions/0017-service-ownership-for-features-006-007-009-010.md) (which service
owns which feature, hence which database),
[ADR-0018](../docs/architecture/decisions/0018-per-sender-webhook-dedup-tables.md) (the collision that produced
this rule).
**Resolves**: two defects found by accident on 2026-08-12, both mechanically detectable, neither previously
guarded.

### Why this rule exists

Two identifiers in this portfolio are joined on by other documents and by migrations, so a duplicate is not
untidiness — it is a wrong answer that looks like a right one.

1. **A table name.** Feature 010 declared a `webhook_events` table for Stripe idempotency. That name **already
   ships** in the very database ADR-0017 puts 010's webhook in (`packages/shared/identity-db/src/schema/webhookEvents.ts`
   — Clerk/svix dedup, keyed `svix_id text PRIMARY KEY`, `identity_id text NOT NULL`), with a disjoint column set.
   Two features writing one table name with different columns and different owners is a migration that **fails at
   deploy time**, or — far worse — one that succeeds and corrupts delivery dedup for **both** senders. A second
   instance was found in the same sweep: feature 011 planned a **second** `recipe_versions` table in its own
   service's database while `recipe_versions` already ships in the recipe service, forking a
   `(recipe_id, version_number)` sequence that is supposed to be single-writer — while 011's own
   pre-implementation review believed it was reusing the shipped table.
2. **A task identifier.** Feature 007 defined **eight** task IDs twice (T-004, T-025, T-027, T-028, T-041, T-043,
   T-044, T-046) — 60 checkbox lines for 52 IDs. A duplicated ID means a traceability row can be closed by the
   **wrong** task, `Depends on: T-043` has no single referent, and the feature's own task count is wrong.

### Rule

- **21-a — One declarer per table name, portfolio-wide.** A table name is declared by **exactly one** owner: one
  feature, or one shipped package. This binds across features, between a feature and shipped code, and between
  two shipped packages. Two writers of one name is a violation even when they target different logical databases,
  because the name is how humans and tools identify the table.
- **21-b — A spec DECLARES a table structurally, or it has not declared it.** A table a feature intends to create
  MUST appear as DDL inside a fenced ` ```sql ` (`CREATE TABLE …`) or ` ```ts ` (`pgTable('…', …)`) block. Prose
  naming a table is a **reference**, not a declaration — which is correct and common (007 reads 001's
  `recipe_ingredients`; every billing surface names `accounts`). This is an obligation on the author, not a
  description of a tool: a prose-only declaration is invisible to the gate, and that is exactly how 011's
  `recipe_versions` survived the first automated pass.
- **21-c — A not-yet-implemented feature MUST NOT `CREATE TABLE` a name that already ships.** Such a statement can
  only fail or clobber. If the intent is to reuse the existing table, the **owning service** writes it and the
  feature calls that service; if the intent is a new table, it needs a new name.
- **21-d — One definition per task ID within a `tasks.md`.** A definition is the line carrying the done-state: a
  checkbox item, or — in a file that uses no checkboxes at all — a heading. Two definitions of one ID is a
  violation. The same task legitimately serving two user stories is **defined once**, tagged with every story it
  serves, with the second site a **non-checkbox pointer**. Two different deliverables need two identifiers.
- **21-e — An exemption requires a written `why`.** A ruled exception to 21-a is recorded in the guard's exemption
  registry with the exact owner set and a **substantive reason**. A missing, blank or one-word reason is a hard
  failure, following the precedent of `contract-gen`'s `AllowedPackageImport.why` and `ColumnAccount.why`. An
  exemption pins the owner set **exactly**: a third declarer appearing later fails, because "two of these were
  ruled acceptable" says nothing about a third.
- **21-f — The gates MUST parse, not grep, and MUST discover rather than enumerate.** Measured reasons, all three
  from this repository: 007's fenced dependency graph makes a line-wise task-ID regex report ~30 phantom
  duplicates; 001's suffixed IDs (`T001`, `T001a`, `T001-alb`) make a `T\d+` match manufacture ~50 more; and
  shipped drizzle schemas write `pgTable(` and the table name on **different lines**, so a single-line regex finds
  3 of this repository's tables instead of all of them. A hardcoded list of features is itself the defect —
  feature 015 would arrive unguarded.

### Acceptance Criteria

- **AC-021-a**: No two owners declare the same table name, across `specs/**` and `packages/**`, except by a
  registry entry naming the exact owner set with a substantive `why`.
- **AC-021-b**: No `tasks.md` defines a task identifier twice.
- **AC-021-c**: Both gates discover their inputs from the tree (`git ls-files`), parse structurally, and are
  mutation-proven against fixtures that violate them — including a fixture whose **comments** contain the exact
  words a text gate would have matched.
- **AC-021-d**: An exemption with no `why`, a one-word `why`, or fewer than two owners FAILS.

### Violation

Declaring a table name a second time without a registry entry; defining a task ID twice; adding a registry entry
without a substantive reason; replacing either gate's parser with a text search; or converting the exemption
registry from an exact owner set to a floor.

### Current State (2026-08-12) — NEW rule, both gates BUILT and GREEN

- ✅ **Both gates exist and are wired**: `packages/infra/global/__tests__/specTaskIds.test.ts` and
  `packages/infra/global/__tests__/specTableCollisions.test.ts`, over the shared parser
  `packages/infra/global/__tests__/specDeclarations.ts`. They run in that workspace's default `test` task, so CI
  picks them up with no new step.
- ✅ **Task IDs: portfolio is CLEAN.** 14 `tasks.md` files, **950 definitions, 0 duplicates** (measured 2026-08-12) — after 007's eight
...
- ✅ **The three exemptions that recorded a DEFECT are RESOLVED.** ⚠️ This bullet previously said `users`,
  ...
  that directory now holds only `__tests__/storageCapacity.test.ts`, and that audit reads the AUTHORITATIVE
...
- ❌ Two shipped tables (`author_handles`, `lifecycle_events`) appear in **no feature spec**, so nothing reserves
  their names against a future feature. They are still covered by AC-021-a, which reads shipped code directly.
...
| ------- | ---------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.5.0   | 2026-08-22 | Repository owner                                | **GR-014 and GR-010 amended** (MINOR — both materially expanded), from feature `016-legal-compliance-framework` C-016-002/C-016-003. **GR-014** gains **AC-014-g/h/i**: a clone MUST carry a substantive edit before it may be **published** — the edit gates publication rather than privacy, and the premium gate on clone-privacy is removed, which finally aligns `001-FR-005` with v3.0.0's withdrawal of the premium-recipe model; a `private` recipe is copyable only by its owner (`circle` grants read, never a copy); and a clone survives the original's deletion while a read-only share does not, with **erasure** stripping identity from surviving clones but keeping a non-identifying provenance marker — the recipe text is generally not personal data, the attribution is. ⚠️ AC-014-g carries an explicit prohibition on citing the modification requirement as a **copyright defence**: a modified copy is a derivative work and a derivative of a protected work still infringes, so what actually defends cloning is AC-014-e plus the content licence. Recorded honestly: `001-FR-005` and the C-004 matrix are amended but **`evaluateVisibility` and `defaultCloneVisibility` are NOT** — the code deltas are specified in `001/data-model.md` as implementation work — and two items block AC-014-g (`001-FR-005b` is unrepresentable for a free-tier user while `001-FR-003` says free-tier recipes are always public, needing D4a or an unpublished-draft state; and `001-FR-003a`'s PRO-badge derivation infers premium from privacy and must be fixed in the same change). **GR-010** is corrected for the fact that **Art. 50 became applicable 2 August 2026** — it is live, AC-010-d's deadline has passed — and split into its two real obligations: 50(1) interaction disclosure (which the rule already had) and **50(2) machine-readable marking of synthetic content**, added as **AC-010-e**, which is a _generation-time_ obligation because a marking cannot be retrofitted onto content already produced. **AC-010-f** stops features restating it or building a second component. 005 predates the split and does not yet reflect AC-010-e; 006 and 009 still carry no mention at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 3.4.0   | 2026-08-12 | Repository owner                                | **GR-021 added** (MINOR — new rule): One Declarer Per Table Name, and One Definition Per Task ID. Two defects found by accident, both mechanically detectable, neither guarded. **(1)** Feature 010 declared a `webhook_events` table for Stripe idempotency — a name that **already ships** in the very database ADR-0017 puts 010's webhook in (Clerk/svix dedup, `svix_id text PRIMARY KEY`, `identity_id text NOT NULL`), with a disjoint column set: a migration that fails at deploy, or one that succeeds and corrupts dedup for **both** senders. Resolved by the new **ADR-0018** — one dedup table per sender, Stripe gets `stripe_webhook_events`, the shipped table is untouched so its `identity_id text NOT NULL` (the constraint GR-019 leans on) survives; the two rejected alternatives are a `source` discriminator on the live table (drops its PK, relaxes that NOT NULL, and lets a billing defect evict Clerk's rows) and a Postgres-schema namespace. The same sweep found a **second** collision nobody had recorded: feature 011 planned a second `recipe_versions` table in its own service's database while `recipe_versions` already ships in the recipe service — forking a single-writer `(recipe_id, version_number)` sequence — while 011's own pre-impl review believed it was reusing the shipped one; ruled NOT created, corrections go through `@kitchensink/recipe-service-client`, and 011's `recipes.audience` DDL was repointed to a recipe-service migration for the same single-writer reason. **(2)** Feature 007 defined **eight** task IDs twice (60 checkbox lines for 52 IDs), so a traceability row could be closed by the wrong task and `Depends on:` had no single referent: seven were one task cross-listed under a second story and are now defined once with non-checkbox pointers, and T-046 was two different deliverables sharing an ID — the E2E half became T-053. Both gates are BUILT, discovery-based and mutation-proven (`specTaskIds.test.ts`, `specTableCollisions.test.ts` over `specDeclarations.ts`); **21-f requires them to parse, not grep**, for three measured reasons (007's fenced dependency graph makes a line-wise task-ID regex report ~30 phantoms, 001's suffixed `T001-alb` IDs make a `T\d+` match manufacture ~50 more, and drizzle writes `pgTable(` and the table name on different lines so a single-line regex sees 3 of the repo's tables). 21-e makes an exemption's `why` **mandatory and substantive** and pins the owner set **exactly**, per the `contract-gen` precedent. Recorded honestly: three exemptions document a **shipped defect** rather than an approval (`users`/`accounts`/`profiles` are drifted duplicates in `packages/services/identity/src/types/schema/*` — `profiles.user_id` is `uuid` there and `text` in the authoritative `identity-db` — under a comment claiming lockstep), and 6 of 14 features declare tables in prose only and are therefore unseen by AC-021-a.                                                                                                                                                                                                                                                                    |
